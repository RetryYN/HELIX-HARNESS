...
| 🧠 **健忘行動** | 文脈を忘れ、引き継ぎが雑になる | DB-backed continuation + **2 層エージェントメモリ**（システム/プロジェクト） | P5·P7 |
...
| `helix status` | 実行状態と DB-backed continuation の確認 |
...
`.helix/evidence` baseline を作ります。session continuation は `harness.db` に記録されます。`--json` では `consumerReadiness` /
