...
| 🧠 **健忘行動** | 文脈を忘れ、引き継ぎが雑になる | `ut-tdd handover` + **2 層エージェントメモリ**（システム/プロジェクト） | P5·P7 |
...
最新の gap・drift・未充足は prose ではなく `ut-tdd status` / `ut-tdd doctor` / HELIX DB が正。
...
ut-tdd setup project
ut-tdd doctor
```

## ⚙️ セットアップ

HELIX project の bootstrap 入口は `ut-tdd setup project` です。初回導入時は `consumerReadiness` /
...
| `ut-tdd db rebuild --json` | HELIX DB の再投影 |
...
```sh
ut-tdd setup project --team --tl-team @org/tl --qa-team @org/qa --po-team @org/po
```
...
```sh
ut-tdd doctor
ut-tdd db rebuild --json
ut-tdd telemetry scan --json
ut-tdd team suggest --task "production security schema migration" --mode hybrid --json
ut-tdd team run --definition .ut-tdd/teams/team.yaml --mode hybrid --json
```

`--execute` は、provider CLI を実際に起動すべきときだけ使います:

```sh
ut-tdd team run --definition .ut-tdd/teams/team.yaml --mode hybrid --execute
```
...
```sh
ut-tdd team suggest --task "subagent runtime adapter refactor" --json
```
...
> `ut-tdd db rebuild --json` で投影を再構築してから `doctor` を再実行してください。
