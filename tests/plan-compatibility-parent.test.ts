...
describe("compatibility-only PLAN authority boundary", () => {
  ...
  it("U-CPP-012: split consumers remain visible in the exact baseline without being edited", () => {
    const consumers = [
      "docs/plans/PLAN-L3-70-windows-lite-canary-admission.md",
      "docs/plans/PLAN-L6-81-drive-route-catalog.md",
      "docs/plans/PLAN-L7-462-issue-closure-contract.md",
    ];
    const baseline = JSON.parse(
      readFileSync("docs/governance/plan-compatibility-parent-baseline.json", "utf8"),
    ) as { grandfathered: Array<{ file: string }> };
    for (const file of consumers) {
      expect(baseline.grandfathered.some((entry) => entry.file === file)).toBe(true);
      expect(readFileSync(file, "utf8")).not.toContain("historical_provenance:");
    }
  });
  ...
  it("U-CPP-013: frontmatter preserves history without reemitting a dependency", () => {
    const raw = {
      plan_id: "PLAN-RECOVERY-1591-fixture",
      title: "compatibility parent fixture",
      kind: "recovery",
      layer: "cross",
      drive: "agent",
      status: "draft",
      agent_slots: [{ role: "aim", slot_label: "AIM — fixture" }],
      dependencies: {
        parent: "docs/design/helix/L3-requirements/github-autonomous-operations-requirements.md",
      },
      historical_provenance: [provenance],
    ...
    expect(parsed.historical_provenance).toEqual([provenance]);
