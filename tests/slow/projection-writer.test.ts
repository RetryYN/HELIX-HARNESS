  ...
  it("projects refactor candidates into quality signals and feedback events", () => {
    ...
    try {
      ...
      try {
        ...
        expect(String(feedback?.next_action ?? "")).toContain("triage refactor candidate");
      } finally {
        db.close();
      }
    } finally {
...
`,
  ...
  it.each([
    ...
    ["registry digest drift", "1.1.5", `sha256:${"0".repeat(64)}`],
  ])(
    ...
    try {
      ...
      writeFileSync(
        ...
        [
          ...
          "    green_commands:",
          "      - kind: unit_test",
          '        command: "npx --no-install vitest run tests/structured.test.ts"',
          "        runner: node",
          "        scope: targeted",
          "        exit_code: 1",
    ...
    try {
      ...
      writeFileSync(
        ...
        [
          ...
          "    green_commands:",
          "      - kind: unit_test",
          '        command: "npx --no-install vitest run tests/reporter.test.ts --reporter=json"',
          "        runner: node",
          "        scope: targeted",
          "        exit_code: 0",
          '        completed_at: "2026-07-03T00:01:00.000Z"',
          ...
          '        command: "npx --no-install playwright test tests/e2e/reporter.spec.ts --reporter=json"',
          "        runner: node",
          "        scope: targeted",
          "        exit_code: 0",
          ...
          '        output_digest: "sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"',
          "      - kind: unit_test",
          '        command: "npx --no-install vitest run tests/reporter.test.ts --reporter=junit"',
          "        runner: node",
          "        scope: targeted",
          "        exit_code: 0",
          '        completed_at: "2026-07-03T00:03:00.000Z"',
      ...
      writeFileSync(
        ...
        [
          JSON.stringify({
            ...
            event_type: "tool_use",
            tool_name: "Bash",
            target: "npx --no-install vitest run tests/projection-writer.test.ts",
            outcome: "ok",
          }),
          JSON.stringify({
