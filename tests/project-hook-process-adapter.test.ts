...
describe("project hook process adapter", () => {
  ...
  it("U-CNWHOOKPROC-005: PID・時刻・digest・graceの不正値では副作用を起こさない", async () => {
    const cases = [
      { child: { ...CHILD, pid: 1 }, grace_ms: 250 },
      { child: { ...CHILD, spawned_at: "2026-08-22" }, grace_ms: 250 },
      { child: { ...CHILD, command_digest: "sha256:BAD" as const }, grace_ms: 250 },
      { child: CHILD, grace_ms: -1 },
      { child: CHILD, grace_ms: 60_001 },
    ];
    for (const candidate of cases) {
      const observed = fake([true]);
      await expect(
        terminateProjectHookChild({ ...candidate, deps: observed.deps }),
