...
describe("source harness-check workflow", () => {
  ...
  it("U-ICLOSE-002: runs GitHub operation guards through the HELIX CLI instead of workflow-local rules", () => {
    ...
    // PLAN-L7-466-pr-scope-contract U-PRSCOPE-003: CI must pass the real base..head paths.
    expect(closureGuard.run).toContain('git diff --name-only -z "$PR_BASE_SHA..$PR_HEAD_SHA"');
