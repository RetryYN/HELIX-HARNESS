...
describe("source harness-check workflow", () => {
  ...
  it("U-CI-PREFLIGHT-AGGREGATION-001: 独立ゲートを観測してから集約し、高コスト工程へfail-closeする", () => {
    ...
    const missingCliExitPropagation = raw.replace('          exit "$status"', "          true");
    expect(preflightGateAggregationViolations(missingCliExitPropagation)).toContain(
      "aggregation_fail_close_contract_invalid",
    );
    const missingPureEvaluatorCall = raw.replace(
      "src/cli/preflight-gate-aggregation.ts",
      "src/cli.ts",
    );
    expect(preflightGateAggregationViolations(missingPureEvaluatorCall)).toContain(
      "aggregation_fail_close_contract_invalid",
    );
