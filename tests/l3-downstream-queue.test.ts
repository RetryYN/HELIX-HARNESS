...
describe("PLAN-L3-33 downstream queue numbering", () => {
  it("numbers all 51 slots exactly once without gaps", () => {
    ...
    expect(manifest.slots).toHaveLength(51);
    expect(slotsById.size).toBe(51);

    const expectedIds = [
      ...Array.from({ length: 23 }, (_, index) => `L3Q-PC-${String(index + 1).padStart(3, "0")}`),
      ...Array.from({ length: 16 }, (_, index) => `L3Q-IT-${String(index + 1).padStart(3, "0")}`),
  ...
  it("keeps the 23/16/12 phase denominators explicit", () => {
    for (const [phase, expected] of [
      ["pair_closure", 23],
      ["implementation_tdd", 16],
      ["refactor", 12],
    ] as const) {
      expect(manifest.counts[phase]).toBe(expected);
      expect(manifest.slots.filter((slot) => slot.phase === phase)).toHaveLength(expected);
    }
    expect(manifest.counts.pre_execution_total).toBe(51);
  });
