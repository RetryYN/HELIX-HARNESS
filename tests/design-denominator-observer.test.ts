...
import {
  DESIGN_DENOMINATOR_MANIFEST,
  observeCanonicalDesignDenominator,
} from "../src/state-db/design-denominator-observer";
...
describe("canonical Design denominator observer", () => {
  ...
  it("duplicate slice IDをstrict manifest errorとして拒否する", () => {
    const root = mkdtempSync(resolve(tmpdir(), "helix-denominator-"));
    temporaryRoots.push(root);
    const manifest = readFileSync(resolve(repoRoot, DESIGN_DENOMINATOR_MANIFEST), "utf8").replace(
      "  - HDS-HIL-02\n",
      "  - HDS-HIL-01\n",
    );
    const target = resolve(root, DESIGN_DENOMINATOR_MANIFEST);
    mkdirSync(dirname(target), { recursive: true });
    writeFileSync(target, manifest);
    expect(() => observeCanonicalDesignDenominator({ repoRoot: root })).toThrow(
      /slice ID set|duplicate IDs/,
    );
  });
  ...
  it("artifact path symlinkを拒否するreaderを共有する", () => {
    const root = mkdtempSync(resolve(tmpdir(), "helix-denominator-"));
    temporaryRoots.push(root);
    const manifestTarget = resolve(root, DESIGN_DENOMINATOR_MANIFEST);
