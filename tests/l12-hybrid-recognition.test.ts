...
describe("L12/hybrid recognition-risk scanner", () => {
  ...
  it("assigns exactly one reviewed final disposition to every candidate", () => {
    ...
    expect(candidates).toHaveLength(874);
  ...
  it("U-WLIFE-004: pins lifecycle authority and the routing-by-final cross table", () => {
    ...
    expect(REVIEWED_SAFE_DISPOSITIONS).toHaveLength(514);
    ...
    expect(cross).toEqual({
      ...
      plan_review: { compatibility_labeled: 1, conflict: 168, false_positive: 424 },
    });
