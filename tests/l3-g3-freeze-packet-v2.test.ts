...
describe("L3 G1/G3 freeze packet v2", () => {
  ...
  it("contains the five-question batch and unresolved issue dispositions", () => {
    ...
    expect(packet).toContain("L3Q-PC-001..023");
    expect(packet).toContain("この23はpair closure分母として固定");
    expect(packet).toContain("6+1+5=12小PR");
    expect(packet).toContain("合計16");
    expect(packet).toContain("pair closure 23 + L6/L7 16 + refactor 12 = 51 slot");
    expect(packet).toContain("この51にはL8〜L12実行receipt");
    for (const planId of [
