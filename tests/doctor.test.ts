...
function consumerClaudeAgentTemplate(name: string): string {
  return [
    ...
    "- `ut-tdd status`、`ut-tdd completion decision-packet --json`、`ut-tdd version-up dry-run --current v0.1.0 --target v0.1.3 --json`、`ut-tdd doctor --profile consumer` を HELIX local state evidence として使う。",
...
function consumerClaudeCommandTemplate(name: string): string {
  return [
    ...
    `Command: ${name}`,
    "",
    "現行 `ut-tdd` CLI 経由で repository-local HELIX command を使う。最初に `ut-tdd status --json`、`ut-tdd completion decision-packet --json`、`ut-tdd version-up dry-run --current v0.1.0 --target v0.1.3 --json` を実行し、必要な verification を走らせ、workflow または gate behavior に影響する場合は `ut-tdd doctor --profile consumer` で閉じる。",
    "",
  ].join("\n");
}
