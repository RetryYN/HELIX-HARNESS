...
const signals = (enabled: string[] = []) => Object.fromEntries(
...
describe("workflow interview unresolved engine", () => {
  it("U-UWINT-001: signal 0でもcore questionをexactly once選択する", () => {
    expect(evaluateWorkflowInterview(input())).toMatchObject({
      ok: true, freeze_allowed: true, selected_question_ids: ["core.workflow"],
    });
  });

  it("U-UWINT-002: true signalだけを選択し非該当回答を拒否する", () => {
    const exact = evaluateWorkflowInterview(input(["approval", "pii"], [
      answer("core.workflow"), answer("conditional.approval"), answer("conditional.pii"),
    ]));
    expect(exact.selected_question_ids).toEqual(["core.workflow", "conditional.approval", "conditional.pii"]);
    expect(exact.freeze_allowed).toBe(true);
    expect(evaluateWorkflowInterview(input([], [answer("core.workflow"), answer("conditional.billing")])).findings)
  ...
  it("U-UWINT-003: 未回答・矛盾・authority不足・branch gapを履歴付きでblockする", () => {
    const candidate = input(["approval"], [
    ...
    expect(result.unresolved_items.map((item) => item.kind)).toEqual(expect.arrayContaining(["contradiction", "branch_missing"]));
    expect(result.unresolved_items.every((item) => item.source_span && item.question_history.length > 0)).toBe(true);
    const missingAuthority = evaluateWorkflowInterview(input([], [answer("core.workflow", { authority: "unknown" })]));
    expect(missingAuthority.unresolved_items[0]?.kind).toBe("authority_missing");
  });

  it("U-UWINT-004: stale source bindingのanswerを再利用しない", () => {
    for (const overrides of [{ source_digest: `sha256:${"b".repeat(64)}` }, { source_revision: "r0" }]) {
  ...
  it("U-UWINT-005: 空sourceとunknown version／fieldをschema rejectする", () => {
    const empty = input(); empty.source.text = "";
