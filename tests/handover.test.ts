...
describe("U-HOVER-020 §3 workflow next action seed + anchor gate", () => {
  ...
  it("outstanding 指定で §3 に workflowNextActions 由来の機械次手と packet 要約を出力する", () => {
    ...
    expect(section).toContain("semantic-frontier-records: count=2");
    expect(section).toContain(
  ...
  it("§3 の packet 要約に semantic frontier / confirmed meaning が無ければ fail-close", () => {
    ...
    expect(r.ok).toBe(false);
    expect(r.messages[0]).toContain("semantic frontier");
  });
  ...
  it("§3 の packet 要約に source-delta field が無ければ fail-close", () => {
    ...
    expect(r.ok).toBe(false);
    expect(r.messages[0]).toContain("確認field件数/確認field/matrix必須field");
  });
  ...
  it("§3 の packet 要約が machine review route を確認観点に露出したら fail-close", () => {
    ...
    expect(r.ok).toBe(false);
    expect(r.messages[0]).toContain("確認観点へ露出");
  });
