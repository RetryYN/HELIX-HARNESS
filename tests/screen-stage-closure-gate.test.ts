...
import {
  buildScreenStageClosureCommit,
  createInMemoryScreenApplicabilityStore,
} from "../src/design/screen-applicability-store";

const NOW = "2026-08-07T00:00:00Z";

function seed(): ScreenStoreSeedV1 {
  return {
...
describe("U-SAP-011 commitStageClosureAndGate", () => {
  ...
  it.each([
    ...
    [
      ...
      (c: ScreenStageClosureCommitV1) => ({
        ...c,
        append_order: ["gate_receipt", "stage_completion", "stage_projection", "terminal_receipt"],
      }),
    ],
    [
      "write_set_digest改変",
  ...
  it("store登録済みplan routeとのreceipt digest不一致（seed側差し替え）はplan_route_swapで増分0", async () => {
    ...
    const store = createInMemoryScreenApplicabilityStore(world, NOW);
    const result = await store.commitStageClosureAndGate(validCommit());
    expect(result.ok).toBe(false);
    expect(store.committedGateReceiptCount()).toBe(0);
  });
  ...
  it("read系APIの未登録IDはfail-close（planRoute/skip/skipAuthority）", async () => {
    ...
    expect((await store.readSkipAuthority("skip-authority-1", "sha256:other", NOW)).ok).toBe(false);
    expect(
      (
  ...
  it("skip receiptのfreshness超過（expires_at <= trustedNow）はstage/gate増分0", async () => {
    const store = createInMemoryScreenApplicabilityStore(seed(), "2026-09-02T00:00:00Z");
    const result = await store.commitStageClosureAndGate(validCommit());
    expect(result.ok).toBe(false);
    expect(store.committedGateReceiptCount()).toBe(0);
  });
