---
description: frozen spec/test design に基づき UT-TDD quality gate 付きで実装する
argument-hint: "<PLAN id or feature>"
---

UT-TDD では incrementally に実装する。`incremental-implementation` skill を使う。

Target: $ARGUMENTS

Entry condition: PLAN の `parent_design` が既存の L5/L6 design を指し、`ut-tdd plan lint` が通り、
...
`ut-tdd review --uncommitted` は skip しない。trace-freeze 前に必要な evidence を生成するためである。
