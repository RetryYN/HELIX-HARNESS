...
- `PreToolUse(Edit|Write|MultiEdit)` は、current Claude session が触っていない uncommitted files への編集を block する。
  ...
  `.helix/state/foreign-edit-override` へ non-empty reason を書く方法だけを認める。marker bypass は
  `.helix/logs/foreign-edit-overrides.jsonl` に audit される。empty marker は bypass しない
