...
current canonicalはL1-L12であり、正規pairは`L1↔L12` / `L2↔L11` / `L3↔L10` /
`L4↔L9` / `L5↔L8` / `L6↔L7`である。L0 charterは層外anchorとする。
L0-L14はcompatibility projectionに限定し、Claude runtimeが新規成果物やCI期待値を旧体系へ戻してはならない。
詳細authorityは`../docs/governance/l12-canonical-vmodel-direction-directive_v0.1.md`を参照する。

## Hook 方針

`.claude/settings.json` の active hooks は package-local Node runtime commandsだけを呼ぶ。
personal legacy runtime pathsやBunに依存するhooksは有効化しない。

- `PreToolUse(Agent)`: `npx --no-install tsx "$CLAUDE_PROJECT_DIR/.claude/hooks/agent-guard.ts"`
- `PreToolUse(Edit|Write|MultiEdit)`: `npx --no-install tsx "$CLAUDE_PROJECT_DIR/.claude/hooks/work-guard.ts"`
- `SessionStart`: `npx --no-install tsx "$CLAUDE_PROJECT_DIR/src/cli.ts" session start`
- `PostToolUse(Edit|Write|MultiEdit|Bash)`: `npx --no-install tsx "$CLAUDE_PROJECT_DIR/src/cli.ts" hook post-tool-use`
- `Stop`: `npx --no-install tsx "$CLAUDE_PROJECT_DIR/src/cli.ts" session summary`
- `SubagentStop`: `npx --no-install tsx "$CLAUDE_PROJECT_DIR/src/cli.ts" hook subagent-stop`
...
`helix plan lint`、targeted tests、`helix doctor` を使う。
...
HELIX work の通常経路として raw `codex exec` や raw `claude` を使わない。
session lifecycle、DB-backed continuation、audit evidence を記録できるように HELIX wrapper を使う。
...
```bash
npx --no-install tsx "$CLAUDE_PROJECT_DIR/.claude/hooks/agent-guard.ts"
```
...
- `PreToolUse(Edit|Write|MultiEdit)` は、current Claude session が触っていない uncommitted files への編集を block する。
  ...
  `HELIX_ALLOW_FOREIGN_EDIT=1`（env、human/out-of-band）または session 中に
  `.helix/state/foreign-edit-override` へ non-empty reason を書く方法だけを認める。env/marker bypass は
  `harness.db.guard_override_transactions` にdigest-onlyでauditされる。empty marker は bypass しない
  （recorded reason なしの silent override を許可しない）。marker は **one-shot** であり、許可した foreign edit で
  consume（delete）されるため stale marker は guard bypass を継続できない。env override はsession/subject単位の
  virtual one-shot capabilityとして同じDB transactionを通り、同一nonceの再利用をblockする。
...
- Linuxはfull canonical gate、Native Windows／macOSは同一fixtureのcompatibility gateとする。
  WSL2はoptional compatibilityでありrequired conditionではない。

## Cutover 境界

HELIXはprevious frameworkからbehavior／design conceptsを取り込む。Pythonはversioned contract内の恒久semantic coreである。ADR-009/010のtargetはPython semantic coreと
TypeScript/Node transactional boundaryの層別authorityである。Nodeが全出力を再検証して唯一のtransaction writerとなり、
legacy Python modulesやcommandsをcurrent operating pathとして説明しない。
TypeScript/NodeとPythonの責務境界だけをcurrent execution authorityとし、Bunは廃止・active dependency 0とする。
historical evidenceに残す旧Bun文字列を実行・rollback・完了根拠へ再昇格しない。
Python workerはnetwork default denyとし、DB path／credential／repository／`.helix/`を渡さない。Python出力のcommand、SQL、
absolute path、codeを実行せず、proposal bytesをNodeがschema／digest／authority policyで再検証する。
...
- Node.js LTSとPythonだけをruntime前提とし、BunがPATH上に無いclean環境で全gateを実行する。
- target Node.jsはADR-009のLTS範囲を満たす。
- `CLAUDE_PROJECT_DIR` は hook execution 中に repository root を指す。
- `.helix/` は writable generated runtime state である。
- `.claude/settings.json` は package-local command だけを使う。
- Personal absolute paths は通常運用では不要である。

## HELIX Adapter Rule Markers（アダプタールール marker）
...
- セットアップ: `helix setup project`
- セッション証跡: `helix status` / `harness.db` continuation projection
- 診断: `helix doctor`
