...
personal legacy runtime paths に依存する hooks は有効化しない。下記はADR-009 cutover完了までの
known-good経路であり、target authorityではない。Node hookへの置換は全hookを同じcutover transactionで行う。
...
HELIXはprevious frameworkからbehavior／design conceptsを取り込む。ADR-009を部分改定するADR-010により、
Pythonはversioned contract内の恒久semantic core、Nodeは全出力を再検証する唯一のtransaction writerである。
動作済みPython意味コアのTS/Node一括再実装と、PythonからDB/Git/GitHubへのauthoritative writeをどちらも禁止する。
terminal receipt前のactive execution authorityは既存Bun、terminal後はreceipt-bound Node、承認済みrollback時だけ
Node receiptをstale化してBunを一時再activationする。Bun経路をNode/Python完了の根拠にしない。
Python semantic coreはnetwork default denyとし、DB path／credential／repository／`.helix/`を渡さない。Python出力のcommand、SQL、
absolute path、codeを実行せず、semantic result bytesをNodeがschema／digest／authority policyで再検証する。
...
- cutover期間は既存known-good検証のため`bun`がPATH上で利用可能である。target clean Linux gateはBunなしで実行する。
