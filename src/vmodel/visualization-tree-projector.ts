...
type ProjectHandoffArtifactStatus =
  VisualizationContract["project"]["current_location"]["recovery_handoff_artifacts"]["items"][number];
...
function node(input: {
  ...
  return {
    ...
    kind: input.contextValue,
    expanded: input.expanded,
    action: input.commandPointer
      ? {
          kind: "copy-pointer",
          pointer: readOnlySummaryPointer(input.commandPointer),
        }
      : undefined,
    children,
  };
...
function activeApprovalDraftForAction(
  artifacts: VisualizationContract["project"]["current_location"]["recovery_handoff_artifacts"],
  action: string,
...
function metricRows(parentId: string, rows: MetricRow[]): GenericTreeNode[] {
  return rows.map((row) =>
    node({
...
function boundaryNode(
  ...
  boundary: VisualizationContract["view_boundaries"]["project"],
): GenericTreeNode {
  return node({
...
function projectCurrentLocation(vm: VisualizationContract): GenericTreeNode {
...
function projectLayerProgress(vm: VisualizationContract): GenericTreeNode {
  return node({
...
function projectRuntimeEvidence(vm: VisualizationContract): GenericTreeNode {
  return node({
...
function projectRoot(vm: VisualizationContract): GenericTreeNode {
  return node({
    id: "project",
    label: "Project",
...
function harnessRoot(vm: VisualizationContract): GenericTreeNode {
  return node({
    id: "harness",
    label: "HARNESS",
    description: vm.source_clock ?? "no snapshot",
...
export function buildVisualizationTree(vm: VisualizationContract): GenericTree {
  return {
