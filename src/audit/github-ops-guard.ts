...
export function evaluateGithubOpsGuard(input: GithubOpsGuardInput): GithubOpsGuardResult {
  ...
  const normalizedHeadRef = normalizeBranchRef(headRef);
  ...
  if (type === "poc" && normalizedBaseRef === "main") {
    findings.push({
      ...
      severity: "error",
      evidence: `${normalizedHeadRef}->${normalizedBaseRef}`,
...
export function renderGithubOpsGuard(result: GithubOpsGuardResult): string {
  const lines = [
    `github guard: ${result.ok ? "ok" : "failed"} head=${result.normalizedHeadRef} base=${result.normalizedBaseRef} type=${result.branchType}`,
  ];
...
export function buildReleasePublicationPlan(input: {
  ...
  const tarball = `.helix/release/${tag}.tar.gz`;
  return {
    schemaVersion: "helix-github-release-publication-plan.v1",
...
export function renderReleasePublicationPlan(plan: ReleasePublicationPlan): string {
  const lines = [
    `github release-plan: ${plan.ok ? "ok" : "invalid"} tag=${plan.tag} repo=${plan.repo} dryRun=${plan.dryRun}`,
    `  - externalPublishRequiresApproval=${plan.externalPublishRequiresApproval}`,
    `  - mustNotApplyWithoutApproval=${plan.mustNotApplyWithoutApproval}`,
  ];
