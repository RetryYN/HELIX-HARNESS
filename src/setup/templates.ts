...
function agentTemplate(name: string, description: string): string {
  return [
    ...
    "- PLAN-M-02 で CLI 名と state dir が変更されるまでは、HELIX local state evidence は `.ut-tdd` 配下を正本とし、`ut-tdd status`、`ut-tdd completion decision-packet --json`、`ut-tdd completion review-bundle --json`、`ut-tdd version-up dry-run --current v0.1.0 --target v0.1.4 --release-remote https://github.com/RetryYN/HELIX-HARNESS-OS.git --json`、`ut-tdd doctor --profile consumer` を使う。completion review-bundle は exact digest と semantic digest の両方を確認する。",
...
export const BUILTIN_GITHUB_TEMPLATES: TemplateSet = {
  ...
  "adapter/AGENTS.md": [
    ...
    "# HELIX アダプター",
    "",
    "この project は HELIX lifecycle を現行 `ut-tdd` command で扱う。PLAN-M-02 で atomic identifier migration が行われるまでは、CLI 名は `ut-tdd` のまま扱う。",
    "",
  ...
  "adapter/CLAUDE.md": [
    ...
    "# HELIX 共有コンテキスト",
    "",
    "harness state と delegation には repository-local の現行 `ut-tdd` command を使う。PLAN-M-02 までは command 名を `ut-tdd` とする。",
    "",
    "PO への進捗報告・調査結論・確認依頼など chat 出力は日本語を既定とする。docs / handover / adapter prose も日本語を基本とし、CLI 名・識別子・技術用語は原語のまま扱ってよい。",
    "",
  ...
  "project/.vscode/tasks.json": [
    ...
    '      "type": "shell",',
    '      "command": "bun run ut-tdd version-up dry-run --current v0.1.0 --target v0.1.4 --release-remote https://github.com/RetryYN/HELIX-HARNESS-OS.git --json",',
    '      "problemMatcher": []',
    "    },",
    "    {",
    ...
    '      "type": "shell",',
    '      "command": "bun run ut-tdd team run --definition .ut-tdd/teams/default-hybrid.yaml --mode hybrid --json",',
    '      "problemMatcher": []',
    "    }",
    "  ]",
    "}",
