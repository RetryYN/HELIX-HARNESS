...
function agentTemplate(name: string, description: string): string {
  return [
    ...
    "- PLAN-M-02 で CLI 名が変更されるまでは、`ut-tdd status`、`ut-tdd completion decision-packet --json`、`ut-tdd version-up dry-run --current v0.1.0 --target v0.1.3 --release-remote https://github.com/unison-ai-product/UT-TDD_AGENT-HARNESS-Pack.git --json`、`ut-tdd doctor --profile consumer` を HELIX local state evidence として使う。",
...
function commandTemplate(name: string, description: string): string {
  return [
    ...
    "対象: $ARGUMENTS",
    "",
    "現行 `ut-tdd` CLI 経由で repository-local HELIX command を使う。最初に `ut-tdd status --json`、`ut-tdd completion decision-packet --json`、`ut-tdd version-up dry-run --current v0.1.0 --target v0.1.3 --release-remote https://github.com/unison-ai-product/UT-TDD_AGENT-HARNESS-Pack.git --json` を実行し、対象に必要な narrow verification を走らせ、workflow または gate behavior に影響する場合は `ut-tdd doctor --profile consumer` で閉じる。",
    "",
  ].join("\n");
}
...
export const BUILTIN_GITHUB_TEMPLATES: TemplateSet = {
  ...
  "adapter/.claude/CLAUDE.md": [
    ...
    "PO への進捗報告・調査結論・確認依頼など chat 出力は日本語を既定とする。docs / handover / adapter prose も日本語を基本とし、CLI 名・識別子・技術用語は原語のまま扱ってよい。",
    "",
    "- Session evidence: `ut-tdd status`、`ut-tdd completion decision-packet --json`、`ut-tdd version-up dry-run --current v0.1.0 --target v0.1.3 --release-remote https://github.com/unison-ai-product/UT-TDD_AGENT-HARNESS-Pack.git --json`、`ut-tdd rename plan --json`、`ut-tdd handover`",
  ...
  "adapter/.claude/commands/helix-status.md": [
    ...
    "---",
    "",
    "`ut-tdd status --json`、`ut-tdd completion decision-packet --json`、`ut-tdd version-up dry-run --current v0.1.0 --target v0.1.3 --release-remote https://github.com/unison-ai-product/UT-TDD_AGENT-HARNESS-Pack.git --json`、`ut-tdd doctor --profile consumer` を実行し、HELIX mode、completionClaimAllowed、version-up plan-only 境界、未完了 blocker、hard-gate failure を要約する。",
    "",
  ].join("\n"),
  "adapter/.claude/commands/helix-test.md": [
    ...
    "対象: $ARGUMENTS",
    "",
    "最初に `ut-tdd status --json`、`ut-tdd completion decision-packet --json`、`ut-tdd version-up dry-run --current v0.1.0 --target v0.1.3 --release-remote https://github.com/unison-ai-product/UT-TDD_AGENT-HARNESS-Pack.git --json` を確認し、narrow Vitest target、`bun run typecheck`、`bun run lint` を実行する。変更が HELIX workflow または gate に影響する場合は `ut-tdd doctor --profile consumer` で閉じる。",
    "",
  ].join("\n"),
