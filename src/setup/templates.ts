...
function agentTemplate(name: string, description: string): string {
  return [
    ...
    "- PLAN-M-02 で CLI 名が変更されるまでは、`ut-tdd status`、`ut-tdd completion decision-packet --json`、`ut-tdd completion review-bundle --json`、`ut-tdd version-up dry-run --current v0.1.0 --target v0.1.3 --release-remote https://github.com/unison-ai-product/UT-TDD_AGENT-HARNESS-Pack.git --json`、`ut-tdd doctor --profile consumer` を HELIX local state evidence として使う。",
...
export const BUILTIN_GITHUB_TEMPLATES: TemplateSet = {
  ...
  "adapter/CLAUDE.md": [
    ...
    "- `ut-tdd completion review-bundle --json` は S4 / version-up / rename / action-binding の scoped review packet と digest を確認する。",
  ...
  "adapter/.claude/CLAUDE.md": [
    ...
    "PO への進捗報告・調査結論・確認依頼など chat 出力は日本語を既定とする。docs / handover / adapter prose も日本語を基本とし、CLI 名・識別子・技術用語は原語のまま扱ってよい。",
    "",
    "- Session evidence: `ut-tdd status`、`ut-tdd completion decision-packet --json`、`ut-tdd completion review-bundle --json`、`ut-tdd version-up dry-run --current v0.1.0 --target v0.1.3 --release-remote https://github.com/unison-ai-product/UT-TDD_AGENT-HARNESS-Pack.git --json`、`ut-tdd rename plan --json`、`ut-tdd handover`",
  ...
  "adapter/.claude/commands/helix-status.md": [
    ...
    "---",
    "",
    "`ut-tdd status --json`、`ut-tdd completion decision-packet --json`、`ut-tdd completion review-bundle --json`、`ut-tdd version-up dry-run --current v0.1.0 --target v0.1.3 --release-remote https://github.com/unison-ai-product/UT-TDD_AGENT-HARNESS-Pack.git --json`、`ut-tdd doctor --profile consumer` を実行し、HELIX mode、completionClaimAllowed、completion review bundle、version-up plan-only 境界、未完了 blocker、hard-gate failure を要約する。",
    "",
  ].join("\n"),
