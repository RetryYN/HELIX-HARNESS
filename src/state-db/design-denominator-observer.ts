...
import { z } from "zod";
import { canonicalJson, sha256Digest, type Sha256Digest } from "../runtime/digest";
...
const manifestSchema = z.object({
  ...
  slice_ids: z.array(z.enum(SLICE_IDS)), slice_count: z.literal(19), slice_set_digest: digestSchema,
  artifact_inventory: z.array(artifactSchema), artifact_count: z.literal(76),
  artifact_path_content_set_digest: digestSchema,
  canonical_unit_ids: z.array(idSchema), canonical_unit_count: z.literal(475), canonical_unit_set_digest: digestSchema,
  canonical_integration_ids: z.array(idSchema), canonical_integration_count: z.literal(360), canonical_integration_set_digest: digestSchema,
  canonical_hst_ids: z.array(idSchema), canonical_hst_count: z.literal(411), canonical_hst_set_digest: digestSchema,
  canonical_quartet_count: z.literal(835), canonical_total_count: z.literal(1246), canonical_combined_set_digest: digestSchema,
  supporting_meta_oracle_receipt: z.record(z.string(), z.unknown()),
  expected_terminal_receipt_id: z.string().min(1), expected_terminal_receipt_digest: digestSchema,
...
export function observeCanonicalDesignDenominator(input: {
  ...
  if (document.errors.length > 0) throw new Error(`invalid denominator YAML: ${document.errors[0]?.message}`);
  ...
  exactUnique(manifest.artifact_inventory.map((entry) => entry.path), "artifact path denominator");
  for (const ids of [manifest.canonical_unit_ids, manifest.canonical_integration_ids, manifest.canonical_hst_ids])
    exactUnique(ids, "canonical assertion denominator");
  if (manifest.canonical_unit_ids.length !== manifest.canonical_unit_count ||
  ...
  const artifactObservations = manifest.artifact_inventory.map((entry) => {
    ...
    try { headContentDigest = sha256Digest(execFileSync("git", ["-C", root, "show", `HEAD:${entry.path}`])); } catch { headContentDigest = null; }
    return {
  ...
  const currentArtifactSetDigest = sha256Digest(canonicalJson(
    artifactObservations.map(({ sliceId, path, currentContentDigest }) => ({ sliceId, path, currentContentDigest })),
  ));
  ...
  const layerLedgerRevision = {
    ...
    declaredHistoricalDesignSnapshotDigest: manifest.snapshot_binding.design_snapshot_digest as Sha256Digest,
  };
  const sourceRepository = { headOid: git(root, "HEAD"), treeOid: git(root, "HEAD^{tree}") };
  const historicalDigestMatchCount = artifactObservations.filter((entry) => entry.matchesHistoricalDigest).length;
  const headDigestMatchCount = artifactObservations.filter((entry) => entry.matchesHead).length;
  const resultWithoutDigest = {
    manifestPath, manifestDigest: manifestSource.digest, authorityModel, layerLedgerRevision, sourceRepository,
    sliceIds: [...SLICE_IDS], artifactObservations, currentArtifactSetDigest,
    historicalArtifactSetDigest: manifest.artifact_path_content_set_digest as Sha256Digest,
    historicalDigestMatchCount, historicalDigestMismatchCount: 76 - historicalDigestMatchCount,
    headDigestMatchCount, headDigestMismatchCount: 76 - headDigestMatchCount,
  };
  return { ...resultWithoutDigest, observationDigest: sha256Digest(canonicalJson(resultWithoutDigest)) };
}
