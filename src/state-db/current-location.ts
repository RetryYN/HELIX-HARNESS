...
export interface ProjectClosureAutoApprovalRecord {
  ...
  target_plan_ids: string[];
  evidence: {
    review_bundle_digest_matches: boolean;
    tests_green: boolean;
    gates_green: boolean;
    apply_dry_run_succeeded: boolean;
  };
...
export interface ProjectClosureAutoApprovalBatch {
  ...
  approval_record: ProjectClosureAutoApprovalRecord;
  apply_plan: ProjectClosureApplyPlan;
  blocked_reasons: string[];
  write_policy: "read-only";
  source_command: "helix closure auto-approve --dry-run --json";
}
...
const IRREVERSIBLE_CLOSURE_PLAN_IDS = new Set(["PLAN-L7-146", "PLAN-M-02"]);

function isIrreversibleClosureCandidate(item: ProjectClosureQueueItem): boolean {
  const normalized = `${item.planId} ${item.kind} ${item.sourcePath}`.toLowerCase();
  return (
    [...IRREVERSIBLE_CLOSURE_PLAN_IDS].some(
      (id) => item.planId === id || item.planId.startsWith(`${id}-`),
    ) ||
    normalized.includes("external-publish") ||
    normalized.includes("external_publish") ||
    normalized.includes("charter-p8") ||
    normalized.includes("charter_p8")
  );
}

/**
 * close_ready の可逆な status closure だけを対象に、PO 承認済みの機械 evidence 条件を評価する。
 * publish / cutover / activation / charter P8 は混在した batch 全体を fail-close にする。
 */
export function buildProjectClosureAutoApprovalBatch(
  snapshot: ProjectCurrentLocationSnapshot,
  input: { limit?: number; offset?: number } = {},
): ProjectClosureAutoApprovalBatch {
  ...
  const irreversible = bundle.candidates.filter(isIrreversibleClosureCandidate);
  const testsGreen =
    bundle.candidates.length > 0 &&
    bundle.candidates.every(
      (candidate) =>
        candidate.evidence.testRuns.total > 0 &&
        candidate.evidence.testRuns.failed === 0 &&
        candidate.evidence.testRuns.passed === candidate.evidence.testRuns.total,
    );
  const gatesGreen =
    bundle.candidates.length > 0 &&
    bundle.candidates.every(
      (candidate) =>
        candidate.evidence.gateRuns.total > 0 &&
        candidate.evidence.gateRuns.failed === 0 &&
        candidate.evidence.gateRuns.passed === candidate.evidence.gateRuns.total,
    );
  const approvalRecordText = [
    ...
    `approval_scope_digest: ${bundle.review_scope.approval_scope_digest}`,
    "approval_mode: machine-evidence",
    "reason: review-bundle digest、tests/gates green、closure apply dry-run の機械証跡に基づく自走承認",
  ].join("\n");
  const applyPlan = buildProjectClosureApplyPlan(snapshot, {
    approvalRecordPath: ".helix/audit/closure-auto-approval.yml",
    approvalRecordText,
    limit,
    offset,
  });
  const reviewDigestMatches =
    applyPlan.approval.approval_scope_digest === bundle.review_scope.approval_scope_digest;
  const preliminaryBlockers = [
    ...(reviewDigestMatches ? [] : ["review-bundle digest が apply scope と一致しない"]),
    ...(testsGreen ? [] : ["対象 PLAN の tests が全件 green ではない"]),
    ...(gatesGreen ? [] : ["対象 PLAN の gates が全件 green ではない"]),
    ...(irreversible.length === 0
      ? []
      : [
          `後戻り不能境界は human approval 必須: ${irreversible.map((item) => item.planId).join(",")}`,
        ]),
    ...applyPlan.blocked_reasons,
  ];
  const dryRunSucceeded = preliminaryBlockers.length === 0 && applyPlan.allowed_to_apply;
  const auditPayload = {
    approval_scope_digest: bundle.review_scope.approval_scope_digest,
    target_plan_ids: bundle.review_scope.plan_ids,
    tests_green: testsGreen,
    gates_green: gatesGreen,
    apply_dry_run_succeeded: dryRunSucceeded,
    blocked_plan_ids: irreversible.map((item) => item.planId),
  };
  const record: ProjectClosureAutoApprovalRecord = {
    schema_version: "project-closure-auto-approval.v1",
    decision_id: "closure-review:close_ready",
    outcome: "approve_closure_claim",
    approval_scope_digest: bundle.review_scope.approval_scope_digest,
    ...
    target_plan_ids: [...bundle.review_scope.plan_ids],
    evidence: {
      review_bundle_digest_matches: reviewDigestMatches,
      tests_green: testsGreen,
      gates_green: gatesGreen,
      apply_dry_run_succeeded: dryRunSucceeded,
    },
    irreversible_boundary: {
      human_required: irreversible.length > 0,
      blocked_plan_ids: irreversible.map((item) => item.planId),
    },
    audit_digest: `sha256:${createHash("sha256").update(JSON.stringify(auditPayload)).digest("hex")}`,
    reason:
      "review-bundle digest、tests/gates green、closure apply dry-run の機械証跡に基づく自走承認",
  };
  return {
    schema_version: "project-closure-auto-approval-batch.v1",
    dry_run: true,
    ...
    listed: bundle.listed,
    omitted: bundle.omitted,
    allowed_to_execute: dryRunSucceeded,
    approval_record: record,
    apply_plan: applyPlan,
    blocked_reasons: preliminaryBlockers,
    write_policy: "read-only",
    source_command: "helix closure auto-approve --dry-run --json",
  };
}
