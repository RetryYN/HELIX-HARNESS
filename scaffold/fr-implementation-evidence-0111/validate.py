#!/usr/bin/env python3
"""SCF-B-0111の固定BASE静的証拠bundleをfail-closedで検査する。"""
...
SCHEMA = "fr-implementation-evidence-0111/v1"
...
SOURCE_IDS = [f"HIL-FR-{n:02d}" for n in range(41, 70)]
UNIT_PATTERN = re.compile(r"^IRUNIT-HIL-FR-(4[1-9]|5[0-9]|6[0-9])-HELIX-(OS|HARNESS)$")
CONTEXT_INPUTS = [
...
def coverage_failure(edge: dict[str, Any]) -> Any:
    coverage = edge.get("coverage", {})
    return coverage.get("failure") if isinstance(coverage, dict) else None
...
def target_crosswalk() -> list[dict[str, Any]]:
    ...
    if len(rows) != 37 or any(not UNIT_PATTERN.fullmatch(row.get("unit_candidate_id", "")) for row in rows):
        raise ValueError("HIL-FR-41〜69 crosswalk unit集合が不正")
    if len({row["unit_candidate_id"] for row in rows}) != 37:
        raise ValueError("HIL-FR-41〜69 unitが重複している")
    return rows
...
def old_asset_record(asset_id: str, edges: list[dict[str, Any]], disposition: dict[str, dict[str, Any]],
    ...
    coverage_failures = sorted({failure for failure in (coverage_failure(edge) for edge in asset_edges) if failure})
...
def expected_failure(edges: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "records": [{
            "review_id": edge["review_id"], "asset_id": edge["asset_id"],
            "coverage_failure": coverage_failure(edge),
...
class Validator:
    ...
    def finish(self) -> int:
        if self.errors:
            ...
            return 1
        print("SCF-B-0111 validate: PASS (37 units, 111 review edges, 75 old assets; static-only)")
        return 0
    ...
    def check_asset(self, asset: dict[str, Any], expected: dict[str, Any]) -> None:
        if asset != expected:
            if asset.get("ledger_record") != expected.get("ledger_record"):
                ...
                self.error("E_OLD_ASSET_HISTORY", f"{asset.get('asset_id')} old asset evidence不一致")
    ...
    def validate(self) -> int:
        ...
        expected_inventory_keys = {
            "schema", "binding_id", "status", "authority_effect", "new_build", "base", "scope",
            "anchor_digest_policy", "input_snapshot", "counts", "unit_ids", "unit_declarations",
            "partition_contract", "evidence_partition", "negative_case_codes", "prohibited_inference", "unresolved",
        }
        if set(inventory) != expected_inventory_keys:
            self.error("E_INVENTORY_DECLARATION", "inventoryトップレベルkey集合が期待値と一致しない")
        ...
        if [row.get("unit_candidate_id") for row in evidence] != units or len(set(row.get("unit_candidate_id") for row in evidence)) != len(units):
            self.error("E_UNIT_SET", "FR41〜69 unit集合の順序・分母・重複が不一致")
        ...
        for source in crosswalk:
            ...
            if current is None:
                continue
            expected_unit_keys = {
                "schema", "unit_candidate_id", "source_requirement", "source_anchor", "semantic_review_edges",
                "asset_set", "old_asset_evidence", "implementation_evidence", "degradation_evidence",
                "failure_evidence", "consumer_evidence", "representative_assets", "current_context",
                "current_implementation_evidence", "unimplemented_assessment", "authority_boundary", "unresolved",
            }
            ...
            if current.get("old_asset_evidence") != {
                "assets": expected_asset_records, "static_only": True, "not_implementation_proof": True,
            }:
                self.error("E_OLD_ASSET_HISTORY", f"{unit} old_asset_evidence全体が期待値と一致しない")
