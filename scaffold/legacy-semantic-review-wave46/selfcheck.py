#!/usr/bin/env python3
"""Run Wave46 validation and explicit fail-closed mutations."""
...
spec = importlib.util.spec_from_file_location("wave46_validator", HERE / "validate.py")
...
def rejected(label: str, mutate) -> None:
    ...
    mutate(mutated_rows)
    try:
        with tempfile.TemporaryDirectory(prefix="wave46-selfcheck-") as tmp:
            root = Path(tmp)
...
def rejected_meta(label: str, mutate) -> None:
    ...
    meta = json.loads(original_meta.read_text())
    try:
        with tempfile.TemporaryDirectory(prefix="wave46-meta-selfcheck-") as tmp:
...
def rejected_document(label: str, module_attr: str, mutate) -> None:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="wave46-document-selfcheck-") as tmp:
...
def mutate_pool_consumed_overclaim(meta) -> None:
    reconciliation = next(
        ...
        if item["unit_candidate_id"] == "IRUNIT-HIL-NFR-27-HELIX-HARNESS" and item["role_kind"] == "implementation_source"
    )
...
def mutate_design_reconciliation_omission(meta) -> None:
    receipt = next(
        ...
        if item["unit_candidate_id"] == "IRUNIT-HIL-NFR-30-HELIX-OS" and item["role_kind"] == "design"
    )
...
def reject_wave45_copy_residue() -> None:
    """Reject stale Wave45 template text in human-facing Wave46 artifacts."""
    forbidden = (
        "Wave45 legacy semantic review Scaffold",
        "SCF-B-0082",
        "f8abbba3c04fbbd3e0a4787701a53d854d37b889",
    ...
    for path in (HERE / "README.md", HERE / "plan.json", HERE / "inventory.json"):
        text = path.read_text()
        assert not any(marker in text for marker in forbidden), f"stale Wave45 copy residue: {path}"


reject_wave45_copy_residue()
