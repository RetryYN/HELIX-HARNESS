#!/usr/bin/env python3
"""Generate the Wave46 schema10 research-premise ledger.
...
UNIT_SPECS = [
    {
        "unit": "IRUNIT-HIL-NFR-27-HELIX-HARNESS",
        "query": {"anchors": ["translator", "ambiguity", "template", "source"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-A6F899B80CE9AEA61B82",
        "implementation_asset": None,
        "missing_roles": ["implementation_source"], "atom_prefix": "NFR27-HARNESS", "composite_unresolved_count": 1,
        "evidence": {"design": ("docs/design/helix/L3-requirements/vmodel-docgen-fit.md", 1, 277, [])},
    },
    {
        "unit": "IRUNIT-HIL-NFR-27-HELIX-OS",
        "query": {"anchors": ["template", "active", "authority", "source"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-D11F51092619506417E4",
        "implementation_asset": "LEGACY-ASSET-93D79C2EF43DB865BF66",
        "missing_roles": [], "atom_prefix": "NFR27-OS", "composite_unresolved_count": 1,
        "evidence": {"design": ("docs/design/helix/L3-requirements/multimodal-design-harness-authority.md", 1, 192, []), "implementation_source": ("src/doctor/universal-improvement-source-registry-check.ts", 1, 13, [])},
    },
    {
        "unit": "IRUNIT-HIL-NFR-28-HELIX-HARNESS",
        "query": {"anchors": ["requirement", "coverage", "oracle", "source"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-E9CE4A7754A195D6186E",
        "implementation_asset": None,
        "missing_roles": ["implementation_source"], "atom_prefix": "NFR28-HARNESS", "composite_unresolved_count": 1,
        "evidence": {"design": ("docs/design/helix/L3-requirements/technology-stack-authority.md", 1, 170, [])},
    },
    {
        "unit": "IRUNIT-HIL-NFR-28-HELIX-OS",
        "query": {"anchors": ["oracle", "receipt", "stale", "source"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-664EDC7E6280C3DD1691",
        "implementation_asset": None,
        "missing_roles": ["implementation_source"], "atom_prefix": "NFR28-OS", "composite_unresolved_count": 1,
        "evidence": {"design": ("docs/design/helix/L12-vmodel/vmodel-solo-tailoring-profile.md", 1, 137, [])},
    },
    {
        "unit": "IRUNIT-HIL-NFR-30-HELIX-HARNESS",
        "query": {"anchors": ["authoring", "authority", "scope", "source"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-5412A166E9D88C3C2436",
        "implementation_asset": None,
        "missing_roles": ["implementation_source"], "atom_prefix": "NFR30-HARNESS", "composite_unresolved_count": 1,
        "evidence": {"design": ("docs/design/harness/L2-screen/business-flow.md", 1, 150, [])},
    },
    {
        "unit": "IRUNIT-HIL-NFR-30-HELIX-OS",
        "query": {"anchors": ["canonical", "automation", "authority", "source"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": None,
        "implementation_asset": None,
        "missing_roles": ["design", "implementation_source"], "atom_prefix": "NFR30-OS", "composite_unresolved_count": 1,
        "evidence": {},
    },
]
...
REVIEW_SCOPE = "Wave46 schema10 research-premise candidate: exact requirement source / product boundary / phase candidate / catalog asset role"
...
def prior_artifact(wave: int, suffix: str) -> tuple[str, Path]:
    ...
    if wave in {37, 38, 39, 40, 41, 42, 43, 44, 45}:
...
def build_evidence_ref(role: str, source_path: str, start: int, end: int, ordinal: int, relation: str) -> dict:
    ...
    return {
        "archive_path": full,
        "artifact_role": role,
        "evidence_ref_id": f"LEGACY-ASSET-WAVE46-{role}-{ordinal}",
...
def main() -> None:
    ...
    for spec in UNIT_SPECS:
        ...
        for role, asset in asset_by_role:
            ...
            if role == "requirement":
                req_ranges = {
                    "HIL-NFR-27": (5541, 5583),
                    "HIL-NFR-28": (5584, 5626),
                    "HIL-NFR-30": (5670, 5712),
                }
            ...
            row = {
                ...
                "review_id": f"LSRW46-EDGE-{len(rows) + 1:03d}",
    ...
    for spec in UNIT_SPECS:
        ...
        for role in spec.get("missing_roles", []):
            ...
            candidate_pool_reconciliations.append({
                ...
                "pool_usage_basis": "prior Wave1-45 non-requirement ledger assets plus current Wave46 non-requirement selected assets; requirement asset excluded",
    ...
    ledger_path = OUT / "legacy-requirement-direct-semantic-review-wave46.jsonl"
    ...
    meta = {
        ...
        "wave45_candidate_parent": "d0c2e2e2aab746a60f3d51b20647de8e59fbfa09",
        "wave45_candidate_ref": "research/legacy-semantic-review-wave45",
        "wave45_candidate_root": "d0c2e2e2aab746a60f3d51b20647de8e59fbfa09",
        "wave45_exact_head": "d0c2e2e2aab746a60f3d51b20647de8e59fbfa09",
    }
    # Preserve the historical lineage fields carried by schema10 Wave36.
    previous_meta = json.loads((ROOT / "scaffold/legacy-semantic-review-wave45/legacy-requirement-direct-semantic-review-wave45.meta.json").read_text())
    for key in previous_meta:
        if key.startswith("wave") and key not in meta:
            meta[key] = previous_meta[key]
    meta_path = OUT / "legacy-requirement-direct-semantic-review-wave46.meta.json"
