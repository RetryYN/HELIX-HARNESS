#!/usr/bin/env python3
"""Static negative checks for SCF-B-0117; archive/runtime code is never executed."""
from __future__ import annotations
import copy, hashlib, importlib.util, json, tempfile
from pathlib import Path

HERE = Path(__file__).resolve().parent
spec = importlib.util.spec_from_file_location("scf_b_0117_validate", HERE / "validate.py")
...
def run_case(label, expected_code, mutate_rows=None, mutate_inventory=None, mutate_module=None):
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="scf-b-0117-selfcheck-") as tmp:
            root = Path(tmp)
...
def run_generator_case(label, expected_code, mutate):
    gs = importlib.util.spec_from_file_location("scf_b_0117_generate_mutation", HERE / "generate.py")
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="scf-b-0117-generator-") as tmp:
...
run_generator_case("generator review spec tamper", "E_PROFILE", lambda g: g.REVIEW_SPECS["upstream-adoption"].update(category="insufficient_basis"))
run_generator_case("generator anchor tamper", "E_SOURCE_ANCHOR", lambda g: g.REVIEW_SPECS["upstream-adoption"].update(marker="export function classifyUpstreamA146Finding"))
run_generator_case("generator L1 tamper", "E_SEMANTIC_REVIEW", lambda g: g.L1_RANGES["HELIX-OS"].__setitem__(0, (22, 26)))
print("SCF-B-0117 selfcheck: PASS negative_cases=37")
