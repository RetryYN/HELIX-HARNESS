#!/usr/bin/env python3
"""Fail-closed static verifier for the Wave42 schema10 candidate."""
...
BATCH = "LEGACY-SEMANTIC-WAVE42-2026-09-22"
BASE = "74f724b4699feca7f9f2e87e939fe6578f75669e"
MAIN_MERGE_PARENTS = [
    "b343360a104d377e545adc404a4151d70eac024e",
    "596560427669779522f85afb0ffc40faaad9ec1f",
]
...
SUPPLEMENTAL_ASSET_IDS = {"LEGACY-ASSET-7F5DBC909D6DCBA27665", "LEGACY-ASSET-787B224EBE62ACB57F52", "LEGACY-ASSET-451D9907D88A929B5E02", "LEGACY-ASSET-4715728481A85D429B84"}
UNITS = [
    "IRUNIT-HIL-NFR-09-HELIX-HARNESS",
    "IRUNIT-HIL-NFR-09-HELIX-OS",
    "IRUNIT-HIL-NFR-11-HELIX-HARNESS",
    "IRUNIT-HIL-NFR-11-HELIX-OS",
    "IRUNIT-HIL-NFR-12-HELIX-HARNESS",
]

SELECTED = {
    "IRUNIT-HIL-NFR-09-HELIX-HARNESS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9", "LEGACY-ASSET-4FC5BCF06EE23819C5D6", "LEGACY-ASSET-25D8367FB27CCF6AACA8"},
    "IRUNIT-HIL-NFR-09-HELIX-OS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9", "LEGACY-ASSET-13362A81144CD6852A5B", "LEGACY-ASSET-20B55B3292190135B82D"},
    "IRUNIT-HIL-NFR-11-HELIX-HARNESS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9", "LEGACY-ASSET-2B787F62211AD9705592"},
    "IRUNIT-HIL-NFR-11-HELIX-OS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9"},
    "IRUNIT-HIL-NFR-12-HELIX-HARNESS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9", "LEGACY-ASSET-CFA0C589C5D817BB5DD8"},
}

QUERIES = {
    "IRUNIT-HIL-NFR-09-HELIX-HARNESS": {"anchors": ["gate", "check", "requirement"], "match_mode": "archive_file_contains_any_utf8_anchor"},
    "IRUNIT-HIL-NFR-09-HELIX-OS": {"anchors": ["source"], "match_mode": "archive_file_contains_all_utf8_anchors"},
    "IRUNIT-HIL-NFR-11-HELIX-HARNESS": {"anchors": ["workflow", "adapter"], "match_mode": "archive_file_contains_all_utf8_anchors"},
    "IRUNIT-HIL-NFR-11-HELIX-OS": {"anchors": ["skip"], "match_mode": "archive_file_contains_all_utf8_anchors"},
    "IRUNIT-HIL-NFR-12-HELIX-HARNESS": {"anchors": ["source"], "match_mode": "archive_file_contains_all_utf8_anchors"},
}
...
MISSING_EVIDENCE_REASONS = {
    ("implementation_source", "IRUNIT-HIL-NFR-11-HELIX-HARNESS"): "crosswalk implementation_source candidates are already consumed by prior waves; no unused direct asset evidence was selected",
    ("design", "IRUNIT-HIL-NFR-11-HELIX-OS"): "crosswalk phase_and_product_candidate_asset_count is zero; no direct asset evidence was selected",
    ("implementation_source", "IRUNIT-HIL-NFR-11-HELIX-OS"): "crosswalk phase_and_product_candidate_asset_count is zero; no direct asset evidence was selected",
    ("implementation_source", "IRUNIT-HIL-NFR-12-HELIX-HARNESS"): "crosswalk implementation_source candidates are already consumed by prior waves; no unused direct asset evidence was selected",
}
...
def verify_role(row: dict) -> None:
    ...
    require(row["review_scope"].startswith("Wave42 schema10"), f"review scope {row['review_id']}")
    ...
    if row["unit_candidate_id"] in {"IRUNIT-HIL-NFR-09-HELIX-HARNESS", "IRUNIT-HIL-NFR-09-HELIX-OS"}:
        require(row["source_connective_fragments"] == ["Linux primaryで全core gateを実行", "adapter contract testで検出する", "OS別logic forkを作らない"], f"shared connective fragments {row['review_id']}")
...
def verify() -> None:
    ...
    require(len(rows) == 11 and meta["record_count"] == 11, "record count")
    require([row["review_id"] for row in rows] == [f"LSRW42-EDGE-{i:03d}" for i in range(1, 12)], "review order")
    ...
    require(meta["source_main_base_revision"] == BASE and meta["wave41_exact_head"] == "5f198e4390d624355028287d04643d750cfe11e5", "source base")
    ...
    require(meta["semantic_link_counts"] == {"confirmed": 5, "rejected": 0, "unresolved": 6}, "semantic counts")
    require(meta["cumulative_reviewed_unit_count"] == 171 and meta["cumulative_reviewed_edge_count"] == 497 and meta["remaining_unit_count"] == 47, "cumulative counts")
    require(meta["output_sha256"] == file_digest(LEDGER), "ledger digest")
    require(meta["semantic_atom_count"] == 11 and meta["composite_unresolved_count"] == 5, "atomization counts")
    ...
    require(inspected == selected_assets | SUPPLEMENTAL_ASSET_IDS and len(inspected) == 11, "inspected asset denominator")
    ...
    require(len(prior_units | set(UNITS)) == 171 and len(prior_edges | current_edges) == 497, "cumulative union")

    by_unit = {unit: [row for row in rows if row["unit_candidate_id"] == unit] for unit in UNITS}
    expected_missing = {("implementation_source", "IRUNIT-HIL-NFR-11-HELIX-HARNESS"), ("design", "IRUNIT-HIL-NFR-11-HELIX-OS"), ("implementation_source", "IRUNIT-HIL-NFR-11-HELIX-OS"), ("implementation_source", "IRUNIT-HIL-NFR-12-HELIX-HARNESS")}
    ...
    for unit in UNITS:
        ...
        for atom in req["covered_requirement_atoms"]:
            ...
            if unit in {"IRUNIT-HIL-NFR-09-HELIX-HARNESS", "IRUNIT-HIL-NFR-09-HELIX-OS"}:
                counterpart = "IRUNIT-HIL-NFR-09-HELIX-OS" if unit.endswith("HARNESS") else "IRUNIT-HIL-NFR-09-HELIX-HARNESS"
                expected_shared = [counterpart] if any(fragment in atom["text"] for fragment in ["Linux primaryで全core gateを実行", "adapter contract testで検出する", "OS別logic forkを作らない"]) else []
    ...
    print("Wave42 static schema10 verification: PASS (source chain, boundary, lineage and negative mutations)")
