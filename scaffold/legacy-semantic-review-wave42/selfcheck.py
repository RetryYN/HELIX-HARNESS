#!/usr/bin/env python3
"""Run Wave42 validation and explicit fail-closed mutations."""
...
spec = importlib.util.spec_from_file_location("wave42_validator", HERE / "validate.py")
...
def rejected(label: str, mutate) -> None:
    ...
    mutate(mutated_rows)
    try:
        with tempfile.TemporaryDirectory(prefix="wave42-selfcheck-") as tmp:
            root = Path(tmp)
...
def rejected_meta(label: str, mutate) -> None:
    ...
    meta = json.loads(original_meta.read_text())
    try:
        with tempfile.TemporaryDirectory(prefix="wave42-meta-selfcheck-") as tmp:
...
def mutate_shared_connection(mutated_rows) -> None:
    shared = next(row for row in mutated_rows if row["unit_candidate_id"] == "IRUNIT-HIL-NFR-09-HELIX-OS")
...
print("Wave42 selfcheck: PASS (validator plus fourteen negative mutations)")
