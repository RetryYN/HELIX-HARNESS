#!/usr/bin/env python3
"""Generate the Wave42 schema10 research-premise ledger.
...
UNIT_SPECS = [
    {
        "unit": "IRUNIT-HIL-NFR-09-HELIX-HARNESS",
        "query": {"anchors": ["gate", "check", "requirement"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-4FC5BCF06EE23819C5D6",
        "implementation_asset": "LEGACY-ASSET-25D8367FB27CCF6AACA8",
        "missing_roles": [],
        "atom_prefix": "NFR09-HARNESS",
        "composite_unresolved_count": 1,
        "shared_with": ["IRUNIT-HIL-NFR-09-HELIX-OS"],
        "shared_fragments": ["Linux primaryで全core gateを実行", "adapter contract testで検出する", "OS別logic forkを作らない"],
        "evidence": {
            "design": ("docs/design/harness/L6-function-design/gate-confirm.md", 1, 45, []),
            "implementation_source": ("src/lint/l1-l2-gap-check.ts", 1, 80, []),
        },
    },
    {
        "unit": "IRUNIT-HIL-NFR-09-HELIX-OS",
        "query": {"anchors": ["source"], "match_mode": "archive_file_contains_all_utf8_anchors"},
        "design_asset": "LEGACY-ASSET-13362A81144CD6852A5B",
        "implementation_asset": "LEGACY-ASSET-20B55B3292190135B82D",
        "missing_roles": [],
        "atom_prefix": "NFR09-OS",
        "composite_unresolved_count": 1,
        "shared_with": ["IRUNIT-HIL-NFR-09-HELIX-HARNESS"],
        "shared_fragments": ["Linux primaryで全core gateを実行", "adapter contract testで検出する", "OS別logic forkを作らない"],
        "evidence": {
            "design": ("docs/design/helix/L5-detail/worker-isolation-broker.md", 1, 45, []),
            "implementation_source": ("src/runtime/ci-execution-telemetry.ts", 1, 60, []),
        },
    },
    {
        "unit": "IRUNIT-HIL-NFR-11-HELIX-HARNESS",
        "query": {"anchors": ["workflow", "adapter"], "match_mode": "archive_file_contains_all_utf8_anchors"},
        "design_asset": "LEGACY-ASSET-2B787F62211AD9705592",
        "implementation_asset": None,
        "missing_roles": ["implementation_source"],
        "atom_prefix": "NFR11-HARNESS",
        "composite_unresolved_count": 1,
        "evidence": {
            "design": ("docs/design/helix/L3-requirements/workflow-classification-terminal-fullback-authority.v1.json", 1, 32, []),
        },
    },
    {
        "unit": "IRUNIT-HIL-NFR-11-HELIX-OS",
        "query": {"anchors": ["skip"], "match_mode": "archive_file_contains_all_utf8_anchors"},
        "design_asset": None,
        "implementation_asset": None,
        "missing_roles": ["design", "implementation_source"],
        "atom_prefix": "NFR11-OS",
        "composite_unresolved_count": 1,
        "evidence": {},
    },
    {
        "unit": "IRUNIT-HIL-NFR-12-HELIX-HARNESS",
        "query": {"anchors": ["source"], "match_mode": "archive_file_contains_all_utf8_anchors"},
        "design_asset": "LEGACY-ASSET-CFA0C589C5D817BB5DD8",
        "implementation_asset": None,
        "missing_roles": ["implementation_source"],
        "atom_prefix": "NFR12-HARNESS",
        "composite_unresolved_count": 1,
        "evidence": {
            "design": ("docs/design/helix/L6-function-design/review-db-candidate-tree-binding.md", 1, 36, []),
        },
    },
]
...
REVIEW_SCOPE = "Wave42 schema10 research-premise candidate: exact requirement source / product boundary / phase candidate / catalog asset role"
...
def build_evidence_ref(role: str, source_path: str, start: int, end: int, ordinal: int, relation: str) -> dict:
    ...
    return {
        "archive_path": full,
        "artifact_role": role,
        "evidence_ref_id": f"LEGACY-ASSET-WAVE42-{role}-{ordinal}",
...
def main() -> None:
    ...
    for spec in UNIT_SPECS:
        ...
        for role, asset in asset_by_role:
            ...
            if role == "requirement":
                req_ranges = {"HIL-NFR-09": (4767, 4777), "HIL-NFR-11": (4853, 4863), "HIL-NFR-12": (4896, 4906)}
            ...
            row = {
                ...
                "review_id": f"LSRW42-EDGE-{len(rows) + 1:03d}",
        ...
        if spec.get("missing_roles"):
            ...
            for role in spec["missing_roles"]:
                missing_evidence_receipts.append({
                    ...
                    "reason": ("crosswalk phase_and_product_candidate_asset_count is zero; no direct asset evidence was selected" if pool["phase_and_product_candidate_asset_count"] == 0 else ("crosswalk implementation_source candidates are already consumed by prior waves; no unused direct asset evidence was selected" if role == "implementation_source" and unit in {"IRUNIT-HIL-NFR-11-HELIX-HARNESS", "IRUNIT-HIL-NFR-12-HELIX-HARNESS"} else "crosswalk candidate pool contains no implementation_source asset available for this role; no direct asset evidence was selected")),
                    "phase_candidates": candidate["direct_phase_candidates"],
    ...
    ledger_path = OUT / "legacy-requirement-direct-semantic-review-wave42.jsonl"
    ...
    meta = {
        ...
        "wave42_candidate_parent": BASE,
        "wave42_candidate_ref": "research/legacy-semantic-review-wave42",
        "wave42_candidate_root": BASE,
        "wave42_exact_head": BASE,
    }
    ...
    meta_path = OUT / "legacy-requirement-direct-semantic-review-wave42.meta.json"
