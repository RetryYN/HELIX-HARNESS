...
`independent-source-audit.py`はgenerator/validatorをimportせず、固定BASE disposition、Git object、archive MANIFEST、手動profile spanと出力ledgerを直接照合します。72/72件のsource path/SHA/blob/type/mode/MANIFESTとspan文字列、AICR-06のadapter template物理集合34/34、最新pinned main 496件（#2078統合済み）・pinned #2090 41件とのID/path-SHA重複0を確認し、span digestごとの結果を`independent-source-audit.json`へ記録します。#2092のSCF-B-0144は36件の既存main asset照合であり、`adds_assets_to_main_union=false`、main ID分母の増分0、SCF-B-0145 targetとのID/path-SHA重複0を独立照合します。この独立監査は意味解釈を承認しません。
...
validator/selfcheckは新世代research bundleだけを検査します。旧archiveは`git show` / `git ls-tree`による静的readのみです。mainは`7bed4fcd1f50721592b0ce25a8c5d4ee71220b0b`、#2090は`4b6e1bbf122b03fd3531047290161aced34eefda`へ再baselineしました。#2090の41件classification ledger bytesとID/path/SHA集合は旧pinから同一で、inventory snapshotのみ新HEADのbytesに更新しています。ここでのPASSはこれらの明示固定revisionに対する証拠を再現できることを示し、将来のmainまたは#2090 HEADを自動追随しません。

Draftを提出しreviewを依頼する前に、#2090の意図するexact HEADとmainの比較revisionを人手で再照合します。どちらかが進んでいたら、この固定証拠のPASSを現行性の根拠にせず、提出を止めて新しい依存HEADに対するoverlap/source digestを再baselineし、生成・source audit・検証を行います。再baseline前も固定証拠は元revisionに対する記録として保持します。review依頼、merge、closeは行いません。
