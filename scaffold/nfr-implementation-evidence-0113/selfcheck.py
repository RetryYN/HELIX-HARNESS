#!/usr/bin/env python3
"""SCF-B-0113 validatorの意味ある負例と期待error codeを確認する。"""
...
def run_case(name: str, expected_code: str, mutate, mutate_inventory=None) -> None:
    with tempfile.TemporaryDirectory(prefix=f"scf-b-0113-{name}-") as tmp:
        target = Path(tmp)
...
def main() -> None:
    ...
    def tampered_unit_schema(rows):
        rows[0]["schema"] = "nfr-implementation-evidence-0113/v1/tampered"
    ...
    print("SCF-B-0113 selfcheck: PASS (24 negative cases; expected error codes matched)")
