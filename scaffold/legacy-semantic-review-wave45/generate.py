#!/usr/bin/env python3
"""Generate the Wave45 schema10 research-premise ledger.
...
UNIT_SPECS = [
    {
        "unit": "IRUNIT-HIL-NFR-20-HELIX-HARNESS",
        "query": {"anchors": ["reverse", "artifact", "source"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-C67D3019CB8F51EEF09F",
        "implementation_asset": None,
        "missing_roles": ["implementation_source"], "atom_prefix": "NFR20-HARNESS", "composite_unresolved_count": 1,
        "evidence": {"design": ("docs/design/harness/L8-integration/integration-evidence-boundary.md", 1, 33, [])},
    },
    {
        "unit": "IRUNIT-HIL-NFR-22-HELIX-HARNESS",
        "query": {"anchors": ["coverage", "receipt", "stale", "source"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-97638738D0C48E1FE897",
        "implementation_asset": None,
        "missing_roles": ["implementation_source"], "atom_prefix": "NFR22-HARNESS", "composite_unresolved_count": 1,
        "evidence": {"design": ("docs/design/helix/L4-basic-design/project-hook-authority-boundary.md", 1, 80, [])},
    },
    {
        "unit": "IRUNIT-HIL-NFR-25-HELIX-HARNESS",
        "query": {"anchors": ["domain", "identity", "invariant", "source"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-809B35B3C91567A97AF5",
        "implementation_asset": None,
        "missing_roles": ["implementation_source"], "atom_prefix": "NFR25-HARNESS", "composite_unresolved_count": 1,
        "evidence": {"design": ("docs/design/harness/L5-detailed-design/source-boundary-architecture.md", 1, 71, [])},
    },
    {
        "unit": "IRUNIT-HIL-NFR-31-HELIX-OS",
        "query": {"anchors": ["all-or-nothing", "fault injection", "partial", "source"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": None,
        "implementation_asset": None,
        "missing_roles": ["design", "implementation_source"], "atom_prefix": "NFR31-OS", "composite_unresolved_count": 1,
        "evidence": {},
    },
    {
        "unit": "IRUNIT-HIL-NFR-39-HELIX-OS",
        "query": {"anchors": ["privacy", "security", "sandbox", "source"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": None,
        "implementation_asset": None,
        "missing_roles": ["design", "implementation_source"], "atom_prefix": "NFR39-OS", "composite_unresolved_count": 1,
        "evidence": {},
    },
    {
        "unit": "IRUNIT-HIL-TR-06-HELIX-HARNESS",
        "query": {"anchors": ["dependency", "runtime", "offline", "source"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-AA1944E57B2FEAE69873",
        "implementation_asset": "LEGACY-ASSET-8DDE1228F91519E011E8",
        "missing_roles": [], "atom_prefix": "TR06-HARNESS", "composite_unresolved_count": 1,
        "evidence": {"design": ("docs/design/helix/L6-function-design/impact-ci-recovery.md", 1, 80, []), "implementation_source": ("src/lint/change-impact.ts", 1, 80, [])},
    },
    {
        "unit": "IRUNIT-HIL-TR-11-HELIX-HARNESS",
        "query": {"anchors": ["Bun", "Node", "distribution", "source"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-9FD4DE252D3C27F88915",
        "implementation_asset": "LEGACY-ASSET-88F29580A6D436F27DC4",
        "missing_roles": [], "atom_prefix": "TR11-HARNESS", "composite_unresolved_count": 1,
        "evidence": {"design": ("docs/design/helix/L6-function-design/distribution-lite-consumer-canary.md", 1, 71, []), "implementation_source": ("src/lint/right-arm-verification-strategy.ts", 1, 80, [])},
    },
]
...
REVIEW_SCOPE = "Wave45 schema10 research-premise candidate: exact requirement source / product boundary / phase candidate / catalog asset role"
...
def prior_artifact(wave: int, suffix: str) -> tuple[str, Path]:
    ...
    if wave in {37, 38, 39, 40, 41, 42, 43, 44}:
...
def build_evidence_ref(role: str, source_path: str, start: int, end: int, ordinal: int, relation: str) -> dict:
    ...
    return {
        "archive_path": full,
        "artifact_role": role,
        "evidence_ref_id": f"LEGACY-ASSET-WAVE45-{role}-{ordinal}",
...
def main() -> None:
    ...
    for spec in UNIT_SPECS:
        ...
        for role, asset in asset_by_role:
            ...
            if role == "requirement":
                req_ranges = {
                    "HIL-NFR-20": (5240, 5282), "HIL-NFR-22": (5326, 5368),
                    "HIL-NFR-25": (5455, 5497), "HIL-NFR-31": (5713, 5755),
                    "HIL-NFR-39": (6067, 6114), "HIL-TR-06": (6378, 6420),
                    "HIL-TR-11": (6593, 6635),
                }
            ...
            row = {
                ...
                "review_id": f"LSRW45-EDGE-{len(rows) + 1:03d}",
    ...
    meta = {
        ...
        "wave44_candidate_parent": "30e372e6da1ae79baf2bd51e3ef5943eee74acf7",
        "wave44_candidate_ref": "research/legacy-semantic-review-wave44",
        "wave44_candidate_root": "30e372e6da1ae79baf2bd51e3ef5943eee74acf7",
        "wave44_exact_head": "30e372e6da1ae79baf2bd51e3ef5943eee74acf7",
    }
    # Preserve the historical lineage fields carried by schema10 Wave36.
    previous_meta = json.loads((ROOT / "scaffold/legacy-semantic-review-wave44/legacy-requirement-direct-semantic-review-wave44.meta.json").read_text())
    for key in previous_meta:
        if key.startswith("wave") and key not in meta:
            meta[key] = previous_meta[key]
    meta_path = OUT / "legacy-requirement-direct-semantic-review-wave45.meta.json"
