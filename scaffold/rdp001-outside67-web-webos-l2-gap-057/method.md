...
7. J01対応として、atomの4意味欄を個別record（固定keyset、status、source reference）へ正規化し、regular／compositeのatom全keysetを固定した。inventoryのfindings／unresolved_questions／prohibited_inferenceはF／Q／P id、status、textのrecord配列へ固定し、未知key・順序・statusをnegative caseで閉じた。base gateはlive origin/main同値ではなく、記録base commitからexact HEADへのancestor関係を検査する。
