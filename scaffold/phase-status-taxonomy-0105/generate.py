...
TAXONOMY_STATUS = {
    "CROSS_CUTTING_PHASE_NA_CANDIDATE": {
        "label": "PHCAP機構を直接要求しない横断制約／phase非適用の研究候補",
...
CLASSIFICATION = {
    ...
    "IRUNIT-HIL-FR-33-HELIX-HARNESS": ("M-CROSS-CONSTRAINT", "CROSS_CUTTING_PHASE_NA_CANDIDATE", "active surfaceからBun依存を抽出しclassified ledgerを作るcoverage／toolchain gate。CI／releaseの横断条件でありPHCAP-20直接機構ではない。", ["HARNESS phase非適用候補のhuman確認", "active surface／consumer closureとsuccessor assignment"]),
    "IRUNIT-HIL-FR-33-HELIX-OS": ("M-CROSS-CONSTRAINT", "CROSS_CUTTING_PHASE_NA_CANDIDATE", "active Bun countを監査するdependency coverage条件。OSのCI／release境界を補助する横断制約でPHCAP-20直接機構ではない。", ["OS／HARNESS owner境界のhuman確認", "consumer closureとsuccessor assignment"]),
    "IRUNIT-HIL-FR-46-HELIX-OS": ("M-CROSS-CONSTRAINT", "CROSS_CUTTING_PHASE_NA_CANDIDATE", "L1–L12 layer ledgerのnode／edge／authority／gate／revisionを登録するcatalog契約。phaseを実行する機構ではなく、PHCAP-20直接責務もない。", ["layer ledgerとPHCAP capabilityの境界確認", "authority／consumer closureとsuccessor assignment"]),
    "IRUNIT-HIL-FR-52-HELIX-OS": ("M-CROSS-CONSTRAINT", "CROSS_CUTTING_PHASE_NA_CANDIDATE", "Markdown、asset、event ledger、trace、projection、receiptのall-or-nothing更新とCASを定めるcanonicalization transaction。汎用atomicity／authority制約でPHCAP-20直接機構ではない。", ["canonicalizationとmemory／continuationの境界をhuman確認", "write authority／consumer closure"]),
    "IRUNIT-HIL-FR-53-HELIX-OS": ("M-CROSS-CONSTRAINT", "CROSS_CUTTING_PHASE_NA_CANDIDATE", "immutable asset IDとrevision、rename／split／merge／supersedeの履歴を保つidentity契約。履歴はPHCAP-20 retentionの直接責務を意味しない。", ["asset historyとPHCAP-20 retentionの境界確認", "authority／oracle／consumer closure"]),
    "IRUNIT-HIL-NFR-02-HELIX-HARNESS": ("M-CROSS-CONSTRAINT", "CROSS_CUTTING_PHASE_NA_CANDIDATE", "worker、verifier、knowledge promoterの自己承認を分離するrole／authority制約。memory昇格という語はあるが、PHCAP-20機構を要求せず、昇格権限の境界を定める。", ["knowledge promotionとPHCAP-20 ownershipのhuman確認", "role／consumer closureとsuccessor assignment"]),
    "IRUNIT-HIL-NFR-03-HELIX-HARNESS": ("M-CROSS-CONSTRAINT", "CROSS_CUTTING_PHASE_NA_CANDIDATE", "Reverse処理量、phase skip、未完obligation免除を禁止する工程契約。phase運用条件でありPHCAP-20のmemory／handover機構ではない。", ["工程契約としてのphase非適用候補確認", "HARNESS owner／consumer closure"]),
    "IRUNIT-HIL-NFR-05-HELIX-OS": ("M-CROSS-CONSTRAINT", "CROSS_CUTTING_PHASE_NA_CANDIDATE", "untrusted input、実行命令、metadata、evidenceの分離を定めるsecurity／intake境界。raw input intakeの安全制約でありPHCAP-20直接機構ではない。", ["security boundaryのhuman確認", "product／consumer closureとsuccessor assignment"]),
    "IRUNIT-HIL-NFR-06-HELIX-OS": ("M-CROSS-CONSTRAINT", "CROSS_CUTTING_PHASE_NA_CANDIDATE", "認証、認可、決済、PII、secret、license、migration、破壊的操作、外部infraのaction-binding approval条件。横断安全制約でPHCAP-20直接機構ではない。", ["安全境界とphase非適用候補のhuman確認", "authority／consumer closure"]),
    "IRUNIT-HIL-NFR-07-HELIX-OS": ("M-CROSS-CONSTRAINT", "CROSS_CUTTING_PHASE_NA_CANDIDATE", "complexity、public surface、運用負債とminimum-necessary proofを用いるscope gate。拡張抑制の横断制約でPHCAP-20直接機構ではない。", ["scope gateとphase capabilityの境界確認", "authority／consumer closure"]),
    "IRUNIT-HIL-NFR-11-HELIX-OS": ("M-CROSS-CONSTRAINT", "CROSS_CUTTING_PHASE_NA_CANDIDATE", "画面対象／非対象のprototype／skip条件を定めるUI工程制約。画面工程の適用条件でありPHCAP-20直接機構ではない。", ["FR-17／FR-20との重複atomをhuman確認", "product／consumer closure"]),
    "IRUNIT-HIL-NFR-12-HELIX-OS": ("M-CROSS-CONSTRAINT", "CROSS_CUTTING_PHASE_NA_CANDIDATE", "source coverage、path／entry、digest、抽出時点への再現可能なcustodyを要求する完全性条件。PHCAP-20の保持機構を直接要求しない。", ["source custodyとretentionの境界確認", "consumer closureとsuccessor assignment"]),
    "IRUNIT-HIL-NFR-23-HELIX-OS": ("M-CROSS-CONSTRAINT", "CROSS_CUTTING_PHASE_NA_CANDIDATE", "scope derivation graphのacyclic root到達とcycle拒否を要求する構造制約。graph integrityの横断条件でPHCAP-20直接機構ではない。", ["graph constraintとphase capabilityの境界確認", "authority／consumer closure"]),
    ...
    "IRUNIT-HIL-NFR-31-HELIX-OS": ("M-CROSS-CONSTRAINT", "CROSS_CUTTING_PHASE_NA_CANDIDATE", "Authoring正本、ledger、trace、projection、receiptのall-or-nothingとfault後partial state 0件を要求するatomicity制約。PHCAP-20直接機構ではない。", ["atomicityとmemory／continuation境界のhuman確認", "authority／consumer closure"]),
    "IRUNIT-HIL-NFR-32-HELIX-OS": ("M-CROSS-CONSTRAINT", "CROSS_CUTTING_PHASE_NA_CANDIDATE", "意味変更時のauthority、impact、pair、oracle、rollback、stale propagationの同時成立を要求するrevision制約。汎用変更管理でPHCAP-20直接機構ではない。", ["revision／rollbackとretentionの境界確認", "authority／consumer closure"]),
    "IRUNIT-HIL-TR-04-HELIX-HARNESS": ("M-CROSS-CONSTRAINT", "CROSS_CUTTING_PHASE_NA_CANDIDATE", "Linux、macOS、Windowsのportable／compatibility profileを定めるruntime portability条件。環境制約でPHCAP-20直接機構ではない。", ["HARNESS portabilityとしてのhuman確認", "product／consumer closure"]),
    "IRUNIT-HIL-TR-04-HELIX-OS": ("M-CROSS-CONSTRAINT", "CROSS_CUTTING_PHASE_NA_CANDIDATE", "Linux、macOS、Windowsのportable／compatibility profileを定めるOS運転環境条件。環境制約でPHCAP-20直接機構ではない。", ["OS portability／authority境界のhuman確認", "consumer closureとsuccessor assignment"]),
    ...
    "IRUNIT-HIL-TR-08-HELIX-HARNESS": ("M-CROSS-CONSTRAINT", "CROSS_CUTTING_PHASE_NA_CANDIDATE", "Node↔Pythonのversioned JSON Lines IPC envelope、stdout／stderr、sequence、deadline、payload digestを定めるtransport contract。PHCAP-20直接機構ではない。", ["HARNESS transport contractとしてのhuman確認", "product／consumer closureとsuccessor assignment"]),
}

MATRIX = {
    "M-CROSS-CONSTRAINT": {
        "status": "CROSS_CUTTING_PHASE_NA_CANDIDATE",
        "exclusive_group": "phase-status-primary",
        "rule": "原文の主体がprotocol、security、custody、gate、portability、atomicity、identity、工程制約のいずれかで、PHCAP-20のmemory／continuation／handover／retention責務を直接要求しない場合は、phase非適用を研究候補として記録する。",
        "required_evidence": ["exact source anchor", "Wave edge is candidate-only or contract-only", "no direct PHCAP-20 responsibility"],
        "next_decision": "人間のphase非適用確認、product／authority／consumer closure",
    },
...
def make_bundle() -> None:
    ...
    print(f"generated SCF-B-0105 bundle: units={len(units)} cross_cutting={inventory['taxonomy']['status_counts']['CROSS_CUTTING_PHASE_NA_CANDIDATE']} unresolved={inventory['taxonomy']['status_counts']['UNRESOLVED_SOURCE_OR_HUMAN_REVIEW']}")
