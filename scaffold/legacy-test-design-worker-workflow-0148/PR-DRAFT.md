...
- 固定比較snapshot main `bae18e16cb6823ee4234b5a4d0b29aa580d88f2c` の674 unique ID（#2094、#2096、merge済み#2097を含む）を静的比較し、0148対象52件とのID/path/SHA各射影の重複0を再計算。#2097はmainへ一度だけ計上し、別のopen PR集合として加算しない。main HEAD進行を確認した場合は結果をstaleとして扱い、再導出まで新たな比較結果として利用・報告しない。validatorはremote HEADの鮮度を自動取得・保証しない。mainの候補を採択・authorityにしない。
