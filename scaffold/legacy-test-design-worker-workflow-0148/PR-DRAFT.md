...
全52件の製品境界とphase admissionは未決。分類ledgerは固定manifestの全record canonical digestとledger byte digestで固定し、manifest/inventory/Bindingのauthority keyset/valueとupstream digestを静的検証する。PHCAP-07は候補として保持。test-designや引用されたoracleは実装・実行・pass証拠ではない。phase-level degradationは個別asset状態から分離し、consumer closureは未完。
