#!/usr/bin/env python3
"""Negative checks for the PHCAP-15 Deploy final8 static research scaffold."""
...
def main() -> int:
    ...
    if validate(baseline):
        print("FAIL final8 selfcheck baseline")
        return 1
    cases = [
        ...
        ("provenance-fifth12-nested-key", lambda d: d["provenance"]["reviewed_sets"]["fifth12"].update(unexpected_key=True), "KEYSET"),
        ("provenance-sixth12-nested-key", lambda d: d["provenance"]["reviewed_sets"]["sixth12"].update(unexpected_key=True), "KEYSET"),
    ...
    if failed:
        return 1
    print(f"PASS final8 selfcheck: {len(cases)} negative cases rejected")
    return 0
