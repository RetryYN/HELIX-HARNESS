#!/usr/bin/env python3
"""Fail-closed static verifier for the Wave49 schema10 candidate."""
...
LEDGER = HERE / "legacy-requirement-direct-semantic-review-wave49.jsonl"
META = HERE / "legacy-requirement-direct-semantic-review-wave49.meta.json"
...
UNITS = [
    ...
    "IRUNIT-HIL-TR-05-HELIX-HARNESS", "IRUNIT-HIL-TR-05-HELIX-OS",
    "IRUNIT-HIL-TR-06-HELIX-OS", "IRUNIT-HIL-TR-11-HELIX-OS",
]

SELECTED = {
    "IRUNIT-HIL-TR-04-HELIX-HARNESS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9"},
    "IRUNIT-HIL-TR-04-HELIX-OS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9"},
    "IRUNIT-HIL-TR-05-HELIX-HARNESS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9", "LEGACY-ASSET-C92153AA69C31107617D"},
    "IRUNIT-HIL-TR-05-HELIX-OS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9", "LEGACY-ASSET-2C8125EAAADD462E2FDB"},
    "IRUNIT-HIL-TR-06-HELIX-OS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9", "LEGACY-ASSET-D94F2C0530850989B5F4"},
    "IRUNIT-HIL-TR-11-HELIX-OS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9", "LEGACY-ASSET-D888FB040D52499EE09E"},
}

QUERIES = {
    "IRUNIT-HIL-TR-04-HELIX-HARNESS": {"anchors": ["Linux", "Windows", "WSL"], "match_mode": "archive_file_contains_any_utf8_anchor"},
    "IRUNIT-HIL-TR-04-HELIX-OS": {"anchors": ["Linux", "Windows", "WSL"], "match_mode": "archive_file_contains_any_utf8_anchor"},
    "IRUNIT-HIL-TR-05-HELIX-HARNESS": {"anchors": ["CI", "lockfile", "platform"], "match_mode": "archive_file_contains_any_utf8_anchor"},
    "IRUNIT-HIL-TR-05-HELIX-OS": {"anchors": ["adapter", "smoke", "Windows"], "match_mode": "archive_file_contains_any_utf8_anchor"},
    "IRUNIT-HIL-TR-06-HELIX-OS": {"anchors": ["CI", "adapter", "path"], "match_mode": "archive_file_contains_any_utf8_anchor"},
    "IRUNIT-HIL-TR-11-HELIX-OS": {"anchors": ["CI", "path", "read-only"], "match_mode": "archive_file_contains_any_utf8_anchor"},
}
...
MISSING_ROLES = {
    "IRUNIT-HIL-TR-04-HELIX-HARNESS": {"design", "implementation_source"},
    "IRUNIT-HIL-TR-04-HELIX-OS": {"design", "implementation_source"},
    "IRUNIT-HIL-TR-05-HELIX-HARNESS": {"implementation_source"},
    "IRUNIT-HIL-TR-05-HELIX-OS": {"implementation_source"},
    "IRUNIT-HIL-TR-06-HELIX-OS": {"implementation_source"},
    "IRUNIT-HIL-TR-11-HELIX-OS": {"implementation_source"},
}
...
def verify_role(row: dict) -> None:
    ...
    require(row["review_scope"].startswith("Wave49 schema10"), f"review scope {row['review_id']}")
...
def verify() -> None:
    ...
    require([row["review_id"] for row in rows] == [f"LSRW49-EDGE-{i:03d}" for i in range(1, expected_current_edge_count + 1)], "review order")
    ...
    require(meta["source_main_base_revision"] == BASE and meta["wave48_exact_head"] == "3feadfbc3be690bed7cf7b98c41a4ab50d40a5c0", "source base")
    ...
    for unit in UNITS:
        ...
        for atom in req["covered_requirement_atoms"]:
            ...
            if atom["shared_with_units"]:
                require(atom["text"] in req["source_connective_fragments"], f"shared atom connective relation {unit}")
                require(set(atom["shared_with_units"]) <= {
                    other
                    for connection in req["connection_records"]
                    if connection.get("relation") == "shared_source_span"
                    for other in connection.get("shared_with_units", [])
                }, f"shared atom targets {unit}")
            else:
    ...
    print("Wave49 static schema10 verification: PASS (source chain, boundary, lineage and negative mutations)")
