#!/usr/bin/env python3
"""REQATOM A1 0051--0070 validator の意味ある負例 self-check。"""
...
def main() -> int:
    ...
    proposal_negative(
        ...
        lambda rows: rows[5]["candidate_atoms"][0].update(candidate_target="HELIX-Web"),
        "semantic normalized/product/unresolved",
    )
    proposal_negative(
        "negative-relation-drift",
        lambda rows: rows[5]["candidate_atoms"][0]["typed_relation"].update(polarity="retracted"),
        "typed_relation",
    )
    proposal_negative(
        "composite-loss",
        lambda rows: rows[4]["composite_unresolved"].pop(),
