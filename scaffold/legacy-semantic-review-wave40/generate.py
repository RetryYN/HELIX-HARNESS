#!/usr/bin/env python3
"""Generate the Wave40 schema10 research-premise ledger.
...
UNIT_SPECS = [
    {
        "unit": "IRUNIT-HIL-BR-20-HELIX-HARNESS",
        "query": {"anchors": ["evidence", "CI", "quarantine"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-174B63F415197A4B09C5",
        "implementation_asset": "LEGACY-ASSET-112AF1725DFB5F16FB6A",
        "atom_prefix": "BR20-HARNESS",
        "composite_unresolved_count": 1,
        "evidence": {
            "design": ("docs/design/helix/L9-system/system-evidence-index.md", 1, 34, []),
            "implementation_source": ("src/lint/telemetry-closure.ts", 1, 226, []),
        },
    },
    {
        "unit": "IRUNIT-HIL-FR-04-HELIX-OS",
        "query": {"anchors": ["review", "receipt", "process"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-0449CFE6165EF25515B7",
        "implementation_asset": "LEGACY-ASSET-172EBB6DBA696EC454E9",
        "atom_prefix": "FR04-OS",
        "composite_unresolved_count": 1,
        "evidence": {
            "design": ("docs/design/helix/L5-detail/worker-independent-review.md", 1, 66, []),
            "implementation_source": ("src/orchestration/durable-loop-epoch-node.ts", 1, 754, []),
        },
    },
    {
        "unit": "IRUNIT-HIL-FR-33-HELIX-OS",
        "query": {"anchors": ["Bun", "Dependency", "active"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": None,
    ...
    {
        "unit": "IRUNIT-HIL-NFR-03-HELIX-OS",
        "query": {"anchors": ["memory", "slot", "handover"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-09D2A49CAA02AAFAC195",
        "implementation_asset": "LEGACY-ASSET-0F66E2A823A36187D315",
        "atom_prefix": "NFR03-OS",
        "composite_unresolved_count": 1,
        "evidence": {
            "design": ("docs/design/helix/L6-function-design/slot-scheduler-quota-handover.md", 1, 143, []),
            "implementation_source": ("src/runtime/claude-memory-wake.ts", 1, 1644, []),
        },
    },
    {
        "unit": "IRUNIT-HIL-NFR-05-HELIX-OS",
        "query": {"anchors": ["untrusted", "metadata", "evidence"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": None,
        "implementation_asset": None,
        "missing_roles": ["design", "implementation_source"],
        "atom_prefix": "NFR05-OS",
        "composite_unresolved_count": 1,
        "evidence": {},
    },
]
...
REVIEW_SCOPE = "Wave40 schema10 research-premise candidate: exact requirement source / product boundary / phase candidate / catalog asset role"
...
def build_evidence_ref(role: str, source_path: str, start: int, end: int, ordinal: int, relation: str) -> dict:
    ...
    return {
        "archive_path": full,
        "artifact_role": role,
        "evidence_ref_id": f"LEGACY-ASSET-WAVE40-{role}-{ordinal}",
...
def main() -> None:
    ...
    for spec in UNIT_SPECS:
        ...
        for role, asset in asset_by_role:
            ...
            if role == "requirement":
                refs = [
                    build_evidence_ref("requirement", "requirements-ir/requirements.json", {"HIL-BR-20": 821, "HIL-FR-04": 1557, "HIL-FR-33": 2804, "HIL-NFR-03": 4511, "HIL-NFR-05": 4597}[req_id], {"HIL-BR-20": 821, "HIL-FR-04": 1557, "HIL-FR-33": 2804, "HIL-NFR-03": 4511, "HIL-NFR-05": 4597}[req_id], 1, policy["semantic_relation"]),
            ...
            row = {
                ...
                "review_id": f"LSRW40-EDGE-{len(rows) + 1:03d}",
    ...
    ledger_path = OUT / "legacy-requirement-direct-semantic-review-wave40.jsonl"
    ...
    meta = {
        ...
        "consumer_closure_status": "pending",
        "cumulative_reviewed_edge_count": 476,
        "cumulative_reviewed_unit_count": 161,
        ...
        "schema_revision": 10,
        "semantic_link_counts": {"confirmed": 5, "rejected": 0, "unresolved": len(rows) - 5},
        ...
        "wave40_candidate_parent": BASE,
        "wave40_candidate_ref": "research/legacy-semantic-review-wave40",
        "wave40_candidate_root": BASE,
        "wave40_exact_head": BASE,
    }
    ...
    meta_path = OUT / "legacy-requirement-direct-semantic-review-wave40.meta.json"
