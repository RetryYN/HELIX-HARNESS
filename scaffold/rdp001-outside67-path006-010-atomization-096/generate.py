...
def main():
    ...
    inv = {
        ...
        "scope": {
            ...
            "base_drift_policy": "提出前にorigin/mainの進行を確認し、先行#2043またはsource lineageが変わった場合は停止してsource holding・snapshot・current counterpartを再照合し、この束を新baseへrebaselineする。validatorは固定BASEの祖先性と記録digestを検証し、merge後にlive remoteへ遡及しない。",
            "base_drift_observed": False,
            "base_rebaseline_count": 0,
            "stop_condition": "提出前に#2043のreview修正・merge、origin/mainの進行、またはsource lineage変更を確認した場合は、current HEADを固定して停止し再baselineする。merge後のlive origin/main進行は固定BASE検証を壊さない。",
