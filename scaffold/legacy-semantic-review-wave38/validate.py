#!/usr/bin/env python3
"""Fail-closed static verifier for the Wave38 schema10 candidate."""
...
LEDGER = HERE / "legacy-requirement-direct-semantic-review-wave38.jsonl"
META = HERE / "legacy-requirement-direct-semantic-review-wave38.meta.json"
...
UNITS = [
    "IRUNIT-HIL-NFR-02-HELIX-OS",
    "IRUNIT-HIL-NFR-06-HELIX-OS",
    "IRUNIT-HIL-TR-07-HELIX-OS",
]
SELECTED = {
    "IRUNIT-HIL-NFR-02-HELIX-OS": {REQUIREMENT_ASSET, "LEGACY-ASSET-E8B32F8D6519FFCED39A", "LEGACY-ASSET-8ABCBFC4F18A6AD865EB"},
    "IRUNIT-HIL-NFR-06-HELIX-OS": {REQUIREMENT_ASSET, "LEGACY-ASSET-2291163DF1ADDB5E11FD", "LEGACY-ASSET-26391F9FB236CBD4D270"},
    "IRUNIT-HIL-TR-07-HELIX-OS": {REQUIREMENT_ASSET, "LEGACY-ASSET-8771887517A619A2D501", "LEGACY-ASSET-C6C0601C113C6B713D78"},
}
QUERIES = {
    UNITS[0]: {"anchors": ["Codex", "worker", "audit"], "match_mode": "archive_file_contains_any_utf8_anchor"},
    UNITS[1]: {"anchors": ["action-binding", "approval", "PII", "schema"], "match_mode": "archive_file_contains_any_utf8_anchor"},
    UNITS[2]: {"anchors": ["harness.db", "SQLite", "write authority", "Python"], "match_mode": "archive_file_contains_any_utf8_anchor"},
}
ROW_FIELDS = {
...
def verify_role(row: dict) -> None:
    ...
    require(row["review_scope"].startswith("Wave38 schema10"), f"review scope {row['review_id']}")
...
def verify() -> None:
    ...
    require([row["review_id"] for row in rows] == [f"LSRW38-EDGE-{i:03d}" for i in range(1, 10)], "review order")
    ...
    require(meta["source_main_base_revision"] == BASE and meta["wave38_exact_head"] == BASE, "source base")
    ...
    require(meta["cumulative_reviewed_unit_count"] == 151 and meta["cumulative_reviewed_edge_count"] == 450 and meta["remaining_unit_count"] == 67, "cumulative counts")
    require(meta["output_sha256"] == file_digest(LEDGER), "ledger digest")
    require(meta["semantic_atom_count"] == 3 and meta["composite_unresolved_count"] == 3, "atomization counts")
    ...
    require(len(prior_units | set(UNITS)) == 151 and len(prior_edges | current_edges) == 450, "cumulative union")
    ...
    for unit in UNITS:
        ...
        require(len(hold) == 1 and hold[0]["status"] == "product_boundary_shared_atom_hold" and hold[0]["shared_atom_ids"] == [] and hold[0]["composite_unresolved_count"] == 1, f"atomization hold {unit}")
    ...
    print("Wave38 static schema10 verification: PASS (source chain, boundary, lineage and negative mutations)")
