#!/usr/bin/env python3
"""Generate the Wave38 schema10 research-premise ledger.
...
UNIT_SPECS = [
    {
        "unit": "IRUNIT-HIL-NFR-02-HELIX-OS",
        "query": {"anchors": ["Codex", "worker", "audit"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-E8B32F8D6519FFCED39A",
        "implementation_asset": "LEGACY-ASSET-8ABCBFC4F18A6AD865EB",
        "atom_prefix": "NFR02-OS",
        "composite_unresolved_count": 1,
        "evidence": {
            "design": ("docs/design/helix/L6-function-design/worker-independent-review.md", 1, 42, []),
            "implementation_source": ("src/runtime/worker-review-receipt.ts", 1, 203, []),
        },
    },
    {
        "unit": "IRUNIT-HIL-NFR-06-HELIX-OS",
        "query": {"anchors": ["action-binding", "approval", "PII", "schema"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-2291163DF1ADDB5E11FD",
        "implementation_asset": "LEGACY-ASSET-26391F9FB236CBD4D270",
        "atom_prefix": "NFR06-OS",
        "composite_unresolved_count": 1,
        "evidence": {
            "design": ("docs/design/helix/L6-function-design/action-binding-approval-readiness.md", 1, 50, [("NFR06-OS-A01", ["action-binding"], ["approval"], [])]),
            "implementation_source": ("src/lint/action-binding-approval-readiness.ts", 154, 205, [("NFR06-OS-A01", ["action-binding"], ["approval"], [])]),
        },
    },
    {
        "unit": "IRUNIT-HIL-TR-07-HELIX-OS",
        "query": {"anchors": ["harness.db", "SQLite", "write authority", "Python"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-8771887517A619A2D501",
        "implementation_asset": "LEGACY-ASSET-C6C0601C113C6B713D78",
        "atom_prefix": "TR07-OS",
        "composite_unresolved_count": 1,
        "evidence": {
            "design": ("docs/adr/ADR-007-harness-db-sqlite-projection.md", 1, 39, []),
            "implementation_source": ("src/state-db/index.ts", 1, 5, [("TR07-OS-A01", ["Python"], ["transaction"], [])]),
        },
    },
]
...
REVIEW_SCOPE = "Wave38 schema10 research-premise candidate: exact requirement source / product boundary / phase candidate / catalog asset role"
...
def build_evidence_ref(role: str, source_path: str, start: int, end: int, ordinal: int, relation: str) -> dict:
    ...
    return {
        "archive_path": full,
        "artifact_role": role,
        "evidence_ref_id": f"LEGACY-ASSET-WAVE38-{role}-{ordinal}",
...
def main() -> None:
    ...
    for spec in UNIT_SPECS:
        ...
        holds.append({"unit_candidate_id": unit, "status": "product_boundary_shared_atom_hold", "shared_atom_ids": [], "composite_unresolved_count": spec["composite_unresolved_count"]})
        atom_specs = []
        ...
        for role, asset in asset_by_role:
            ...
            if role == "requirement":
                refs = [
                    build_evidence_ref("requirement", "requirements-ir/requirements.json", {"HIL-NFR-02": 4475, "HIL-NFR-06": 4647, "HIL-TR-07": 6430}[req_id], {"HIL-NFR-02": 4475, "HIL-NFR-06": 4647, "HIL-TR-07": 6430}[req_id], 1, policy["semantic_relation"]),
            ...
            row = {
                ...
                "review_id": f"LSRW38-EDGE-{len(rows) + 1:03d}",
    ...
    ledger_path = OUT / "legacy-requirement-direct-semantic-review-wave38.jsonl"
    ...
    meta = {
        ...
        "consumer_closure_status": "pending",
        "cumulative_reviewed_edge_count": 450,
        "cumulative_reviewed_unit_count": 151,
        ...
        "wave38_candidate_parent": BASE,
        "wave38_candidate_ref": "research/legacy-semantic-review-wave38",
        "wave38_candidate_root": BASE,
        "wave38_exact_head": BASE,
    }
    ...
    meta_path = OUT / "legacy-requirement-direct-semantic-review-wave38.meta.json"
