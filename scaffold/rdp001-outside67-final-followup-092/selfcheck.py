#!/usr/bin/env python3
"""Negative selfcheck for the final static outside67 bundle."""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="outside67-residual-090-selfcheck-") as temp:
        ...
        case("source-diff", lambda root: mutate_json(root / (REL + "source-diffs.json"), lambda value: value["items"][1].__setitem__("status", "different")), "E_DIFF:OUTSIDE67-PATH-049")
        ...
        case("snapshot", lambda root: (root / (REL + "source-snapshots/OUTSIDE67-PATH-048/pre-isolation.md")).write_text("tampered\n", encoding="utf-8"), "E_SNAPSHOT:OUTSIDE67-PATH-048:pre_isolation")
        ...
        if failed:
            ...
            return 1
    print("PASS outside67 final follow-up selfcheck: baseline + 25 negative cases")
    return 0
