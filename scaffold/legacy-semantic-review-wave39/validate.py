#!/usr/bin/env python3
"""Fail-closed static verifier for the Wave39 schema10 candidate."""
...
LEDGER = HERE / "legacy-requirement-direct-semantic-review-wave39.jsonl"
META = HERE / "legacy-requirement-direct-semantic-review-wave39.meta.json"
...
BATCH = "LEGACY-SEMANTIC-WAVE39-2026-09-22"
BASE = "38d38f47794e70d621403ad8dbeac56518a76766"
MAIN_MERGE_PARENTS = [
    "dbcc0f332704302d2f8181479552e7198f102db7",
    "41a792110329e904c667896568cc6c4e7392c5d6",
]
...
SUPPLEMENTAL_ASSET_IDS = {"LEGACY-ASSET-08488BBF033F37F536AB", "LEGACY-ASSET-50A93B0E753DC3840E03", "LEGACY-ASSET-F15959C1E9E9256ED64B"}
UNITS = [
    "IRUNIT-HIL-NFR-14-HELIX-OS",
    "IRUNIT-HIL-NFR-21-HELIX-OS",
    "IRUNIT-HIL-TR-01-HELIX-OS",
    "IRUNIT-HIL-TR-08-HELIX-HARNESS",
    "IRUNIT-HIL-TR-08-HELIX-OS",
]
SELECTED = {
    "IRUNIT-HIL-NFR-14-HELIX-OS": {REQUIREMENT_ASSET, "LEGACY-ASSET-F67008331E92FA0A5773", "LEGACY-ASSET-2E0E6FFB913F2A4DA67B"},
    "IRUNIT-HIL-NFR-21-HELIX-OS": {REQUIREMENT_ASSET, "LEGACY-ASSET-4FED0B46C22941761357", "LEGACY-ASSET-943484807D702E10503B"},
    "IRUNIT-HIL-TR-01-HELIX-OS": {REQUIREMENT_ASSET, "LEGACY-ASSET-1B413588CFF3B1360B49", "LEGACY-ASSET-04C6A8AFA15A1BDF0A1D"},
    "IRUNIT-HIL-TR-08-HELIX-HARNESS": {REQUIREMENT_ASSET, "LEGACY-ASSET-BC2275DCE9BFFCF813C8", "LEGACY-ASSET-AF48B1E70AF60FEEE287"},
    "IRUNIT-HIL-TR-08-HELIX-OS": {REQUIREMENT_ASSET, "LEGACY-ASSET-BC2275DCE9BFFCF813C8", "LEGACY-ASSET-61D85EE372040EBCB68B"},
}

QUERIES = {
    "IRUNIT-HIL-NFR-14-HELIX-OS": {"anchors": ["worker", "Codex"], "match_mode": "archive_file_contains_any_utf8_anchor"},
    "IRUNIT-HIL-NFR-21-HELIX-OS": {"anchors": ["review", "receipt", "PLAN"], "match_mode": "archive_file_contains_any_utf8_anchor"},
    "IRUNIT-HIL-TR-01-HELIX-OS": {"anchors": ["Node", "Bun", "distribution"], "match_mode": "archive_file_contains_any_utf8_anchor"},
    "IRUNIT-HIL-TR-08-HELIX-HARNESS": {"anchors": ["Python", "worker", "review"], "match_mode": "archive_file_contains_any_utf8_anchor"},
    "IRUNIT-HIL-TR-08-HELIX-OS": {"anchors": ["stdio"], "match_mode": "archive_file_contains_any_utf8_anchor"},
}
...
def prior_artifact(wave: int, suffix: str) -> tuple[str, Path]:
    if wave == 37:
        rel = f"scaffold/legacy-semantic-review-wave37/legacy-requirement-direct-semantic-review-wave37.{suffix}"
    elif wave == 38:
        rel = f"scaffold/legacy-semantic-review-wave38/legacy-requirement-direct-semantic-review-wave38.{suffix}"
    else:
...
def verify_role(row: dict) -> None:
    ...
    require(row["review_scope"].startswith("Wave39 schema10"), f"review scope {row['review_id']}")
    ...
    if row["unit_candidate_id"] in {"IRUNIT-HIL-TR-08-HELIX-HARNESS", "IRUNIT-HIL-TR-08-HELIX-OS"}:
        require(row["source_connective_fragments"] == ["child process＋versioned JSON Lines over stdio"], f"TR08 connective {row['review_id']}")
        require(len(row["connection_records"]) == 1 and row["connection_records"][0]["relation"] == "shared_source_span", f"TR08 shared connection {row['review_id']}")
        require(row["connection_records"][0]["source_fragment"] == "child process＋versioned JSON Lines over stdio", f"TR08 shared fragment {row['review_id']}")
        require(row["connection_records"][0]["shared_with_units"] == ["IRUNIT-HIL-TR-08-HELIX-OS" if row["unit_candidate_id"].endswith("HARNESS") else "IRUNIT-HIL-TR-08-HELIX-HARNESS"], f"TR08 shared counterpart {row['review_id']}")
    elif row["unit_candidate_id"] == "IRUNIT-HIL-TR-01-HELIX-OS":
        require(row["connection_records"] == [{"decision_id": "IR-ROUTE-Q1", "status": "unresolved", "reason": "runtime_or_technology_constraint_product_contract_vs_internal_implementation"}], f"TR01 routing hold {row['review_id']}")
        require(row["source_connective_fragments"] == [], f"TR01 connective {row['review_id']}")
    else:
...
def verify() -> None:
    ...
    require(len(rows) == 15 and meta["record_count"] == 15, "record count")
    require([row["review_id"] for row in rows] == [f"LSRW39-EDGE-{i:03d}" for i in range(1, 16)], "review order")
    ...
    require(meta["source_main_base_revision"] == BASE and meta["wave39_exact_head"] == BASE, "source base")
    ...
    require(meta["semantic_link_counts"] == {"confirmed": 5, "rejected": 0, "unresolved": 10}, "semantic counts")
    require(meta["cumulative_reviewed_unit_count"] == 156 and meta["cumulative_reviewed_edge_count"] == 465 and meta["remaining_unit_count"] == 62, "cumulative counts")
    require(meta["output_sha256"] == file_digest(LEDGER), "ledger digest")
    require(meta["semantic_atom_count"] == 7 and meta["composite_unresolved_count"] == 5, "atomization counts")
    ...
    require(inspected == selected_assets | SUPPLEMENTAL_ASSET_IDS and len(inspected) == 13, "inspected asset denominator")
    require(len(meta["supplemental_asset_receipts"]) == 3, "supplemental receipt count")
    ...
    require(len(prior_units | set(UNITS)) == 156 and len(prior_edges | current_edges) == 465, "cumulative union")
    ...
    print("Wave39 static schema10 verification: PASS (source chain, boundary, lineage and negative mutations)")
