#!/usr/bin/env python3
"""Run Wave39 validation and explicit fail-closed mutations."""
...
spec = importlib.util.spec_from_file_location("wave39_validator", HERE / "validate.py")
...
def rejected(label: str, mutate) -> None:
    ...
    mutate(mutated_rows)
    try:
        with tempfile.TemporaryDirectory(prefix="wave39-selfcheck-") as tmp:
            root = Path(tmp)
...
def rejected_meta(label: str, mutate) -> None:
    ...
    meta = json.loads(original_meta.read_text())
    try:
        with tempfile.TemporaryDirectory(prefix="wave39-meta-selfcheck-") as tmp:
...
def mutate_routing_hold(mutated_rows) -> None:
    row = next(row for row in mutated_rows if row["unit_candidate_id"] == "IRUNIT-HIL-TR-01-HELIX-OS")
    row["connection_records"] = []


def mutate_shared_connection(mutated_rows) -> None:
    row = next(row for row in mutated_rows if row["unit_candidate_id"] == "IRUNIT-HIL-TR-08-HELIX-OS")
    row["connection_records"][0]["shared_with_units"] = []
...
rejected("TR01 unresolved routing hold deletion", mutate_routing_hold)
rejected("TR08 shared connection deletion", mutate_shared_connection)
...
print("Wave39 selfcheck: PASS (validator plus eleven negative mutations)")
