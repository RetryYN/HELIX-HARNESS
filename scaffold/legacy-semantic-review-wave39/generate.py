#!/usr/bin/env python3
"""Generate the Wave39 schema10 research-premise ledger.
...
UNIT_SPECS = [
    {
        "unit": "IRUNIT-HIL-NFR-14-HELIX-OS",
        "query": {"anchors": ["worker", "Codex"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-F67008331E92FA0A5773",
        "implementation_asset": "LEGACY-ASSET-2E0E6FFB913F2A4DA67B",
        "atom_prefix": "NFR14-OS",
        "composite_unresolved_count": 1,
        "evidence": {
            "design": ("docs/design/helix/L4-basic-design/worker-isolation-broker.md", 1, 71, [("NFR14-OS-A01", ["worker"], ["worker"], [])]),
            "implementation_source": ("src/runtime/codex-native-worker-policy.ts", 1, 58, []),
        },
    },
    {
        "unit": "IRUNIT-HIL-NFR-21-HELIX-OS",
        "query": {"anchors": ["review", "receipt", "PLAN"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-4FED0B46C22941761357",
        "implementation_asset": "LEGACY-ASSET-943484807D702E10503B",
        "atom_prefix": "NFR21-OS",
        "composite_unresolved_count": 1,
        "evidence": {
            "design": ("docs/design/helix/L6-function-design/review-receipt-plan-binding.md", 1, 56, []),
            "implementation_source": ("src/runtime/review-receipt-plan-binding.ts", 1, 364, []),
        },
    },
    {
        "unit": "IRUNIT-HIL-TR-01-HELIX-OS",
        "query": {"anchors": ["Node", "Bun", "distribution"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-1B413588CFF3B1360B49",
        "implementation_asset": "LEGACY-ASSET-04C6A8AFA15A1BDF0A1D",
        "atom_prefix": "TR01-OS",
        "composite_unresolved_count": 1,
        "evidence": {
            "design": ("docs/adr/ADR-009-node-python-linux-runtime.md", 1, 172, []),
            "implementation_source": ("src/setup/distribution-consumer-node-adapter.ts", 1, 143, []),
        },
        "routing_hold": {"decision_id": "IR-ROUTE-Q1", "status": "unresolved", "reason": "runtime_or_technology_constraint_product_contract_vs_internal_implementation"},
    },
    {
        "unit": "IRUNIT-HIL-TR-08-HELIX-HARNESS",
        "query": {"anchors": ["Python", "worker", "review"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-BC2275DCE9BFFCF813C8",
        "implementation_asset": "LEGACY-ASSET-AF48B1E70AF60FEEE287",
        "atom_prefix": "TR08-HARNESS",
        "composite_unresolved_count": 1,
        "evidence": {
            "design": ("docs/design/helix/L5-detail/python-worker-runtime.md", 1, 314, []),
            "implementation_source": ("src/lint/review-evidence.ts", 1, 1183, []),
        },
        "shared_with": ["IRUNIT-HIL-TR-08-HELIX-OS"],
        "shared_fragment": "child process＋versioned JSON Lines over stdio",
        "routing_boundary": "split_required_pending_human_product_boundary_decision",
    },
    {
        "unit": "IRUNIT-HIL-TR-08-HELIX-OS",
        "query": {"anchors": ["stdio"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-BC2275DCE9BFFCF813C8",
        "implementation_asset": "LEGACY-ASSET-61D85EE372040EBCB68B",
        "atom_prefix": "TR08-OS",
        "composite_unresolved_count": 1,
        "evidence": {
            "design": ("docs/design/helix/L5-detail/python-worker-runtime.md", 1, 314, []),
            "implementation_source": ("src/lint/legacy-orchestration-semantic-consumers.ts", 1, 623, []),
        },
        "shared_with": ["IRUNIT-HIL-TR-08-HELIX-HARNESS"],
        "shared_fragment": "child process＋versioned JSON Lines over stdio",
        "routing_boundary": "split_required_pending_human_product_boundary_decision",
    },
]
...
REVIEW_SCOPE = "Wave39 schema10 research-premise candidate: exact requirement source / product boundary / phase candidate / catalog asset role"
...
def prior_artifact(wave: int, suffix: str) -> tuple[str, Path]:
    """Return the checked-in path for a prior wave, including merged Wave37."""
    if wave == 37:
...
def build_evidence_ref(role: str, source_path: str, start: int, end: int, ordinal: int, relation: str) -> dict:
    ...
    return {
        "archive_path": full,
        "artifact_role": role,
        "evidence_ref_id": f"LEGACY-ASSET-WAVE39-{role}-{ordinal}",
...
def main() -> None:
    ...
    for spec in UNIT_SPECS:
        ...
        if spec.get("shared_with"):
            for atom in atom_specs:
                if spec.get("shared_fragment") in atom["text"]:
        ...
        for role, asset in asset_by_role:
            ...
            if role == "requirement":
                refs = [
                    build_evidence_ref("requirement", "requirements-ir/requirements.json", {"HIL-NFR-14": 4984, "HIL-NFR-21": 5285, "HIL-TR-01": 6165, "HIL-TR-08": 6466}[req_id], {"HIL-NFR-14": 4984, "HIL-NFR-21": 5285, "HIL-TR-01": 6165, "HIL-TR-08": 6466}[req_id], 1, policy["semantic_relation"]),
            ...
            row = {
                ...
                "connection_records": ([{
                    ...
                    "source_fragment": spec.get("shared_fragment"),
                ...
                "review_id": f"LSRW39-EDGE-{len(rows) + 1:03d}",
                ...
                "source_connective_fragments": ([spec["shared_fragment"]] if spec.get("shared_fragment") else []),
    ...
    ledger_path = OUT / "legacy-requirement-direct-semantic-review-wave39.jsonl"
    ...
    meta = {
        ...
        "consumer_closure_status": "pending",
        "cumulative_reviewed_edge_count": 465,
        "cumulative_reviewed_unit_count": 156,
        ...
        "schema_revision": 10,
        "semantic_link_counts": {"confirmed": 5, "rejected": 0, "unresolved": 10},
        ...
        "wave39_candidate_parent": BASE,
        "wave39_candidate_ref": "research/legacy-semantic-review-wave39",
        "wave39_candidate_root": BASE,
        "wave39_exact_head": BASE,
    }
    ...
    meta_path = OUT / "legacy-requirement-direct-semantic-review-wave39.meta.json"
