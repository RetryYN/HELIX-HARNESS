...
def main() -> int:
    ...
    run_case("inventory equivalence promotion", mutate_inventory=lambda d: d.update({"equivalence_claim": "semantically_equivalent"}), expected_text="inventory equivalence_claimが固定候補境界と不一致")
    run_case("inventory meaning change promotion", mutate_inventory=lambda d: d.update({"meaning_change_applied": True}), expected_text="inventory meaning_change_appliedが固定候補境界と不一致")
    run_case("inventory decision forgery", mutate_inventory=lambda d: d.update({"human_decision_ref": "TAMPERED"}), expected_text="inventory human_decision_refが固定候補境界と不一致")
    run_case("inventory successor forgery", mutate_inventory=lambda d: d.update({"successor_requirement_ids": ["TAMPERED"]}), expected_text="inventory successor_requirement_idsが固定候補境界と不一致")
    run_case("inventory status promotion", mutate_inventory=lambda d: d.update({"status": "approved"}), expected_text="inventory statusが固定候補境界と不一致")
