#!/usr/bin/env python3
"""Fail-closed static verifier for the Wave41 schema10 candidate."""
...
LEDGER = HERE / "legacy-requirement-direct-semantic-review-wave41.jsonl"
META = HERE / "legacy-requirement-direct-semantic-review-wave41.meta.json"
...
BATCH = "LEGACY-SEMANTIC-WAVE41-2026-09-22"
BASE = "f83b277c6e3e9d48e3ce2f2b0faeed7ac2caf50f"
MAIN_MERGE_PARENTS = [
    "95ba7e1813116632fd68ab3dcfc3b52ef18712b1",
    "3cfa8d99650a04330363c435f57b024344b6b172",
]
...
SUPPLEMENTAL_ASSET_IDS = {"LEGACY-ASSET-02319C2481B9E01698D5", "LEGACY-ASSET-9F48ADEEB477DCA54039", "LEGACY-ASSET-614BF1FA7A7310E3BB4A", "LEGACY-ASSET-B3866EECAF22235E9EB5"}
UNITS = [
    "IRUNIT-HIL-NFR-06-HELIX-HARNESS",
    "IRUNIT-HIL-NFR-07-HELIX-HARNESS",
    "IRUNIT-HIL-NFR-07-HELIX-OS",
    "IRUNIT-HIL-NFR-08-HELIX-HARNESS",
    "IRUNIT-HIL-NFR-08-HELIX-OS",
]
SELECTED = {
    "IRUNIT-HIL-NFR-06-HELIX-HARNESS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9", "LEGACY-ASSET-5429AA05B022E9F49B0A"},
    "IRUNIT-HIL-NFR-07-HELIX-HARNESS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9", "LEGACY-ASSET-5429AA05B022E9F49B0A"},
    "IRUNIT-HIL-NFR-07-HELIX-OS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9"},
    "IRUNIT-HIL-NFR-08-HELIX-HARNESS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9", "LEGACY-ASSET-EA8C51BDF69B34E896AA"},
    "IRUNIT-HIL-NFR-08-HELIX-OS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9", "LEGACY-ASSET-A4DD57E39031B1402494", "LEGACY-ASSET-B9FEE35D2F064EEA4784"},
}

QUERIES = {
    "IRUNIT-HIL-NFR-06-HELIX-HARNESS": {"anchors": ["evidence", "gate"], "match_mode": "archive_file_contains_all_utf8_anchors"},
    "IRUNIT-HIL-NFR-07-HELIX-HARNESS": {"anchors": ["evidence", "gate"], "match_mode": "archive_file_contains_all_utf8_anchors"},
    "IRUNIT-HIL-NFR-07-HELIX-OS": {"anchors": ["evidence", "gate"], "match_mode": "archive_file_contains_all_utf8_anchors"},
    "IRUNIT-HIL-NFR-08-HELIX-HARNESS": {"anchors": ["evidence", "oracle"], "match_mode": "archive_file_contains_all_utf8_anchors"},
    "IRUNIT-HIL-NFR-08-HELIX-OS": {"anchors": ["evidence", "gate"], "match_mode": "archive_file_contains_all_utf8_anchors"},
}
...
MISSING_EVIDENCE_REASONS = {
    ("implementation_source", "IRUNIT-HIL-NFR-06-HELIX-HARNESS"): "crosswalk candidate pool contains no implementation_source asset available for this role; no direct asset evidence was selected",
    ("implementation_source", "IRUNIT-HIL-NFR-07-HELIX-HARNESS"): "crosswalk candidate pool contains no implementation_source asset available for this role; no direct asset evidence was selected",
    ("design", "IRUNIT-HIL-NFR-07-HELIX-OS"): "crosswalk phase_and_product_candidate_asset_count is zero; no direct asset evidence was selected",
    ("implementation_source", "IRUNIT-HIL-NFR-07-HELIX-OS"): "crosswalk phase_and_product_candidate_asset_count is zero; no direct asset evidence was selected",
    ("implementation_source", "IRUNIT-HIL-NFR-08-HELIX-HARNESS"): "crosswalk candidate pool contains no implementation_source asset available for this role; no direct asset evidence was selected",
}
...
def verify_role(row: dict) -> None:
    ...
    require(row["review_scope"].startswith("Wave41 schema10"), f"review scope {row['review_id']}")
    ...
    if row["unit_candidate_id"] in {"IRUNIT-HIL-NFR-07-HELIX-HARNESS", "IRUNIT-HIL-NFR-07-HELIX-OS"}:
        require(row["source_connective_fragments"] == ["complexity、public surface、運用負債を測り", "拡張を拒否する"], f"shared connective fragments {row['review_id']}")
...
def verify() -> None:
    ...
    require([row["review_id"] for row in rows] == [f"LSRW41-EDGE-{i:03d}" for i in range(1, 11)], "review order")
    ...
    require(meta["source_main_base_revision"] == BASE and meta["wave40_exact_head"] == "95ba7e1813116632fd68ab3dcfc3b52ef18712b1", "source base")
    ...
    require(meta["semantic_link_counts"] == {"confirmed": 5, "rejected": 0, "unresolved": 5}, "semantic counts")
    require(meta["cumulative_reviewed_unit_count"] == 166 and meta["cumulative_reviewed_edge_count"] == 486 and meta["remaining_unit_count"] == 52, "cumulative counts")
    require(meta["output_sha256"] == file_digest(LEDGER), "ledger digest")
    require(meta["semantic_atom_count"] == 9 and meta["composite_unresolved_count"] == 5, "atomization counts")
    ...
    require(inspected == selected_assets | SUPPLEMENTAL_ASSET_IDS and len(inspected) == 9, "inspected asset denominator")
    ...
    require(len(prior_units | set(UNITS)) == 166 and len(prior_edges | current_edges) == 486, "cumulative union")

    by_unit = {unit: [row for row in rows if row["unit_candidate_id"] == unit] for unit in UNITS}
    expected_missing = {("implementation_source", "IRUNIT-HIL-NFR-06-HELIX-HARNESS"), ("implementation_source", "IRUNIT-HIL-NFR-07-HELIX-HARNESS"), ("design", "IRUNIT-HIL-NFR-07-HELIX-OS"), ("implementation_source", "IRUNIT-HIL-NFR-07-HELIX-OS"), ("implementation_source", "IRUNIT-HIL-NFR-08-HELIX-HARNESS")}
    ...
    for unit in UNITS:
        ...
        if unit in {"IRUNIT-HIL-TR-08-HELIX-HARNESS", "IRUNIT-HIL-TR-08-HELIX-OS"}:
            require(hold[0]["status"] == "split_required_pending_human_product_boundary_decision", f"TR08 split hold {unit}")
        elif unit == "IRUNIT-HIL-TR-01-HELIX-OS":
            require(hold[0]["status"] == "product_boundary_shared_atom_hold" and hold[0]["routing_hold"]["decision_id"] == "IR-ROUTE-Q1", f"TR01 routing hold {unit}")
        else:
        ...
        for atom in req["covered_requirement_atoms"]:
            ...
            if unit in {"IRUNIT-HIL-NFR-07-HELIX-HARNESS", "IRUNIT-HIL-NFR-07-HELIX-OS"}:
                counterpart = "IRUNIT-HIL-NFR-07-HELIX-OS" if unit.endswith("HARNESS") else "IRUNIT-HIL-NFR-07-HELIX-HARNESS"
                expected_shared = [counterpart] if any(fragment in atom["text"] for fragment in ["complexity、public surface、運用負債を測り", "拡張を拒否する"]) else []
                require(atom["shared_with_units"] == expected_shared, f"shared atom counterpart {unit}")
            if unit in {"IRUNIT-HIL-TR-08-HELIX-HARNESS", "IRUNIT-HIL-TR-08-HELIX-OS"} and "child process＋versioned JSON Lines over stdio" in atom["text"]:
                counterpart = "IRUNIT-HIL-TR-08-HELIX-OS" if unit.endswith("HARNESS") else "IRUNIT-HIL-TR-08-HELIX-HARNESS"
                require(atom["shared_with_units"] == [counterpart], f"shared atom counterpart {unit}")
    ...
    print("Wave41 static schema10 verification: PASS (source chain, boundary, lineage and negative mutations)")
