...
trial("input_main_bundle_stale", inventory(lambda value: value["input_digests"][value["input_digests"].index(next(x for x in value["input_digests"] if "0107/inventory" in x["path"]))].__setitem__("sha256", "sha256:" + "0" * 64)), "E_INPUT_DIGEST")
...
trial("inventory_conditional_overlap_claim_tamper", inventory(lambda value: value["research_union"].__setitem__("target_overlap_conditional_old_union", 0)), "E_RESEARCH_UNION")
