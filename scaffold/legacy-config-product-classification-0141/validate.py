...
"config/drive-route-catalog.json": ("multi_product_conflict", ["HELIX-HARNESS", "HELIX-OS", "HELIX-Web", "HELIX-Web-OS"], 2, 22, "drive model routes, route allocation, and compatibility route inventory", ["Route allocation spans Harness workflow contracts, OS execution, Web interaction, and Web-OS integration; the four L1s are all concrete candidates."]),
...
"config/plan-legacy-workflow-identity-inventory.json": ("multi_product_conflict", ["HELIX-HARNESS", "HELIX-OS", "HELIX-Web", "HELIX-Web-OS"], 2, 35, "compatibility workflow identity entries spanning legacy plan, route, and product surfaces", ["Workflow identity is a Harness contract, execution is OS, user route is Web, and integration is Web-OS; all four boundaries appear in the span."]),
...
"config/universal-improvement-source-registry.v1.json": ("multi_product_conflict", ["HELIX-HARNESS", "HELIX-OS", "HELIX-Web", "HELIX-Web-OS"], 2, 85, "universal improvement requirements authority, IDs, acceptance cases, and source registry contracts", ["The registry spans requirement meaning, improvement execution, Web-facing evidence, and Web-OS projection; all four L1s are present."]),
...
"config/workflow-classification-catalog.v1.json": ("multi_product_conflict", ["HELIX-HARNESS", "HELIX-OS", "HELIX-Web", "HELIX-Web-OS"], 2, 30, "generated workflow classification projection with typed identity and fail-close policy", ["Workflow meaning is Harness, execution is OS, user route is Web, and integrated projection is Web-OS; the projection intentionally crosses all four."]),
...
def verify_record(row: dict, asset: dict, phase: dict, disp_line: int, disp: dict, decisions: list[tuple[int, dict]], read_after: list[tuple[int, dict]]) -> None:
    ...
    bootstrap = sorted(set(phase.get("candidate_product_targets") or [])); alignment = "match" if bootstrap == sorted(products) else "diverges_to_manual_semantic_span" if bootstrap and products else "manual_span_found_without_bootstrap_candidate" if products else "both_insufficient"
...
def verify() -> None:
    ...
    if dict(counts) != {"direct_product_basis": 26, "multi_product_conflict": 13, "insufficient_basis": 2} or inv["classification_counts"] != dict(counts): fail("E_CATEGORY_PARTITION", f"counts={dict(counts)}")
    ...
    print("SCF-B-0141 validate PASS records=41 counts={'direct_product_basis': 26, 'multi_product_conflict': 13, 'insufficient_basis': 2} target_edges=1 authoritative_union=429 conditional_projection=496/537")
