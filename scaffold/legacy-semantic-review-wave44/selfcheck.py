#!/usr/bin/env python3
"""Run Wave44 validation and explicit fail-closed mutations."""
...
spec = importlib.util.spec_from_file_location("wave44_validator", HERE / "validate.py")
...
def rejected(label: str, mutate) -> None:
    ...
    mutate(mutated_rows)
    try:
        with tempfile.TemporaryDirectory(prefix="wave44-selfcheck-") as tmp:
            root = Path(tmp)
...
def rejected_meta(label: str, mutate) -> None:
    ...
    meta = json.loads(original_meta.read_text())
    try:
        with tempfile.TemporaryDirectory(prefix="wave44-meta-selfcheck-") as tmp:
...
def rejected_document(label: str, module_attr: str, mutate) -> None:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="wave44-document-selfcheck-") as tmp:
...
def mutate_shared_connection(mutated_rows) -> None:
    ...
    row["connection_records"] = [{
        ...
        "shared_with_units": ["IRUNIT-HIL-NFR-34-HELIX-OS"],
...
def mutate_selected_role_asset_count(meta) -> None:
    meta["selected_role_asset_count"] = 20


def mutate_plan_count(plan) -> None:
    plan["counts"]["selected_role_assets"] = 20
...
def mutate_prior_cumulative_counts(meta) -> None:
    meta["cumulative_reviewed_unit_count"] = 176
    meta["cumulative_reviewed_edge_count"] = 509
