...
`capture_head`は調査時点のmain来歴であり、検証器はPRのHEADがその子孫であることを確認する。調査時点と同じHEADであることは要求しない。
...
監査結果は [report.json](report.json) にある。8文書の旧原文は合計137,328 bytes、1,506 linesで、archive blob/SHA照合は8/8、現行同pathは0/8。行数は終端改行を余分な行として数えず、archive blobの`splitlines()`と一致させた。asset dispositionは8件すべて `historical / unresolved / unknown / unreviewed`、decision recordなし、consumer_refsなしである。語彙件数は意味上のfailure・consumer・decision・実装完了を確定しない。
