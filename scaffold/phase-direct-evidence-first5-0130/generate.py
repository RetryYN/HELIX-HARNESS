...
def build_bundle() -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    inventory = {
        ...
        "input_snapshot": [{"path": path, "sha256": digest(base_bytes(path), prefix=False)} for path in INPUT_PATHS], "counts": {"units": 5, "helix_os_units": 4, "helix_harness_units": 1, "semantic_review_edges": len(all_edges), "unique_old_assets": len(unique_assets), "decision_records_for_assets": sum(bool(asset["decision_evidence"]["records"]) for row in evidence for asset in row["old_asset_evidence"]["assets"]), "read_after_records_for_assets": sum(bool(asset["read_after_evidence"]["records"]) for row in evidence for asset in row["old_asset_evidence"]["assets"]), "failure_receipts": 0, "consumer_closures": 0},
...
if __name__ == "__main__":
    ...
    print("SCF-B-0130 bundle generated: 5 units, 13 edges, 9 assets")
