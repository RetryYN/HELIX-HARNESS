#!/usr/bin/env python3
"""Build the fixed-base, research-only HELIX-HARNESS FR first-ten evidence slice."""
    ...
    for number, row in read_jsonl(CROSSWALK):
        ...
        if unit.startswith("IRUNIT-HIL-FR-") and unit.endswith("-HELIX-HARNESS") and row.get("product_scope") == ["HELIX-HARNESS"]:
            ...
            if len(result) == 10:
                break
    if len(result) != 10:
        raise RuntimeError(f"expected ten FR HELIX-HARNESS units, got {len(result)}")
    return result
...
def build() -> None:
    ...
    for crosswalk_line, crosswalk in targets:
        ...
        rows.append({
            "schema": "fr-harness-first10-evidence-0132/v1",
