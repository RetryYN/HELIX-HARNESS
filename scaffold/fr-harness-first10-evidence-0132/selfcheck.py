#!/usr/bin/env python3
"""Mutation self-checks for SCF-B-0132 fail-closed boundaries."""
...
def clone() -> Path:
    root = Path(tempfile.mkdtemp(prefix="scf-b-0132-selfcheck-"))
...
def main() -> int:
    ...
    expect("selection product scope tamper", lambda inv, rows: inv["selection"].update(product_scope="HELIX-OS"), "E_SELECTION")
    ...
    expect("source requirement unknown field", lambda inv, rows: rows[0]["source_requirement"].update(fabricated_field=True), "E_SOURCE_REQUIREMENT")
    ...
    expect("current partition boundary tamper", lambda inv, rows: rows[0]["current_evidence_partition"].__setitem__("base_pin_limitation", "fabricated"), "E_CURRENT_PARTITION")
    expect("existing binding field tamper", lambda inv, rows: rows[0]["existing_218_binding"].__setitem__("existing_partition_line", 999), "E_SCOPE")
    expect("Wave edge tamper", lambda inv, rows: rows[0]["semantic_review_edges"][0]["edge"].update(review_id="FAKE-EDGE"), "E_EDGE_SET")
    expect("duplicate Wave edge", lambda inv, rows: rows[0]["semantic_review_edges"].insert(1, copy.deepcopy(rows[0]["semantic_review_edges"][0])), "E_EDGE_SET")
    ...
    expect("asset decision history tamper", lambda inv, rows: rows[0]["asset_evidence"][0]["decision_records"].append({"asset_id": "FAKE-ASSET"}), "E_HISTORY_RECORD")
    expect("asset read-after history tamper", lambda inv, rows: rows[0]["asset_evidence"][0]["read_after_records"].append({"asset_id": "FAKE-ASSET"}), "E_HISTORY_RECORD")
    expect("asset edge refs tamper", lambda inv, rows: rows[0]["asset_evidence"][0]["edge_refs"].append("FAKE-EDGE"), "E_EDGE_SET")
    ...
    expect("asset boundary reason tamper", lambda inv, rows: rows[0]["asset_evidence"][0]["evidence_boundary"].update(reason="fabricated"), "E_STATUS_PROMOTION")
    ...
    expect("old implementation asset set tamper", lambda inv, rows: rows[0]["status_partition"]["old_implementation"].update(asset_ids=["FAKE-ASSET"]), "E_ASSET_SET")
    ...
    expect("consumer closure field promotion", lambda inv, rows: rows[0]["status_partition"]["old_consumer"].update(closure_status="closed"), "E_STATUS_PROMOTION")
    ...
    expect("phase candidate set tamper", lambda inv, rows: rows[0]["status_partition"]["old_phase"].update(phase_ids=["FAKE-PHASE"]), "E_PHASE_RECORD")
    expect("status implementation observation tamper", lambda inv, rows: rows[0]["status_partition"]["old_implementation"].update(asset_level_observations=["fabricated"]), "E_STATUS_PARTITION")
    expect("status degradation observation tamper", lambda inv, rows: rows[0]["status_partition"]["old_degradation"].update(constraint_observations=[{"fabricated": True}]), "E_STATUS_PARTITION")
    expect("status failure receipt tamper", lambda inv, rows: rows[0]["status_partition"]["old_failure"].update(observed_failure_receipts=["FAKE-RECEIPT"]), "E_STATUS_PARTITION")
    expect("status consumer refs tamper", lambda inv, rows: rows[0]["status_partition"]["old_consumer"].update(observed_consumer_refs=["FAKE-CONSUMER"]), "E_STATUS_PARTITION")
    expect("status current partition tamper", lambda inv, rows: rows[0]["status_partition"]["current_implementation"].update(partition_status="implemented"), "E_CURRENT_PARTITION")
    expect("status partition unknown field", lambda inv, rows: rows[0]["status_partition"]["acceptance"].update(fabricated_field=True), "E_STATUS_PARTITION")
    ...
    expect("directly established tamper", lambda inv, rows: rows[0]["directly_established"].append("fabricated"), "E_STATUS_REASON")
    expect("unresolved tamper", lambda inv, rows: rows[0]["unresolved"].append("fabricated"), "E_STATUS_REASON")
    expect("forbidden operations tamper", lambda inv, rows: inv["forbidden_operations"].append("execute-old-runtime"), "E_FORBIDDEN_OPERATION")
    expect("inventory scope product tamper", lambda inv, rows: inv["scope"].update(product="HELIX-OS"), "E_SCOPE")
    expect("inventory selection semantics tamper", lambda inv, rows: inv["selection"].update(candidate_pool_semantics="implementation-proof"), "E_SELECTION")
    expect("inventory authority rule tamper", lambda inv, rows: inv["authority_boundary"].update(status_rule="implemented_when_candidate_exists"), "E_AUTHORITY_BOUNDARY")
    expect("inventory bundle kind tamper", lambda inv, rows: inv.update(bundle_kind="fabricated"), "E_SCHEMA")
    ...
    print("PASS SCF-B-0132 selfcheck: 46 negative cases")
    return 0
