#!/usr/bin/env python3
"""Fail-closed static verifier for the Wave50 schema10 candidate."""
...
LEDGER = HERE / "legacy-requirement-direct-semantic-review-wave50.jsonl"
META = HERE / "legacy-requirement-direct-semantic-review-wave50.meta.json"
...
UNITS = [
    "IRUNIT-HIL-NFR-20-HELIX-OS", "IRUNIT-HIL-NFR-22-HELIX-OS",
]

SELECTED = {
    "IRUNIT-HIL-NFR-20-HELIX-OS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9", "LEGACY-ASSET-614BF1FA7A7310E3BB4A"},
    "IRUNIT-HIL-NFR-22-HELIX-OS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9"},
}

QUERIES = {
    "IRUNIT-HIL-NFR-20-HELIX-OS": {"anchors": ["evidence", "manifest", "source"], "match_mode": "archive_file_contains_any_utf8_anchor"},
    "IRUNIT-HIL-NFR-22-HELIX-OS": {"anchors": ["stale", "child", "coverage"], "match_mode": "archive_file_contains_any_utf8_anchor"},
}
...
MISSING_ROLES = {
    "IRUNIT-HIL-NFR-20-HELIX-OS": {"implementation_source"},
    "IRUNIT-HIL-NFR-22-HELIX-OS": {"design", "implementation_source"},
}
...
def verify_role(row: dict, reviewed_unit_ids: set[str] | None = None) -> None:
    ...
    require(row["review_scope"].startswith("Wave50 schema10"), f"review scope {row['review_id']}")
...
def verify() -> None:
    ...
    require([row["review_id"] for row in rows] == [f"LSRW50-EDGE-{i:03d}" for i in range(1, expected_current_edge_count + 1)], "review order")
    ...
    require(meta["source_main_base_revision"] == BASE and meta["pre_stack_main_revision"] == "dbe43847bfc7f4677c7e5d186b0afb82d38bab3b" and meta["wave49_exact_head"] == "36795d575e379f18eb67e21cc56dde38c4057c63", "source base")
    ...
    print("Wave50 static schema10 verification: PASS (source chain, boundary, lineage and negative mutations)")
