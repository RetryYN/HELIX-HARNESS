#!/usr/bin/env python3
"""Generate the static outside67 017/021/025/026/027 research bundle."""
...
KINDS = {
    "OUTSIDE67-PATH-017": [
        "claude_review_date_boundary",
        "claude_review_verdict_boundary",
        "claude_review_scope_boundary",
        "claude_review_permission_boundary",
        "claude_review_timeout_boundary",
    ],
    "OUTSIDE67-PATH-021": [
        "functional_crosswalk_source_boundary",
        "functional_crosswalk_transaction_boundary",
        "functional_crosswalk_digest_boundary",
        "functional_crosswalk_mapping_boundary",
        "functional_crosswalk_evidence_boundary",
    ],
    "OUTSIDE67-PATH-025": [
        "acceptance_patch_test_boundary",
        "acceptance_patch_digest_boundary",
        "acceptance_patch_statement_boundary",
        "acceptance_patch_replace_boundary",
        "acceptance_patch_freeze_boundary",
    ],
    "OUTSIDE67-PATH-026": [
        "requirements_patch_test_boundary",
        "requirements_patch_digest_boundary",
        "requirements_patch_statement_boundary",
        "requirements_patch_replace_boundary",
        "requirements_patch_freeze_boundary",
    ],
    "OUTSIDE67-PATH-027": [
        "contract_patch_test_boundary",
        "contract_patch_digest_boundary",
        "contract_patch_behavior_boundary",
        "contract_patch_replace_boundary",
        "contract_patch_freeze_boundary",
    ],
}
...
def make() -> None:
    ...
    rows = holding_rows()
    previous = json.loads(
        (ROOT / "scaffold/rdp001-outside67-governance-crosswalk-followup-085/inventory.json").read_text(
            encoding="utf-8"
        )
    )
    existing = previous["scope"]["existing_reviewed_ids"] + previous["scope"]["candidate_ids"]
    assert len(existing) == 52 and not set(existing) & set(IDS)
    ...
    inventory = {
        ...
        "candidate_id": "RDP-001-OUTSIDE67-CONCEPT-FREEZE-FOLLOWUP-017-021-025-026-027",
        ...
        "source_holding": {
            ...
            "candidate_count": 5,
            "reviewed_count": 52,
            "combined_reviewed_and_candidate_count": 57,
            "remaining_after_research": 10,
            "accounting_status": "provisional_57_of_67_no_formal_holding_admission",
        },
        "selection": {
            "candidate_ids": IDS,
            "excluded_existing_ids": existing,
            "reason": "the next ascending residual path_revision_pairs after the SCF-B-0085 52/67 accounting; 017 and 021 preserve the Concept/Infinity source chain, while 025/026/027 preserve the three related L2 freeze patch sources without exact ID/path overlap",
        },
        ...
        "findings": [
            "017/021/025/026/027 cover an independent Claude review source, Infinity functional crosswalk, and three related L2 freeze patch files without formal requirement identity.",
            "017/025/026/027 are byte-equal between pre-isolation and archive; 021 has an archive revision. Current counterparts are all path relocations; 017/025/026/027 match archive bytes and 021 has current content drift.",
...
if __name__ == "__main__":
    make()
    print("generated outside67 017/021/025/026/027 static bundle")
