...
expect_failure("inventory root key injection", lambda x: x.update(unexpected_key=True))
expect_failure("inventory nested key injection", lambda x: x["scope"]["candidate_units"][0].update(unexpected_key=True))
expect_failure("failure unresolved body tamper", lambda x: x["failure_residual"]["unresolved"].append("invented failure"))
expect_failure("consumer unresolved body tamper", lambda x: x["consumer_residual"]["unresolved"].append("invented consumer"))
expect_failure("negative case body tamper", lambda x: x["verification_contract"]["negative_cases"].append("invented negative"))
expect_failure("anchor meaning tamper", lambda x: x["legacy_phase_assessment"]["assets"][0]["source_anchors"][0].update(meaning="invented meaning"))
...
print("PASS PHCAP-15 selfcheck: 41 negative cases")
