...
def validate(bundle, repo):
    ...
    for path,expected in [(HOLDING,HOLDING_SHA),(REGISTER,REGISTER_SHA),("docs/concept/product-boundary.md",PRODUCT_BOUNDARY_SHA),("docs/governance/new-generation-start-here.md",ENTRY_SHA)]+[(p,d) for p,d in L1.values()]:
    ...
    if pins != {"product_boundary":PRODUCT_BOUNDARY_SHA,"web_l1":L1["HELIX-Web"][1],"webos_l1":L1["HELIX-Web-OS"][1],"new_generation_entry":ENTRY_SHA}: fail("E_INPUT_DIGEST","inventory input digest pins changed")
