    ...
    for product in ("HELIX-Web", "HELIX-Web-OS"):
        ...
        for rid in ROW_LINES[product]:
            ...
            candidate_units.append({
                ...
                "current_l1_status": "draft", "current_l1_authority_status": "awaiting_parent_approval",
                ...
                "evidence_basis": [f"{sid}:pre_isolation_sha256", f"{sid}:archive_sha256", f"{product}:current_l1:{rid}:{x['source_path']}:{x['line']}"],
                "judgment_waiting": ["人間によるConcept／L1採択", "旧pathからformal requirementへ接続するrow／phase／implementation evidenceの追加"],
            })
