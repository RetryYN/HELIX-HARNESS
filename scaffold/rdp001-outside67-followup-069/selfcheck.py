...
HERE=Path(__file__).resolve(); TARGET=HERE.parent; VALIDATOR=TARGET/'validate.py'; REL='scaffold/rdp001-outside67-followup-069/'
...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix='outside67-followup-069-selfcheck-') as td:
  ...
  case('duplicate_anchor',lambda c:mutatel(c/(REL+'product-units.jsonl'),1,lambda d:d['source_anchor']['pre_isolation'].__setitem__('line',15)),'E_DUP_ANCHOR')
