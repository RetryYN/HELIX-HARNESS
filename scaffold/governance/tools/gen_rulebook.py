...
def load():
    ...
    for line in text.split("\n"):
        ...
        if m:
            reqs.append({"id": m.group(1), "product": m.group(2), "group": group, "text": None,
                         "n_primary": int(m.group(3)), "n_secondary": int(m.group(4))})
            continue
        if reqs and reqs[-1]["text"] is None and line.strip() and not line.startswith("-") and not line.startswith("#"):
            reqs[-1]["text"] = line.strip()
...
def build(reqs, atoms):
    ...
    for r in reqs:
        ...
        for a in sorted(byp[r["id"]], key=lambda a: a["rule_id"]):
            ...
            o.append("| `%s` | %s | %s | %s | %s |" % (a["rule_id"], esc(a["rule_text"]), a["kind"], "／".join(a["enforced_by"]), esc(src)))
        o.append("\n## 副として対応づいた規則（%d件）\n" % len(bys[r["id"]]))
    ...
    for a in sorted(byp["LEGACY-ONLY"], key=lambda a: a["rule_id"]):
        src = "; ".join("%s:%s" % (s["path"].replace("archive/legacy-generation-2026-09-14/root/", ""), s["lines"]) for s in a["sources"])
        lo.append("| `%s` | %s | %s | %s |" % (a["rule_id"], esc(a["rule_text"]), esc(a.get("legacy_only_reason", "")), esc(src)))
