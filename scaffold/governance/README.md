...
旧HELIXのルール群（規則atom 7,113件、要求候補57本）を、要求1本につき1 fileの仮のルール集として
...
| `rules/<要求ID>.md` | 要求文、主として対応づいた全atom（規則・種類・強制・出どころ）、副として対応づいたatomのID |
...
| `tools/govcheck_selftest.py` | 一時directoryの写しを壊して、govcheckが拒否することを確かめる否定例7件 |
...
1. 台帳の全atomが、ルール集のどれか1 fileにちょうど1回、主として現れる（欠落0、重複0、余分0、置き場違い0）
2. 副の対応づけが該当fileの「副」節にすべて現れる
3. 57本すべてにfileがあり、要求文と主／副の件数が要求候補本文と一致する
4. ルール集が生成器の出力と一致する（手で書き換えていない）
5. ルール集とbindingが記録する正本のsha256が現物と一致する（違えば`stale`）
...
- 正本（`docs/governance/`）を書き換えない。旧資産（`archive/`）を実行・読込しない。
