...
9. ヘリックスコアが保持する製品固有の原本と、利用者へ渡す成果物の正本について、どのような関係を想定していますか。原本を後で解体して汎用パターンだけを残す考えは、どの条件やタイミングを想定していますか。
10. HARNESS改善への利用について、利用者にはどのような説明が必要だと考えていますか。元の設計や個人・組織を特定できる情報の扱いについて、意図している境界はありますか。
...
- 導出物の具体アルゴリズム、順位付けの決定主体、図の意味、ヘリックスコア/BRAIN/OS/Intelligenceの責務、原本・正本・利用条件は未決。
