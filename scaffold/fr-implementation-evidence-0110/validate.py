#!/usr/bin/env python3
"""SCF-B-0110の固定BASE静的証拠bundleをfail-closedで検査する。"""
...
def target_crosswalk() -> list[dict[str, Any]]:
    ...
    if len(rows) != 27 or any(not UNIT_PATTERN.fullmatch(row.get("unit_candidate_id", "")) for row in rows):
        raise ValueError("HIL-FR-21〜40 crosswalk unit集合が不正")
    if len({row["unit_candidate_id"] for row in rows}) != 27:
        raise ValueError("HIL-FR-21〜40 unitが重複している")
    return rows
...
class Validator:
    ...
    def finish(self) -> int:
        if self.errors:
            ...
            return 1
        print("SCF-B-0110 validate: PASS (27 units, 79 review edges, 50 old assets; static-only)")
        return 0
    ...
    def validate(self) -> int:
        ...
        if [row.get("unit_candidate_id") for row in evidence] != units or len(set(row.get("unit_candidate_id") for row in evidence)) != len(units):
            self.error("E_UNIT_SET", "FR21〜40 unit集合の順序・分母・重複が不一致")
