...
HERE=Path(__file__).resolve(); TARGET=HERE.parent; VALIDATOR=TARGET/'validate.py'; REL='scaffold/rdp001-outside67-governance-crosswalk-followup-085/'
...
def main():
 cases=[]
 with tempfile.TemporaryDirectory(prefix='outside67-followup-085-selfcheck-') as td:
  ...
  case('duplicate_anchor',lambda c:mutatel(c/(REL+'product-units.jsonl'),1,lambda d:d['source_anchor']['pre_isolation'].__setitem__('line',7)),'E_DUP_ANCHOR')
  ...
  case('counterpart_relation_table',lambda c:mutate(c/(REL+'evidence-scan.json'),lambda d:d.__setitem__('current_counterpart_relocation_ids',[])),'E_CURRENT_COUNTERPART_RELATION')
  ...
  case('diff_status',lambda c:mutate(c/(REL+'source-diffs.json'),lambda d:d['items'][1].__setitem__('status','same')),'E_DIFF_PROV')
