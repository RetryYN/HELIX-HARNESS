#!/usr/bin/env python3
"""Run Wave47 validation and explicit fail-closed mutations."""
...
spec = importlib.util.spec_from_file_location("wave47_validator", HERE / "validate.py")
...
def rejected(label: str, mutate) -> None:
    ...
    mutate(mutated_rows)
    try:
        with tempfile.TemporaryDirectory(prefix="wave47-selfcheck-") as tmp:
            root = Path(tmp)
...
def rejected_meta(label: str, mutate) -> None:
    ...
    meta = json.loads(original_meta.read_text())
    try:
        with tempfile.TemporaryDirectory(prefix="wave47-meta-selfcheck-") as tmp:
...
def rejected_document(label: str, module_attr: str, mutate) -> None:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="wave47-document-selfcheck-") as tmp:
...
def mutate_pool_consumed_overclaim(meta) -> None:
    reconciliation = next(
        ...
        if item["unit_candidate_id"] == "IRUNIT-HIL-NFR-23-HELIX-HARNESS" and item["role_kind"] == "implementation_source"
    )
...
def mutate_design_reconciliation_omission(meta) -> None:
    receipt = next(
        ...
        if item["unit_candidate_id"] == "IRUNIT-HIL-NFR-23-HELIX-OS" and item["role_kind"] == "design"
    )
...
def reject_wave46_copy_residue() -> None:
    """Reject stale Wave46 template text in human-facing Wave47 artifacts."""
    forbidden = (
        "Wave46 legacy semantic review Scaffold",
        "SCF-B-0086",
        "3cdde5dfedfc51ff7c757a2f5fb2eb11a3c6b64c",
    ...
    for path in (HERE / "README.md", HERE / "plan.json", HERE / "inventory.json"):
        text = path.read_text()
        assert not any(marker in text for marker in forbidden), f"stale Wave46 copy residue: {path}"


reject_wave46_copy_residue()
...
print("Wave47 selfcheck: PASS (validator plus thirty-nine negative mutations)")
