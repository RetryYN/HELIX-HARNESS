#!/usr/bin/env python3
"""Generate the Wave47 schema10 research-premise ledger.
...
UNIT_SPECS = [
    {
        "unit": "IRUNIT-HIL-NFR-23-HELIX-HARNESS",
        "query": {"anchors": ["pair", "mock", "canonical", "screen"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-C4CEF7FCC912FC833714",
        "implementation_asset": None,
        "missing_roles": ["implementation_source"], "atom_prefix": "NFR23-HARNESS", "composite_unresolved_count": 1,
        "evidence": {"design": ("docs/design/harness/L2-screen/README.md", 1, 74, [])},
    },
    {
        "unit": "IRUNIT-HIL-NFR-23-HELIX-OS",
        "query": {"anchors": ["scope", "cycle", "root", "reject"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": None,
        "implementation_asset": None,
        "missing_roles": ["design", "implementation_source"], "atom_prefix": "NFR23-OS", "composite_unresolved_count": 1,
        "evidence": {},
    },
    {
        "unit": "IRUNIT-HIL-NFR-24-HELIX-HARNESS",
        "query": {"anchors": ["refactor", "consumer", "rollback", "semantic"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-ABFB9E11830D370D3141",
        "implementation_asset": "LEGACY-ASSET-DCF5C98796FC654D17C4",
        "missing_roles": [], "atom_prefix": "NFR24-HARNESS", "composite_unresolved_count": 1,
        "evidence": {"design": ("docs/design/helix/L3-requirements/skill-applicability-registry.v1.json", 1, 55, []), "implementation_source": ("src/workflow/contracts.ts", 690, 783, [])},
    },
    {
        "unit": "IRUNIT-HIL-NFR-24-HELIX-OS",
        "query": {"anchors": ["refactor", "consumer", "rollback", "semantic"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-DD7B5EE4399473BA4FF6",
        "implementation_asset": "LEGACY-ASSET-B2FF7D3277DFA9874AA2",
        "missing_roles": [], "atom_prefix": "NFR24-OS", "composite_unresolved_count": 1,
        "evidence": {"design": ("docs/design/helix/L3-requirements/workflow-classification-registry.v1.json", 1, 298, []), "implementation_source": ("src/lint/forward-convergence.ts", 1, 363, [])},
    },
    {
        "unit": "IRUNIT-HIL-NFR-26-HELIX-HARNESS",
        "query": {"anchors": ["obligation", "oracle", "TBD", "coverage"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-55D80AF7AE6E1413BF01",
        "implementation_asset": None,
        "missing_roles": ["implementation_source"], "atom_prefix": "NFR26-HARNESS", "composite_unresolved_count": 1,
        "evidence": {"design": ("docs/design/harness/L6-function-design/review-evidence-stale.md", 1, 48, [])},
    },
    {
        "unit": "IRUNIT-HIL-NFR-26-HELIX-OS",
        "query": {"anchors": ["obligation", "oracle", "TBD", "coverage"], "match_mode": "archive_file_contains_any_utf8_anchor"},
        "design_asset": "LEGACY-ASSET-B3866EECAF22235E9EB5",
        "implementation_asset": None,
        "missing_roles": ["implementation_source"], "atom_prefix": "NFR26-OS", "composite_unresolved_count": 1,
        "evidence": {"design": ("docs/design/helix/L11-uat/uat-evidence-boundary.md", 1, 42, [])},
    },
]
...
REVIEW_SCOPE = "Wave47 schema10 research-premise candidate: exact requirement source / product boundary / phase candidate / catalog asset role"
...
def build_evidence_ref(role: str, source_path: str, start: int, end: int, ordinal: int, relation: str) -> dict:
    ...
    return {
        "archive_path": full,
        "artifact_role": role,
        "evidence_ref_id": f"LEGACY-ASSET-WAVE47-{role}-{ordinal}",
...
def main() -> None:
    ...
    for spec in UNIT_SPECS:
        ...
        for role, asset in asset_by_role:
            ...
            if role == "requirement":
                req_ranges = {
                    "HIL-NFR-23": (5369, 5410),
                    "HIL-NFR-24": (5412, 5453),
                    "HIL-NFR-26": (5498, 5539),
                }
            ...
            row = {
                ...
                "review_id": f"LSRW47-EDGE-{len(rows) + 1:03d}",
    ...
    ledger_path = OUT / "legacy-requirement-direct-semantic-review-wave47.jsonl"
    ...
    meta = {
        ...
        "wave46_candidate_parent": "bd927e917c4a982844de14ac25b64b350f56055c",
        "wave46_candidate_ref": "research/legacy-semantic-review-wave46",
        "wave46_candidate_root": "bd927e917c4a982844de14ac25b64b350f56055c",
        "wave46_exact_head": "bd927e917c4a982844de14ac25b64b350f56055c",
    }
    # Preserve the historical lineage fields carried by schema10 Wave36.
    previous_meta = json.loads((ROOT / "scaffold/legacy-semantic-review-wave46/legacy-requirement-direct-semantic-review-wave46.meta.json").read_text())
    for key in previous_meta:
        if key.startswith("wave") and key not in meta:
            meta[key] = previous_meta[key]
    meta_path = OUT / "legacy-requirement-direct-semantic-review-wave47.meta.json"
