...
PATH011 は既存束 [`rdp001-outside67-web-webos-l2-gap-057`](../rdp001-outside67-web-webos-l2-gap-057) が保持する40 `atomized_candidate`を、atom本文の複製なしに `reused-atom-references.jsonl` のID・canonical JSON digest参照へ移した。frontmatter内とline 13の既存excluded atomは候補参照へ再利用せず、新規 `semantic-atoms.jsonl` は PATH012–015 の185候補だけを含み、二重計上を拒否する。

この束の Scaffold Binding は `SCF-B-0099` である。inventoryにも同じbinding identityを固定し、別束とのbinding衝突をvalidator／selfcheckで拒否する。

pre-isolation と archive revision の各行は、revision差分による対応行を含めて、pre側・archive側それぞれ一度だけ `line-coverage.jsonl` へ置く。冒頭`---`からkey形式のfrontmatterを経て閉じ`---`までを全PATH共通で `metadata_only` とし、等しい非空の説明・要求行は `atomized_candidate`、表の内容行・複数責務・revision差分行は `composite_unresolved` とする。規範内容を metadata へ退避させず、atomized_candidateとatom_idsは双方向に一致させる。

`source-diffs.json` の `status`、`hunks`、`hunk_count`、`unified_diff` は10 source snapshotから再導出し、`meaning_equivalence` は `unresolved` から昇格させない。
