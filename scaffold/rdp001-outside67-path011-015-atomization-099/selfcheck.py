...
def rejected(label: str, mutate) -> None:
    data = {"inv": load("inventory.json"), "selected": load("selected-source-items.jsonl"), "atoms": load("semantic-atoms.jsonl"), "reused": load("reused-atom-references.jsonl"), "coverage": load("line-coverage.jsonl"), "legacy": load("legacy-evidence.jsonl")}
    mutate(data)
    try:
        v.validate(data["inv"], data["selected"], data["atoms"], data["reused"], data["coverage"], data["legacy"])
