...
def holding_rows() -> dict[str, dict]:
    rows = read_jsonl(relocated(HOLDING))
...
def line_category(sid: str, text: str, line_no: int, metadata_lines: set[int]) -> str:
    if line_no in metadata_lines or generic_metadata_line(text):
...
def main() -> None:
    ...
    for sid in SELECTED:
        ...
        diffs[sid] = derive_source_diff(pre, arch)
        pre_metadata = metadata_line_numbers(pre)
        arch_metadata = metadata_line_numbers(arch)
        for pre_no, arch_no, alignment in pair_rows(pre, arch):
            ...
            category = line_category(sid, pre_text, pre_no, pre_metadata) if pre_no and same else ("metadata_only" if ((pre_text is None or pre_no in pre_metadata or generic_metadata_line(pre_text)) and (arch_text is None or arch_no in arch_metadata or generic_metadata_line(arch_text))) else "composite_unresolved")
            atom_ids = []
            if sid == "OUTSIDE67-PATH-011" and pre_no in prior and same:
                for old in prior[pre_no]:
                    if old["atomization_status"] == "atomized_candidate":
                        category = "atomized_candidate"
