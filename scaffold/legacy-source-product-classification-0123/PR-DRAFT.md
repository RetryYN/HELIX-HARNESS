...
- validatorは59件の固定target set、Wave/source overlap、record全top-level keyとnested field、history／failure／consumer、inventoryのtop-level keyと全nested declarationを固定BASEから完全再導出する。カテゴリは direct=候補product 1件＋`reviewed_candidate`＋対応L1、conflict=2件＋`reviewed_conflict`、insufficient=0件＋`reviewed_insufficient_basis`を相互照合する。`failure_consumer_static_refs`はfailure／consumerの厳密2キー、unit候補は厳密3キーとし、asset_id欠落・unit非object・unexpected keyを拒否する。strict JSON loaderはledger／inventoryの重複key・構文不正・非objectを`E_JSON`でfail-closeし、Binding upstreamは全66件の非archive input_digestsをpath／digest一致で閉包する。HELIX-LABO候補は対象製品集合に含めず、旧実装・縮退・consumer closureを候補分類から正式statusへ昇格しない。
...
最新origin/main `ccb27727a60bf13816a542efd1b2abb827b6d522` 上の実測は `scfctl validate: bindings=129 fail=0`、`stale=0`、`residuals=0`。対象59件のrecord、target/overlap集合、旧実装・縮退・consumerの非昇格境界はvalidator/selfcheckで再確認した。
