...
- validatorは59件の固定target set、Wave/source overlap、record全top-level keyとnested field、history／failure／consumer、inventoryのtop-level keyと全nested declarationを固定BASEから完全再導出する。カテゴリは direct=候補product 1件＋`reviewed_candidate`＋対応L1、conflict=2件＋`reviewed_conflict`、insufficient=0件＋`reviewed_insufficient_basis`を相互照合する。`failure_consumer_static_refs`はfailure／consumerの厳密2キー、unit候補は厳密3キーとし、asset_id欠落・unit非object・unexpected keyを拒否する。HELIX-LABO候補は対象製品集合に含めず、旧実装・縮退・consumer closureを候補分類から正式statusへ昇格しない。
...
- `python3 scaffold/legacy-source-product-classification-0123/selfcheck.py`（56負例、期待error code照合）
