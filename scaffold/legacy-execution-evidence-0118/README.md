...
- JSONはtest/suiteの必須count、非負整数、総和整合、正のpass countを満たさない限りverdictを出しません。todo countは未完了としてpendingと同様に`pass_with_pending`へ含め、suite/test countが整合していても完了passへ昇格しません。textのpass summaryは行頭から行末までの構造化行だけを認識し、正のpass countには明示的なexit=0を必須化します。`vitest exit=0`、`Exit code: 0`、`exited with code 0`を認識し、同一行に複数あるexit codeは全件記録し、1つでもnonzeroならverdictなしへ倒します。exit欠落、負値を含むnonzero、正のfailed件数、fatal/error/failureをverdictなしへ倒します。`0 failed`／`0 errors`と単語の一部に埋まった文字列はfailure markerにせず、散文中のpass表現もpassにしません。識別情報だけのreceiptもtest verdictにはしません。
- 今回の再分類は直前生成値から `pass_observed` 24→22、verdictなし3→5、`pass_with_pending` 1件不変です。監査再現のJSON `success=0/fail=2`、JSON count全zero、JSON suite count欠測／不整合、整合したsuite/test todo未完了、text `0 passed/2 failed`、pass summary後のmixed exit（`exit=0`と`exit=2`）、exit欠落、負値exit、`0x1`、散文passはいずれもverdictなしです。CE767は混在markerをverdictなし＋asset-level failure observedとして保存しました。unit-level verdict／acceptance verdictは生成していません。
