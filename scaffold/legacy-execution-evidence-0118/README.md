...
- JSONはtest/suiteの必須count、非負整数、総和整合、正のpass countを満たさない限りverdictを出しません。todo countは未完了としてpendingと同様に`pass_with_pending`へ含め、suite/test countが整合していても完了passへ昇格しません。textのpass summaryは行頭から行末までの構造化行だけを認識し、正のpass countには明示的なexit=0を必須化します。検出語彙は`vitest exit`、`exit`／`exit code`、`exit_code`、`exited with code`、`exited with status`、`returncode`／`return_code`、`rc`です。各語彙の値領域全体を符号付き十進整数として厳格にparseし、値を伴うalias表記を対象にします。`,`／`;`／`|`は次の認識済みmarkerが続く場合に限りmarker間の区切りとして扱い、単独marker値の後ろにある余剰token（例:`0,0x1`、`0|N/A`、`0; N/A`）や`0exit code 1`のような境界不明markerを`unparseable_exit_observations`とreasonへ保持してverdictなしにします。既存receiptの`0 at YYYY-MM-DDTHH:MM:SSZ` timestamp annotationは維持します。parse不能markerは実際のexit値として扱わず、`failure_observation`は別fieldに保ちます。ただしparse不能値に先頭の非ゼロ十進整数がある場合は、曖昧なfailure候補として行をasset-level `failure_observation`へ記録し、unit-level verdictへは昇格しません。正常な複数decimal exitは全件記録し、1件でもnonzeroならverdictなし・asset-level failure observationにします。exit欠落、負値を含むnonzero、正のfailed件数、fatal/error/failureもverdictなしです。`0 failed`／`0 errors`と単語の一部に埋まった文字列はfailure markerにせず、散文中のpass表現もpassにしません。識別情報だけのreceiptもtest verdictにはしません。
- 修正後に固定BASEから再分類した28件は`pass_observed=22`、`pass_with_pending=1`、verdictなし=5、failure observed=1で、以前の確定集計を維持しました。追跡対象28件にparse不能exitはなく、今回の不具合はselfcheckのsynthetic observationで再現しています。JSON failure／欠測／zero total／不整合／todo、text `0 passed/2 failed`、mixed decimal exit、parse不能hex・N/A・空値、exit欠落、負値exit、散文passはいずれもverdictなしです。F1/F2再現では句読点付きnonzero値と境界不明の`0exit code 1`をverdictなし＋asset-level failure observedとして保存し、`exit_code`／`rc`／`returncode`／`exited with status`混在時も0 markerだけでpassにしません。識別子の一部として出た`exit_code`／`rc`／`returncode` pathは正例で確認し、exit markerにしません。CE767は混在markerをverdictなし＋asset-level failure observedとして保存しました。unit-level verdict／acceptance verdictは生成していません。
...
`generate.py`は`common.py`を使い、validatorは独立したoracle実装を持ちます。selfcheckが両者の出力一致を検査し、意味的独立性は主張しません。selfcheckはO01–O50の50観測状態（JSON failure／欠測／zero total／suite不整合／todo、textの厳格summary、複数exit全件保持、hex／N/A／空値／一部parse不能なexit、単独markerの余剰token、正当なcomma／pipe区切りと許可外boundary、timestamp suffix、nonzero候補のfailure記録、4種alias語彙とdigit-adjacent marker、path中aliasの誤検出防止、exit形式／欠落／負値、0 failed／0 errors、FAIL／failed／Segmentation fault、path中のerror語、散文、pending、identity）と、N01–N36の36負例を期待error code付きで検査します。N01–N32は既存の負例を維持し、新たなN33–N36はbool/intおよびfloat/intの型混同をinventoryとnested recordで拒否します。FAIL_REはpath区切り、dot、ハイフンに連結された語をmarkerにせず、bare FAIL／failed／fatal／error／segmentation fault／npm ERR!を維持します。validatorはinventory／record全体をrecursive exact-type比較し、Pythonの`True == 1`および`28.0 == 28`による通過を拒否します。
