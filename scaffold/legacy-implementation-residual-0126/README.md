...
負例IDはvalidatorの`EXPECTED_NEGATIVE_CASES`、inventory、selfcheck実行収集で順序付き完全一致・重複なしを検査します。inventoryの`classification_rule`、`authority_boundary.classification_state`、`binding_id`、`wave_source_paths`、`old_asset_source_mode`、`counts.target_assets`、`phase_candidate_distribution`、`edge_contract.missing_edges_forbidden`、`history_failure_consumer.disposition_rows`も個別に改竄拒否します。BASE ancestry/sourceと固定product research入力の取得失敗、欠損identity rowを同一case内の複数fault injectionで`E_BASE_NOT_ANCESTOR`／`E_BASE_SOURCE`／`E_PRODUCT_RESEARCH_INPUT`へfail-closeします。

record全体とinventoryの各宣言はnested JSON型まで再帰比較します。validatorのrecord/source_exact/inventory field-type schemaは宣言済み全keyを網羅します。inventory宣言型とinput digest行型を利用前に検証し、recordのboundary/source nested valueも型不整合を`E_RECORD_SCHEMA`／`E_SOURCE_ANCHOR`で拒否します。整数fieldは`type(x) is int`、boolean fieldは`type(x) is bool`で扱い、Pythonの`False == 0`／`True == 1`による通過を防ぎます。selfcheckにはrecord `wave_edge_count=false`、inventory `counts.target_wave_edges=false`、非object ledger行、nested nonobject containerの負例を含みます。

変換bytes/sha256と変換規則はrecord provenanceおよびinventory mismatch recordに固定し、validatorは固定BASE Git objectとMANIFESTから独立再計算します。record provenanceとinventory mismatch recordのtranslated byte count／digest改竄はいずれも`E_ARCHIVE_MANIFEST`で拒否します。diagnosticは物理archive bytesもMANIFESTも書き換えず、reuse/admissionの根拠にしません。

validatorは`generate.py`をoracle importせず、固定BASEのgit treeでsource path/type/mode（blobかつ100644/100755）を照合し、全50 Wave入力を再走査してedge598・unique asset355を独立再集計した上で、旧union、最新mainの8 bundle（commit、blob、bytes、SHA、global union429、unresolved union280）、初期120と重複53、新規67、全nested record/inventoryを独立再導出します。record全体とinventory各宣言の再帰比較はJSON scalar型も厳密照合し、すべてのinteger/boolean fieldにbool/int混同を許しません。inventoryの184 input digestのうち63非archive入力に加えて、8つの製品研究bundleをBinding upstreamへ登録し、path/raw SHAを照合します。archive静的121入力はSCF-OS-003のためupstreamへ入れず、inventory/recordで保持して非実行境界を明示します。selfcheckは100 negative cases（31 distinct error codes）を期待error code付きで実行し、BASE/product fail-close、bool/int混同、strict/nonobject JSON、line-ending diagnostic改竄、source anchor/全record、union/overlap分母、Binding upstream、generator分類pin、authority境界を検査します。
