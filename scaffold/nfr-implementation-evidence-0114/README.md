# SCF-B-0114 HIL-NFR-21〜40 implementation evidence scaffold

このbundleは、固定BASE `0d51a994f418450efc40438244f0552e428fc207` におけるHIL-NFR-21〜40の20 source IDから派生した29 product unit（HELIX-OS 18、HELIX-HARNESS 11）、Wave1〜50の598 scan row、61 semantic review edge、旧asset 33件を静的に照合する研究用Scaffoldである。NFR全体は60 unitであり、本束はNFR-21〜40の29 unitだけをscopeとし、NFR-01〜20を混入させない。旧実装source、旧failure／degradation、旧consumer、現行実装、現行受入を別partitionに保持し、直接証拠がないstatusはunknown／pendingと理由、counter-evidence、未解決欄を伴って保留する。

対象成果物は次のとおりである。

- `inventory.json`: 20 source ID／29 unit／61 edge／33 assetの分母、product別unit数、Wave入力、current static refs 261件、BASE祖先性、入力digest、anchor hash provenance、partition／禁止推論／未解決宣言を固定する。
...
## 検証
    ...
    python3 scaffold/nfr-implementation-evidence-0114/build.py
    SCF-B-0114 bundle generated
