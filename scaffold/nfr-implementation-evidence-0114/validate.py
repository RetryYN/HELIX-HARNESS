#!/usr/bin/env python3
"""SCF-B-0114の固定BASE静的証拠bundleをfail-closedで検査する。"""
...
SCHEMA = "nfr-implementation-evidence-0114/v1"
UNIT_SCHEMA = SCHEMA + "/unit"
BINDING_ID = "SCF-B-0114"
BASE_DECLARATION = {
...
SOURCE_IDS = [f"HIL-NFR-{n:02d}" for n in range(21, 41)]
UNIT_PATTERN = re.compile(r"^IRUNIT-HIL-NFR-(2[1-9]|3[0-9]|40)-HELIX-(OS|HARNESS)$")
CONTEXT_INPUTS = [
...
def target_crosswalk() -> list[dict[str, Any]]:
    ...
    if len(rows) != 29 or any(not UNIT_PATTERN.fullmatch(row.get("unit_candidate_id", "")) for row in rows):
        raise ValueError("HIL-NFR-21〜40 crosswalk unit集合が不正")
    if len({row["unit_candidate_id"] for row in rows}) != 29:
        raise ValueError("HIL-NFR-21〜40 unitが重複している")
    return rows
...
class Validator:
    ...
    def finish(self) -> int:
        if self.errors:
            ...
            return 1
        print("SCF-B-0114 validate: PASS (29 units; counts derived from fixed BASE; static-only)")
        return 0
    ...
    def validate(self) -> int:
        ...
        if [row.get("unit_candidate_id") for row in evidence] != units or len(set(row.get("unit_candidate_id") for row in evidence)) != len(units):
            self.error("E_UNIT_SET", "NFR21〜40 unit集合の順序・分母・重複が不一致")
