...
product unit 26件は固定keysetと4意味recordを持ち、inventoryのF/Q/P catalogは7／7／7件で固定しました。source ID／unit ID／line anchor、candidate product、selection reason、phase、scope keysetは独立canonical tableで固定し、4件の追加negative caseで改変を拒否します。validatorは記録base `294bfd90bf58390798733a1437f1c91b8dc7fce8`からexact HEADへのancestorだけを確認し、live origin/mainの同値を要求しません。#2008 merge後のlive origin/main `ce66af3b36727872369a549f01941b26c2b19088`へbranchをrebaseしてmaterializeし、記録baseは再baselineせず静的再照合しました。今回検証幅は5 source／26 unitであり、安全なbatch拡大上限は断定しません。
