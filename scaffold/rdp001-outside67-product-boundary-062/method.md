...
7. #2007検収で固定した境界をこの束にも反映した。product unitの4意味recordとinventoryのF/Q/P catalogを固定keyset・件数・status・順序で検査し、source fragmentのanchor mirrorを検証する。さらにsource ID／unit ID／pre・archive lineを独立canonical tableで固定し、candidate product、selection reason、source path、phase、anchor commit、source_anchor直下keyset、scopeの非digest文字列を改変しても通過しない負例を置いた。
