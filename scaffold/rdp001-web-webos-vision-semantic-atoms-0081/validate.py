...
ORDER=['WEB-ATOM-001','WEB-ATOM-002','WEB-ATOM-003','WEB-ATOM-004','WEB-ATOM-005','WEBOS-ATOM-001','WEBOS-ATOM-002','WEBOS-ATOM-003','WEBOS-ATOM-004','WEBOS-COMPOSITE-001','WEBOS-COMPOSITE-002','WEB-COMPOSITE-001','WEB-COMPOSITE-002','WEBOS-COMPOSITE-003','WEBOS-COMPOSITE-004']
ATOM_EXPECTED={
 ...
 'WEBOS-COMPOSITE-002':('VISION-O04','HELIX-Web-OS','connector_mcp_job_lifecycle','service_runtime',264,264,['、','・','、','・','も'],'composite_unresolved'),
...
def validate(inv,atoms,legacy):
    ...
    if s['semantic_atom_record_count']!=15 or s['atomized_candidate_count']!=9 or s['composite_unresolved_count']!=6 or s['web_user_atom_count']!=7 or s['webos_runtime_atom_count']!=8:fail('E_COUNTS')
    # product counts are records by candidate product: Web 7, Web-OS 8.
    ...
    if len(atoms)!=15 or [x.get('atom_id') for x in atoms]!=ORDER:fail('E_ATOM_ORDER')
    ...
    for a in atoms:
        ...
        if parent not in GROUPS or parent not in parent_by or a['source_path']!=VISION:fail('E_ATOM_SOURCE',aid)
...
def main():
    inv=read_json(HERE/'inventory.json');atoms=read_jsonl(HERE/'semantic-atoms.jsonl');legacy=read_jsonl(HERE/'legacy-links.jsonl');validate(inv,atoms,legacy);print('PASS validate: 15 source-bound candidates (9 atoms + 6 composites), 10/29 parent spans, 6 legacy links')
