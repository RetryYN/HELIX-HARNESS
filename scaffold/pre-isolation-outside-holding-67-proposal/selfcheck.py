...
cases = [
    ...
    ("live roster source digest tamper", lambda i, s, p: i["live_holdings"][0].update(source_atom_set_sha256="0" * 64)),
    ("scope same blob denominator tamper", lambda i, s, p: i["scope"].update(same_blob_count=38)),
    ("scope archive root denominator tamper", lambda i, s, p: i["scope"].update(archive_root_present_count=1)),
    ("findings text tamper", lambda i, s, p: i["findings"].__setitem__(0, "changed")),
    ("verification checks text tamper", lambda i, s, p: i["verification_scope"]["checks"].__setitem__(0, "changed")),
    ("inventory nested keyset tamper", lambda i, s, p: i["source_items"][0].update(unexpected_key=True)),
    ("formal source ref regression", lambda i, s, p: p.update(source_atom_set_ref="scaffold/pre-isolation-outside-holding-67-proposal/source-items.jsonl")),
    ("formal coverage ref regression", lambda i, s, p: p.update(coverage_receipt_ref="scaffold/pre-isolation-outside-holding-67-proposal/inventory.json")),
