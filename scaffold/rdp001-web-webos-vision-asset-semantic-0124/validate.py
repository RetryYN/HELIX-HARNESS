...
L1_COMMIT="cb5a45fea289d61b67cba100fd2406813021ef48"
L1_PATH="scaffold/rdp001-outside67-web-webos-l1-anchor-0121/candidate-units.jsonl"
L1_BLOB_OID="8d034128cc09a3b2c03abcfe31e89f8719ca170b"
L1_SHA="0c00dfa823ac0302e3ab496f9b5c264c18d8239a679ff99bb9c8515744bb64fe"
...
OTHER_DIGESTS={
 ...
 "docs/governance/decisions/concept-v4.1-and-four-l1-approval-2026-09-17.md":"b512098481cb282d066b37383cfcd932ef137e86e604a46f965fc605d52698f2",
}
...
NEGATIVE_CODES=["E_INPUT_DIGEST","E_BASE_COMMIT","E_CANDIDATE_SET","E_SPAN_ANCHOR","E_ASSET_SET","E_ASSET_SOURCE","E_ASSET_HISTORY","E_ASSET_LINK","E_L1_ANCHOR","E_L1_COMMIT_ANCESTOR","E_L1_BLOB","E_PHASE_BOUNDARY","E_FORMAL_BOUNDARY","E_PRODUCT_BOUNDARY","E_MATRIX_CARDINALITY","E_INVENTORY_DECLARATION"]
...
def git_blob(repo,commit,path):
 try:return subprocess.check_output(["git","rev-parse",f"{commit}:{path}"],cwd=repo,text=True).strip()
 except subprocess.CalledProcessError:fail("E_L1_BLOB",f"git blob missing {commit}:{path}")
...
def validate(bundle,repo):
 ...
 if subprocess.run(["git","merge-base","--is-ancestor",L1_COMMIT,"HEAD"],cwd=repo,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL).returncode: fail("E_L1_COMMIT_ANCESTOR","#2073 L1 merge commit is not an ancestor of HEAD")
 if git_blob(repo,L1_COMMIT,L1_PATH)!=L1_BLOB_OID: fail("E_L1_BLOB","#2073 L1 anchor blob OID changed")
 l1_bytes=git_show(repo,L1_COMMIT,L1_PATH)
 if sha_bytes(l1_bytes)!=L1_SHA or l1_bytes!=l1path.read_bytes(): fail("E_INPUT_DIGEST","#2073 L1 anchor snapshot does not match immutable commit/blob digest")
 l1inv=load_json(bundle/"inputs/l1-anchor-0121/inventory.json","E_INPUT_DIGEST")
 if sha_file(bundle/"inputs/l1-anchor-0121/inventory.json")!="03e2a28a25c734e4b8f4dde3d23e2f492c6d08dad3de5d431aebd16f5affdc95": fail("E_INPUT_DIGEST","#2073 inventory digest changed")
 if sha_file(bundle/"inputs/l1-anchor-0121/SCF-B-0121.json")!="09be7ef90f2025321c3af6726d84a319e51f322cca34cbd44fcb08cfcdfd2804": fail("E_INPUT_DIGEST","#2073 binding digest changed")
 ...
 # Current L1 input rows remain exact against current docs and approved-decision boundary.
 l1_rows=load_jsonl(l1path,"E_L1_ANCHOR")
 if len(l1_rows)!=11: fail("E_L1_ANCHOR","L1 anchor row count changed")
 for x in l1_rows:
  ...
  product=x["candidate_product"]
  expected_decision={"HELIX-Web":"HDEC-HELIXWEB-L1-01","HELIX-Web-OS":"HDEC-HELIXWEBOS-L1-01"}.get(product)
  if expected_decision is None or x.get("current_l1_raw_status")!="draft" or x.get("current_l1_raw_authority_status")!="awaiting_parent_approval" or x.get("current_l1_effective_authority_status")!="approved" or x.get("current_l1_effective_decision_id")!=expected_decision or x.get("authority_status")!="none": fail("E_L1_ANCHOR",f"L1 raw/effective authority boundary changed {x['candidate_id']}")
  if "current_l1_status" in x or "current_l1_authority_status" in x or "人間によるConcept／L1採択" in x.get("judgment_waiting",[]): fail("E_L1_ANCHOR",f"stale L1 authority field or adoption wait remains {x['candidate_id']}")
 ...
 if inv.get("negative_case_codes")!=NEGATIVE_CODES or inv.get("negative_case_count")!=18: fail("E_INVENTORY_DECLARATION","negative case declaration changed")
