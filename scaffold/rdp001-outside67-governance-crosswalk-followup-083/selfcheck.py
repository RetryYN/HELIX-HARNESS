...
def main():
 cases=[]
 with tempfile.TemporaryDirectory(prefix='outside67-followup-083-selfcheck-') as td:
  ...
  case('duplicate_anchor',lambda c:mutatel(c/(REL+'product-units.jsonl'),1,lambda d:d['source_anchor']['pre_isolation'].__setitem__('line',8)),'E_DUP_ANCHOR')
  ...
  case('counterpart_hash_relation',lambda c:mutatel(c/(REL+'selected-source-items.jsonl'),0,lambda d:d['archive_counterpart'].__setitem__('content_hash_matches_archive',True)),'E_COUNTERPART')
