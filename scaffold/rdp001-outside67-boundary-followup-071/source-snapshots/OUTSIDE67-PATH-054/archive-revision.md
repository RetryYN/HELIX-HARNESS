...
| 統合順序・統合単位・検証実行計画の導出と再計画 | HELIXOS-L1-010／L2-011 | 独立要求を追加。検証義務の正本はHARNESS |
...
| 検収 | HARNESS契約から統合順序、統合単位、検証実行計画を導出し、実行結果から再計画して収束させる | 検証義務を改変せず、reviewerやCI greenだけで完了を生成しない |
