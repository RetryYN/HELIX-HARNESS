...
`validate.py`は`build.py`をimportせず固定BASEから再導出します。BASE祖先性、canonical input digest、source record集合、217／1分母、598 edge集合、355 asset集合、per-record重複、7 status、required evidence schema、authority noneをfail-closeで検査します。

`selfcheck.py`は次の16改竄を期待error code付きで棄却します。
...
- `validate.py`: PASS
- `selfcheck.py`: PASS（16 negative cases）
- `py_compile`: PASS
- `scfctl validate`: 実行予定
- `scfctl stale`／`residuals`: 実行予定
- `git diff --check`: 実行予定
