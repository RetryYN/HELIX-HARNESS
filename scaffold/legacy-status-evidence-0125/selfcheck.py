...
def main():
 ...
 expect("inventory bundle kind tamper",lambda i,r:i.update(bundle_kind="formal_implementation"),"E_SCHEMA")
 expect("inventory anchor rule tamper",lambda i,r:i["anchor_rule"]["claim_rules"].update({next(iter(i["anchor_rule"]["claim_rules"])):"fabricated"}),"E_CLAIM_ANCHOR")
 expect("inventory search boundary tamper",lambda i,r:i["search_boundaries"].update(all_ledger_rows=0),"E_SCOPE")
 expect("inventory top-level key omission",lambda i,r:i.pop("bundle_kind"),"E_SCHEMA")
 ...
 print("PASS SCF-B-0125 selfcheck: 29 negative cases")
