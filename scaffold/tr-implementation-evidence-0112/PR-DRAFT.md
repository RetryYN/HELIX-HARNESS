...
research: HIL-TR-01-11 16 product unit implementation evidence crosswalk (SCF-B-0112)

## 目的

固定BASE `0d51a994f418450efc40438244f0552e428fc207` を基準に、HIL-TR-01〜11の11 source IDから派生した16 product unit（HELIX-OS 11、HELIX-HARNESS 5）、Wave1〜50、39 semantic review edge、旧asset 23件を静的に照合する。旧実装source、旧failure／degradation、旧consumer、現行実装、現行受入を独立partitionへ分け、直接証拠のないstatusはunknown／pendingと理由、counter-evidence、未解決欄を伴って保持する。
...
- inventoryで11 source ID、16 unit、39 edge、23 old asset、144 current static refs、Wave1〜50の598 scan row、BASE ancestry、input digest、anchor provenance、evidence partitionを固定する。
...
- Binding `SCF-B-0112`へbundle全成果物を登録する。
