#!/usr/bin/env python3
"""SCF-B-0112の固定BASE静的証拠bundleをfail-closedで検査する。"""
...
SCHEMA = "tr-implementation-evidence-0112/v1"
UNIT_SCHEMA = SCHEMA + "/unit"
BINDING_ID = "SCF-B-0112"
BASE_DECLARATION = {
...
SOURCE_IDS = [f"HIL-TR-{n:02d}" for n in range(1, 12)]
UNIT_PATTERN = re.compile(r"^IRUNIT-HIL-TR-(0[1-9]|1[01])-HELIX-(OS|HARNESS)$")
CONTEXT_INPUTS = [
...
def target_crosswalk() -> list[dict[str, Any]]:
    ...
    if len(rows) != 16 or any(not UNIT_PATTERN.fullmatch(row.get("unit_candidate_id", "")) for row in rows):
        raise ValueError("HIL-TR-01〜11 crosswalk unit集合が不正")
    if len({row["unit_candidate_id"] for row in rows}) != 16:
        raise ValueError("HIL-TR-01〜11 unitが重複している")
    return rows
...
class Validator:
    ...
    def finish(self) -> int:
        if self.errors:
            ...
            return 1
        print("SCF-B-0112 validate: PASS (16 units; counts derived from fixed BASE; static-only)")
        return 0
    ...
    def validate(self) -> int:
        ...
        if [row.get("unit_candidate_id") for row in evidence] != units or len(set(row.get("unit_candidate_id") for row in evidence)) != len(units):
            self.error("E_UNIT_SET", "TR01〜11 unit集合の順序・分母・重複が不一致")
