#!/usr/bin/env python3
"""SCF-B-0112 のHIL-TR-01〜11 静的証拠partitionを生成する。
...
def target_crosswalk() -> list[dict[str, Any]]:
    ...
    if len(rows) != 16 or any(not UNIT_PATTERN.fullmatch(row.get("unit_candidate_id", "")) for row in rows):
        raise ValueError("HIL-TR-01〜11の製品unit集合が16件のTR専用集合ではない")
    if any("HIL-BR-" in row.get("unit_candidate_id", "") for row in rows):
        raise ValueError("BR unitがTR bundleへ混入した")
    if len({row["unit_candidate_id"] for row in rows}) != 16:
        raise ValueError("TR unitが重複している")
    return rows
...
def build_bundle() -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    if set(edges_by_unit) != {row["unit_candidate_id"] for row in crosswalk}:
        raise ValueError("TR unitのWave edgeが欠落または余分")
...
if __name__ == "__main__":
    ...
    print("SCF-B-0112 bundle generated")
