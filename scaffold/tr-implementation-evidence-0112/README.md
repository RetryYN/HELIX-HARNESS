# SCF-B-0112 HIL-TR-01〜11 implementation evidence scaffold

このbundleは、固定BASE `0d51a994f418450efc40438244f0552e428fc207` におけるHIL-TR-01〜11の11 source IDから派生した16 product unit（HELIX-OS 11、HELIX-HARNESS 5）、Wave1〜50の598 scan row、39 semantic review edge、旧asset 23件を静的に照合する研究用Scaffoldである。旧実装source、旧failure／degradation、旧consumer、現行実装、現行受入を別partitionに保持し、直接証拠がないstatusはunknown／pendingと理由、counter-evidence、未解決欄を伴って保留する。

対象成果物は次のとおりである。

- `inventory.json`: 11 source ID／16 unit／39 edge／23 assetの分母、product別unit数、Wave入力、current static refs 144件、BASE祖先性、入力digest、anchor hash provenance、partition／禁止推論／未解決宣言を固定する。
...
## 検証
    ...
    python3 scaffold/tr-implementation-evidence-0112/build.py
    SCF-B-0112 bundle generated

    python3 scaffold/tr-implementation-evidence-0112/validate.py
    SCF-B-0112 validate: PASS (16 units; counts derived from fixed BASE; static-only)

    python3 scaffold/tr-implementation-evidence-0112/selfcheck.py
    SCF-B-0112 selfcheck: PASS (24 negative cases; expected error codes matched)
