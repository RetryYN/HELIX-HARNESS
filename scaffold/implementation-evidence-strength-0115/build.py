#!/usr/bin/env python3
"""SCF-B-0115 の固定BASE選定20 unit静的証拠partitionを生成する。
...
BINDING_ID = "SCF-B-0115"
SCHEMA = "implementation-evidence-strength-0115/v1"
...
def _selection_metrics_from_base() -> tuple[list[dict[str, Any]], dict[str, dict[str, Any]]]:
    """固定BASEの全candidate unitから選定を再導出する。

    ここでIRCONNの接続行を除外する理由はID schema境界だけであり、手作業の
    unit集合を信用しない。anchorの解決は重いため、選定スコアではedgeの
    catalog/ledger fieldsだけを使い、最終bundleでは全edgeを完全保存する。
    """
    rows = [
    ...
    edge_by_unit: dict[str, list[dict[str, Any]]] = {row["unit_candidate_id"]: [] for row in rows}
    for wave, path in WAVE_PATHS.items():
        for edge in base_jsonl(path):
            unit = edge.get("unit_candidate_id")
            if unit in edge_by_unit:
                edge_by_unit[unit].append(edge)
    decision_by_asset: dict[str, int] = {}
    read_after_by_asset: dict[str, int] = {}
    for record in decisions:
        aid = record.get("asset_id")
        if aid:
            decision_by_asset[aid] = decision_by_asset.get(aid, 0) + 1
    for record in read_after:
        aid = record.get("asset_id")
        if aid:
            read_after_by_asset[aid] = read_after_by_asset.get(aid, 0) + 1
    metrics: dict[str, dict[str, Any]] = {}
    for row in rows:
        unit = row["unit_candidate_id"]
        edges = edge_by_unit[unit]
        asset_ids = {edge.get("asset_id") for edge in edges if edge.get("asset_id")}
        ...
        failure_count = sum(bool(edge.get("coverage", {}).get("failure")) if isinstance(edge.get("coverage"), dict) else False for edge in edges)
        observed = sum(len(edge.get("observed_consumer_refs", [])) for edge in edges)
        ledger = sum(len(disposition.get(aid, {}).get("consumer_refs", [])) for aid in asset_ids)
        decision_count = sum(decision_by_asset.get(aid, 0) for aid in asset_ids)
        read_after_count = sum(read_after_by_asset.get(aid, 0) for aid in asset_ids)
        score = len(impl) * 5 + len(tests) * 4 + failure_count * 2 + observed + ledger + decision_count + read_after_count
        metrics[unit] = {
            "family": unit.split("-")[2],
            "asset_count": len(asset_ids), "edge_count": len(edges),
            "implementation_source_asset_count": len(impl),
            "test_evidence_asset_count": len(tests), "coverage_failure_count": failure_count,
            "observed_consumer_ref_count": observed, "ledger_consumer_ref_count": ledger,
            "decision_record_count": decision_count, "read_after_record_count": read_after_count,
            "selection_score": score,
        }
    selected = set()
    for family in ("BR", "FR", "TR", "NFR"):
        family_units = [unit for unit in metrics if metrics[unit]["family"] == family]
...
def target_crosswalk() -> list[dict[str, Any]]:
    ...
    if len(rows) != 20 or any(not UNIT_PATTERN.fullmatch(row.get("unit_candidate_id", "")) for row in rows):
        raise ValueError("implementation-strength選定unit集合が20件ではない")
    if len({row["unit_candidate_id"] for row in rows}) != 20:
        raise ValueError("implementation-strength選定unitが重複している")
    return rows
...
def scan_edges() -> tuple[dict[str, list[dict[str, Any]]], int, list[str]]:
    ...
    for wave, path in WAVE_PATHS.items():
        for row in base_jsonl(path):
            ...
            if unit and unit in SELECTED_UNITS and UNIT_PATTERN.fullmatch(unit):
...
def old_asset_record(
    ...
    return {
        ...
        "source": {
            ...
            "body_read": {
                "static_read": True, "not_implementation_proof": True,
                "byte_count": len(source_bytes), "line_count": len(source_lines),
                "body_sha256": sha256(source_bytes),
                "first_line_sha256": sha256(source_lines[0]) if source_lines else None,
                "last_line_sha256": sha256(source_lines[-1]) if source_lines else None,
            },
        },
        "history": {
        ...
        "failure": {
            ...
            "observed_failure_status": "receipt_present_static_unexecuted" if failure_receipts else "unknown_no_static_failure_receipt",
            "observed_failure_receipts": failure_receipts,
        },
        "consumer": {
            "static_only": True,
...
def strength_metrics(unit: str, edges: list[dict[str, Any]], assets: list[dict[str, Any]]) -> dict[str, Any]:
    asset_ids = {asset["asset_id"] for asset in assets}
    implementation_asset_ids = sorted({edge["asset_id"] for edge in edges if edge.get("artifact_evidence_kind") == "implementation_source"})
    test_asset_ids = sorted({edge["asset_id"] for edge in edges if edge.get("artifact_evidence_kind") in {"test_source", "test_design"}})
    failure_count = sum(bool(edge.get("coverage", {}).get("failure")) for edge in edges)
    observed_consumer_ref_count = sum(len(edge.get("observed_consumer_refs", [])) for edge in edges)
    ledger_consumer_ref_count = sum(len(asset["consumer"]["ledger_consumer_refs"]) for asset in assets)
    decision_count = sum(len(asset["consumer"]["decision_records"]) for asset in assets)
    read_after_count = sum(len(asset["consumer"]["read_after_records"]) for asset in assets)
    return {
        "family": unit.split("-")[2],
        "asset_count": len(asset_ids), "edge_count": len(edges),
        "implementation_source_asset_count": len(implementation_asset_ids),
        "test_evidence_asset_count": len(test_asset_ids), "coverage_failure_count": failure_count,
        "observed_consumer_ref_count": observed_consumer_ref_count,
        "ledger_consumer_ref_count": ledger_consumer_ref_count,
        "decision_record_count": decision_count, "read_after_record_count": read_after_count,
        "selection_score": len(implementation_asset_ids) * 5 + len(test_asset_ids) * 4 + failure_count * 2
        + observed_consumer_ref_count + ledger_consumer_ref_count + decision_count + read_after_count,
        "implementation_source_asset_ids": implementation_asset_ids,
        "test_evidence_asset_ids": test_asset_ids,
    }


def strength_assessment(unit: str, source: dict[str, Any], edges: list[dict[str, Any]], assets: list[dict[str, Any]]) -> dict[str, Any]:
    metrics = strength_metrics(unit, edges, assets)
    coverage_findings = [{"review_id": edge["review_id"], "failure": edge.get("coverage", {}).get("failure")} for edge in edges if edge.get("coverage", {}).get("failure")]
    failure_receipts = [receipt for asset in assets for receipt in asset["failure"]["observed_failure_receipts"]]
    return {
        "selection": {
            "family": metrics["family"], "quota": 5, "selection_score": metrics["selection_score"],
            "selection_rule": "fixed_BASE_candidate_pool_family_quota_score_desc_with_test_evidence_coverage_then_unit_id_asc",
            "candidate_pool_scope": "HIL-BR/HIL-FR/HIL-TR/HIL-NFR crosswalk units",
        },
        "old_implementation": {
            "unit_status": "unknown", "direct_unit_implementation_proof": False,
            "evidence_presence": "implementation_source_present_unexecuted" if metrics["implementation_source_asset_count"] else "none_found",
            "implementation_source_asset_ids": metrics["implementation_source_asset_ids"],
            "test_evidence_asset_ids": metrics["test_evidence_asset_ids"],
            "legacy_execution_performed": False,
            "why_unit_status_remains_unknown": "source／test bytesの存在とsemantic edgeはunit実装成立の実行・受入証拠ではない",
        },
        "old_degradation": {
            "unit_status": "unknown", "direct_unit_degradation_proof": False,
            "phase_transition_record_count": len(source.get("phase_capability_evidence", [])),
            "edge_constraint_record_count": len(edges),
            "why_status_remains_unknown": "phase transition／constraintはunit縮退のreceiptではない",
        },
        "old_failure": {
            "unit_status": "unknown", "coverage_failure_findings": coverage_findings,
            "failure_receipt_status": "receipt_present_static_unexecuted" if failure_receipts else "unknown_no_static_failure_receipt",
            "failure_receipts": failure_receipts, "legacy_execution_performed": False,
            "why_status_remains_unknown": "coverage.failureはreview findingであり、旧runtime/testのfailure receiptを代替しない",
        },
        "old_consumer": {
            "closure_status": "pending", "observed_edge_ref_count": metrics["observed_consumer_ref_count"],
            "ledger_consumer_ref_count": metrics["ledger_consumer_ref_count"],
            "decision_record_count": metrics["decision_record_count"],
            "read_after_record_count": metrics["read_after_record_count"],
            "why_closure_remains_pending": "consumer refs／decision／read-afterはclosure receiptではない",
        },
        "old_nonimplementation": {
            "status": "not_assessed", "explicit_nonimplementation_claim": False,
            "why_not_claimed": "source／test存在、failure finding、consumer refsの有無から未実装を断定できない",
        },
...
def novel_evidence(source: dict[str, Any], edges: list[dict[str, Any]], assets: list[dict[str, Any]]) -> dict[str, Any]:
...
def build_bundle() -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    if set(edges_by_unit) != {row["unit_candidate_id"] for row in crosswalk}:
        raise ValueError("implementation-strength選定unitのWave edgeが欠落または余分")
    ...
    for source in crosswalk:
        ...
        evidence.append({
            ...
            "strength_assessment": strength_assessment(unit, source, edges, assets),
            "novel_evidence": novel_evidence(source, edges, assets),
    ...
    selected_failure_receipt_count = sum(
        1 for row in read_after
        if row.get("asset_id") in unique_assets and row.get("failure")
    )
    counts = {
        "units": len(evidence),
        ...
        "old_failure_receipts": selected_failure_receipt_count,
    ...
    inventory = {
        ...
        "scope": {
            ...
            "br_units_included": sum(unit.split("-")[2] == "BR" for unit in SELECTED_UNITS),
        },
...
if __name__ == "__main__":
    ...
    print("SCF-B-0115 bundle generated")
