#!/usr/bin/env python3
"""SCF-B-0115の固定BASE静的証拠bundleをfail-closedで検査する。"""
...
def strength_metrics(unit: str, edges: list[dict[str, Any]], assets: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "family": unit.split("-")[2], "asset_count": len(asset_ids), "edge_count": len(edges),
...
def expected_strength_assessment(unit: str, source: dict[str, Any], edges: list[dict[str, Any]], assets: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "old_implementation": {
            ...
            "test_evidence_asset_ids": metrics["test_evidence_asset_ids"], "legacy_execution_performed": False,
...
class Validator:
    ...
    def finish(self) -> int:
        if self.errors:
            ...
            return 1
        print("SCF-B-0115 validate: PASS (20 selected units; counts derived from fixed BASE; static-only)")
        return 0
    ...
    def validate(self) -> int:
        ...
        if [row.get("unit_candidate_id") for row in evidence] != units or len(set(row.get("unit_candidate_id") for row in evidence)) != len(units):
            self.error("E_UNIT_SET", "implementation-strength selected unit集合の順序・分母・重複が不一致")
        ...
        expected_scope = {
            ...
            "unique_old_asset_count": len(expected_assets), "br_units_included": sum(unit.split("-")[2] == "BR" for unit in SELECTED_UNITS),
        }
        if inventory.get("scope") != expected_scope:
            self.error("E_INVENTORY_DECLARATION", "inventory scope宣言が導出値と不一致")
        expected_old_failure_receipt_count = sum(
            1 for row in read_after
            if row.get("asset_id") in expected_assets and row.get("failure")
        )
        expected_counts = {
            ...
            "current_static_refs": len(units) * len(CONTEXT_INPUTS),
            "old_failure_receipts": expected_old_failure_receipt_count,
        ...
        for source in crosswalk:
            ...
            expected_selection["selected_metrics"][unit] = strength_metrics(unit, edges, expected_asset_records)
