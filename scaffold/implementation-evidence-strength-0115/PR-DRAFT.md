...
- 27負例でsource／anchor／edge／asset／ledger／partition／strength assessment／novel evidence／current／未実装／inventory／BASEの改竄を期待error codeと照合する。
...
期待値は `20 selected units / 60 semantic review edges / 38 old assets / 598 Wave scan rows / 180 current context refs`、validator PASS、selfcheck 27負例PASS、scfctl fail 0である。stale／residual／diff checkも実施する。
