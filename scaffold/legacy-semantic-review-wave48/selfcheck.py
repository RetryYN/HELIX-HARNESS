#!/usr/bin/env python3
"""Run Wave48 validation and explicit fail-closed mutations."""
...
spec = importlib.util.spec_from_file_location("wave48_validator", HERE / "validate.py")
...
def rejected(label: str, mutate) -> None:
    ...
    mutate(mutated_rows)
    try:
        with tempfile.TemporaryDirectory(prefix="wave48-selfcheck-") as tmp:
            root = Path(tmp)
...
def rejected_meta(label: str, mutate) -> None:
    ...
    meta = json.loads(original_meta.read_text())
    try:
        with tempfile.TemporaryDirectory(prefix="wave48-meta-selfcheck-") as tmp:
...
def rejected_document(label: str, module_attr: str, mutate) -> None:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="wave48-document-selfcheck-") as tmp:
...
def mutate_pool_consumed_overclaim(meta) -> None:
    reconciliation = next(
        ...
        if item["unit_candidate_id"] == "IRUNIT-HIL-NFR-29-HELIX-HARNESS" and item["role_kind"] == "implementation_source"
    )
...
def mutate_design_reconciliation_omission(meta) -> None:
    receipt = next(
        ...
        if item["unit_candidate_id"] == "IRUNIT-HIL-NFR-29-HELIX-OS" and item["role_kind"] == "design"
    )
...
def mutate_prior_cumulative_counts(meta) -> None:
    meta["cumulative_reviewed_unit_count"] = 205
    meta["cumulative_reviewed_edge_count"] = 577


def reject_wave47_copy_residue() -> None:
    """Reject stale Wave47 template text in human-facing Wave48 artifacts."""
    forbidden = (
        "Wave47 legacy semantic review Scaffold",
        "SCF-B-0089",
        "f15c3ca2ed1dc865b242b0a21e1516fdeec6f9f6",
    ...
    for path in (HERE / "README.md", HERE / "plan.json", HERE / "inventory.json"):
        text = path.read_text()
        assert not any(marker in text for marker in forbidden), f"stale Wave47 copy residue: {path}"


reject_wave47_copy_residue()
...
print("Wave48 selfcheck: PASS (validator plus thirty-nine negative mutations)")
