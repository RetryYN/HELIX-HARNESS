...
- validate.py: 19 unit／55 edge／38 assetの集合、unit別asset集合、inventoryのscope／count／evidence partition宣言、source snapshot、review edge、旧asset ledger完全一致、代表候補record完全一致、旧実装のedge別寄与、旧failure／degradationのcoverage／ledger／source span導出、独立したevidence refのpath／line／blob、current span、input digest、BASE祖先性、consumer／counter-evidence／unresolvedをfail-closedで検査する。
- selfcheck.py: blob、current span、edge multiset／件数、unit別asset集合・重複、inventory宣言、current／old status、旧実装edge別寄与、failure／degradation span・ledger、ledger／consumer、代表候補record、counter-evidence、unresolved、input digest、anchor policy／resolutionを壊す24負例と期待error codeを照合する。
...
## 検証
    ...
    SCF-B-0106 selfcheck: PASS (24 negative cases; expected error codes matched)
