#!/usr/bin/env python3
"""SCF-B-0106のfail-closed静的validator。旧世代の実行は行わない。"""
...
class Validator:
    ...
    def validate(self) -> int:
        ...
        if inventory.get("schema") != "br-implementation-evidence-0106/v1":
        ...
        if [x.get("unit_candidate_id") for x in evidence] != UNITS:
            self.error("E_UNIT_SET", "19 unitの順序・分母・重複が期待値と異なる")
        ...
        if [x.get("unit_candidate_id") for x in crosswalk] != UNITS:
            self.error("E_CROSSWALK_SET", "crosswalk HIL-BR-21〜33 unitが期待値と異なる")
        ...
        if len(actual_edges) != 55:
            self.error("E_REVIEW_EDGE_COUNT", f"review edge分母が{len(actual_edges)}（期待55）")
    ...
    def finish(self) -> int:
        if self.errors:
            for item in self.errors:
                print(item)
            return 1
        print("SCF-B-0106 validate: PASS (19 units, 55 review edges, 38 old assets; static-only)")
        return 0
