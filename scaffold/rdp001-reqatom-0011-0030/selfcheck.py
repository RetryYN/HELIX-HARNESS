...
def main() -> int:
    ...
    generator_negative(
        ...
        lambda plan: plan["line_specs"]["REQSRC-LINE-00027"]["atomized"].pop(),
        "exact atomized line count",
    )
    generator_negative(
        "generator-composite-loss",
        lambda plan: plan["line_specs"]["REQSRC-LINE-00028"]["composite_unresolved"].pop(),
        "exact composite line count",
    )

    def replace_target(plan: dict) -> None:
        spec = plan["line_specs"]["REQSRC-LINE-00027"]["atomized"][0]
