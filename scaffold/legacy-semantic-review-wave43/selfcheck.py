#!/usr/bin/env python3
"""Run Wave43 validation and explicit fail-closed mutations."""
...
spec = importlib.util.spec_from_file_location("wave43_validator", HERE / "validate.py")
...
def rejected(label: str, mutate) -> None:
    ...
    mutate(mutated_rows)
    try:
        with tempfile.TemporaryDirectory(prefix="wave43-selfcheck-") as tmp:
            root = Path(tmp)
...
def rejected_meta(label: str, mutate) -> None:
    ...
    meta = json.loads(original_meta.read_text())
    try:
        with tempfile.TemporaryDirectory(prefix="wave43-meta-selfcheck-") as tmp:
...
def mutate_shared_connection(mutated_rows) -> None:
    shared = next(row for row in mutated_rows if row["unit_candidate_id"] == "IRUNIT-HIL-NFR-15-HELIX-OS")
