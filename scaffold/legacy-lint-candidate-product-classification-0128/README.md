...
- 候補分類は direct product basis **38**、multi-product conflict **8**、insufficient basis **5** です。各分類は具体的なarchive source marker/span、四製品L1、`docs/concept/product-boundary.md`の候補境界、反証を突き合わせたものです。path・filename・汎用語だけでownerを決めていません。
...
各recordは旧phase/disposition行のdigest、archive Git blob・bytes・sha256・line count、具体source spanと行テキストdigestを持ちます。候補分類にはcandidate products、意味説明、counter-evidenceを保持します。
...
# SCF-B-0128 selfcheck: PASS negative_cases=36
