...
| 5 | **テスト実施** | `npm run test`（CIではfast／slowを実行）等で全テスト実行 | aim |
...
重複・依存の機械検出は実装済みの`helix doctor` `relation-graph`を活用する。
`connection-deficiency`は未実装carryであり、配線完了まではレビュー観点として扱う。
出典: concept v3.1 §3.1.4

---

## 4. G7: 4 artifact trace freeze（4成果物 trace freeze）
...
縮退時に cross-agent review が self-review に化けないよう、判断ゲートは必ず `helix status` の execution mode を参照する。
