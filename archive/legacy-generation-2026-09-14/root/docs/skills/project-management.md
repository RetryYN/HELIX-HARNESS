...
stall 検出、program boundary での continuation discipline を管理する。
...
- milestone または session boundary 前に、複数 active PLAN をまたいだ program 状態を確認する。
...
- session boundary を越え、program-level continuation state を確認する必要がある。
...
3. parallel / serial grouping を PLAN 工程表と `review_evidence` に記録する。memory だけに頼らない。

`helix graph` の cycle は hard block である。直接または推移的に自分自身へ依存する PLAN は前進できない。
共有 dependency を新しい upstream PLAN として切り出して解消する。

## Milestone continuation discipline（milestone 継続規律）
...
3. `helix status` で `harness.db` continuation projection の active PLAN と next action を確認する。
4. projected stateを PLAN status、`git log`、event frontierと照合し、既に `done` のitemを持ち越さない。
5. milestone 固有の判断は PLAN `review_evidence` または ADRへ記録し、memoryだけを正本にしない。

continuation projection の整合証跡が無い milestone は closed ではない。
...
| Scrum | Sprint boundary = milestone continuation。projected item は code と照合して review する。 |

## Anti-patterns（避けるパターン）

- memory breadcrumbをPLAN status読解の代替にする。DB projectionと`helix status`で検証する。
