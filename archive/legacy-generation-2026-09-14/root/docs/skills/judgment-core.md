...
`judgment_core: v2` marker を持つ。SSoT との乖離は doctor gate `judgment-core-coverage` が
...
- **着手前**: 依頼の 1 文ごとに検証手段（test / command / 実測）を先に決める。検証手段が
  ...
  実装提案の冒頭で「書く前の 7 段の問い」のどこで止まったかを 1 行宣言する
  （code-minimalism §1。「設定で解決可能、コード不要」で終わる提案は満点回答）。
...
- **完了宣言前**: typecheck / lint / test / doctor green を実測してから宣言する。falsifiable claim
  ...
  **全テストが一発で緑なら喜ばずに疑う**（「実装が完璧」より「テストが弱い」確率のほうが
  ずっと高い。test-thinking §0）。検証を打ち切る判断は残余リスクの言語化とセットで行う
  （「〜は未検証。壊れた場合の影響は〜」の一文が書けないなら打ち切りが甘い。test-thinking §5）。
...
- **evidence → verdict の順序**: 基準ごとに根拠（引用・`file:line`・実測）を cite してから
  PASS / FAIL / UNCERTAIN を宣言する。verdict が先に来る所見は信用しない。根拠不足を推測で
  埋めず UNCERTAIN と明記する（analytic rubric、§6 出典）。簡潔な所見は同等の正しさなら
  冗長な所見より劣らない（verbosity bias 対策）。
- **改善なら承認 / Nit 分離**: 完璧を求めて改善を止めない。全体のコード品質が確実に改善する
  変更は approve する。個人的嗜好は `Nit:` ラベルで区別し blocking にしない。reviewer と
  author の対立は放置せず、evidence を追加するか escalate する（Google eng-practices、
  CC-BY 3.0、§6 出典）。adversarial stance（拒否の規律）と両立する: 拒否は spec / AC 違反の
  evidence があるときであり、嗜好の違いではない。
...
4 点セットに加え、worker のモデル帯と役割に応じた記述パターン（自由度の段階分け・checklist・
feedback loop・evidence→verdict rubric・Chain-of-Verification 等）の選び方は
`skill-authoring` pack が正本である。
...
v2 追補（2026-07-11、PLAN-L7-419）:

- Anthropic 公式の skill authoring 指針（判断自由度の段階分け・チェックリスト・フィードバックループ）:
  https://platform.claude.com/docs/en/agents-and-tools/agent-skills/best-practices
- analytic rubric（evidence→verdict 順序・UNCERTAIN 許容・verbosity bias 対策）:
  https://www.appen.com/llm-as-a-judge-rubric-design
- Google eng-practices code review 標準（改善なら承認 / Nit 分離 / 対立を放置しない。CC-BY 3.0）:
  https://github.com/google/eng-practices
- vmodel-docgen skills（PO 提供 zip、2026-07-11）: 全緑を疑う・止めどきの残余リスク言語化・
  7 段の問いの 1 行宣言の参考元。
