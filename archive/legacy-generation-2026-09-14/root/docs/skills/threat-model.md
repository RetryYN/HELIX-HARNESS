...
- `helix guardrail` が unresolved finding を報告する。
...
1. **Agent tool invocations.** subagent 内から呼べる任意の `helix` command。
...
3. **Agent allowlist.** `.claude/settings.json` の subagent type allowlist。
   Threat: unlisted agent type が `HELIX_ALLOW_RAW_AGENT=1` なしに guard を bypass する。
4. **State files.** doctor と projection writer が読む `.helix/` YAML/JSON file。
...
| Tampering | agent が doctor を evades する形で `.helix/` state へ write できるか。 |
| Repudiation | 重要な agent action はすべて `.helix/audit/` に記録されるか。 |
...
- **No credentials in state.** `.helix/`、`docs/`、audit evidence、handover file に
  API key、password、session token を含めない。これらの path に触る PLAN を accept する前に
  `helix guardrail` を実行する。
- **Audit trail.** すべての guard bypass（`HELIX_ALLOW_RAW_AGENT=1` 経由）は
  `.helix/audit/` に evidence record を書く。trace の無い bypass は禁止。
...
- threat model output を handover file へ直接書く。
  artifact を versioned にし、`helix doctor` が見つけられるよう `docs/design/L3/` を使う。
- `helix guardrail` green を complete threat model と扱う。
