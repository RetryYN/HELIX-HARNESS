...
| HC-CHAT-040 | canonical L1–L12の各Lへ連鎖台帳を置き、層外L0 charter anchorを保持する。templateから機械抽出・追加記載・台帳ベース設計refactorを行い、上下隣接pairと左右V-pairの両方を必須にする | HIL-BR-25、HIL-FR-46..50、HIL-NFR-29 | §4.8 Layer Ledger Chain / L12 canonical directive | HOT-HIL-44..47、HST-HIL-030..033 | mapped-rebased-l12 |
...
| 前身Git exact 2 repo | authority receipt 0/2、ref/content/edge分母pending | aggregateはhistorical seedのみ。atomic child未閉鎖 | pending | current receiptなしで合格算入不可 | 0 atomic join |
