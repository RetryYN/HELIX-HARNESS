...
| 能力目標 | 目指すこと | 現在との境界 |
| --- | --- | --- |
| 1.0 | 複数プロダクトの実開発とHELIX自身への適用を通じ、HELIX-HARNESS製品群の開発・検証・提供・運用を成立させる | 対象要求・consumer・受入範囲と実証portfolioの確定が必要。HELIX-Web展開の必須前提 |
...
Web、Connector、HDA、FACTORY、MARKETINGとの接続は別案件の長期参照とする。HELIX-Web自体をHARNESS Version 1の
必須実装へ追加しないが、その展開にはVersion 1でHARNESS製品群が完成していることを要求する。
...
RLSの「8+1 Bundle」は初期8候補にLifecycle Operations Bundle 1候補を追加した構成を指し、公開済みBundle数ではない。
...
| 長期目標の隔離 | Visionは対象要求を明示した能力目標とし、未採択の将来能力を現在の必須依存へ追加しないこと | Web自体をHARNESS Version 1の完成分母へ含めず、HARNESS Version 1未完成時のWeb展開も拒否する。目標達成を文書存在だけで認定しない | Concept / World Governance |
...
原文の `basis/` リンクは本節の実配置リンクで補完する。原文bytesと受領時checksumは変更せず保全し、日本語の説明は本整理版だけに置く。
