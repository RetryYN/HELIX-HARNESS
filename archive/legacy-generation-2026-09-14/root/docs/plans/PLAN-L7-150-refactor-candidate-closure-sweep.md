...
| `src/lint/version-up-readiness.ts` | `split-module` | `accepted_debt` + `attached_plan=PLAN-L7-150` | version-up readiness gate の正本（2026-07-06 の version-up bundle 拡張で 2,800 行超に成長）。readiness 判定 / review matrix / activation packet の単位で抽出し、`tests/version-up-readiness.test.ts` を fence にする。 |
...
| `src/config/requirements-binding.ts#policy:sha256:39498a6e822a` | `externalize-policy` | `resolved` + `attached_plan=PLAN-L7-351` | `requirements-binding.ts` は schema / parse / load の SSoT として残し、運用可変の policy/default catalog を `src/config/requirements-binding-policy.ts` へ分離した。detector rule は変更せず、`*-policy.ts` exemption と整合させた。 |
| `src/setup/templates.ts#literal:sha256:4b69611db37f` | `externalize-literal` | `resolved` + `attached_plan=PLAN-L7-351` | hook JSON の `"type": "command"` 行を `src/setup/template-markers.ts` へ定数化し、配布 template manifest digest 不変で閉じた。 |
| `src/setup/templates.ts#literal:sha256:7111fbd12620` | `externalize-literal` | `resolved` + `attached_plan=PLAN-L7-351` | hook JSON の `"timeout": 5` 行を `src/setup/template-markers.ts` へ定数化し、配布 template manifest digest 不変で閉じた。 |
