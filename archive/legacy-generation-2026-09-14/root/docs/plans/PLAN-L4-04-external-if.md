...
HELIX harness と外部 service の**境界 (what/形状)**を確定する。各境界に: ① どの service の何の操作を使うか、② harness 側の前提 (precondition) と保証 (postcondition)、③ 失敗時の振る舞い (fail-close / degradation)、④ 秘密情報の扱い (禁止事項: API key/credential を doc/example に書かない)。粒度は **what/形状まで** (how/contract 詳細は L5 D-API、IMP-018)。
...
> 外部 IF は標準語 (DbC/adapter) 主体。HELIX 固有語の新規導入は想定しない。
