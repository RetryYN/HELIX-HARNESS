...
| V16 | (minor) gates.md cross-mode 注記 / forward overview §6 freeze 注記 / `REVERSE_ROUTING_GATE_MAP` (schema TODO、gate cmd 実装時) / pair_artifact を kind=design 条件付き必須に昇格(lint) | gates.md/forward/overview/src/schema/src/plan/lint.ts | spike正本化 + helix gate/lint 実装 PLAN |
...
> - critical 修正 (confirmed_reverse_type、`c203d32`)
...
|------|---------|--------------|------|
| DISCOVERY-01 (workflow metamodel) | poc | `fullstack` | harness methodology = HELIX TS fullstack 作業 |
...
| DISCOVERY-04 (process docs) | poc | `fullstack` | process methodology = harness 作業 |
