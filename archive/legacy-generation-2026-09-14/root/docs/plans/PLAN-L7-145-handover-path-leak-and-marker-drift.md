...
## 1. 欠陥 #1 (Defect #1) — handover 生成器が絶対個人パスを漏洩
...
## 2. 欠陥 #2 (Defect #2) — marker-drift (active-plan が done PLAN を指し false in_progress)

### 2.1 根本

active-plan は `.helix/state/current-plan` marker (手動 `helix plan use` 設定のみ)。in-session
...
### 2.3 被害封じ込め (mitigation / contain) + design 課題化
...
- 欠陥 #2 (Defect #2) の marker auto-advance 設計 (§2.3)。
