...
- **continuation readiness**: 当時の bypass / stale pointer 検出は historical evidence として保持する。現行は authored source・append-only event・harness.db projection・bounded memory の整合を検査し、旧 writer / pointer を current glossary へ再登録しない（PLAN-REVERSE-344 fullback）。
