...
直列理由: downstream_dependency
`helix plan lint` は violation で fail、doctor も `planSchedule.ok` を `runDoctor.ok` に連動する hard/fail-close surface とする。
...
- [x] `helix plan lint` と doctor hard/fail-close の一致を明記する
