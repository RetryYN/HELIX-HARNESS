...
- `l12-hybrid-recognition` は citation 更新で
  ...
  編集前後の signal 集合が **bit 同一**（同一 id の legacy-risk signal 3 件。内訳は
  `src/lint/l12-hybrid-recognition.ts` の `SIGNAL_PATTERNS` が正本）であることを
