...
review_evidence:
  - reviewer: codex-tl
    ...
    verdict: approve
    scope: "completion decision-packet の schemaVersion contract と、setup first-run version-up dry-run の配布 remote evidence を同じ consumer doctor / distribution acceptance 境界まで同期する。"
    worker_model: codex
...
配布 remote を渡さないため、consumer 初回検証で配布 tag 存在を remote source に結び付ける証跡が弱かった。
...
- setup first-run verification、VSCode task、GitHub CI smoke、adapter template、consumer doctor が参照する version-up dry-run command を、配布 remote 付きの同一 command に揃える。
- 検証中に配布 repo `main` の実測 HEAD が進んだため、objective external ledger と関連 oracle を `e454190d433292f5e9409033823a05e9dad61b67` へ同期する。
- L3/L6/L7 test design と既存 PLAN-L7-03 の記述を、remote source 付き first-run evidence へ同期する。

## 境界

- 配布 target tag `v0.1.3` の採用、version-up activation、remote write、cutover apply は実行しない。
