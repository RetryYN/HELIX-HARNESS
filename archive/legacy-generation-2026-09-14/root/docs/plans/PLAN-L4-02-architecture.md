...
L4 Master (`PLAN-L4-00-master`) §2 の ① 必須 sub-doc「architecture」を詳細化する design PLAN。出力 = `docs/design/harness/L4-basic-design/architecture.md`。data.md (PLAN-L4-01) の 5 集約を構成単位 (building block) に配置し、arc42 (§4 Solution Strategy / §5 Building Block View / §9 ADR) を根拠に方式設計を確定する。

## §1 目的

HELIX harness を **どう実現するか** (方式) を L4 レベルで確定する: ① 主要技術決定 (TS/Bun + file-based state + CLI/library + agent orchestration) の根拠、② ソフトウェアを構成する構成単位 (building block) とその責務・境界、③ 横断方針 (hook / CI / fail-close)、④ 設計判断を ADR として記録する仕組み。data.md (構造) に対し、本 doc は **実行構造 / 制御フロー / 依存方向**を担う。

## §2 背景

- 上流: data.md §2 (5 集約) + L3 functional (FR 26) + L1 §10.2 持ち越し (carry)
...
方式 → 詳細 (L5 contract/D-API、L6 機能設計 pseudocode) への持ち越し (carry) を明示。

## §4 受入条件 / DoD

- [x] Step 1〜8 のすべてが `architecture.md` に存在
- [x] arc42 §4 (Solution Strategy: 品質目標→技術決定) が存在
- [x] arc42 §5 の構成単位 (building block、Level 1/2、責務・依存方向) が存在
- [x] data.md 5 集約 → src/ module マッピング表 (IMP-025) が存在
- [x] ADR テンプレート + L4 ADR 候補 (IMP-023) が存在
- [x] hook/CI 配線方針が存在 (agent-guard 既存 + 目標 hook)
...
|---|---|---|---|
| 構成単位 (building block、方式設計の構成単位) | 参照 | arc42 §5 標準語、独自定義せず参照 | back-merge 不要 |
| ADR (Architecture Decision Record) | 参照 | arc42 §9 標準語、独自定義せず参照 | back-merge 不要 |

> 方式設計は標準語 (arc42/ADR/building block) 主体のため、HELIX 固有用語の新規導入は想定しない。固有名 (module 名等) が確定したら §10.1 と突合。
