...
title: "PLAN-REVERSE-226 (reverse): verification profile と右腕 gate metadata の接続"
kind: reverse
layer: cross
workflow_phase: R4
drive: agent
...
agent_slots:
  - role: tl
    slot_label: "TL - verification profile 右腕 metadata backfill"
generates:
...
# PLAN-REVERSE-226: verification profile と右腕 gate metadata の接続
...
`missing-drive-g10-profile` として fail-close する。drive 別 L10 coverage は browser profile に限定し、
非 browser profile が G10 を名乗っても browser evidence の代替にしない。
...
- L7/test-design: U-MCPPROFILE-015/016/017 を追加し、catalog metadata、drive L10 hard gate、G10 browser evidence semantic gate を oracle 化する。
