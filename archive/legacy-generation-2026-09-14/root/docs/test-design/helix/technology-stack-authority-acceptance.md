...
## §1 acceptance oracle 受入判定一覧

| AC ID | trace 対応 | positive oracle 正常系 | negative oracle 異常系 |
|---|---|---|---|
...
| `TECH-STACK-AC-004` | R-02／§2 | 一次情報receiptに束縛したNode 24 LTSとTypeScript 7 target、manifest宣言rangeとlock済みresolved version、6 API互換境界を区別する | Node Current自動採用、`^5.6.3`／`5.9.3`の混同、一次情報だけの無検証cutover、TS6/7恒久二重authorityならfail |
...
| `TECH-STACK-AC-012` | §2／§3 | 一次情報の確認日・採否・workflow影響と5 unresolved itemを明示し、`none`もdispositionとして保持する | source URLだけでcutover、または未測定項目を暗黙採用・完了として隠したらfail |
