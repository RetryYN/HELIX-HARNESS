...
| `U-OSSC-007` | `resolveCanonicalDependencyGraph` | Node npm/package.json/package-lock以外、Bun lock混入、Python toolchain未freeze／複数、direct/transitive欠落、floating version、artifact digest欠落、OS別lockを個別投入 | stable graphを順序非依存生成し、authority不一致・未freeze・欠落・floating・OS forkを`HIL_SUPPLY_CHAIN_LOCK_DRIFT`で拒否 | `tests/supply-chain-dependency-graph.test.ts` |
...
public API ownerは13 APIと`U-OSSC-001`〜`013`の一対一exact joinとする。各unit fixtureはL6 local
`OsContractFacetV1`の9値と`OsSupplyChainFailureCodeV1`の21値（L5 §6の詳細failure 12件＋HST境界failure 7件＋adapter境界internal failure 2件）だけを受理し、旧名、自由文字列、L5 allowlistとの集合差を拒否する。
primary caseは同じUを共有してもcase ID、pre/expected state、failureを別々に採点し、owner重複へまとめない。
さらに各public result `*V1`の必須field、closed enum、nested digest、count、authority/freshness fieldを一件ずつmutationし、
欠落・余剰・旧抽象名・`unknown`への弱化が全てRedになることを型契約oracleとして採点する。
OS adapterとsupply-chain adapterへunexpected exceptionを一件ずつ注入し、それぞれ`HIL_OS_INTERNAL_ERROR`、
`HIL_SUPPLY_CHAIN_INTERNAL_ERROR`へcause digest付きで境界変換され、state・receipt・completion current増分が0であることを検証する。
この2 mutationは診断境界oracleであり、既存19 HSTの分母へ加算しない。
