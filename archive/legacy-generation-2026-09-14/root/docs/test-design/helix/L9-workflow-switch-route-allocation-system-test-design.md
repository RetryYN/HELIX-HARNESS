...
ST-UWJ-011..013はL8 contractをsystem fixtureへcompositionした後のbinding欠落だけを所有する。field型、
...
|---|---|---|
| `ST-UWJ-AUTH-001` | registryのversion／digest drift | `workflow_identity_authority_drift`、side effect 0 |
| `ST-UWJ-AUTH-002` | `mode`／`model`／`catalog_route_id`／`route_class`だけを入力 | compatibility推測せず拒否 |
| `ST-UWJ-AUTH-003` | AI proposalがcommit/dispatch authorityを主張 | authority denied、DB/Git/GitHub/worker write 0 |
| `ST-UWJ-AUTH-004` | generated catalogだけを意味authorityとして提示 | requirements registry欠落で拒否 |
| `ST-UWJ-AUTH-005` | provider名またはbenchmark score単独でcandidate選択 | capability/capacity evidence欠落で拒否 |
| `ST-UWJ-AUTH-006` | fallback/dead-letterで元constraintを緩和 | constraint violation |
| `ST-UWJ-AUTH-007` | measurement failureをtest greenで相殺 | completion denied |
...
| L4 component | oracle |
|---|---|
| WorkflowIdentityAdmissionPort | ST-UWJ-AUTH-001／002／004 |
| WorkflowSwitchBoundary | ST-UWJ-011 |
| WorkflowRouteBoundary | ST-UWJ-012 |
| WorkflowAllocationBoundary | ST-UWJ-013 |
| WorkflowMeasurementBindingPort | ST-UWJ-014 |
| ProposalAuthorityPort | ST-UWJ-AUTH-003 |
| WorkflowPublicationBoundary | ST-UWJ-015 |
