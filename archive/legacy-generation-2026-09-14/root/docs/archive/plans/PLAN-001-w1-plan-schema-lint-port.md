...
agent_slots:
  ...
  - role: tl
    slot_label: "TL — adapt 設計レビュー（HELIX_PROJECT_ROOT 対応、enum 同期）"
  - role: se
    slot_label: "SE — src/helix/ 配下 Python 移植実装"
  - role: docs
    slot_label: "Docs — テンプレ＆ガバナンス docs 整合チェック"
generates:
  - artifact_path: src/helix/plan_frontmatter.py
    artifact_type: python_module
  - artifact_path: src/helix/plan_parser.py
    artifact_type: python_module
  - artifact_path: src/helix/plan_schema.py
    artifact_type: python_module
  - artifact_path: src/helix/plan_validator.py
    artifact_type: python_module
  - artifact_path: src/helix/plan_lint.py
    artifact_type: python_module
  - artifact_path: src/helix/plan_deps_helper.py
    artifact_type: python_module
  - artifact_path: src/helix/tests/test_plan_frontmatter.py
    artifact_type: test_code
  - artifact_path: src/helix/tests/test_plan_parser.py
    artifact_type: test_code
  - artifact_path: src/helix/tests/test_plan_schema.py
    artifact_type: test_code
  - artifact_path: src/helix/tests/test_plan_validator.py
    artifact_type: test_code
  - artifact_path: src/helix/tests/test_plan_deps_helper.py
    artifact_type: test_code
  - artifact_path: src/helix/tests/test_plan_lint.py
    artifact_type: test_code
...
HELIX-HARNESS の最優先 wave（W1）として、HELIX snapshot の `cli/lib/plan_*.py` 5 本 + テスト + impl/design template を `src/helix/` 配下へ adapt 移植する。本 PLAN は HELIX 流（PLAN-NNN + gate）で進める porting 期間中の起票第1号。

## §1 目的

`helix plan lint` の中核となる PLAN frontmatter parser / schema / validator / lint engine を HELIX 所有のコードとして確立する。これにより:

- PLAN-002 以降の W2/W3/W4 PLAN を HELIX-owned lint で検証できる
- `helix` CLI を経由せず PLAN を機械検証できる足場ができる
- `requirements_v1.1.md` の VALID_* enum を schema 側で正本化できる

## §2 背景

- 現状、PLAN lint は HELIX snapshot (`vendor/helix-source/`) 経由でしか動かない。porting 期間中は HELIX で代行できるが、HELIX 独自運用に切替えるためには W1 を最初に通す必要がある。
...
- `vendor/helix-source/cli/lib/plan_frontmatter.py` → `src/helix/plan_frontmatter.py`
  - `.helix` default を `.helix` へ
  - `HELIX_PLAN_FRONTMATTER_FAIL_STAGE` → `HELIX_*` rename
...
- `vendor/helix-source/cli/lib/plan_parser.py` → `src/helix/plan_parser.py`（**SQLite upsert 部分は除外、`parse_frontmatter` のみ 軽量版**）
- `vendor/helix-source/cli/lib/plan_schema.py` → `src/helix/plan_schema.py`（**mini-plan 系統と `evaluate_g2_design_evidence` は除外**、`PLAN_ID_RE` を HELIX §1.10 形式へ）
- `vendor/helix-source/cli/lib/plan_validator.py` → `src/helix/plan_validator.py`
  ...
  - **`_plan_search_directories` の `parents[2]` 逆算を `HELIX_PROJECT_ROOT` 環境変数または引数化**
- `vendor/helix-source/cli/lib/plan_lint.py` → `src/helix/plan_lint.py`
  - `draft|finalized|completed` ハードコード → HELIX 4 値
  - `SELF_REFERENCE_PLAN_NUMBERS = {36, 37}` / `plan_number < 36` 削除
- `vendor/helix-source/cli/lib/plan_deps_helper.py` → `src/helix/plan_deps_helper.py`（`HELIX_PROJECT_ROOT` → `HELIX_PROJECT_ROOT`）
- 補助 module の最低限移植: `vendor/.../yaml_parser.py` は直接 PyYAML 使用へ置換（独自 wrapper を持ち込まない）
- 対応する pytest 5 本のうち以下を port:
  - test_plan_frontmatter: parametrize 11 件削除、HELIX 独自 fixture で再構築
  ...
  - test_plan_deps_helper: 素直に port、PLAN-091 ID を HELIX PLAN ID へ書換え
  - **test_plan_parser は SQLite v35 依存のため W1 範囲外**（PLAN-001-b として carry、W2 で実施）
- `vendor/helix-source/cli/templates/plan/impl/template.md` → `docs/templates/plan/impl/template.md`（HELIX 主語化、`artifact_type: test` → `test_code`、agent_slots を HELIX 7 role へ）
...
- **PLAN-001-c（別 PLAN として起票予定）**: `plan_schema.py` の `evaluate_g2_design_evidence` を HELIX §2 V-model 4 artifact 評価に再設計
- W6 `helix plan lint` CLI entrypoint shim（PowerShell / POSIX）— Python module までで止める
- W7 hook 連携（.claude/settings.json）
- W4 team runner との接続
- 全 PLAN template（impl / design 以外、recovery / poc / refactor 等は後続 wave）
- `helix` CLI の置き換え（porting 期間中は HELIX 流継続、`helix` 化は別 PLAN）
- mini-plan (MPLAN-NNN) サポート — HELIX 要件不在のため **完全削除、復活させない**
...
- skeleton 確認: `src/helix/__init__.py`, `pyproject.toml`, `src/helix/tests/__init__.py` の有無

### Sprint .2: skeleton 作成（最小）

- `src/helix/__init__.py`（空）
- `src/helix/tests/__init__.py`（空）
...
- 書き換え軸:
  - `from helix.*` / `from cli.lib.*` 相対 import を `from helix.*` 絶対 import へ
  - `.helix` リテラルを `.helix` へ（path defaults）
  - VALID_KIND / VALID_LAYER / VALID_DRIVE / VALID_STATUS を `requirements_v1.1.md` と差分突合
  - HELIX_PROJECT_ROOT 環境変数は HELIX_PROJECT_ROOT として残し HELIX 名は移植元コメントに留める
  - Windows path（大文字 / 小文字 mismatch）は realpath 比較ではなく `Path.resolve()` 統一

### Sprint .4: テスト移植

- vendor test 5 本を `src/helix/tests/` へ
- import path 修正、`.helix` リテラルを `.helix` へ
- fixture path（temp dir）は OS 中立に
- 追加: enum drift fixture（`requirements_v1.1.md` ↔ schema の整合 1 ケース）

### Sprint .5: 機械チェック + 全テスト

- `python -m py_compile src/helix/*.py`
- `python -m pytest src/helix/tests/ -q`
- 失敗 0 を確認
- `src/helix/plan_lint.py` を impl template に走らせて lint clean を確認

### Sprint .6: docs/templates 整備

- impl template と design template を `docs/templates/plan/impl/template.md` / `docs/templates/plan/design/template.md` へ copy + HELIX 主語化
...
- [ ] `src/helix/plan_{frontmatter,parser,schema,validator,lint,deps_helper}.py` 6 本が存在
- [ ] `src/helix/tests/test_plan_*.py` 5 本が port されており `pytest` が PASS
- [ ] `python -m py_compile src/helix/plan_*.py` が exit 0
- [ ] `python -m src.helix.plan_lint docs/plans/PLAN-001-w1-plan-schema-lint-port.md` が clean
...
- [ ] 1 commit、メッセージは `feat(plan-lint): port W1 plan schema and lint engine to src/helix`

## §6 carry 記録 / debt

- W7 hook 連携は別 PLAN（本 PLAN では `.claude/settings.json` を触らない）
- `helix` CLI shim は W6 PLAN で扱う
...
- **PLAN-001-c として別起票**: `plan_schema.py` の `evaluate_g2_design_evidence` を HELIX §2 V-model 4 artifact 評価器へ再設計（HELIX D-shard 命名規約からの脱却）
...
- ROLE_MAP.md 廃止は本 PLAN で確定 → HELIX 7 role enum (`po/tl/qa/aim/uiux/se/docs`) が `src/helix/plan_validator.py` に hardcode される。将来 role を増やすときは `requirements_v1.1.md §1.8` と同時更新する運用に切替え
- test_plan_frontmatter の vendor PLAN-001〜011 hash 固定 fixture 削除に伴う「legacy 保証なし」リスクは W1 commit で文書化（HELIX は vendor PLAN の bit-for-bit 互換を保証しない、独自テストで等価性を担保）
...
- `.claude/hooks/pretooluse-agent-guard.sh` は agent-guard 用途のみ enable。残 13 hook script は HELIX runtime (`cli/lib/helix-common.sh`, `helix.db`) 強依存のため未 enable、W7 で HELIX-owned hook 再実装時に整理
- `.claude/agents/*.md` 19 件は helix init 配置のままで動作確認済 (pmo-sonnet smoke PASS)、W7 で HELIX-spec の subagent 定義へ整理
- helix init が生成した `docs/design/L2-design.md`, `docs/requirements/L1-requirements.md`, `docs/features/PLAN-001/D-*` は HELIX template scaffolding で HELIX 経路では参照しない。本 commit には含めない、W7/W8 で整理または削除
