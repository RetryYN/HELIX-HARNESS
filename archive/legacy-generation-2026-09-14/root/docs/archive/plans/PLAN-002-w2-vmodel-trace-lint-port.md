...
generates:
  - artifact_path: src/helix/vmodel_lint.py
    artifact_type: python_module
  - artifact_path: src/helix/tests/test_vmodel_lint.py
...
W2 第2波。HELIX vendor (`vendor/helix-source/cli/lib/vmodel_*.py`) から V-model trace lint engine を `src/helix/` へ adapt port する。HELIX requirements_v1.1 §2.1-2.7 (4 artifact + 必須 8 directed edge + G3.8 + L6 QA 分離) と §7.3 (exit code 0/2/1) に整合させ、`helix vmodel lint` の核心となる検証 engine を確立する。

## §1 目的

W1 で凍結した plan_schema / plan_lint engine を踏み台に、V-model 4 artifact (① 設計 / ② 実装 / ③ テスト設計 / ④ テストコード) の双方向 trace 検証 engine を載せる。**Sprint .1 解析の結果、当初想定の vmodel_loader port は不要 (HELIX §2 に対応する semantic config が無く、§2 markdown 仕様で直接 lint 可能) と判明したため、scope を vmodel_lint.py 1 module に絞り rewrite する**。

- vendor `vmodel_lint.py` (9,890 B) を **count ベース → §2.4 必須 8 directed edge 個別検証へ rewrite**。`src/helix/vmodel_lint.py` に置く
- vendor `test_vmodel_lint.py` の構造を流用しつつ、HELIX §2.7 受入条件 (Pair freeze / 8 edge / kind dispatcher / L6 QA / 逆ピラミッド) を網羅する fixture へ rewrite
- vendor `cli/templates/state/vmodel.json` / `deliverables.json` を `docs/templates/state/` に port、L3.8 と L6 QA doc-first trace を追加

W2 範囲外として明示 carry:
- `helix vmodel lint` CLI binding は W6 (`helix doctor` / runtime detection) で総合実装
...
- vmodel_loader.py + `cli/config/vmodel-semantics.yaml` port は HELIX で対応 config が必要になった時点で `PLAN-002-d` として再評価 (現時点では §2 markdown 仕様で十分)
- vendor `deliverables.json` (HELIX V2 の 30 D-* deliverable × feature 単位 state) は HELIX では PLAN frontmatter `generates` が deliverable trace を担うため不要。`PLAN-002-e` として「HELIX V2 D-* 30 種を HELIX で正本化するか」の判断のみ carry

## §2 背景

- W1 で plan_lint / plan_validator が HELIX §1 仕様準拠で完成。次の必須 lint である vmodel_lint を載せないと G3 / G3.8 / G4 が機械検証できない
- HELIX vendor は 8 directed edge を 12 directed edge として実装しており、HELIX §2.4 の R-I5 fix (必須 8 + warn 4 の分離) に合わせて再実装が必要
- HELIX §7.3 R-I7 fix で exit code 3 段階 (0 / 2 / 1) が定義された。vendor は 2 段階 (0 / 1) なので exit code logic を rewrite
- HELIX §2.5 で L6 QA 追加テストが正本化された。vendor は QA 追加テスト分離概念を持たないため新規実装
...
- HELIX §2 / §7.3 との **drift table** 作成 (HELIX 12 edge → HELIX 8+4、HELIX 2-state exit → HELIX 3-state exit、HELIX QA 分離なし → HELIX §2.5 追加)
- 成果物: `docs/plans/PLAN-002-sprint-1-analysis.md`

### Step 2: Sprint .2 着手前調査 / skeleton

- W1 と同様、`src/helix/__init__.py` への新 module 追加方針確認
- `helix code find vmodel` 相当の流用候補確認 (W1 で port 済の `plan_validator.py` の exit code pattern を流用)
- skeleton 作成: `src/helix/vmodel_loader.py` (空 dataclass + load 関数 stub) + `src/helix/vmodel_lint.py` (exit code 関数 + check 関数 stub)
...
- vendor の grandfather 機構 (`deferred-findings.yaml` + `VMODEL_ENFORCEMENT_DATE = 2026-05-18` legacy) は **削除**、HELIX carry rule (P0/P1/P2/P3) で別途吸収
- vendor の path constant (`HELIX_ROOT` / `PLANS_DIR`) を `HELIX_PROJECT_ROOT` env / `.helix/plans/` へ rewrite
- DESIGN/IMPL/TEST_DESIGN/TEST_CODE pattern を vendor `cli/lib/*` → HELIX `src/...` / `tests/...` / `docs/design/<feature>/` / `docs/test-design/<feature>/` に rewrite
...
- `pytest src/helix/tests/` で W1 124 PASS + W2 追加分の全 PASS 確認
...
- 1 PLAN = 1 commit (`feat(W2): port HELIX vmodel trace lint to HELIX src/helix`)
...
- [ ] `src/helix/vmodel_lint.py` が py_compile clean
- [ ] `src/helix/tests/test_vmodel_lint.py` が pytest PASS
...
- **D4**: vmodel_loader.py + vmodel-semantics.yaml は HELIX §2 直接 lint で十分 → W2 scope から除外、PLAN-002-d carry
- **D5/D8**: kind 別経路分岐 + G1-G3 Pair freeze は vendor に無く HELIX で新規実装
- **D6/D7**: L6 QA 分離 + 逆ピラミッド検出は vendor に無く HELIX §2.5/§2.6 で新規実装
- **D3**: vendor grandfather + VMODEL_ENFORCEMENT_DATE は HELIX carry rule (P0/P1/P2/P3) で吸収、削除
...
- **W6 連携**: `helix vmodel lint` CLI binding は W6 で実装 (`helix doctor` と統合)
