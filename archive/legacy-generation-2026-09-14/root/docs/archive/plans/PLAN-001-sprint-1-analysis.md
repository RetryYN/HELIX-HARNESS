...
vendor/helix-source/cli/lib/plan_*.py 6 本 + tests 5 本 + templates 2 本 + HELIX requirements §1.2-1.8 enum 突合。Read / Grep / Glob のみ、ファイル変更なし。

## 1. enum drift サマリ

| enum | vendor 状況 | HELIX 要件 | drift 種別 | W1 対応 |
|---|---|---|---|---|
...
- `PLAN_ID_RE` が HELIX §1.10 形式 (`^PLAN-\d{3}(-[a-z0-9-]+)?$` / `^PLAN-MM-\d{3}$`) と非互換
...
- **`load_valid_roles()` が `cli/ROLE_MAP.md` を runtime read** → HELIX 移植時に最初に潰す必要 (P0)
- `_plan_search_directories` が module 配置から `parents[2]` で repo root 逆算 → src/helix 配置で path 階層が変わるため **`HELIX_PROJECT_ROOT` 環境変数 or 引数化** 必須
...
- `docs/plans/PLAN-*.md` glob は HELIX §1.10 と一致 → OK
...
|---|---|---|---|---|---|
| test_plan_frontmatter.py (191 行) | 4 + 11 parametrize | tmp_path, `FAIL_STAGE_ENV` | `parents[3]` repo root, **vendor PLAN-001〜011 body hash 固定** | `os.replace` 同名 lock 干渉懸念 | parametrize 11 件削除、HELIX 独自 fixture で再構築 |
...
横断問題:
- 全 test が `sys.path.insert(0, LIB_DIR)` で cli/lib を直接 import → src/helix 配置で全面書換え
...
- HELIX 主語化が必要な行: L21, L25, L29-32, L36-37, L41-43, L49, L57, L67, L75
...
- 本文 §3 Step 1-5 (Web 検索 / tl-advisor adversarial / ADR snapshot / PLAN 起票 / 双方向 trace) を HELIX §6 (GitHub 統制) と整合させて要減量
...
- mini-plan (MPLAN-NNN) サポート全体 → HELIX 要件不在のため **削除**
...
3. **`test_plan_frontmatter.py` の vendor PLAN-001〜011 body hash 固定**: HELIX 配置で `parents[3]` は別物。delete 推奨、削除前に「legacy 保証なし」リスクを文書化
