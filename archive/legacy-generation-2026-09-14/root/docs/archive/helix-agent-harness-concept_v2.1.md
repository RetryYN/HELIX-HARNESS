...
| 区分 | 件数 | 主な内容 |
|---|---|---|
| Critical(着手前ブロッカー) | 5 | `helix` CLI 統一(シェルラッパー) / Windows 対応 WSL2 必須化 / 3 層 YAML を設計仕様書化 / `scrum_reverse_lint` 仕様化 / layer×workflow_phase 排他 |
...
| `helix` CLI 形式 | **シェルラッパー方式**(`scripts/helix` ディスパッチャ + `scripts/helix-<sub>.sh`) |
...
`HELIX-HARNESS` はこの 4 問題に **3 つの実装経路 + 機械検証** で構造的に対処する。
...
└── harness-blueprint.md   # ← 本書(HELIX-HARNESS 構想書)
```
...
HELIX-HARNESS は **チーム開発で発生する全実装パターン** を 3 つの経路で網羅し、4 つの補助軸で支える:
...
PLAN doc の YAML frontmatter は **機械検証可能な 9 必須フィールド** + 7 軸の enum で構成する。`helix plan lint`(§12.1)が CI で fail-close 動作する。
...
`helix doctor --check vmodel`(§12.2)で機械検証。検証 logic は §4.7 / §12 参照。
...
[L3.5] 機能設計(tl)
       ① endpoint / 関数 schema
       ③ 単体テスト設計(pair freeze)★これが HELIX の核
...
各ゲートは `helix gate <G番号>` CLI(§12)で判定。
...
S4: Decide(tl)
    ...
    helix scrum decide --confirmed --reverse-merge
```
...
`helix scrum decide --confirmed --reverse-merge --scrum-type <type>` で matrix lookup → 自動的に `helix reverse <reverse-type> --from-scrum --scrum-hypothesis <id>` を起動。
...
`scripts/scrum_reverse_lint.sh` は `helix doctor` から呼ばれる。`scripts/lib/scrum_reverse_validator.py` の thin wrapper。
...
`docs/plans/PLAN-NNN-recovery-<incident-id>.md` には以下を必須記述(`helix plan lint` で検証):
...
| 2. carry 残件明記 | `.helix/audit/deferred-findings.yaml` に carry note 追加(空ならスキップ可) |
| 3. 認識訂正があれば memory 更新 | recovery kind PLAN を起票していれば自動 OK |
| 4. recovery kind PLAN draft 起票 | 認識訂正がある session(`.helix/audit/recognition-fixes-this-session` フラグ立っている場合)は recovery PLAN を必ず起票 |
...
[3] aim or se: 最小修正実装(PR 説明欄に hotfix 宣言)
       ↓
[4] helix plan create --kind recovery --incident <id>
...
jobs:
  feature-lint:
    ...
    steps:
      ...
      - run: bash scripts/helix-plan-lint.sh
...
**v2.1 変更**: v2.0 にあった `helix-doctor` job は削除(`harness-check.yml` に集約、§12.3 参照)。重複実行排除でコスト削減効果を確保。
      ...
      - name: PoC merge guard (fail-close)
        ...
        run: |
          if [ "${PR_BASE:-}" = "main" ]; then
            ...
            echo "確定後は helix scrum decide --confirmed --reverse-merge で Reverse 起動を実施"
            exit 1
          fi
...
# 検証 4 項目: 全部 checked
VERIFY_CHECKED=$(echo "$PR_BODY" | grep -cE '^\- \[x\] (テスト実行済み|`bash scripts/vmodel_lint\.sh`|`helix doctor`|既存テスト全パス)' || true)
...
repos:
  ...
  # ローカル: HELIX-HARNESS 固有検証
  - repo: local
    hooks:
      - id: helix-plan-lint
        name: helix plan lint
        entry: bash scripts/helix-plan-lint.sh
        language: system
...
**実行**: `helix gate G4`(§12)が `vmodel_lint` の結果 + カバレッジを取得し、本 YAML の `condition` を **設計参照しつつ bash で実装** する。`condition` 文字列の式言語パーサは導入しない。
...
scripts/check-escalation-level.sh が plan_id + failure_type を集計
  ...
  ↓
helix doctor が check-escalation-level を呼び、結果を表示
  ↓
...
## 10.7 失敗ログ(.helix/audit/failure_log.jsonl、v2.1 concurrency 対策)
...
**git 管理方針**: `failure_log.jsonl` は **git 管理対象**(チーム共有の audit trail)。`.helix/cache/` のみ `.gitignore`。
...
## 12.1 helix CLI 統一(v2.1 確定)

**`helix` CLI はシェルラッパー方式**(`scripts/helix` ディスパッチャ)。bootstrap 時に `PATH` に `<repo-root>/scripts` を追加する(`scripts/install-hooks.sh` で `.bashrc` / `.zshrc` への追記を案内)。

### scripts/helix(ディスパッチャ)
...
case "$SUB" in
  plan)
    ...
    case "$SUBSUB" in
      lint)     exec bash "$DIR/helix-plan-lint.sh" "$@" ;;
      create)   exec bash "$DIR/helix-plan-create.sh" "$@" ;;
      validate) exec bash "$DIR/helix-plan-lint.sh" "$@" ;;  # alias
      *)        echo "usage: helix plan {lint|create|validate}"; exit 1 ;;
    esac ;;
  scrum)   exec bash "$DIR/helix-scrum.sh" "$@" ;;
  reverse) exec bash "$DIR/helix-reverse.sh" "$@" ;;
  doctor)  exec bash "$DIR/helix-doctor.sh" "$@" ;;
  gate)    exec bash "$DIR/helix-gate.sh" "$@" ;;
  help|*)
    cat <<EOF
helix: チーム開発 AI ハーネス CLI
  helix plan {lint|create|validate}
  helix scrum {init|backlog|plan|poc|verify|decide}
  helix reverse {init|from-scrum} [--reverse-type <type>]
  helix doctor
  helix gate <G1-G11>
EOF
...
| CLI | 役割 | 委譲先スクリプト |
|---|---|---|
| `helix plan lint` | PLAN frontmatter 検証(§12.2 詳細)| `scripts/helix-plan-lint.sh` |
| `helix plan create --kind <kind>` | PLAN テンプレートから新規作成 | `scripts/helix-plan-create.sh` |
| `helix plan validate` | `helix plan lint` のエイリアス | 同上 |
| `helix scrum init --scrum-type <type>` | Scrum モード初期化 | `scripts/helix-scrum.sh` |
| `helix scrum backlog add --hypothesis <id> --title <t>` | 仮説追加 | 同上 |
| `helix scrum plan` | Sprint Plan | 同上 |
| `helix scrum poc --hypothesis <id>` | PoC 実装着手 | 同上 |
| `helix scrum verify` | 全 verify/*.sh 実行 | 同上 |
| `helix scrum decide --confirmed --reverse-merge` | S4 decide + Reverse 自動 routing | 同上 |
| `helix reverse <type> --from-scrum --scrum-hypothesis <id>` | Reverse R0 初期化 | `scripts/helix-reverse.sh` |
| `helix doctor` | 統合検証(§12.3)| `scripts/helix-doctor.sh` |
| `helix gate <G番号>` | ゲート判定(§5.3)| `scripts/helix-gate.sh` |

## 12.2 helix plan lint(frontmatter 検証、nullglob + 排他制約)

`scripts/helix-plan-lint.sh`:
...
if [ ${#PLANS[@]} -eq 0 ]; then
  echo "[helix-plan-lint] PLAN ファイル無し、PASS"
  exit 0
fi
...
## 12.3 helix doctor(統合検証、harness-check.yml 集約)

`scripts/helix-doctor.sh`:
...
# 1. plan lint(§12.2)
bash scripts/helix-plan-lint.sh
...
jobs:
  harness-check:
    ...
    steps:
      ...
      - name: helix doctor
        run: bash scripts/helix-doctor.sh
...
**v2.1 変更**:
- `feature.yml` の `helix-doctor` job を削除し本 workflow に集約(重複排除)
...
| 実行内容 | 時間目安 | 重い処理? |
|---|---|---|
| `helix-plan-lint`(frontmatter 検証)| < 1 秒 | × |
...
│
└── .helix/                                   # ハーネス状態(audit は git 管理、cache は gitignore)
    ├── VERSION
    ├── audit/
...
8. `.helix/` 配下初期化(`VERSION: 2.1.0` / `audit/failure_log.jsonl` 空 / `audit/deferred-findings.yaml` 空)
9. **実行権限付与**(macOS/Linux: `chmod +x scripts/*.sh scripts/helix`、**Windows: `git update-index --chmod=+x scripts/*.sh scripts/helix`** を `install-hooks.sh` で実行)
10. `bash scripts/install-hooks.sh` 実行(Python 依存 + pre-commit + 実行権限)
11. `helix doctor` 実行(全項目 PASS 確認)

### Phase 0-B: GitHub 反映

12. **初回コミットを main に直接 push**(`feat: HELIX-HARNESS 初期化 (bootstrap)`)
...
if [ "${OS:-}" = "Windows_NT" ] || [ "${OSTYPE:-}" = "msys" ] || [ "${OSTYPE:-}" = "cygwin" ]; then
  ...
  for f in scripts/helix scripts/*.sh scripts/lib/python_cmd.sh; do
    [ -f "$f" ] && git update-index --chmod=+x "$f" 2>/dev/null || true
  done
else
  chmod +x scripts/helix scripts/*.sh scripts/lib/python_cmd.sh
fi
...
これにより 'helix doctor' などのコマンドが利用可能になります。
...
| `scripts/helix` + `scripts/*.sh`(全件)| ★★★ | CLI + 機械検証 |
...
- [ ] `helix doctor` が PASS
- [ ] `helix plan lint`(PLAN ファイルが無くても PASS、`shopt -s nullglob` で空配列を許容)
...
- [ ] Windows(WSL2)から `bash scripts/helix-doctor.sh` が動作する
...
| **helix doctor** | 統合検証 CLI(plan lint + vmodel + governance + scrum/reverse + recovery 等)|
...
| **2.1** | **2026-05-20** | **TL レビュー第 1+2 回 計 29 件を反映。Critical 5 / High 3 / Medium 3 / Important 6 / Minor 6 / PM 見落とし 6 を解消。設計判断確定(WSL2 必須化 / YAML 設計仕様書化 / `helix` シェルラッパー統一 / pre-push 軽量化)** | **PM + TL** |

---

**本書は HELIX-HARNESS の単独完結設計書である。実装エージェント(Claude Code / Codex)はリポジトリ初期化時に本書 §14 を実装根拠として参照すること。**
