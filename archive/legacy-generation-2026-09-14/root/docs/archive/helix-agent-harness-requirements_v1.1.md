...
- [ ] 形式が `^PLAN-\d{3}(-[a-z0-9-]+)?$` または `^PLAN-MM-\d{3}$` に一致 (`helix plan lint` で正規表現検証)
...
- **validator 同期方針**: `src/helix/plan_validator.py` の VALID_* リテラルは本書 §1 表から手動同期。drift 検知のため、`src/helix/plan_validator.py` 冒頭に「最終同期: requirements vM.N §1.X」コメントを必須化、`helix doctor` が本書の更新日と validator のコメントを比較し 30 日以上乖離なら warning。
...
|--|--------------|-----------------|------------------|------------------------|-------------------|
| **`hypothesis-test`** | **Primary 推奨** | Alt 候補 | Alt 候補 | Alt 候補 | Alt 候補 |
| **`tech-spike`** | Alt 候補 | **Primary 推奨** | Alt 候補 | Alt 候補 | Alt 候補 |
| **`design-spike`** | Alt 候補 | **Primary 推奨** | Alt 候補 | Alt 候補 | Alt 候補 |
| **`perf-spike`** | Alt 候補 | Alt 候補 | **Primary 推奨** | Alt 候補 | Alt 候補 |
| **`security-spike`** | **Primary 推奨** | Alt 候補 | Alt 候補 | Alt 候補 | Alt 候補 |
| **`ux-spike`** | Alt 候補 | **Primary 推奨** | Alt 候補 | Alt 候補 | Alt 候補 |
...
| 4 | **次セッションへの引き継ぎメモ** | `.helix/handover/CURRENT.md` の `updated_at` が 24h 以内 | **warning** (exit 0、stderr) |
...
## 6.5 CODEOWNERS bootstrap 2-stage 手順
...
`helix` は **薄い OS 別ラッパー + Python core** で実装する。Windows / macOS / Linux の entrypoint は同一 Python core を呼び、OS 差分は wrapper 層に閉じ込める。Windows では PowerShell entrypoint を提供し、Git Bash が必要な既存 hook / shell script は明示的に bridge する。

```
scripts/
├── helix                    # POSIX / Git Bash ディスパッチャ
├── helix.ps1                # Windows PowerShell ディスパッチャ
├── helix-plan-lint.sh
├── helix-vmodel-lint.sh
├── helix-doctor.sh
├── helix-gate.sh
├── install-hooks.sh
├── install-hooks.ps1
...
PowerShell と POSIX shell の entrypoint は同じ `python -m helix.cli` を呼び、検証結果と exit code を一致させる。`scripts/` 配下は薄い entrypoint / installer / CI helper に限定し、validator や runtime 判定などの実体は `src/helix/` に置く。OS 片系だけが通る状態は Phase 0 受入不可とする。
...
`runtime.json` は generated state とし、Git 管理しない。`helix status --json` は `mode`, `available_runtimes`, `missing_runtimes`, `optional_adapters`, `enabled_commands`, `disabled_commands`, `next_action` を返す。
...
#### role × capability_class matrix の対応表
...
|---------|---------------|------------|
| `task classify` | `fast-checker` / rule-based | `L` / `XL` / confidence < 0.7 → `frontier-reviewer` review を要求 |
| `task estimate` | rule-based + `fast-checker` | risk_factor ≥ 1.6 または production impact → `frontier-reviewer` review を要求 |
| `skill suggest` | `fast-checker` / rule-based | missing required skill または vendor_candidate → `tl` review を要求 |
...
HELIX 由来 skill は `vendor/helix-source/` から直接実行しない。HELIX で使うものだけを `docs/skills/*.md` に **skill pack** として正本化し、`artifact_type=skill_doc` の PLAN 成果物として管理する。

## 7.7.1 移植候補 skill pack

| skill pack | 主な移植元 | HELIX での用途 |
|------------|------------|-----------------|
...
- `HELIX` 固有名、個人プロジェクト前提、WSL2 固定、絶対パスは HELIX 用語と相対パスへ置換する。
...
| session 断絶・認識ずれ | `.helix/handover/CURRENT.md` または `recovery` PLAN | pre-push は handover なしを warning、recovery 7 セクション欠落を exit 1 |
...
│   ├── helix-plan-lint.sh                       # A
│   ├── helix-vmodel-lint.sh                     # A
│   ├── helix-doctor.sh                          # A
│   ├── helix-gate.sh                            # A
│   ├── install-hooks.sh                          # A
...
**本書は HELIX-HARNESS の要件定義書である。構想 (WHY/WHAT) は `helix-agent-harness-concept_v3.0.md` を、各 enum・スクリプト・workflow YAML の実装詳細は将来の個別 PLAN-XXX 詳細設計を参照。**
