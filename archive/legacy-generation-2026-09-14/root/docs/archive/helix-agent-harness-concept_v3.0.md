...
[L3.5] 機能設計 (tl)
       ① endpoint / 関数 schema / ③ 単体テスト設計 ★HELIX の核
...
R4 で Gap を整理した後、HELIX-HARNESS の Forward 経路に接続。R4 outcome に応じて L1 (要件) / L2 (全体設計) / L3 (詳細設計) のどこに合流するかを決める。
...
HELIX 由来のスキル群は、個人プロジェクト用の原文をそのまま使わず、HELIX 向けの **skill pack** として `docs/skills/*.md` に正本化する。移植対象は「追加機能設計」「ドキュメント」「実装」「テスト」「Reverse」「運用」の単位に分け、各 skill pack は必ず workflow / harness / gate のどれに接続するかを明記する。
...
| **Scrum 6 種別** | `hypothesis-test` / `tech-spike` / `design-spike` / `perf-spike` / `security-spike` / `ux-spike` |
| **Reverse 5 種別** | `code` / `design` / `upgrade` / `normalization` / `fullback` |
...
**本書は HELIX-HARNESS の概念定義書である。受入条件・enum 詳細・Phase 0 受入条件は `helix-agent-harness-requirements_v1.1.md` を参照。実装詳細 (validator / workflow YAML / hook script) は将来の個別 PLAN-XXX で詳細設計する。**
