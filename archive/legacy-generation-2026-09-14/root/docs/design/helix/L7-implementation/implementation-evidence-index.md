...
|---|---|---|---|
| L6 unit oracle trace | `docs/test-design/helix/L6-pillar-unit-test-design.md` の `HU-PILLAR-*` 46 件 | `tests/vmodel-pair.test.ts` の L6/L7 oracle count assertion | L6 function contract と L7 unit oracle の接続を検査する。実 runtime claim の代替にはしない。 |
...
| setup/distribution implementation | `src/setup/**` (`buildCleanDistributionPlan`) / `src/cli.ts` / `src/lint/identifier-rename.ts` | `tests/setup.test.ts` / `tests/distribution-acceptance.test.ts` / `tests/identifier-rename.test.ts` | consumer setup と rename rehearsal は no-write / plan-only 証跡であり、release/tag publish や cutover apply ではない。 |
...
- 旧 state path から現行 `.helix` への cutover は L14 cutover 承認前なので L7 実装済み扱いにしない。
