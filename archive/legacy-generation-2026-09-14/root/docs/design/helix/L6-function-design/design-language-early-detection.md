...
- unknown gate が `ok: true` を返す。prototype 継承 property 名（`toString` 等）を gate id として渡しても同様に拒否する。
- CLI の `--gate` が scope 検証より前に評価される、または `--profile` / `--setup-smoke` / `--scope toolchain` との併用を黙って受理する。
- preflight の gate が shard 起動より後ろへ移動する。

検査 oracle は `U-DESLANG-013` / `U-DESLANG-014` / `U-DESLANG-015` / `U-DESLANG-016` / `U-DESLANG-017` の 5 件で固定する。
