...
const CURRENT_MAIN_SOURCE_HEAD = "e588b5378492b0e7336055b6cf2b2586f6059ba2";
const CURRENT_MAIN_CLI_BYTES = 717603;
const CURRENT_MAIN_SOURCE_BRANCH = "refactor/1697-cli-r00-clean";
...
describe("CLI-R00 throughput baseline", () => {
  ...
  it("U-CLIR00-005: Node versionまたはenvironmentが違うと比較できない", () => {
    ...
    expect(nodeDrift.condition.node_version).not.toBe(baseline.condition.node_version);
  ...
  it("U-CLIR00-006: source_head blobから構造snapshotを決定的に再抽出する", () => {
    ...
    expect(first.cli_bytes).toBe(CURRENT_MAIN_CLI_BYTES);
  ...
  it("U-CLIR00-008: frozen artifactが検証に通り、構造proxyがsource_head blobと一致する", () => {
    ...
    expect(artifact.source_head).toBe(CURRENT_MAIN_SOURCE_HEAD);
    expect(artifact.source_branch).toBe(CURRENT_MAIN_SOURCE_BRANCH);
    const snapshot = pinnedSnapshot(artifact.source_head);
  ...
  it("U-CLIR00-009: 未知keyと短縮HEADを拒否する", () => {
    ...
    shortHead.source_head = "e588b5378";
