...
describe("plan schedule lint (IMP-081)", () => {
  ...
  // PLAN-L7-673-reverse-fullback-scope-all-entry-validation: U-RFSCOPE-001..003
  it("U-RFSCOPE-001: required外 layerのinvalid decision", () => {
    const doc = reverseFullbackPlanDoc(
      "PLAN-REVERSE-198-extra-scope-invalid",
      [
        "docs/plans/PLAN-REVERSE-198-extra-scope-invalid.md",
        "docs/governance/helix-harness-requirements_v1.2.md",
      ],
      {
        extra: `${fullbackScope}  - layer: verification-design\n    decision: inferred\n    reason: "The decision vocabulary is intentionally invalid."\n`,
      },
    );

    const violation = analyzePlanGovernance([doc]).violations.find(
      (entry) => entry.reason === "reverse_fullback_scope_missing",
    );

    expect(violation?.detail).toContain("verification-design:invalid_decision");
  });

  it("U-RFSCOPE-002: required外 layerの未生成updated evidence", () => {
    const doc = reverseFullbackPlanDoc(
      "PLAN-REVERSE-198-extra-scope-invalid",
      [
        "docs/plans/PLAN-REVERSE-198-extra-scope-invalid.md",
      ...
      {
        extra: `${fullbackScope}  - layer: verification-design\n    decision: updated\n    evidence_path: docs/test-design/harness/not-generated.md\n    reason: "The evidence path is not generated by this PLAN."\n`,
      },
    );
    ...
    expect(violation?.detail).toContain("verification-design:missing_generated_evidence");
  });

  it("U-RFSCOPE-003: required外 layerのvalid not_impacted decisionは受理する", () => {
    const doc = reverseFullbackPlanDoc(
      "PLAN-REVERSE-198-extra-scope-valid",
      [
        "docs/plans/PLAN-REVERSE-198-extra-scope-valid.md",
        "docs/governance/helix-harness-requirements_v1.2.md",
      ],
      {
        extra: `${fullbackScope}\n  - layer: verification-design\n    decision: not_impacted\n    reason: "The existing verification contract is not changed."\n`,
      },
    );

    const reasons = analyzePlanGovernance([doc]).violations.map((entry) => entry.reason);
