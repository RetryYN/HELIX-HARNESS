...
current answer: current projection scope では automatic であり、`helix doctor` により hard-gated される。
...
|---|---|---|---|
| PLAN schedule shape | automated | `helix doctor` -> `plan-schedule - OK` | current PLAN section shape では `Closed` |
| V-model pair freeze | automated | `helix vmodel lint`, doctor `pair-freeze` | document pair links では `Closed` |
