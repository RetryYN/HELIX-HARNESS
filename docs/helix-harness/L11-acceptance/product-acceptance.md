...
- HARNESS-L2-002：リリースカンバンと開発方式を組み合わせた対象で、リリースカンバンが順序と優先を決め、各リリース単位の中は選んだ開発方式で進むことを確認する。リリースカンバンを「開発方式の複数選択」として拒否する判定、リリースカンバンだけで開発方式を選ばずに工程を進める入力、順序と優先の根拠がないリリース単位の着手を不成立とする。
