...
|---|---|---|---|
| HARNESS-L2-010 | HARNESS-L2-002 | 「ticketはHELIX-OSの推進が導いて発行する作業の単位とする。」を満たす結果を確認できる | HARNESSがticketを発行する |
| HARNESS-L2-011 | HARNESS-L2-008 | 「HARNESSは、OSとBRAINがticketを導くためのコアを持つ。」を満たす結果を確認できる | OSまたはBRAINにコア定義を移す |
| HARNESS-L2-012 | HARNESS-L2-008 | 「コアは、工程の語彙・順序・停止・差戻し・完了条件と、落としてはならない工程義務（必要な層と対、成果物、oracle、人の判断が要る場所、戻し先）を、HELIX-JSONの定義と、JSONどうしの意味をつなぐPythonの意味導出コアとして提供する。」を満たす結果を確認できる | 列挙した工程義務のいずれかを欠く |
| HARNESS-L2-013 | HARNESS-L2-008 | 「BRAINはコアから判断する」を満たす結果を確認できる | BRAIN判断・OS発行・検収の責務を混同する |
| HARNESS-L2-014 | HARNESS-L2-002 | 「HARNESS自身はticketを発行しない。」を満たす結果を確認できる | HARNESSがticketを発行する |
| HARNESS-L2-015 | HARNESS-L2-002 | 「開発方式が変わっても、コアの工程義務を落とさない。」を満たす結果を確認できる | 開発方式変更時に工程義務を落とす |
| HARNESS-L2-016 | HARNESS-L2-008 | OSがticketとその中の動的ワークフローを導いて発行した結果を確認できる | HARNESS自身が発行する、またはBRAINの判断を発行済みにする |
| HARNESS-L2-017 | HARNESS-L2-005 | 検収がticketから必要な検証を導いた結果を確認できる | ticketの内容を見ずに固定CIだけで検収する |
