...
- HARNESS-L2-002／003：`ProvisionalVProjection`をcanonical traceやrelease判定へ使う入力、SR4 receiptなしの`CanonicalVPublication`、4 entityを単一進捗値へ縮退する入力を拒否する。Design Refactorでobservable behaviorを変える入力と、baseline／budget／workload／回帰oracleを先に固定しないPerformance Refactorを拒否する。
