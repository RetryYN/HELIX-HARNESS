...
（FR-L1-13 Forward workflow、FR-L1-21 review evidence）。`helix review --uncommitted` が entry point
...
- `helix claude --role code-reviewer` 経由で code-reviewer subagent を dispatch する。
...
```
helix claude --role code-reviewer --task "review PLAN-<id> at trace-freeze" --execute
```
