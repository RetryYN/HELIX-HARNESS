...
- `tests/` に single behaviour（単一の振る舞い）を exercise する `describe` / `it` block を 1 つ書く。
  `npm run test` を実行し、test が **fails**（Red）することを確認する。
...
- `npm run test` を再実行し、既存 tests がまだ pass し、new test が Green になったことを確認する。
- `npm run typecheck` と `npm run lint` を実行する。new violations は許可しない。
...
- 各 structural change（構造変更）後に full suite
  （`npm run typecheck && npm run lint && npm run test && helix doctor`）を再実行する。
...
- [ ] `npm run test` が 0 で終了し、PLAN-linked rationale の無い `.skip` または `.todo` が残っていない。
- [ ] `npm run typecheck` exits 0.
- [ ] `npm run lint` が exit 0（Biome check: format + lint。`biome lint` 単体ではない）。
...
- `npm run test` ではなく `vitest` の native runner を直接実行する。
