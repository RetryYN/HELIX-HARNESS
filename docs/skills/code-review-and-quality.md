...
W-gate のテスト観点品質確認と標準 code review（コードレビュー）を統合する複合レビュー手順を扱う。
FR-L1-21（cross-agent review 証跡）、FR-L1-03（descent obligations）、
FR-L1-18（cross-detection aggregation）の品質要件を満たすために使う。
PLAN が implementation layer（実装層 L7）と test design layer（テスト設計層 L6/L8）をまたぐ場合、
...
| W-gate | Design side（設計側） | Test side（テスト側） | Accept 条件 |
|--------|-------------|-----------|-----------------|
...
**Step 2 — テスト実体（test substance）監査:**
...
- mock scope（mock 範囲）が最小限である。integration path では full database mock ではなく、
  実体のある test double を使う（FR-L1-03 descent obligation）。

**Step 3 — layer obligation（層責務）確認:**

変更された各 module について、V-model sibling set（兄弟成果物群）が揃っていることを確認する。
