...
- dependency 変更後に `helix guardrail` が non-zero で終了する。
...
- [ ] `helix guardrail` が 0 で終了する。committed file に API key pattern、session token、
...
- [ ] 新しい global environment variable を導入する場合は、variable name、purpose、
      ...
      `HELIX_` prefix を使う。
- [ ] Hook entry point は package-local の `helix` command だけを呼ぶ。
...
- [ ] `docs/`、`.helix/handover/`、`.helix/audit/` 配下の新規 file に PII
...
```
.helix/audit/<PLAN-id>-hardening.json
{
  "plan_id": "<id>",
  "gate": "accept | L12",
...
- `helix guardrail` を sprint 末だけ実行する。
  `docs/`、`.helix/`、`src/` に触れる commit のたびに実行する。
