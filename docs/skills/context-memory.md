...
`.helix/handover/CURRENT.json` と session log digests を使って sessions 間の continuity を維持する方法、
...
`helix handover` は `.helix/handover/CURRENT.json` を書く。この file は次を含む。
...
両方の outputs を session-close evidence として `.helix/audit/<session-id>-close.txt` に capture する。
handover narrative だけに依存しない。

## Session-start procedure（開始時手順）

1. `.helix/handover/CURRENT.json` が存在し stale でない場合に読む
   （timestamp が約 24 時間以内）。
2. 各 `carry` item を `git log` と current file state で verify する。
   既に commit 済みの item を re-report しない。
3. `open_plans` を `helix doctor` output と照合する。
...
`helix db rebuild` を実行し、current `docs/plans/` と `.helix/` on-disk state から harness state を re-project する。
...
- drive-cycle boundary で `helix handover` を skip する。
