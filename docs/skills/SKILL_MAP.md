...
2. `helix skill suggest --current-location --summary-json` — DB 現在地と駆動モデルから推奨 pack の順位を確認する。
...
| テストケース設計 / 壊れ方の発見 / テストの止めどき判断 | test-thinking |
| 受入条件（AC）設計 / Done 判定 / accept gate の実体検査 | acceptance-criteria-thinking |
| 実装前の要否判断 / 依存追加 / ハードコード回避 / 差分最小化 | code-minimalism |
| design doc の採否 / 粒度 / 決定の記録先の判断 | design-tailoring |
| skill pack / 委譲ブリーフ / review rubric の作成・改修 | skill-authoring |
...
| Reverse R0-R4 / RGC の分析 | reverse-analysis, reverse-r0, reverse-r1, reverse-r2, reverse-r3, reverse-r4, reverse-rgc |
