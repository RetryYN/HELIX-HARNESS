...
## HELIX 内部実装の strangler-fig 段階

```
Phase 0  旧 path は稼働中。新 path は HELIX_* opt-in flag の背後に置く
Phase 1  新 path を default にする。旧 path は warn-deprecated を log する
Phase 2  旧 path を削除し、helix doctor asset-drift を green にする
Phase 3  deprecation notice / compat shim を削除する
```
