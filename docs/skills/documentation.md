...
V-model design docs や ADRs とは別の、人間が読む operational documentation を作る場合に適用する
...
- 繰り返し使う operational procedure（例: PAT rotation、`harness.db` rebuild、doctor triage）の runbook を作成する。
...
- **実行可能な example.** すべての code block は、current codebase に対して実際に動く command または output にする。
...
1. **目的** — 1 段落。この component が何を行い、どの system に仕えるか。
2. **前提条件** — `bun`、`ut-tdd`、外部 dependency と minimum version。
...
2. **影響** — condition が続く間に何が壊れる、または degraded するか。
