...
- `helix doctor` check、CI failure、runtime error が defect と確認済み
...
spec と矛盾する source を、spec 更新なしに fix しない。これは `helix doctor` が検出できない可能性のある
descent gap を作る。
...
- fix が public API または `.helix/` state structure に触れる場合、PLAN close 前に
