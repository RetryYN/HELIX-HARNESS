...
evidence はその仮定を覆す必要があり、覆い隠してはいけない。特に probing する failure mode は次のとおり。

- **中身のない coverage。** `ut-tdd doctor` が green で `ut-tdd vmodel lint` が pass しても、
...
1. `ut-tdd review --uncommitted` に blocking finding が無い。
