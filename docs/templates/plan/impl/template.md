...
# confirmed TDD PLAN では、既存の test path または解決可能な oracle_id と kill/fail/red 実測を記録する。
# mutation_oracle_evidence: "tests/<module>.test.ts::U-<DOMAIN>-001 が seeded defect を kill（exit 1）"
...
- 参照 docs: `docs/governance/helix-harness-requirements_v1.2.md`、`docs/adr/ADR-001-helix-harness-redesign-and-language.md`

## §6 用語更新 (living glossary delta)

当該工程で新規導入 / 精緻化したユビキタス言語を記録し、L0 §10 用語集 (`docs/governance/helix-harness-concept_v3.1.md`) へ back-merge する (独自定義禁止、anti-corruption layer / 要件 §1.10.G.9)。
