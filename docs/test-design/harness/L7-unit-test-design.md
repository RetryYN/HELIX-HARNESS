...
| U-SETUP-037 | `runHelixProjectSetup` / `bootstrapProjectPackageLockfile` | fresh consumer repo で `ut-tdd setup project` を wet run した場合、`package.json` と `bun.lock` を生成し、`bun install --lockfile-only` を package root で実行する。生成 `package.json` は `scripts.ut-tdd="ut-tdd"`、`scripts.typecheck="bun run ut-tdd status --json"`、`scripts.test="bun run ut-tdd completion review-bundle --json"`、`devDependencies.ut-tdd=github:unison-ai-product/UT-TDD_AGENT-HARNESS-Pack#v0.1.0`、`devDependencies.typescript=^5.6.3`、`private=true` を持つ。`consumerReadiness.ok=true`、`cliResolution.packageScriptAvailable=true`、`postSetupWorkflow.nextRoute=ready` を返し、VSCode task / CI が package-local `bun run ut-tdd ...` を実行できる surface を持たない fresh repo を ready にしない。 |
