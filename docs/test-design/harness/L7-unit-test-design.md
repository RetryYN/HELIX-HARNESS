...
| U-HOVER-017 | `ut-tdd handover status --json` / `readPointer` CLI preflight | `CURRENT.json` 不在なら exit 1 + `handover: CURRENT.json not found`、生成や補完はしない。`ut-tdd handover --plan X` で生成後、`handover status --json` は `active_plan/status/generated_by/latest_doc/outstanding.completionReadiness` を JSON で返す。text mode は active/status/updated_at/latest_doc を返す。provider handover status とは別 surface で、通常 handover の `.ut-tdd/handover/CURRENT.json` を読む。 |
...
- **handover PLAN-L7-88 + status preflight 増分 (capWithBreadcrumb / summary cap / `ut-tdd handover status --json`) → U-HOVER-016〜017** (context 注入 cap とセッション開始時の通常 handover CURRENT.json read-only preflight。孤児 0)
