...
| U-MCPPROFILE-015 | `catalogVerificationProfiles` right-arm metadata | external / test-foundation profiles carry `recommendedGates` / `recommendedDrives` so G8-G14 evidence profile selection is not detached from the profile catalog. Browser profiles map to G10/G11 and `fe` / `fullstack` / `agent`; Testcontainers/MSW/MCP Inspector map to G8/G9; doctor/GitHub read-only map to G12-G14. |
| U-MCPPROFILE-016 | `analyzeRightArmVerificationProfileCoverage` drive L10 hard gate | `fe` / `fullstack` / `agent` always require L10; removing all G10 browser profile mappings for those drives returns `missing-drive-g10-profile` and `missing-right-arm-gate-profile`. `be` / `db` remain `ui_only` but expose the same G10 profile list when UI exists. |

> **Status (PLAN-L7-33, 2026-06-11; PLAN-L7-79, 2026-06-19; PLAN-L7-226, 2026-07-02)**: U-MCPPROFILE-001..016 promoted to green `it` in `tests/verification-profile.test.ts` against `src/lint/verification-profile.ts` — catalog/profile metadata, Docker MCP Toolkit readiness metadata, generated local MCP config rendering (incl. tokenized launcher argv, U-MCPPROFILE-013), launcher readiness probing (U-MCPPROFILE-014), right-arm gate / drive L10 metadata (U-MCPPROFILE-015/016), safety findings, and activation planning are pure and do not install packages, enable servers, run external tools, or write committed MCP config.
