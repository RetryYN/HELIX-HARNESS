...
| U-HOVER-004 | `renderHandoverScaffold` | **純関数**。§6.8.5 の 6 セクション (①-⑥) を含む / `outstanding` 未指定の後方互換経路では③-⑥に `TODO(human)` placeholder / `outstanding` 指定時は③Next Action を workflowNextActions 由来の marker + PLAN/route/packet commands で seed し、⑤未了 PO 判断を outstanding marker で seed する / **具体 oracle: 入力 `HandoverDoc.plans[0].summary` に `token=secret123` を含めると出力は `secret123` を含まず `token=***` を含む (render 時 `sanitize` 適用の defense-in-depth、tracked md への流出ゼロ)** |
...
| U-HOVER-020 | `renderHandoverScaffold` / `checkHandoverNextActionAnchor` / doctor hard gate | `outstanding` 指定時、§3 Next Action は `HANDOVER_NEXT_ACTION_MARKER`、PLAN ID、required action、next workflow route、primary packet、supporting packet commands を出し、`TODO(human): 順序付き次手` を残さない。最新 handover entry の §3 に marker があれば OK。§3 欠落、latest_doc 読取不能、または §3 が TODO のままなら fail-closeし、`runDoctor.ok` に連動する。複数 entry では最新 entry の §3 だけを見る。 |
