...
| U-SETUP-006 | `applyBranchProtection` | `apply≠true` → `{applied:false, reason:"emit-only"}` (gh 呼ばれない) / `apply=true` でも action-binding approval が無ければ `{applied:false, reason:"approval-required"}` で gh を呼ばない / approval 後に `gh auth status` が失敗すれば `{applied:false, reason:"gh-auth-required"}` / approval、`gh auth`、repository admin 権限が揃えば `gh api -X PUT repos/{owner}/{repo}/branches/main/protection` を呼び、`harness-check` required status、admin enforcement、review count 1 を適用して `{applied:true, reason:"applied"}` を返す。token/secret は state/docs/log に保存しない |
| U-SETUP-007 | `runSetup` (orchestration) | ①フラグあり→フラグ値採用 / ②フラグ無し+対話→confirm 結果 / ③フラグ無し+非対話→`0-A` (fallback) / ④`apply=true` は action-binding approval が無い場合 `approval-required` で gh auth/admin preflight 前に停止する / CLI text は親 `helix setup` を legacy solo/team adapter setup と表示し、HELIX project bootstrap は `helix setup project`、親 setup は VS Code task / `.helix` project baseline / rename packet を生成しないため L14 completion evidence ではないことを表示する |
...
| U-SETUP-021 | `runHelixProjectSetup` | `helix setup project --apply-branch-protection` は、action-binding approval が無い場合 `approval-required` で停止し、`gh auth status` や remote branch protection apply へ進まない。承認済み evidence がある場合だけ gh auth/admin preflight 後に remote apply できる。 |
...
fail-close する。
| U-SETUP-022 | `loadTemplates` / `BUILTIN_GITHUB_TEMPLATES` / `runHelixProjectSetup.consumerReadiness` / `runConsumerDoctor` / `tests/distribution-acceptance.test.ts` | legacy `helix setup` が生成する `team/setup-branch-protection.sh` と built-in fallback は `HELIX_BRANCH_PROTECTION_APPROVAL_RECORD`、`action_binding_approval_record` check、`gh auth status`、`gh api -X PUT`、`/branches/main/protection`、`harness-check` context、admin enforcement、review count 1 を含む approval-bound apply-capable script である。setup artifact readiness と consumer doctor は token/secret 永続化を禁止しつつ、script が approval record + gh auth/admin preflight 付きで remote apply できることを `consumer-branch-protection-script` で検査する。distribution acceptance は clean artifact から生成された consumer repo の script 実体を読み、approval-bound apply-capable 境界を確認する。 |
...
| U-SETUP-037 | `loadBranchProtectionApprovalEvidence` / `runSetup` / `runHelixProjectSetup` | `--apply-branch-protection` の apply surface は `HELIX_BRANCH_PROTECTION_APPROVAL_RECORD` の action-binding approval record（実在 + `action_binding_approval_record` marker）で解錠され、gh auth/admin preflight を経て `applied` に到達できる。env 未設定・record 不在・marker 欠落は `approval-required` で fail-close し、marker 無し record の偽装承認を通さない（apply surface 到達不能の再発防止、scripts/setup-branch-protection.sh と同一契約）。 |
