...
| U-CXHOOK-012 | `CODEX_NOT_APPLICABLE` / `CODEX_DEFERRED_SURFACE` | `subagent-stop` is the only true N/A; Codex agent-guard remains a documented deferred surface, not an accidental omission. |
| U-CXHOOK-013 | `evaluateWorkGuard` / `evaluateAgentGuard` | Shared guard logic is runtime-agnostic: the same pure functions block foreign edits and non-allowlisted subagents regardless of whether the caller is Claude or Codex. |
