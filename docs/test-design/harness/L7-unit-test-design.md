...
| U-OUTSTANDING-014b | `helix closure review-bundle --summary-json` / `helix closure transition-plan --summary-json` / Project view summary catalog | Project closure review-bundle summary は `total`、`listed`、`omitted`、`window` だけでなく、`current_window_command`、`previous_window_command`、`next_window_command`、`transition_window_command`、`full_source_command` を持つ。Project closure transition-plan summary も `current_window_command`、`previous_window_command`、`next_window_command`、`full_source_command` を持つ。343 件など複数 window の close-ready approval を PO がページ単位で確認する際、offset 値を手計算させず、current / next / previous / transition plan へ機械的に遷移できることを保証する。summary catalog の `closure-review-bundle` surface は action 明示の `helix closure review-bundle --action close_ready --summary-json`、`closure-transition-plan` surface は action + decision 明示の `helix closure transition-plan --action close_ready --decision approve_closure_claim --summary-json` を source とし、full JSON は `full_source_command` に限定する。window command 欠落、action/decision 省略、raw `--json` を通常 command field に混入させる、または summary catalog の source command drift は fail-close する。 |
