...
| U-DECISIONREC-011 | `buildIdentifierRenameCutoverPlan` / `ut-tdd rename plan` | `cutoverCategoryChecklist[]` は category / hits / files だけでなく、代表 `samplePaths[]` と category 別 `verificationCommand` を持つ。`verificationCommandMatrix[]` は baseline、targeted-regression、static-gates、state-and-doctor、full-regression、compiled-dist-smoke の各 phase、command、expected、evidence を持つ。text mode は checklist 件数と verification command 件数を出し、承認前 dry-run evidence が自由文だけに戻らない。 |
| U-DECISIONREC-012 | `buildVersionUpActivationPackets` / `ut-tdd version-up activation-packet` | `Version-up source ledger` は `OWASP Web Security Testing Guide` row を持ち、security testing が `external_rehearsal_plan` / `dry_run_plan` / `activation_provenance_requirements` に接続される。activation packet は `activationVerificationCommandMatrix[]` を持ち、activation-packet-baseline、version-dry-run、external-rehearsal、security-testing、state-and-doctor、targeted-regression、static-gates、full-regression、approval-packet の各 phase、command、expected、evidence を出す。text mode は verification command 件数を出し、version-up activation の承認前 evidence が自由文だけに戻らない。matrix は `activationSnapshot.evidenceDigest` に含まれ、承認前 review material drift で snapshot が変わる。 |
| U-DECISIONREC-013 | `buildS4DecisionPackets` / `ut-tdd s4 decision-packet` | S4 decision packet は `decisionEvidenceChecklist[]` / `outcomeRouteMatrix[]` だけでなく、`decisionVerificationCommandMatrix[]` を持つ。matrix は decision-packet-baseline、source-ledger-freshness、s3-verification-evidence、requirements-trace、targeted-regression、static-gates、full-regression、completion-frontier の各 phase、command、expected、evidence を出す。text mode は evidence check 件数、outcome route 件数、verification command 件数を出し、PO/S4 判断前の検証 evidence が自由文だけに戻らない。 |
| U-DECISIONREC-014 | `buildActionBindingApprovalPackets` / `ut-tdd action-binding approval-packet` | action-binding approval packet は `approvalBindingChecks[]` だけでなく、`approvalVerificationCommandMatrix[]` を持つ。matrix は approval-packet-baseline、sibling-decision-packets、least-privilege-binding、snapshot-binding、security-boundary、targeted-regression、static-gates、full-regression、completion-frontier の各 phase、command、expected、evidence を出す。text mode は binding check 件数と verification command 件数を出し、GitHub Environments required reviewers / NIST least privilege / OWASP LLM06 の意味を actor/tool/target/params/snapshot/expiry/audit の承認前 evidence に接続する。approvalAllowed=false と approvalCommandAvailable=false は固定で、matrix は承認・apply surface を作らない。 |
