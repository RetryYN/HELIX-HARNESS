...
| U-VISUAL-003 | `buildVisualizationSnapshot` + VSCode Project Tree View | `close_ready` の `.helix/tmp/closure/close_ready-decision-draft.yml` は recovery handoff artifact として検出され、`reviewed_count`、`approval_scope_digest`、`pending_human_review` / `approve_closure_claim` / rejection outcome を read-only に表示する。Tree View は artifact path / generation command / scope status を copy pointer と tooltip に出すが、承認や apply は実行しない。 |
