...
| U-SETUP-007 | `runSetup` (orchestration) | ①フラグあり→フラグ値採用 / ②フラグ無し+対話→confirm 結果 / ③フラグ無し+非対話→`0-A` (fallback) / ④`apply=true`+非対話→`non-interactive` / ⑤`apply=true`+対話+admin+confirm でも approval 無しなら `action-binding-approval-required` で止まり remote apply しない / CLI text は親 `helix setup` を legacy solo/team adapter setup と表示し、HELIX project bootstrap は `helix setup project`、親 setup は VS Code task / `.helix` project baseline / rename packet を生成しないため L14 completion evidence ではないことを表示する |
...
| U-SETUP-021 | `runHelixProjectSetup` | `helix setup project --apply-branch-protection` は、対話 session / admin / confirm が揃っていても action-binding approval 入力を持たない限り remote branch protection を適用しない。result は `{applied:false, reason:"action-binding-approval-required"}` を返し、`gh auth status` / `gh api -X PUT` に進まない。 |
