...
| U-ID | 検証対象 | oracle (DbC) |
|---|---|---|
| U-SOURCELEDGER-001 | `sourceLedgerCheckedDateViolation` + `analyzeRightArmVerificationStrategy` | `Verification source ledger (checked YYYY-MM-DD)` が 90 日超 stale の場合、右腕 verification strategy は source ledger violation として fail-close し、G8-G14 pass claim の根拠にしない。 |
