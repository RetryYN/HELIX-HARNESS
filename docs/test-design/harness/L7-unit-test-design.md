...
| U-OUTSTANDING-014b | `helix closure review-bundle --summary-json` / `helix closure transition-plan --summary-json` / Project view summary catalog | Project closure review-bundle summary は `total`、`listed`、`omitted`、`window` だけでなく、`approval_window_count`、全 page の offset/range/review command/transition command を列挙する `review_window_index`、window 単位の `review_scope`、全件の `aggregate_review_scope`、`current_window_command`、`previous_window_command`、`next_window_command`、`transition_window_command`、`full_source_command` を持つ。Project closure transition-plan summary も `current_window_command`、`previous_window_command`、`next_window_command`、`full_source_command` を持つ。343 件など複数 window の close-ready approval を PO がページ単位で確認する際、offset 値を手計算させず、current / next / previous / 任意 page / transition plan へ機械的に遷移でき、かつ window 内証跡量と全件証跡量を混同しないことを保証する。summary catalog の `closure-review-bundle` surface は action 明示の `helix closure review-bundle --action close_ready --summary-json`、`closure-transition-plan` surface は action + decision 明示の `helix closure transition-plan --action close_ready --decision approve_closure_claim --summary-json` を source とし、full JSON は `full_source_command` に限定する。window command 欠落、aggregate 欠落、action/decision 省略、raw `--json` を通常 command field に混入させる、または summary catalog の source command drift は fail-close する。 |
| U-OUTSTANDING-014c | `helix progress tree-view --summary-json` / Project frontier summary | Project view summary は `project_frontier_summary` を top-level に持ち、`current`、`drive_model`、`closure_frontier`、`vmodel_fit`、`skill_binding`、`commands` を read-only で返す。`closure_frontier` は close-ready review の `total` / `listed` / `omitted` / `approval_window_count`、全 page の `review_window_index`、window 単位の `review_scope`、全件の `aggregate_review_scope`、current / previous / next / transition window command を持ち、`vmodel_fit` は current-location / recovery runway / recovery handoff / approval review gate を同じ summary 内で参照できる。`skill_binding` は selected model、workflow/layer、required/recommended/optional count、top item summary を返すが、全 skill item や closure queue 全件は展開しない。Project view から現在地・駆動モデル・承認 frontier・skill injection 導線を一度に機械検出できる一方、承認 record 作成や apply は実行せず、raw `--json` は `full_*` field 以外に混入させない。 |
