...
| U-SETUP-006 | `applyBranchProtection` | `apply≠true` → `{applied:false, reason:"emit-only"}` (gh 呼ばれない) / `apply=true` で `gh auth status` が失敗すれば `{applied:false, reason:"gh-auth-required"}` / `gh auth` と repository admin 権限が揃えば `gh api -X PUT repos/{owner}/{repo}/branches/main/protection` を呼び、`harness-check` required status、admin enforcement、review count 1 を適用して `{applied:true, reason:"applied"}` を返す。token/secret は state/docs/log に保存しない |
| U-SETUP-007 | `runSetup` (orchestration) | ①フラグあり→フラグ値採用 / ②フラグ無し+対話→confirm 結果 / ③フラグ無し+非対話→`0-A` (fallback) / ④`apply=true` は `applyBranchProtection` の gh auth/admin preflight を経由し、preflight 失敗時は reason 付きで停止、成功時だけ remote apply する / CLI text は親 `helix setup` を legacy solo/team adapter setup と表示し、HELIX project bootstrap は `helix setup project`、親 setup は VS Code task / `.helix` project baseline / rename packet を生成しないため L14 completion evidence ではないことを表示する |
...
| U-SETUP-021 | `runHelixProjectSetup` | `helix setup project --apply-branch-protection` は、`gh auth status` と repository admin 権限を preflight し、成功時だけ remote branch protection を適用する。result は `{applied:true, reason:"applied"}`、または `gh-auth-required` / `admin-permission-required` などの reason 付き fail-close を返す。 |
...
fail-close する。
| U-SETUP-022 | `loadTemplates` / `BUILTIN_GITHUB_TEMPLATES` / `runHelixProjectSetup.consumerReadiness` / `runConsumerDoctor` / `tests/distribution-acceptance.test.ts` | legacy `helix setup` が生成する `team/setup-branch-protection.sh` と built-in fallback は `gh auth status`、`gh api -X PUT`、`/branches/main/protection`、`harness-check` context、admin enforcement、review count 1 を含む apply-capable script である。setup artifact readiness と consumer doctor は token/secret 永続化を禁止しつつ、script が gh auth/admin preflight 付きで remote apply できることを `consumer-branch-protection-script` で検査する。distribution acceptance は clean artifact から生成された consumer repo の script 実体を読み、apply-capable 境界を確認する。 |
