...
| U-DECISIONREC-011 | `buildIdentifierRenameCutoverPlan` / `ut-tdd rename plan` | `cutoverCategoryChecklist[]` は category / hits / files だけでなく、代表 `samplePaths[]` と category 別 `verificationCommand` を持つ。`verificationCommandMatrix[]` は baseline、targeted-regression、static-gates、state-and-doctor、full-regression、compiled-dist-smoke の各 phase、command、expected、evidence、source、sourceUrl を持つ。text mode は checklist 件数、verification command 件数、`verification-source:` を出し、承認前 dry-run evidence と source が自由文だけに戻らない。 |
