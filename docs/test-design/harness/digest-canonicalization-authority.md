...
| U-DIGEST-004 | 移行4 consumerの公開API digestがhardcoded oracleとbyte-for-byte一致する | `tests/digest-consumer-compatibility.test.ts` |
| U-DIGEST-005 | production hitがtyped variantまたは非移行理由を持つ | `tests/digest.test.ts` |
| U-DIGEST-006 | handover resurrectionの純lint境界を維持する | `tests/digest.test.ts` |
