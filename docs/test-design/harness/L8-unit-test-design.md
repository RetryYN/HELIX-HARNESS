...
| U-FLIFE-014 | torn write 復旧 | 最終 JSON 行の任意 byte 位置で切れた journal を、次の同一 operationId retry で全 receipt が valid 行として存在し `damaged` が空になるまで収束させる (1 回の write は atomic ではない) | `tests/feedback-lifecycle.test.ts` |
