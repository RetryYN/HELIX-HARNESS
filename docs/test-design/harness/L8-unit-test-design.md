...
| U-CARRY-014 | terminal | draftの未解決を許しterminal化でfail | `tests/left-arm-carry-log.test.ts` |
| U-CARRY-015 | legacy boundary | backdateとbaseline改変によるpresence回避をfail | `tests/left-arm-carry-log.test.ts` |
| U-CARRY-016 | wiring | real repo、schema、doctor、G7/trace-freeze配線をpin | `tests/left-arm-carry-log.test.ts`, `tests/slow/doctor.test.ts` |
