...
| active PLAN選択整合性 | PLAN-L6-68 / `docs/design/harness/L6-function-design/active-plan-selection.md` | `U-APSEL-001..005`。exact canonical ID受理、截断候補提示、registry検証不能fail-close、全writer集約、watermark以後orphan 0を固有testへ降下する |
...
| U-APSEL-004 | writer集約 | CLI/commit hookのinvalid activationでmarker不変、exact IDだけ更新 | `tests/session-log.test.ts` |
