...
| Claude headless adapter completion | PLAN-RECOVERY-42 / `session-log.md` §8 | `U-ADAPTER-012/013`。execute限定settings/marker、headless Stop wake無副作用、Claude限定worker budget由来hard timeout、`provider_timeout`型を`tests/runtime-adapter.test.ts`と`tests/runtime-hook-entrypoints.test.ts`が担い、7 mutationをkillする |
...
| U-ADAPTER-012 | headless completion分離 | Claude executeだけが`user,project` settingsとheadless markerを持ち、marker付きwakeはstate/generation/claimを0件のまま終了する。dry-run、Codex、通常VS Code wakeは不変 | `tests/runtime-adapter.test.ts` / `tests/runtime-hook-entrypoints.test.ts` |
| U-ADAPTER-013 | bounded provider lifecycle | Claudeだけでsealed `budget.time_ms`をtimeout＋`SIGKILL`へ写像し、`ETIMEDOUT`を`provider_timeout`へ型付けする。Ubuntu fake processの無限holdを250msでkillし、wrapper復帰、未到達sentinel、session_endを実測する。Codexはtimeout非適用、不正Claude budgetは起動前に拒否し、CLI spawn接続を破壊するmutationもRedにする | `tests/runtime-adapter.test.ts` / `tests/runtime-hook-entrypoints.test.ts` / `tests/tools/claude-headless-completion-mutation/run-mutation.ts` |
