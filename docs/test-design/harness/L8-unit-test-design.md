...
| session handover retirement | PLAN-L6-61 / `handover-retirement.md` §8 | `U-HRET-001..015`。typed disposition、phase journal、移管reconcile、at-least-once delivery、旧surface不存在・復活検出、action-binding approvalを下表の単体oracleへ降下する |
...
| PLAN固有Vペア4点binding | PLAN-L6-65 / `plan-descent-specific-parent-binding.md` §5 | `U-PSPB-006..020`。PLAN ID・L6 parent・L8 oracle・生成test pathを同一tupleへ結合し、path存在だけの偽Vペアをfail-closeする |
