...
| U-IHIER-013 | dependency migration dry-run | active hierarchy正本からadd／replace、exact `depends_on`／`blocks`、既存PLAN束縛を保持したmigration candidateを決定的に投影し、既に一致するIssueを除外する | `tests/issue-hierarchy.test.ts` |
