...
| U-IHIER-013 | dependency migration dry-run | active hierarchy正本からadd／replace、exact `depends_on`／`blocks`、PLAN束縛、parser往復可能なcanonical contract blockを持つmigration candidateを決定的に投影し、既に一致するIssueとscalar／set競合を拒否する | `tests/issue-hierarchy.test.ts` |
