...
| U-IHIER-005 | PR focus / repository full audit | PR focusはclosure graphから到達するdependency componentだけを検査して無関係driftを隔離する。main/scheduledのfocusなし監査は全採用Issueを検査し、missing PLANをfail-closeする | `tests/issue-hierarchy.test.ts`、`tests/harness-check-workflow.test.ts` |
