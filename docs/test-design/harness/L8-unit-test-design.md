...
| Claude headless adapter completion | PLAN-RECOVERY-42 / `session-log.md` §8 | `U-ADAPTER-012/013`。execute限定settings/marker、headless Stop wake無副作用、direct／pair／loopのClaude限定worker budget由来hard timeout、`provider_timeout`型を4 test fileが担い、9 mutationをkillする |
...
| U-ADAPTER-013 | bounded provider lifecycle | Claudeだけでsealed `budget.time_ms`をtimeout＋`SIGKILL`へ写像し、`ETIMEDOUT`を`provider_timeout`へ型付けする。Ubuntu fake processの無限holdを250msでkillし、direct CLI、pair-agent phase、loop workerの有限復帰と未到達sentinelを実測する。Codexはtimeout非適用、不正Claude budgetは起動前に拒否し、3 spawn接続を破壊するmutationもRedにする | `tests/runtime-adapter.test.ts` / `tests/runtime-hook-entrypoints.test.ts` / `tests/pair-agent.test.ts` / `tests/orchestration/loop-bridge.test.ts` / `tests/tools/claude-headless-completion-mutation/run-mutation.ts` |
