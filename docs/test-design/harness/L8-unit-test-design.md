...
| SessionStart 予算収束 | PLAN-L7-471 / `orchestration-memory.md` / `feedback-lifecycle.md` | `U-SSBUDGET-001..006`。hook 経路からの full lifecycle reconcile 分離、batch append による全 ref receipt 維持、stdout/stderr 経路分離、`session_start` / memory recall の先行確定、`helix feedback reconcile` による保守本体を `tests/session-start-budget.test.ts` が担う |
