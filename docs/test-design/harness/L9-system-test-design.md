...
| ST-DATA-06 | continuation read model（data §8 / architecture §5） | clean起動で旧CURRENT/prose不在を保ち、DB projectionを優先してnext actionを表示。crash後replayは冪等 | L9 IT-CONT-01..04 + 後続L7 resurrection detector（現R1 inventory gateを完了証拠にしない） |
...
| ST-ARCH-05 | session/prose handover廃止境界 | legacy command/module/CURRENT schema/HandoverPanelの復活をhard failし、provider delegation evidenceとoperations transitionは原本digest/provenance/query可用性を保つ別型として保持 | 後続L7 resurrection detector + integration fixture。R4までは設計oracleであり実装greenを主張しない |
...
> 個別 FR の AC レベル受入は L12 受入テスト (AT-*) が担う。L9 は **複数 FR/module をまたぐ統合挙動**を対象 (L12 との責務分界)。駆動モデルの状態遷移 pseudocode / CLI signature（CLI 署名）は L4 §3.6 で L5/L6 へ defer のため、ST-FUNC は **system 粒度（システム粒度）の遷移成立・合流先・優先度・サインオフ**を対象 (関数粒度の単体は L7 U-* が担う)。
...
| ST-UI-04 | FE descent 鎖 | L2 画面設計 → L4 ui-standard/tokens → L7 src/web → L10 UX 磨きの層配置が drift しない | frontend-design-coverage / sub-doc-catalog-drift |
