...
related_l4_tokens: docs/design/harness/L4-basic-design/tokens.yaml
related_plan_l4_internal: docs/plans/PLAN-L4-10-internal-asset-master.md
related_plan_l4_ui_standard: docs/plans/PLAN-L4-14-ui-standard.md
next_pair_freeze: L4
...
# HELIX-HARNESS — L10 システム/Real UX evidence 検証設計 (④ / ST-*)
...
> **PLAN**: `docs/plans/PLAN-L4-{01..04,14}-*.md` の pair_artifact / DoD で本書参照

## §0 量閉じ原則 (L4 ↔ L9)

L4 基本設計の各設計要素が L10 システム/Real UX evidence 検証 (ST-*) で被覆されること (孤児 = 0)。

- **data.md**: 4 集約の不変条件 (§6) / continuation read model / 集約間整合 (§7) / state schema (§8) → 整合性 ST 必須
...
- **ui-standard.md / tokens.yaml**: UI 設計標準 / UI 部品カタログ / design tokens / a11y 目標 / L2→L4→L7→L10 descent → 画面横断 ST 必須
- 孤児 = 0 (機械検証は L7 で `helix vmodel lint` / trace check に接続)
...
| ST-DATA-06 | continuation read model（data §8 / architecture §5） | clean起動で旧CURRENT/prose不在を保ち、DB projectionを優先してnext actionを表示。crash後replayは冪等 | L9 IT-CONT-01..04 + U-HRET-003..007/013 |
...
| ST-ARCH-05 | session/prose handover廃止境界 | legacy command/module/CURRENT schema/HandoverPanelの復活をhard failし、provider delegation evidenceとoperations transitionは原本digest/provenance/query可用性を保つ別型として保持 | U-HRET-008..014 + IT-CONT-03/04。complete checkpoint後のenforceをsystem acceptanceとする |
...
> 個別 FR の AC レベル受入は L12 運用/価値検証 (AT-*) が担う。L9 は **複数 FR/module をまたぐ統合挙動**を対象 (L12 との責務分界)。駆動モデルの状態遷移 pseudocode / CLI signature（CLI 署名）は L4 §3.6 で L5/L6 へ defer のため、ST-FUNC は **system 粒度（システム粒度）の遷移成立・合流先・優先度・サインオフ**を対象 (関数粒度の単体は L7 U-* が担う)。

### §1.3.1 ST-ASSET (function.md §1.1 由来 — 内部資産 roster/command system 挙動、A-85)

> L4=L10 システム/Real UX evidence 検証粒度で**書ける範囲のみ**。関数粒度 (各 subcommand signature / capability resolver アルゴリズム) は仕様未確定のため **placeholder + 依存明示** (back-fill 対象、§4)。

| ST-ID（候補） | 検証対象 (function §1.1) | 想定シナリオ (system 粒度 / システム粒度) | 機械検証 (carry) |
|---|---|---|---|
...
| ST-ASSET-03 | 内部資産 command end-to-end (FR-L1-48) | `helix roster list/check` / `helix asset` が system として動く | vitest CLI 統合 |
| ST-ASSET-05 | skills building block (FR-L1-47、architecture §3) | 実装済み L7 evidence（証跡）は `src/workflow/contracts.ts#catalogSkills`、`src/workflow/contracts.ts#recommendSkills`、`src/workflow/contracts.ts#suggestSkillInjection`。projection-backed assets は `src/skills/recommend.ts` と `src/state-db/projection-writer.ts` へ続く。 | `tests/workflow-contracts.test.ts` + vitest skills + dependency lint |
| ST-ASSET-06 | 内部資産 drift lint (FR-L1-49、architecture §4.1) | 現行 hard gate slice は実装済み: legacy source/path residue / legacy runtime delegation residue / docs-skills vacancy / guard allowlist missing agent docs → fail-close。Roster capability resolution は ST-ASSET-07 evidence で被覆する。 | vitest `asset-drift` rule (implemented) |
| ST-ASSET-07 | roster↔guard 整合 (Critical-2、function §1.1) | 実装済み L7 evidence: `src/runtime/agent-slots.ts#resolveRosterCapability` は provider credential なしで roster snapshot から role/capability を解決し、guard allowlist drift は `src/lint/asset-drift.ts` で fail-close され続ける。 | `tests/agent-slots.test.ts` U-FR-L1-46 + `tests/asset-drift.test.ts` |
| **ST-ASSET-04 (placeholder（仮置き）、欠番は意図的)** | **各 subcommand / skill recommender / drift 判定の関数仕様** | 実装済み asset-drift slice については、L6 が function-level signature / U-* oracle を持つ。未実装の roster / skill recommender / command surface に残る system-test detail（総合テスト詳細）は **implementation-detail carry（実装詳細の持ち越し）** であり、Phase 2 blocker ではない。 | A-118 carry: L7/L9 back-fill when roster/skills command surfaces materialize |
...
### §1.5 ST-UI (ui-standard.md / tokens.yaml 由来 — FE 設計標準 / UI部品 / design tokens)

| ST-ID（候補） | 検証対象 (ui-standard.md / tokens.yaml) | 想定シナリオ | 機械検証 (carry) |
|---|---|---|---|
| ST-UI-01 | token SSoT / hardcode 禁止 | src/web 実装が `tokens.yaml` を唯一出典にし、部品や画面に独自色・寸法を持たない | frontend-design-coverage + UI lint (L7/L10) |
| ST-UI-02 | UI 部品カタログ / 状態網羅 | L2 `ui-element` 由来部品が props/state/event と ok/warn/error/empty/loading の状態網羅を持つ | component-derived tests (L7) |
| ST-UI-03 | a11y / visual 一貫性 | StatusBadge 等が色だけに依存せず、contrast / keyboard / read-only + CLI copy 制約を満たす | a11y/VRT profile (L10) |
| ST-UI-04 | FE descent 鎖 | L2 画面設計 → L4 ui-standard/tokens → L7 src/web → L10 システム/Real UX evidenceの層配置が drift しない | frontend-design-coverage / sub-doc-catalog-drift |
...
- **function.md §1.1 C12 内部資産 roster/command (FR-L1-46/48) → ST-ASSET-01〜03 / architecture §3 skills (FR-L1-47) → ST-ASSET-05 / architecture §4.1 drift lint (FR-L1-49) → ST-ASSET-06/07 実装済み evidence (`src/runtime/agent-slots.ts`, `src/workflow/contracts.ts`, `src/lint/asset-drift.ts`)**
- external-if.md §3 境界 4 / §4 degradation → ST-EXT-01〜04
- ui-standard.md / tokens.yaml → ST-UI-01〜04 (token SSoT、部品状態網羅、a11y/visual、FE descent 鎖)
- **孤児 (設計要素で ST 未被覆) = 0** を L9 本起票で機械確認する。現行 hard evidence は pair-freeze orphan 0、実装済み asset-drift slice、L7 roster/skill/command contract evidence である。本書に active な ST-ASSET L7 carry は残っていない。

## §3 trace (④ → ②)

本書の各 ST-* は `docs/design/harness/L4-basic-design/{data,architecture,function,external-if,ui-standard}.md` と `tokens.yaml` の設計要素と相互 reference する。**G4 (基本設計ゲート)** で L4 sub-doc/asset 全体 ⇔ 本書 1 doc の pair 宣言を確定し、双方向 trace freeze は G7 (trace freeze) で実施する (L3↔L12 と同型)。
