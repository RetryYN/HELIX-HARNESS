...
> **status**: confirmed (G4/A-101 freeze — ST カテゴリ ⇔ L4 設計要素の被覆を凍結、孤児 0)。個別 ST ケース (Given-When-Then) は検証 band (L8-L14) / L9 本起票で展開する。旧 ST-ASSET roster/skill carry row は、現在は実装済み L7 evidence で被覆済み。
...
| ST-ASSET-05 | skills building block (FR-L1-47、architecture §3) | 実装済み L7 evidence は `src/workflow/contracts.ts#catalogSkills`、`src/workflow/contracts.ts#recommendSkills`、`src/workflow/contracts.ts#suggestSkillInjection`。projection-backed assets は `src/skills/recommend.ts` と `src/state-db/projection-writer.ts` へ続く。 | `tests/workflow-contracts.test.ts` + vitest skills + dependency lint |
...
| **ST-ASSET-04 (placeholder、欠番は意図的)** | **各 subcommand / skill recommender / drift 判定の関数仕様** | 実装済み asset-drift slice については、L6 が function-level signature / U-* oracle を持つ。未実装の roster / skill recommender / command surface に残る system-test detail は **implementation-detail carry** であり、Phase 2 blocker ではない。 | A-118 carry: L7/L9 back-fill when roster/skills command surfaces materialize |
