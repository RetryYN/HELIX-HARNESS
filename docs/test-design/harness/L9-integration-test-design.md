...
pre-cutover shadowではfresh/brownfield/command/distribution全projectionをnamespaced scanし、新規finding、
distribution source欠落、actual path衝突をdoctor hard failへ束縛する。旧token 0のpost-cutover greenは未達である。
