...
| IT-CONT-03 | repoに旧command/module/schema/panel名または`handover/CURRENT.json`生成fixtureを混入する | 後続L7で実装するresurrection detectorをdoctor経由で実行する | resurrection findingがhard failになる | CLI/module/schema/docs/UI boundary | finding code、offending path、exit non-zero | 現R1 inventory gateのgreenを本oracleの実装証拠にしない。provider delegation evidence/operations transitionは誤検出せず、retirement前後のcount/digest/provenance/query可用性を保持する |
