...
| IT-CONT-03 | repoに旧command/module/schema/panel名または`handover/CURRENT.json`生成fixtureを混入する | resurrection detectorをdoctor経由で実行する | complete checkpoint後はfinding 1件でもhard failする | CLI/module/schema/docs/UI boundary | finding code、offending path、exit non-zero | provider delegation evidence/operations transitionは型・digest・nonjoin条件付きで誤検出せず、retirement前後のcount/digest/provenance/query可用性を保持する |
...
| IT-FLIFE-003 | group-firstで多数のopen feedback sourceが同一表示groupへ束ねられ、lifecycle journalとDB projectionが存在する | SessionStart feedback surfaceを実行する | group表示の件数上限にかかわらず全source refのreceiptを単一batchで記録し、同一sessionの再表示は0、次sessionでは再表示する | runtime→lifecycle journal→SQLite projection boundary | receipt件数、`readEvents`/lock/resolve各1回、projection 1回、30秒以内の完走 | stale generation、operation intent conflict、append途中失敗は部分的greenを返さずrecoveryまたはdiagnosticへrouteする |

実装binding（PLAN-L7-416 Sprint 2〜3）: `IT-CONT-01/02`のruntime↔real SQLite↔memory/feedback join、
fsync JSONL、crash replay、DB rebuild、同sequence異payload、strict parserは
`tests/continuation-event-first.test.ts`へcitationする。CLI/Stop/`plan complete` production routeはevent append→DB projection→bounded memoryの順序とし、`IT-CONT-04`で旧surface不存在を検証する。
`IT-CONT-03`のdoctor負例は`tests/handover-resurrection.test.ts`へbindingし、
baseline同時改ざん、R4 provider evidence削除、allowlist schema/content spoofもdoctor fail-closeへ束縛する。
post-complete production enforceとfresh/brownfield/distributionを同じacceptance matrixで検証する。
fresh/brownfield/command/distribution全projectionをnamespaced scanし、residual finding、distribution source欠落、actual path衝突をdoctor hard failへ束縛する。旧token 0をpost-cutover greenの必要条件とする。
