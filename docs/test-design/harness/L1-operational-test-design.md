...
- 全 FR-L1-* (**51 件** = FR-L1-01〜51) / OT 対応必須 (孤児 = 0。軽量原則で代表 OT へ集約可。FR-L1-36/38/43 は BR-21 派生として OT-18/OT-40 に接続し、FR-L1-51 = artifact progress color は OT-44 色分けへ集約)。**量閉じ = functional 正本 51 件**（FR-L1-36/38/43 は PLAN-L7-53 で登録・実装済みのため除外しない）
...
- **孤児 = 0** (各カテゴリ末尾「孤児 X = 0」を §3 で宣言。機械検証 = `helix plan lint --gate G1-trace`)

補足:
- NFR / UX / 成功条件は OT を共有または専用 OT で被覆し、§3 で対応を一覧する。
- 「合否目安」は L14 運用で観測する pass 条件の方向であり、数値しきい値の確定は L3 AC / L12 運用/価値検証へ送る。
...
| **OT-02** | BR-02 | AI agent roster (人間 = PO 1 名は承認のみ、実装/レビュー/検証 = 別ランタイム/別モデル AI、worker≠verifier) が日常 PR を出し、gate・レビュー・役割境界が無理なく回る | PR が gate/レビュー経路に乗り、creation≠judgement の役割逸脱 (worker=verifier 兼任) が検知される |
...
| **OT-09** | NFR-01 | harness の主要コマンド・hook が **開発者本人の単一プラットフォームで第一級ネイティブ動作**する (bash/python3 等の環境依存に阻まれない)。他 OS は移植性 goal として動作 (team 全体の多 OS 同時第一級保証は不要、solo narrow) | 本人環境で主要経路が第一級動作 (例: subagent guard は bun で環境非依存動作) / 他 OS は goal 水準 |
...
| **OT-15** | PM-01〜PM-06 (PM 画面群全般) | L1 画面要求として宣言した PM カテゴリ 6 画面 (PM-01 俯瞰ダッシュボード / PM-02 工程ビュー / PM-03 Gate+詰まり要因 / PM-04 Trace+V-pair / PM-05 Continuation / PM-06 設計書ビューア) が L2 モックで lift され、PO が「これで業務要求を満たす」と判定できる (PM-06 固有挙動は OT-47) | L2 mock で PO 判定 pass |
...
| **OT-18** | BR-21 (AI 実行成果評価) | business sub-doc §1 に BR-21「AI 実行成果の継続評価と改善サイクル」が宣言されており、BR-21 派生 FR-L1-36/38/43 との接続が §7 対応表に明記されていることを確認 | BR-21 宣言が business sub-doc に存在 / §7 pair 表に FR-L1-36/38/43 接続が明記 / 不実装宣言なし |
...
| **OT-20** | business §4 ステークホルダー権限分離 (solo+autonomy、charter §3) | business sub-doc §4 が **PO(本人)=L0–L3 承認 gate + 不可逆 escalation / AI roster verifier=L4+ gate(worker≠verifier) / AI worker=commit/merge 自律** の権限境界を定義し、**L4 以降に human per-gate サインオフが無い**（charter §3 と整合）ことを確認 | §4 が solo+autonomy 権限境界を明示 / L4+ human signoff 0 / 空白ロールなし |
| **OT-21** | business §6.5 業務 KPI D-01〜D-09 | business sub-doc §6.5 の KPI 表 (D-01〜D-09) を観測し、KPI 計測式・目標値・計測場所 (helix doctor / dashboard / audit log) が整合しており、NFR-13 gate 通過率 ≥90% と矛盾しないことを確認 | D-01〜D-09 全件に計測式・目標値・計測場所が記載 / NFR-13 と矛盾なし |
...
| **OT-23** | FR-L1-44 (途中導入 onboarding workflow) | 既存リポジトリ (helix 未導入) に対して `helix onboarding` を実行し、baseline PLAN セットが生成され、欠損 sub-doc が skip_sub_doc で段階整備モードに設定されることを確認 | baseline PLAN が生成 / skip_sub_doc 設定が有効 / onboarding 進捗がダッシュボードで可視 |
...
| **OT-27** | FR-L1-40 drive 別 state 区画 integrity | drive 別 state 区画 (`drive-be` / `drive-fe` 等) が technical sub-doc §4 に記載されており、skip_sub_doc 機械強制の検証スクリプトが helix doctor でパスすることを確認。区画跨ぎ汚染が検出 → block されることを代表ケースで観測 | drive 別 state ディレクトリ分離が機械確認 / 区画跨ぎ書き込みが block / skip_sub_doc 違反が doctor に集約 |
| **OT-28** | FR-L1-42 AI provider 引継ぎ (Claude ↔ Codex provider evidence) | Claude から Codex への provider 切替で `.helix/handover/provider/CURRENT.json` の provider evidence package が生成・検証され、PLAN registry / audit log の連続性が保たれることを確認する。session continuation は別契約として `harness.db` projection から復元され、provider evidence を continuation SSoT として読まないことも観測する | provider evidence status が green / PLAN registry 状態が切替後も一致 / DB continuation projection と audit log に中断・再開 record が存在 / session 用 CURRENT を生成しない |
...
| **OT-30** | NFR-13 (gate 通過率 ≥90% KPI 計測) 観測タイミング | D-07 (gate 通過率) の計測が `helix doctor` 実行後の audit log / dashboard で観測可能であり、NFR-13 の ≥90% 目標値と KPI D-07 の計測式が整合することを確認 | audit log に gate_pass_rate が記録 / dashboard で D-07 が可視 / NFR-13 目標値と KPI 定義が矛盾なし |
| **OT-31** | NFR-14 (agent guard bypass = PO 承認 + audit) 監査ログ取得 | `HELIX_ALLOW_RAW_AGENT=1` bypass 実行時に S-03 (bypass 権限 = PO 専属) チェックと B6 (agent guard bypass 監査) の audit log が生成されることを確認。bypass 理由が会話/final report に記録されており、監査ログが `.helix/audit/` に永続されることを観測 | bypass 実行時に audit log 生成 / S-03 権限チェック pass / B6 連動で audit エントリ存在 / log が `.helix/audit/` に永続 |
...
| **OT-40** | HM-08 (AI 効果データ + Learning Engine: BR-21 派生機能、L1 宣言確認) | HM-08 AI 効果データ + Learning Engine ビューが screen sub-doc に宣言されており、BR-21 派生 FR-L1-36/38/43 との接続が §7 対応表に明記されていることを確認。L1 では宣言接続を確認し、詳細な関数契約・oracle は L6/L7 に委譲する | HM-08 宣言が screen sub-doc に存在 / BR-21 派生 FR-L1-36/38/43 が §7 に明記 / 詐称なし |
...
| **OT-42** | §3 横断原則「AI 自律 + 人間 residue」確認 (charter §3) | 全 15 画面 (PM-01〜06 / HM-01〜08 / GD-01) で AI 指示 copy UI が存在し、S-01「AI は UI 操作なし」と整合することを確認。AI が UI を直接操作する導線が実装上 0 件であることを観測。人間 (本人/PO) の判断点が **L0–L3 承認 ＋ 不可逆 escalation ＋ 異常時 override (residue)** に限定され、L4+ は AI verifier 通過の UX フローであることを確認 (not-implemented) | AI 指示 copy UI が全 15 画面に存在 / AI 直接 UI 操作の導線 0 / 人間判断点 = L0–L3+escalation に限定 |
...
| ② AI roster (worker≠verifier) が日常 PR で gate | OT-02 |
...
### business サブ文書 (BR-*) — `docs/design/harness/L1-requirements/business-requirements.md`
- BR-01→OT-01 / BR-02→OT-02 / BR-03→OT-03 / BR-04→OT-04 / BR-05→OT-05 / BR-06→OT-06 / BR-07→OT-07 / BR-08→OT-08 / BR-22→OT-46。**孤児 BR = 0**。
- BR-21 (AI 実行成果評価) → **OT-18** (宣言確認 + BR-21 派生 FR-L1-36/38/43 接続確認)。
- business §3.3 9 mode 統一合流原則 + Add-feature 例外 → **OT-19** (concept §2.5 整合確認)。
- business §4 ステークホルダー権限分離 (solo+autonomy: PO=L0–L3 / AI roster verifier=L4+ / worker=自律) → **OT-20** (権限境界確認)。
- business §6.5 業務 KPI D-01〜D-09 → **OT-21** (KPI 計測式・目標値・計測場所 整合確認)。**孤児 BR = 0**。

### functional サブ文書 (FR-L1-*) — `docs/design/harness/L1-requirements/functional-requirements.md`
...
- FR-L1-36/38/43 (BR-21 派生) → **OT-18/OT-40** (BR-21 宣言確認と画面接続。詳細な関数契約・oracle は L6/L7 に委譲)。
- 詳細な機能要件レベルの受入条件は L3 で FR-* + AC-* として詳細化 (本書は L1 軽量、量閉じ確認のみ)。**孤児 FR-L1 = 0** (FR-L1-01〜51 = 計 51 件、全被覆)。

### screen サブ文書 (SR-*) — `docs/design/harness/L1-requirements/screen-requirements.md`
...
- §3 横断原則「AI 自律 + 人間 residue」(charter §3) → **OT-42** (全 15 画面 AI copy UI 確認)。
...
- BR-22 内部資産 HELIX 化 → **OT-46** (FR-L1-46〜49 / W6/W7/W10/W11/W12/W16 / IMP-033 trace、runtime 無修正転用 0)。
- 画面個別の受入観点は L10 システム/Real UX evidence + L11 受入/human visual で詳細化。**孤児 SR = 0** (15 画面全被覆)。

### technical サブ文書 (TR-*) — `docs/design/harness/L1-requirements/technical-requirements.md`
...
- AI provider 引継ぎ (FR-L1-42、technical §5 追記) → **OT-28** (provider evidence と DB continuation の境界整合確認)。
- 具体的なアーキ整合は L4 基本設計 / L10 システム/Real UX evidence 検証で確認。**孤児 TR = 0**。

### nfr サブ文書 (NFR-*) — `docs/design/harness/L1-requirements/nfr.md`
...
本書の各 OT は `../../design/harness/L1-requirements/{business,functional,screen,technical,nfr}-requirements.md` の 5 sub-doc 内の BR-*/FR-L1-*/SR-*/TR-*/NFR-* と相互 reference する (v1.2 で 5 sub-doc 構造に再編、構想書 §3.1.2.1)。G1 (業務要求ゲート) で **5 sub-doc 全体 ⇔ 本書 1 doc** の pair freeze を PO サインオフで確定する。具体しきい値・受入条件は L3 AC / L12 運用/価値検証設計へ送る。
