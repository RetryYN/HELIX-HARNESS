...
- 全 FR-L1-* (**48 件** = FR-L1-01〜35 + 37/39/40/41/42/44/45/46/47/48/49/50/51) / OT 対応必須 (孤児 = 0。軽量原則で代表 OT へ集約可。FR-L1-51 = artifact progress color は OT-44 色分けへ集約)。**FR-L1-36/38/43 のみ上記から除外** (BR-21 派生の L3 forward carry。L14 時点では OT-18 で宣言確認のみ、FR 詳細化は L3) → G1-trace の孤児誤検知対象外。**量閉じ = functional 正本 51 件 − L3 carry 3 件 = 48**（PLAN-L1-06 Step 5 Codex review で FR-L1-51 反映、旧 47 件は stale）
...
| **OT-36** | HM-01 (機能一覧: FR-L1 48 件 × implementation_status 全件表示) | HM-01 機能一覧ビューで FR-L1-01〜35/37/39/40/41/42/44/45/46/47/48/49/50/51 の 48 件が implementation_status (not-implemented / design-only / implemented / deprecated) 付きで全件表示されることを確認。孤児 FR・status 欠落行が 0 件であることを観測 (not-implemented) | FR-L1 48 件が全件表示 / implementation_status 列が存在 / 孤児・欠落 0 |
...
- 詳細な機能要件レベルの受入条件は L3 で FR-* + AC-* として詳細化 (本書は L1 軽量、量閉じ確認のみ)。**孤児 FR-L1 = 0** (FR-L1-01〜35 + 37/39/40/41/42/44/45/46/47/48/49/50/51 = 計 48 件、全被覆。FR-L1-36/38/43 は L3 carry)。
...
- HM-01〜HM-08 (HM 画面群 8 画面):
  - HM-01 機能一覧 → **OT-36** (FR-L1 48 件 × implementation_status 全件表示)。
