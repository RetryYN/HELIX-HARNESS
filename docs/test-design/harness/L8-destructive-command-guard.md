...
| U-SAFETY-001 | narrow delete fence | repo内の静的な単一ファイル削除はpassし、recursive/複数/glob/変数/repo外をblockする。`sudo -u`等の引数付きoption、`timeout` / `nohup` / `nice` / `ionice` / `setsid` / `stdbuf` / `time` / `doas` prefix、`bash -lc`等の結合short flagでも判定を維持する | `tests/machine-safety-guard.test.ts` |
