...
failure oracleはin-memory fakeだけでなくtemp filesystemのread-only directory、append failure、remove failureを使う。
raw command中のsecret/pathがaudit bytesへ現れないことも検査する。
audit schemaの全field、partial/torn record、fsync failure、nonce ledger restartをfault injection対象にする。
