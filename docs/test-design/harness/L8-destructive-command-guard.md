...
| U-GITGUARD-006 | failure injection | SQLite transaction/commit、consume、crash、retryの各failureでexit 2、audit失敗時marker保持、二重allow 0を証明する | `tests/guard-override-transaction.test.ts`、`tests/git-command-guard.test.ts` |
