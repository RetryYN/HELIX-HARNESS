...
> **granularity correction (2026-06-08)**: resolved。各 IT-* は §5 で GWT、fixture、module boundary setup、mock/adapter 条件、期待 assertion、negative/edge coverage を備える。§1/§2 は §5 が upgrade する candidate skeleton として残す。
...
本節は前段の candidate skeleton を confirmed integration-test design 粒度へ引き上げる。すべての IT-* 行は GWT、fixture、module boundary setup、assertion、negative/edge coverage を持つ。L6/L7 carry items は implementation-detail carry に限り、本節では integration boundary と期待 behavior を凍結する。
...
本節は L8/G8 を close するための executable workflow 粒度を定義する。上記の confirmed IT-* case design は維持しつつ、case rows を repeatable verification process へ変換する不足 layer を追加する。model は一般的な test strategy -> test plan -> test condition / coverage item -> test procedure -> evidence -> exit gate chain に従い、HELIX artifacts へ map する。

| Workflow key | G8 contract |
|---|---|
| `test_strategy` | L5 contracts 向け risk-based integration verification。changed module、state、adapter、asset、DB、search、feedback、automation、guardrail、relation graph、document export、proposal coverage boundaries を優先する。 |
