...
| U-PWFA-005 | canonical layer boundary | current Forward／L1設計文書のV-pairまたはdownstream routeがL14／L8-L14へ戻ったらred。compatibility-only本文の存在だけではredにしない | `tests/process-vmodel-legacy-boundary.test.ts` |
