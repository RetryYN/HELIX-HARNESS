...
| HAT-P1-02 | HR-FR-P1-02 | HAC-P1-02a/b | version_target と tag bump dry-run が migration/rollback を出す。parked PLAN の activation packet は activation/parked review/action-binding approval と source ledger freshness を判断材料として出すが、plan-only で apply/activation permission を持たない。source ledger stale / 必須 source 欠落は activation 前 blocker になる | version-up tests / plan lint / activation-packet CLI smoke |
