...
| U-DTRACE-008 | artifact／trace source identity不一致 | 2 transition fixtureの非先頭要素を使い、artifactのsnapshot／revision／orphanを`artifacts.1`、traceのsnapshot／revision／orphanを`trace.1`へsite別exact固定する | `tests/derived-requirement-trace.test.ts` |
...
| U-DTRACE-011 | layer placement欠落 | 第2transitionの非先頭layerを欠落させ、`layer_placement_missing`を`retry-order.L4`へexact固定する | `tests/derived-requirement-trace.test.ts` |
| U-DTRACE-012 | V-pair／reverse multiplicity | 第2transitionの非正規edge、pair重複、reverse重複を別legにし、missing／noncanonical／duplicate／reverse mismatchをidentityへexact固定する | `tests/derived-requirement-trace.test.ts` |
