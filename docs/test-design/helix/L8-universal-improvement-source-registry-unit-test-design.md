...
| U-UILSRC-005 | malformed admission | unknown source、wrong schema／detector／source revision、必須field欠落、invalid digest／timestampに加え、`access_token`／`tokens`／`user_pii`等のsnake_case sensitive fieldを個別に拒否する | `tests/universal-improvement-source-registry.test.ts` |
