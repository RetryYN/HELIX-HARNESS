...
| `IT-OSSC-005` | canonical manifests/locks→online install→component graph | clean LinuxでNode npm/package.json/package-lockとpair-frozen Python toolchainからdirect/transitive dependency、Node package、wheel/sdistをonline解決。Node別manager/Bun lock、Python未選定／複数toolchainも個別投入 | lock不変、floating/OS lock 0、全component artifact digestあり。Node authority差、Python未freeze、lock/source/rollback evidence欠落を`HIL_SUPPLY_CHAIN_LOCK_DRIFT`で拒否 | `§4 canonical dependencyとoffline再現性`、`§6 HIL_SUPPLY_CHAIN_LOCK_DRIFT` |
