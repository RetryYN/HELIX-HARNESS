...
| GH-T-024 | GH-AC-024 | production deployから承認、staging、branch/preview、backup、rollback、health/monitoringを一つずつ欠落または別HEAD化し、staging自動rollbackとproduction approval境界を交差させる | stagingでは自動deploy/検証/rollbackを反復でき、完全なaction-bound receiptだけproduction実行を許可する |
