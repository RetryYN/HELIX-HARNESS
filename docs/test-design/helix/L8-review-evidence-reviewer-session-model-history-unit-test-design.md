...
| U-DUR-003（追加ケース） | doctor の history 読込境界 | 壊れた JSON（未知例外）は `reviewer_session_model_history_invalid:unknown cause_kind=… cause_digest=sha256:…` のみ surface し、tmp root path や `Unexpected token` を含まない。schema 違反（既知診断）は `…invalid:schema_version` の typed reason をそのまま surface。既存 ratchet（`error.message` 露出禁止）は CI run 33952469801 で red を実測（2bdc1b8ce）、対策後 green | `tests/doctor-cause-digest-contract.test.ts` |
