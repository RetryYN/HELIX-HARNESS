...
| U-RVIDENT-020 | typed receiptによるopen window追従 | tracked Claude sessionでPR #1675 receipt境界の1秒前はFable 5.1、境界一致はOpus 5。旧windowをopenのままにする、境界をreceipt以前へ遡及する、同時刻を旧modelへ残す変更は期待値不一致またはparse重複でred | `tests/review-evidence.test.ts` |
