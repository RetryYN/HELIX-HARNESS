...
| U-RVIDENT-020 | transcript観測によるopen window追従 | tracked Claude sessionで最初のOpus 5観測境界の1秒前はFable 5.1、境界一致はOpus 5。旧windowをopenのままにする、境界をreceipt公開時刻へ遅延する、同時刻を旧modelへ残す変更は期待値不一致またはparse重複でred | `tests/review-evidence.test.ts` |
| U-RVIDENT-021 | terminal evidence-only Recovery | terminal PLANの非空`review_evidence`だけを訂正するRecoveryは異kind所有者へ数えず、本文変更・draft化・他field混載はmetadata-onlyと判定しない | `tests/branch-kind.test.ts` |
