...
| `U-URR-026` | `buildUniversalReverseCommitBundle` | Reverse本体write set、FK target、expected heads/revisions、operation/payload欠落を拒否し、stale/refreeze fieldの混載も拒否 | `HAC-HIL-04a`, `HAC-HIL-04b`, `HAC-HIL-04c` | supporting | `prepared` | `commit_ready` | `HIL_REVERSE_TRANSACTION_INVALID` | `tests/universal-reverse-transaction.test.ts` |
