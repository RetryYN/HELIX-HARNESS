...
本書は#624と#895のtarget acceptanceを定義する。#624は`CNW-AC-001..007`、#895は`CNW-AC-009..013`、
親Issue #92は`CNW-AC-008`を所有する。current ownerであるPLAN-L3-64のrequirements authority sliceが機械検証するのは、
...
| `CNW-AC-009` | `CNW-R-06` | SessionStart／doctor／status／dispatchのhook identityを取得する | project root identity、repository HEAD、hooks digest、guard digest、policy digestのexact field setが全surfaceで一致する | いずれかの欠落、別root、別digestを拒否する |
...
| `CNW-AC-013` | `CNW-R-08` | terminal review result後にmemory wake hookをtimeoutさせる | 既定15秒／hard ceiling 60秒を強制し、`project_hook_lifecycle_timeout`へexecution root、loader root、hook kind、deadline、source identityを記録し、親processをterminal化してresult、session、HEAD、verdictを保持し、長期通知待機をbounded workerへ分離する | 60秒超過、result消失、無期限hang、timeout後の親process残留、同期hookでの長期通知待機、raw bypassでの成功化を拒否する |
