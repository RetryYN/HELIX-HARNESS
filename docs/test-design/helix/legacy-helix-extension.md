...
# HELIX test-design — 旧 HELIX extension 採用

旧 HELIX 由来 extension の L3-L6 降下を、単一 pair test-design で逆参照する。

## §1 coverage matrix 対応表

| 旧 source | L3 | L4 | L5 | L6 oracle |
|---------------|----|----|----|-----------|
| `HELIX_RUNTIME_RULES.md` 由来 runtime rule | HLX-FR-01 | HLX-SYS-01 | HLX-C01 | U-HLX-001 |
| `pretooluse-askuserquestion.sh` 由来 user question guard | HLX-FR-02 | HLX-SYS-02 | HLX-C02 | U-HLX-002 |
| `detectors/registry.py` / `axis_*.py` 由来 detector axis | HLX-FR-03 | HLX-SYS-03 | HLX-C03 | U-HLX-003 / U-HLX-004 |
| `skill_recommender.py` / `code_recommender.py` / `command_catalog.py` 由来 recommendation | HLX-FR-04 | HLX-SYS-04 | HLX-C04 | U-HLX-005 |
| `helix-debug` / `helix-test-debug` 由来 RUN & Debug | HLX-FR-05 | HLX-SYS-05 | HLX-C05 | U-HLX-006 |
| `core-manifest.tsv` / `HELIX_CORE.md` / runtime adapters 由来 core injection | HLX-FR-06 | HLX-SYS-06 | HLX-C06 | U-HLX-007 |
| `.claude/settings.json` / `.claude/hooks/*` 由来 hook surface | HLX-FR-07 | HLX-SYS-07 | HLX-C07 | U-HLX-008 |
| `.claude/agents/*.md` / `cli/roles/*.conf` 由来 agent role | HLX-FR-08 | HLX-SYS-08 | HLX-C08 | U-HLX-009 |
| `HELIX-workflows/helix-process/*.md` 由来 workflow | HLX-FR-09 | HLX-SYS-09 | HLX-C09 | U-HLX-010 |
| `helix_db.py` / registries / HTTP routes 由来 DB/API | HLX-FR-10 | HLX-SYS-10 | HLX-C10 | U-HLX-011 |
| `helix-auto-run` / scheduler / budget helpers 由来 continuous run | HLX-FR-11 | HLX-SYS-11 | HLX-C11 | U-HLX-012 |
| `learning_engine.py` / `feedback_hook.py` / recipes 由来 learning feedback | HLX-FR-12 | HLX-SYS-12 | HLX-C12 | U-HLX-013 |
...
- この採用では Python/Bash runtime code を移植しない。
- この採用では現行 machine identifier を rename しない。CLI/state rename は引き続き `PLAN-M-02-helix-identifier-rename.md` が govern する。
- この採用では old HELIX catalog output を hardening なしに current truth として扱わない。
