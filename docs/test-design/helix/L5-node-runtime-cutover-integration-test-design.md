...
| `IT-NCUT-011` | cutover→monitor→rollback | artifact/DB/pointerのprepare、staging verification、commit-point CAS、recovery、compensationへfaultを個別注入 | commit前current不変、commit後は旧known-goodへ完全収束または補償しheterogeneous混在0。同一reconcile同receipt、rollback後cutover PASS 0 | `tests/node-runtime-cutover-rollback.integration.test.ts` |
...
| `IT-NCUT-009` | `HAC-HIL-13b`; `HAC-HIL-13c`; `HIL-FR-33` | `HIL_BUN_ACTIVE_DEPENDENCY`, `HIL_BUN_HISTORICAL_ALLOWLIST_INVALID` |
...
`U-NCUT-013` / `IT-NCUT-011`の同一transaction supporting edgeで、別owner重複へ数えない。canonical 11 caseはL6/L7/L8間で
