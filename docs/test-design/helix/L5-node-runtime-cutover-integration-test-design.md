---
title: "HELIX L9 結合テスト設計 — Node runtime cutover"
layer: L5
...
# HELIX L9 結合テスト設計 — Node runtime cutover

## §0 共通oracle

全caseは未実装である。ADR-009はtargetをacceptedとしたが、Red fixture／設計oracleだけではactive authorityを変更しない。
全実行は固定HEAD、inventory/schema/rule、Node/npm、manifest/lock、artifact、workflow、
...
| `IT-NCUT-011` | `planNodeCutoverRollback` → `executeNodeCutoverRollback` → `commitNodeCutoverRollback` → `reconcileNodeCutoverRollback` | forwardと同一branded store instanceでrollback_approved→rolling_back→rolled_back event chain、artifact/DB/pointer staging/CAS/recoveryへfault注入 | 別store facadeは開始0。commit前current不変、commit後は旧known-goodへ完全収束、rollback event欠落0、同一reconcile同receipt | `tests/node-runtime-cutover-rollback.integration.test.ts` |
| `IT-NCUT-012` | provisional gate→writer epoch→immutable generation→authority pointer | 初期bun_active singletonから、実SQLite/filesystem/固定bootstrapでdrain、exclusive claim、generation staging、single pointer CAS。pre-epoch null制約、bootstrap反例、各resource/CAS前後crashを個別注入 | 初期activation receipt nullを許可。反例commit 0。正常時authority revision +1、activation receipt 1、partial current 0 | `tests/node-runtime-cutover-activation.integration.test.ts` |
| `IT-NCUT-013` | activation receipt→monitoring→terminal／rollback | 新terminal writer epoch/lease/fenceとterminal approvalを取得し、monitoring/health後にterminal event／authority projection／receipt各append直後faultをcommit/reconcile | activation/terminal row各1、上書き0、3 step exact write set、checkpoint replay同receipt。lease/approval失効と異digest/CAS loser action 0 | `tests/node-runtime-cutover-activation.integration.test.ts` |
...
| `IT-NCUT-011` | rollback安全契約 | `HIL_NODE_CUTOVER_ROLLBACK_UNSAFE`, `HIL_NODE_CUTOVER_MONITORING_FAILED` |
| `IT-NCUT-012` | forward activation唯一writer | `HIL_NODE_CUTOVER_APPROVAL_MISSING`, `HIL_NODE_CUTOVER_WRITER_EPOCH_CONFLICT`, `HIL_NODE_CUTOVER_LEGACY_WRITER_ACTIVE`, `HIL_NODE_CUTOVER_AUTHORITY_REVISION_CONFLICT`, `HIL_NODE_CUTOVER_RECONCILIATION_FAILED` |
| `IT-NCUT-013` | terminal authority | `HIL_NODE_CUTOVER_TERMINAL_PRECONDITION`, `HIL_NODE_CUTOVER_MONITORING_FAILED` |

### §1.1 canonical assertion primary表

次表がHST-HIL-013のprimary integration採点表である。上の13 integration oracleはworkflow／faultを共有するsupporting実行表であり、
...
Node minimumは`IT-NCUT-001`〜`IT-NCUT-005`の限定receiptを発行できるが、L8合格ではない。最終合格は13/13、
...
L6 public API→L7 U→L8 ITの全edgeを逆引きし、22 APIすべてにownerを一件要求する。rollbackのexecute/commit/reconcileは
`U-NCUT-013` / `IT-NCUT-011`の同一transaction supporting edgeである。plan→execute→commit→reconcileの4関数を固定順で全て実行し、
forward activationの4関数も`U-NCUT-014` / `IT-NCUT-012`の同一transaction supporting edgeとする。一部関数のgreenや別owner重複へ数えない。canonical 11 caseはL6/L7/L8間で
