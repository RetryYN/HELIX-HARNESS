...
`U-SAP-001`は`IT-SAP-004`の先頭supporting oracleとして`HST-CASE-012-08`へreverse joinする。exact compositionは
`canonicalizeScreenScope` → `evaluateScreenApplicability` → `planPrototypeDiscovery`であり、scope正規化mutationは
route遷移・task生成mutationと別に採点する。primary U/HST分母は増やさない。
