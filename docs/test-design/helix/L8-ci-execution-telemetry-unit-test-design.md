...
| U-TELE-006 | setup、test、artifact upload/downloadを別cost nodeへ分類する | testをbuildへ、artifact方向・transfer operationを誤らせ、transfer digest欠落を通さない | `tests/ci-execution-telemetry.test.ts` |
...
| U-TELE-008 | 一つのrun/attemptの依存DAGとartifact edgeを検証する | context違い、node重複、missing dependency、時間逆転、cycle、artifact input/output不一致を拒否する | `tests/ci-execution-telemetry.test.ts` |
...
| U-TELE-010 | profile/surface/runner/environment/cache別にp50/p95/p99を算出する | cancel/supersededを母集団へ混ぜず、runner identityの異なるseriesを合算せず、有効標本なしを0と表示しない | `tests/ci-execution-telemetry.test.ts` |
