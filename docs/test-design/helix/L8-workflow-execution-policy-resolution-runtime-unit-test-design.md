...
| U-WFEPOLRES-003 | ambiguous policy | canonical registryの重複はschemaで拒否し、schema通過後のintegrity driftによる同一precedence複数一致も`policy_ambiguous`で閉じる | `tests/workflow-execution-policy-registry.test.ts` |
