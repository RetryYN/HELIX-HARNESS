...
| `U-ICN-006` | `commitIssueAdmissionExactlyOnce` | 各append直後fault、並行CAS、再送でpartial 0、winner一件。initial cause/transition/budget metadataも同じtransactionへbind | `HAC-HIL-01b` | `HST-CASE-001-08` | `assertion_input_ready` | `assertion_pass` | `HIL_IDEMPOTENCY_VIOLATION` | `tests/issue-admission-transaction.test.ts` |
