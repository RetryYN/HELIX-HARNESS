...
| `IT-CIQ-015` | stage完了、quarantine application、self-heal outcomeのcheck/event/projection/receipt/lineage各appendへfaultを注入し、同operation同digest／異digest、stale headを再送 | fault/CAS conflictは全write 0。同operation同digestはreceipt一件、異digestはconflict。immutable provider/check/event evidence reconcile後もrequired workload数、lineage、application/outcome digestが一致 | `HAC-HIL-06a`, `HAC-HIL-06b`, `HAC-HIL-06c` | supporting transaction oracle | supporting | `tests/ci-mutation-transaction.integration.test.ts` |
