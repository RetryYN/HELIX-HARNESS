...
| HST-HIL-005 | HIL-BR-17, HIL-FR-09, HIL-FR-30 | HST-A-005 | Finding disposition / promotion | current contract内の局所findingと独立改善findingを生成し、後者のIssue生成途中でtransactionを失敗させる | 局所findingは`current_pr_fix` writer返却だけ、独立改善は`successor_issue`としてIssue、Reverse、memory、queueが全件commitまたは全件rollback | `HIL_FINDING_DISPOSITION_INVALID`, `HIL_FINDING_PROMOTION_PARTIAL` | 設計済み | 未実装 |
