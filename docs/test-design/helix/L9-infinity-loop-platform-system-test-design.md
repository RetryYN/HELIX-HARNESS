...
| HST-HIL-039 | GH-FR-021, GH-NFR-013..014, GH-AC-023..026 | HST-A-039 | EnvironmentPromotionController→CloudDeploymentAdapter→ProductionMigrationGate | 正式/補助操作面、AWS reference、provider交換、staging再現度、environment/secret、artifact、production承認・migration完全性を個別投入する | GitHub正式面と同一workflow補助面、production同等VPS staging、provider非依存promotion、zero-downtime migrationを強制する | `HIL_PRODUCTION_PROMOTION_UNSAFE` | 設計済み | 未実装 |
