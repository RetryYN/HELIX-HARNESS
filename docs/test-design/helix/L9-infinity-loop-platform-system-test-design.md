...
| HST-HIL-039 | GH-FR-021, GH-NFR-013, GH-AC-023..024 | HST-A-039 | EnvironmentPromotionController | environment/secret混同、artifact drift、staging未検証、production承認・backup・rollback・monitoring欠落を個別投入する | VPS stagingとcloud productionを分離し、同一artifactの承認付きpromotionだけ許可する | `HIL_PRODUCTION_PROMOTION_UNSAFE` | 設計済み | 未実装 |
