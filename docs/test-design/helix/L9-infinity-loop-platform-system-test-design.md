...
| HST-HIL-039 | GH-FR-021, GH-NFR-013..014, GH-AC-023..025 | HST-A-039 | EnvironmentPromotionController→CloudDeploymentAdapter→ProductionMigrationGate | provider交換、environment/secret混同、artifact drift、staging未検証、production承認・backup・rollback・monitoring・migration完全性欠落を個別投入する | GitHub操作面でVPS stagingとcloud productionを分離し、provider非依存の同一artifact promotionと完全なproduction migrationだけ許可する | `HIL_PRODUCTION_PROMOTION_UNSAFE` | 設計済み | 未実装 |
