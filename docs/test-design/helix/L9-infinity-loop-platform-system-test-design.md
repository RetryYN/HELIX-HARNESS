...
| HST-HIL-030 | HIL-BR-25, HIL-FR-46, HIL-FR-47, HIL-NFR-29 | HST-A-030 | Template→Layer Ledger Registry | L0–L14 templateへ正常、空、TBD、未知field、重複obligationを個別投入する | 原子的proposalとgap findingを決定的に生成しaggregate parentをcoveredにしない | `HIL_LAYER_TEMPLATE_EXTRACTION_INVALID` | 設計済み | 未実装 |
