...
| HST-HIL-039 | GH-FR-021, GH-NFR-013..014, GH-AC-023..027 | HST-A-039 | DeploymentCapabilityPreflight→EnvironmentPromotionController→CloudDeploymentAdapter→ProductionMigrationGate | GitHub plan保護、concurrency、self-review、OIDC、正式/補助操作面、AWS reference、staging再現度、production/migration完全性を個別投入する | production有効化preflight、GitHub正式面、production同等VPS staging、provider非依存promotion、bounded-downtime migrationを強制する | `HIL_PRODUCTION_PROMOTION_UNSAFE` | 設計済み | 未実装 |
