...
| U-CLIR00-006 | 構造snapshot | `src/cli.ts`と`harness-check.yml`からfamily数・byte長・shard jobを決定的に再抽出する | `tests/cli-r00-throughput-baseline.test.ts` |
...
| U-CLIR00-008 | frozen artifact | repo-owned JSONがschema検証に通り、構造proxyがlive snapshotと一致する | `tests/cli-r00-throughput-baseline.test.ts` |
...
| U-CLIR00-012 | 測定不能の数値比較 | unmeasurable同士のdelta比較を拒否し、高速化判定へ使わせない | `tests/cli-r00-throughput-baseline.test.ts` |
