...
| semantic frontier preservation | `semantic_feature_frontier_record` classification joins from PLAN/design state to L8 evidence | `frontier_pending_decision`, `parked_future_version`, or `approval_gated_cutover` is reported as completed integration work |
