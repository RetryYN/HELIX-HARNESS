...
| HU-PILLAR-P6-03 | HR-FR-P6-03 | HC-P6 | `buildDistributionPlan` / `runHelixProjectSetup` / `runConsumerDoctor` / `ut-tdd setup project` / `ut-tdd completion decision-packet` / `ut-tdd doctor --profile consumer` / `ut-tdd rename plan` / `ut-tdd team run` | fresh/brownfield setup は managed diff/import report を出し既存を壊さない。U-SETUP-015 は fresh/dry-run baseline、identifier transition、command availability、PATH 未解決時の `consumerReadiness` remediation、`postSetupWorkflow.nextRoute=fix_consumer_readiness`、`completion-decision-packet` phase、`identifier-cutover-packet` phase、生成 VSCode task の package-local `bun run ut-tdd ...` command、`postSetupWorkflow.verificationMatrix[]` 全 row の `writePolicy=no-write` と text `verification-check:` の expected/evidence 表示を検証し、初回稼働検証が apply surface や L14 完了 claim に化けないことを固定する。加えて matrix row は `availability=dry-run-immediate|post-apply-or-projected` と `requiresMaterializedPaths[]` を持ち、`dryRunVerificationCommands` は setup/status/completion/rename/handover など dry-run 直後の read-only command だけを含み、`postApplyVerificationCommands` は projected/materialized adapter を必要とする consumer doctor と team-run dry-run を含む。U-SETUP-016 は brownfield の existing/skipped/written/mergeable path、`requiresReview`、`importReport.nextRoute=review_import_report`、`postSetupWorkflow.nextRoute=review_import_report` を検証する。U-SETUP-017 は consumer repo で projected hook が呼ぶ `ut-tdd` CLI の PATH 解決性を `consumerReadiness` に記録し、consumer CI smoke が secret 不要であること、`artifactReadiness` が AGENTS / VSCode task / `.ut-tdd` baseline / default-hybrid team だけでなく `CLAUDE.md` / `.claude/CLAUDE.md` / Claude subagent / Claude slash-command の日本語 HELIX prose、completion packet preflight、consumer doctor、rename/cutover 境界、secret/PII guardrail を検査すること、`postSetupWorkflow.nextRoute=ready` を検証する。U-SETUP-019 は `githubPlan` と `doctorBaseline` を plan-only 構造として返し、GitHub rules/checks plan が remote apply surface を持たず、doctor baseline が setup dry-run / status / `ut-tdd completion decision-packet --json` / `ut-tdd doctor --profile consumer` / `ut-tdd rename plan --json` / handover status / team-run dry-run と `.ut-tdd/memory|handover|evidence|teams` を初回稼働証跡として示すことを検証する。consumer doctor profile は dogfood PLAN/design/test-design/runtime state を要求せず、setup 投影済み adapter / VSCode task / `.ut-tdd` baseline / `.ut-tdd/teams/default-hybrid.yaml` を検査し、配布済み Claude subagent / slash-command に `ut-tdd status`、`ut-tdd completion decision-packet --json`、`ut-tdd doctor --profile consumer` の三点 baseline が無ければ fail-close する。承認前 `.helix/**` state、package/bin/scripts、VS Code task、GitHub workflow、Claude/Codex hooks 上の `helix setup|doctor|status|handover|team` alias、doctor task が full `ut-tdd doctor` のまま、または team-run YAML が missing / malformed / single-provider なら fail-close する。U-SETUP-020 は setup が配布する adapter doc / Claude subagent / Claude slash-command / next action を日本語-first に固定し、CLI 名・識別子・stable token だけを原語許容にしつつ、`helix-test` を含む command が completion packet preflight を要求する。U-SETUP-023 は AGENTS の team-run 案内を配布 YAML、VS Code task、consumer CI、distribution acceptance の `ut-tdd team run --definition .ut-tdd/teams/default-hybrid.yaml --mode hybrid --json` dry-run に接続する。setup output は現行 `ut-tdd setup project` を available、将来 `helix setup project` を unavailable として出し、PLAN-M-02 cutover approval 前に package/bin alias が有効であると誤認させない。CLI surface は `post-setup-workflow` / `github-plan` / `doctor-baseline` を text/JSON に出し、manual doc search なしに初回稼働ルートを判断できることを固定する |
| HU-PILLAR-P6-04 | HR-FR-P6-04 | HC-P6 | `planVersionUpgradeDryRun` / `auditIdentifierRenameBlastRadius` / `buildIdentifierRenameCutoverPlan` / `ut-tdd rename audit` / `ut-tdd rename plan` | tag bump に rollback/destructive block/idempotency evidence を要求する。PLAN-M-02 identifier rename は `ut-tdd` / `.ut-tdd` / `area=harness` blast radius を audit し、source/test/runtime-state/adapter-config/consumer-template/plan/design/governance/distribution surface の category 別 hit と cutover action checklist を返す。拡張子つきファイルだけでなく `.gitignore` / `.gitattributes` と `scripts/` 配下の拡張子なし text entrypoint も audit 対象に含める。cutover/action-binding approval が無い限り `.ut-tdd -> .helix` apply 可能と扱わない。`rename plan` は dry-run / rollback / monitoring / approval gate / `cutoverSnapshot.snapshotId` と packet-level `generatedAt` / `sourceCommand` / `freshness` を返すが、approval records が concrete でも plan-only であり apply command を提供しない。`applyAuthorized` 判定は `approve_cutover` / `approve_action_binding` / actor/tool/target だけでなく approved params、current `cutoverSnapshot` sha256 binding、review evidence、expiry、audit、backup/rollback/monitoring evidence を要求する。`cutoverSnapshot` は blast radius / approval scope / evidence digest を束ね、hit set や evidence や packet generation が変わった旧承認材料を cutover に流用させない。`verificationCommandMatrix[]` は post-cutover-consumer-setup-smoke を含み、承認後の `helix setup project --dry-run --json` consumer bootstrap smoke を `rename dist-smoke --no-write --target helix --json` の no-write packet material として審査する。`relatedDecisionPackets[]` は primary rename plan と supporting action-binding approval packet を保持する |
