...
| HU-PILLAR-P2-04 | HR-FR-P2-04 | HC-P2 | `writeLoopTraceSpan` / `selectWorkflowComplexity` / `buildPairAgentTddPlan` / `runPairAgentTddPlan` / `ut-tdd pair-agent plan/run` | PLAN/tool/handoff/guardrail/eval span を残し、simple workflow を既定にする。TDD pair route では smart review agent が最初に test/oracle を作り、light implementation agent が実装し、smart review agent がテスト・レビュー・VERDICT を出す。light agent は closing authority を持たず、fail verdict と smart review の fix instruction は bounded transcript として次の light implementation prompt へ渡る。`--save-evidence` は `.ut-tdd/evidence/pair-agent/` に replayable evidence を残す。T0 smart agent は実行前に explicit frontier approval を要求する |
