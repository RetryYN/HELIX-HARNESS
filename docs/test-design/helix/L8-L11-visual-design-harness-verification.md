...
| pair | design side | verification side |
|---|---|---|
| L5↔L8 | Detailed Visual Contract、Frontend Binding、state/region/slot oracle | atomic visual/binding verification |
| L4↔L9 | Pattern Contract、Product UI Profile、visual system policy | cross-screen visual integration |
| L3↔L10 | Screen FR/AC/NFR、role/data/workflow outcome | browser/data visual system verification |
| L2↔L11 | Prototype Agreement、visual intent、expression/preference criteria | human visual acceptance |
...
- source commit/tree/build digest、Design contract/profile/binding/prototype revision
- OS、browser engine/version、font set、DPR、viewport、locale、timezone、theme、text scale、motion/contrast preference
- fixtureまたはredacted product snapshot digest、tool/version/config digest
- oracle ID、expected/actual、producer/verifier、artifact digest、started/completed timestamp
- screen/state/region/slot/action/binding ID、matrix cell ID、failure code、applicability receipt
...
## §2 L8 Visual Detail / Binding Verification
...
| case ID | kind | fixture / stimulus | oracle | expected / failure code |
|---|---|---|---|---|
...
## §3 L9 Cross-screen Visual Integration
...
| VIS-L9-F04 | negative | motion duration/repeat/layout shift/reduced-motion drift | runtime motion trace | `HIL_VIS_L9_MOTION_CONTRACT_DRIFT` |
...
## §4 L10 Browser/Data Visual System Verification
...
## §6 Layer Freeze eligibility bridge
...
| layer | designed cases | execution |
|---|---:|---:|
| L8 | 6 | 0/6 |
| L9 | 7 | 0/7 |
| L10 | 8 | 0/8 |
