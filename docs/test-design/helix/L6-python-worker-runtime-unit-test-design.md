...
|---|---|---|---|---|---|---|
| `U-PYWR-001` | `parsePythonWorkerDescriptor`, `resolvePythonWorkerDescriptor` | `HAC-HIL-12a` | `HST-CASE-007-14` | 5合法class、descriptor欠落、unknown key、空schema、digest不正、未登録runtime、未知／欠落／case variant capability、registry 0件／複数件／inactive／class不一致／digest drift | 不正系は`HIL_PYTHON_PLANE_BOUNDARY_INVALID`でspawn 0。valid descriptorはclosed listのexactly-one capability、registry revision/digest、canonical digestを維持 | `tests/python-worker-registry.test.ts` |
...
| `U-PYWR-003` | `createPythonWorkerRun` | `HAC-HIL-12b` | `HST-CASE-007-12` | 同一operation/input/configを時刻、PID、OS pathだけ変更。request/resolutionのclass、worker、registry revision/digest、resolution digestを1 fieldずつ改変 | 同一payloadのrun identityは不変。hop不一致は`HIL_PYTHON_PLANE_BOUNDARY_INVALID`または`HIL_WORKER_PAYLOAD_DIGEST_MISMATCH`で拒否し、shell invocation 0 | `tests/python-worker-broker.test.ts` |
...
| `U-PYWR-012` | `stagePythonWorkerResult` | `HAC-HIL-12a`, `HAC-HIL-12b` | `HST-CASE-007-18` | complete 0/複数、chunk欠落、件数/set digest/schema/provenance不一致、run→stagedのclass／registry／resolution差替え | assertion token `HIL_IPC_FAIL_OPEN`を0に保ち、詳細cause `HIL_WORKER_RESULT_SCHEMA_INVALID`、partial staging/current 0 | `tests/python-result-ingestion.test.ts` |
...
| `U-PYWR-016` | `recordWorkerTerminalReceipt` | `HAC-HIL-12a`, `HAC-HIL-12b`, `HAC-HIL-12c` | `HST-CASE-007-13`, `HST-CASE-007-18` | terminal 0/二重/異status、run→terminalのclass／registry／resolution差替え、receipt write failure、同一receipt再入力 | pointer別に`HIL_WORKER_PROTOCOL_INVALID`、`HIL_IPC_FAIL_OPEN`で競合を拒否し、runごとexactly-one receiptを同digestへ固定 | `tests/python-worker-terminal-receipt.test.ts` |
