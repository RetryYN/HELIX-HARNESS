...
| U-BIOMEFAST-001 | Biomeをpreflightの依存導入直後に一回実行 | 削除、finalizeへ逆戻し、重複、command差替え、条件付きskip、continue-on-errorを拒否 |
...
| U-BIOMEFAST-001 | workflow preflight | 既存lintの早期実行・単一実行と6種の退行を検査 | `tests/harness-check-workflow.test.ts` |
