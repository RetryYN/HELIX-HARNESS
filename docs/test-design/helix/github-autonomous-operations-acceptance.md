...
| GH-T-022 | GH-AC-022 | main merge後Full失敗にRecovery修正Issue、修正PR、doctor、独立review、GitHub CI、Issue closureを個別に欠落させる | 通常merge停止を維持し、全修正証拠が同一HEADへ収束しIssue closeした場合だけ解除する |
...
| GH-T-027 | GH-AC-027 | required reviewer非対応plan、self-review許可、同時deploy二件、長期AWS key、広すぎるOIDC trust、staging/production role共有を個別投入する | setup preflightでproduction有効化を拒否する |
| GH-T-028 | GH-AC-028 | 技術要件なしcontainer、DB不要、production dataあり、監査ログへsecretを含むfixtureを投入する | Fargate/CDK、DBなし、別account、redacted 90日/1年profileを質問なしで決定する |
| GH-T-029 | GH-AC-029 | 正常future update、label欠落、矛盾lifecycle、trace欠落、active化、closureを個別投入する | 正常openをbacklog表示し、異常だけfinding化し、active/closed遷移をreceiptで追跡する |
