...
| U-CLSO-009 | actual CLI negative oracle | `progress frontier --summary-json`の実出力top-level keyに`drive_model`が存在しないことを直接検証し、余剰fieldを許す`toMatchObject`だけに依存しない。Issue #1277の範囲外となる全key集合の固定や、別責務のnested compatibility観測は本oracleへ混載しない | `tests/cli-surface.test.ts` |
