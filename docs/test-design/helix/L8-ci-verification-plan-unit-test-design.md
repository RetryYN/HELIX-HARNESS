...
| U-CIVPLAN-011 | deferred receipt | wrong HEAD、terminal digest欠落、未知stateを拒否 | `tests/ci-verification-plan.test.ts` |
| U-CIVPLAN-012 | aggregate mutation | required obligationを一件削除または重複するとaggregate admissionがred | `tests/ci-verification-plan.test.ts` |
