...
| `IT-URR-023` | manifest/event/run/R0–R4 artifact/workload/pair/route/stale/reentry/refreeze各appendへfault、CAS loser、same/different digest retry | fault/CAS/conflictは全write 0。同operation同digestは同receipt一件、異digestはconflict。event rebuild digest一致 | `HAC-HIL-04a`, `HAC-HIL-04b`, `HAC-HIL-04c` | supporting atomic commit oracle | `commit_ready` | `committed` | `HIL_REVERSE_TRANSACTION_CONFLICT` | `tests/universal-reverse-transaction.integration.test.ts` |
...
| `IT-URR-026` | stale edge中間appendと、stale supersession/pair/oracle/refreeze中間appendへfaultを注入 | stale closure/refreezeは各operationでpartial 0。refreeze成功後だけstale 0とclaim eligibilityを公開 | `HAC-HIL-04c` | supporting stale/refreeze oracle | `pair_stale` | `refrozen` | `HIL_REDESIGN_REFREEZE_COMMIT_FAILED` | `tests/redesign-refreeze-transaction.integration.test.ts` |
