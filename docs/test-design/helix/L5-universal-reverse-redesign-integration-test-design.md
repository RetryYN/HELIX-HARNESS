...
| `IT-URR-027` | base/stale/refreeze各operationでreceipt消失、projection欠損、crash後同operation retryをimmutable artifact/eventからreconcile | operation kind別receiptを読み、artifact/route/stale/freezeを推測せず、親receiptとevidence一致時だけ当該receipt一件と同projection digestへ復旧 | `HAC-HIL-04a`, `HAC-HIL-04b`, `HAC-HIL-04c` | supporting reconcile oracle | `commit_pending` | `committed` | `HIL_REVERSE_RECONCILE_FAILED` | `tests/universal-reverse-reconcile.integration.test.ts` |
