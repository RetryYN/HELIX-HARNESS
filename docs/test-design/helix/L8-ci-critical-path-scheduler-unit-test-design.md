...
| U-CISCHED-002 | critical path | dependencyとdurationから決定的なparallel group／critical pathを返す | `tests/ci-critical-path-scheduler.test.ts` |
| U-CISCHED-003 | artifact identity | wrong HEAD/platform/lockfile/toolchain/digestを個別拒否する | `tests/ci-critical-path-scheduler.test.ts` |
...
| U-CISCHED-006 | bounded execution | quota/backpressure/timeout/cancel/stale HEADをboundedに扱う | `tests/ci-critical-path-scheduler.test.ts` |
