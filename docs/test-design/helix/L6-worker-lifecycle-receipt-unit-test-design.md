...
`tests/worker-isolation-broker.test.ts`の`U-WLIFE-001..003`を実行oracleとし、`tests/design-reality-binding.test.ts::U-DRB-023`がrun/review seal、proposal join、terminal分岐、event previous digestを個別に除去するとRedになることを要求する。
