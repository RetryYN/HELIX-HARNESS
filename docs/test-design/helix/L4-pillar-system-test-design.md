...
- G-SF `semantic_feature_frontier_record` が `frontier_pending_decision` / `parked_future_version` /
  ...
  または先行 first-response artifact をもって revised request 全体の system completion としない。
...
- HST-P10 相当の UI / Webview / dashboard claim は、Playwright trace/report または render smoke と
  ...
  UAT/pass とせず、manual accessibility または inclusive-user review の route を G11 に残す。
...
| HST-P6-04 | HR-FR-P6-04 | HB-P6 | tag bump が migration / rollback / idempotency plan を出し破壊的 apply を止める。PLAN-M-02 rename cutover は full approval evidence と current `cutoverSnapshot` sha256 binding が無い限り `applyAuthorized=false` のままになる |
