...
| U-DISTART-002a | catalog disposition boundary | profile exclusionに依存せず、catalogで`excluded`のcapability選択を拒否しartifact集合を空にする | `tests/distribution-artifact-projection.test.ts` |
...
| U-DISTART-003d | relative traversal | `foo/../../bar`を他のabsolute path failureに依存せず単独拒否しartifact集合を空にする | `tests/distribution-artifact-projection.test.ts` |
