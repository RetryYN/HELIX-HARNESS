...
| `U-NCUT-013` | rollback plan／commit／reconcile | known-good互換、artifact/DB/pointer各prepare・staging検証・pointer CAS・recovery・compensation faultを個別注入 | commit前current不変、commit後はrecovery/compensationで新旧混在0。同一reconcile同receipt、競合action 0 | `tests/node-runtime-cutover-rollback.test.ts` |
...
| `HST-CASE-013-08` | `U-NCUT-012` | `evaluateBunCutover` | `IT-NCUT-008` | `assertion_input_ready` | `assertion_pass` | `HIL_BUN_ACTIVE_DEPENDENCY` |
...
| `U-NCUT-013` | rollback安全契約 | `IT-NCUT-011` | cutover操作承認の安全契約 |
