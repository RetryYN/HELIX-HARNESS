...
全caseは未実装であり、ADR-009 acceptedだけではcutover authorityを持たない設計oracleである。mockのexit 0だけでなく、finding、failure code、
...
| `U-NCUT-013` | `planNodeCutoverRollback` → `executeNodeCutoverRollback` → `commitNodeCutoverRollback` → `reconcileNodeCutoverRollback` | shared branded store不一致、rollback event欠落、不正phase、known-good互換、artifact/DB/pointer staging、CAS/recovery/compensation faultを個別注入。commit前current不変、commit後新旧混在0 | `tests/node-runtime-cutover-rollback.test.ts` |
| `U-NCUT-014` | `planNodeCutover` → `executeNodeCutover` → `commitNodeCutover` → `reconcileNodeCutover` | 初期bun_active projection、pre-epoch null制約、approval/snapshot drift、stale epoch/fence、legacy active、CAS/write-set差、bootstrap adapter missing／unverified／digest tamper／write-set混入、各resource/CAS/event/projection/receipt faultを個別注入 | `tests/node-runtime-cutover-activation.test.ts` |
| `U-NCUT-015` | `commitNodeCutoverTerminal` → `reconcileNodeCutoverTerminal` | activation receipt欠落、monitoring未完、health差、新terminal epoch/lease/fenceまたはterminal approval欠落/expiry、exact write set/checkpoint差、rollback代用、CAS loser、同operation異digest、terminal 3 step faultを拒否しexactly-oneへreconcile | `tests/node-runtime-cutover-activation.test.ts` |

### canonical assertion primary表

次表がHST-HIL-013のprimary unit採点表である。上の15 unit oracleはAPI別mutationを共有するsupporting実行表であり、
...
|---|---|---|---|---|---|---|
| `HST-CASE-013-01` | `U-NCUT-015` | `commitNodeCutoverTerminal` | `IT-NCUT-013` | `activated_monitoring` | `verified` | `なし（正常系）` |
...
| `HST-CASE-013-11` | `U-NCUT-014` | `commitNodeCutover` | `IT-NCUT-012` | `assertion_input_ready` | `assertion_pass` | `HIL_BUN_CUTOVER_INCOMPLETE` |
...
| `U-NCUT-014` | `planNodeCutover` → `executeNodeCutover` → `commitNodeCutover` → `reconcileNodeCutover` | `IT-NCUT-012` | forward activation唯一writer契約 |
| `U-NCUT-015` | `commitNodeCutoverTerminal` → `reconcileNodeCutoverTerminal` | `IT-NCUT-013` | monitoring後terminal authority／fault reconcile契約 |

15/15 Red/Green、全failure union、activation／rollback fault、同一入力のdigest冪等性、別runtime reviewが揃うまでL7をgreenにしない。

public API→U owner監査では22 APIを15 Uへexact joinする。`U-NCUT-001`〜`012`は各一API、`U-NCUT-013`は
...
`U-NCUT-014`もplanをprimary、execute/commit/reconcileを同一forward transactionのsupporting APIとして所有する。
activation／rollback反例はlocal `NodeCutoverFailureV1`の43-code allowlistと必須fieldを直接検証し、旧名、自由文字列code、欠落fieldを拒否する。

`U-NCUT-013`は上記4関数を固定順で全て実行し、plan検証、非current staging、pointer CAS commit、checkpoint
reconcileの各mutationを省略しない。一部関数のgreenを15/15の合格へ代用しない。
