...
| `U-MEMWAKE-003` | `buildClaudeInboxEntry` / `publishClaudeInboxEntry` | 汎用builderと汎用publisherは`claude-inbox:pr:`予約namespaceを拒否する。canonical payload・origin・URLを持つ偽造entryでも直接publishできず、実測coreだけが内部capabilityで配送できる | 公開低層publisher直呼びによるmeasured dispatch迂回、consumer検証だけへの依存 |
