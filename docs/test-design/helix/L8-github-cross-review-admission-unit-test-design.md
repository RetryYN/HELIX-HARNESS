...
| `U-CPRCONV-024` | `persistClaudePrReviewReceipt` | mixed authorshipのClaude/Codex receiptを同一PR/HEADへreviewer別immutable pathで保存・再読込でき、同一reviewerの異内容receiptはconflictになる | reviewerを含まないfilenameへの退行、同一reviewer上書き |
