...
| `U-WDA-004` | agent/version identityをexactly-one解決後、capabilityを別検証 | version無視、capability mismatch不可達化、provider fallbackを拒否 | ST-WDA-001/003/004 |
| `U-WDA-005` | exactly-one active entryをadmit | 0件、2件、inactiveを別reasonで拒否しspawn 0 | ST-WDA-003 |
| `U-WDA-006` | 実在specialist agent entryを明示binding付きでread-only projection | capability暗黙推論、source固有fieldのauthority昇格、source writeを拒否 | ST-WDA-001 |
| `U-WDA-007` | Python worker descriptor contractを明示mapping付きでprojection | Python registry実装済みの虚偽claim、第二registry新設、Python固有keyでの解決を拒否 | ST-WDA-007 |
| `U-WDA-008` | descriptor／source entry／snapshot digestを全検証 | 1面だけの照合、別provider greenでの相殺を拒否 | ST-WDA-005 |
| `U-WDA-009` | request/snapshot/descriptor/source entry不変のdecisionをcurrent判定 | 各bindingを1件ずつ変えたstale decisionを拒否 | ST-WDA-005 |
