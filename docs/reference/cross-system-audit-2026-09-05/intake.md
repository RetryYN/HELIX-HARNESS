...
2026-09-05にoriginから取得したHEAD `b1b4046934f9e5a5a93649314503c2b98b2abeb9`のGit blobでも14点のサイズ・SHA-256一致を再確認した。
これは入力保全の証拠であり、全secret/PII検査・全参照更新・欠陥是正完了の証拠ではない。

- C01〜C11の責務衝突、X01〜X05の修正済み対照、未検証範囲を保持する。
- ローカルに展開内容を保存済み。元ZIP自体は再配置せず、入力ハッシュと内容の対応を保持する。
- remote入力保全は確認済み。独立検収・全参照更新・各所見の修復は別途確認する。
- 修復PRの局所green、main到達、独立検収、read-afterを別状態として追跡する。
- 元MD/ZIPはユーザーの整理依頼に従い、リポジトリ外へ回復可能な形で退避した。root不存在を確認済み。完全消去ではないため`original_inputs_deleted`はfalseを維持し、`original_input_disposition`で区別する。
