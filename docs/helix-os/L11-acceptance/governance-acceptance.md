...
- HELIXOS-L2-001／002／007／010：local ticketのexact revisionをGitHubへ一方向投影し、write後のremote本文・state・target HEADをread-afterする。自己投影のwebhook／取得結果はoriginating commandへ束縛したreceiptまたはduplicate eventとし、新しい送信commandを生成しない。外部comment／review／check／手編集はsource system、delivery identity、remote revision付き原eventへ戻すが、localで新revisionとして採否されるまで要求・ticket・decisionの上書きや再投影を行わない。
