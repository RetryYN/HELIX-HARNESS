...
出典: concept v3.1 §2.5 (Retrofit workflow) / §2.6.1 / §2.6.3 (config_drift 承認者) / requirements v1.3.12 §9.2、§10 / workflow classification registry v1.1.4
...
| 2. 影響評価 | `retrofit-matrix`: 旧→新 対応と影響範囲を整理。`upgrade` 高リスク時は `helix doctor --preflight upgrade` 必須 (§7.8.3 requires_preflight) | retrofit-matrix.md |
...
承認記録は `.helix/audit/` に append すること (§7.8.3)。
...
| `upgrade` preflight fail | `helix doctor --preflight upgrade` pass まで移行計画に進まない |
