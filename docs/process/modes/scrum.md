...
翻案注記: HELIX route は Scrum の fullback を `ut-tdd reverse fullback ...` として扱う。旧 source process command 名は現行導線にしない。HELIX Scrum は Scrum ガイドの構造 (ロール/イベント/作成物) を概念として取り込みつつ、increment=1 PLAN 完了の粒度定義と Reverse fullback による昇華義務を HELIX 独自の追加定義とする。
