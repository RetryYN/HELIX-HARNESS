...
>
> **重要 (なぞらず翻案)**: source process reference の workflow ファイル名と HELIX の`kind`は1:1でない。
> DiscoveryとProduction Scrumが現状同じ`kind=poc`を共有するのは互換schema制約であり、意味上の同一性を示さない。
> `delivery_route`でDiscovery / Production Scrum / V設計＋Scrum実装Hybridを必ず識別し、schema未追従中は
> Production ScrumをPoC扱いしてCI・release gateを省略してはならない。
>
...
| **Scrum** | [scrum.md](scrum.md) | `poc`（互換値、`delivery_route=production_scrum`必須） | 専門職継承 | `cross` | **S0-S4** | po + aim | L3要件変更はPO | incrementごとにScrum ReverseでV正本同期、RCで全pair closure |
...
| Discovery | requirement_undefined | `poc/*` | 隔離PoC用CI。production claim・直接merge不可 | S4昇格時 |
| Production Scrum / V設計＋Scrum実装Hybrid | user_feedback / staged_release | `feature/*` | 内部主CI＋GitHub重要検査。sliceごとに必須 | Scrum Reverse＋RC全pair closure |
