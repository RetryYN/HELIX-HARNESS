...
| 再発防止テスト | L9 結合テスト / L10 システム/Real UX evidence 検証 |
...
| Discovery (前段) | 要件が未確定の場合、Incident 着手前に Discovery を前段起動する (concept / requirements の mode routing と discovery.md §6 の reciprocal)。下の「Discovery post-deploy trigger」行とは方向が逆 (前段起動 vs 後段の検出止まり) |
...
翻案注記: source process reference には `incident` 専用 kind がなく legacy source も `troubleshoot`+`recovery` の内包として扱う。HELIX では concept / requirements の mode routing を正本とし、layer=L7(+cross) と三者承認を HELIX 固有の追加定義として確定している。

---

出典再掲: concept v3.1 §2.5/§2.6.3/§2.6.5 / requirements v1.2 §1.3/§1.5/§1.8 / source process reference (incident workflow)
