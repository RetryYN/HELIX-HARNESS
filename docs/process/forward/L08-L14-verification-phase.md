...
`ut-tdd completion decision-packet --json` / `ut-tdd status` / handover から action-binding packet へ入る場合も、
...
`ut-tdd rename plan --json` は `sourceLedgerFreshness`、`cutoverRunbook[]`、`stateBackupManifest[]`、
...
row または packet-only row として扱う。legacy `ut-tdd cutover --dry-run --json` helper は、承認レビュー資料に
...
全体 completion readiness: `ut-tdd status --json` の
