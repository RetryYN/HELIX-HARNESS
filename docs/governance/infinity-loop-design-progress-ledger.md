...
| 親HST内atomic case到達 | 141/153 | 92.16% | 既存448 atomic case。追加12 HIL requirementは未降下、GH-FR-018..021・性能/環境NFR由来37 caseは別のGitHub運用横断family |
...
| HST込み全canonical inventory execution | 0/1,283 | **0.00%** | canonical quartet 835＋canonical HST 448 |
...
| L10 総合テスト | HAT 24＋HST 39 family / 448 atomic case | draft作成済み | L3 trace 72/72に加えGH-FR-018..021・性能/環境NFR横断case 37件、全case `not-implemented` | PO承認、別runtime review、72 ACとGH横断AC実行evidence |
...
いずれもcanonical実行分母には加算しない。canonical HST 448件を加えた全canonical inventoryは1,283件である。
設計inventoryと実行率を混同せず、現在はcanonical quartet oracle execution 0/835、全canonical inventory execution
0/1,283である。source behavior atom分母は未抽出のため別途unknownを維持する。
