...
## 10. Coding Rule Gate（コーディングルールゲート）
...
6. 全 affected feature が全 Gate pass なら Merge Gate へ → 4-tier 判定 → safe ならAI-B明示merge候補
...
漏れ検知 (機能変更だが 3 点のいずれか不足) → block + report
   ↓
全 feature pass → Merge Gate → 4-tier 判定 → safe ならAI-B明示merge候補
```
...
| **CI (GHA)** | guardrail (executory) | PR open / sync / required status checks → safe ならAI-B最終照合待ち |
...
- 4-tier 分類で **safe → AI-B明示merge候補** (人間負荷ゼロ) / **caution → TL/AI review** (1 layer) / **danger → human** (residue) / **unknown → fail-close** (machine 拒否、原因調査は人間)。safeでもnative auto-mergeは設定せず、AI-Bがcurrent HEAD、review、CI、DB receiptを再照合する。
