...
|---|---|---|---|
| Production Scrum | Discovery PoCと同じ扱い、CIなし | 小規模・継続成長・高feedbackの本番delivery engine | `delivery_route` schema、CI selector、fixture |
| V設計＋Scrum実装 | route自体が欠落 | L1〜L5 V freeze後、Scrum increment、RCで全V-pair再収束 | router、PLAN template、DB projection |
