...
本書が証明する基本境界は**固定したentry集合の閉包**と**全entryへの構造分類の付与**である。file、branch、
module群をbehaviorと同一視しない。例外としてUniversal Workflowだけは別の機械台帳で19 atomの登録集合を閉じたが、
runtime verificationとcoverage creditは0である。その他familyの`behavior_atom_closed`は`false`であり、
...
| `ZIP-UNIVERSAL-WORKFLOW-V110` | `repo://UNIVERSAL-WORKFLOW-REQUIREMENTS-SKILL_v1.1.0.zip` | 14 entries / 19 behavior atoms | `sha256:8ae2f2c26cf9a62e853d986b5f36cf92b1a15a17073543c0e13cbe4b37d72dca` | `sha256:a78559cdcfbc05e6f3644437d3888db9d8a2a853204265b1a4d269646bff437f` | `true` | `true`（登録閉包のみ、runtime verified 0/14） |
...
| behavior atom closure | `false` | `true`（19/19登録、runtime verified 0/14） |
...
coverage creditを0とする。Universal Workflowの14 entryは
`docs/governance/generated/universal-workflow-entry-atom-dispositions-v1.json`で19 behavior atomへ原子化し、
`adopt=4/redesign=7/reference=3`のentry disposition、canonical target、assertion、gateを登録した。
これはatom集合の登録閉包であってruntime verificationではない。Universal Workflowを既存`judgment-core`へ名前の類似だけで吸収済みと判定しない。
...
|---|---|---|---|---|---|
| Hybrid ZIP | PASS 703/703 | PASS 703/703 | archive digest固定 | **FAIL** | FAIL |
| Universal Workflow ZIP | PASS 14/14 | PASS 14/14 | archive digest固定 | PASS 19/19登録、runtime verified 0/14 | FAIL |
