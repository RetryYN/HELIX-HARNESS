...
> **Authority transition — `VMAUTH-2026-07-16-01`**
> 本書のgate、state、security、trace要件はlegacy runtime compatibility要件としてcurrentである。L0–L14層番号、旧V-pair、
> L7 implementation、TypeScript/Bun実装記述は新規authoring authorityではない。L1–L12 candidate receiptがpendingの間は
> 新規freeze/実装を禁止し、承認後もimplementation preflightとruntime cutoverを独立gateで判定する。

- **Version**: 1.2
- **対応構想書**: `helix-harness-concept_v3.1.md`
- **位置付け**: 要件定義 (L1-L3 受入条件層)
- **工程層体系**: 本文のL0-L14はlegacy runtime/schema compatibility。L1-L12 candidateはreceipt承認後にactive化する
...
本書は構想書 v3.1 に対するlegacy L0-L14 runtime compatibility要件を記録する。新規authoringの確定は
`vmodel-authority-receipt-v1.md`とL1/L3要件を含むfreeze receiptで行う。本文の**実装詳細 (TypeScript/Bun core / YAML 全文 /
hook wrapper 本体)** はtarget runtimeを意味せず、ADR-009がNode＋Python／Linux-primaryを上位で定める。
...
| R-C7 | failure_log.jsonl .gitignore vs 必須構造矛盾 | §9.1 で「`.helix/audit/` ディレクトリ自体が必須、`failure_log.jsonl` は生成時に作成」と明記 |
...
PLAN ドキュメントの YAML frontmatter は **機械検証可能な必須フィールド** + **enum 制約** で構成する。`helix plan lint` (§7.1) が CI / pre-push で fail-close 動作する。
...
- `kind=poc + status in [confirmed, completed]` → `workflow_phase=S4` かつ `decision_outcome` 必須。S3 verify pass は **検証証跡が揃った非終端状態**であり、terminal status / Forward 昇格 / Reverse 起票の根拠にしない。
...
| `L4` | 基本設計 (外部設計) | ① アーキ/ADR / system verification boundary (旧 L9-system shim は段階移行) | 旧 L2 全体設計 | （層意味論: canonical L9=integration）
...
> - **trace seed (PO 承認 2026-07-05、S4 confirmed)**: L1⟷L2 反復ループ + 収束 gate（A-40 back-propagation 精緻化）+ AI gap-check（read-only）は `PLAN-DISCOVERY-11-l1-l2-elicitation-cycle` の S4 confirmed と `PLAN-REVERSE-329` fullback により正式採用済み。人が L1/L2 を作成・収束宣言し、AI は欠落候補と trace 断絶を列挙するだけとする。収束後は要件定義 = 決定事項として下流を拘束する。
...
| `S3` | poc | Verify (回帰蓄積)。検証証跡は揃っても `status=draft` のまま outstanding に残し、S4 決定前に terminal 化しない |
| `S4` | poc | Decide (decision_outcome 必須)。`confirmed` / `rejected` / `pivot` の PO 判定を記録して初めて terminal / 昇格判断に使える |
| `R0` | reverse | 証跡取得 |
| `R1` | reverse | 観測契約 (§3.3 で reverse_type 別 skip 判定) |
| `R2` | reverse | 現状設計 |
| `R3` | reverse | 意図仮説 (**po 検証**) |
| `R4` | reverse | ギャップと routing (forward_routing 必須) |
...
| drive | 専門職 | L10 (システム検証＋Real UX evidence) 要否 |
|-------|------|----------------------------|
...
| `skill_doc` | HELIX 正本化済み skill doc (`docs/skills/*.md`) | — |
...
| `yaml_config` | YAML 設定 | — |
| `json_config` | JSON 設定 | — |
| `workflow_config` | GitHub Actions workflow / harness YAML 設定 | — |
...
| `qa` | 品質責任者 — テスト戦略 / G8-G9 ゲート (L8 単体・詳細・L9 結合) |
| `aim` | AI実装・保守 |
| `uiux` | UI/UX デザイン |
...
- [ ] 形式が `^PLAN-(L(?:[0-9]|1[0-4])|DISCOVERY|REVERSE|RECOVERY|M)-\d{2,}(-[a-z0-9-]+)?$` に一致 (`helix plan lint` で正規表現検証)
...
#### E2. back-fill pairing 完全性 (駆動モデル整理 / IMP-051、`helix doctor` checkBackfill)
...
- 機構: `helix doctor` の `backfill` 行が `reverseOrphans` / `reverseLinkMissing` / `legacyAuditGaps` /
  `glossaryGaps` / `conditionalPending` / `conditionalDecisionMissing` を surface し、hard violation は
  doctor exit code に連動する。

#### E3. 全プログラム被覆 (program coverage / PLAN-RECOVERY-04、`helix doctor` program-coverage)
...
- 機構: `helix doctor` の `program-coverage —` 行が covered / uncovered (= frontier) を surface。`analyzeProgramCoverage` (`src/lint/roadmap-registry.ts`) が判定。`parkedBandIds` 配線 (park 宣言の単一正本化 + doctor 連動) は **PLAN-REVERSE-44 Step 3 (schema 拡張) で実装予定 (carry)**。lint engine 実装時に exit code 連動 (fail-close) へ昇格予定。

#### F. enum source-of-truth と drift 検知 (R-P1 fix)

- **正本**: 本書 §1 の各 enum 表が正本。
- **validator 同期方針**: VALID_* は `src/schema/*.ts` の **zod enum/literal を単一正本**として定義し本書 §1 表と整合させる (型推論 + 実行時検証を 1 本化、enum drift を型で抑止)。drift 検知のため schema 冒頭に「最終同期: requirements vM.N §1.X」コメントを必須化、`helix doctor` が本書の更新日と schema のコメントを比較し 30 日以上乖離なら warning。
- **将来移行**: 将来的に enum を YAML schema ファイル (`docs/governance/schema/frontmatter-schema.yaml`) に切り出して両者が読み込む構造にする (個別 PLAN-XXX で詳細設計)。

#### G. L 別 sub-doc 構造 (v1.2 で V2 source snapshot reference を HELIX 正本へ再定義、構想書 §3.1.2.1 / §3.1.3.1)

V2 source snapshot reference の設計概念では L1-L6 設計層が sub-doc 分割を持つ。HELIX ではこれを正本要件として再定義する。`kind=design` の PLAN は単一 sub-doc を generates し、複数関心を 1 PLAN に混在させない (構想書 §3.5 AP-11/AP-12)。
...
> **正本同期 (IMP-141 解消、2026-06-22)**: 上表は `src/schema/index.ts` の `VALID_SUB_DOCS` (line 50、正本) を mirror する従属物。L3 は slug `business`/`functional`/`nfr` (実 PLAN `PLAN-L3-01〜03` が使用)、L4 は `screen` を含まない (画面は L2 画面専用層が持つ)。本表と schema の drift は `helix doctor` (`sub-doc-catalog-drift` gate) が fail-close で照合する (silent fix 禁止、errata は双方向に保つ)。
...
5 sub-doc 各々の必須 § (構想書 §3.1.2.1 表) を `helix plan lint` で fail-close 検証:
...
を h2 (`^## §<N> <名称>`) として持つことを `helix plan lint` が fail-close 検証する
...
L0 → L1 → L4 のドメイン継承チェーンを `helix plan lint` (sub_doc=business 時) で検証:

- [ ] business sub-doc §10.1 の業務 entity 一覧 table が以下 4 列を持つ:
  - `業務 entity` / `L0 用語 (参照 path 含む)` / `業務的意味 (BR で扱う側面)` / `対応 .helix state / CLI subcommand / file`
...
- [ ] §10.2 L4 carry section に **集約境界 / 値オブジェクト / entity ID 規約 / ライフサイクル / 不変条件 / 集約間整合性 / `helix doctor check_business_entity_coverage` 新設** の 7 項目が列挙されている (欠落 → P1 warning)
...
各 L 層 design / impl PLAN の §用語更新 section を `helix plan lint` で検証 (living glossary の back-merge 強制、ユビキタス言語の各工程更新):
...
機能一覧 (L1 functional §1) を FR registry の**単一 SSoT** とし、`helix doctor` (`fr-registry-audit` gate) / `src/lint/fr-registry-audit.ts` で **漏れ監査を自動化**する (手動 audit A-51/52/54 の lint 化、doctor 配線は PLAN-L7-95)。漏れ 5 型は doc 間 ID 整合で自動判定:
...
> **architecture 注記**: `implementation_status` (installed/partial/not-implemented) は変動する **runtime state** (`.helix/state/`) に置き、版管理対象の spec table (§1) には**列として持たない** (mutable status を spec に混入させない)。HM-01 は §1 registry (静的属性) × runtime status を join して表示する。`導入工程` (provenance) は現状 §1 `出典 doc` 列に自由記述で内包 (例: "L3 back-propagation")、正規化列化は将来 increment。

##### G.11 doc 間整合チェック (doc-consistency lint、構想書 §3.1.2.2 / A-58)

doc 間の整合を `src/lint/doc-consistency.ts` で自動検証 (`helix doctor` の `doc-consistency` gate として配線、PLAN-L7-95) = L3 到達までの手動 audit (A-51/52/54) の機械化。retro (ledger mine) で「手戻りの大半は既に 3 lint 化済 (g3-trace/entity-coverage/fr-registry-audit)、残自動化可は本 G.11 へ集約」と確認。
...
「作業中に発見した不備・改善を蓄積 → triage → 機能化」する living backlog。SSoT = `docs/improvement-backlog.md` §1。FR-L1-19 (Learning Engine) 本実装までの**手動の橋渡し**。`src/lint/improvement-backlog.ts` で構造健全性を検証 (`helix doctor` の `improvement-backlog` gate として配線、PLAN-L7-95):
...
- CLI: `helix plan lint --gate G1-trace` で実行
...
- 本 §H は §G (L 別 sub-doc 構造) の後段に位置する。`helix plan lint` は §G の sub-doc 種別・フィールド・本文構造検証を先に実行し、G 系 check が pass した screen sub-doc に対して §H の G1-trace lint を実行する (依存順序固定)。
...
| **G4** 基本設計凍結 | L4 ① アーキ/ADR ⇔ system verification boundary (旧 L9-system shim は段階移行) | 片方欠落 → fail | （層意味論: canonical L9=integration）
| **G5** 詳細設計凍結 | L5 ① D-API/D-DB/D-CONTRACT ⇔ L5 ③ 結合テスト設計 (→L9) | 片方欠落 → fail (API/Schema Freeze) |
| **G6** 機能設計凍結 | L6 ① 関数 schema/エッジケース+WBS ⇔ L8 単体テスト設計 | 片方欠落 / WBS 不在 / 関数 sig 未確定 → fail |
...
G6 通過後、L7 実装本体へ入る前に、L8 単体テスト設計に対応する ④ 単体テストコードを先行作成する (L7 スプリントの最初のステップ)。対象テストは未実装の ② 実装理由で fail してよいが、構文エラー・import 経路不備・fixture 不備で fail してはいけない。

| ゲート | Red freeze 対象 | fail-close 条件 |
|--------|------------------|------------------|
| **L7 entry (TDD Red)** | L8 単体テスト設計 ⇔ 先行 ④ 単体テストコード | ③ に対応する ④ が無い / テストが収集不能 / 失敗理由が未実装以外 → fail |
...
|-----------|------|----------|------------|
| 単体テスト | aim / se | L8 単体テスト設計 | L7 実装スプリント内 + L8 単体検証境界 |
| 結合テスト | aim / se | L9 結合テスト設計 | L9 結合テスト |
| 総合テスト (E2E) | qa / aim | system verification boundary | 旧 L9-system shim は段階移行し、L9 結合と混同しない | （層意味論: canonical L9=integration）
| 受入テスト | po / qa | L3 受入テスト設計 | L12 運用・価値検証 |
...
- [ ] 右側工程 (L8 単体・詳細 / L9 結合) でペア未凍結のテスト設計を新規起票 → P0 fail-close (V-model 違反、構想書 v3.1 §3.5 AP-7)
- [ ] 逆ピラミッド (① ② のみ存在) → P0 fail-close
- [ ] G8-G10 (L8 単体・詳細 / L9 結合 / L10 UX) と G11-G14 (総合レビュー+UAT / デプロイ+受入 / デプロイ後検証 / 運用検証) の fail-close 条件は本版では概念定義 (構想書 v3.1 §3.1 / §3.3) に留め、機械検証は将来 PLAN-XXX で詳細設計する (本 §2 は G7 実装凍結までを機械化)
...
| scrum_type（仮説種別） | reverse: code | reverse: design | reverse: upgrade | reverse: normalization | reverse: fullback |
|--|--------------|-----------------|------------------|------------------------|-------------------|
| **hypothesis-test** | **主** | 代替 | 代替 | 代替 | 代替 |
| **tech-spike** | 代替 | **主** | 代替 | 代替 | 代替 |
| **design-spike** | 代替 | **主** | 代替 | 代替 | 代替 |
| **perf-spike** | 代替 | 代替 | **主** | 代替 | 代替 |
| **security-spike** | **主** | 代替 | 代替 | 代替 | 代替 |
| **ux-spike** | 代替 | **主** | 代替 | 代替 | 代替 |
...
| confirmed_reverse_type | R1 (観測契約) |
|------------------------|--------------------------|
...
>
> **`L7` / `L8`-`L11` を forward_routing に含めない理由 (PM アーキ判断で確定、DISCOVERY-04 監査 2026-06-02)**: Reverse は **必ず設計層 (L1/L3/L4/L5) に再入し ①⇔③ pair-freeze gate (G1/G3/G4/G5) を通す** のが V-model 規律。L7 (実装) / L8-L11 (検証) へ直接 routing するのは pair-freeze をバイパスする違反 (source snapshot の緩いモデルでは L7/L8-L11 routing があったが HELIX は不採用)。「実装だけで閉じる」案件は `L5` (→pair-freeze→L7)、fullback の文書整合は対象 ③ の設計層へ routing するか `gap-only`。enum は 5 値で確定 (拡張不要)。
...
`helix plan lint` は recovery kind PLAN の本文に上記 7 セクション header (h2、`## §N <名称>` 形式) があるか機械検証。1 つでも欠落 → exit 1。
...
| 4 | **継続状態の整合** | authored sources・harness.db continuation projection・必要な fenced memory breadcrumb が同一 PLAN / Next Action / carry を指す | **warning** (exit 0、stderr) |
...
- [ ] pre-push hook が §5.3 の判定列に従う:
  ...
  - item 3 (failure_log) と item 4 (continuation readiness) → warning (exit 0、stderr 出力)
...
| subjob（検査単位） | feature | design | research | poc | reverse | add | hotfix | refactor | docs | chore |
|--------|---------|--------|----------|-----|---------|-----|--------|----------|------|-------|
| `plan-lint` | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | — | — |
| `vmodel-lint` | ✓ | ✓ | — | — | — | ✓ | — | ✓ | — | — |
| `branch-kind-check` | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | — | — |
...
> **`helix setup` solo/team がこの 2-stage の emission を担う** (PLAN-L6-05/L7-03、REVERSE-04 back-fill): 参加規模 (owner 種別 / collaborator 数 / 既存 CODEOWNERS・protection) を gh で検出 → solo(0-A)/team(0-B) を提案 → 人間確認 → `.helix/state/setup.json` に確定値記録 → phase 別の GitHub 設定を出し分け生成。数だけで自動確定しない (提案どまり)。`--solo`/`--team` で上書き、検出不能・非対話は solo 安全フォールバック。CODEOWNERS / workflow / ISSUE・PR テンプレ等の **ファイルは harness が emit** するが、**branch protection / Required 化は gh-api 操作で emit-only 既定**とし、明示 `--apply-branch-protection` 時も action-binding approval と gh auth/admin preflight が揃った場合だけ適用する。境界契約は L4 external-if §3 を正本とする。
...
HELIX の全作業は **問題 / ギャップ起点**である。Forward 自体も「発注元 Issue (要件 = 埋めるべきギャップ)」起点 (構想書 §2.5 経路1 entry)。よって **GitHub Issue を作業追跡のスパイン (背骨)** とし、次の一本道で管理する:
...
- 右腕 CI (harness-check schedule job) が **失敗を検知したら Issue を自動起票** (`regression_*` signal 付き) し、**concept §3.1.5「右腕工程の差し戻しルール (L8-L14)」正本**の差し戻し先 (L8→L6/L7・L9→L5・L10→L2・L11→L3/L1・L12→L3/L7・L13→Incident・L14→L1/L3) と紐づける。
...
### 6.8.5 PLAN 完了時 continuation checkpoint (必須、PLAN-REVERSE-344)

**PLAN が完了状態へ遷移する時、authored sources を根拠にappend-only session eventを先にdurable appendし、harness.db continuation projectionを冪等投影してからcheckpointを公開**する。event append後・projection前のcrashは同じevent IDからreplayし、「完了したが何を次にやるか不明」を構造的に防ぐ。JSONLとSQLiteを同一transactionとは見なさない。session/prose handover、`.helix/handover/CURRENT.json`、`helix handover` は正規経路ではなく、生成・読取・freshness 判定を廃止する。

- **必須 projection**: ① PLAN identity / kind / layer / status、②成果物と commit、③順序付き Next Action、④ carry、⑤ escalation、⑥壊してはならない constraint、⑦ source event / artifact provenance。
- **入力**: authored PLAN / design / test-design、session-log の PLAN ダイジェスト、commit・test・gate evidence。人間判断を要する Next Action / carry / escalation は承認済み authored source または明示的な decision event に由来し、AI が欠落値を捏造しない。
- **正本境界**: authored sources が意味の正本、append-only event が事実トレイル、harness.db が再構築可能な進捗・継続 projection、fenced memory が decision / constraint / next action の bounded recall を担う。memory は provenance・scope・TTL を持ち、DB と不整合なら authored sources + DB を優先する。
- **検証**: Stop-hook と `helix doctor` は continuation readiness (projection 欠落 / stale / 別 PLAN drift / provenance 欠落 / memory による上書き) を surface する。さらに retired prose-handover writer・reader・`CURRENT.json`・旧 CLI・旧 aggregate の active surface 復活を fail-close で検出する。
- **型の分離**: `.helix/handover/provider` 等に残る provider delegation evidence、operations transition、Recovery PLAN、archive の歴史記録は session/prose continuation ではない別型として保持する。名前の一致を理由に削除・継続正本化してはならない。

### 6.8.6 進捗管理 = authored sources + event log + state DB + fenced memory

進捗は単一機構でなく、責務の異なる **4 面の組合せ**で管理する:

| 層 | 実体 | 役割 (何を答えるか) | 性質 |
|----|------|---------------------|------|
| **UT-harness DB (state)** | `.helix/harness.db` SQLite projection DB + `.helix/` YAML/JSON state。主要 table: plan_registry / artifact_registry / model_runs / trace_edges / coverage / findings / gate_runs (FR-L1-06) | **今どこまで進んだか** (V-model 製本 state、別駆動 model の実行結果、孤児 0 / coverage / ゆがみ・もれを機械保証) | 機械 SSoT、doctor / vmodel lint で fail-close ([[feedback_vmodel_state_db_completeness]]) |
...
| **authored sources** | PLAN / design / test-design / ADR / decision record | **何を意味し、何を次に行うべきか** | 意味の SSoT、Git tracked |
| **fenced memory** | provenance・scope・TTL 付き decision / constraint / next-action breadcrumb | **次 session で何を bounded recall するか** | 補助 projection、DB を上書きしない |

**組合せ原則**: authored sources = 「意味と判断」、state DB = 「正本から再構築した進捗と継続 projection」、log = 「事実の裏付け」、fenced memory = 「bounded recall」。session-log の PLAN ダイジェストは event→DB projection の入力であり、provenance を保った breadcrumb だけが memory へ昇格できる。session/prose handover をこの結節点へ戻してはならない。
...
### §6.8.7 DB 参照フィードバックと自動化基盤 bundle (2026-06-08)
...
| 探すコストを下げる | PLAN/artifact/finding/skill/model/session の検索用 projection を持ち、`helix find` 相当の CLI が path/ID/reason/evidence を返せる。 |
...
**完了判定**: 下位 L で発見した追加・起票が `requires_design_normalization` / `requires_requirement_backprop` / `requires_concept_policy` のまま未処理なら、元 PLAN を `completed` / `confirmed` と呼ばない。やむを得ず先行実装する場合は `add-design` / `add-impl` と `reverse/*` の pairing を明示し、未完了 carry ではなく back-prop 未了として authored PLAN + DB projection に残す。

**記録項目**: PLAN §7 機能要求更新、audit record、または `docs/improvement-backlog.md` は `backprop_decision`、`reverse_type`、`target_layer`、`upstream_docs`、`evidence_path`、`closure_status` を持つ。`helix doctor` は `improvement-backlog` lint の `missingBackpropClassification` で、下位 L 由来の追加起票がこの分類を持たない場合に hard-gate する。G7 / accept では分類未記録を fail-close として扱う。
...
### §6.8.9 成果物横断 relation graph / visualization / tool adapters (A-124, 2026-06-09)

HELIX は「1 つを直したら、関連する設計・コード・テスト・DB projection・PLAN・FR も合わせて直す」ために、横断 relation graph を `harness.db` projection として持つ。これは authoring source ではなく、docs / source / tests / PLAN / state / logs から再構築できる derived graph である。
...
- `relation_graph -> diagram`: graph snapshot を Mermaid / DOT / D2 のいずれかへ export し、review / gate evidence として読める図にする。
...
### §6.8.10 MCP / 外部 testing tool の適用範囲と workflow trigger (A-125, 2026-06-09)

A-124 の relation graph / diagram / impact expansion は、MCP server や外部テスト基盤を使うことで大幅に強化できる。ただし MCP server は host 権限・filesystem・browser・GitHub・DB へ接続し得るため、常時接続や raw tool output gate は禁止する。HELIX は **allow-list された tool profile を workflow trigger で必要時だけ起動し、結果を DB projection へ正規化して gate が見る**。

#### 採用候補 (Web research 2026-06-09)

| カテゴリ | 候補 | 適用範囲 | 採用方針 |
|---|---|---|---|
| MCP discovery / trust | MCP Registry | 公開 MCP server metadata / install metadata / namespace verification | discovery metadata だけの候補 source とする。Registry metadata は security scan ではない。 |
| MCP debug / test | MCP Inspector | MCP server の tools/resources/prompts 接続確認、local server smoke | HELIX-HARNESS 所有 MCP server または設定済み server profile の推奨 verification tool とする。 |
| Browser automation MCP | Microsoft Playwright MCP (`@playwright/mcp`) | exploratory browser verification、self-healing E2E investigation、screenshots | 任意の interactive verification profile。決定論的 CI では Playwright CLI/tests を優先する。 |
| GitHub workflow MCP | GitHub MCP Server | issues / PR / repos / actions / code_security toolsets | issue / PR / backlog automation 用の任意 profile。既定では narrow toolsets または read-only mode を使う。 |
| Reference MCP servers | filesystem / git / memory / fetch / postgres / sqlite | local file、git、memory graph、web fetch、DB inspection | reference または制御済み local profile に限る。default profile で production credential を使わない。 |
| Containerized MCP gateway | Docker MCP Toolkit | signed/attested container images、OAuth、resource limits、profile-based MCP gateway | Docker Desktop が使える team/enterprise runtime profile では優先候補とする。 |
| Test foundation | Vitest Browser Mode + Playwright provider | browser-native component tests と UI interaction checks | UI/browser 対象 harness または対象 repo の任意 L7/L8 test profile。 |
| Test foundation | Testcontainers for Node.js | integration/smoke tests 用の使い捨て DB/service containers | Docker が使える場合の任意 integration-test profile。 |
| API mocking | MSW | browser / Node tests 用の reusable REST/GraphQL/WebSocket mocks | API-bound tests と fixture standardization 用の任意 mock profile。 |

#### workflow trigger rules（発火ルール）

- `signal=ui_flow` / `web_target` / `browser_regression` は `mcp_profile=playwright` または `test_profile=vitest-browser-playwright` を推薦する。
- `signal=external_issue` / `ci_failure` / `pr_review` / `backlog_sync` は、まず `mcp_profile=github-readonly` を推薦する。write toolsets には明示的な人間承認を必須とする。
- `signal=db_integration` / `migration` / `service_contract` は `test_profile=testcontainers` と DB projection review を推薦する。
- `signal=api_mock_gap` / `flaky_external_api` は `test_profile=msw` を推薦する。
- `signal=mcp_server_added` / `mcp_profile_changed` は MCP Inspector smoke (`tools/list` 最小) を実行し、`mcp_server_runs` を記録する。

#### Safety and automation constraints（安全性と自動化制約）

- MCP profiles は既定で disabled とする。`helix mcp profile enable <name>` は将来 scope であり、Git 管理 secrets の外側に generated local config を書く。
...
- Default GitHub MCP profile は read-only とし、discovery/status に必要な最小 toolsets だけを有効にする。PR/issue write actions には明示的な `requires_human_approval` を必須とする。
- Filesystem / Git MCP profiles は workspace root に制限し、global home-directory mounts を渡してはならない。
...
- `mcp_server_runs` / `verification_recommendations` は **`session_id` / `plan_id` を保持**し、どの PLAN / session に紐づく外部検証かを trace 可能にする (continuation projection / review evidence と突合できること。physical-data §9.6 カラムの要求根拠、A-128 F-3 back-fill)。
...
- `helix mcp profile list --json`
- `helix mcp profile probe <name>`
- `helix mcp inspect <name> --method tools/list [--allow-external]`
- `helix verify recommend --changed <path> [--format text|json|mermaid] [--save-evidence]` -> changed-file signal graph から推奨 MCP/test profiles を返す
- `helix verify run --profile <name> [--dry-run] [--allow-external] [--save-evidence]` -> 既定では built-in profiles を実行する。external profiles は実行前に明示 allow-list と probe checks 充足を必須とする

`--save-evidence` は後続 DB collector/rebuild のため、正規化済み JSON records を `.helix/evidence/verification-profiles/` 配下に書く。これらのファイルは bounded metadata evidence であり、raw provider transcripts や secret を含む tool output ではない。

**完了判定**: A-125 は、requirements、physical data、ADR/backlog/audit、workflow docs が candidate tools、trigger rules、safety constraints、DB projection tables、commands を定義した時点で scoped とする。最初の runtime slice は、`helix mcp profile list/probe`、`helix mcp inspect` readiness gating、`helix verify recommend`、`helix verify run --dry-run`、`--save-evidence`、`doctor` が profile catalog / readiness / recommendation evidence を surface した時点で implemented とする。full implementation には、実際の MCP Inspector server invocation、external profile execution evidence、external verification rows 向け DB collector/rebuild がなお必要である。

### §6.8.11 Canonical document export（正本文書 export） (A-126, 2026-06-09)

A-124/A-125 により relation graphs、diagrams、MCP/test profiles、evidence は query 可能になる。human reviewers には、canonical HELIX documents (concept / planning、requirements、detailed design、PLAN、ADR、test-design documents) を spreadsheet / Excel / PPTX へ変換する手段も必要である。したがって HELIX は **canonical document export** を derived artifacts として scope する。

**Source-of-truth boundary（正本境界）**:

- Markdown/source documents、PLANs、ADRs、test-design docs、DB projection rows、tests、evidence records を authoritative source として維持する。
- CSV / Markdown summary / XLSX / PPTX files は generated conversion artifacts に限る。
- Exported files は source document paths、source section IDs、snapshot hash、renderer、format、path、redaction profile、evidence path を記録する。
- export artifact の削除や手動編集は harness truth を変更してはならない。export を根拠にした human decision は、review / gate / decision evidence として別途 import または記録する。

**Baseline outputs（標準出力）**:

- `doc-csv-matrix`: built-in / zero-dependency とし、requirements、design、PLAN、ADR、test-design matrices に deterministic columns を提供する。
- `doc-markdown-summary`: built-in とし、source links と section IDs を含む GitHub-readable conversion summary を生成する。

**Optional renderer outputs（任意 renderer 出力）**:

- `doc-xlsx-workbook`: concept、requirements、design、PLAN、ADR、trace、test-design rows を複数 sheet に分ける Excel workbook。candidate adapters は ExcelJS または SheetJS。
- `doc-pptx-deck`: concept、requirements、detailed design、PLAN、ADR、test-design structure から生成する PowerPoint deck。candidate adapter は PptxGenJS。
- `doc-d2-pptx-diagram`: D2 readiness が証明済みの場合、architecture / workflow / relation graph visuals を PPTX へ export する。

**Trigger rules（発火ルール）**:

- `requirements_export` / `fr_ac_at_matrix` / `acceptance_review` は CSV と任意 XLSX を推薦する。
- `concept_export` / `planning_review` / `stakeholder_brief` は Markdown summary と任意 PPTX を推薦する。
- `detailed_design_export` / `architecture_review` / `db_contract_review` / `api_contract_review` は CSV/XLSX と任意 PPTX を推薦する。
- `plan_export` / `adr_export` / `test_design_export` / `continuation_snapshot` は document family に応じて Markdown summary、CSV、任意 XLSX/PPTX を推薦する。snapshot は generated view であり正本ではない。
- `document_export_profile_changed` は accept 前に renderer probe evidence を必須とする。

**Safety and quality constraints（安全性と品質制約）**:

- Exports は rendering 前に redact しなければならない。将来の human-approved policy が redacted attachment profile を定義しない限り、raw provider transcripts、credentials、secrets、PII、raw MCP payloads、screenshots、browser traces を含めてはならない。
- Optional renderers は既定で disabled とする。ExcelJS / SheetJS / PptxGenJS / D2 が使えない場合は finding を返し、暗黙 install は行わない。
- Source section IDs / FR IDs / AC IDs / AT IDs / PLAN IDs / ADR IDs は generated spreadsheet/deck output 内で見える状態を保つ。
- Generated spreadsheets と decks は、明示 timestamp metadata を除き、同一 source snapshot に対して deterministic でなければならない。
- Large exports は黙って truncate せず、document family または section ごとに rows を chunk するか sheets/slides を分割する。

**完了判定**: A-126 は、requirements、research、audit、physical-data、ADR/backlog、workflow docs、L6 function contracts、L7 unit oracles、L6/L7/Reverse PLANs が canonical document export profiles と safety boundaries を定義した時点で scoped とする。runtime implementation には将来の L7 TDD Red entry が必要であり、本節は source changes を承認しない。
...
```
scripts/
├── helix                    # POSIX / Git Bash ディスパッチャ (compiled core を呼ぶ)
├── helix.ps1                # Windows PowerShell ディスパッチャ (compiled core を呼ぶ)
├── install-hooks.sh
└── install-hooks.ps1
```
...
`helix` は Claude Code / Codex 連携を必須にしない。起動時に runtime を検出し、以下の mode を `.helix/state/runtime.json` に記録する。

| mode | 検出条件 | 動作方針 |
|------|----------|----------|
| `claude-only` | `.claude/` 設定または Claude Code hook が存在し、Codex CLI が未検出 | Claude hook / prompt 生成 / DB + fenced memory continuation を有効化。Codex 委譲は `not-available` |
...
| `hybrid` | Claude Code runtime と Codex CLI の両方を検出 | `team run` / role delegation / cross-agent review / DB + fenced memory continuation を有効化 |
| `standalone` | どちらも未検出 | `setup` / `doctor` / `plan lint` / `vmodel lint` / `gate` のローカル検証のみ有効化 |

`runtime.json` は generated state とし、Git 管理しない。`helix status --json` は `RuntimeDetection` の 6 フィールド + `nextAction` を返す = `mode`, `claude`, `codex`, `currentRuntime`, `availableRuntimes`, `missingRuntimes`, `nextAction` (`src/runtime/detect.ts`、function-spec §6 型表 / §1.2)。camelCase のこの 7 フィールドが公開サーフェス契約 (実装を正本とし、フィールド名は保存する)。加えて judgment gate guidance を machine-readable に降ろす additive field `judgmentReview` を返す。`judgmentReview` は runtime mode から `requiredReviewKind`、`crossAgentReview`、`requiredAction`、`gateCommandTemplate`、`requiredEvidence[]` を出し、hybrid では `cross_agent` + worker/reviewer model、単一 runtime では `intra_runtime_subagent` + checklist、standalone では human approval を要求する。加えて whole-program / L14 completion guidance は runtime mode とは別の additive field `workflowNextAction` で返す。`nextAction` は mode→judgment-gate guidance、`workflowNextAction` は `outstanding.completionReadiness` 由来の completion guidance であり、未了 blocker があれば `completion-blocked:`、未了なしの場合だけ `completion-ready:` で始まる。これにより hybrid の `cross-review-ready:` が PO/S4 判断、version-up parked、L14 irreversible cutover signoff を隠さない。`optional_adapters` / `enabled_commands` / `disabled_commands` の付加と snake_case 別名は**未実装の forward 要件**である。これらの **taxonomy 区分 (A-138 ITEM-1、cross_agent TL/Codex 裏取り済)**: requirements field は 3 区分で扱う — **current** = 実装済かつ公開契約 / **future** = 要件にあるが未実装で着地予定 (追跡 PLAN なし) / **carry** = 能動 defer で追跡 PLAN/gate あり。この区分に従い、`optional_adapters` / `enabled_commands` / `disabled_commands` は **`future`** (adapter/command surface の設計が固まるまで実装しない)。`next_action` は **`current`** に昇格した: 正式フィールド名は既存 6 フィールドの **camelCase 公開契約に揃え `nextAction`** で確定し (snake_case 別名は付さない)、値域は mode→judgment-gate guidance の安定機械契約文字列 (`standalone`=`human-review-required:…` / 単一 runtime=`single-runtime:…` (intra_runtime_subagent 証跡) / `hybrid`=`cross-review-ready:…`)。先頭 token (`:` 手前) で機械 switch でき後続が人間可読。**PLAN-L7-84 で実装・carry discharge 済** (`nextActionForMode`、U-DETECT-001..005)。standalone の判断ゲート提示 (§7.8.7.1 / 「複数 AI orchestration」節) の要件意味と整合。要件を黙って削除しない (実装事実 + 区分を記録)。

### 複数 AI orchestration

`hybrid` mode または `task-capable` 以上の optional adapter が複数ある場合、`helix team run` は以下の原則で role を割り当てる。モデルの実名は頻繁に変わるため、要件定義では能力クラスを正本とし、実モデル名は `.helix/teams/*.yaml` または local override で指定する。
...
#### role × capability_class matrix（対応表）
...
`helix team run --definition .helix/teams/<team>.yaml` は実行前に以下を検証する。
...
### Optional AI IDE adapters（任意 AI IDE adapter）
...
| `other` | user-defined adapter config | detect / status | `.helix/adapters/*.yaml` で拡張可能 |

adapter 結果は `.helix/state/tool-adapters.json` に generated state として保存する。`helix adapter list --json` は adapter name、probe_status、version、capabilities、missing、hosted_runtimes、stability (`stable` / `preview` / `experimental`)、integration_level、safe_commands を返す。
...
adapter は「存在する」と「harness から連携できる」を分けて判定する。`helix adapter probe <name>` は以下の順で評価し、到達した最大段階を `integration_level` として返す。
...
`detected` だけでは `helix adapter run` を許可しない。`adapter run` は `task-capable` 以上、かつ `safe_commands` に含まれる command のみ実行する。`roundtrip-capable` でない adapter は実行後に「人間確認待ち」の finding / Next Action を harness.db continuation projection に記録する。
...
| Subcommand | 用途 |
|------------|------|
| `helix status` | mode / runtime / continuation readiness / next action の状態確認 |
| `helix plan lint` | frontmatter schema 検証 |
| `helix vmodel lint` | 4 artifact + trace 検証 |
| `helix doctor` | 統合検証 |
| `helix self-test` | harness 内蔵の小テスト (CLI routing / schema smoke / fixture smoke) |
| `helix setup` | 初期ディレクトリ / hook / local config の bootstrap |
| `helix task classify` | 入力文 / PLAN / diff から kind / drive / size / complexity を仮判定 |
| `helix task estimate` | 三点見積もり + リスク係数で effort_hours / story_points を算出 |
| `helix skill suggest` | PLAN / diff / text から適用 skill pack 候補を推挙 |
| `helix gate G<N>` | G0.5-G14 ゲート判定 |
| `helix claude ...` | Claude Code 用 prompt / hook 状態操作。`claude-only` / `hybrid` で有効 |
| `helix codex ...` | Codex CLI 実行。`codex-only` / `hybrid` で有効 |
| `helix team run ...` | Claude Code × Codex 連携実行。`hybrid` のみ有効 |
| `helix adapter list` | optional AI IDE adapter の検出状態を表示 |
| `helix adapter probe <name>` | adapter の integration_level と capability を再判定 |
| `helix adapter run <name> ...` | safe_commands に含まれる adapter command のみ実行 |

詳細実装は将来の個別 PLAN-XXX で詰める。

### mode 別コマンド保証

| command group（コマンド群） | standalone | claude-only | codex-only | hybrid |
|---------------|------------|-------------|------------|--------|
| `setup` / `status` / `doctor` | ✓ | ✓ | ✓ | ✓ |
| `plan lint` / `vmodel lint` / `gate` | ✓ | ✓ | ✓ | ✓ |
| `task classify` / `task estimate` / `skill suggest` | ✓ | ✓ | ✓ | ✓ |
| `claude` / Claude hook guard | — | ✓ | 利用不可 | ✓ |
| `codex` | — | not-available | ✓ | ✓ |
| `team run` | — | 利用不可 | 利用不可 | ✓ |
| DB + fenced memory continuation | ✓ | ✓ | ✓ | ✓ |
...
```bash
helix task classify --text "audit log の vmodel lint を追加"
helix task classify --plan docs/plans/PLAN-123-audit.md
helix task classify --diff origin/main...HEAD
```
...
```bash
helix task estimate --plan docs/plans/PLAN-123-audit.md
helix task estimate --text "Windows 対応込みで CLI を追加"
```
...
```bash
helix skill suggest --plan docs/plans/PLAN-123-audit.md
helix skill suggest --text "追加機能設計と QA 追加テストを整理"
```
...
| command（コマンド） | primary class（主 class） | escalation（エスカレーション） |
|---------|---------------|------------|
| `task classify` | `fast-checker` / rule-based | `L` / `XL` / confidence < 0.7 → `frontier-reviewer` review（判定 review） |
| `task estimate` | rule-based + `fast-checker` | risk_factor ≥ 1.6 または production impact → `frontier-reviewer` review |
| `skill suggest` | `fast-checker` / rule-based | required skill 欠落または vendor_candidate → `tl` review |

> **Dynamic skill injection materialization（skill 注入の具体化、PLAN-L7-135, 2026-06-23）**:
> `helix skill suggest --inject --json` は provider-neutral manifest を返し、
> その内容は skill paths/reasons のみに限定する。`helix codex --plan ...`、
> `helix claude --plan ...`、`helix team run --plan ...`、`helix task route
> --plan ... --execute` は、その manifest を argv ではなく provider stdin へ materialize しなければならない。
> これにより、Claude と Codex は `docs/skills/*` body を全件 load せず、同一の scoped context を受け取る。
...
| Hook | 検証内容 | 想定時間 |
|------|----------|----------|
| **pre-commit** | gitleaks / commitlint format / 軽量 lint (markdown / yaml) + `helix self-test --smoke` | < 5s |
...
| tier | 内容 | 実行場所 | 目的 |
|------|------|----------|------|
| `smoke` | CLI 起動、subcommand routing、schema fixture、adapter probe dry-run | local hook / `helix self-test --smoke` | 即時フィードバック |
...
原則として、`full` と `nightly` をローカル hook の必須経路に入れない。ローカルで実行したい場合は明示コマンド (`helix self-test --full`) とする。

## 7.6 受入条件 (機械検証)

- [ ] `helix plan lint` が §1 の全 enum 違反を fail-close
- [ ] `helix vmodel lint` が §7.3 の kind 別経路で正しく分岐し、§7.3 の 3 段 exit code を返す
...
- [ ] `helix self-test --smoke` はネットワーク不要・外部 AI runtime 不要で通る
- [ ] PR merge gate は GitHub Actions `harness-check` のみを正本とし、ローカル hook 成否だけを merge 条件にしない
- [x] **ルール同一性 (MUST、構想書 §2.1.0)**: gate / V-model / checklist / enum / route の正本は `helix` core + governance docs に単一定義され、`.claude/CLAUDE.md` / `AGENTS.md` がルールを再定義・分岐していない (doctor が両 adapter のルール重複・drift を検出し、検出時 fail)。`src/lint/rule-drift.ts` + doctor `checkRuleDrift` が AGENTS / CLAUDE adapter docs の必須 mode / command marker drift を fail-close 検出 (2026-06-08)。
- [x] **rule parity test**: 同一 PLAN / diff を claude-only と codex-only で処理した際、`helix gate` / `helix plan lint` / `helix vmodel lint` の **判定結果と exit code が一致**する (runtime 差で結果が変わらない)。`helix gate` の判断ゲート review-tier は `evaluateGateReview` parity test で codex-only/claude-only 同一結果を機械検証 (2026-06-08)。
- [x] **hybrid 機能分散 (MUST、構想書 §2.1.0)**: `helix team run` が `hybrid` で判断系 / 実行系を別 runtime に割り当て、同一 role の同一 runtime 重複・同一作業の二重実行を exit 1 で弾く (§7.1 team run 検証 7)。`validateTeamRun` が worker/reviewer provider 分離・duplicate role/provider を fail-close (2026-06-08)。
...
- [x] Integration GWT: L8 IT-* row は 前提/操作/期待結果 を持つ。
...
| skill pack | 主な参照元 | HELIX での用途 |
|------------|------------|-----------------|
| `planning-pack` | historical requirements-handover / dev-policy / schedule-wbs | 企画、要件、WBS、継続判断の authored source 化 |
...
- `legacy source` 固有名、個人プロジェクト前提、WSL2 固定、絶対パスは HELIX 用語と相対パスへ置換する。
...
- [ ] skill pack は vendor snapshot を直接参照せず、HELIX 正本化済みの本文を参照する

---

# §7.8 配線要件 (signal routing / RecommendedCommandV1 / orchestration_mode / layer-context 注入)

構想書 v3.1 §2.6 の配線を機械検証可能な形に確定する。V2 (`route_engine.py` / ADR-042 / `vmodel-semantics.yaml`) の routing/injection を HELIX 向けに翻案する。legacy runtime command name・legacy DB 依存・個人絶対パスは持ち込まず、`helix *` 相当・`.helix/` state・package-local に再定義する。

## 7.8.1 signal → mode routing 要件

`helix route eval --signal <signal> [--env <env>] [--drift-type <type>]` が signal を 9 mode (構想書 §2.5) のいずれかに解決する。最小マッピング (config 化、`.helix/config/route-map.yaml` 相当):

| signal | mode | 補足 |
|--------|------|------|
| `drift` (+drift_type=schema/contract) | reverse | normalization（正規化） |
...
| `pair_agent_tdd` / `pair-agent-tdd` / `pair-agent TDD route` / `pair programming` | add-feature | smart test author -> lightweight implementation -> smart review の pair-agent TDD 経路に入る。mode は add-feature のまま、`recommended_command.command` は `helix pair-agent plan` |
...
route 結果は **人間向け表示 (`suggest_command`、文字列)** と **機械契約 (`recommended_command`、JSON)** を分離する。`helix route eval --format json` が以下を返す:
...
- `command` は必ず `helix` 始まり (legacy runtime command name を含めば exit 1)。
...
- pair-agent TDD 系 signal は `mode=add-feature`、`recommended_command.command="helix pair-agent plan"`、`args.pair_route="smart_test_author_to_light_implementation_to_smart_review"`、`args.requires_plan_id=true` を返す。これは route eval の推薦面だけを切り替えるもので、独立 mode や完了承認ではない。
...
`safety.requires_human_approval: true` のとき、**承認者を `.helix/config/approval-policy.yaml` から解決** し、未承認では当該 command を実行しない。最小ポリシー:
...
- 承認記録 (承認者 identity / timestamp / 対象) を `.helix/` audit に append する。
- `requires_preflight: true` (upgrade 高リスク) は `helix doctor --preflight <type>` 相当の前段検証 pass を要求する。

## 7.8.4 layer-context 注入 + orchestration_mode 要件

`.helix/config/vmodel-semantics.yaml` 相当が drive × layer (L0-L14) に以下を注入する。`helix vmodel show <drive> <layer> --injection` が取得する:
...
interrupt / debt / drift-check / readiness と doctor 検出器 (relation-graph / doc-drift / connection-deficiency / regression) + test-perspective-gate を `helix doctor` / `helix plan lint` に束ねる。検出は `.helix/` state を参照し、legacy DB には依存しない。test-perspective-gate は V-model 各ペアの **観点網羅** (抜け) と **レベル間非重複** (重複) を `--static-only` で fail-close 検証する。

## 7.8.6 受入条件 (配線)

- [x] `helix route eval --signal <s> --format json` が RecommendedCommandV1 (schema_version/command/args/safety) を返す (`src/workflow/contracts.ts` + `src/cli.ts`, 2026-06-23)
...
- [x] `command` が legacy runtime command name を含めば exit 1、`helix` 始まりのみ許可 (`recommendedCommandV1Schema` + route-map fail-close, 2026-06-23)
- [x] `requires_human_approval: true` で承認者ポリシー未解決または未承認なら exit 1 + 承認記録を audit に残す (`src/workflow/contracts.ts` + `.helix/audit/route-approval.jsonl`, 2026-06-23)
- [x] `helix route eval` が escalation 境界を `escalation_boundaries[]` として検出し、mode に依存せず `requires_human_approval=true` / 未承認 exit 1 にする (`src/workflow/contracts.ts`, 2026-06-23)
- [x] `helix vmodel show <drive> <layer> --injection` が 5 注入 key を返し、`orchestration_mode` は VALID_ORCHESTRATION_MODES のいずれか (`src/vmodel/injection.ts`, 2026-06-23)
- [x] 配線 config / 検出器が legacy `legacy DB` / 個人絶対パスに依存しない (`.helix/` YAML/JSON state + `.helix/harness.db` projection DB のみ、route-map config fail-close 2026-06-23)

## 7.8.7 execution mode × レビューゲート切り分け (gate 崩壊防止、構想書 v3.1 §2.1.2.1)

cross-agent review は別 runtime / 別モデルが前提のため、単一エージェント環境では成立しない。`helix gate` が execution mode を見ずに判断ゲートを通すと self-review が cross-agent review に化けて崩れる。これを機械的に防ぐ。レビュー強度は 3 ティア (構想書 §2.1.2.1): ① cross-agent (hybrid) / ② 専門サブエージェント (単一エージェント、明文化 checklist 駆動) / ③ naive self-review (判断ゲート通過根拠として不可)。
...
`helix gate <G>` は起動時に `helix status` の `mode` を読み、**判断ゲート (G0.5 / G2 / G4-G7 / R4)** で以下を適用する:
...
- XR-2: escalation 境界 (本番影響 / 認証 / 認可 / 決済 / PII / secret / ライセンス / schema migration / destructive / 外部 API・infra 変更) の該当判定。該当すれば「人間サインオフ必須」フラグを立てる (mode 問わず hard-block)
...
- DUP-1: **重複実装 / 機能被りが無い** — 同等機能の既存実装と被っていない。L7 着手前に `helix code find` / `helix code dup` (相当) で流用候補を確認した記録があること。被りがある場合は再実装せず Add-feature / Refactor mode へ回す (アンチパターン AP-9)
...
> `helix code find` / `dup` の code-index 実体が未整備の環境では、grep + L6 機能設計 doc / dependency-map との突合で代替し、その手段を根拠欄に記録する。

### 受入条件 (gate 崩壊防止)

- [x] `helix gate <判断ゲート>` は `helix status` の `mode` を参照し、`hybrid` 以外では gate 結果 JSON に `cross_agent_review: unavailable` を必ず含める (含まなければ実装不備として exit 1)
...
- [x] `orchestration_mode` が要求する agent が現 execution mode で不在の場合、silent fallback せず縮退 (§7.8.4 / 構想書 §2.1.2.1) を適用し、`degraded_from` / `degraded_to` を記録する (`helix vmodel show ... --injection --mode <mode>`, 2026-06-23)
- [x] **escalation 境界 (本番影響 / 認証 / 認可 / 決済 / PII / secret / ライセンス / schema migration / destructive / 外部 API・infra 変更)** に該当する変更 (XR-2 で検出) は execution mode を問わず人間サインオフ必須。未承認なら mode に関わらず exit 1 (② でも代替不可、§8 と整合)。`helix route eval` が signal から escalation 境界を検出し、`mode: "*", condition: "escalation"` または該当 mode の approval rule が無ければ fail-close する (2026-06-23)。2026-06-30 に version-up activation の HMAC/webhook/access-control/external-infrastructure 語彙を追加し、PLAN-L7-146 の future 保全と外部配信 activation を分離して検出する。
...
|---------|------|---------|--------------|
| **個人作業ログ** | `.helix/audit/failure_log.jsonl` | **`.gitignore`** | ローカル pre-push hook / `scripts/log-failure.sh` |
...
### GitHub failure corpus（GitHub 失敗証跡 corpus）
...
```
.helix/audit/failure_log.jsonl
.helix/audit/escalation_state.json
.helix/cache/*
!.helix/cache/.gitkeep
```
...
5. 結果を GitHub Actions artifact / PR label / PR comment に書く。ローカル実行時のみ `.helix/audit/escalation_state.json` に advisory cache を生成する
```
...
| session 断絶・認識ずれ | harness.db continuation projection + fenced memory、または `recovery` PLAN | pre-push は continuation readiness 不整合を warning、旧 prose-handover 復活または recovery 7 セクション欠落を exit 1 |
...
- debt register / deferred finding（繰延 finding）
- skill pack update
- orchestration policy update
- 継続 checkpoint / fenced memory 方針の更新
...
│   ├── helix.ps1                                # A (Windows PowerShell entrypoint)
│   ├── install-hooks.sh                          # A
...
> **scripts/ 整流 (ADR-001)**: `plan lint` / `vmodel lint` / `doctor` / `gate` / escalation / failure log の各機能は個別 `.sh` ではなく compiled `helix` の**サブコマンド** (TS core、§7.1) として実装する。`scripts/` には OS entrypoint (`helix` / `helix.ps1`) と installer / 一回限り ops のみを置く。TS 依存・config は **root の `package.json` / `tsconfig.json` に集約**し、`scripts/` 配下に重複させない (repository-structure.md §8)。
...
- [ ] `.gitignore` に §8.4 のエントリ (`failure_log.jsonl` / `escalation_state.json` / `.helix/cache/*` + `.gitkeep` 例外) が含まれる
...
| 3 | `.gitignore` に §8.4 エントリ | bash: `grep -q 'failure_log.jsonl' .gitignore && grep -q 'escalation_state.json' .gitignore && grep -q '.helix/cache/\\*' .gitignore && grep -q '!.helix/cache/.gitkeep' .gitignore` / PowerShell: `Select-String` で同等確認 |
...
- [ ] §5.3 の pre-push 4 項目が動作 (continuation projection / memory drift で warning、旧 CURRENT.json 復活で fail-close する等)
