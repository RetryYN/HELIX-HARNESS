...
- MCP profiles は既定で disabled とする。`ut-tdd mcp profile enable <name>` は将来 scope であり、Git 管理 secrets の外側に generated local config を書く。
...
- `ut-tdd verify recommend --changed <path> [--format text|json|mermaid] [--save-evidence]` -> changed-file signal graph から推奨 MCP/test profiles を返す
- `ut-tdd verify run --profile <name> [--dry-run] [--allow-external] [--save-evidence]` -> 既定では built-in profiles を実行する。external profiles は実行前に明示 allow-list と probe checks 充足を必須とする

`--save-evidence` は後続 DB collector/rebuild のため、正規化済み JSON records を `.ut-tdd/evidence/verification-profiles/` 配下に書く。これらのファイルは bounded metadata evidence であり、raw provider transcripts や secret を含む tool output ではない。

**完了判定**: A-125 は、requirements、physical data、ADR/backlog/audit、workflow docs が candidate tools、trigger rules、safety constraints、DB projection tables、commands を定義した時点で scoped とする。最初の runtime slice は、`ut-tdd mcp profile list/probe`、`ut-tdd mcp inspect` readiness gating、`ut-tdd verify recommend`、`ut-tdd verify run --dry-run`、`--save-evidence`、`doctor` が profile catalog / readiness / recommendation evidence を surface した時点で implemented とする。full implementation には、実際の MCP Inspector server invocation、external profile execution evidence、external verification rows 向け DB collector/rebuild がなお必要である。
...
`ut-tdd` は **薄い OS 別ラッパー + TypeScript core (Bun)** で実装する (ADR-001)。これは **harness 自身の実装言語**であり、HELIX-HARNESS が統制する **対象リポジトリの言語は非依存** (§2.3 等の trace 例に出る `.py` / `tests/*` は target repo の一例で、TS への統一対象ではない)。Windows / macOS / Linux の entrypoint は同一 TypeScript core を呼び、OS 差分は wrapper 層に閉じ込める。Windows では PowerShell entrypoint を提供し、Git Bash が必要な既存 hook / shell script は明示的に bridge する。
...
> `ut-tdd skill suggest --inject --json` は provider-neutral manifest を返し、
> その内容は skill paths/reasons のみに限定する。`ut-tdd codex --plan ...`、
> `ut-tdd claude --plan ...`、`ut-tdd team run --plan ...`、`ut-tdd task route
...
構想書 v3.1 §2.6 の配線を機械検証可能な形に確定する。V2 (`route_engine.py` / ADR-042 / `vmodel-semantics.yaml`) の routing/injection を HELIX 向けに翻案する。legacy runtime command name・legacy DB 依存・個人絶対パスは持ち込まず、`ut-tdd *` 相当・`.ut-tdd/` state・package-local に再定義する。
...
- [x] **escalation 境界 (本番影響 / 認証 / 認可 / 決済 / PII / secret / ライセンス / schema migration / destructive / 外部 API・infra 変更)** に該当する変更 (XR-2 で検出) は execution mode を問わず人間サインオフ必須。未承認なら mode に関わらず exit 1 (② でも代替不可、§8 と整合)。`ut-tdd route eval` が signal から escalation 境界を検出し、`mode: "*", condition: "escalation"` または該当 mode の approval rule が無ければ fail-close する (2026-06-23)。2026-06-30 に version-up activation の HMAC/webhook/access-control/external-infrastructure 語彙を追加し、PLAN-L7-146 の future 保全と外部配信 activation を分離して検出する。
