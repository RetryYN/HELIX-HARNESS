#!/usr/bin/env python3
"""Wave23 schema10 research-premise verifier.
...
LEDGER = ROOT / "docs/governance/legacy-requirement-direct-semantic-review-wave23.jsonl"
META = ROOT / "docs/governance/legacy-requirement-direct-semantic-review-wave23.meta.json"
...
PARENT = "c354b7d9177ad3ea92dec30c66c36e6ce2d66ae3"
MAIN_MERGE_PARENTS = ["ac54c3f9008bb501d6f0c501e66137c21cd5f149", "30e87714ef09c89258a93bbbd5567ea753fbbf96"]
REQ_SHA = "80e965736a91f99b2ebb77fba2e63a4bf86d5ab5df6fde1d9685f57b42457688"
UNITS = ["IRUNIT-HIL-FR-01-HELIX-OS", "IRUNIT-HIL-FR-02-HELIX-OS"]
SELECTED = {
    UNITS[0]: {"LEGACY-ASSET-3FEC0B8E1BD14109D214", "LEGACY-ASSET-A60CF91DD2AF6693E6F9", "LEGACY-ASSET-BB08D70A42B6445B2D1E"},
    UNITS[1]: {"LEGACY-ASSET-944C5027F71733A26597", "LEGACY-ASSET-A60CF91DD2AF6693E6F9", "LEGACY-ASSET-DFF9BADFBFC93A632EBA"},
}
REQ_DIGESTS = {
    "HIL-FR-01": "sha256:bf1e51a051e81e130843ed584640fcc1df69d9b5d84f22f22aa29c261b03c3b7",
    "HIL-FR-02": "sha256:022383e2542716540cf4fc42ef8d52b58606a03fdf50eb77de378a5d374236b7",
}
ROW_FIELDS = {
...
def verify_prior_lineage(meta: dict) -> None:
    require(len(meta["prior_review_batches"]) == 22, "prior batch count")
...
def verify() -> None:
    ...
    require(meta["main_merge_revision"] == PARENT and meta["main_merge_parents"] == MAIN_MERGE_PARENTS, "merge lineage")
    require(meta["source_main_base_revision"] == PARENT and meta["reviewed_unit_ids"] == UNITS, "source base/units")
    require([r["review_id"] for r in rows] == [f"LSRW23-EDGE-{i:03d}" for i in range(1, 7)], "review order")
    ...
    require(len(prior_units | set(UNITS)) == 72, "cumulative units")
    require(len(prior_edges | current_edges) == 213, "cumulative edges")
    require(meta["cumulative_reviewed_unit_count"] == 72 and meta["cumulative_reviewed_edge_count"] == 213 and meta["remaining_unit_count"] == 146, "cumulative receipt")
    ...
    for unit in UNITS:
        ...
        require(hold is not None and hold["status"] == "product_boundary_shared_atom_hold" and hold["shared_atom_ids"] == [], f"product boundary hold {unit}")
    ...
    print("Wave23 static schema10 verification: PASS")
