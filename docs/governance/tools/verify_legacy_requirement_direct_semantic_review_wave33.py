#!/usr/bin/env python3
"""Wave33 schema10 research-premise verifier.
...
LEDGER = ROOT / "docs/governance/legacy-requirement-direct-semantic-review-wave33.jsonl"
META = ROOT / "docs/governance/legacy-requirement-direct-semantic-review-wave33.meta.json"
...
PARENT = "ed3d949cb6588454d7f98a768a3411a5186c93cd"
...
UNITS = ['IRUNIT-HIL-FR-38-HELIX-HARNESS', 'IRUNIT-HIL-FR-38-HELIX-OS', 'IRUNIT-HIL-FR-39-HELIX-HARNESS', 'IRUNIT-HIL-FR-39-HELIX-OS', 'IRUNIT-HIL-FR-40-HELIX-HARNESS', 'IRUNIT-HIL-FR-41-HELIX-HARNESS', 'IRUNIT-HIL-FR-42-HELIX-HARNESS', 'IRUNIT-HIL-FR-42-HELIX-OS']

SELECTED = {'IRUNIT-HIL-FR-38-HELIX-HARNESS': {'LEGACY-ASSET-E457437BBE673A6C9B65', 'LEGACY-ASSET-F776727C5B3C373A545E', 'LEGACY-ASSET-A60CF91DD2AF6693E6F9'}, 'IRUNIT-HIL-FR-38-HELIX-OS': {'LEGACY-ASSET-B54FA1B297C091A8C67E', 'LEGACY-ASSET-EC07511FF3E241F15359', 'LEGACY-ASSET-A60CF91DD2AF6693E6F9'}, 'IRUNIT-HIL-FR-39-HELIX-HARNESS': {'LEGACY-ASSET-3C4A61F623B392DC88D6', 'LEGACY-ASSET-B8BBC1D5A8C91E746405', 'LEGACY-ASSET-A60CF91DD2AF6693E6F9'}, 'IRUNIT-HIL-FR-39-HELIX-OS': {'LEGACY-ASSET-4357DACF4BA46C0B044F', 'LEGACY-ASSET-978C267AADC50615A1E2', 'LEGACY-ASSET-A60CF91DD2AF6693E6F9'}, 'IRUNIT-HIL-FR-40-HELIX-HARNESS': {'LEGACY-ASSET-EBEF9C2559936172AD8F', 'LEGACY-ASSET-524FAF5D640C472BB3E4', 'LEGACY-ASSET-A60CF91DD2AF6693E6F9'}, 'IRUNIT-HIL-FR-41-HELIX-HARNESS': {'LEGACY-ASSET-FE1DFD8D5909E3B08524', 'LEGACY-ASSET-EF18357A994D043719D5', 'LEGACY-ASSET-A60CF91DD2AF6693E6F9'}, 'IRUNIT-HIL-FR-42-HELIX-HARNESS': {'LEGACY-ASSET-AE694B38547318AFF25E', 'LEGACY-ASSET-FEB591CA3369A4AF7729', 'LEGACY-ASSET-A60CF91DD2AF6693E6F9'}, 'IRUNIT-HIL-FR-42-HELIX-OS': {'LEGACY-ASSET-9BD3210DB33460273F84', 'LEGACY-ASSET-78704C8347EBDD58FC52', 'LEGACY-ASSET-A60CF91DD2AF6693E6F9'}}

REQ_DIGESTS = {'HIL-FR-38': 'sha256:1e9fcc442e00d47b8e3a876bb7eb139356f9447fbce661f77c6cd5c9cd6a870b', 'HIL-FR-39': 'sha256:91cbf666593f3585f2d859baf3578b9075d7a92e9b3d936d2c9c4cd47b4f60fc', 'HIL-FR-40': 'sha256:3465949f7d44977d31e9087cf0637261068640d74c72dd1ce5b271fc84378948', 'HIL-FR-41': 'sha256:aaddd9f6348808fe30657b87bdd014b99df774b3786d9502a7229e8b490008f7', 'HIL-FR-42': 'sha256:d6ac9d8a9b359a2a9860550bfcfdddac56f9414d9fc7483ac0566b76b4f36eda'}
...
def verify_prior_lineage(meta: dict) -> None:
    require(len(meta["prior_review_batches"]) == 32, "prior batch count")
...
def verify() -> None:
    ...
    require(len(rows) == 24 and meta["record_count"] == 24, "record count")
    ...
    require([r["review_id"] for r in rows] == [f"LSRW33-EDGE-{i:03d}" for i in range(1, 25)], "review order")
    require(meta["output_sha256"] == file_digest(LEDGER), "ledger digest")
    require(meta["semantic_link_counts"] == {"confirmed": 8, "rejected": 0, "unresolved": 16}, "semantic counts")
    ...
    require(len(prior_units | set(UNITS)) == 119, "cumulative units")
    require(len(prior_edges | current_edges) == 354, "cumulative edges")
    require(meta["cumulative_reviewed_unit_count"] == 119 and meta["cumulative_reviewed_edge_count"] == 354 and meta["remaining_unit_count"] == 99, "cumulative receipt")
    ...
    outside = "LEGACY-ASSET-WAVE33-OUTSIDE-CANDIDATE"
    ...
    print("Wave33 static schema10 verification: PASS")
