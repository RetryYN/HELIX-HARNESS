#!/usr/bin/env python3
"""Wave26 schema10 research-premise verifier.
...
LEDGER = ROOT / "docs/governance/legacy-requirement-direct-semantic-review-wave26.jsonl"
META = ROOT / "docs/governance/legacy-requirement-direct-semantic-review-wave26.meta.json"
...
PARENT = "d7f515ec15a19a55f98edabe22d1018a912c7760"
MAIN_MERGE_REVISION = "d7f515ec15a19a55f98edabe22d1018a912c7760"
MAIN_MERGE_PARENTS = ['86989d74923a7a30c5cf503075a336bc836a18f9', '17a321721aa5f613f64883084ef13e85648aac58']
REQ_SHA = "80e965736a91f99b2ebb77fba2e63a4bf86d5ab5df6fde1d9685f57b42457688"
UNITS = ['IRUNIT-HIL-FR-07-HELIX-HARNESS', 'IRUNIT-HIL-FR-07-HELIX-OS', 'IRUNIT-HIL-FR-08-HELIX-OS']
SELECTED = {
    UNITS[0]: set(['LEGACY-ASSET-A60CF91DD2AF6693E6F9', 'LEGACY-ASSET-9E1F87AFB52388BA1A48', 'LEGACY-ASSET-54515309EF4528FEED70']),
    UNITS[1]: set(['LEGACY-ASSET-A60CF91DD2AF6693E6F9', 'LEGACY-ASSET-0604C89E6A1ECEEA98A0', 'LEGACY-ASSET-54515309EF4528FEED70']),
    UNITS[2]: set(['LEGACY-ASSET-A60CF91DD2AF6693E6F9', 'LEGACY-ASSET-3E1C053D694BC48E3017', 'LEGACY-ASSET-FC45A779AE4D3F9FE8DE']),
}
REQ_DIGESTS = {"HIL-FR-07": "sha256:a79c1994beaedc462b691f9b398bdd0ef9d64fb3b49678b5617c96bfa85dbbb0", "HIL-FR-08": "sha256:e0118fde6e7618d078d4eeb752a429a1618da24859d31be1b4b930ae168ccf05"}
ROW_FIELDS = {
...
def verify_prior_lineage(meta: dict) -> None:
    require(len(meta["prior_review_batches"]) == 25, "prior batch count")
...
def verify() -> None:
    ...
    require([r["review_id"] for r in rows] == [f"LSRW26-EDGE-{i:03d}" for i in range(1, 10)], "review order")
    ...
    require(len(prior_units | set(UNITS)) == 79, "cumulative units")
    require(len(prior_edges | current_edges) == 234, "cumulative edges")
    require(meta["cumulative_reviewed_unit_count"] == 79 and meta["cumulative_reviewed_edge_count"] == 234 and meta["remaining_unit_count"] == 139, "cumulative receipt")
    ...
    for unit in UNITS:
        ...
        expected_shared = {"IRUNIT-HIL-FR-07-HELIX-HARNESS": ["FR07-HARNESS-A01"], "IRUNIT-HIL-FR-07-HELIX-OS": ["FR07-OS-A01"], "IRUNIT-HIL-FR-08-HELIX-OS": []}[unit]
    ...
    print("Wave26 static schema10 verification: PASS")
