#!/usr/bin/env python3
"""Wave27 schema10 research-premise verifier.
...
LEDGER = ROOT / "docs/governance/legacy-requirement-direct-semantic-review-wave27.jsonl"
META = ROOT / "docs/governance/legacy-requirement-direct-semantic-review-wave27.meta.json"
...
PARENT = "3df81ad27157c471e004083783f37a5860eaa2ee"
MAIN_MERGE_REVISION = "3df81ad27157c471e004083783f37a5860eaa2ee"
MAIN_MERGE_PARENTS = ['d7f515ec15a19a55f98edabe22d1018a912c7760', '49ed8acf548fa2ee599a9787c2e5cee85763001f']
REQ_SHA = "80e965736a91f99b2ebb77fba2e63a4bf86d5ab5df6fde1d9685f57b42457688"
UNITS = ['IRUNIT-HIL-FR-09-HELIX-HARNESS', 'IRUNIT-HIL-FR-09-HELIX-OS', 'IRUNIT-HIL-FR-10-HELIX-OS', 'IRUNIT-HIL-FR-11-HELIX-HARNESS']
SELECTED = {
    UNITS[0]: set(['LEGACY-ASSET-1B131E1B4D35900C81F0', 'LEGACY-ASSET-A60CF91DD2AF6693E6F9', 'LEGACY-ASSET-ACDA251E358A7F419507']),
    UNITS[1]: set(['LEGACY-ASSET-1B131E1B4D35900C81F0', 'LEGACY-ASSET-A60CF91DD2AF6693E6F9', 'LEGACY-ASSET-ACDA251E358A7F419507']),
    UNITS[2]: set(['LEGACY-ASSET-A60CF91DD2AF6693E6F9', 'LEGACY-ASSET-AB9A16E8A8DB2F094CC5', 'LEGACY-ASSET-E2208C8FDEDCABF63A79']),
    UNITS[3]: set(['LEGACY-ASSET-617D1B11905560615D04', 'LEGACY-ASSET-99C939E249CAF40935CB', 'LEGACY-ASSET-A60CF91DD2AF6693E6F9']),
}
REQ_DIGESTS = {"HIL-FR-09": "sha256:5594aeaf9047a9dd14646c862da184acf810231d961af8cdadeebf2e54d407c3", "HIL-FR-10": "sha256:1fbb80b64b2182063ff3a301d80e56bace634c8b9e362d61d860c6cbc54bcd65", "HIL-FR-11": "sha256:2e149ad2069ec8f3a8e5c8138b31bf34bbd5ad2c6830dd4c35c4d19536b9ead6"}
ROW_FIELDS = {
...
def verify_prior_lineage(meta: dict) -> None:
    require(len(meta["prior_review_batches"]) == 26, "prior batch count")
...
def verify() -> None:
    ...
    require([r["review_id"] for r in rows] == [f"LSRW27-EDGE-{i:03d}" for i in range(1, 13)], "review order")
    ...
    require(len(prior_units | set(UNITS)) == 83, "cumulative units")
    require(len(prior_edges | current_edges) == 246, "cumulative edges")
    require(meta["cumulative_reviewed_unit_count"] == 83 and meta["cumulative_reviewed_edge_count"] == 246 and meta["remaining_unit_count"] == 135, "cumulative receipt")
    ...
    for unit in UNITS:
        ...
        expected_shared = {"IRUNIT-HIL-FR-09-HELIX-HARNESS": ["FR09-HARNESS-A06"], "IRUNIT-HIL-FR-09-HELIX-OS": ["FR09-OS-A02"], "IRUNIT-HIL-FR-10-HELIX-OS": [], "IRUNIT-HIL-FR-11-HELIX-HARNESS": []}[unit]
    ...
    print("Wave27 static schema10 verification: PASS")
