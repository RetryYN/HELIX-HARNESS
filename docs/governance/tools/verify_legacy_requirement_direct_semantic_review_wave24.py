#!/usr/bin/env python3
"""Wave24 schema10 research-premise verifier.
...
LEDGER = ROOT / "docs/governance/legacy-requirement-direct-semantic-review-wave24.jsonl"
META = ROOT / "docs/governance/legacy-requirement-direct-semantic-review-wave24.meta.json"
...
PARENT = "4974e66e3890e41515004d18c49d59b3e49f8f68"
MAIN_MERGE_REVISION = "4974e66e3890e41515004d18c49d59b3e49f8f68"
MAIN_MERGE_PARENTS = ["e85a549f96778fc79021710805f10723978ec668", "fa8f5426882ee56a56e28975746e2db590cd515f"]
REQ_SHA = "80e965736a91f99b2ebb77fba2e63a4bf86d5ab5df6fde1d9685f57b42457688"
UNITS = ["IRUNIT-HIL-FR-03-HELIX-HARNESS", "IRUNIT-HIL-FR-04-HELIX-HARNESS"]
SELECTED = {
    UNITS[0]: {"LEGACY-ASSET-4A6FA124DB6FEC0DBA7F", "LEGACY-ASSET-A60CF91DD2AF6693E6F9", "LEGACY-ASSET-D8787D8B14310B1A1E77"},
    UNITS[1]: {"LEGACY-ASSET-295B373ABCF388FDB10A", "LEGACY-ASSET-A60CF91DD2AF6693E6F9", "LEGACY-ASSET-7642909D8AA62C75E88D"},
}
REQ_DIGESTS = {
    "HIL-FR-03": "sha256:be8c55e18f6bd462e31eb805edfa5dd4e378c69b86753b7a2cbfb143e6ab068c",
    "HIL-FR-04": "sha256:4d8c9fcc0db7c1e522c6e8b42f49521348381d44a6b9a53b01fb47c9f7e5d97b",
}
ROW_FIELDS = {
...
def verify_prior_lineage(meta: dict) -> None:
    require(len(meta["prior_review_batches"]) == 23, "prior batch count")
...
def verify() -> None:
    ...
    require(len(rows) == 6 and meta["record_count"] == 6, "record count")
    ...
    require([r["review_id"] for r in rows] == [f"LSRW24-EDGE-{i:03d}" for i in range(1, 7)], "review order")
    require(meta["output_sha256"] == file_digest(LEDGER), "ledger digest")
    require(meta["semantic_link_counts"] == {"confirmed": 2, "rejected": 0, "unresolved": 4}, "semantic counts")
    ...
    require(len(prior_units | set(UNITS)) == 74, "cumulative units")
    require(len(prior_edges | current_edges) == 219, "cumulative edges")
    require(meta["cumulative_reviewed_unit_count"] == 74 and meta["cumulative_reviewed_edge_count"] == 219 and meta["remaining_unit_count"] == 144, "cumulative receipt")
    ...
    for unit in UNITS:
        ...
        require(hold is not None and hold["status"] == "product_boundary_shared_atom_hold" and hold["shared_atom_ids"] == (["FR04-HARNESS-A02"] if unit == "IRUNIT-HIL-FR-04-HELIX-HARNESS" else []), f"product boundary hold {unit}")
    ...
    print("Wave24 static schema10 verification: PASS")
