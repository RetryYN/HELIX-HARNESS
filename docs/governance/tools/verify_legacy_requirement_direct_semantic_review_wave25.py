#!/usr/bin/env python3
"""Wave25 schema10 research-premise verifier.
...
LEDGER = ROOT / "docs/governance/legacy-requirement-direct-semantic-review-wave25.jsonl"
META = ROOT / "docs/governance/legacy-requirement-direct-semantic-review-wave25.meta.json"
...
PARENT = "2fa9aca42ff3ffdd5dea9b2186c49ee50db7dc2c"
MAIN_MERGE_REVISION = "2fa9aca42ff3ffdd5dea9b2186c49ee50db7dc2c"
MAIN_MERGE_PARENTS = ['4974e66e3890e41515004d18c49d59b3e49f8f68', '5bb9ce4ca2a5fdd220be3a2874af201763ca4016']
REQ_SHA = "80e965736a91f99b2ebb77fba2e63a4bf86d5ab5df6fde1d9685f57b42457688"
UNITS = ['IRUNIT-HIL-FR-05-HELIX-HARNESS', 'IRUNIT-HIL-FR-05-HELIX-OS']
SELECTED = {
    UNITS[0]: set(['LEGACY-ASSET-A60CF91DD2AF6693E6F9', 'LEGACY-ASSET-94D54E3E1A229FB0684D', 'LEGACY-ASSET-659BC8C9409A09406296']),
    UNITS[1]: set(['LEGACY-ASSET-A60CF91DD2AF6693E6F9', 'LEGACY-ASSET-94D54E3E1A229FB0684D', 'LEGACY-ASSET-659BC8C9409A09406296']),
}
REQ_DIGESTS = {"HIL-FR-05": "sha256:e547dbc9b001e344da3953e672675621d353d69137f1e69c2b548c8c232a163d"}
ROW_FIELDS = {
...
def verify_prior_lineage(meta: dict) -> None:
    require(len(meta["prior_review_batches"]) == 24, "prior batch count")
...
def verify() -> None:
    ...
    require([r["review_id"] for r in rows] == [f"LSRW25-EDGE-{i:03d}" for i in range(1, 7)], "review order")
    ...
    require(len(prior_units | set(UNITS)) == 76, "cumulative units")
    require(len(prior_edges | current_edges) == 225, "cumulative edges")
    require(meta["cumulative_reviewed_unit_count"] == 76 and meta["cumulative_reviewed_edge_count"] == 225 and meta["remaining_unit_count"] == 142, "cumulative receipt")
    ...
    for unit in UNITS:
        ...
        expected_shared = {"IRUNIT-HIL-FR-05-HELIX-HARNESS": ["FR05-HARNESS-A04", "FR05-HARNESS-A05"], "IRUNIT-HIL-FR-05-HELIX-OS": ["FR05-OS-A02", "FR05-OS-A03"]}[unit]
    ...
    print("Wave25 static schema10 verification: PASS")
