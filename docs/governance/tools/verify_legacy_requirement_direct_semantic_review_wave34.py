#!/usr/bin/env python3
"""Wave34 schema10 research-premise verifier.
...
LEDGER = ROOT / "docs/governance/legacy-requirement-direct-semantic-review-wave34.jsonl"
META = ROOT / "docs/governance/legacy-requirement-direct-semantic-review-wave34.meta.json"
...
PARENT = "aee323a016315b0562f7d44e197df681c663cb2a"
...
UNITS = ['IRUNIT-HIL-FR-37-HELIX-OS', 'IRUNIT-HIL-FR-44-HELIX-HARNESS', 'IRUNIT-HIL-FR-44-HELIX-OS', 'IRUNIT-HIL-FR-45-HELIX-OS', 'IRUNIT-HIL-FR-46-HELIX-HARNESS', 'IRUNIT-HIL-FR-46-HELIX-OS', 'IRUNIT-HIL-FR-47-HELIX-HARNESS', 'IRUNIT-HIL-FR-48-HELIX-HARNESS', 'IRUNIT-HIL-FR-48-HELIX-OS']

SELECTED = {'IRUNIT-HIL-FR-37-HELIX-OS': {'LEGACY-ASSET-A60CF91DD2AF6693E6F9', 'LEGACY-ASSET-0EFB21D7C0DC01F9A35B', 'LEGACY-ASSET-DDDED793C93712190619'}, 'IRUNIT-HIL-FR-44-HELIX-HARNESS': {'LEGACY-ASSET-A60CF91DD2AF6693E6F9', 'LEGACY-ASSET-E7AB06BE3282A7D4CBDA', 'LEGACY-ASSET-2A13F8D0A8B9E9DC370F'}, 'IRUNIT-HIL-FR-44-HELIX-OS': {'LEGACY-ASSET-A60CF91DD2AF6693E6F9', 'LEGACY-ASSET-0C17E0C7EB73F5DD44A7', 'LEGACY-ASSET-F6DE183FCC59B754E83F'}, 'IRUNIT-HIL-FR-45-HELIX-OS': {'LEGACY-ASSET-A60CF91DD2AF6693E6F9', 'LEGACY-ASSET-4922801D985490246821', 'LEGACY-ASSET-868B4523D132273D9055'}, 'IRUNIT-HIL-FR-46-HELIX-HARNESS': {'LEGACY-ASSET-A60CF91DD2AF6693E6F9', 'LEGACY-ASSET-5A0973C716302683BEE4', 'LEGACY-ASSET-29B37C44079F551E95F7'}, 'IRUNIT-HIL-FR-46-HELIX-OS': {'LEGACY-ASSET-A60CF91DD2AF6693E6F9', 'LEGACY-ASSET-221A3B6F6F3969228F5D', 'LEGACY-ASSET-D385E8FAD478091775A6'}, 'IRUNIT-HIL-FR-47-HELIX-HARNESS': {'LEGACY-ASSET-A60CF91DD2AF6693E6F9', 'LEGACY-ASSET-C8EAF2E1E94496B02260', 'LEGACY-ASSET-1C8D592306BF5917880C'}, 'IRUNIT-HIL-FR-48-HELIX-HARNESS': {'LEGACY-ASSET-A60CF91DD2AF6693E6F9', 'LEGACY-ASSET-EC9FF83E69C5B50090EF', 'LEGACY-ASSET-C3E9F5F1618847273CDB'}, 'IRUNIT-HIL-FR-48-HELIX-OS': {'LEGACY-ASSET-A60CF91DD2AF6693E6F9', 'LEGACY-ASSET-08BD701EF2577835D9F7', 'LEGACY-ASSET-EE04E18F84F035A337D5'}}

REQ_DIGESTS = {'HIL-FR-37': 'sha256:34d1fde675a0d97523f479286326cfb162b2b897e5d76921a7dcae514bcb8482', 'HIL-FR-44': 'sha256:49bee96c5bb5d35a05301d3c0c4c7c4f31eb842b5bad1d58028c4d0c72bdf4f6', 'HIL-FR-45': 'sha256:e13451d529dd90b9d37e1b42eb180f20b183ec7b20f5ece2e6b8b02acdd7acda', 'HIL-FR-46': 'sha256:0255c7c9107d749eee633f9b94612ee7da059e544021824a3fa65b299afae7b3', 'HIL-FR-47': 'sha256:b1a61c1eb5f24a741168801fed3b323578151edabb2f62d1d47e239861dc1dca', 'HIL-FR-48': 'sha256:49d1f634eefe71ad7a79be172439935c1ba75c7bb889e05746c5c98fe28e9329'}
...
def verify_prior_lineage(meta: dict) -> None:
    require(len(meta["prior_review_batches"]) == 33, "prior batch count")
...
def verify() -> None:
    ...
    require(len(rows) == 27 and meta["record_count"] == 27, "record count")
    ...
    require([r["review_id"] for r in rows] == [f"LSRW34-EDGE-{i:03d}" for i in range(1, 28)], "review order")
    require(meta["output_sha256"] == file_digest(LEDGER), "ledger digest")
    require(meta["semantic_link_counts"] == {"confirmed": 9, "rejected": 0, "unresolved": 18}, "semantic counts")
    ...
    require(len(prior_units | set(UNITS)) == 128, "cumulative units")
    require(len(prior_edges | current_edges) == 381, "cumulative edges")
    require(meta["cumulative_reviewed_unit_count"] == 128 and meta["cumulative_reviewed_edge_count"] == 381 and meta["remaining_unit_count"] == 90, "cumulative receipt")
    ...
    for unit in UNITS:
        ...
        expected_shared = {'IRUNIT-HIL-FR-48-HELIX-HARNESS': ['FR-48-HARNESS-A02'], 'IRUNIT-HIL-FR-48-HELIX-OS': ['FR-48-OS-A01']}.get(unit, [])
    ...
    print("Wave34 static schema10 verification: PASS")
