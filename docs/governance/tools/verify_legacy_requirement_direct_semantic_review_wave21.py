#!/usr/bin/env python3
"""Wave21 schema10 research-premise verifier.
...
LEDGER = ROOT / "docs/governance/legacy-requirement-direct-semantic-review-wave21.jsonl"
META = ROOT / "docs/governance/legacy-requirement-direct-semantic-review-wave21.meta.json"
...
PARENT = "053943791ceda83366fca01d375308ba5f7deb28"
MAIN_MERGE_PARENTS = ["2ec95d10d7233cbf64782f6120ffcb7ad150242a", "1041f07d29a8cf8b55a55ee2ee11c378f683dd88"]
...
CURRENT_SOURCE_BASE = "053943791ceda83366fca01d375308ba5f7deb28"
...
BR30_CONNECTOR_HOLD = "source_atomization_review_pending;product_boundary_pending_human_decision;connector_gap_unresolved"
BR30_CONNECTOR = {
    "connector_id": "BR30-HARNESS-A01-to-BR30-OS-A01",
    "from_atom_id": "BR30-HARNESS-A01",
    "to_atom_id": "BR30-OS-A01",
    "relation": "source_clause_continuation_across_product_split",
    "status": "unresolved",
    "reason": "upstream decomposition assigns runtime projection to OS and contract generation to HARNESS but does not define an explicit A01 product connector; owner, boundary, and lossless split remain pending human review",
    "source_proof": {
        "source_requirement_id": "HIL-BR-30",
        "source_requirement_pointer": "archive/legacy-generation-2026-09-14/root/requirements-ir/requirements.json#/HIL-BR-30",
        "source_statement_semantic_digest": "sha256:7805010c12510bd817953e2d115e50791e617dcbd627ed9d599762c277c72711",
        "harness_source_span_indexes": [0, 1],
        "os_source_span_indexes": [0],
        "relation": "adjacent_raw_source_spans_not_shared_atom",
    },
}
UNITS = [
    "IRUNIT-HIL-BR-28-HELIX-HARNESS",
    "IRUNIT-HIL-BR-29-HELIX-HARNESS",
    "IRUNIT-HIL-BR-29-HELIX-OS",
    "IRUNIT-HIL-BR-30-HELIX-HARNESS",
    "IRUNIT-HIL-BR-30-HELIX-OS",
]
SELECTED = {
    "IRUNIT-HIL-BR-28-HELIX-HARNESS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9", "LEGACY-ASSET-D8A9E08B9BDD62096AEF", "LEGACY-ASSET-462F60E486F8DB80687E"},
    "IRUNIT-HIL-BR-29-HELIX-HARNESS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9", "LEGACY-ASSET-D107FD145A2588FAAD09", "LEGACY-ASSET-AB6F8E1928385699AD6A"},
    "IRUNIT-HIL-BR-29-HELIX-OS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9", "LEGACY-ASSET-C544D3D166699301485D", "LEGACY-ASSET-9CC8ADC4CDBC20F12A64"},
    "IRUNIT-HIL-BR-30-HELIX-HARNESS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9", "LEGACY-ASSET-D27D4A1511BFD43623A9", "LEGACY-ASSET-78D55762187C612C93C4"},
    "IRUNIT-HIL-BR-30-HELIX-OS": {"LEGACY-ASSET-A60CF91DD2AF6693E6F9", "LEGACY-ASSET-6DA5F7026F38B4C091AF", "LEGACY-ASSET-4895AB4D1A11B9202C34"},
}
REQ_DIGESTS = {
    "HIL-BR-28": "sha256:d4e7f2d1a599e2d6e59fc913bfbe0d8006d6f60b251f02dd3b5c65c97025c653",
    "HIL-BR-29": "sha256:05eba9d85097424f46afa0356b747e930077ff8b31b8264784eb54af709dc148",
    "HIL-BR-30": "sha256:7805010c12510bd817953e2d115e50791e617dcbd627ed9d599762c277c72711",
}
ROW_FIELDS = {
...
def verify_br30_connector_gap(rows_by_unit: dict[str, list[dict]], holds: list[dict]) -> None:
    """Keep the raw clause overlap as an unresolved connector, not OS completion."""
    for unit in ("IRUNIT-HIL-BR-30-HELIX-HARNESS", "IRUNIT-HIL-BR-30-HELIX-OS"):
        for row in rows_by_unit[unit]:
            require(row["atomization_hold"] == BR30_CONNECTOR_HOLD, f"BR30 connector hold {row['review_id']}")
            require(row["connection_records"] == [BR30_CONNECTOR], f"BR30 connector record {row['review_id']}")
    for unit in ("IRUNIT-HIL-BR-30-HELIX-HARNESS", "IRUNIT-HIL-BR-30-HELIX-OS"):
        hold = next(item for item in holds if item.get("unit_candidate_id") == unit)
        require(hold["row_atomization_hold"] == BR30_CONNECTOR_HOLD, f"BR30 meta connector hold {unit}")
        require(hold.get("connector_gap_records") == [BR30_CONNECTOR], f"BR30 meta connector record {unit}")
    h_atoms = {atom["atom_id"]: atom for atom in rows_by_unit["IRUNIT-HIL-BR-30-HELIX-HARNESS"][0]["covered_requirement_atoms"]}
    o_atoms = {atom["atom_id"]: atom for atom in rows_by_unit["IRUNIT-HIL-BR-30-HELIX-OS"][0]["covered_requirement_atoms"]}
    require(h_atoms["BR30-HARNESS-A01"]["shared_with_units"] == [] and o_atoms["BR30-OS-A01"]["shared_with_units"] == [], "BR30 A01 unresolved ownership")
    require(h_atoms["BR30-HARNESS-A02"]["shared_with_units"] == ["IRUNIT-HIL-BR-30-HELIX-OS"] and o_atoms["BR30-OS-A02"]["shared_with_units"] == ["IRUNIT-HIL-BR-30-HELIX-HARNESS"], "BR30 A02 shared closure")
...
def verify() -> None:
    ...
    require([row["review_id"] for row in rows] == [f"LSRW21-EDGE-{i:03d}" for i in range(1, 16)], "review order")
    ...
    require(len(prior_units | set(UNITS)) == 67, "unit cumulative count")
    require(len(prior_edges | current_edges) == 198, "edge cumulative count")
    require(meta["cumulative_reviewed_unit_count"] == 67 and meta["cumulative_reviewed_edge_count"] == 198 and meta["remaining_unit_count"] == 151, "cumulative receipt")
    ...
    verify_br30_connector_gap(by_unit, meta["source_atomization_holds"])
    ...
    os_requirement = by_unit["IRUNIT-HIL-BR-30-HELIX-OS"][0]
    tampered_os_requirement = deepcopy(os_requirement)
    tampered_os_requirement["covered_requirement_atoms"][0]["text"] = "任意のsubagent定義へ射影してよい"
    ...
    print("Wave21 static schema10 verification: PASS")
