#!/usr/bin/env python3
"""wave 9 direct semantic reviewをarchive実行なしで静的検証する。"""
...
LEDGER=GOV/'legacy-requirement-direct-semantic-review-wave9.jsonl'; META=GOV/'legacy-requirement-direct-semantic-review-wave9.meta.json'
STATUS=GOV/'audits/source-rebaseline/legacy-requirement-direct-semantic-review-wave9-status-2026-09-21.md'
...
PHASE=GOV/'phase-capability-inventory.json'; PRIOR=[(GOV/f'legacy-requirement-direct-semantic-review-wave{n}.jsonl',GOV/f'legacy-requirement-direct-semantic-review-wave{n}.meta.json') for n in (1,2,3,4,5,6,7,8)]

ATOMS={'IRUNIT-HIL-FR-60-HELIX-OS': [{'atom_id': 'FR60-OS-A01', 'kind': 'measurable_muster_benefit', 'text': '専門知識、独立context、並列性、blind verificationのいずれかに測定可能な利益がある場合だけ生成agentをmusterする', 'source_fragments': ['Specialist Muster Gateは専門知識、独立context、並列性、blind verificationのいずれかに測定可能な利益がある場合だけ生成agentをmusterし'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR60-OS-A02', 'kind': 'existing_role_reuse', 'text': '単一agentで十分なtaskは既存roleへ送る', 'source_fragments': ['単一agentで十分なtaskは既存roleへ送る'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR60-OS-A03', 'kind': 'runtime_projection', 'text': '生成agentをallowlist済みruntime型へ射影する', 'source_fragments': ['生成agentをallowlist済みruntime型へ射影し'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR60-OS-A04', 'kind': 'worker_verifier_separation', 'text': 'workerとverifierのprovider、model、authorityを分離し、lease、fencing、retireを検査する', 'source_fragments': ['workerとverifierのprovider/model/authority分離、lease、fencing、retireを検査する'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR60-OS-A05', 'kind': 'muster_evidence', 'text': 'specialization decision、TeamDefinition、runtime projection、worker/verifier separation、lifecycle receiptを証拠にする', 'source_fragments': ['specialization decision、TeamDefinition、runtime projection、worker/verifier separation、lifecycle receipt'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}], 'IRUNIT-HIL-FR-61-HELIX-OS': [{'atom_id': 'FR61-OS-A01', 'kind': 'separate_smoke_blind_bench', 'text': '候補runtimeごとに短い機械判定smokeとblind judgeを含むfull benchを分離実行する', 'source_fragments': ['Worker Acceptance Benchは候補runtimeごとに短い機械判定smokeとblind judgeを含むfull benchを分離実行し'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR61-OS-A02', 'kind': 'fixed_bench_rubric', 'text': 'correctness、mutation kill、instruction/scope following、skill A/B、品質、簡潔性、security、第二diffによる拡張性を同一fixtureとversioned rubricで採点する', 'source_fragments': ['correctness、mutation kill、instruction/scope following、skill A/B、品質、簡潔性、security、第二diffによる拡張性を同一fixtureとversioned rubricで採点する'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR61-OS-A03', 'kind': 'bench_admission_evidence', 'text': 'bench manifest、machine score、blind-judge score、fixture/rubric digest、admission decisionを証拠にする', 'source_fragments': ['bench manifest、machine score、blind-judge score、fixture/rubric digest、admission decision'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}], 'IRUNIT-HIL-FR-62-HELIX-OS': [{'atom_id': 'FR62-OS-A01', 'kind': 'real_task_scorecard_scope', 'text': 'Task Performance ScorecardをHELIX実taskごとに記録する', 'source_fragments': ['Task Performance ScorecardはHELIX実taskごとに'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR62-OS-A02', 'kind': 'real_task_metrics', 'text': 'first_pass、retry_count、proposal_diff_size、lint_violation_count、品質judge、実効costを記録する', 'source_fragments': ['`first_pass`、`retry_count`、`proposal_diff_size`、`lint_violation_count`、品質judge、実効costを記録する'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR62-OS-A03', 'kind': 'effective_cost', 'text': '実効costはAPI相当単価とretryを含める', 'source_fragments': ['実効costはAPI相当単価とretryを含め'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR62-OS-A04', 'kind': 'use_case_decisions', 'text': '用途別の採用、限定、quarantine、retire判断へ接続する', 'source_fragments': ['用途別の採用、限定、quarantine、retire判断へ接続する'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR62-OS-A05', 'kind': 'scorecard_evidence', 'text': 'real-task scorecard、effective-cost breakdown、用途別decision、trend/failure findingを証拠にする', 'source_fragments': ['real-task scorecard、effective-cost breakdown、用途別decision、trend/failure finding'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}]}
QUERIES={'IRUNIT-HIL-FR-60-HELIX-OS': ['HIL-FR-60', 'Specialist Muster Gate', 'specialization decision', 'worker', 'verifier', 'allowlist', 'lease', 'lifecycle receipt'], 'IRUNIT-HIL-FR-61-HELIX-OS': ['HIL-FR-61', 'Worker Acceptance Bench', 'blind judge', 'versioned rubric', 'fixture/rubric digest', 'admission decision', 'WorkerBlindBenchmark', 'freezeWorkerBlindBenchmark', 'evaluateWorkerBlindBenchmark'], 'IRUNIT-HIL-FR-62-HELIX-OS': ['HIL-FR-62', 'Task Performance Scorecard', 'cost policy', 'full selection', 'provenance tuple', 'aggregate_effective_cost', 'min_blind_score', 'WorkerRiskAdmission']}
DECISIONS={('IRUNIT-HIL-FR-60-HELIX-OS', 'LEGACY-ASSET-A60CF91DD2AF6693E6F9'): ('confirmed', 'same_requirement_id_exact_source_contract_not_implementation', ('FR60-OS-A01', 'FR60-OS-A02', 'FR60-OS-A03', 'FR60-OS-A04', 'FR60-OS-A05')), ('IRUNIT-HIL-FR-60-HELIX-OS', 'LEGACY-ASSET-A642730A06BD1F95BF41'): ('unresolved', 'design_contract_evidence', ('FR60-OS-A03', 'FR60-OS-A04')), ('IRUNIT-HIL-FR-60-HELIX-OS', 'LEGACY-ASSET-C7235C17F972EC7A663A'): ('unresolved', 'partial_implementation_behavior_evidence_unexecuted', ('FR60-OS-A03', 'FR60-OS-A04')), ('IRUNIT-HIL-FR-61-HELIX-OS', 'LEGACY-ASSET-A60CF91DD2AF6693E6F9'): ('confirmed', 'same_requirement_id_exact_source_contract_not_implementation', ('FR61-OS-A01', 'FR61-OS-A02', 'FR61-OS-A03')), ('IRUNIT-HIL-FR-61-HELIX-OS', 'LEGACY-ASSET-E6F2B47DE3B426527636'): ('unresolved', 'design_contract_evidence', ('FR61-OS-A02', 'FR61-OS-A03')), ('IRUNIT-HIL-FR-61-HELIX-OS', 'LEGACY-ASSET-D390493C08A4F62A178C'): ('unresolved', 'partial_implementation_behavior_evidence_unexecuted', ('FR61-OS-A03',)), ('IRUNIT-HIL-FR-62-HELIX-OS', 'LEGACY-ASSET-A60CF91DD2AF6693E6F9'): ('confirmed', 'same_requirement_id_exact_source_contract_not_implementation', ('FR62-OS-A01', 'FR62-OS-A02', 'FR62-OS-A03', 'FR62-OS-A04', 'FR62-OS-A05')), ('IRUNIT-HIL-FR-62-HELIX-OS', 'LEGACY-ASSET-FF974AF822E918BBF446'): ('unresolved', 'design_contract_evidence', ('FR62-OS-A03',)), ('IRUNIT-HIL-FR-62-HELIX-OS', 'LEGACY-ASSET-BF2D3287A89CD8C5877D'): ('unresolved', 'partial_implementation_behavior_evidence_unexecuted', ('FR62-OS-A03', 'FR62-OS-A04'))}
...
def main():
 ...
 require(meta['parent_revision']=='0e6c6f8346a1381972b4a8d0b1edf33b0b328389','parent revision不一致')
 ...
 require(meta['cumulative_reviewed_edge_count']==len(prior_edges|edges)==78 and meta['cumulative_reviewed_unit_count']==len(prior_units|set(ATOMS))==26,'累積件数不一致')
 text=STATUS.read_text(); batch=re.search(r'218要求unitのうち新たに(\d+) unit、候補edge (\d+)件',text); cumulative=re.search(r'wave 1〜8と合わせて(\d+) unit、(\d+) edge',text); remaining=re.search(r'残る(\d+) unitは未着手',text)
 ...
 print(f'legacy requirement direct semantic review wave9: schema10 / 9 edges / {sum(len(atoms) for atoms in ATOMS.values())} atoms / bounded search + phase candidates verified')
