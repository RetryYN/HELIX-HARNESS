#!/usr/bin/env python3
"""wave 6 direct semantic reviewをarchive実行なしで静的検証する。"""
...
LEDGER=GOV/'legacy-requirement-direct-semantic-review-wave6.jsonl'; META=GOV/'legacy-requirement-direct-semantic-review-wave6.meta.json'
STATUS=GOV/'audits/source-rebaseline/legacy-requirement-direct-semantic-review-wave6-status-2026-09-21.md'
...
PHASE=GOV/'phase-capability-inventory.json'; PRIOR=[(GOV/f'legacy-requirement-direct-semantic-review-wave{n}.jsonl',GOV/f'legacy-requirement-direct-semantic-review-wave{n}.meta.json') for n in (1,2,3,4,5)]

ATOMS={'IRUNIT-HIL-FR-28-HELIX-OS': [{'atom_id': 'FR28-OS-A01', 'kind': 'stage_evidence_recording', 'text': '各段の必須check、対象SHA/tree digest、結果、artifactを記録する', 'source_fragments': ['Three-stage CI Orchestratorは各段の必須check、対象SHA/tree digest、結果、artifactを記録し'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR28-OS-A02', 'kind': 'monotonic_three_stage_transition', 'text': 'local_prejoinからinternal_postjoinを経てgithub_externalへ単調遷移する', 'source_fragments': ['`local_prejoin→internal_postjoin→github_external`の単調遷移を強制する'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR28-OS-A03', 'kind': 'stage_admission_receipt', 'text': 'stage receipt、SHA binding、next-stage可否を証拠にする', 'source_fragments': ['stage receipt、SHA binding、next-stage可否'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}], 'IRUNIT-HIL-FR-32-HELIX-OS': [{'atom_id': 'FR32-OS-A01', 'kind': 'active_lifecycle_sequence', 'text': '登録から実行、checkpoint、完了分岐、検証、releaseまでを管理する', 'source_fragments': ['Agent Lifecycle Controllerは`registered→eligible→mustered→leased→running→checkpointed→completed/failed/cancelled→verified→released`を管理し'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR32-OS-A02', 'kind': 'terminal_lifecycle_branches', 'text': 'quarantinedとretiredを終端分岐にする', 'source_fragments': ['`quarantined/retired`を終端分岐として持つ'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR32-OS-A03', 'kind': 'lifecycle_receipt_set', 'text': 'instance event、lease/heartbeat、context/result/verification receiptを証拠にする', 'source_fragments': ['instance event、lease/heartbeat、context/result/verification receipt'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}], 'IRUNIT-HIL-FR-64-HELIX-OS': [{'atom_id': 'FR64-OS-A01', 'kind': 'versioned_sandbox_template', 'text': 'versioned templateで第三者CLI workerの実行環境を定義する', 'source_fragments': ['Worker Sandbox Contractはversioned templateで第三者CLI workerの実行環境を定義し'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR64-OS-A02', 'kind': 'writable_scope', 'text': '書込可能領域を払い出しworktree/scratchに限定する', 'source_fragments': ['書込可能領域を払い出しworktree/scratchへ限定'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR64-OS-A03', 'kind': 'inference_endpoint_allowlist', 'text': 'networkを推論API endpointだけのhost+path allowlistに制限する', 'source_fragments': ['networkを推論API endpointのみのhost+path allowlistへ制限'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR64-OS-A04', 'kind': 'non_bind_authority_paths', 'text': 'credential、.helix、harness DBをbindしない', 'source_fragments': ['credential・`.helix/`・harness DBを非bind'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR64-OS-A05', 'kind': 'third_party_cli_config_update', 'text': '第三者CLI設定変更をbackup付きmerge/appendだけにする', 'source_fragments': ['第三者CLI設定変更をbackup付きmerge/appendのみとし'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR64-OS-A06', 'kind': 'drift_fail_close', 'text': 'template逸脱とegress実測乖離をfail-closeまたはquarantineする', 'source_fragments': ['template逸脱とegress実測乖離をfail-close/quarantineする'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR64-OS-A07', 'kind': 'sandbox_receipt_set', 'text': 'sandbox template digest、egress計測、逸脱finding、quarantine receiptを証拠にする', 'source_fragments': ['sandbox template digest、egress計測、逸脱finding、quarantine receipt'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}]}
...
def main():
 ...
 require(meta['schema_revision']==8 and meta['status']=='research_premise_candidate','schema/status不一致')
 require(meta['parent_revision']=='89d8579f8dd0fcd4a4aaa0e50727bd5dc918663e','parent revision不一致')
 ...
 for r in records:
  ...
  require(r['schema_revision']==8 and r['selection_route']=='bounded_global_search' and r['candidate_membership_semantics']=='bounded_global_search_candidate_only_not_semantic_evidence',f'schema/search authority不一致: {rid}')
 ...
 require(meta['cumulative_reviewed_edge_count']==len(prior_edges|edges)==51 and meta['cumulative_reviewed_unit_count']==len(prior_units|set(ATOMS))==17,'累積件数不一致')
 text=STATUS.read_text(); batch=re.search(r'218要求unitのうち新たに(\d+) unit、候補edge (\d+)件',text); cumulative=re.search(r'wave 1〜5と合わせて(\d+) unit、(\d+) edge',text); remaining=re.search(r'残る(\d+) unitは未着手',text)
 ...
 print('legacy requirement direct semantic review wave6: schema8 / 9 edges / 13 atoms / bounded search + phase candidates verified')
