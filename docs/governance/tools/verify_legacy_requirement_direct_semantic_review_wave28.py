#!/usr/bin/env python3
"""Wave28 schema10 research-premise verifier.
...
LEDGER = ROOT / "docs/governance/legacy-requirement-direct-semantic-review-wave28.jsonl"
META = ROOT / "docs/governance/legacy-requirement-direct-semantic-review-wave28.meta.json"
...
PARENT = "b27e61f079edf64eeddc43eb8095159b19730b94"
MAIN_MERGE_REVISION = "b27e61f079edf64eeddc43eb8095159b19730b94"
MAIN_MERGE_PARENTS = ['3df81ad27157c471e004083783f37a5860eaa2ee', 'bd468075abd1c1a95c655acdc9cd00b8fe1d898a']
REQ_SHA = "80e965736a91f99b2ebb77fba2e63a4bf86d5ab5df6fde1d9685f57b42457688"
UNITS = ['IRUNIT-HIL-FR-13-HELIX-OS', 'IRUNIT-HIL-FR-14-HELIX-OS', 'IRUNIT-HIL-FR-15-HELIX-OS', 'IRUNIT-HIL-FR-16-HELIX-OS', 'IRUNIT-HIL-FR-17-HELIX-HARNESS', 'IRUNIT-HIL-FR-17-HELIX-OS']
SELECTED = {
    UNITS[0]: set(['LEGACY-ASSET-AD7B3AF280F803882487', 'LEGACY-ASSET-A60CF91DD2AF6693E6F9', 'LEGACY-ASSET-1E1272FA8EE236C4FC4F']),
    UNITS[1]: set(['LEGACY-ASSET-67B016392E3F7D58B053', 'LEGACY-ASSET-A60CF91DD2AF6693E6F9', 'LEGACY-ASSET-E1937310AD75D57B1D83']),
    UNITS[2]: set(['LEGACY-ASSET-74D116A3BCB28DDB1C09', 'LEGACY-ASSET-A60CF91DD2AF6693E6F9', 'LEGACY-ASSET-D08867CDFC82AD93E596']),
    UNITS[3]: set(['LEGACY-ASSET-3B0429583DCEA146D4DD', 'LEGACY-ASSET-77EDDD8BBB1F3863D007', 'LEGACY-ASSET-A60CF91DD2AF6693E6F9']),
    UNITS[4]: set(['LEGACY-ASSET-A60CF91DD2AF6693E6F9', 'LEGACY-ASSET-51B78F6A5128E2CD1448', 'LEGACY-ASSET-FC956FCBF42A39429E96']),
    UNITS[5]: set(['LEGACY-ASSET-A60CF91DD2AF6693E6F9', 'LEGACY-ASSET-51B78F6A5128E2CD1448', 'LEGACY-ASSET-FC956FCBF42A39429E96']),
}
REQ_DIGESTS = {
    'HIL-FR-13': 'sha256:8472fdc576a52718f2d43f49e410565600d8fc3c0b374110dfad8886e37ccb50',
    'HIL-FR-14': 'sha256:32c9cca689abf14f1a6dba50ff9144ff11791ba29f625b5eb1e4d41abeffff35',
    'HIL-FR-15': 'sha256:73dfa183c290d8b7f4e9698e5933b93c4643d9347a6ef8256cdb4c9d95a33ed0',
    'HIL-FR-16': 'sha256:3f8059576753f54ac5cbc51a29d19afcc1189f6aee6403196ff3ac7540b0c574',
    'HIL-FR-17': 'sha256:6c714b85354c436aadad677cb1a379eb4f0ba007f847eec60391618fb07573c7',
}
ROW_FIELDS = {
...
def verify_prior_lineage(meta: dict) -> None:
    require(len(meta["prior_review_batches"]) == 27, "prior batch count")
...
def verify() -> None:
    ...
    require(len(rows) == 18 and meta["record_count"] == 18, "record count")
    ...
    require(meta["source_main_base_revision"] == PARENT and meta["reviewed_unit_ids"] == UNITS, "source base/units")
    require([r["review_id"] for r in rows] == [f"LSRW28-EDGE-{i:03d}" for i in range(1, 19)], "review order")
    require(meta["output_sha256"] == file_digest(LEDGER), "ledger digest")
    require(meta["semantic_link_counts"] == {"confirmed": 6, "rejected": 0, "unresolved": 12}, "semantic counts")
    ...
    require(not set(UNITS) & {"IRUNIT-HIL-FR-06-HELIX-HARNESS", "IRUNIT-HIL-FR-06-HELIX-OS"}, "already reviewed FR06 units")
    ...
    require(len(prior_units | set(UNITS)) == 89, "cumulative units")
    require(len(prior_edges | current_edges) == 264, "cumulative edges")
    require(meta["cumulative_reviewed_unit_count"] == 89 and meta["cumulative_reviewed_edge_count"] == 264 and meta["remaining_unit_count"] == 129, "cumulative receipt")
    ...
    for unit in UNITS:
        ...
        expected_shared = {"IRUNIT-HIL-FR-13-HELIX-OS": [], "IRUNIT-HIL-FR-14-HELIX-OS": [], "IRUNIT-HIL-FR-15-HELIX-OS": [], "IRUNIT-HIL-FR-16-HELIX-OS": [], "IRUNIT-HIL-FR-17-HELIX-HARNESS": [], "IRUNIT-HIL-FR-17-HELIX-OS": []}[unit]
    ...
    print("Wave28 static schema10 verification: PASS")
