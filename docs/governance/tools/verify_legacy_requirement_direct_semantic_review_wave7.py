#!/usr/bin/env python3
"""wave 7 direct semantic reviewをarchive実行なしで静的検証する。"""
...
LEDGER=GOV/'legacy-requirement-direct-semantic-review-wave7.jsonl'; META=GOV/'legacy-requirement-direct-semantic-review-wave7.meta.json'
STATUS=GOV/'audits/source-rebaseline/legacy-requirement-direct-semantic-review-wave7-status-2026-09-21.md'
...
PHASE=GOV/'phase-capability-inventory.json'; PRIOR=[(GOV/f'legacy-requirement-direct-semantic-review-wave{n}.jsonl',GOV/f'legacy-requirement-direct-semantic-review-wave{n}.meta.json') for n in (1,2,3,4,5,6)]

ATOMS={'IRUNIT-HIL-BR-20-HELIX-OS': [{'atom_id': 'BR20-OS-A01', 'kind': 'temporary_evidence_preserving_quarantine', 'text': '既知の内部CI failureを証拠を保持した機械的quarantineで一時隔離できる', 'source_fragments': ['既知の内部CI failureは証拠を保持した機械的quarantineで一時隔離できるが'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'BR20-OS-A02', 'kind': 'nonmatching_failure_fail_close', 'text': '対象外failureまたは新規fingerprintを無視しない', 'source_fragments': ['対象外failure、新規fingerprint'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'BR20-OS-A03', 'kind': 'minimum_gate_fail_close', 'text': '最低代替gate失敗を無視しない', 'source_fragments': ['最低代替gate失敗を無視してはならない'], 'shared_with_units': ['IRUNIT-HIL-BR-20-HELIX-HARNESS'], 'boundary_review_state': 'product_boundary_pending_human_decision'}], 'IRUNIT-HIL-FR-29-HELIX-OS': [{'atom_id': 'FR29-OS-A01', 'kind': 'exact_failure_identity', 'text': '既知failureをcheck名、failure fingerprint、baseline SHAへ限定する', 'source_fragments': ['CI Quarantine Managerは既知failureをcheck名、failure fingerprint、baseline SHAへ限定し'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR29-OS-A02', 'kind': 'mandatory_remediation_fields', 'text': '理由、是正Issue、owner、期限またはiteration上限、代替minimum gateを必須にする', 'source_fragments': ['理由、是正Issue、owner、期限/iteration上限、代替minimum gateを必須化する'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR29-OS-A03', 'kind': 'fingerprint_change_reset', 'text': 'fingerprint変化を通常failureへ戻す', 'source_fragments': ['fingerprint変化は通常failureへ戻す'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'FR29-OS-A04', 'kind': 'quarantine_evidence_set', 'text': 'quarantine receipt、expiry/stale判定、remediation Issueを証拠にする', 'source_fragments': ['quarantine receipt、expiry/stale判定、remediation Issue'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}], 'IRUNIT-HIL-NFR-16-HELIX-OS': [{'atom_id': 'NFR16-OS-A01', 'kind': 'exact_fingerprint_scope', 'text': 'quarantineをexact fingerprintだけへ適用する', 'source_fragments': ['quarantineはexact fingerprintだけに適用し'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'NFR16-OS-A02', 'kind': 'broad_scope_prohibition', 'text': '無期限、wildcard、directory単位、全check一括を禁止する', 'source_fragments': ['無期限、wildcard、directory単位、全check一括を禁止する'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}, {'atom_id': 'NFR16-OS-A03', 'kind': 'automatic_invalidation', 'text': '期限切れ、対象変更、fingerprint変化で失効する', 'source_fragments': ['期限切れ、対象変更、fingerprint変化で失効する'], 'shared_with_units': [], 'boundary_review_state': 'product_boundary_pending_human_decision'}]}
QUERIES={'IRUNIT-HIL-BR-20-HELIX-OS': ['HIL-BR-20', '既知の内部CI failure', 'quarantine', 'minimum gate', 'replacement_oracle_id'], 'IRUNIT-HIL-FR-29-HELIX-OS': ['HIL-FR-29', 'CiQuarantineManager', 'failure fingerprint', 'baseline SHA', 'remediation Issue', 'replacement_oracle_id'], 'IRUNIT-HIL-NFR-16-HELIX-OS': ['HIL-NFR-16', 'exact fingerprint', 'wildcard', 'directory丸ごと', 'expires_at', 'quarantine_invalid']}
DECISIONS={('IRUNIT-HIL-BR-20-HELIX-OS', 'LEGACY-ASSET-719D5EC9C06FC4AAD0FF'): ('confirmed', 'same_requirement_id_exact_source_contract_not_implementation', ('BR20-OS-A01', 'BR20-OS-A02', 'BR20-OS-A03')), ('IRUNIT-HIL-BR-20-HELIX-OS', 'LEGACY-ASSET-F23AB691AD5152B1C0E9'): ('unresolved', 'design_contract_evidence', ('BR20-OS-A01', 'BR20-OS-A02', 'BR20-OS-A03')), ('IRUNIT-HIL-BR-20-HELIX-OS', 'LEGACY-ASSET-CF70C0AC609614D8AF2E'): ('rejected', 'adjacent_implementation_nonmatching', ()), ('IRUNIT-HIL-FR-29-HELIX-OS', 'LEGACY-ASSET-719D5EC9C06FC4AAD0FF'): ('confirmed', 'same_requirement_id_exact_source_contract_not_implementation', ('FR29-OS-A01', 'FR29-OS-A02', 'FR29-OS-A03', 'FR29-OS-A04')), ('IRUNIT-HIL-FR-29-HELIX-OS', 'LEGACY-ASSET-F23AB691AD5152B1C0E9'): ('unresolved', 'design_contract_evidence', ('FR29-OS-A01', 'FR29-OS-A02', 'FR29-OS-A03', 'FR29-OS-A04')), ('IRUNIT-HIL-FR-29-HELIX-OS', 'LEGACY-ASSET-CF70C0AC609614D8AF2E'): ('rejected', 'adjacent_implementation_nonmatching', ()), ('IRUNIT-HIL-NFR-16-HELIX-OS', 'LEGACY-ASSET-719D5EC9C06FC4AAD0FF'): ('confirmed', 'same_requirement_id_exact_source_contract_not_implementation', ('NFR16-OS-A01', 'NFR16-OS-A02', 'NFR16-OS-A03')), ('IRUNIT-HIL-NFR-16-HELIX-OS', 'LEGACY-ASSET-F23AB691AD5152B1C0E9'): ('unresolved', 'design_contract_evidence', ('NFR16-OS-A01', 'NFR16-OS-A02', 'NFR16-OS-A03')), ('IRUNIT-HIL-NFR-16-HELIX-OS', 'LEGACY-ASSET-CF70C0AC609614D8AF2E'): ('rejected', 'adjacent_implementation_nonmatching', ())}
...
def main():
 ...
 require(meta['schema_revision']==9 and meta['status']=='research_premise_candidate','schema/status不一致')
 require(meta['parent_revision']=='bc9b73da77e4d3f3219a04cb3e0d42d4359e3160','parent revision不一致')
 ...
 for r in records:
  ...
  require(r['schema_revision']==9 and r['selection_route']=='bounded_global_search' and r['candidate_membership_semantics']=='bounded_global_search_candidate_only_not_semantic_evidence',f'schema/search authority不一致: {rid}')
 ...
 counts={s:sum(r['semantic_link_status']==s for r in records) for s in ('confirmed','rejected','unresolved')}; require(meta['semantic_link_counts']==counts=={'confirmed':3,'rejected':3,'unresolved':3},'batch count不一致')
 ...
 require(meta['cumulative_reviewed_edge_count']==len(prior_edges|edges)==60 and meta['cumulative_reviewed_unit_count']==len(prior_units|set(ATOMS))==20,'累積件数不一致')
 text=STATUS.read_text(); batch=re.search(r'218要求unitのうち新たに(\d+) unit、候補edge (\d+)件',text); cumulative=re.search(r'wave 1〜6と合わせて(\d+) unit、(\d+) edge',text); remaining=re.search(r'残る(\d+) unitは未着手',text)
 ...
 print('legacy requirement direct semantic review wave7: schema9 / 9 edges / 10 atoms / bounded search + phase candidates verified')
