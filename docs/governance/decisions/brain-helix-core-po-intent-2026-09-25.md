...
| 設計ユニット・パーツ | 「設計ユニットやパーツって考え方で取り入れる感じ」「ほかに工程表とか遷移図とか導けるようになってない？」 |
| 実装の優先順位 | 「実装の優先順位とかも出るだろ」 |
...
| バグbotの発行時期 | 「バグボットとかはCIでよくミスる系のものとかのログがたまって機械判定ができるようになったら発行していく」 |
...
- **HELIX-Intelligence**：HELIXについて最も知っている部位。ローカルLLMに判断を依頼し、バグbot・ヘルプbot・クローラーを発行して、HELIX全体の監査に寄せる。バグbotは、CIでよく失敗する種類のログがたまり、機械で判定できるようになったものから発行する。
...
- **導出物**：設計ユニット・パーツとその依存から、工程表、遷移図、実装の優先順位を導く。
...
- **実装の優先順位の材料**（AIの調査）：ハイブリッド設計ドキュメントは、優先順位そのものを計算する道具を持たないが、材料を持つ。
  - 要求の優先度（MoSCoW。`templates/02_要求定義書.yaml`、`templates/43_要求・要件一覧(管理台帳).yaml`）
  - バックログの優先順位付けの基準（WSJF＝遅延コスト÷SPを目安に、POが最終決定。`templates/112_プロダクトバックログ.yaml`）
  - 設計間の依存グラフと循環の検出（`tools/build.py deps`・`check`）
  - 工程表の先行関係と、その循環・日程逆転の検出（`tools/schedule.py`）
  - 旧HELIXでは、実装順は設計文書ごとに手で書いていた（例：`.../docs/design/helix/L5-detail/requirement-refinement-authority.md`「5. 実装順」）。依存グラフから安定したトポロジカル順を出す型は、`.../docs/design/helix/L6-function-design/issue-scope-authority-gates.md`の`AcyclicGraphV1.stable_topological_order`にある。
  - 設計ユニット・パーツの依存と要求の優先度から優先順位を導くのは、これらを接続する点で旧HELIXからの変更点である。
...
- **Intelligenceの導入版**：BRAINとIntelligenceは1.0からコネクタで接続できるようにする（PO）。現行Conceptの版表では、Intelligenceは3.0で加わるとしている。バグbotは、版ではなく、ログがたまって機械で判定できるようになった時点で発行する（PO）。ヘルプbot・クローラー・全体監査をどの版から稼働させるかは未確定。
- **既存判断との整合**：
