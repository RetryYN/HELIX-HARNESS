...
この判断は、旧資産の要求意味を一括採用すること、全資産のsemantic atom閉包を撤廃すること、または選択資産を一括rejectすることを意味しない。旧統制の「要求欠落を防ぐ閉包」条件2（Concept、要求、候補、IR、設計、test、runtimeのbehavior atomを一度ずつ接続する条件）を全体的に撤廃しない。L3での適用時期をscope選定後とするだけであり、選択した資産の確認を省略しない。
