...
- **L2.5**（HIL-FR-19／20）：画面プロトタイプとPoCは、要求と要件の間のL2.5に置く。PO「画面は要求とPoCして不確定要素を減らしてから要件定義に入る」「ハーネスみたいなのを作るのであれば画面プロトは当然スキップするから、2.5のPoCも同様」。その後、Codex reviewの指摘（PR #2128）を受けて、POは「画面がなくてもPoCは必要な場合があるだろ」と補足した。Prototypeは画面の有無で、PoCは技術的な成立性が不明かどうかで別に判定し、両方とも非適用のときだけL2.5を飛ばす。
...
- **各種類についてのPO判断**：
  ...
  - Reverse：突発の発行に加え、旧HELIXで必須だったScrum Reverseのcheckpoint（sprint review前、release candidate合流前など）からも計画的に発行する。「旧HELIXで実現されつつあったものは、すべて採用する」に基づきAIが保持した点であり、Codex reviewの指摘（PR #2128）による。
