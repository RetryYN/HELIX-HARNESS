...
本索引のSRV-FR／SRV-AC写しは非実行archive内の`docs/design/helix/L3-requirements/scrum-reverse-entity-model.md`にあり、資産台帳`LEGACY-ASSET-873BE1F8C64356A2FA0F`へ固定されている。宣言oracle文書のSRV-AC、fixture 16、entity全state表、confirmed親文書を含む3文書300非空行は[専用全量台帳](../delegated-document/scrum-reverse-source-line-inventory.md)と管理層source holdingで保持し、本索引には未収載である。confirmed親文書と対になる受入文書は30非空行をまだ行台帳へ展開せず、[file-blob holding](../delegated-document/delegated-requirement-document-source-inventory.md)で保持する。Scrum Reverse要求PRはそのatom化と仮登録を先行条件にする。本索引は24行をhistorical `draft` sourceとして可視化するだけで、旧L3をcurrent authorityへ昇格させない。
