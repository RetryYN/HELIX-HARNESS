...
├── vitest.config.ts              # Vitest coverage reporter + fast/slow project 分割 (PLAN-L7-348)
...
- **test = vitest**。`vitest.config.ts` は G7 coverage-summary evidence (`json-summary`) と fast/slow project 分割 (PLAN-L7-348) を一元管理する tracked exception とする。
...
→ target root configは **`package.json` / `package-lock.json` / `tsconfig.json` / `.editorconfig` / `biome.json` / `vitest.config.ts`**を上限とする。transition lockは分母外で別管理する。
