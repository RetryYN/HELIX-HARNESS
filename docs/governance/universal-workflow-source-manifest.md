...
extractor/canonicalization versionとsafe ZIP scan receiptは未登録である。14 entryの内容由来atomと採否は
`docs/governance/generated/universal-workflow-entry-atom-dispositions-v1.json`を正本とし、本表はentry inventoryに限定する。
...
- atomic entry disposition: 14/14、behavior atom 19/19登録、runtime verified 0/14 （日本語の機械契約記述）
...
したがって14 entryの観測inventoryとentry内behavior atom分母は閉じた。ただしこれは名称類似による`judgment-core`吸収ではなく、
各atomの入力・出力・副作用・failure、canonical target、assertion、gateを個別登録した設計判断である。branch単体custody、
runtime verification、current requirement authority、active package、独立reviewは未完了であり、coverage creditは0を維持する。
