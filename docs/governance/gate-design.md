...
| G10 | L10 システム/Real UX evidence | a11y / visual regression | uiux/PO (提案) | — |
| G11 | L11 受入/human visual | 受入 | ★ **PO** | — |
...
> 本節の駆動モデル詳細は concept §2.5/§4 + 各 `legacy PR commandocess/*-workflow.md` (参照 snapshot) を根拠とし、HELIX では `helix` CLI の mode routing として実装 (FR-08、L7)。**§4 自動追加型クロスチェックエンジンは Forward/駆動 双方の成果物に適用**される (frontmatter の kind/workflow_phase 駆動でルール enroll)。
...
| G6 | ✅ **PASS (remediation accepted / 是正受入)** | **A-109 (initial L6 completion audit, 2026-06-09) + A-110 (independent re-audit, 2026-06-09) + A-111 (post-remediation re-audit, 2026-06-09)** | A-111 で A-110 remediation を受入済み。MUST-1 readability と MUST-2 FR addendum substance は content fix と machine gate で close 済み。code-reviewer hardening は IMP-092/093 follow-up として実装済みで、freeze blocker ではない。 |
...
**フロー (FR-05/18)**: `helix doctor` / `helix gate <G-ID>` → [1] レジストリ構築 → [2] 該当ルール解決 → [3] 全 doc に適用 → 結果集約 (severity 別) → fail-close。
...
- **L7 carry**: engine 実装 + 既存 5 lint のリファクタ吸収 + `gate-checks.yaml` + `helix gate`/`doctor` 配線。ADR-002 (dependency-drift) / IMP-001/002/003/006 / **FR-L1-49 (asset-drift、A-85)** を本エンジンのルール型として統合。
  - **partial discharge (PLAN-L7-95)**: この carry のうち **「`doctor` 配線」部分**を先行 discharge。`doc-consistency` / `entity-coverage` / `fr-registry-audit` / `improvement-backlog` の 4 audit を standalone で `helix doctor` へ実配線 (helper 再利用のみで本体 audit が inert だった死蔵を解消)。あわせて **IMP-006 (lint-coverage-map)** を `src/lint/lint-wiring.ts` meta-gate として実装 = 「全 `src/lint/*` は runtime 経路から到達可能 or DEFERRED 登録済み」を fail-close (死蔵ルールの再発防止)。汎用 engine 本体 + `gate-checks.yaml` + rule-type 吸収は **IMP-033 に残置** (standalone 配線はエンジン到来時に rule instance へ移行可能、互換)。
...
> **A-105 note（注記、2026-06-08, G5 add-design: harness.db reference-feedback and automation foundation）**: A-104 後、ユーザーは SQLite を V-model state cache だけでなく feedback mechanism として扱う必要があると明確化した。mechanical checks、DB references、drive/mode logs、skill firing rates、workflow automation readiness、guardrail safety、skill/roster/command docs、search-cost reduction は data-backed でなければならない。PLAN-L5-08 はこの L5 slice を追加し、L8 IT-DB/IT-SEARCH/IT-FEEDBACK/IT-AUTOMATION/IT-GUARDRAIL/IT-ASSET-DB と pair する。G5 は core scope では PASS のままであり、これは rollback ではなく additive re-bless である。
