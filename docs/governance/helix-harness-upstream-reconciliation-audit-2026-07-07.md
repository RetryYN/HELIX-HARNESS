...
- D-CONTRACT DSL、G9/G10 gate、L14 close audit、context doc-router、runtime telemetry provenance、
  skill scaffold、model override hardening、adapter runtime hardening、route mode projection、
  relation-graph node scope、toolchain pin、workflow/document parity。
...
### 3.1 clean distribution sync / package commands
...
### 3.2 Pack update-check advisory
...
### 3.3 doctor check registry / timing / setup-smoke scope
...
| PLAN | follow-up status | green evidence |
|---|---|---|
