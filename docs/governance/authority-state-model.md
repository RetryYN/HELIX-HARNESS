...
scope: upstream documents, requirement carry-forward, work tickets, GitHub projections
...
文書の既存metadata名は移行中の表示であり、この四軸を一つのfieldへ押し込まない。後続の要求登録・分類機構では四軸を別fieldとして持つ。現在のMarkdownでは、文脈が曖昧な`status`だけを根拠に状態遷移させず、台帳とdecision recordを併読する。
