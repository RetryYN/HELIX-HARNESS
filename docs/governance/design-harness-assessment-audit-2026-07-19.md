...
- `design-refactoring-domain-model.md`（externalize / commonize / objectize / semantic rename /
...
1. **DESIGN HARNESS を第一級サブシステムへ統合**: 現在別 slice の
   screen applicability / prototype / requirement translator / design obligation / domain model /
   design refactor / document metadata / semantic diff / visualization を
   Product Design / Experience Design / System Design / Design Governance の 4 層へ統合する。
...
3. **UI 設計用ドメインモデルの追加**: 既存 Domain Model Catalog（DDD/BE 寄り）とは別に
   Page / User Flow / Navigation / UI Component / Interaction Pattern / Design Token /
   Content Block / Feedback / Empty State / Error Recovery / Permission State / Analytics Event。
...
3. executable prototype manifest + walkthrough receipt
