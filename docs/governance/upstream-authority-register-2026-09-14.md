...
| 要求要否・重複・技術代替・影響simulation review program | 親1件、子3件 | [要否・再配置](requirement-disposition-review-program.md)、[責務・機能重複](requirement-overlap-review-program.md)、[技術代替可能性](requirement-technical-substitutability-review-program.md)、[交換可能な定義packageと影響simulation](definition-package-impact-simulation-research.md) | 全要求保持、重複候補の意味比較、技術と意味機能の分離、定義変更の影響可視化条件を定義。個別判断・successor・実装は未開始 | Concept／対象別L1承認後、source-qualified identityまたはatomごとに人間decisionと要求PRへ分け、定義package／adapter方式はresearch後に判断する |
...
| 旧HARNESS要求群 | 5文書。10 BR、3 UX、51 FR、15 NFR、15画面、技術要求7節 | [旧HARNESS要求の新世代対象別対応](audits/source-rebaseline/legacy-harness-requirements-source-crosswalk.md) | 全文・同一byte copyを保持し対象別分類済み。原要求単位の再配置・L2合意待ち | 原要求を保持してHARNESS／HELIX-OS／個別製品L2へ再配置する。旧実装方式の採用判断は要求意味と分ける |
...
| 要求整理Issue projection | open 4件（#1813..#1816） | [投影記録](audits/source-rebaseline/github-requirement-review-program-projection-2026-09-15.md) | RDP-001の要否・再配置、RDP-002の責務・機能重複、RDP-003の技術代替可能性、RDP-004の定義package・影響simulationをlocal sourceから投影しread-after済み | 親子Issueを進行共有だけに使い、closeやcheckboxから個別要求の採否・統合・技術選定・実装許可を生成しない |
