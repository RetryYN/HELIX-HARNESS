...
|---|---:|---|---|---|
| 最新責務境界 | 1 | [HARNESS・HELIX-OS・個別製品の責務記録](../concept/product-boundary.md)、[承認decision record](decisions/concept-v4.1-and-four-l1-approval-2026-09-17.md) | 旧本文SHA `097f27311060c56e387cf49fe6ec75731e5fd9dc04ac1a4be987d285e02ee038`は2026-09-17に承認済み。現行本文SHA `9268e35757c9a78baa47122917215b30ece1905d2b3f384c89fbfd4eb2b3ac0a`は[2026-09-24 PO判断](decisions/concept-requirement-po-decisions-2026-09-24.md)でPOが採用 | [PO判断パッケージ](crosswalks/concept-requirement-po-decision-packet.md)で旧revisionとの差分を判断する。現行Conceptを親境界とし、旧承認を現行L2/L11へ継承しない |
...
| 新世代Concept | 1文書（その場改訂） | [HELIX Concept](../concept/helix-concept.md)。旧来の[v4.1履歴](https://github.com/RetryYN/HELIX-HARNESS/blob/11a22679dc2bfce57d3294759531282445625001/docs/concept/helix-concept-v4.1.md)（[承認decision](decisions/concept-v4.1-and-four-l1-approval-2026-09-17.md)）、[v4.2履歴](https://github.com/RetryYN/HELIX-HARNESS/blob/11a22679dc2bfce57d3294759531282445625001/docs/concept/helix-concept-v4.2.md)（[承認decision](decisions/concept-v4.2-goal-owners-approval-2026-09-24.md)）は、下位文書の親付替えと同時に削除済み | 1ファイルをその場で改訂する。人の指示をAIが反映し、変更の履歴はgitに残す。版ごとのファイルと改訂ごとの承認記録は置かない | v4.3改訂に紐づく4対象L1、5大目標、七大原則、製品責務境界の親付替えと候補差分を記録済み。v4.1／v4.2の版別ファイルは削除済み |
| HELIX自体の5大目標 | 1文書、5目標 | [5大目標本文](../concept/helix-five-goals.md)、[承認decision record](decisions/helix-five-goals-approval-2026-09-24.md) | **旧本文SHA-256** `cfade733b9023bcc3329916206a911794e13f2b4f4f1e198d6c57618049771ca`のみ承認済み。本文SHA `e9668e77…2f29`の内容は[2026-09-24 PO判断](decisions/concept-requirement-po-decisions-2026-09-24.md)でPOが採用し、関与表を参照用に縮約するよう指示された。縮約後の本文はPO最適ドラフトPRで対象revisionを確認する。対象別L1・個別要求・実装・達成は未承認 | [PO判断パッケージ](crosswalks/concept-requirement-po-decision-packet.md)で旧承認revisionとの差分を判断後、現行Conceptから対象別L1へ分解する |
...
| 対象別L1 | 4文書、32企画要求 | HARNESS／HELIX-OS／HELIX-Web／HELIX-Web-OSの各`L1-planning/`、[承認decision record](decisions/concept-v4.1-and-four-l1-approval-2026-09-17.md) | HARNESS 9、HELIX-OS 12、HELIX-Web 6、HELIX-Web-OS 5の**旧exact revision**は2026-09-17に承認済み。[2026-09-24 PO判断](decisions/concept-requirement-po-decisions-2026-09-24.md)で、HELIX-OSの現行本文SHA `ffbafa47…51bc`をPOが採用した。HARNESSは本文SHA `ece3e268…4e96`の内容を採用し、1.0土台の追記を指示された（追記後の本文はPO最適ドラフトPRで確認）。HELIX-WebとHELIX-Web-OSは、要求層から外してVisionレベルの材料へ分類し直した | 旧承認revisionの責務境界を履歴根拠として保持し、現行差分は[PO判断パッケージ](crosswalks/concept-requirement-po-decision-packet.md)へ送る。現行本文をL2／L11の承認済み親にしない |
...
| 新世代要求候補 | 機構の候補12文書、再構築の進め方の候補5文書 | 機構の候補は各機構の`candidates/`（HARNESS 5、HELIX-OS 4、BRAIN 1、LABO 1、Intelligence 1）。再構築の進め方の候補は`docs/governance/candidates/` | [2026-09-25 判断記録](decisions/mechanism-placement-po-decisions-2026-09-25.md)で、機構の要求候補を担当する機構のフォルダへ移し、HARNESSとHELIX-OSにまたがる候補は機構ごとに分けた。HARNESS／HELIX-OSへ分離した候補。要求エンジン、意味密度抽出、Design Template、typed ticket候補、旧ルール群由来の要求候補、仮設束縛（Scaffold Binding）候補、WBS台帳候補を含む。うち`scaffold-binding`と`wbs-ledger`は2026-09-19の[decision record](decisions/l2d-s0-approval-and-s1-01-defer-2026-09-19.md)で承認済み（L2／L11未適用）。typed ticket候補（`development-ticket-derivation`）は[2026-09-24 PO判断](decisions/concept-requirement-po-decisions-2026-09-24.md)で退役した。他はdraft | 現行Conceptを親に、対象別L1は旧承認SHAと現行未承認差分を区別して、対象別L2／L11へ個別採否する |
...
| L2 source採否queue | S1–S4の29 decision unit＋S0の2 decision unit | [対象別L2 source採否順序](audits/source-rebaseline/l2-source-adoption-sequence.md) | Concept系列を除く29候補系列をS1–S4へ配置。2026-09-18にS0 `scaffold-binding`と`wbs-ledger`を追加。旧4対象L1のexact SHAのみ承認済み。現行L1本文は未承認候補。2026-09-19にS0の2 unitはapprove、`L2D-S1-01`はdefer（[decision record](decisions/l2d-s0-approval-and-s1-01-defer-2026-09-19.md)）。L2／L11本文への適用は全unitで未了 | S0の2 unitはL2／L11適用の要求PRへ。`L2D-S1-01`は未評価11 holdingの処分後に人間判断へ戻す。他unitは各unitを別decisionとして対象別L2／L11へ採否する |
| 新世代CI要求候補 | HARNESS 4要求、HELIX-OS 8要求、L11候補8項目 | [HARNESS側](../helix-harness/candidates/next-generation-ci-requirements.md)、[HELIX-OS側](../helix-os/candidates/next-generation-ci-requirements.md) | draft、要求整理のみ。CI relationを分離し、既存CIはlegacy source | HARNESS検証契約とHELIX-OS実行統制を対象別L1／L2へ接続し、L3／L10以降は上流確定まで待つ |
| 新世代AI可読文書要求候補 | HARNESS 3要求、HELIX-OS 9要求、L11候補8項目 | [HARNESS側](../helix-harness/candidates/ai-readable-authority-requirements.md)、[HELIX-OS側](../helix-os/candidates/ai-readable-authority-requirements.md) | 旧AI実行面をarchiveへ隔離し、最小上流入口を配置。L2／L11要求はhuman approval待ち、生成器は未設計 | 承認済み親revisionに対してL2／L11を個別採否し、manifest／生成器はL3／L10から再導出する |
...
| WBS台帳要求候補 | HARNESS 1要求、HELIX-OS 8要求（管理6＋接続2）、L11候補10項目 | [HELIX-OS側](../helix-os/candidates/wbs-ledger-requirements.md)、[HARNESS側](../helix-harness/candidates/wbs-ledger-requirements.md) | **approve**（[2026-09-19 decision record](decisions/l2d-s0-approval-and-s1-01-defer-2026-09-19.md)、`HDEC-L2D-S0-02`、正規語彙`split`。SHA-256 `34711045…`）。承認対象はこのSHAの本文であり、[2026-09-25 判断記録](decisions/mechanism-placement-po-decisions-2026-09-25.md)で2文書へ分けた後の本文は承認対象と別revisionである。L2本文未接続。WBSエンジンの実装・起動を許可しない（解除は別PRで決める） | L2／L11適用は別の要求PRで行う。`development-ticket-derivation`との統合可否は未決。パイロット：管理層取込→開発方式選定（PoCは別identity。本候補では実施を許可しない）→仮設束縛（`L2D-S0-01`承認後）→要件定義 |
...
| CI・AI既存候補の新世代対応 | 3系列、22旧要件、25旧受入 | [新世代CI・AI source crosswalk](audits/source-rebaseline/new-generation-ci-ai-source-crosswalk.md) | semantic atom照合済み、新世代で再承認待ち | 現行Conceptを親に、対象別L1は旧承認SHAと現行未承認差分を区別して、NCI／AIDOCのL2／L11で個別採否する |
| authority・全資産統制の新世代対応 | 3系列、30旧要件、44旧受入、4旧L12観測 | [新世代authority・asset crosswalk](audits/source-rebaseline/new-generation-authority-asset-governance-crosswalk.md) | JSON-only／旧owner／旧実装再利用前提を分離済み、新世代で再承認待ち | 現行Conceptを親に、対象別L1は旧承認SHAと現行未承認差分を区別して、HELIX-OS L2／L11で個別採否する |
| 提供構成の新世代対応 | 1系列、9旧利用者要求、6旧feature contract、24旧要件、26旧受入 | [新世代提供構成source crosswalk](audits/source-rebaseline/new-generation-release-composition-source-crosswalk.md) | HARNESS契約とOS実行を分離済み。旧CI先行利用は棄却、Cursor固有条件はWorkerへ移送、新世代で再承認待ち | 現行Conceptを親に、対象別L1は旧承認SHAと現行未承認差分を区別して、HARNESS／HELIX-OS L2／L11で個別採否する |
| 運用品質の新世代対応 | 1系列、9旧要求群、5旧L1要求、9旧L3要件、9旧L10受入 | [新世代運用品質source crosswalk](audits/source-rebaseline/new-generation-operational-quality-source-crosswalk.md) | 工程契約・OS運用・製品固有品質へ分離済み。旧owner／engine再利用と候補からの操作認可を棄却、新世代で再承認待ち | 現行Conceptを親に、対象別L1は旧承認SHAと現行未承認差分を区別して、対象別L2／L11へ採否し、L3／L10はその後に再導出する |
| Concept・Package取込の新世代対応 | 2整理文書、受領原文10文書、旧PKG 13候補、長期Vision 5段階 | [新世代Concept・Package source crosswalk](audits/source-rebaseline/new-generation-concept-package-source-crosswalk.md) | Concept・HARNESS・OS・個別製品へ分離済み。旧Package／Module／Bundle数、旧CI先行利用、旧main／Issue／PR authorityを棄却、新世代で再承認待ち | 現行の単一Conceptを親に、現行未承認L1候補との差分を判断してからL2／L11へ個別採否する |
| 管理変更入口の新世代対応 | 旧policy 1文書、旧受入4件 | [新世代管理変更source crosswalk](audits/source-rebaseline/new-generation-management-change-source-crosswalk.md) | OS管理統制とHARNESS差戻し条件へ分離済み。Issue-first、PLANを含むGit一括authority、旧workflow／adapter／CIを棄却、新世代で再承認待ち | 現行Conceptを親に、対象別L1は旧承認SHAと現行未承認差分を区別して、HARNESS／HELIX-OS L2／L11へ個別採否する |
...
| 限定修復の新世代対応 | 1系列、2旧利用要求、5旧要件、7旧受入、未提供別紙3件 | [新世代限定修復source crosswalk](audits/source-rebaseline/new-generation-bounded-repair-source-crosswalk.md) | HARNESS検証契約とOS修復統制へ分離済み。旧GH-FR-011権限、既存CI／DB／transactionを棄却、新世代で再承認待ち | HELIX-OS L2採否後、操作別authorityとL3／L10／L11を再導出する |
...
| Worker capacityの新世代対応 | 1系列、1旧利用要求、6旧要件、6旧受入、1旧L12認識候補 | [新世代Worker capacity crosswalk](audits/source-rebaseline/new-generation-worker-capacity-source-crosswalk.md) | HARNESS独立検証とOS capacity統制へ分離済み。三社・provider固定数・8-slot・既存CI／Merge Trainを棄却、新世代で再承認待ち | HELIX-OS L2採否後、resource profileとL3／L10／L12を再導出する |
| Security engagementの新世代対応 | 1系列、旧価値5項目、12旧機能要件、6旧非機能要件、12旧受入 | [新世代Security engagement crosswalk](audits/source-rebaseline/new-generation-security-engagement-source-crosswalk.md) | 個別製品security、HARNESS検証、OS操作統制へ分離済み。旧broker／provider／DB／CIを棄却し、実行権限なし、新世代で再承認待ち | 対象別L2採否後、data・操作authorityとL3／L10／L11を再導出する |
...
| 開発投資候補の新世代対応 | INV-001..072 exact 72件 | [新世代investment candidate crosswalk](audits/source-rebaseline/new-generation-investment-candidate-crosswalk.md) | 5群へ全件分類済み。旧P0..P4、Issue／owner、既存CI／Cursor／DB／scheduler、実装順を棄却し、新世代で再採否待ち | 現行Conceptを親に、対象別L1は旧承認SHAと現行未承認差分を区別して、意味候補だけを対象別L2へ採否し、方式はL3以降で再導出する |
...
| 要求整理Issue projection | open 3件（#1813..#1815） | [投影記録](audits/source-rebaseline/github-requirement-review-program-projection-2026-09-15.md) | RDP-001の要否・再配置、RDP-002の責務・機能重複、RDP-003の技術代替可能性をlocal sourceから投影しread-after済み。#1813は2026-09-19にmain `3469266e`のRDP-001へ再投影した（`RDPPROJ-1813-20260919-005`）。この再投影は実行時点で操作authorityが未成立であり、2026-09-20にPOが事後追認した（rollbackなし） | 親子Issueを進行共有だけに使い、closeやcheckboxから個別要求の採否・統合・技術選定を生成しない |
...
| 製品責務 | HARNESS、HELIX-OS、HELIX-Web、HELIX-Web-OSの旧所有範囲、service log export、改善接続 | [対象別責務記録](../concept/product-boundary.md) | 旧本文SHAのみ承認済み、現行責務表は未承認候補 | 旧Concept v4.1と4対象L1のexact revisionへ束縛した履歴。現行Conceptの機構区分とOS／LABO／Intelligenceの差分を[判断パッケージ](crosswalks/concept-requirement-po-decision-packet.md)で照合（[2026-09-17 decision record](decisions/concept-v4.1-and-four-l1-approval-2026-09-17.md)）。L2／L11の親は現行Conceptとし、本記録は旧境界と現行候補の差分照合に使う |
| 最新Concept | HARNESS自己改善を含む最新責務、新世代境界、authority、上流順序、5大目標、七大原則、Version 1の複数プロダクト・自己適用検証とWeb展開依存が一つのrevisionへ束縛される | 現行は[HELIX Concept](../concept/helix-concept.md)（1ファイルをその場で改訂）。4対象L1、5大目標、七大原則、製品責務境界は現行Conceptを親として記載する（L1の承認は[2026-09-17 decision record](decisions/concept-v4.1-and-four-l1-approval-2026-09-17.md)、v4.2本文の承認は[2026-09-24 decision record](decisions/concept-v4.2-goal-owners-approval-2026-09-24.md)） | 現行Conceptは機構再編後の本文。下位文書は親付替え済みで、意味差分は判断候補 | 下位文書の親をHELIX Conceptへ付け替え、差分を判断候補へ記録済み。v4.1／v4.2は削除済み |
| 対象別L1 | ConceptからHARNESS、HELIX-OS、HELIX-Web、HELIX-Web-OSの企画・価値・対象外が分冊される | 対象別L1 4文書。HARNESS 9、HELIX-OS 12、HELIX-Web 6、HELIX-Web-OS 5要求。L0柱とL2接続案を記載 | 旧exact revisionのみ承認済み。現行本文は未承認差分 | 旧承認revisionと現行候補を別に照合し、PO判断前の現行L1をL2／L11の承認済み親としない |
...
要求整理の直列境界は、`現行Concept → 対象別L1の現行差分判断 → source atom無損失再配置 → 対象別L2・prototype合意`
である。旧v4.1と旧4対象L1のexact revisionは2026-09-17のdecision recordで承認済みだが、現行Conceptに付け替えたL1差分の採否・L2／prototype合意は生成されていない。対象別L2・prototype合意が閉じる前にL3／L10、AI文書生成器、新世代CI、runtime、archive資産の意味移管・最終退役へ進まない。
