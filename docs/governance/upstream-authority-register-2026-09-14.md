...
| 旧ルール群由来の要求候補 | 規則atom 6,590件、要求候補55本（HARNESS／HELIX-OS） | [旧HELIXのルール群から導いた要求候補](candidates/legacy-rule-derived-requirements.md)、[規則atom台帳](legacy-rule-atom-inventory.jsonl)、[出どころfile一覧](legacy-rule-atom-source-files.jsonl) | draft、要求整理のみ。未承認。旧hook・旧lint・旧CIを実行も復活もしない | 既存L2・既存候補との重複と包含を付け、システム群ごとのIssue（#1852〜#1861、#1864）の入力にする |
