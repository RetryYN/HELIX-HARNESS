...
| WBS台帳要求候補 | HELIX-OS 管理 6要求＋接続 3要求、L11候補8項目 | [要求からの作業分解（WBS）台帳の管理層要求候補](candidates/wbs-ledger-requirements.md) | draft、要求整理のみ。未承認、L2本文未接続。WBSエンジンの実装・起動を許可しない | `L2D-S2-02 execution-ticket`と同じ判断単位で人間判断へ送る。パイロット：管理層取込→開発方式選定（scaffold仮組み）→要件定義 |
