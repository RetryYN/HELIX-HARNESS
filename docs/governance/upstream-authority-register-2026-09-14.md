...
| 仮設束縛の仮組み | binding 1件、検査tool、L11写し27 case | [scaffold/README.md](../../scaffold/README.md)、`scaffold/bindings/`、`scaffold/tools/scfctl.py` | scaffold（正式実装ではない）。`SCF-B-0001`として要求候補のrevisionへ束縛。証拠種別`scaffold`。差し替え台帳Issue #1866 | 正式なHELIX-OS実装（L3／L10導出）が入ったら`check-replacement`→`retire`で置換・撤去する |
