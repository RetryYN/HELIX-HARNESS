...
| archive資産の完全一致再利用 | manifest 4020件 | [完全一致再利用統制](legacy-asset-reuse-control.md) | `LICENSE` 1件をarchive sourceとSHA一致で確認。残りは意味再導出または未判定で、全asset atom閉包は未完 | 資産ごとにcopy可否、owner、親要求、consumer、権利、実行性を判定し、適格なものだけarchiveから同一byteで現行pathへコピーする |
