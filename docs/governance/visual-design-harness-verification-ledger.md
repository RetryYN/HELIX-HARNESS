...
## §1 existing asset disposition

| source | capability | disposition | hardening / boundary |
|---|---|---|---|
...
| layer | denominator | required evidence |
|---|---|---|
| L7 | screen/component mission×required state/oracle | source/test/contract ID、DOM/a11y tree、render、command digest |
| L8 | component/region/slot×9 state×theme×viewport×binding negative path | network/event/state trace、computed style/token、render/a11y artifact |
| L9 | screen×state×viewport×theme×locale×text scale×motion preference | baseline/candidate/diff heatmap、geometry、token provenance、a11y/perf report |
| L10 | AC×role×critical journey×browser/OS/viewport/DPR×data scenario | sanitized product snapshot、journey/video/render、timing、DB/API lineage |
| L11 | agreement criterion×current evidence packet×human actor | revision-bound annotated capture/session note、PASS/FAIL/UNCERTAIN、dissent/backprop |
...
|---|---:|---|
| source capability inventory | main families identified | PARTIAL |
| L8–L11 layer contract | designed | DRAFT |
| typed evidence schema implementation | 0 | FAIL |
| actual browser/VRT/a11y runtime | 0 | FAIL |
| current false-green reclassification | design only | FAIL |
| L8/L9/L10 executed matrix cell | 0 | FAIL |
| L11 human acceptance receipt | 0 | FAIL |
...
| corrected finding | 4 |
| case / obligation / oracle / failure orphan | 0 / 0 / 0 / 0 |
| authority / Design AC18 / schema orphan | 0 / 0 / 0 |
| CAS/security direct negative | `VIS-L8-B01` |
| baseline laundering direct negative | `VIS-L9-F05` |
| execution / authority activation / coverage | 0 / 0 / 0 |
