...
ScrumをVモデルの簡易版・縮退版・例外経路として扱わない。Vモデルはlayer/pairを先に束縛して降下する経路、
Scrumは価値sliceとfeedback cadenceを先に束縛して反復する経路であり、どちらも同じ品質属性、二主体review、
...
| route | 適用条件 | 工程規律 |
|---|---|---|
| `FULL_L1_L12_V` | 要求が十分確定、system/境界/依存が複雑、高リスク・規制対象、一括整合が重要 | L1〜L12を完全実施し、設計・実装・検証をV-pairで閉じる |
| `PRODUCTION_SCRUM_REDUCED_V` | 実装規模が小さい、継続成長させるproduct、feedback頻度が高い、一般的Scrumの価値sliceで独立検証可能 | Production Scrumを主engineとしてbacklog→sprint→increment→review→retroを反復する。各sliceはScrum ReverseでVモデル正本へ還流し、release時にcanonical V-pair evidenceを閉じる。route名の`REDUCED_V`は互換識別子であり品質縮退を意味しない |
| `V_DESIGN_SCRUM_IMPLEMENTATION` | systemは大規模・複雑で上流設計を先に通す必要があるが、実装・統合・段階releaseは価値sliceで進める方が速い | L1〜L5をVモデルで凍結し、その設計境界内をProduction Scrumで実装する。各incrementをL6↔L7から右腕へ検証し、Scrum Reverseで実測差分をL1〜L5へ返す。release candidateごとにV-pair全体を再収束する |
| `DISCOVERY_POC` | 非productionの仮説探索 | S0〜S4。S4決定前にproduction Forwardへ昇格しない |

route判定は、(1)要求確定度、(2)system/境界/依存の複雑性、(3)実装規模、(4)継続成長とfeedback頻度、
...
`SCRUM_REVERSE`を発火し、実装・実測・運用事実からL1〜L5の設計資産を逆生成・補正する。
...
Discovery PoCはS0〜S4でvision/prototype仮説を探索できるが、S4の人間判断前に`implemented`、`ux_verified`、production-readyを主張しない。採用する仮説は判定6軸により`FULL_L1_L12_V`、`PRODUCTION_SCRUM_REDUCED_V`、または`V_DESIGN_SCRUM_IMPLEMENTATION`へ昇格して正規V-pairを閉じる。
