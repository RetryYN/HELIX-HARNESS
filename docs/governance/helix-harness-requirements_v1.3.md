...
- **Version**: 1.3.4-draft
- **Status**: draft（2026-07-22 対話型要件形成中。current revision全体の最終合意receipt前はconfirmedにしない）
...
Infinity Loopは予定外findingのIssue起票・追跡・closureを本線とし、ルール違反、進行上のtrouble、将来的な進行改善を自走対象にする。
局所的に完結し、要求された検証receiptまで揃ったIssueは閉じる。検証が必要かつ未完了のIssueは優先度にかかわらず
閉じず、検証scope、oracle、owner、再開条件を保持してopenにする。Updateは自然な導入checkpointが到来した場合、または大幅な改善効果が
...
PoC artifactをproductionへ利用する場合は、原則としてForwardまたはProduction ScrumのPLANを新たに束縛し、L1〜L3の
承認済み契約、L4/L5設計、security、保守性、DB event/projection/checkpoint/schema追従、test、独立reviewを閉じて
製本する。PoCの動作実績、smoke、速度だけをproduction昇格receiptにせず、PoC artifactはこの契約内の入力または
実装候補として扱う。
...
fail-closeする。既存flat PLANを別Migration Featureへ先送りせず、PLAN体系、設計、generator、DB ingestion、trace、testを
Forwardで全面再設計し、全PLANを依存順に修正する。移行中もcanonicalとcompatibilityをdual-greenに保ち、event、trace、
dependencyを失わない。
...
依存閉包はPLAN間edgeに限定せず、requirement、design artifact、implementation、test／verification、DB event・projection・
checkpoint・schema、Issue、PR、receiptまでを含む。この閉包定義も上記Forward再設計で全体を見直し、部分的なFeatureで
継ぎ足さない。未解決の必須edgeは開始・closureをblockする。

計画済みの耐久性、性能p95、運用観測等は独立Issueへ逃がさず、左側の検証設計と対応する右側V-pair executionとして
ForwardまたはProduction Scrum内で製本する。Issueは予定外failure、rule violation、drift、operation trouble等の受付を
基本とする。Issueのclosureは抽象的な親子状態ではなく、対象Issueを閉じるPRへの明示リンク、同一HEADのverification、
review、DB closure receipt、mergeによって決める。
...
人間はProjects boardとIssue階層から追加ツールなしでactive frontierを読める。CIは内部CIを主品質gate、
GitHub Actionsを重要検査の独立ダブルチェック、merge後/nightlyを検知・Recovery用追加検査とする。
PR前・merge前内部CIのFull verificationとrequired gateをmerge後へ先送りせず、無根拠なgate削除・閾値緩和による
軽量化を禁止する。
