...
| `PRODUCTION_SCRUM_REDUCED_V` | L1〜L3とroute承認後、小規模・継続成長・高feedbackのproductを要件単位のsliceで進める方が適する | L3確定要件をbacklog/sliceへ束縛し、各sliceで正規のL4/L5設計、実装、検証を行う。合流時はDesign Refactorで粒度・重複・境界を正規化し、typed traceでVモデル正本へ接着する |
...
`SCRUM_REVERSE`を発火する。Scrum内でも正規設計artifactを作るため、Vモデル合流時は既存設計を再生成せず、
source slice、要件、設計、test、実測をtyped traceで対応V-layerへ接着する。この接着の標準工程として
**Design Refactor**を実行し、受入済み外部契約を保ったままslice由来の設計粒度、重複、責務境界、命名、依存を
canonical V設計へ正規化する。外部契約・要求・ACの意味変更を検出した場合はDesign Refactorで吸収せず、
先にRedesignとしてL1〜L3のユーザー承認へ戻し、承認後にDesign Refactorで構造統合する。
