...
| `PRODUCTION_SCRUM_REDUCED_V` | 段階release、feedback駆動、価値sliceで独立検証可能 | Production Scrumを主engineとして反復し、必要なV-pair evidenceをsliceへ束縛してrelease合流時に閉じる。route名の`REDUCED_V`は互換識別子であり品質・権限の縮退を意味しない |
| `DISCOVERY_POC` | 非productionの仮説探索 | S0〜S4。S4決定前にproduction Forwardへ昇格しない |

unknown、複合、route判定不能は`FULL_L1_L12_V`へfail-closeする。これは安全側の分類既定値であり、Scrumの格付けを下げる規則ではない。Scrumは文書・品質工程の省略機構ではなく、価値slice単位の一線級反復機構である。TDD、Reverse、受入条件、migration、rollback、security、release evidence、L12運用を省略しない。
