...
desired stateはharness.db / repo-owned ledgerのcanonical packetから生成する。GitHubのopen/closed、PR参照、
board列だけから工程状態や完了を推定しない。`Todo / In Progress / Done`はcanonical PLAN binding、active frontier、
typed closure receiptと終端PR mergeのjoinへそれぞれ束縛し、全page取得・API上限・rate-limit再開・option identity・
read-backをfail-closeで検証する。人間はProjects boardとIssue階層からsnapshot時刻とdriftを含むactive frontierを読める。
CIは独立verification groupとして3段の重み配分
（外部PR CI=typecheck+変更影響targeted+critical gate、full回帰と全gate=merge後内部CI+nightly、
無根拠なgate削除・閾値緩和による軽量化は禁止）とし、Project投影のgreenと相互に完了を代替しない。
