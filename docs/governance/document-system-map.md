...
> **工程authority**: 現行layer、名称、V-pairは`helix-harness-requirements_v1.3.md`のL1〜L12を正とする。本書のL0〜L14表は業界標準groundingを保持するlegacy compatibility projectionであり、新規authoring schemaではない。
...
## §1 工程 × 成果物 × 標準 旧マスター表（L0〜L14 compatibility）
...
> 🟢 本docで確定済: §0（基本設計=外部設計）、§1（旧L0〜L14から現行L1〜L12へ写像する標準grounding）、§3（配線図=DbC）。工程番号のauthorityはrequirements v1.3、標準との意味対応は本書を正とする。
