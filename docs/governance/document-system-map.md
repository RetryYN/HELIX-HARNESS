...
|---|---|---|
| L1 企画 | charter projection | L12 運用テスト |
| L2 要求 | BR/NFR/画面prototype | L11 受入テスト |
| L3 要件 | FR/AC/HAT | L10 総合テスト |
| L4 基本設計 | architecture/data/external IF/UI standard | L9 結合テスト |
| L5 詳細設計 | internal contract/physical data/test design | L8 単体テスト |
| L6 実装 | product code | L7 test implementation/TDD closure |

本番リリースはL11⇔L12間のmilestone。L0 charterは層外anchor。下記のL0-L14表と旧pair説明は出典・物理path対応を保存するcompatibility appendixであり、current成果物配置を決めない。
...
> **重要 (5) — current右腕**: 検証本質はデータ・環境の実在性を段階的に上げること。canonical pairはL5↔L8、L4↔L9、L3↔L10、L2↔L11、L1↔L12であり、旧L13/L14は独立層にしない。

## §1 Compatibility appendix: 旧工程 × 成果物 × 標準表（L0-L14、current authorityではない）
...
| **L10** UX 磨き | FE デザイン確定 / UX 検証 (WCAG、impl 後) | 実装済 UI を磨き WCAG 実比検証 (L2 の右腕ペア) | WCAG 2.2 / ISO 9241-110 | boundary confirmed: `docs/design/helix/L10-ux/ux-evidence-boundary.md`。完了証跡は将来 UX/WCAG cycle |
| **L11** 総合レビュー+UAT | 画面 ↔ BR/画面要求 突合 / PO UAT | 画面が業務要求を満たすかの受入前確認 | ISO 29119-3 Acceptance | boundary confirmed: `docs/design/helix/L11-uat/uat-evidence-boundary.md`。completion decision 2 件が残る間は close 不可 |
| **L12** デプロイ+受入 | 画面 AC 受入チェックリスト | 全画面 AC の受入判定 | IEEE 829 / ISO 25010 | evidence index confirmed: `docs/design/helix/L12-acceptance/acceptance-evidence-index.md`。version-up/cutover packet が fresh な場合のみ L14 へ接続 |
