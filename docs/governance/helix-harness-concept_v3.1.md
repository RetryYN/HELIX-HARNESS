...
- **工程層体系**: 現行正本はrequirements v1.3の**L1〜L12 Vモデル＋Scrum**。v3.1本文のL0〜L14はV2 source snapshotを読むcompatibility projectionであり、新規authoring authorityではない。
...
| V2 | **9-mode ecosystem（historical delta）** | §2.5新設時の記録。現行の出口はrequirements v1.3のForward L1〜L12へ合流する。Discovery / Refactor / Retrofit / screen-design / frontend-designを追加した履歴を保持する。 |
...
> **v3.1履歴注記**: この「3実装経路 + 4補助軸」はv3.1で9-mode ecosystemへ再編した。現行route判定と合流先はrequirements v1.3 §4〜§5を正とし、出口はForward L1〜L12へ合流する。旧mode名はcompatibility inputとして扱う。
...
### V-model: 左（設計）↔右（検証）の対（旧v3.1 L0〜L14 compatibility projection）

以下は旧V2成果物の読取表である。現行の正規V-pairはrequirements v1.3の`L1↔L12`、`L2↔L11`、`L3↔L10`、`L4↔L9`、`L5↔L8`、`L6↔L7`だけとする。左側で設計・test/measurement contractを先行凍結し、右側で対応する環境・データ実在性を使って検証する原則は継承する。
...
入口の状況に応じてworkflowを使い分け、**出口は必ずrequirements v1.3のForward L1〜L12へ合流**させる。modeは入口条件と文脈遷移を明示するだけで、設計・実装・検証・計測・運用の接続を分断しない。
...
# §3 Forward mode（現行L1〜L12、旧L0〜L14読取表を併記）

## 3.1 概念（L1〜L12 Vモデル＋Scrum）

Forwardは**L1→L12をV-model（左=要求・設計降下 / 右=実装・検証・計測上昇）**で進む中核経路である。本格systemはFull V、段階release・小規模systemはProduction Scrum Reduced Vを選ぶ。Scrum sliceもSR0〜SR4で設計資産へ戻してからForwardへ再合流する。L5までにtest designとverification/measurement contractを先行凍結し、L7〜L12で実装・検証する。直後のL0〜L14図は既存artifact読取専用である。
...
> **現行mode対応**: 本§4以降の旧mode説明はcompatibility referenceである。現行route、Scrum Reverse、exactly-one refactor routing、Forward合流層はrequirements v1.3 §4〜§5を正とする。
...
| **Forward spine (主線)** | 現行L1〜L12 V-model本線。各駆動モデルとScrum Reverseが出口で合流する終着であり、駆動モデルと並ぶmodeの1つではない。旧L0〜L14入力はv1.3 §9で写像する。 | legacy L0 | canonical L4 |
...
| **L0-L14 + V-model** | V2由来の旧15工程。compatibility input限定。現行はL1〜L12と6組のpairへv1.3 §9で写像する。 |
