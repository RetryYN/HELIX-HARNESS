...
このため、HELIX では「失敗をログに残す」だけでは不十分とする。失敗は、可能な限り **gate、validator、test、skill pack、handover、postmortem、orchestration policy** のいずれかに還元する。
...
| **業務 entity 列挙** | L1 業務要求 §10 (要件レベル、L0 用語と 1:1) | HELIX ドメイン entity (例: plan / gate / artifact / pair / mode / drive / agent_slot / handover / sprint / phase / carry / trace) を業務側面で列挙、対応 `.helix/` state / CLI subcommand / file を併記 |
...
| **handover** | セッション間の作業引き継ぎ状態 (`.helix/handover/CURRENT.json`) | L0 | L1 |
...
| **handover bypass 検知** | `helix handover` 機構を経ない手書き更新を `checkHandoverBypass` が表面化する検査。CURRENT.json の `generated_by` 署名欠落 (手書き pointer) / latest_doc の `# Session Handover` entry 数 > 記録値 (手書き追記) で検知。presence/stale/drift の `checkHandoverDiscipline` と責務分離 (導入層 L6、IMP-078 gap①)。`src/handover/index.ts` |
...
| **handover 機械ポインタ (CURRENT.json)** | `.helix/handover/CURRENT.json`。active PLAN / status / 最新 handover doc への pointer / digest 要約を機械可読で保持する単一 SSoT (gitignored)。`helix handover` が生成、CLAUDE.md ワークフロー・pre-push stale 検知の参照先 (旧 CURRENT.md は廃止、導入層 L6、PLAN-L6-06/L7-04) |
...
| **handover discipline (規律 surface)** | PLAN 活動 (active_plan + digest あり) があるのに `CURRENT.json` が未生成 / stale / 別 PLAN を指す (drift) 状態を機械が warn すること (`checkHandoverDiscipline`)。handover-on-completion を agent 記憶でなく Stop-hook + `helix doctor` で表面化する (導入層 L6 更新、IMP-047) |
...
| **直列化 3 条件** | タスクを直列実行すべきかの機械判定キー: **file_conflict** (同一ファイルを書く) / **downstream_dependency** (前段成果物・判断に依存) / **shared_state** (DB / current-plan / handover 等の共有 state を変更)。いずれか true → 直列化必須 (`mustSerialize`)、すべて false → 並列可 (上限 8)。PLAN §工程表 の各手順で `[並列]/[直列]` + 該当条件を明示 (要件 §G.4、導入層 L6、IMP-049) |
...
| **adapter lifecycle wrapper** | `helix codex\|claude --execute` が provider CLI 実行を SessionStart / PostToolUse / Stop で包み、session-log PLAN digest と handover warning の表面化を記録する機構。raw guard 共存 env は wrapper が付与する (導入層 L6、PLAN-L6-20/L7-21) |
