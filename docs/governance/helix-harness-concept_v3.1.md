...
v3.1 は、参照 snapshot V2 (source snapshot reference) の **開発工程・開発モード・配線 (routing/injection)** を HELIX-HARNESS のチーム開発向けに翻案して取り込む。コード/state DB は取り込まず、**工程・フロー・仕組み**を概念層に統合する。
...
**チーム翻案の原則** (V2 の個人前提 → HELIX-HARNESS のチーム前提):
...
- `vendor source snapshot` は read-only。V2 文言は概念参照のみで、HELIX-HARNESS 正本は本書と要件定義書に再記述する。

---

# §1 Why — なぜ HELIX-HARNESS か
...
`HELIX-HARNESS` はこの 4 問題に **3 つの実装経路 + 4 つの補助軸** で構造的に対処する。
...
HELIX-HARNESS は、失敗事例を隠すための管理ツールではない。AI 開発で実際に起きる失敗を観測し、次回以降の実行品質を上げるための **再利用可能な制御構造** に変換するための harness である。
...
このため、HELIX-HARNESS では「失敗をログに残す」だけでは不十分とする。失敗は、可能な限り **gate、validator、test、skill pack、handover、postmortem、orchestration policy** のいずれかに還元する。
...
HELIX-HARNESS は **個人開発を AI に委譲する際に発生する全実装パターン** を 3 つの経路で網羅し、4 つの補助軸で支える（旧 framing「チーム開発で発生する全実装パターン」を HELIX solo 化 PLAN-L1-06 で改訂。経路/補助軸の機械機構は不変、複数人前提のみ solo+AI roster へ）。
...
複数 runtime / adapter が使える場合、HELIX-HARNESS は「判断」と「実行」を分離して割り当てる。
...
HELIX-HARNESS は PLAN 起票前後に、タスクの難易度・エフォート・適用 skill を機械的に仮判定する。これは実装を止める gate ではなく、経路選択、orchestration、レビュー強度、必要 skill pack を決めるための事前分類である。
...
**HELIX-HARNESS 自身は HELIX W を適用しない (単一 V で進める)**。理由 (PO):
...
mode と工程を「絵」で終わらせず自動で繋ぐ仕組み。V2 の routing/injection を HELIX-HARNESS のチーム前提に翻案する (legacy runtime command → helix、legacy DB → `.helix/` state、個人パス排除)。
...
**チーム翻案 (最重要)**: V2 の `requires_human_approval: true` は「止まるシステム」しか示さない。HELIX-HARNESS では **「誰がサインオフするか」** を定義しないと形骸化する。
...
> 注: SKILL_MAP では L9-L11 が「デプロイ検証 / 観測 / 運用学習」だったが、HELIX-HARNESS は V2 process docs を正本として L9=総合テスト / L10=UX 磨き / L11=総合レビュー+UAT / L12=デプロイ+受入 / L13=デプロイ後検証 / L14=運用検証 を採用。本表は SKILL_MAP の挙動を V2 process L 番号に合わせて remap している。
...
R4 で Gap を整理した後、HELIX-HARNESS の Forward 経路に接続。R4 outcome に応じて L1 (要求定義) / L3 (要件定義 FR+AC) / L4 (基本設計) / L5 (詳細設計) / gap-only (差分集約のみ、新規層なし) のどこに合流するかを決める (L0-L14 体系。旧 L2/L3 = 全体設計/詳細設計 は L4/L5 に remap。要件定義書 §3.4)。
...
**本書は HELIX-HARNESS の概念定義書である。受入条件・enum 詳細・Phase 0 (初期フェーズ) 受入条件は `helix-harness-requirements_v1.2.md` を参照。実装詳細 (validator / workflow YAML / hook script) は将来の個別 PLAN-XXX で詳細設計する。**
