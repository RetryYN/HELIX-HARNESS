...
- **対象リポジトリ言語**: 言語非依存 (`helix` の test adapter / repository-local test command で吸収)

## 本書の位置付け

> **Current authority override（2026-07-19）**: 層番号とV-pairは
> `l12-canonical-vmodel-direction-directive_v0.1.md`を正とする。正規pairは`L1↔L12` / `L2↔L11` /
> `L3↔L10` / `L4↔L9` / `L5↔L8` / `L6↔L7`で、L0 charterは層外anchorである。
> 本書内に残るL0-L14記述はcompatibility projection debtであり、新規設計・gate・CI期待値を決めるauthorityではない。

本書は **構想書 (concept document)** として、**なぜ / 何を / どう繋がるか** のみを定義する。**どう実現するか** は別ファイル `helix-harness-requirements_v1.2.md` (要件定義書) で定義し、さらに各 enum・スクリプト・workflow YAML の **詳細実装** は L5 詳細設計 PLAN で詰める。

| ファイル | 役割 | 抽象レベル |
|---------|------|------------|
| 本書 (構想書 v3.1) | 概念 / モード / 経路 / 配線 / 補助軸 / 役割 | L1 概念 |
| 要件定義書 v1.2 | 受入条件 / enum / fail-close 条件 / Phase 0 (初期フェーズ) 受入条件 | L1-L3 要件 |
| 個別 PLAN-XXX (将来) | validator 実装 / workflow YAML / hook script | L5 詳細設計 |

## v3.0 の主旨 (v2.1 からの分離 + TL Round 3 重大指摘反映)

v2.1 (2215 行) を構想書 v3.0 と要件定義に分離し、現行要件は v1.1 を正本とする。Round 3 で TL から指摘された **概念レベルの重大指摘 5 件** を本書で対応:
...
v3.1 は、参照 snapshot V2 (source snapshot reference) の **開発工程・開発モード・配線 (routing/injection)** を HELIX の超個人開発向けに翻案して取り込む。コード/state DB は取り込まず、**工程・フロー・仕組み**を概念層に統合する。

| # | 取り込み | v3.1 での反映 |
|---|----------|---------------|
| V1 | **V-model工程体系（historical adoption）** | v3.1当時はL0-L14を採用したが、現行はL1-L12へ再投影済み。旧採番はcompatibility historyとして保持 |
| V2 | **9-mode ecosystem** | 入口を9 mode + 2工程専門に分け、出口は必ずForward L1-L12へ合流。Discovery / Refactor / Retrofit / screen-design / frontend-designを含む |
...
**HELIX 翻案の原則** (V2 の個人前提 → HELIX の超個人開発前提):

- V2 の legacy runtime command・個人絶対パス・legacy DB 依存は持ち込まず、`helix *` 相当・`.helix/` state・package-local に読み替える (要件定義書 §7 で機械化)。
- V2 の「PM=AI / PO=本人」固定を、**PO 1 名 + AI agent roster** 前提へ翻案。各ゲートは人間サインオフ点、`safety.requires_human_approval` は「誰の承認か」を具体化する (§2.6 / §9)。
- cross-agent review は「人を跨ぐレビュー」へ拡張 (§9、要件定義書 §6 CODEOWNERS)。
- `vendor source snapshot` は read-only。V2 文言は概念参照のみで、HELIX 正本は本書と要件定義書に再記述する。

---

# §1 Why — なぜ HELIX か

## 1.1 開発を AI に委譲すると起こる 4 問題

> **HELIX solo 改訂 (PLAN-L1-06)**: 本ハーネスは超個人開発システム HELIX の土台。WHY は「複数人チーム」前提でなく「**1 人の開発者が開発を AI に委譲する（超個人開発）**」前提に置く。下記 4 問題は AI 委譲で常態化するもので、P2 は人間の多人数役割でなく **AI agent roster の責務境界**として読む。

AI 駆動開発の現実観察から、以下 4 問題が常態化している:

| # | 問題 | 具体的影響 |
|---|------|------------|
| **P1** | 設計・実装・テストの乖離 | AI が「テストも書いた」と言うが設計 doc とテストコードが対応しない、逆ピラミッド化 |
| **P2** | AI agent の責務境界が曖昧 | 実装/レビュー/検証を担うサブエージェントの責任分担が崩れ、creation と judgement の分離（別ランタイム検証）が曖昧化する |
| **P3** | PoC が独り歩き | 仮説検証で書いた PoC コードが本実装で再実装される、知見が文書化されない |
| **P4** | 既存実装への破壊的追加 | AI が「より良い形」と称して既存設計を改変、既存テストを書き換えて回帰検知不能 |

`HELIX` はこの 4 問題に **3 つの実装経路 + 4 つの補助軸** で構造的に対処する。
...
> **HELIX solo 読み替えコンベンション (PLAN-L1-06 手順 1、本書全体に適用)**: HELIX は超個人開発システムで、
...
├── helix-harness-concept_v3.1.md
└── helix-harness-requirements_v1.2.md
```

## 1.4 失敗を仕組みに変換する原則

HELIX は、失敗事例を隠すための管理ツールではない。AI 開発で実際に起きる失敗を観測し、次回以降の実行品質を上げるための **再利用可能な制御構造** に変換するための harness である。
...
| session 断絶・認識ずれ | harness.db continuation projection / fenced memory / `recovery` PLAN / failure log | 次の AI session が同じ前提誤読から始まらない |
...
本ハーネスは Claude Code と Codex の連携を必須にしない。`helix` CLI と workflow / hook は、現在利用できる実行主体を検出し、以下 4 mode のいずれでも同じ状態モデルを扱う。

| mode | 利用主体 | 目的 | 必須条件 |
|------|----------|------|----------|
| `claude-only` | Claude Code + `.claude/` hook | 対話 UI と hook による設計・実装・停止時検証 | `helix` CLI + Claude Code project context |
| `codex-only` | Codex CLI + `AGENTS.md` | Codex 単体での TL 駆動実装・レビュー・検証 | `helix` CLI + Codex project rules |
| `hybrid` | Claude Code + Codex CLI | 役割分担、DB + fenced memory による継続、review、team run | `claude-only` と `codex-only` の両方 |
| `standalone` | `helix` CLI のみ | setup / doctor / lint / gate のローカル検証 | Claude Code / Codex なし |

`helix status` / `helix doctor` は mode、検出済み runtime、欠落 runtime、推奨 next action を表示する。`hybrid` 専用の委譲コマンドは、片方しか無い環境では fail ではなく明示的な `not-available` と fallback 手順を返す。

Cursor / Google Antigravity / GitHub Copilot などの周辺 AI IDE は、必須 runtime ではなく **optional adapter** として扱う。検出できた場合は `helix status` に表示し、CLI 経由で安全に呼べる範囲だけ `helix adapter <name> ...` に公開する。公開 CLI や automation API が不安定な adapter は、状態検出と手順提示までに留める。
...
複数 runtime / adapter が使える場合、HELIX は「判断」と「実行」を分離して割り当てる。
...
- 同一実行主体が自分の成果を承認しない。AI-A（作成・修正）とAI-B（監査・merge判断）のidentity/session/contextを分離し、provider/model差を独立性の必要条件にしない。

モデルの実名は変動するため構想書には固定しない。`.helix/teams/*.yaml` で provider / command / model / role / budget を宣言し、`helix status` が利用可否を表示する。これにより「Claude Code が中核」「Codex が実装/並列」「optional adapter が補助」という現在の構成を保ちつつ、将来のモデル更新に追従できる。

### 2.1.2.1 実行モードによるレビューゲート切り分け (gate 崩壊防止)

judgment reviewはruntime/providerの数で強度を変えない。全modeでAI-AとAI-Bを別identity/session/contextとして
起動し、同一HEADを独立に読む。別providerでも同一主体・同一contextの自己承認なら無効、同一provider/modelでも
独立した2実行主体なら有効とする。
...
| 条件 | 判定 |
|---|---|
| AI-AとAI-Bのidentity/session/context分離、同一HEAD、checklist、receipt digest一致 | full review成立 |
| provider/modelが異なるだけでidentity/contextが分離されていない | 不成立 |
| AI-A自身の読み直し、単一AI fallback、`degraded_mode` | 判断gate根拠として不可 |

#### execution mode 別の判断ゲート挙動

| execution mode | cross-agent review | 判断ゲート (G0.5 / G2 / G4-G7 / R4) の扱い |
|---|---|---|
| `hybrid` | 可能 | AI-A/AI-B分離を完全強制 |
| `claude-only` / `codex-only` | 可能（同一adapter上の別実行主体） | AI-A/AI-B分離を完全強制。review tierを縮退させない |
| `standalone` | 不可 (AIなし) | 機械lintのみ。二主体reviewが必要な判断gateはblock |

#### 核心ルール

1. self-reviewを判断gateの通過根拠にしない。AI-A/AI-Bのどちらかを確保できなければexit 1とする。
2. `review_evidence`へ両主体のidentity/session/context digest、HEAD、role、verdictを記録し、同一主体・stale HEAD・digest不一致をfail-closeする。
3. review観点は明文化checklistを逐条評価し、各項目にpass/fail/n-aと根拠を記録する。
4. adapter不在を理由に単一AI fallbackやreview tier縮退へ落とさない。利用可能な同一adapter上へ別実行主体を割り当てる。
5. **escalation 境界 (本番影響 / 認証 / 認可 / 決済 / PII / secret / ライセンス / schema migration / destructive / 外部 API・infra 変更) は execution mode を問わず人間サインオフ必須** (② でも代替不可。強制ブロック。§8 エスカレーションと整合)。
...
HELIX は PLAN 起票前後に、タスクの難易度・エフォート・適用 skill を機械的に仮判定する。これは実装を止める gate ではなく、経路選択、orchestration、レビュー強度、必要 skill pack を決めるための事前分類である。

- `helix task classify`: 入力文または PLAN から kind / drive / size / complexity / split_required を判定する。
- `helix task estimate`: 三点見積もりとリスク係数で effort_hours / story_points / buffer を出す。
- `helix skill suggest`: PLAN の kind / layer / drive / touched files から `docs/skills/*.md` の候補を推挙する。
...
| # | 経路 | トリガー | 主な kind |
|---|------|----------|-----------|
| **経路 1** | V-model Forward | L3要件・route承認後もslice化しない | `design` + `impl` |
| **経路 2** | Production Scrum | L3要件・route承認後にslice化 | slice PLAN + typed V trace。必要時`reverse` |
| **経路 3** | V設計＋Scrum実装Hybrid | L5詳細設計後にslice化 | `design` + slice PLAN + typed V trace。必要時`reverse` |
| **経路 4** | add-design / add-impl | 既存 PLAN への拡張要求 | `add-design` + `add-impl` |
...
> **v3.1 注記**: この「3 実装経路 + 4 補助軸」は v3.1 で **9-mode ecosystem (§2.5)** に再編した。経路 1=Forward、経路 2=Reverse / Scrum / Discovery、経路 3=Add-feature、補助 1=Recovery / Incident に対応し、さらに Refactor / Retrofit / screen-design / frontend-design を追加する。入口 (mode) は状況で分岐するが、出口は必ず Forward L1-L12 (§3) に合流する。mode の自動判定と委譲の配線は §2.6。
...
> **用語確定**: 上記 **L0-L14 の左右ペア = V-model**。**W-model (HELIX W) はこれとは別概念**で、**V で確定した仕様 (一般システム = 外殻) を土台に、AI エージェントを構築するための設計** = **V を 2 回通す** (1 回目 = 仕様確定 / 2 回目 = エージェント構築) ため V が 2 つ重なって W 字になる (`docs/design/harness/L1-requirements/functional-requirements.md` FR-L1-28 / two-stage-agent-design)。

| | 構造 | 適用 |
|---|---|---|
| **V-model** | L0-L14 を 1 回。左 (設計+テスト設計凍結) ↔ 右 (検証) | 通常開発 (一般システム) |
| **HELIX W (2段V)** | L0 (全体 1 回) → **フェーズ 1: L1-L9 (一般システム、drive=be/fe/db)** + **フェーズ 2: L1-L9 (エージェント昇華、drive=agent)** → **フェーズ 3: L10-L14 で合流** | **AI エージェントシステムを作る場合のみ** |
...
**本ハーネス自身は HELIX W を適用しない (単一 V で進める)**。理由 (PO):
- harness は **VSCode 内 (Claude Code) にシステムを作っている**ため、**一般システム = 外殻 (VSCode / Claude Code) は既存** ＝ フェーズ 1 (V1) は終了している前提に近い。
- その外殻の中で、エージェントが外殻のツールを使って動く仕組み = **内部整備**を作るのが harness の仕事。これは HELIX W の **2 段目の V (内部整備 / AI が使いやすい仕組み)** に位置づく。
- **内部整備のみ**なので **片方の V (単一 V) だけでよい**。よって L4/L5 等を単一 V で設計したのは正しい。

> HELIX W は **harness が対象リポジトリへ提供する機能** (FR-L1-28、対象が agent システムの時に使う) であり、harness 自身の開発プロセスではない。

### 3 段階 freeze (v3.0 で明示分離 — 元重大指摘 C1 対応。v3.1 で L0-L14 へ remap)
...
| **段階 A2: TDD Red freeze** | ② 実装コード作成前 | ③ + ④ の単体テスト設計/先行テストコード | L7 入口 (G6 通過直後) |
...
| Commit | pre-commit hook + commitlint | gitleaks / commitlint 実行 |
...
入口の状況に応じて **9 mode + 2 工程専門 workflow** を使い分け、**出口は必ず Forward L1-L12 (§3) に合流** させる。mode は「入口条件」と「文脈遷移 (昇華)」を明示するだけで、完了先 (設計・実装・検証・運用の同一接続) を分断しない。これにより入口を散らさず工程を一本化する。
...
mode と工程を「絵」で終わらせず自動で繋ぐ仕組み。V2 の routing/injection を HELIX の超個人開発前提に翻案する (legacy runtime command → helix、legacy DB → `.helix/` state、個人パス排除)。

### 2.6.1 signal → mode 自動 routing

検出 signal から mode を機械判定する (V2 `SIGNAL_TO_MODE` / `DRIFT_TYPE_TO_ROUTE` 相当を `helix` config に再定義)。例:
...
- `requires_human_approval`: **action-bound人間承認必須**（本番・権限・データ完全性等の高影響actionに対してtrue。Recoveryという分類だけでは立てない）
- `requires_preflight`: 前段 preflight 必須 (upgrade 高リスク時)

**HELIX 翻案 (最重要)**: V2 の `requires_human_approval: true` は「止まるシステム」しか示さない。HELIX では **「誰がサインオフするか」** を定義しないと形骸化する。

| 引き金 | 承認者 (人間サインオフ) |
|--------|------------------------|
| Recoveryの診断・branch修正・PR作成 | AI自走。承認待ちにせずmain復旧を最優先する |
| production deploy / rollback / destructive migration | 対象action、環境、HEAD、影響、rollbackを束縛したPO承認 |
| staging検証・rehearsal | policy範囲内でAI自走し、productionとは環境receiptを分離する |

この承認者定義は `.helix/` policy または `.claude/CLAUDE.md` Guard Rules に置き、`requires_human_approval` が立ったとき自動参照する (要件定義書 §7)。`command` の値は legacy runtime command ではなく `helix *` 相当に置換する。
...
**execution mode との結合 (重要)**: provider/runtime名を含む既存`orchestration_mode`値は互換projectionである。
現行契約は全execution modeでAI-A（作成・修正）とAI-B（監査・merge判断）の2実行主体を要求し、
identity/session/contextを分離する。同一provider/modelでも独立条件を満たせば有効、別providerでも自己承認なら
無効とする。二主体を確保できない場合はgateをblockし、単一AI fallbackや`degraded_mode`へ落とさない。
...
これら検出器は `.helix/` state を参照し、`helix doctor` / `helix plan lint` に束ねる (要件定義書 §7)。

---

# §3 Forward mode: V-model L1-L12（要件確定時の通常開発）

## 3.1 概念（L1-L12 canonical V-model）

ForwardはL0層外charterをanchorとして、**L1→L6/L7→L12**をV-model（下降=企画・要求・要件・設計・実装、上昇=テスト）で進む一線級delivery engineである。Production Scrumも同格のdelivery engineであり、V設計＋Scrum実装Hybridを組み合わせとして持つ。各modeは選択中engineへ接続し、最終要求・設計・検証contractはVモデル資産へ収束する。正規pairはL1↔L12 / L2↔L11 / L3↔L10 / L4↔L9 / L5↔L8 / L6↔L7である。
...
L0 は「リポジトリ初期化」ではなく **企画工程**。企画書 PLAN は背景・目的・スコープ (高レベル方向性) を持つ **feed-forward 文書**であり、L1 へ渡すことが役割。**社内システム/開発基盤では ROI・KGI/KPI の定量化を企画書段階で強制しない** (定量指標・受入条件は L1 業務要求 / L3 で定義し、企画書との二重記述を避ける)。想定リスクは判明分のみ任意。企画書は `kind=charter` (layer=L0、`parent_design` 不要 = root) で起票し、必須 role は `po` (要件定義書 §1.8)。**G0.5 企画突合**は軽量ゲートで、高レベル方向性が L1 業務要求へ trace できるか + 整合性破綻がないかだけを軽く確認する (完全性は求めない。書きすぎ = L1/L3 相当の詳細は穴とせず L1 へ降ろす。軽い他者レビューを推奨するが強制必須にしない。fail 条件は要件定義書 §2.1.1)。リポジトリ初期化・Branch Protection 等の基盤整備は Phase 0 (要件定義書 §10) として工程外で扱う。
...
### 3.1.2.1 L1 sub-doc 構造 (V2 source snapshot reference を HELIX 正本へ再定義、必須 § 含む)

V2 source snapshot reference の process doc / L1 requirements 実体 doc は **L1 を 1 doc にまとめず 5 sub-doc に分割**する。HELIX はこれを設計概念として参照し、各 sub-doc の **必須 § 構造**まで規定する:

| sub-doc (5 種) | 必須 § (source snapshot reference 実体 doc に準拠) | PLAN 命名 (HELIX) |
|----------------|------------------------------------------|--------------------|
| **業務要求** (business) | §1 目的・背景 (WHY/WHAT/WHO) / §2 対象業務一覧 / §3 業務フロー (Forward V-model 主線 + 9 mode 分岐 + cross-cutting 横断機構) / §4 ステークホルダー / §5 現状課題 → あるべき姿 / §6 業務スコープ外 (本 BR で扱わない: FR / 画面 / 技術 / NFR / 実装) / §7 L14 運用テスト pair 対応表 (BR-* ⇔ OT-* 1:1) / §8 関連 doc / §9 carry / 既知の不足 + §9.1 上流 baton carry 一覧 / **§10 業務 entity 列挙 (DDD 適用、要件レベル / 詳細は L4)** + §10.1 主要業務 entity 一覧 (L0 用語と 1:1 対応 / 業務的意味 / 対応 schema・CLI・file の 4 列 table) + §10.2 L4 carry (集約境界 / 値オブジェクト / entity ID 規約 / ライフサイクル / 不変条件 / 集約間整合性 / `helix doctor check_business_entity_coverage` 新設) + §10.3 SSoT 参照 (ユビキタス言語 / Bounded Context / 業界標準整合) | `PLAN-L1-01-business-requirements` |
| **機能要求** (functional) | §1 機能一覧 (**FR-L1 現行 51 件、P0: 19 / P1: 24 / P2: 8 で確定**、`docs/design/harness/L1-requirements/functional-requirements.md` §1 が正本) / §2 利用シナリオ (ユースケース) / §3 操作とデータの流れ / §4 入出力 / §5 上流 baton 反映 (L0 企画書バトン項目と本 doc FR-L1-* の対応表 + carry 先) / §6 関連 doc | `PLAN-L1-02-functional-requirements` |
| **画面要求** (screen) | §1 画面一覧 / §2 画面遷移の要望 / §3 表示・操作への要望 / §4 関連 doc (具体的画面設計は L2、本 sub-doc は要求レベル) | `PLAN-L1-03-screen-requirements` |
| **技術要求** (technical) | §1 採用技術・技術制約 / §2 外部連携 + IF 要望 / §3 既存システム制約 / **§4 state schema 二層構造** (HELIX では `.helix/` 配下、core tables + audit/event tables + derived views + 補助 state、closure event 契約 = `idempotency_key = mode + plan_id + closure_event_id` + rollback + conflict resolution) / **§5 工程別 skill 注入機構** (`docs/skills/<L>-injection.yaml` 相当、`owner_role` / `mandatory_agents` / `recommended_agents` / `recommended_skills` / `recommended_commands` / `orchestration_mode` の 6 フィールド) / **§6 9 mode 共通基盤** (R0-R4 + RGC を Reverse 専用ではなく共通 closure language として再利用、Forward 接続 event の state 登録 + 補助 state への中間 state 保存 + discrepancy_log からの機械起動) / **§7 drift 解消方針** (detector の週次以上起動 + inventory schema による工程双方向 mapping + 新規 asset 工程未割当不許容 + Reverse normalization 接続 + 運用目標「新規 drift 0 件 / week」) / §8 関連 doc | `PLAN-L1-04-technical-requirements` |
...
V2 source snapshot reference の業務要求 doc §10 を採用し、HELIX は L0 → L1 → L4 のドメイン継承チェーンを以下で固定する (DDD anti-corruption layer 原則):
...
[L4 基本設計 - データ設計 sub-doc]   ドメインモデル詳細 (arc42 §5 Building Block View)
  + helix doctor check_business_entity_coverage で entity ↔ schema / CLI 整合検出
```

| chain step | 担当 doc | 規約 |
|---|---|---|
| **ユビキタス言語 SSoT** | L0 企画書 §Glossary (HELIX では `docs/governance/helix-harness-concept_v3.1.md §10 用語集`) | L1 sub-doc は L0 用語を **parent_doc reference** とし、独自定義禁止 (anti-corruption layer) |
| **Bounded Context SSoT** | L0 企画書 §BC (HELIX では §2.5 9-mode ecosystem) | L1 の業務スコープ・mode 接続は L0 BC を参照 |
| **業界標準整合 SSoT** | L0 企画書 §業界標準 (HELIX では §11 参考文献) | L1 NFR の IPA × ISO 25010 二軸 / 業務 entity の DDD 適用 等は L0 で宣言された業界標準と整合 |
| **業務 entity 列挙** | L1 業務要求 §10 (要件レベル、L0 用語と 1:1) | HELIX ドメイン entity (例: plan / gate / artifact / pair / mode / drive / agent_slot / continuation / sprint / phase / carry / trace) を業務側面で列挙、対応 `.helix/` state / CLI subcommand / file を併記 |
| **ドメインモデル詳細化** | L4 基本設計 (データ設計 sub-doc) で carry 確定 | 集約境界 / 値オブジェクト / entity ID 規約 / ライフサイクル / 不変条件 / 集約間整合性ルール を確定、`helix doctor check_business_entity_coverage` で機械検出 |

**anti-corruption layer 原則**: L1 で entity を独自定義することを禁止し、L0 用語との 1:1 対応を `helix plan lint` (sub_doc=business 時) で機械検証する。詳細は要件定義書 §1.10.G.7 (新)。
...
| 要素 | 内容 | 検証 |
|---|---|---|
| **SSoT 参照宣言ブロック** | `ユビキタス言語 = L0 §10 用語集 / 業界標準整合 = L0 §11 / Bounded Context = §2.5 9-mode。本 doc は L0 を parent_doc reference とし、用語独自定義は行わない (anti-corruption layer)` を明示 | `helix plan lint` (sub_doc 指定 PLAN 全件) |
...
V2 は設計を **L4 基本設計 (外部設計: アーキ/ADR)** → **L5 詳細設計 (内部設計: D-API/D-DB/D-CONTRACT、API/Schema Freeze)** → **L6 機能設計 (関数 signature・エッジケース + WBS)** の 3 段に分け、それぞれ G4/G5/G6 で独立凍結する。旧 harness の L2 全体設計 / L3 詳細 / L3.5 機能はこの L4 / L5 / L6 に remap される。

### 3.1.3.1 L2-L6 sub-doc 構造 (V2 source snapshot reference)

各設計層も sub-doc 分割を取る (V2 source snapshot reference の各 process doc から HELIX 向けに再定義):
...
依存・重複の機械検出は §2.6.5 の横断検出 (relation-graph / connection-deficiency) と code-index (`helix code find` / `dup` 相当) を用いる。具体チェック項目は要件定義書 §7.8.7.1 (DEP / DUP / MOD)。
...
G1 (L1 業務要求 pair freeze) は以下 3 sub-gate を **全件通過** で完了する。gate 番号体系は変えない (G1 は G1 のまま、内部を 3 段に細分化)。

```
G1 (L1 業務要求 pair freeze)
  ├─ G1-content : 5 sub-doc 全件起票完了 + 件数確定
  ├─ G1-pair   : L1↔L14 OT 量閉じ (孤児 0)
  └─ G1-trace  : 業務 ⇔ 画面 ⇔ 機能 双方向 trace 整合  ← 新規 (DD1=a)
```
...
G1-trace (業務⇔画面⇔機能 双方向 trace 整合確認)
  ↓ pass
G1 通過 → L2 画面設計 (G2) へ進行
```

各 sub-gate fail 時は当該 sub-gate に戻り修正。G1 通過は 3 sub-gate 全件通過までブロック (fail-close)。
...
SSoT: screen sub-doc §5 trace マトリクス (手順 J で起票)。`helix plan lint --gate G1-trace` で機械的に一次判定する。
...
以下は AI 実装が踏みやすい V-model / V-model 違反。`helix plan lint` / `vmodel_validator` で機械検出し、`frontier-reviewer` の 3 点レビュー観点にも組み込む (要件定義書 §2 / §7)。
...
| AP-9 | 重複実装 / 機能被り | 既存に同等機能があるのに再実装する (着手前の `helix code find` 流用確認を怠る。被りは Add-feature / Refactor へ回す) |
...
工程表の手順には **review (self / pmo-sonnet / tl-advisor) を必ず固定手順として組み込む** (self-review 前置原則と整合、`.claude/CLAUDE.md` Guard Rules)。

## 3.7 駆動別 L2-L14 挙動表 (source skill-map reference + V2 process L 番号 remap)

各 drive (be / fe / fullstack / db / agent) で L2-L14 の中身とゲート判定が変わる。本節の表を HELIX 正本とする。source skill-map reference と V2 process docs は翻案元 reference であり、現行判定は本書と要件定義書に従う:
...
| **L7 実装順** | ロジック → API → FE | BE (契約 base) ∥ FE (**モック → 本実装昇格**) → 統合 | スキーマ → CRUD → API → FE | フェーズ A: BE Sprint ∥ FE Sprint (**mock 起点**) → フェーズ B: L5/L8 結合 | ツール → オーケストレーション → UI |
...
> **2026-05-28 PO 指摘修正**: 旧版では「be (BE-only) = L2 全 skip 可」と一括判定していたが、helix 自身を含む「UI を持つ be」では画面要求 3 sub-doc は必須 (PO 指摘「L2 スキップすんな。モックは作らなくてもせめて画面要求は作れよ」)。skip 対象は wireframe (High-Fi モック) のみ。画面要求の機械検証義務は drive 非依存。

> 注: SKILL_MAP では L9-L11 が「デプロイ検証 / 観測 / 運用学習」だったが、HELIX は V2 process docs を正本として L9=総合テスト / L10=UX 磨き / L11=総合レビュー+UAT / L12=デプロイ+受入 / L13=デプロイ後検証 / L14=運用検証 を採用。本表は SKILL_MAP の挙動を V2 process L 番号に合わせて remap している。

---

# §4 経路 2: Discovery / Production Scrum × Reverse routing

> **v3.1 mode 対応**: 本 §4 (経路 2) は 9-mode (§2.5) の **Scrum / Reverse / Discovery** に、§5 (経路 3) は **Add-feature** に、§6 (緊急経路) は **Recovery / Incident** に対応する。各 mode の入口判定・推奨コマンド・承認者は §2.6 の配線で決まる。Forward 合流先の層番号は L1-L12 (§3) を正とする (旧 L0-L11 表記が本節以降に残る場合は §3 に読み替える)。

## 4.1 概念

本節は2経路を区別する。**Discovery Scrum**は要件未確定・仮説検証のPoCをS0〜S4で判断し、採用時に
Full V、Production Scrum、またはV設計＋Scrum実装Hybridへ昇格する。**Production Scrum**は小規模・
継続成長・高feedbackのproductを価値sliceで本番開発する一線級engineであり、PoCではない。
両経路ともReverseを使うが、Discoveryは仮説の昇格、Production Scrumは実測知見のV正本同期を目的とする。

## 4.2 Discovery Scrum (S0-S4)
...
各 type は S0 (バックログ) → S1 (スプリント計画) → S2 (PoC 実装) → S3 (検証) → S4 (判断: confirmed / rejected / pivot) のフェーズを辿る。`agent_slots` の `aim` が PoC 実装を担当、`tl` が S4 判断を担当。

## 4.3 Reverse モード (R0-R4)

DiscoveryでconfirmedになったPoCの昇格、またはProduction Scrum incrementの実測をV正本へ同期するために使う。**5 type** に分類:
...
各 type は R0 (証拠) → R1 (観測済み契約、type により skip) → R2 (現状設計) → R3 (意図仮説、**po 検証**) → R4 (差分と Forward 合流経路) を辿る。

## 4.4 30 cell matrix (Scrum 6 type × Reverse 5 type — 元重大指摘 C4 対応)
...
R4 で Gap を整理した後、HELIX の Forward 経路に接続。R4 outcome に応じて L1 (要求定義) / L3 (要件定義 FR+AC) / L4 (基本設計) / L5 (詳細設計) / gap-only (差分集約のみ、新規層なし) のどこに合流するかを決める (L0-L14 体系。旧 L2/L3 = 全体設計/詳細設計 は L4/L5 に remap。要件定義書 §3.4)。
...
- 未 commit ファイルの取り残し
- 認識ずれの記録
- authored sources・harness.db continuation projection・fenced memory breadcrumb の整合
...
## 7.2 Required Status Checks の方針 (元重大指摘 C6 対応)
...
内部CIを主品質gateとし、GitHub Actionsの`harness-check`は重要検査の独立ダブルチェックとする。
PR化前に作成AIが内部CIを通し、監査AIが文脈・DB追従・修正差分をreviewして再検査する。GitHub Actionsが
greenでも内部CI未実行・失敗・receipt/HEAD不一致ならmerge不可。重要検査は内部CIとGitHub Actionsの各々で
60秒以内を目標とし、遅延時は検査を省略せずRecoveryで性能改善を最優先起票する。
...
具体的な workflow event 設定は要件定義書 §6 で確定する (v2.1 の C5 指摘に対応)。
...
source-derived のスキル群は、個人プロジェクト用の原文をそのまま使わず、HELIX 向けの **skill pack** として `docs/skills/*.md` に正本化する。curate 対象は「追加機能設計」「ドキュメント」「実装」「テスト」「Reverse」「運用」の単位に分け、各 skill pack は必ず workflow / harness / gate のどれに接続するかを明記する。
...
## 8.3 エスカレーション L0-L3 (元重大指摘 C8 対応)
...
## 8.5 failure_log の取扱い (元重大指摘 C7 対応)
...
| 役割 | 略号 | 上流 / 下流 | 主責任 |
|---|---|---|---|
| **発注元** | po | (両端) | なぜ / 何を / 受入基準 / R3 Intent 検証 / リリース承認 |
...
| **AI実装・保守** | aim | (中間) | AI 指示 / L7 実装委譲 / 3 点レビュー / アラート対応 / エスカレーション初動 / 4 段レビュー レイヤー 2 |
...
| **aim** | L7 実装委譲 / 3 点レビュー / 4 段レイヤー 2 | S0-S3 PoC 実装 / R0 証拠 | 既存テスト維持確認 | 初動アラート対応 |
...
| レイヤー | レビュアー | 観点 | 応答目安 |
|---|---|---|---|
| **レイヤー 1** | AI (自動) | コード規約 / 明らかなバグ / 典型的脆弱性 | PR 作成直後 |
| **レイヤー 2** | aim | テスト不足 / 観点漏れ / 運用影響 / vmodel 整合 | 1 営業日以内 |
| **レイヤー 3** | tl (必要時) | アーキ判断 / 技術選定の妥当性 / 大規模変更影響 | 1 営業日以内 |
| **レイヤー 4** | qa (リリース前) | 品質ゲート観点 / E2E / 運用観点 / G9·G11 判定 | リリース前 |

CODEOWNERS でレイヤー 3 / レイヤー 4 が自動アサインされる (具体的 path → owner マッピングは要件定義書 §6)。
...
## §10.0 coding-rule governance terms（coding rule 統治用語）

| 用語 | 定義 |
|---|---|
| **coding-rules SSoT** | TypeScript/Node transactional boundary と Python semantic core の coding rule 正本。`docs/governance/coding-rules.md` を rule ID と workflow placement の SSoT とし、doctor `checkCodingRules` が強制 gate として検証する (導入層 L6、PLAN-L6-23/L7-24/REVERSE-23) |
...
| **rethrow-only catch** | `throw` だけを持つ catch block。failure state の変換・記録が無いため structured-error-handling violation とする (導入層 L7、PLAN-L7-25) |
...
| **integration-gwt** | integration test design の `IT-*` 行が **前提 / 操作 / 期待結果** の粒度を持つことを検査する rule (導入層 L6/L8、PLAN-L6-30/L7-31/REVERSE-30) |
...
> 本節は HELIX ユビキタス言語の**単一 SSoT** (§3.1.2.2 anti-corruption layer)。各工程は語を独自定義せず、新規導入 / 精緻化した語をここへ **back-merge** する **living glossary** とする。各語に **導入層** (初出工程) と **更新層** (意味を更新した工程、無ければ —) を記録する。§10.3 機構用語は既定で導入層 = L0。機械検証は要件定義書 §1.10.G.9。
...
| **continuation** | authored sources を根拠に harness.db projection と fenced memory が保持する、セッション間の bounded な継続状態 | L0 | L1 |
| **sprint** | L7 実装スプリント単位 (TDD Red→Green→3 点 R) | L0 | L1 |
| **phase** | 現在の V-model 工程位置 (`.helix/phase.yaml`) | L0 | L1 |
...
>
> **L4 集約 (Aggregate、導入層 L4)**: 上記 entity を DDD 集約に grouping した **Plan / Artifact / Workflow / Evaluation の4集約** (data.md §2)。各集約ルート = plan / artifact / phase / evaluation_batch。continuationはappend-only eventから導出するread modelであり独立集約・prose SSoTを持たない。集約間は ID 参照のみ。詳細は [L4 データ設計 data.md](../design/harness/L4-basic-design/data.md)。
...
| **§工程表 (PLAN 内手順)** | PLAN (=区間/span) 本文の §工程表。その区間を進める **作成手順 + 進捗 (☐/🔄/✅) + 各手順の [並列]/[直列]**。**工程表 (roadmap) の leaf 側**であり top レベルの roadmap とは別 (同名異義)。区間内の AI 開発オーケストレーション (依存洗い出し→難易度分類→agent 割当→並列/直列) を表す。 | L0 | L6 |
...
| **全プログラム被覆 (program coverage)** | 工程表 (roadmap) が forward 全バンド (L0-L3 / L4-L6 / L7 / L8-L14 + cutover) を登録被覆している状態。doctor が未登録 forward work を強制違反として検出し、「実装どこまで?」へ機械的に回答可能にする (柱3 state DB 完全性)。検査 = `src/lint/roadmap-registry.ts` + doctor (fail-close、未登録バンドを park 宣言可)。 | L0 | L4 |
...
| **Forward spine (主線)** | L1-L12 V-model 本線（L0は層外anchor）。9 駆動モデルが出口で合流する終着であり、駆動モデルと並ぶ「mode の 1 つ」ではない (IMP-069 reconcile、PO 2026-06-05「Forward=spine」確定)。operational 正本 mode 構成 = Forward spine(1) + 駆動モデル(9、Research 含む) + 工程専門(screen/frontend、2) | L0 | L4 |
...
| **fullback (V字回帰)** | 駆動プラン (検証) の終了後、本線 V-model へ合流復帰すること (kind=reverse + forward_routing で復帰先指定) | L0 | — |
| **decision_outcome** | 検証 (PoC) の終了判定: confirmed / rejected / pivot (S4 必須) | L0 | L3 |
...
| **module-drift lint** | architecture §3.1 設計 module 集合 ⊇ `src/` 実在 module の包含 drift (impl→design back-fill 漏れ) を doctor が強制違反として検出する検査 (`src/lint/module-drift.ts`、`checkModuleDrift`、fail-close)。A-103 で旧 handover/setup/web を「将来」放置した歴史的 meta-drift を再発防止する ADR-002/IMP-032 の最小スライス。asset-drift (roster/skills の内容整合、IMP-033) / dependency-drift (import グラフ循環・逆依存、IMP-032 本体) とは別検査 (導入層 L6、IMP-075) |
| **検証発火 (verification trigger)** | V-model 層群の Forward freeze 完了を検知して検証サイクル発火タイミングを表面化する機構 (doctor `checkVerificationGroups`)。検証ロードマップの「いつ検証するか」を人の記憶でなく V-model 構造で機械化 = 崩れ防止の全体調整 (導入層 L6、IMP-068)。`src/vmodel/lint.ts` |
...
| **retired prose-handover resurrection 検知** | 廃止済みの session/prose handover writer・reader・`CURRENT.json`・`helix handover`・旧 aggregate が active surface に復活した場合に fail-close する検査。provider delegation evidence、operations transition、Recovery PLAN、歴史記録は別型として許可する (導入層 L6、PLAN-REVERSE-344) |
...
| **判断コア (judgment core)** | 実装/レビュー agent の判断規律 SSoT (`docs/skills/judgment-core.md`)。advisor-fable の 5 軸 (PLAN-L7-306) を普遍 7 原則へ昇格し、モデル別調整 (Opus/Sonnet/Haiku/GPT 系)・レビュー規律 (adversarial framing / false-positive 抑制 / severity-first)・委譲 4 点セットを単一正本化。agent/command は frontmatter `judgment_core: v<N>` marker で参照し、乖離は judgment-core-coverage lint が fail-close (導入層 L7、PLAN-L7-335) |
| **judgment-core-coverage lint** | 判断コア SSoT の `judgment_core_version` と `.claude/agents/*.md` / `.claude/commands/*.md` の marker + SSoT path 参照の同期検査 (fail-close、対象 0 件も違反)。`src/lint/judgment-core-coverage.ts` (導入層 L7、PLAN-L7-335)。allowlist-sync lint (`src/lint/allowlist-sync.ts`、SUBAGENT_ALLOWLIST 正本 ⇔ .claude/CLAUDE.md 転記の set 比較) と対で agent governance の silent drift を塞ぐ |
| **委譲ブリーフ (delegation brief)** | subagent 委譲 prompt の最低要件 = 4 点セット (objective / output format / tool guidance / task boundary、judgment-core §5)。Claude Agent spawn では `agent-guard` が 4 marker (`【objective】【output format】【tool guidance】【境界】` 等、英日ラベル可) の欠落を fail-close で block する。正本 = `agent-guard-policy.ts` `DELEGATION_BRIEF_MARKERS` (導入層 L7、PLAN-L7-337) |
| **role 判断ブリーフ (role judgment brief)** | `helix codex/claude --role` 委譲 prompt へ adapter が機械注入する role archetype (worker / verify / consult) 別の判断規律。Codex 側に role 別判断基準の正本が無い非対称を塞ぎ、ブリーフ無し委譲を作らない。正本 = `src/runtime/role-judgment.ts` (導入層 L7、PLAN-L7-337) |
| **思考レンズ (task lens)** | 委譲 task 本文の keyword 判定で検出する領域別思考チェックリスト (設計 / 検証 / テスト戦略 / トラブルシューティング)。該当領域があるときだけ adapter が委譲 prompt へ注入し (無関係レンズの常時注入は context 浪費)、各レンズは観点 distillation + 詳細 skill pack pointer を持つ。role 判断ブリーフ (どう振る舞うか) と対で「何を考えるべきか」を機械保証する。正本 = `src/runtime/task-lens.ts` (導入層 L7、PLAN-L7-338) |
...
| **Phase 0-A (solo) / Phase 0-B (team)** | CODEOWNERS bootstrap 2-stage (要件 §6.5)。0-A=branch protection なし / CODEOWNERS なし / harness-check 非 Required。0-B=CODEOWNERS + branch protection + Required。solo→team の格上げは opt-in + gh auth/admin preflight 付きのガバナンス変更。`helix setup` が出し分け emission を担う (導入層 L6、PLAN-L6-05) |
| **参加規模検出 (project scale detection)** | owner 種別 / collaborator 数 / 既存 CODEOWNERS・protection から solo/team を**提案**する検出 (`helix setup`)。確定は人間確認 + state 記録 (数だけで自動確定しない)。検出不能は solo に安全フォールバック (導入層 L6) |
| **emit-only (GitHub 設定)** | branch protection 等の GitHub 設定操作を既定では自動適用せず、スクリプト + 手順の生成にとどめる方針。明示 `--apply-branch-protection` 時も action-binding approval と gh auth/admin preflight が揃った場合だけ適用する。token は保持しない (導入層 L6) |
| **continuation checkpoint** | authored PLAN / design / test-design と append-only session event を根拠に、harness.db が PLAN status・成果物・Next Action・carry・escalation を projection する bounded な継続状態。prose document や local pointer は SSoT にしない (導入層 L6、PLAN-REVERSE-344) |
| **fenced memory breadcrumb** | continuation checkpoint の decision / constraint / next action だけを provenance・scope・TTL 付きで recall 可能にする補助記憶。DB projection を上書きせず、不整合時は authored sources と DB を優先する (導入層 L6、PLAN-REVERSE-344) |
| **plan_id 活性化 (current-plan)** | `.helix/state/current-plan` を `helix plan use <id>` で設定し session-log の PLAN digest を populate させる経路。solo/main 直で branch から PLAN を読めず plan_id が null になる Gap を埋める (`resolveActivePlan` の入力、本体は不変、導入層 L6) |
| **continuation readiness** | PLAN 活動に対して authored source、append-only event、harness.db projection、必要な fenced memory breadcrumb が整合し、Next Action / carry / escalation が欠落・stale・別 PLAN drift でない状態。Stop-hook と `helix doctor` が表面化する (導入層 L6、PLAN-REVERSE-344) |
| **plan family / dedup 正本化** | bare plan_id と slug 付きを同一 PLAN family と見なし (`sameFamilyPlan`、`-` 境界 prefix・対称・推移)、最長 (最具体) id を正本に digest を union 集約する (`dedupeDigests`)。continuation projection の `(unknown)` ゴーストを排除 (導入層 L6 更新、IMP-048) |
| **agent-slot / slot lifecycle** | subagent / team member の fire→release を機械記録するレイヤー 2 オーケストレーション単位 (`Slot`、`.helix/state/agent-slots.json`、`slot_source` = agent_guard/team_runner/manual、gitignored)。source reference の agent slot lifecycle 挙動を HELIX-HARNESS TypeScript/Node 実装として再定義したもの = `src/runtime/agent-slots.ts` (fire/release/listActive/listStale/`recordGuardFire`、全 fail-open、導入層 L6、IMP-050) |
| **peak_parallel** | 与えた slot 群の同時実行ピーク数 (sweep-line、`peakParallel`)。`helix doctor` が stale slot (5 分超 release なし) と併せて表面化する (導入層 L6、IMP-050) |
| **直列化 3 条件** | タスクを直列実行すべきかの機械判定キー: **file_conflict** (同一ファイルを書く) / **downstream_dependency** (前段成果物・判断に依存) / **shared_state** (DB / current-plan / continuation projection 等の共有 state を変更)。いずれか true → 直列化必須 (`mustSerialize`)、すべて false → 並列可 (上限 8)。PLAN §工程表 の各手順で `[並列]/[直列]` + 該当条件を明示 (要件 §G.4、導入層 L6、IMP-049) |
| **team 定義 (strategy)** | `.helix/teams/*.yaml` (`teamDefinitionSchema`、`src/schema/team.ts`)。`strategy: sequential|parallel` + `max_parallel` + `serialization` 3 条件 + `members[].serialize_after` で直列/並列を宣言。source reference の team runner 挙動を HELIX-HARNESS TypeScript/Node 実装として再定義したもので、`helix team run` (hybrid) の入力 (導入層 L6、IMP-050) |
...
| **adapter lifecycle wrapper** | `helix codex\|claude --execute` が provider CLI 実行を SessionStart / PostToolUse / Stop で包み、session-log PLAN digest と continuation readiness finding の表面化を記録する機構。raw guard 共存 env は wrapper が付与する (導入層 L6、PLAN-L6-20/L7-21) |
| **plan metadata separation** | `helix codex\|claude --plan <id>` の PLAN は harness/session-log metadata として保持し、provider CLI 引数へ `--plan-id` を転送しない規約。provider 境界を汚さず digest plan_id だけに使う (導入層 L6、PLAN-L6-20/L7-21) |
...
| **back-fill pairing** | 駆動モデルが「設計ドキュメントまで戻す」完全性。bottom-up build した impl を上位設計/governance へ Reverse 合流させ、§6 用語更新を L0 §10 へ back-merge する。`src/lint/backfill-pairing.ts` が「Reverse 無き impl」「glossary 未 merge」「legacy conditional debt 監査 drift」を検知 (`helix doctor`、fail-close、導入層 L6、IMP-051)。検査の主要出力 = `reverseOrphans` / `reverseLinkMissing` / `legacyAuditGaps` / `glossaryGaps` / `conditionalPending` / `conditionalDecisionMissing`。複合ラベルの表記ゆれは `normalizeTerm` (先頭コア語) で吸収 |
| **L6 completion readiness** | L6 docs、対応 L6 design/add-design PLAN、L7 unit-test design、G6 gate status を集約し、L6 完了可否を明示する機械判定。`src/lint/l6-completion.ts` が warn-only で表面化し、G6 freeze 前の未完了条件を draft docs / draft PLANs / L7 draft / G6 not-pass として列挙する。導入層 L6、PLAN-L6-22。 |
| **L6 completion readiness lint** | `analyzeL6Completion` / `checkL6Completion` による L6 completion readiness の実装名。doctor に未完了条件を表示し、G6 audit 時に強制化できる。導入層 L7、PLAN-L7-23。 |
...
- `helix-harness-concept_v3.1.md` (本書)
- `helix-harness-requirements_v1.2.md` (要件定義書)
...
- 参考: IEEE Wikipedia: V-model (software development)
- DO-178C 開発ライフサイクル仕様
- 参考: Parasoft: ISO 26262 Requirements Traceability
- CMMI v2.0 SP 1.4 Requirements Management
- IEEE 829-2008 テスト成果物
- ISO/IEC/IEEE 29119-2 テスト設計仕様

### Scrum + Reverse engineering（参考文献）

- 参考: Scrum.org — What is a Spike?
- Agile Alliance — Spikes
- 参考: Martin Fowler — Exploratory Testing
- 参考: SAFe — Spikes (enabler spike)
- 参考: Mike Cohn — Spikes (time-box + validated)
- 参考: Basecamp Shape Up — Uncertainty Reduction
- 参考: OMG MOF 2.0 — Model-Driven Architecture
- arc42 — Reverse Engineering Integration

### GitHub Actions + ブランチパイプライン

- Conventional Commits v1.0.0 specification
- 参考: commitlint official docs — @commitlint/config-conventional
- 参考: GitHub branch protection rules — required status checks
- 参考: GitHub Actions — workflow syntax / job ids / status check names
- 参考: CODEOWNERS syntax and examples
- 参考: Atlassian — Branch per feature workflow
...
- 参考: PagerDuty Escalation Policy Design
- 参考: AWS Incident Manager Escalation Plans
- 参考: Martin Fowler: Approval Workflow Pattern
- 参考: Google SRE — Escalation chapter
...
| **3.0** | **2026-05-20** | **構想書と要件定義書に分離。本書は構想 (なぜ / 何を / どう繋がるか) のみ。TL Round 3 の概念レベル重大指摘 5 件 (C1/C4/C6/C7/C8) + I22 を反映** | **PM + TL** |
