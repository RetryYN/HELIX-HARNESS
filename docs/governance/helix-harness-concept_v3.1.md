...
| **emit-only (GitHub 設定)** | branch protection 等の GitHub 設定操作を既定では自動適用せず、スクリプト + 手順の生成にとどめる方針。明示 `--apply-branch-protection` 時だけ gh auth/admin preflight 後に適用する。token は保持しない (導入層 L6) |
