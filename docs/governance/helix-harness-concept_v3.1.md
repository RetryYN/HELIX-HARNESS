...
> 本書の価値、gate、state、security、trace構想はcurrentである。L0–L14層番号、旧V-pair、Windows-firstに読めるOS表現は
> active runtime/schema compatibilityと既存artifact解釈に限定する。L1–L12 authoringは承認待ちcandidateであり、receiptが
> pendingの間はどちらの意味でも新規freeze/実装へ進まない。implementation preflightはblocked、runtime cutoverはpendingである。

- **版**: 3.1
- **位置付け**: 構想書 (L1 概念層) / 要件定義書は別ファイル
- **対応構想書**: AI駆動開発チーム構想書 v1.1
- **対応運用ルール書**: AI駆動開発チーム運用ルール書 v1.1
- **工程層体系**: 本文のL0-L14はlegacy runtime compatibility。L1-L12 candidateはauthority receipt承認後にactive化する
...
cross-agent review は **別 runtime / 別モデルのレビュアー**を前提とする。単一エージェント環境 (claude-only / codex-only) ではこれが物理的に不可能なため、execution mode (§2.1.1) を参照せず判断ゲートを「レビュー済み」で通すと、**self-review が cross-agent review に化けてゲートが崩れる**。`helix gate` は必ず `helix status` の mode を参照する。
...
|---|---|---|
| `hybrid` | 可能 (worker ≠ frontier-reviewer) | 完全強制。worker と reviewer の (provider, model) 同一なら承認無効化し exit |
| `claude-only` / `codex-only` | 不可 | **② 専門サブエージェント review を必須化** (強制)。明文化 checklist の逐条実行記録が無ければ gate を **exit 1** で止める。実行時は `review_kind: intra_runtime_subagent` + `cross_agent_review: unavailable` を記録 |
| `standalone` | 不可 (AI なし) | サブエージェントも起動不可 → 機械 lint のみ pass し、判断ゲートは **人間レビュー必須** を `next_action` に出す (自動 pass 不可) |

#### 核心ルール

1. **self-review (③) を判断ゲートの通過根拠にしない**。単一エージェント時は ② 専門サブエージェント review を強制要件とし、未実行なら exit 1 (無言通過禁止)。
...
| L2 画面設計 | (実データ検証 = mock / 本 UX) | L10 システム検証＋Real UX evidence | **実データ検証**: 本番の実データで画面/表示が成立するか |
| L3 要件定義 (FR+AC) | 受入テスト設計 | L12 運用・価値検証 | **本番受入**: 本番環境で要件 (FR+AC) が満たせるか |
| L4 基本設計 | 総合テスト設計 | L9 結合検証 | **総合**: テスト環境・全体で方式が成立するか |
| L5 詳細設計 | 結合テスト設計 | L8 単体・詳細検証 | **結合**: テスト環境・モジュール間契約が成立するか |
...
| Deploy | Incident mode の incident-log + Protected Branch (L12 運用・価値検証) | `gh` CLI |
...
L1 要求定義     (po主体) 業務要求 BR-*/NFR-* ① + 運用テスト設計③ ──→ L14 で実施
  └ G1 (po+tl)                                                     ▲ G12
L2 画面設計     (uiux)   ワイヤーモック ① ─────(mock がペア)─────→ L10 システム検証＋Real UX evidence (uiux) G10
  └ G2 (pm+uiux)
L3 要件定義     (tl主体) FR-*/AC-* ① (BR-* から trace) + 受入テスト設計③ ──→ L12 運用・価値検証 (pm+po)
  └ G3 (po+tl)                                                     ▲ G9
L4 基本設計     (tl)     アーキ/ADR ① + 総合テスト設計③ ──────────→ L9 結合検証 (qa) G9
  └ G4 (tl+pm, tl-advisor 必須)                                    ▲ G8
L5 詳細設計     (tl+se)  D-API/D-DB/D-CONTRACT ① + 結合テスト設計③ ─→ L8 単体・詳細検証 (aim/qa) G8
...
L7 実装スプリント (aim→se)  ② 実装コード / ④ テストコード
  ...
  └ G7 実装凍結 (4 artifact trace freeze、6 pair 双方向)
        │
        ▼
L8 単体・詳細 →G8→ L9 結合 →G9→ L10 システム検証＋Real UX evidence →G10→ L11 受入＋human visual acceptance (pm+po) →G11
   → L12 運用・価値検証 (pm+po) →G12→ L13 デプロイ後検証 (自動/pm) →G13
...
- **L11 受入＋human visual acceptance フィードバック → L3 要件は `add-design`、L1 業務要求は `kind=design layer=L1`** (`add-design` は L3-L6 限定のため L1 不可。既存 doc は不可変)
...
### 3.1.6 L11 受入＋human visual acceptance (要件巻き取り)
...
| **L7 実装順** | ロジック → API → FE | BE (契約 base) ∥ FE (**モック → 本実装昇格**) → 統合 | スキーマ → CRUD → API → FE | フェーズ A: BE Sprint ∥ FE Sprint (**mock 起点**) → フェーズ B: L5/L8 単体・詳細 | ツール → オーケストレーション → UI |
| **L10 システム検証＋Real UX evidence** | 薄い (表示確認) | **厚い** (デザイン駆動) | 薄い (管理画面確認) | 標準 (結合後に Visual Refinement) | 会話 UI / デモ確認 |
...
### L10 (システム検証＋Real UX evidence) 要否 (drive 別)
...
| **be (UI を持つ、ダッシュボード等)** | **画面要求 3 sub-doc (画面一覧 / 遷移 / UI 要素) 必須**、wireframe (モック High-Fi) のみ省略可 (`skip_sub_doc: ["L2-wireframe"]` + 理由 `"Low-Fi で代替、High-Fi は L10 system/Real UX evidence"` または `"High-Fi モックは外部依頼 (Figma 等)"`、A-39/A-40 参照)。**High-Fi モックは ケース別判断** (harness 内保持 OR 外部依頼、外部依頼は許容オプションで必須ではない)。**外部依頼時は要件修正 back-propagation の可能性あり** (G1-trace 再検証必須) |
| fe | 全必須 |
| db (UI 無し) | 全 skip 可 |
...
> 注: SKILL_MAP では L9-L11 が「デプロイ検証 / 観測 / 運用学習」だったが、HELIX は V2 process docs を正本として L9=総合テスト / L10=UX 磨き / L11=総合レビュー+UAT / L12=デプロイ+受入 / L13=デプロイ後検証 / L14=運用検証 を採用。本表は SKILL_MAP の挙動を V2 process L 番号に合わせて remap している。 （層意味論: canonical L10=system/Real UX evidence; canonical L11=acceptance/human visual; canonical L12=operational/value; canonical L9=integration）
...
| **QA** (品質責任者) | qa | **下流** | テスト戦略 / G8-G9 ゲート (L8 単体・詳細・L9 結合) / インシデント指揮 / 観点リスト整備 / failure_pattern Issue 月次レビュー |
| **AI実装・保守** | aim | (中間) | AI 指示 / L7 実装委譲 / 3 点レビュー / アラート対応 / エスカレーション初動 / 4 段レビュー レイヤー 2 |
| **UI/UX デザイン** | uiux | (横断) | Figma / モック (L2 画面設計) / state-events / L10 システム検証＋Real UX evidence (screen-design / frontend-design mode) |
...
|---|---|---|---|---|
| **po** | L1 業務要求 / L3 受入条件 / G1·G3 / L11 受入＋human visual acceptance / L12 運用・価値検証 | **R3 Intent 検証** / Discovery 成功条件 | (通常は不要) | P0/P1 で連絡 / Recovery スコープ承認 |
| **tl** | G0.5 企画突合 / L4-L6 設計 / G4-G6 | S4 decide / R1-R2 / R4 routing | 既存 doc 整合判断 | 技術対応指揮 / リオープン確認 |
| **qa** | G8-G9 / L8 単体・詳細・L9 結合検証 | (通常は L8/L9 で合流) | 回帰確認 | インシデント指揮 |
| **aim** | L7 実装委譲 / 3 点レビュー / 4 段レイヤー 2 | S0-S3 PoC 実装 / R0 証拠 | 既存テスト維持確認 | 初動アラート対応 |
| **uiux** | L2 画面設計 / L10 システム検証＋Real UX evidence | UX-spike PoC | 既存 UX との整合 | (通常は不要) |
...
| 用語 | 定義 |
|---|---|
| **coding-rules SSoT** | TypeScript/Bun core の coding rule 正本。`docs/governance/coding-rules.md` を rule ID と workflow placement の SSoT とし、doctor `checkCodingRules` が強制 gate として検証する (導入層 L6、PLAN-L6-23/L7-24/REVERSE-23) |
...
| **agent-slot / slot lifecycle** | subagent / team member の fire→release を機械記録するレイヤー 2 オーケストレーション単位 (`Slot`、`.helix/state/agent-slots.json`、`slot_source` = agent_guard/team_runner/manual、gitignored)。source reference の agent slot lifecycle 挙動を HELIX-HARNESS TS/Bun 実装として再定義したもの = `src/runtime/agent-slots.ts` (fire/release/listActive/listStale/`recordGuardFire`、全 fail-open、導入層 L6、IMP-050) |
...
| **team 定義 (strategy)** | `.helix/teams/*.yaml` (`teamDefinitionSchema`、`src/schema/team.ts`)。`strategy: sequential|parallel` + `max_parallel` + `serialization` 3 条件 + `members[].serialize_after` で直列/並列を宣言。source reference の team runner 挙動を HELIX-HARNESS TS/Bun 実装として再定義したもので、`helix team run` (hybrid) の入力 (導入層 L6、IMP-050) |
