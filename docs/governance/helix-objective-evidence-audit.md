...
| distribution_pack_repo | `git ls-remote https://github.com/RetryYN/HELIX-HARNESS-OS.git refs/heads/main` | `refs/heads/main` | `unpublished` | repository reachable; main branch unpublished（repo 到達、main branch 未公開） | distribution repo moved to current HELIX-HARNESS-OS surface（配布 repo 確認先を現行 HELIX-HARNESS-OS へ移動） | current distribution surface; publish/tag activation required before adoption（現行配布面。採用前に publish/tag activation が必要） | distribution-version-binding gate retained（配布 version binding gate を維持） |
| distribution_pack_latest_tag | `git ls-remote --tags https://github.com/RetryYN/HELIX-HARNESS-OS.git` | `refs/tags/unpublished` | `unpublished` | no distribution tag published（配布 tag 未公開） | distribution tag check moved to current HELIX-HARNESS-OS surface（配布 tag 確認先を現行 HELIX-HARNESS-OS へ移動） | local distribution tag remains v0.1.0 until publish activation（publish activation まで local distribution tag は v0.1.0 のまま） | version-up activation packet required before tag adoption（tag 採用前に version-up activation packet が必要） |
