...
- `<historical-pack-reference>` default branch `main`: `a43771ab091486520a4970f6b19b1663a009d4d0`（latest tag 確認: `v0.1.4`）
...
| distribution_pack_repo | `git ls-remote <historical-pack-reference> refs/heads/main` | `refs/heads/main` | `a43771ab091486520a4970f6b19b1663a009d4d0` | main branch reachable（main branch 到達確認済み） | changed from 2026-07-03 audit; objective audit refreshed（2026-07-03 監査から差分あり、objective audit を再取得済み） | reference source only; version-up activation required before adopting Pack latest（参照 source のみ、Pack latest 採用前に version-up activation が必要） | distribution-version-binding gate retained（配布 version binding gate を維持） |
| distribution_pack_latest_tag | `git ls-remote --tags <historical-pack-reference>` | `refs/tags/v0.1.4` | `v0.1.4` | latest tag reachable（latest tag 到達確認済み） | none（差分なし） | reference source only; local distribution tag remains v0.1.0（参照 source のみ、local distribution tag は v0.1.0 のまま） | version-up activation packet required before tag adoption（tag 採用前に version-up activation packet が必要） |
