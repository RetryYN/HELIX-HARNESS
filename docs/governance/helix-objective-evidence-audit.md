...
- 2026-07-09 時点の `helix vmodel fit --summary-json` では、ZIP 適合の設計側 gate は `design_coverage=pass`、`zip_adoption=complete`、`tailoring=pass`、`function_design_absorption=pass`、`drive_model=pass` であり、残る `needs_fit` は `current_location=needs_recovery` の 1 blocker に収束している。Recovery runway は `machine_actionable_count=0`、`design_reverse_count=0`、`human_approval_count=343`、`next_phase_action=close_ready`、`approval_scope_digest=sha256:2553bf991dea8d5f4bdfebda9bba88f49ff32c1cb8008f1e7f5610c50280895a` で、`.helix/tmp/closure/close_ready-decision-draft.yml` は `pending_human_review` の non-authorizing draft としてのみ扱う。古い `.helix/tmp` log に machine lane が残っていても、正本判断は `helix closure review-bundle --action close_ready --summary-json`、`helix closure transition-plan --action close_ready --summary-json`、`helix vmodel fit --summary-json` の live 出力を優先する。承認なしの `closure apply --execute` は禁止し、承認前の自動作業は read-only review bundle / transition plan / Project view 表示までに限定する。
