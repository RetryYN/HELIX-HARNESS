...
合計は7+5+4+14+5=35、最小7小PRである。これはL3-28〜30の57 caseに必要なdownstream PRを含まない。
全4 manifestのdownstream queueをexactly-onceで採番・再集計するまで、§4.1の10 PRを
「全残存責務」と呼ばず、packetを`review-ready`へ昇格しない。
