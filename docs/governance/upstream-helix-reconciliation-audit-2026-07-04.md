# 上流 HELIX source 突合 audit (2026-07-04)

この audit は、PO 指示（`/goal` 2026-07-04）に基づき
`https://github.com/unison-ai-product/HELIX-HARNESS`（以下「上流」）の全項目を
...
- **上流** `unison-ai-product/HELIX-HARNESS`（public、MIT、TS 100%、612 commits）は
...
1. lint / gate enforcement（G9 / G10 / L14 / D-CONTRACT DSL）の確認
2. root config & adapter surface（CLAUDE/AGENTS/README/.claude/.codex/templates/CI）の確認
...
- 上流の `helix-tl.md` / `helix-status.md` / `helix-test.md` は LOCAL に
...
fork 独立進化のため **LOCAL の現行 `docs/process/modes/` / `L9-system-test-design.md` / `visual-design.md` に （層意味論: canonical L9=integration）
...
- **team prompt provider routing（上流 commit 00a0cd7）** = trivial・確認済。
  ...
  （1 行追加で解消）。
- **distribution asset projection（L7-190）** = 既に保持（ALREADY-HAVE）。
...
- 旧 `src/schema/team.ts:40` `modelOverrideSchema` は **prefix-only** `/^(gpt-|claude-|codex-)/`＋family alias で、
  prefix 以降の任意文字を許容していた。
- `members[].model` → `modelOverrideSchema` → `buildTeamRunPlan()` → `selectTeamModel()` →
  `buildAdapterPlan()` → provider argv の `-m` / `--model` → `buildProviderInvocation()` →
  `spawn()` の経路を確認した。
- Windows で provider command が `.cmd` / `.bat` の場合、adapter は `shell: true` へ切り替えるため、
  model override 文字列の危険文字を schema で落とさないと injection 面になり得る。
- 一次対策として `src/schema/team.ts` に安全な provider model ID pattern と family alias allowlist を追加し、
  provider 風 ID に shell metacharacter を含む値を reject する regression test を `tests/team-schema.test.ts` に追加した。
- 追加対策として `src/runtime/adapter.ts` の Windows command-script shell wrapping 境界で、schema を迂回した
  unsafe argv（`&` / `|` / quote / 改行等）を fail-close するようにした。`tests/model-override-injection.test.ts` と
  `tests/runtime-adapter.test.ts` で、team schema 経路と adapter 直呼び経路の両方を回帰検査する。

**扱い**: 安全境界（コマンド実行）に関わるため、CLAUDE.md のエスカレーション規則に従って PO escalate 済み。
本 audit では schema entrypoint と adapter shell wrapping 境界の実装対策、および targeted test まで完了。
Windows `.cmd` / `.bat` では `shell: true` 自体は互換維持のため残すが、危険 argv は shell 文字列へ畳む前に拒否する。
...
- **model-override injection（§4）** — PO escalate 済み。schema entrypoint と adapter shell wrapping 境界は対策済み。
  残リスクは Windows command-script 実行互換のため `shell: true` を完全撤去していない点。
...
2. §4 の model-override injection は schema entrypoint と adapter shell wrapping 境界を対策済み。
