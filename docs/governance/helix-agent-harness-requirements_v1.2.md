...
| MCP debug / test | MCP Inspector | MCP server の tools/resources/prompts 接続確認、local server smoke | UT-TDD 所有 MCP server または設定済み server profile の推奨 verification tool とする。 |
...
A-124/A-125 により relation graphs、diagrams、MCP/test profiles、evidence は query 可能になる。human reviewers には、canonical UT-TDD documents (concept / planning、requirements、detailed design、PLAN、ADR、test-design documents) を spreadsheet / Excel / PPTX へ変換する手段も必要である。したがって UT-TDD は **canonical document export** を derived artifacts として scope する。
...
**本書は UT-TDD-agent-harness の要件定義書である。構想 (WHY/WHAT) は `helix-agent-harness-concept_v3.1.md` を、各 enum・スクリプト・workflow YAML の実装詳細は将来の個別 PLAN-XXX 詳細設計を参照。**
