...
本書は、active `SourceCaptureReceipt`が示すZIP、前身exact 2 Git authorities、現行HELIXのref/content/edge
各分母を、採用済みcapability件数として誤って合算しないための機械契約である。旧4,470 entry、固定5 ref、
8,935 ref-entryはhistorical seedに限る。sourceを読んだこと、file pathを分類したこと、代表capabilityを数件起票したことは、
...
### §2.2 前身Git authority snapshot

current入力は`unison-ai-product/UT-TDD_AGENT-HARNESS`と`RetryYN/ai-dev-kit-vscode`のexact 2 current
`GitRefAuthorityReceiptV1`である。全advertised heads/tags/pullをA/B一致、exact OID materialize、object/tree/tag peel
検証済みのsealed mirrorからだけatomizeする。ref denominator、unique content denominator、全ref-entry edge denominatorは
receiptから導出し、live件数を本文へ固定しない。

2026-07-16時点のmachine evidenceは
`docs/governance/generated/git-ref-authority-candidates-exact2-v1.json`
（SHA-256=`95e2f00dc08d239cfaa1b9a4e946d0886573e7daa4330a3caff38627b2a01d2f`）にあるが、
これはephemeral sealのcandidate 2件でありtrusted/current receiptは0件である。trusted storage policyとCAS promotionが
未完了の間、offline capture/classificationは0件、coverage creditは0のままとする。

以下の固定5 ref表は2026-07-14 historical seedであり、`authority_reuse_allowed=false`、
`coverage_reuse_allowed=false`である。

当時の観測remoteは`https://github.com/unison-ai-product/UT-TDD_AGENT-HARNESS.git`だった。`origin/HEAD`は
`origin/main`と同じcommitを指すsymbolic aliasなので旧ref分母へ二重算入しなかった。以下の5 refは当時のhistorical source ref集合だった。
...
| full-tree path count | 2,029（core 1,854＋outside 175、交差0） |
| full-tree entry集合digest | `b73bcae478b3c7a26d083ee056db5d999af251920af40d7a83a586ba8db7b518` |
| full-tree分類digest | `44e4f685a945e70ba47cb9d56bb5bcefc4e0b370524a56b8e346c0ddc2676800` |
| path未分類 | 0 |

core 1,854は`src/tests/docs`、outside 175はhook、agent、CI、config、evidence、entrypoint、root supply-chainを
含む非交差partitionである。2,029件すべてをsource entry分母へ含めるが、root ZIP file自体とZIP内部703 entryを
...
前身Gitの各advertised refを独立copyとして数えると共通tree/blobを重複計上する。Git sourceはauthority receiptの
unique tree content集合と全ref-entry edge集合へ分ける。以下のbaseline/overlay規則は各repositoryのdefault refを
探索基準にする場合にも適用し、pull/tag refを省略する根拠にはしない。
...
| 前身Git exact 2 repo | current authority receipt 0/2 | observed=90 ref/145,276 ref-entry edge、authoritative denominator=pending | atomization未開始 | FAIL |
| 現行HELIX | full-tree 2,029 pathを観測 | partition分類2,029/2,029 | 未閉鎖 | FAIL |
