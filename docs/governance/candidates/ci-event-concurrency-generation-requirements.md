...
source_requests: docs/governance/candidates/ci-event-concurrency-generation-requests.md
...
上流: `CIG-BR-01`。下流: `CIG-AC-001`, `CIG-AC-007`。
...
上流: `CIG-BR-01`, `CIG-BR-02`。下流: `CIG-AC-002`, `CIG-AC-003`, `CIG-AC-007`。
...
上流: `CIG-BR-02`, `CIG-BR-03`。下流: `CIG-AC-004`, `CIG-AC-007`。
...
## CIG-R-04 証拠・telemetry・replayの収束

上流: `CIG-BR-03`。下流: `CIG-AC-005`, `CIG-AC-006`, `CIG-AC-007`。
...
- #735の`ci_evidence_generation`を上位互換の内部fieldとして保持し、既存の
  `run:<id>:attempt:<n>:<conclusion>`文字列identityを本候補で再定義しない。
