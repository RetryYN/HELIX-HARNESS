...
この分担は既存候補[要求からの開発ticket導出](development-ticket-derivation-requirements.md)（`DTK-OS-001`「管理が作業分解や駆動tagを先取りしない」、`DTK-OS-005`、`DTK-OS-007`）と同じである。

本書は要求整理だけを行う。WBSエンジンの実装、schema、DB、GitHub連携を実装・起動しない。

## なぜ要るか

- 要求は既に手で扱える量を超えている。既存L2 69件（HARNESS 38、HELIX-OS 31）、旧ルール群由来の要求候補57本、その他の候補群がある。採否後に人がPLANへ割る運転は続かない。
...
- 台帳が無いまま自走すると、AIが会話ごとに作業を切り直し、同じ作業が別identityで重複し、予算と依存が追えない。旧HELIXでPLANが乱立した原因である。
...
| HELIX-OS 推進 | 採否済み要求と制約から作業graph（unit→connection→composite）を生成し、変更の種類と開発styleに合うHARNESS routeを選ぶ | 台帳の直接書換え、自己検収、要求意味の変更 |
...
| WBS-OS-002 | 各作業が安定したidentity、親要求ID、対象product、種類（unit／connection／composite）、依存、並列・直列、scope、予算上限、期限、停止条件、担当候補、V-pair（対応する検証）を持つ | 親要求を持たない作業、依存が循環する作業、予算の無い作業を登録できない。同じ意味の作業を別identityで二重登録しない（`RUL-TKT-01`） |
...
| WBS-HARNESS-001 | 作業単位が持つべき形（V-pair、受入条件、変更の種類、工程順序、停止・差戻し条件）を規範として定め、OSはそれを台帳のschemaへ写す | HARNESSは台帳を運転せず、OSは規範を改変しない |
...
- 旧ルール群との関係：`RUL-TKT-01`／`02`／`03`、`RUL-OSP-06`の規則atom計336件が入力である。
- 採否順序：[対象別L2 source採否順序](../audits/source-rebaseline/l2-source-adoption-sequence.md)の`L2D-S2-02 execution-ticket`と同じ判断単位で扱う候補。
...
2. 開発方式選定：変更の種類は「新規」、正式なL3が止まっている間は`scaffold/`の仮組み（PoC相当）として成立性を確かめる。仮組みはScaffold Bindingに登録し、上流を本書と`RUL-TKT-*`のrevisionに束縛する。
3. 要件定義：仮組みで確かめた入出力を、`HELIXOS-L2`本文への要求（unit／connection／composite）として要求PRで接続する。
