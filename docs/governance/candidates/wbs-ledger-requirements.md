...
2. 開発方式選定：変更の種類は「新規」とする。台帳の形の成立性を確かめる必要がある場合、それはPoCの作業である。PoCを立てる場合、推進はそれをScaffoldと別identityとして立てる。ticketとしての発行形式は`DTK-OS-005`の採否後に決める。PoCの実施は本候補では許可されない（「現在の停止条件」を参照）。PoCの置き場、停止条件、人間判断は、`L2D-S0-02`の採否後に別PRで決める。
...
4. 要件定義：手順2のPoCと手順3を行った場合はその結果を含めて、本書の入出力を`HELIXOS-L2`本文への要求（要求の粒度 unit／connection／composite）として要求PRで接続する。手順2のPoCと手順3の不実施は、本手順の前提を欠かせない。

各段階は別PRで行い、前の段階の合格から次の段階の承認を生成しない。権限を生成しない条件の正本は「現在の停止条件」に置く。手順側の記述はその参照である。
...
- WBSエンジン、台帳schema、DB、GitHub連携を実装・起動しない。この禁止は、PoCを含め本候補の下のすべての作業に及ぶ。この禁止の解除は、`L2D-S0-02`の採否後に別PRで決める（手順2）。
- Scaffoldとして保持する仮組みは別PRで、Scaffold Bindingの下でだけ行う。PoCの成果物はScaffoldではないため、このScaffold Binding限定の対象外である。PoCの置き場と条件は本候補では決めない（手順2）。
