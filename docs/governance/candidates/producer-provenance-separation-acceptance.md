...
|---|---|---|---|
| `PPS-AC-001` | `PPS-R-01` | 4役割を一件ずつ欠落・入替し、external authorをHELIX workerへ偽変換する | 欠落役割を特定し、別役割の値で補完せず拒否する。externalは観測済み外部identityとして保持する |
...
| `PPS-AC-010` | `PPS-R-07` | direct author=committer、mixed dual-receipt、external author、producer不明の旧receiptをdual-readする | direct／mixed／externalは各versioned互換条件で受理し、unknownだけを推測backfillせず履歴保持する |
