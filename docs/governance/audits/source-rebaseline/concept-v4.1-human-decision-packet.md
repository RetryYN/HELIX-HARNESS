...
- HARNESS: L1–L12、正規V-pair、工程のnormative語彙・trigger・適用条件・route内順序・join、単体・接続・構成体を分ける要求エンジン、Design Templateとseed、条件付きResearch／Discovery・PoC／L2↔UI prototype loop／Feature ticketが満たす開発・検証contract、要求形成・合意・freeze・差戻し・完了、検証義務、外部利用条件、複数プロダクトと自己プロジェクトへの適用検証。operational tag、mapping、個別workflow instance、ticket発行は所有しない。
...
各reviewは対象HEADのfindingであり、人間承認ではない。資産明細台帳reviewの所見を受けた時点では、Conceptへ旧test／fixture／oracleと
旧runtime state／evidenceも完全一致再利用しない条件を追加し、そのreview対象内の4対象L1は変更しなかった。その後、
HARNESS要求エンジン、Design Template、管理→推進→ticket生成の責務をConceptと対象別上流へ追加したため、現在のSHAを再固定し、
この差分を最新review対象とする。判断時には
