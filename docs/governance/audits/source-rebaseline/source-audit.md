...
> **旧世代状態の記録:** 本書は2026-09-14時点のhistorical監査証跡である。本文中の「現行」「正本」
> 「canonical」「正規改訂」は、その時点で確認した旧世代文書の状態または当時の是正案を記録する語であり、
> 新世代authority、現行実行指示、採用済み要求を意味しない。
...
HELIX-WebはOSが管理する個別プロダクトであり、固有要求の所属先を`docs/helix-web/`へ分離した。
HARNESSにL2／L11の6要求、OSに9要求、WebにVision由来8要求を設けた。詳細の入口は
[対象別移管状況](l2-source-register.md)を参照する。
...
候補系列の対象と整理状態は[候補要求源の対象別台帳](candidate-source-target-inventory.md)へ記録した。
...
旧世代でCore Readだった要件v1.3は、HARNESSの工程規則とHELIX-OSの実行・管理機構を同一本文に保持している。
[v1.3対象別責務対応](requirements-v1.3-target-crosswalk.md)で節・ID単位の主対象とL2親を整理した。
...
[対象別責務決定](../../../concept/product-boundary.md)へ上位Conceptの正規投影を追加し、HARNESSを外部提供物、
...
候補directoryの97文書は31系列として対象別台帳へ収容した。L2接続済み、再採否待ち、人間承認待ちを区別し、
...
active authority Markdown相対リンク371件の参照切れは0件である。完全一致保持した旧要件本文内の旧相対リンク2件は、byte一致を壊さないためhistorical pathのまま保持し、このactive link判定から除外する。
...
pair-missingとなった。監査資料と是正proposalを`docs/governance/audits/source-rebaseline/`へ移し、相対参照を更新した。
