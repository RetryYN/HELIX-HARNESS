...
| IRUNIT-HIL-BR-24-HELIX-OS | HELIX-OS | none | unresolved | no direct phase/pool | unresolved_no_direct_phase_or_pool | no direct phase／pool; missing design／implementation evidence |

BR23 は Template Gap feedback を HARNESS／OS間の共有候補として保持します。BR24 は要件台帳の設計責務とOS projectionの境界を保持します。BR24-OS の空phase／空poolから asset、phase、実装を補いません。
