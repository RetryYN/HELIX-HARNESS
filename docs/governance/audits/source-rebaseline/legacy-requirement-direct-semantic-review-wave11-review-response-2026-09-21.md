...
Claude exact HEAD reviewではMajor 1件、Minor 1件を検出した。MajorはNFR17の主語がconnectiveへ吸収され、connectiveへ任意の意味句を逃がせた点であり、`subject_scope` atom追加と限定allowlist検査で解消した。Minorはrequirement roleの証拠path比較が条件式の分岐から漏れた点であり、pathとrelationの検査を分離して全roleへ適用した。roleとcatalog kindの束縛も追加した。これらも解消し、現時点の未解消Blocker／Major／Minorは0件である。
