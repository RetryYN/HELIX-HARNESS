...
| `rdp001-outside67-web-webos-l1-anchor-0121` | 旧4対象boundary digestを前提に11 Web/Web-OS L1 row anchorsを現行候補へ接続。boundary input digestが不一致。 | source pathと過去L1 anchorの出典link。現行L1文書は存在するがVision authority状態。owner `SCF-B-0121`; downstream consumerなし. | **決められない**。旧資料と将来像の対応調査を続けるかで研究目的が変わる。Web/Web-OSを要求として扱わない。 PO質問依存: 「昔のWeb/Web-OS将来像の調査を今後どう扱うか」。回答に応じた処置は末尾の質問案に記す。|
| `rdp001-web-webos-vision-asset-semantic-0124` | 旧Vision 35候補×代表旧asset 12件のsemantic connection候補。current product-boundary digestが不一致。 | Vision sourceとlegacy assetのcandidate-only対応関係を保存する。owner `SCF-B-0124`; downstream consumerなし. | **決められない**。旧資料と将来像の対応調査を続けるかで研究目的が変わる。Web/Web-OSを要求として扱わない。 PO質問依存: 「昔のWeb/Web-OS将来像の調査を今後どう扱うか」。回答に応じた処置は末尾の質問案に記す。|
| `rdp001-web-webos-vision-coverage-0091` | 29 parent span / 35 candidate recordのcoverage、旧product-boundary入力、154 connection matrix。current product-boundary digest mismatch。 | Vision source coverage・未接続行のhistoryとして使える。owner `SCF-B-0091`; downstream consumerなし. | **決められない**。旧資料と将来像の対応調査を続けるかで研究目的が変わる。Web/Web-OSを要求として扱わない。 PO質問依存: 「昔のWeb/Web-OS将来像の調査を今後どう扱うか」。回答に応じた処置は末尾の質問案に記す。|
| `rdp001-web-webos-vision-semantic-atoms-0088` | Web/Web-OSの旧Vision 9 spanをatomized/composite候補化。parent `SCF-B-0080` lineage digestが違う。 | 9 Vision spanのsource lineageと、未分解複合行の保全に有用。owner `SCF-B-0088`; downstream consumers `SCF-B-0091`, `SCF-B-0124`. | **決められない**。旧資料と将来像の対応調査を続けるかで研究目的が変わる。Web/Web-OSを要求として扱わない。 PO質問依存: 「昔のWeb/Web-OS将来像の調査を今後どう扱うか」。回答に応じた処置は末尾の質問案に記す。|
| `rdp001-web-webos-vision-source-0080` | 旧Vision 29 spanを現行Web L2 9件/Web-OS L2 6件へのrequirement source relation候補として固定。現行source status/relationが一致せず`E_CURRENT_SOURCE`。 | 旧Visionから現行文書への起源追跡。owner `SCF-B-0080`; downstream consumers `SCF-B-0081`, `SCF-B-0084`, `SCF-B-0088`, `SCF-B-0091`, `SCF-B-0124`. | **決められない**。旧資料と将来像の対応調査を続けるかで研究目的が変わる。Web/Web-OSを要求として扱わない。 PO質問依存: 「昔のWeb/Web-OS将来像の調査を今後どう扱うか」。回答に応じた処置は末尾の質問案に記す。|
...
- **昔の配備調査**：配備とは、変更を利用環境へ反映する作業を指す。旧HELIXには配備に関係する資料を集めた調査がある。今後のHELIXでも、配備に関する要求を考えるためにこの調査を使いたいか。それとも過去の研究記録として残せば十分か。使いたい場合は、旧assetの出典を保ちつつ現在の対象範囲に対するsource調査へ作り直す。記録として十分な場合は、配備資料の選定・不足・追跡を調べる各検査を退役候補として評価する。現行の配備要求があることをこの記録から推定しない。
