...
HARNESS自身が製品非依存の入力・出力・consumer再検証契約を持ち、HELIX-OSなしでも利用できる。
HELIX内部ではFT-OS-REQREG-001を先に実装してengine入出力を管理へ登録するが、これはdelivery sequenceであって
HARNESS製品の機能依存ではない。意味分類のOS側projectionはFT-OS-REQCLASS-001が所有する。
...
- Pythonは決定論的な意味処理を行い、versioned strict machine contractでproposalだけを返す。wire formatはL3で選定する。
...
- HARNESSのconsumer contractがschema、revision、stale、duplicate、prohibited payloadの再検証条件を定める。外部作用境界の技術はL3で選定する。
- 製品固有の語彙、質問、policy、品質matrixはversioned packとしてcoreから分離する。

## 降下順序

1. HARNESS L1-008／L2-008とL11を承認revisionへ束縛する。
2. FT-HARNESS-SEMEXTRACT-001で旧Requirement Engine、Requirement Compiler、discovery、trace、impact資産からsemantic behavior atomを個別採否する。
3. L3でsemantic contract、schema、pack、resource boundary、determinism、consumer再検証contractを定義する。
4. L10で欠落、過剰抽出、対象違い、粒度混在、構成体成立の誤推定、stale、malformed outputを検証する。
5. Python coreとOS非依存のconsumer contractを実装する。
6. HELIX内部のdelivery sequenceとしてFT-OS-REQREG-001へ接続し、FT-OS-REQCLASS-001へversioned分類結果を投影する。
7. HELIX自身と性質の異なる複数productで評価し、指示との差、訂正率、見逃し、過剰抽出、質問量、trace欠落を再観測する。

## 停止条件

- 親Concept／L1／L2またはHARNESS semantic／consumer contractが未承認・未freeze。
...
現在はticket発行と要求具体化だけを行い、Python実装、外部作用境界との接続、学習、runtime、CIを起動しない。
