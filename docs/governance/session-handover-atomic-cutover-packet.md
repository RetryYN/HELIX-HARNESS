...
実装完了をcutover承認と読み替えない。`approval.status=approved`と承認scope digestが記録されるまで、
...
`status=blocked_pending_po_confirmation`。本packetはdry-run実行計画であり、承認recordではない。
旧surfaceのmutation、enforce pin有効化、archive source削除は未実施である。
