...
| HIL-BR-17 | HIA-BR-017 | Claudeのcurrent contract内局所findingと独立改善findingがある | dispositionとfinding promotionを実行する | `current_pr_fix`はwriter returnだけ、`successor_issue`はIssue、Reverse、memory要約、Codex queueが同一causalityで生成される | `HIL_FINDING_DISPOSITION_INVALID`, `HIL_FINDING_DROPPED` | HOT-HIL-29; HST-HIL-005 | FindingDispositionGate; FindingPromotionPipeline | draft-defined | not-implemented |
...
| HIL-FR-30 | HIA-FR-030 | `current_pr_fix`/`successor_issue` findingと各途中欠落fixtureがある | dispositionとpromotion transactionを実行する | `current_pr_fix`はpromotion 0、`successor_issue`は全成果を原子的に生成し欠落時はreadyを作らない | `HIL_FINDING_DISPOSITION_INVALID`, `HIL_FINDING_PROMOTION_PARTIAL` | HOT-HIL-29; HST-HIL-005 | FindingDispositionGate; FindingPromotionPipeline | draft-defined | not-implemented |
