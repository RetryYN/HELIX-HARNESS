...
`design_status=draft-defined`はL1、L4、L14の設計pointerが存在することだけを表す。
...
| HIL-BR-14 | HIA-BR-014 | ZIP、旧UT全branch、現行HELIXのsnapshotがある | source coverageを判定する | atomic behavior未分類またはtrace孤児が一件でもあれば拒否 | `HIL_SOURCE_AGGREGATE_ONLY` | HOT-HIL-37; HST-HIL-011 | SourceCapabilityAtomizer; CapabilityCoverageGate | draft-defined | not-implemented |
...
| HIL-BR-25 | HIA-BR-025 | L0–L14 ledger、上下edge、正規V-pair fixtureがある | Layer Ledger Chainの工程完了を要求する | 全原子的obligationが上下・左右双方向へ閉じた場合だけ完了する | `HIL_LAYER_LEDGER_CHAIN_INCOMPLETE` | HOT-HIL-44; HST-HIL-030 | LayerLedgerRegistry; LedgerPairGate | draft-defined | not-implemented |
...
| HIL-FR-16 | HIA-FR-016 | 三source familyの固定manifestがある | asset inventoryを実行する | 全atomic capabilityに一件の採否と根拠がありpendingがゼロになる | `HIL_ASSET_DECISION_MISSING` | HOT-HIL-10; HST-HIL-011 | AssetInventory | draft-defined | not-implemented |
...
| HIL-FR-21 | HIA-FR-021 | ZIP entry、remote ref、現行symbolを変更する | snapshotを再取得する | manifest digestが変わり旧coverage receiptがstaleになる | `HIL_SOURCE_SNAPSHOT_STALE` | HOT-HIL-20; HST-HIL-011 | SourceSnapshotter | draft-defined | not-implemented |
...
| HIL-FR-46 | HIA-FR-046 | L0–L14のledger type、粒度、authority、entry/exit fixtureがある | Layer Ledger Registryへ登録してsnapshotを作る | 全layerの型と原子的row revisionを決定的に登録し欠落typeを拒否する | `HIL_LAYER_LEDGER_TYPE_MISSING` | HOT-HIL-44; HST-HIL-030 | LayerLedgerRegistry | draft-defined | not-implemented |
...
|---|---:|---:|---|---|
| BR | 25 | 25 | draft-defined 25件 | not-implemented 25件 |
| FR | 50 | 50 | draft-defined 50件 | not-implemented 50件 |
| TR | 11 | 11 | draft-defined 11件 | not-implemented 11件 |
| NFR | 29 | 29 | draft-defined 29件 | not-implemented 29件 |
| **total** | **115** | **115** | **draft-defined 115件** | **not-implemented 115件** |

次の全条件を満たすまで本台帳を`verified`へ昇格しない。

1. 115 requirement IDと115 assertion IDが一意で、L1の現在集合と完全一致する。
