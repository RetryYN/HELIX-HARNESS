...
`helix memory list harness --json`を安全作業ツリーと共有元worktreeの双方で取得し、後着したworker runtime調査も含むID和集合 **39件**を分母とした。`project` layerは0件だった。本文、key、ID、provenanceを読み、次のいずれかへexactly oneで分類した。
...
| 39 | `worker-runtime-research-2026-07-19` | backfilled | L1 `HBR-P2/HBR-P6`、L3 `HR-FR-P2-05..08`／`HR-FR-P6-06`、requirements §4.10、`PLAN-DISCOVERY-12`へ6指示を粒度別に追突。Python semantic coreと外部AI workerを分離し、bulk importしない |
...
- 分岐間の和集合: 後着1件を含む39件すべてが本台帳の行1〜39にexactly oneで存在する。
- 最終観測: 安全作業ツリーと共有元worktreeはいずれもactive 0／damaged 0。後着した`worker-runtime-research-2026-07-19`は初回authority receiptで正本targetを検証してretireし、共有元はcompaction後37件すべてbody空となった。現行authorityは`source_revision=requirements-reconciliation-2026-07-19-v3`と全target content digestから導出する`sha256:` IDへ再束縛し、同じ39件を再検証した。
- 永続不変条件: `helix memory retire harness <ids...> --consumer <id> --authority <id>`は、追突targetを検証するauthority receipt、fencing、冪等性を必須とし、legacy readerもv2 terminal receiptをactive memoryとして再表示しない。
