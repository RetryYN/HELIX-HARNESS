...
- 製品属性は機構別に`mechanism_product_attributes`へ記録する。HARNESSとWebのみ`製品`、OS・Web-OS・BRAIN・LABO・Intelligence・Securityは`非製品`、CONNECTは`共通部品`である。旧Runner／Sandbox（共通部品）は、2026-09-26のPO判断でWorkerへ統一した。Workerは機構でも共通部品でもない（レーンの主がSubagentとして呼び出して作業させるモデル）ため、`mechanism_candidate`では作業の実行の担い手として`Worker`と書く（[判断記録](../decisions/worker-execution-model-po-decisions-2026-09-26.md)）。HARNESS内のサービス①〜⑦／入口／枠／部品／コアは独立した`harness_placement`へ記録し、製品属性と混ぜない。複数機構の行は単一ownerの意味ではない。
