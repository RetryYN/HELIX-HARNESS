...
- IRは[carry-forward台帳](../legacy-migration/requirement/legacy-requirement-carry-forward.jsonl)の原ID・revision・原文・digest・source pointerを転記し、`target_assessment`を責務候補として扱った。既存のroutingはsuccessor確定ではない。HARNESS・OSのL2候補22件はdraft、DTKはdraft candidate、L1-COVは監査work unitであり要求IDではない。Web・WEB-OSの旧L2候補15件は、2026-09-24のPO判断によりVision材料へ分類され、要求・要件として数えない。
- 製品属性は機構別に`mechanism_product_attributes`へ記録する。HARNESSとWebのみ`製品`、OS・WEB-OS・BRAIN・LABO・Intelligence・Securityは`非製品`、CONNECTは`共通部品`である。旧Runner／Sandbox（共通部品）は、2026-09-26のPO判断でWorkerへ統一した。Workerは機構でも共通部品でもない（レーンの主がSubagentとして呼び出して作業させるモデル）ため、`mechanism_candidate`では作業の実行の担い手として`Worker`と書き、`mechanism_product_attributes`の`Worker`には、機構・共通部品ではないことを書く。`original_text`は原文として書き換えない。旧OS Contract Runner（HIL-FR-34、HIL-NFR-09／19、HIL-TR-04／05）は、検査の実行をWorker、検査の契約の意味をHELIX-HARNESSとして両方を候補にする（[判断記録](../decisions/worker-execution-model-po-decisions-2026-09-26.md)）。HARNESS内のサービス①〜⑦／入口／枠／部品／コアは独立した`harness_placement`へ記録し、製品属性と混ぜない。複数機構の行は単一ownerの意味ではない。2026-09-26のPO判断でConceptに加えたHELIX-INFRASTRUCTURE（HELIX自身の実行環境の資源と状態）は`非製品`の機構であり、1.0から加わる（[判断記録](../decisions/infrastructure-concept-placement-po-decisions-2026-09-26.md)）。本表の既存行の`mechanism_candidate`と`mechanism_product_attributes`には、まだHELIX-INFRASTRUCTUREを加えていない。実際の資源に関わる行（例：HIL-FR-64のWorkerの実行環境、仮ID-BASE-05の隔離の単位の「資源」）へ加えるかは、各行の担当の見直しで扱う。
...
- 現行L2分類台帳でHELIXOS-L2-012／013に使う`labo_technical_research`と`labo_cross_diagnostic`は、[分類語彙表](../legacy-migration/ir/legacy-ir-structure-classification.md)にないLABO用の暫定値である。LABO向けの確定分類語彙がまだ無いため、行の`layer_primary_provisional`と注記で暫定と理由を示す。
- 2026-09-26にPOが示した「HELIX-Web製品群 要求原案」により、HELIX-Webは製品の総称になった（[判断記録](../decisions/helix-web-product-group-po-decisions-2026-09-26.md)）。原案の要求（`HELIXWEB-L2-010`以降、`HELIXWEBOS-L2-007`以降、WEB-HARNESSの7製品・CORE・CONNECTOR、LABOの`HELIXLABO-L2-WEB-`）は要求候補・未採択であり、本表の行に加えない。本表のWeb・WEB-OSの15行（001〜009、001〜006）の意味と所属は変えない。`mechanism_product_attributes`の`HELIX-Web-OS`等の名前は、機械参照のkeyとして今の表記を保つ。
