...
|---|---|---|---|
| 外の開発の記録を分解してBRAINへ入れる（Conceptの2.0） | 構成体 | 外の情報を集める、LABOが分解して評価する、BRAINが構造として蓄える、の一連の処理を1つの構成体要求にする。途中の受け渡しは接続要求として持つ | 何を「外の情報」とするか。Conceptの表は「OSS、設計資料、Issue、PR、文書、研究」と書くが、POは「外部の開発データが何を指しているのか分からない」と述べた |
| LABOの技術調査とIntelligenceのクローラー | 構成体（上と同じ一連の処理の入口） | クローラーが集め、LABOが調べる流れを、上の構成体要求の一部として扱う。担当の関係を単独で決めない | 上と同じ |
