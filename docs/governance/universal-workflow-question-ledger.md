...
## §1 Conditional questions（27/27） （日本語の契約見出し）
...
## §2 Base questions（24/24） （日本語の契約見出し）
...
## §3 Runtime orchestration questions（25/25） （日本語の契約見出し）
...
| exact normalized duplicate | 0 | PASS | （日本語の機械契約記述）
| answered / N/A / deferred / unresolved / rejected | 12 / 0 / 40 / 24 / 0 | OPEN | （日本語の機械契約記述）
| source blob/value digest binding | 76/76 | PASS（answer authorityとは別） |
| requirement/AC/test edge | 0/76 | FAIL | （日本語の機械契約記述）
...
| metric | count / state | freeze verdict | （日本語の機械契約記述）
|---|---:|---|
...
| queue disposition | deferred=40、unresolved=24 | answered by queue 0 | （日本語の機械契約記述）
| migration / queue coverage credit | false / false | FAIL | （日本語の機械契約記述）
...
### §5.3 PO7 activation machine contract （日本語の契約見出し）
