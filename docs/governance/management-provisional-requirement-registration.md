...
現registerは31 revisionを持ち、十三のsource集合を十三の生存中`registered_source_holding`として保持する。
このうち十二は既に全量照合済みである。十三番目（`MPR-SH-LEGACY-RULE-003`。`-001`／`-002`を訂正した生存中revision）は旧HELIXのAI向け指示・運用文書・機械強制codeから抽出した規則atom 7,622件で、対象file 632件のうちatomを得た533件の531件に二巡目を行った。二巡目でも新規が出ており、全量照合済みではなく「発見済み」の集合として保持する。atomの要求への対応づけは候補であり、holdingの保全対象は原文と出どころだけである。初回6 revisionのactor帰属と、続く7 revisionの記録時点は、原行を残した
