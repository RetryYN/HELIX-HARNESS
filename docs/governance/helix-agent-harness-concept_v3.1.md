...
UT-TDD-agent-harness は **個人開発を AI に委譲する際に発生する全実装パターン** を 3 つの経路で網羅し、4 つの補助軸で支える（旧 framing「チーム開発で発生する全実装パターン」を HELIX solo 化 PLAN-L1-06 で改訂。経路/補助軸の機械機構は不変、複数人前提のみ solo+AI roster へ）。
...
**本書は UT-TDD-agent-harness の概念定義書である。受入条件・enum 詳細・Phase 0 (初期フェーズ) 受入条件は `helix-agent-harness-requirements_v1.2.md` を参照。実装詳細 (validator / workflow YAML / hook script) は将来の個別 PLAN-XXX で詳細設計する。**
