...
本書は **構想書 (concept document)** として、**なぜ / 何を / どう繋がるか** のみを定義する。**どう実現するか** は別ファイル `helix-agent-harness-requirements_v1.2.md` (要件定義書) で定義し、さらに各 enum・スクリプト・workflow YAML の **詳細実装** は L5 詳細設計 PLAN で詰める。
...
# §1 Why — なぜ HELIX Agent Harness か
...
`HELIX Agent Harness` はこの 4 問題に **3 つの実装経路 + 4 つの補助軸** で構造的に対処する。
...
├── helix-agent-harness-concept_v3.1.md
└── helix-agent-harness-requirements_v1.2.md
```

## 1.4 失敗を仕組みに変換する原則

HELIX Agent Harness は、失敗事例を隠すための管理ツールではない。AI 開発で実際に起きる失敗を観測し、次回以降の実行品質を上げるための **再利用可能な制御構造** に変換するための harness である。
...
HELIX Agent Harness は **個人開発を AI に委譲する際に発生する全実装パターン** を 3 つの経路で網羅し、4 つの補助軸で支える（旧 framing「チーム開発で発生する全実装パターン」を HELIX solo 化 PLAN-L1-06 で改訂。経路/補助軸の機械機構は不変、複数人前提のみ solo+AI roster へ）。
...
| **UT-TDD W (2段V)** | L0 (全体 1 回) → **フェーズ 1: L1-L9 (一般システム、drive=be/fe/db)** + **フェーズ 2: L1-L9 (エージェント昇華、drive=agent)** → **フェーズ 3: L10-L14 で合流** | **AI エージェントシステムを作る場合のみ** |
...
|---|---|---|
| **ユビキタス言語 SSoT** | L0 企画書 §Glossary (UT-TDD では `docs/governance/helix-agent-harness-concept_v3.1.md §10 用語集`) | L1 sub-doc は L0 用語を **parent_doc reference** とし、独自定義禁止 (anti-corruption layer) |
...
R4 で Gap を整理した後、HELIX Agent Harness の Forward 経路に接続。R4 outcome に応じて L1 (要求定義) / L3 (要件定義 FR+AC) / L4 (基本設計) / L5 (詳細設計) / gap-only (差分集約のみ、新規層なし) のどこに合流するかを決める (L0-L14 体系。旧 L2/L3 = 全体設計/詳細設計 は L4/L5 に remap。要件定義書 §3.4)。
...
- `helix-agent-harness-concept_v3.1.md` (本書)
- `helix-agent-harness-requirements_v1.2.md` (要件定義書)
...
**本書は HELIX Agent Harness の概念定義書である。受入条件・enum 詳細・Phase 0 (初期フェーズ) 受入条件は `helix-agent-harness-requirements_v1.2.md` を参照。実装詳細 (validator / workflow YAML / hook script) は将来の個別 PLAN-XXX で詳細設計する。**
