...
| C-18 | The meaning-based feature-list answer is now hard-gated against live status/handover frontier records. | proved | `src/lint/semantic-frontier-consistency.ts`, `src/lint/outstanding.ts`, `tests/semantic-frontier-consistency.test.ts`, `tests/doctor.test.ts`, `bun run src/cli.ts doctor` | `design_bottomup_mode`, `asset_progress_visualization`, `serverless_readonly_share`, and `name_cutover` must appear both in L3 §0.2 and in live `semanticFeatureFrontierRecords[]` with matching classification, `completionClaimAllowed=false`, non-empty route, and `sourcePaths[]` pointing back to the L3 feature-list source. If the feature list and blockers diverge, including extra live records not listed in L3, doctor fails instead of relying on prose rereads. |
...
## 2026-07-02 Hard-Gate Addendum

The 2026-07-01 re-read is no longer only a prose answer. `semantic-frontier-consistency`
now compares the L3 §0.2 meaning-based feature list against live
`outstanding.semanticFeatureFrontierRecords[]` from status/handover state. The
gate requires:
...
- every record keeps `completionClaimAllowed=false`
- every record cites `docs/design/helix/L3-requirements/pillar-functional-requirements.md`
  in `sourcePaths[]`

This is the current mechanical answer to "機能一覧は本当に合っているのか": the
feature list is allowed to be considered aligned only while the live blockers
carry the same meaning units and classifications.
...
| 機能一覧は本当に合っているのか | The feature list is correct only when read as confirmed 43 items plus explicit frontiers: visualization S4, rename cutover approval, version-up parked work, and runtime/product hardening. Pair-agent and setup/rename are aligned with their current approval boundaries. C-18 now hard-gates this answer against live `semanticFeatureFrontierRecords[]`, so a prose-only feature list cannot silently drift away from status/handover blockers. | `pillar-functional-requirements.md` §0.2 / `pillar-function-design.md` §0.1 / C-17 / C-18 |
