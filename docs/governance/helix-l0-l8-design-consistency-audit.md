...
S4 判断待ちの visualization、future version に parked された serverless sharing、L14 の `.ut-tdd` -> `.helix`
不可逆 cutover は未完了 frontier として残す。

`.ut-tdd` / `ut-tdd` / `area=harness` の残存は意図的な凍結状態である。`bun run src/cli.ts rename plan --json`
...
| C-08 | L8/G8 workflow は selected integration coverage として executable closure を持つが、universal product completion ではない。 | proved | `docs/test-design/harness/L8-integration-test-design.md`, `tests/g8-integration-workflow.test.ts`, `.ut-tdd/evidence/g8-integration/` | `g8-integration-workflow - OK` は selected G8 workflow profile の close。 |
...
| Pair-agent evidence and DB convergence | `--save-evidence` は `.ut-tdd/evidence/pair-agent/*.json` を書き、DB rebuild は `model_runs`、`gate_runs`、`guardrail_decisions` へ投影する。 | `src/cli.ts`, `src/state-db/projection-writer.ts`, `tests/projection-writer.test.ts` | traceability を支える。projection-only rows だけでは runtime claim を close しない。 |
| Setup and HELIX command naming | current command は `ut-tdd setup project`。`helix setup project` は future target であり PLAN-M-02 approval 前は unavailable。 | `docs/design/helix/L3-requirements/pillar-functional-requirements.md`, `docs/design/helix/L6-function-design/pillar-function-design.md`, `tests/setup.test.ts`, `tests/cli-surface.test.ts` | setup success を `.ut-tdd -> .helix` rename completion と誤読しない。 |
...
- Identifier rename: `.ut-tdd` -> `.helix` は L14 frontier。rename plan の `mustNotApply=true` を尊重し、
...
| 全部終わっているのか | いいえ。whole-program completion は blocked。S4 decisions、parked version-up work、approval-gated cutover が残る。doctor green や selected L0-L8 evidence は full completion claim ではない。 | `ut-tdd handover status --json` / completion readiness |
