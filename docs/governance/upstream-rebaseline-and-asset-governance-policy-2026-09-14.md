...
- 旧Requirement IR 153件を原文・digest付きで全件保持し、確認済み意味差分とauthority語彙差分を対象別L2へ無損失で再配置する。保持・再配置、意味変更、retireを別decision種別として、すべての要求PRを対象revision付きの人間判断へ送り、その後に新世代projectionを再導出する。
...
JSON-only authorityや旧ownerを自動継承せず、要求意味を保持したまま新世代ownerへ再配置する単位を管理する。
