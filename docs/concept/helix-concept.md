...
|---|---|---|---|---|---|
| 1.0 | HARNESS Version 1の完成。要求から設計・実装・検証・統合・リリース・運用・保守まで通し、変更を適切な地点へ戻せる。HELIX自身の改善も同じ経路で行う | Web展開を始めるための開発基盤と提供package | 定型保守を開くための基礎 | HARNESS、OS、BRAIN、LABO、Security、Runner／Sandbox、CONNECT | 性質の異なる複数の製品とHELIX自身で、要求から受入・運用評価まで成立する。部品の存在や文書だけで完成としない |
| 1.x | 実案件でpackageと並行開発を磨く | Connector型AI開発SaaS。利用者のPC・WSL・VPS・リポジトリをWebから操作する | 限定された更新・診断・復旧 | Web、Web-OS | 利用者環境で接続・job・切断・取消・再開が成立する。開発engineをWeb用に別実装しない |
| 2.0 | 外部の開発データ（OSS、設計資料、Issue、PR、文書、研究）を分解し、今の案件へ根拠付きで推薦する | 技術・設計・改修の根拠付き推薦を提供する | 影響と選択肢を見て変更を判断する | なし（BRAINを外部データの推薦へ、LABOを推薦の効果の評価へ拡張） | 推薦から出典・版・根拠へ戻れる。適用不能や保留も返せる。外部の成功を自分の環境の成功にすり替えない |
| 3.0 | HELIXの開発データで、ローカルLLMを学習・チューニング・評価する | HELIX-Web 3：HDA。調整済みモデルを分散サーバーから呼び出して開発を補助する | 専門AIと保守・改修を進める | Intelligence | どのデータと設定から生まれたモデルかを追える。学習前と比べ、弱点・適用範囲・撤回先を持つ |
| 4.0 | 調整したモデルを判断能力に組み込み、案件ごとに開発フローを組み立て、途中結果で再計画する | 目的・計画・根拠・進行をWebから扱う | 要望から改修手順を組ませ、実行を任せる | なし（BRAINがIntelligenceの調整済みモデルで開発フローを組むよう拡張） | 未知の案件でも必要な義務を落とさない計画を作れる。推論が誤っても、権限・品質・安全の検査で実行を止められる |
...
flowchart LR
    ...
    subgraph V20["2.0 根拠付きの推薦"]
        B2["BRAINの拡張<br/>外部の開発データから推薦"]
        L2["LABOの拡張<br/>推薦の効果と退行の評価"]
    end
    subgraph V30["3.0 学習"]
        I3["HELIX-Intelligence"]
    end
    subgraph V40["4.0 動的な開発フロー"]
        B4["BRAINの拡張<br/>調整済みモデルで開発フローを組む"]
    end
...
BRAINは1.0からあり、既存の外部モデルで理解・計画・設計判断・レビュー・診断を行う。2.0で外部データの推薦へ、4.0でIntelligenceの調整済みモデルによる開発フローの組み立てへ広がる。
...
| 後から加わる機構の受け口 | 機構どうしの入出力を接続契約で定め、後から加わる機構が使う入出力（評価用の計測、学習用の記録など）も1.0から記録しておく | 3.0のIntelligence |
...
HARNESSは開発PackageをRelease Packにして外部の利用者へ、開発能力はWebを通じて顧客へ提供する。どちらの利用結果も、許可された範囲で改善へ戻す。
...
flowchart LR
    ...
    PKG["HELIX-HARNESS 《製品》<br/>開発Package群 → Release Pack"]
...
HARNESSは、開発の進め方そのものを製品にしたものである。対象製品の言語、AIのprovider、CI製品、画面の方式に依存せず、次の構成要素を提供する。

| 構成要素 | 中身 |
|---|---|
| 工程モデル | 企画（L1）から運用評価（L12）までの層と、V字の対。Full V、Production Scrum、V設計＋Scrum実装、別軸のDiscovery／PoCから、対象に合う進め方を選ぶ |
| 工程の条件 | 各工程の開始・凍結・差戻し・再開・完了の条件。実行が成功しただけでは工程を完了にしない |
| 要求エンジン | 企画と利用者の指示から、要求候補、質問、矛盾、欠落、過剰解釈、変更の影響を示し、人の訂正で要求へ収束させる。単体・接続・構成体の粒度を分ける |
| Design Template | 要求の種類、構成、risk、domainから設計義務を導く版付きのtemplate。足りない要求入力は、推測で埋めず上流へ質問として戻す |
| 追跡と検証の契約 | 要求から設計・テストへの対応、変更したときの再検証範囲、検証義務と証拠の条件、利用者受入と運用評価の条件 |
| workflowの語彙 | 工程の語彙と意味、開始条件、順序、合流、停止・差戻し・完了の条件。個別のticketやworkflowの実体はOSの推進が作る |
| 提供構成 | 利用者が選んだ機能の版、依存、導入、更新、復旧の条件 |

- 利用者は必要なPackageだけを選び、明示された依存とともに導入する。
...
HARNESSは、開発責務ごとの**開発Package**の集まりとして提供する。開発Packageは、利用者が一つの開発責務を実行し、使える成果を受け取れる道具一式である。
利用者は必要なPackageだけを選び、必要最小の基盤と合わせ、版・内容・検収の証拠と結びつけた**Release Pack**として受け取る。
Packageの候補は次の13である。個数や構成は固定せず、採否と構成はHELIX-HARNESSの製品定義で決める。

| 区分 | Package | 利用者が受け取る成果 |
|---|---|---|
| 上流 | D01 要求形成・要件定義 | 要求候補、判断根拠、受入条件 |
| 上流 | D02 構造・UI/UX設計 | 構造・契約・画面の設計と、先に決める検証義務 |
| 開発 | D03 開発計画・進行統制 | 実行計画、割当、停止理由、再開先 |
| 開発 | D04 Worker実行・成果生成 | 候補のコード・文書・成果物と実行記録 |
| 開発 | D05 検証・品質保証 | 検証計画、独立review、品質の証拠 |
| 開発 | D06 CI計画・実行最適化 | 必須検証を保ったCIの計画・結果・診断 |
| 開発 | D07 開発安全・隔離実行 | 操作の認可、隔離実行、安全の証拠 |
| 開発 | D08 変更統合・PR管理 | 統合結果と対象HEADの収束証拠 |
| 提供 | D09 製品リリース | 検証済みの配布物、manifest、更新条件 |
| 提供 | D10 配備・環境適用 | 配備の計画と記録、health、切戻し条件 |
| 運用 | D11 運用・保守 | 稼働の観測、保守義務、改修への受渡し |
| 運用 | D12 障害診断・回復 | 原因候補、反証条件、回復条件 |
| 運用 | D13 構造改善・リファクタリング | 意味を保った再編案、移行・退役の証拠 |

- Packageは利用者から見た提供単位である。コードの所有単位、検証・更新の単位、実際に渡す配布物とは別に扱い、一本の階層に潰さない。
- 依存をゼロにすることは求めないが、必要な安全機能を外して小さく見せることはしない。
- Version 1に含めるPackageは、1.0の受入基準を決めるときに定める。
