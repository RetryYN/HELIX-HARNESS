...
flowchart LR
    ...
    PKG["HELIX-HARNESS 《製品》<br/>開発Package D01〜D13 → Release Pack"]
...
HARNESSは一つの塊として提供しない。利用者が一つの開発責務を実行し、使える成果を受け取れる道具一式を**開発Package**とし、
利用者は必要なPackageだけを選ぶ。選んだPackageと必要最小の基盤を、版・内容・検収証拠と結びつけて**Release Pack**として渡す。
Package候補は次の13であり、個数は固定しない。
...
- 利用者がCIだけを使いたい場合に、HELIX全体の統制や成長の仕組みまで導入させない。HELIX自身を育てる部分（全体統制、学習、LABO、Intelligence）はPackageに同梱しない。
- 依存をゼロにすることは求めないが、必要な安全機能を外して小さく見せることはしない。
- 配布する道具（HARNESS）、道具で作る製品、将来提供する学習済みモデルは、別の成果物として扱う。
- Version 1の完成範囲にどのPackageを含めるかは、1.0の受入基準を決めるときに定める。
