...
HARNESSは必要な製品を選べる形で外部の利用者へ、開発能力はWebを通じて顧客へ提供する。どちらの利用結果も、許可された範囲で改善へ戻す。
...
flowchart LR
    ...
    PKG["HELIX-HARNESS 《製品》<br/>工程・要求・設計・検証の製品群"]
...
HARNESSは、開発の進め方そのものを製品にしたものである。利用者は、企画から運用評価までを一貫したV-modelの契約として扱える。
...
| 製品 | 利用者ができること |
|---|---|
| 工程モデル | 企画（L1）から運用評価（L12）までの層とV字の対で、上流の意図から運用評価までを構成する |
| 開発styleと進行条件 | 対象と変更に合う進め方（Full V、Production Scrum、V設計＋Scrum実装、別軸のDiscovery／PoC）を選び、要求形成・prototype・合意・凍結・差戻し・再開・完了を別々の条件として確かめる |
| 変更影響の追跡 | 要求の変更を設計・実装・検証へ伝え、つながっていない箇所と古くなった箇所を確かめる |
| 検証義務と証拠 | 対象の版とriskに合う検証義務、反例、証拠、差戻しの条件を定める |
| 要求エンジン | 指示と根拠から要求候補を作り、欠落・矛盾・過剰解釈・変更の影響を確かめて、合意した要求へ収束させる |
| Design Template | 要求の種類と構成に合う設計templateから設計義務を導き、足りない要求入力を上流へ戻す |
| 提供構成 | HELIX内部の管理機構に頼らず、明示された版・構成・条件でHARNESSを導入・更新・復旧する |

- 工程の語彙と意味、開始条件、順序、合流、停止・差戻し・完了の条件もHARNESSが持つ。個別のticketやworkflowの実体はOSの推進が作る。
- Worker実行、操作の認可と隔離、配備、運用、障害診断の実行はHARNESSの製品に含めない。これらはOSや各製品が持ち、HARNESSはその条件と契約だけを定める。
- Worker pool、HELIXの内部memory、運用DB、CIの運転、学習の履歴を、利用者の必須構成にしない。HELIX自身を育てる部分（全体統制、学習、LABO、Intelligence）は同梱しない。
- 利用者は必要な製品だけを選び、明示された依存とともに導入する。提供単位の名前と個数は、製品群から導く。
