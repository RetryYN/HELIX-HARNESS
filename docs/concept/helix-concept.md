...
HELIX-HARNESSは、個別にリリースできるHARNESSの集合である。利用者は欲しい成果物を選び、そのHARNESSだけを導入する。
各HARNESSは要求・設計・実装・テスト・検証を一式持ち、互いに密結合させない。密結合にすると、単独でリリースできなくなるためである。
...
| コア | 要求・設計・実装・テストの追跡、依存、古くなった箇所の検出、JSONの意味正本、Pythonの意味コア、Node／TSのテスト、CIの仕組み。すべてのリリース単位を横断して成立させる |
