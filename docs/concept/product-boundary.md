...
HARNESS自身の要求・設計・実装・検証へ戻す。HELIX-OSはPMにあたり、この自己適用・改善loopを回して（LABOが評価して返す改善の提案の登録・振り分け・ticket化と実行統制）、HARNESSを
改善し続ける責務を持つ。改善の評価と提案はLABOがPMOとして担う（[2026-09-26 PO判断](../governance/decisions/handoff-integration-po-decisions-2026-09-26.md)）。管理対象と規則の参照関係を、同一の要求所有へ潰さない。
