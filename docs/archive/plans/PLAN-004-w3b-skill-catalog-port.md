...
generates:
  - artifact_path: src/helix/skill/__init__.py
    artifact_type: python_module
  - artifact_path: src/helix/skill/catalog.py
    artifact_type: python_module
  - artifact_path: src/helix/skill/suggest.py
    artifact_type: python_module
  - artifact_path: src/helix/tests/test_skill_catalog.py
    artifact_type: test_code
  - artifact_path: src/helix/tests/test_skill_suggest.py
    artifact_type: test_code
dependencies:
  parent: null
  requires:
...
HELIX `helix skill suggest` (§7.2) の母集団を作る rule-based catalog engine を確立する:

- `src/helix/skill/catalog.py`: `docs/skills/*.md` (§7.2 仕様 path) を walk して frontmatter (YAML、W1 で使用済の PyYAML) から catalog dict を生成
- `src/helix/skill/suggest.py`: §7.2 `helix skill suggest` JSON contract (`{required:[], optional:[], missing:[]}`) を rule-based matching (triggers + verification + helix_layer) で実装
...
- vendor `command_mapper.derive_commands` は HELIX command 名 (`helix-codex` / `helix-claude` 等) に直結。HELIX は `helix` 名前空間で別物 → port 時に rewrite or skip 判定要
- HELIX では HELIX skill 体系 (`~/ai-dev-kit-vscode/skills/`) を直接参照しない (CLAUDE.md `vendor/helix-source/` は read-only source material)。skill canonical は `docs/skills/*.md`
...
- HELIX §7.2 `skill suggest` JSON contract と vendor `helix skill search` 出力との drift table 作成
- 成果物: `docs/plans/PLAN-004-sprint-1-analysis.md`

### Step 2: Sprint .2 着手前調査 / skeleton

- `src/helix/skill/` package 新設 (W1/W2/W3a と並列、`src/helix/` 直下から分離)
...
- §7.2 catalog entry contract: `{id, name, path, description, helix_layer, triggers, verification, agent_default, source_hash}` (HELIX 仕様に整合、vendor JSONL からは references / classification / commands を除外し W3b-B carry)
...
- vendor `test_skill_catalog.py` の test pattern を流用、HELIX §7.2 JSON contract を fixture で網羅
- 主要 case:
...
- `pytest src/helix/tests/` で W1 124 + W2 35 + W3a 134 + W3b-A 追加分の全 PASS 確認
...
- 1 PLAN = 1 commit (`feat(W3b-A): port HELIX skill_catalog to HELIX rule-based engine`)
...
- [ ] `src/helix/skill/__init__.py` + `catalog.py` + `suggest.py` が py_compile clean
- [ ] vendor test_skill_catalog.py 相当の test_skill_catalog.py + test_skill_suggest.py が `src/helix/tests/` に存在し、すべて PASS
...
- **PLAN-004-b**: vendor `command_mapper.derive_commands` の HELIX command 名前空間 (`helix codex` / `helix skill` 等) 対応版 port (W6 CLI binding と同期)
- **PLAN-004-c**: skill `references/*.md` walk + intro 抽出 (vendor `_extract_reference_intro` 相当)。`docs/skills/<id>/references/` 構造を採用するか `docs/skills/*.md` flat のみとするかは HELIX spec 拡張要
