...
## §2 vendor vs HELIX 仕様 drift

### Drift table (重要度順)

| # | 項目 | vendor 実装 | HELIX 仕様 (§2 / §7.3) | 影響度 | 対応 |
|---|---|---|---|---|---|
...
| **D3** | grandfather 機構 | `deferred-findings.yaml` から `DF-PLAN-NNN-AUDIT-001 + status:open` を抽出 + `VMODEL_ENFORCEMENT_DATE = 2026-05-18` 前の created date を legacy 扱い | HELIX は carry rule (P0/P1/P2/P3) で吸収。HELIX 固有の enforcement date hardcode は廃止 | 中 | **削除**、HELIX carry rule (将来 PLAN で実装) で再吸収 |
| **D4** | semantic config | `cli/config/vmodel-semantics.yaml` schema validator (vmodel_loader.py) | HELIX には対応 config 不在。§2 spec は markdown 仕様で直接 lint 可能 | **大** | **vmodel_loader port を削除**、scope から除外 |
...
| **D11** | path constant | `HELIX_ROOT` / `PLANS_DIR` / `DEFERRED_FINDINGS` (HELIX absolute) | `HELIX_PROJECT_ROOT` env / `.helix/` state | 小 | constant rename + env var 化 (W1 と同パターン) |
| **D12** | yaml parser | `import yaml` (PyYAML) + `yaml is None` fallback | PyYAML>=6.0 (W1 で固定済) | 小 | fallback path 削除 |
| **D13** | DESIGN_PATTERN | `D-API\|D-DB\|D-CONCEPT\|D-FUNC\|docs/v2/L3-detailed-design/` | HELIX §2.1: `docs/design/<feature>/<name>.md` | 中 | pattern rewrite |
| **D14** | TEST_DESIGN_PATTERN | `L4-test-design\|PLAN-\d+(?:[A-Z])?-(?:unit\|integration\|system\|acceptance)-test-design\.md\|D-TEST-DESIGN` | HELIX §2.1: `docs/test-design/<feature>/<name>-test-design.md` | 中 | pattern rewrite |
| **D15** | TEST_CODE_PATTERN | `cli/lib/tests/test_[a-zA-Z0-9_]+\.py` | HELIX §2.1: `tests/...` | 小 | pattern rewrite |
| **D16** | IMPL_PATTERN | `cli/lib/[a-zA-Z0-9_/]+\.py` | HELIX §2.1: `src/...` | 小 | pattern rewrite |
...
| artifact | 内容 |
|---|---|
| `src/helix/vmodel_lint.py` | 必須 8 edge graph 検証 + kind dispatcher + L6 QA 分離 + 逆ピラミッド + exit 0/2/1 |
| `src/helix/tests/test_vmodel_lint.py` | 上記 5 要素の test fixture、HELIX §2.7 受入条件網羅 |
...
3. **kind dispatcher の fail-close**: kind が VALID_KINDS に無い場合、§7.3 では「将来 PLAN で詰める」とあるが、HELIX 仕様としては fail-close (exit 1) が安全。default reject
...
- Sprint .2 (skeleton): `src/helix/vmodel_lint.py` の dataclass + EDGE_ID enum + EXIT 定数 + 関数 stub 作成
- Sprint .3 (実装): 8 edge 個別検証 + kind dispatcher + L6 QA / 逆ピラミッド + exit code 0/2/1
- Sprint .4 (test): vendor test pattern (12 test) を流用しつつ rewrite、HELIX §2.7 受入条件網羅
