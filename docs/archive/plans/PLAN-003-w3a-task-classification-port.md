...
generates:
  - artifact_path: src/helix/task/__init__.py
    artifact_type: python_module
  - artifact_path: src/helix/task/effort.py
    artifact_type: python_module
  - artifact_path: src/helix/task/classifier.py
    artifact_type: python_module
  - artifact_path: src/helix/task/orchestration.py
    artifact_type: python_module
  - artifact_path: src/helix/tests/test_task_effort.py
    artifact_type: test_code
  - artifact_path: src/helix/tests/test_task_classifier.py
    artifact_type: test_code
  - artifact_path: src/helix/tests/test_task_orchestration.py
    artifact_type: test_code
...
W3a 第3波の前段 (W3 を 2 PLAN 分割: 本 PLAN = W3a = task classification、後続 PLAN-004 = W3b = skill subsystem)。HELIX vendor から effort 分類 / task type 推定 / task dispatch の 3 module を `src/helix/task/` へ adapt port する。HELIX requirements_v1.1 §7.2 仕様 (task classify / task estimate JSON output) に整合させ、`helix task classify` / `helix task estimate` の核心 engine を確立する。

## §1 目的

W1/W2 で凍結した plan / vmodel lint engine を踏み台に、PLAN 起票 / sprint 開始時の経路判定を機械化する基盤を整える:

- vendor `effort_classifier.py` (8.6 KB) を `src/helix/task/effort.py` に port、三点見積 + risk_factor を出力する `EffortEstimate` を提供
- vendor `task_type_inference.py` (3.0 KB) を `src/helix/task/classifier.py` に port、kind/drive/size/complexity を推定する `classify_task` を提供
- vendor `task_dispatcher.py` (6.4 KB) を `src/helix/task/dispatcher.py` に port、classify 結果 + estimate 結果から orchestration の routing 推奨を返す `dispatch_task` を提供
...
W3a 範囲外として明示 carry:
- `helix task classify` / `helix task estimate` CLI binding は W6 (`helix doctor` / runtime detection) で総合実装
...
- HELIX vendor は `task_type_inference.py` で kind/drive/size を hard-coded keyword で推定する rule-based、`effort_classifier.py` で三点見積を出す。HELIX §7.2 仕様の JSON contract に揃えれば port 可能
- `task_dispatcher.py` は HELIX role/model 名 (gpt-5.5 high 等) を hard-coded で出力する。HELIX では `cli/config/models.yaml` ではなく **capability class** (`frontier-reviewer` / `worker` / `fast-checker` 等) で抽象化する仕様 (§7.1) のため、model 名 hard-code を rewrite
- HELIX では HELIX skill 体系 (`~/ai-dev-kit-vscode/skills/`) を直接参照しない。skill 推挙は W3b で `docs/skills/*.md` を canonical として再実装
...
- HELIX §7.2 JSON contract との drift table 作成 (model 名 hard-code / capability class / orchestration escalation 等)
- 成果物: `docs/plans/PLAN-003-sprint-1-analysis.md`

### Step 2: Sprint .2 着手前調査 / skeleton

- `src/helix/task/` package 新設 (W1/W2 と並列、`src/helix/` 直下から分離)
...
- HELIX role 廃止 (ROLE_MAP), HELIX §1.8 VALID_ROLES (7 種: po/tl/qa/aim/uiux/se/docs) に合わせる

### Step 4: Sprint .4 テスト rewrite

- vendor test pattern を流用しつつ、HELIX §7.2 JSON contract を fixture で網羅
- 主要 case:
...
- `pytest src/helix/tests/` で W1 124 + W2 35 + W3a 追加分の全 PASS 確認

### Step 6: Sprint .6 prompt template 移植 + dogfood

- vendor `effort-classify.md` を `docs/templates/prompts/effort-classify.md` に port、HELIX capability class 化に合わせて HELIX model 名を除去
...
- 1 PLAN = 1 commit (`feat(W3a): port HELIX task classifier/effort/dispatcher to HELIX`)
...
- [ ] `src/helix/task/__init__.py` + `effort.py` + `classifier.py` + `dispatcher.py` が py_compile clean
- [ ] vendor 3 test file 相当の test_task_*.py が src/helix/tests/ に存在し、すべて PASS
...
- [ ] effort-classify.md prompt template が HELIX capability class 準拠
...
- **D4**: vendor task_type は 4 種日本語 (実装/レビュー/設計/調査) → HELIX VALID_KINDS 11 種への rewrite が必要 (regex pattern 拡張)
...
| Sprint | 内容 | 結果 |
|---|---|---|
| .2 | `src/helix/task/` package + 3 module skeleton | `__init__.py` + `effort.py` + `classifier.py` + `orchestration.py` 作成、py_compile clean |
...
- **Minor [M2]**: `_recommended_gates` で使う `G3.8` の出典 (gate-policy.md §X 参照) を docstring 追記 → PLAN-003-e として carry (W6 helix doctor / gate 整理で gate-policy.md HELIX 化と合わせて反映)
...
- **PLAN-003-b**: `helix task classify --diff origin/main...HEAD` の git diff parse path (CLI binding 時に実装)
- **PLAN-003-c**: vendor `task_dispatcher.py` (automation allowlist execution = helix:command/shell:script/http:webhook 3 種を allowlist で実行する infra) の HELIX への port 評価。本 W3a の §7.2 orchestration 連携とは別物
- **PLAN-003-d**: size=null 出力時 (3 軸すべて軸入力なし) を CLI binding (W6) で `--files` / `--lines` / `--diff` から自動取得する設計に統合
- **PLAN-003-e**: `classifier._recommended_gates` の `G3.8` 出典 (gate-policy.md HELIX 化) を docstring に明示 (Sprint .7 Minor [M2])
...
- **W6 連携**: `helix task classify` / `helix task estimate` CLI binding 統合 (W6 doctor / runtime detection)
