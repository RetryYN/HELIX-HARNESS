...
## §2 vendor vs HELIX §7.2 仕様 drift

### Drift table (重要度順、10 件)

| # | 項目 | vendor | HELIX §7.2 | 影響 | 対応 |
|---|---|---|---|---|---|
...
| **D3** | command_mapper 依存 | `derive_commands()` で HELIX command (`helix-codex` / `helix-claude` / `helix-skill`) を catalog に embed | HELIX は `helix` 名前空間で別物 | 中 | W3b-A では command embed を skip (entry に `commands` field を含めない)、W6 CLI binding と同期して PLAN-004-b で追加 |
| **D4** | references walk | `skill_md.parent / "references" / *.md` を walk して title + intro 抽出 | HELIX spec で references 構造は未明示 | 中 | W3b-A scope 外 (PLAN-004-c carry)、flat 構造のみ対応 |
| **D5** | JSONL catalog (classification 連携) | `_build_jsonl_entry` で classification status (pending/approved/manual) と source_hash を export、`skill_classifier` がこの skeleton を更新 | HELIX は LLM 分類経路を W3b-B (PLAN-005) で実装、本 W3b-A は不要 | 中 | W3b-A は **JSON catalog のみ** port (`save_catalog`/`load_catalog`)、JSONL は PLAN-004-d carry |
...
| **D9** | agent default | vendor `_default_agent()` で skill_id / category / phases から HELIX role (tl/se/pg/security/qa) を返す | HELIX §1.8 VALID_ROLES 7 種 (po/tl/qa/aim/uiux/se/docs) + §7.1 capability class (frontier-reviewer/worker/fast-checker) | 中 | rewrite (capability class 抽象化、HELIX role 名は保存しない) |
| **D10** | vendor_candidate=true 表示 | vendor では vendor_candidate 概念無し (HELIX で新規仕様化) | §7.2: 未移植 skill は `vendor_candidate=true` で表示し正本化なしに gate input にしない | 中 | **新規実装** (catalog 不在 skill 言及時、missing bucket に vendor_candidate=true で出力) |
...
- **新規実装**: `src/helix/skill/suggest.py` で §7.2 `{required/optional/missing}` JSON contract を rule-based matching で実装
...
| artifact | 内容 |
|---|---|
| `src/helix/skill/__init__.py` | package init |
| `src/helix/skill/catalog.py` | rule-based catalog 構築 (`docs/skills/*.md` walk → frontmatter PyYAML parse → CatalogEntry dataclass) + find_skill + save/load |
| `src/helix/skill/suggest.py` | §7.2 skill suggest JSON contract (required/optional/missing rule-based matching、PLAN frontmatter `layer` / task text `triggers` regex / `verification` field を使用) |
| `src/helix/tests/test_skill_catalog.py` | catalog 構築 + frontmatter parse + find_skill + save/load round-trip |
| `src/helix/tests/test_skill_suggest.py` | §7.2 JSON contract 全 field 網羅 + required/optional/missing 振り分け + vendor_candidate=true |
...
1. **`docs/skills/*.md` flat 構造 vs vendor 3 階層**: vendor は `category/name/SKILL.md` で category を path から取得。HELIX flat 構造では category を frontmatter (`metadata.category` field) から取得する必要。skill seed 起票時に強制 (validator 違反 → catalog skip + warn)
2. **PyYAML 採用時の vendor 互換性**: vendor bespoke parser は `[a, b, c]` 文字列を list に展開する `_normalize_frontmatter_value` を持つ。PyYAML は `[a, b, c]` (YAML flow style) を list に展開できるが、`triggers: a, b, c` (CSV) は文字列のまま。HELIX では **YAML flow style か block style のみ正規** とし、CSV は禁止する frontmatter 規約を確立
3. **§7.2 required/optional/missing の判定基準**: vendor は LLM 推挙の rank score、HELIX W3b-A は rule-based。**判定 rule の明示が必要**:
...
4. **vendor_candidate=true の判定**: 「PLAN/text が言及するが catalog に不在」の判定 rule をどう作るか。HELIX vendor では skill_id list を持つが HELIX W3b-A は flat docs/skills のみ。**hard-coded reference list** (例: `R4`/`promotion_strategy` → `reverse-pack` / `postmortem` → `operations-pack`) で代用、W3b-B で LLM 補完
...
- Sprint .2 skeleton: `src/helix/skill/` パッケージと 2 module skeleton (`catalog.py`/`suggest.py`) を作成
