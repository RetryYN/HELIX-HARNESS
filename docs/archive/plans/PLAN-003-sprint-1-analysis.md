...
| **D2** | HELIX cache (`.helix/budget/cache/classify`) | 30 日 TTL ファイル cache | HELIX cache layer 未策定 | 中 | 削除、in-memory のみ |
| **D3** | HELIX role 名 hard-code | tl/se/pg/fe/qa/security/dba/devops/docs/research/legacy/perf 12 種 | HELIX §1.8 7 種 (po/tl/qa/aim/uiux/se/docs) + capability class (frontier-reviewer/worker/fast-checker) | 中 | rewrite |
| **D4** | task_type 推定の出力 | 4 種日本語 (実装/レビュー/設計/調査/不明) | HELIX VALID_KINDS 11 種 (impl/design/poc/reverse/troubleshoot/refactor/retrofit/research/add-design/add-impl/recovery) | **大** | 4→11 rewrite (regex pattern 拡張) |
| **D5** | task_dispatcher.py の正体 | automation allowlist execution (helix:command/shell:script/http:webhook を allowlist で実行する infra) | HELIX §7.2 orchestration 連携 (capability class escalation) は全く別物 | **大** | **W3a port 対象外**。`task/orchestration.py` を新規実装で §7.2 を満たす。vendor task_dispatcher.py の port 評価は別 PLAN (PLAN-003-c) |
| **D6** | model_registry.py / defaults_loader.py 依存 | resolve_role_model() 経由で `cli/config/models.yaml` を引く | HELIX では models.yaml を持たない、capability class で抽象化 (§7.1) | 中 | 依存削除、constants 化 |
| **D7** | task estimate (PERT 三点見積) | **vendor に存在しない** | §7.2 で必須 (optimistic / most_likely / pessimistic / expected = (o+4m+p)/6 / risk_factor 1.0-2.0 / buffered = expected * risk_factor / story_points / risks) | **大** | **HELIX で新規実装** |
...
| **D10** | task_type explicit marker | `[タスク種別]: 実装` で frontmatter override | HELIX では PLAN frontmatter `kind` フィールドが正本、explicit marker は補助 | 小 | marker pattern 流用、PLAN frontmatter `kind` 優先 |
...
| artifact | 内容 |
|---|---|
| `src/helix/task/__init__.py` | package init |
| `src/helix/task/effort.py` | rule-based scoring (vendor `score_task`/`map_to_effort` 流用) + PERT 三点見積 (新規) |
| `src/helix/task/classifier.py` | §7.2 task classify JSON contract (kind 11 種推定 + drive + size 3 軸 max + complexity + split_required + recommended_gates + confidence + reasons) |
| `src/helix/task/orchestration.py` | §7.2 capability class escalation 判定 (frontier-reviewer / worker / fast-checker) |
| `src/helix/tests/test_task_effort.py` | scoring + PERT fixture |
| `src/helix/tests/test_task_classifier.py` | §7.2 JSON contract 全 field 網羅 |
| `src/helix/tests/test_task_orchestration.py` | escalation 経路 fixture (confidence < 0.7 / risk_factor ≥ 1.6 / production impact) |
| `docs/templates/prompts/effort-classify.md` | HELIX capability class 化、HELIX model 名除去 |

## §4 上位 5 落とし穴 (Sprint .3 実装注意)

1. **PLAN frontmatter `kind` 優先 vs 推定**: HELIX では PLAN frontmatter `kind` が正本。classify は frontmatter があれば parse 結果優先、無ければ regex 推定 (`--text` mode のみ regex pure)
...
- Sprint .2 skeleton: `src/helix/task/` パッケージと 3 module skeleton (`effort.py`/`classifier.py`/`orchestration.py`) を作成
