...
| `S3` | Verify (回帰蓄積) |
| `S4` | 判定 (confirmed / rejected / pivot) |
...
- [ ] enum 違反 → `helix plan lint` で exit 1 (fail-close)
...
|  | reverse: code（コード） | reverse: design（設計） | reverse: upgrade（更新） | reverse: normalization（正規化） | reverse: fullback（全面戻し） |
|--|--------------|-----------------|------------------|------------------------|-------------------|
| **hypothesis-test** | **推奨** | 代替 | 代替 | 代替 | 代替 |
| **tech-spike** | 代替 | **推奨** | 代替 | 代替 | 代替 |
| **design-spike** | 代替 | **推奨** | 代替 | 代替 | 代替 |
| **perf-spike** | 代替 | 代替 | **推奨** | 代替 | 代替 |
| **security-spike** | **推奨** | 代替 | 代替 | 代替 | 代替 |
| **ux-spike** | 代替 | **推奨** | 代替 | 代替 | 代替 |
...
`helix plan lint` は recovery kind PLAN の本文に 7 セクション header (h2 / `## §1 事故記録` 形式) があるか機械検証。
...
| 4 | **次セッションへの引き継ぎメモ** | `.helix/handover/CURRENT.md` の `updated_at` が 24h 以内なら OK (それ以外は warning) |
...
`helix` は **シェルラッパー方式** で実装する。

```
scripts/
├── helix                    # ディスパッチャ (bash)
├── helix-plan-lint.sh       # plan lint
├── helix-vmodel-lint.sh     # vmodel lint
├── helix-doctor.sh          # 統合チェック
├── helix-gate.sh            # G1-G11 ゲート判定
└── lib/
...
| `helix doctor` | plan + vmodel + governance + scrum/reverse + recovery 統合検証 |
...
- [ ] `helix vmodel lint` が §7.2 の kind 別経路で正しく分岐
...
- [ ] `helix doctor` が plan + vmodel + governance + scrum/reverse + recovery を統合検証
...
5. 結果を `.helix/audit/escalation_state.json` (これは git 管理) に書く
```
...
| 6 | `scripts/helix*` + `scripts/lib/*` 配備 | `bash scripts/helix --help` 通る |
| 7 | `scripts/requirements.txt` 配備 | `pyyaml>=6.0,<7.0` 含む |
| 8 | `python -m pip install -r scripts/requirements.txt` 通る | exit 0 |
| 9 | `chmod +x scripts/*.sh scripts/helix*` | `[ -x scripts/helix ]` 通る |
