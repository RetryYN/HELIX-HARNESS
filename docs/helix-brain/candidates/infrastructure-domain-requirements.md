...
| 実際に稼働しているInfrastructure（Runtime） | 実際のserver、network、database、資源の状態、配備・backup・restore・scalingの実行（注：HELIX自身のものはHELIX-INFRASTRUCTUREが持ち、対象製品のものは対象製品の側が持つ。[Concept配置の判断記録](../../governance/decisions/infrastructure-concept-placement-po-decisions-2026-09-26.md)） |
