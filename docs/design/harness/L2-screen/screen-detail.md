...
| PM-05 Continuation View | next runtime が stale prose に依存せず正しい state から resume するための画面。 | `harness.db` continuation projection、memory takeover entry、session digest。 | active PLAN、blocker、next authority、feedback、continuation pointer、DB/memory conflict warning。 | next authority から target screen へ navigation、`helix status` command の copy、evidence detail の open。 | projection missing/corrupt は fail-close。DBとmemoryが矛盾する場合はDBを表示しdiagnosticを示す。 | UX-03、FR-L1-01/31/42。event/projection整合とnext authority targetを確認する。 |
...
| HM-06 Recovery View | operator が recovery run、resume point、rollback guidance を確認する画面。 | recovery plan、incident record、continuation projection、audit trail。 | recovery log、resume point list、rollback copy block、current incident status。 | rollback/resume command の copy、PM-03 gate の open、HM-05 evidence の open。 | safe rollback がない場合は generated destructive command ではなく human-escalation message を表示する。 | UX-03、FR-L1-10/16。destructive command が auto-execute されないことを確認する。 |
