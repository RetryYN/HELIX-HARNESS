...
|---|---|---|---|---|---|
| catalog解析 | `analyzeDriveRouteCatalog(raw, documentExists) => DriveRouteCatalogResult` | JSON入力と文書存在portを受ける | exact route集合、kind、遷移、Forward到達可能性、横断construct分類、surface投影、専門工程、文書を純関数で検査する | 入力、PLAN、Issue、DBを書き換えない | `U-DRCAT-001`〜`U-DRCAT-014` |
