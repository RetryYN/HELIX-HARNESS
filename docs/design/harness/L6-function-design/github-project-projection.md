...
- open PRは`In Progress`、open PRからclosing relationまたはcross-referenceを持つopen Issueも
  `In Progress`、その他のopen Issueは`Todo`とする。
...
open workとStatusの投影を最小sliceとして実装する。roadmap iteration、PLAN親子からsub-issueへの写像、
`helix doctor`常設gate、scheduled workflowは後続sliceとする。scheduled workflowの有効化には専用GitHub App
または最小権限tokenの登録が必要であり、本sliceではcredentialを作成しない。
