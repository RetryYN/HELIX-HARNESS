...
`resolvedTombstones[]`は
`{fingerprint,resolved_at,resolution_plan_id,resolution_plan_semantic_digest,previous_digest,entry_digest}` のappend-only
hash chainとする。genesis `previous_digest`はinitial集合digest、各`entry_digest`は
`sha256(previous_digest + "\n" + fingerprint + "\n" + resolved_at + "\n" + resolution_plan_id + "\n" + resolution_plan_semantic_digest)`。
codeはterminal digestをpinし、正当な追加はresolution PLAN、cross-review、constant更新を同一commitで要求する。
resolution PLANは`completed`かつtyped `resolves_authority`でauthority path、対象fingerprint、対象PLAN ID、
解消reasonをexact宣言し、`generates`にauthority configと対象PLANを含める。reviewはworker/reviewer modelが相異なる
`cross_agent`または`intra_runtime_subagent`のapprove verdictとgreen commandを必須とする。
`resolution_plan_semantic_digest`はPLAN設計意味とreview evidenceを固定し、後編集や無関係PLAN流用を拒否する。
解消時点の対象PLANとresolution PLANはともにactive snapshot内で固有bindingがgreenでなければならず、
repo loaderからnon-zero tombstoneを通す統合経路もpure analyzerと同じ判定を使う。
resolution PLAN lifecycleは`draft/confirmed`では解消権を持たず、検証・独立review後に`completed`へ到達した時だけ有効とする。
