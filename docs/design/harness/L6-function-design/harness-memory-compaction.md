...
- layer file 未作成/空のときは backup も書換えも行わず 0 件結果を返す（dryRun と対称、review Important 所見 2026-07-07 の是正）。
...
（oracle-test-trace gate: 宣言 oracle は test citation 必須のため、宣言は実装スライスと同時に行う）。予定 oracle（実装時に `tests/memory-compaction.test.ts` を新設して cover する。本 doc 起草時点で当該テストは未存在）:
