...
|---|---|---|---|---|---|
| codec / validation | `normalizeMemoryEntry(raw, expectedLayer) => NormalizeResult` | raw JSONL行と期待layerを受ける | v1/v2をv2 viewへ正規化し不正v2はdamagedとして返す | 明示v2をv1へfallbackせずsecret/lifecycle矛盾を受理しない | `U-MEMV2-001a/001b/002a/002b` |
| mutation / lifecycle | `writeMemoryV2(input, deps) => WriteMemoryV2Result` | operation ID、provenance、expiry、fencing lockが妥当 | appendとsession eventを冪等化しconsume/expireをstable tombstoneへ収束 | JSONL data SSoT、同intent replay追記0、異intent fail-close | `U-MEMV2-003/004a/004b/005a/005b/008a/008b` |
| surface / compaction | `surfaceMemoryV2(input, deps) => SurfaceResult` | normalized view、clock、件数/文字budgetが妥当 | 決定論選定とhidden/lifecycle集計を返す | compaction前後のactive/tombstone/link viewを変えない | `U-MEMV2-003/006/007a/007b` |
...
| `retireMemory(input, deps): ConsumeResult[]` | 正本へ追突済みのharness/project entryだけをbody-free `memory-consumed:<target-id>` receiptへ退役する。authority IDはsource revision、consumer、layer、全entry、target content digestから導出する。repo外realpathへ解決するsymlinkを拒否し、全IDと正本target／keyの結合を証明できなければ`unauthorized`。authorityはlock前後で再検証し、takeoverは受け付けず、fencingによりcompaction後を含む再実行を`already_consumed`へ収束する。 |
...
- `retireMemory({layer,ids,consumerId,authorityId})`はharness/projectだけを遷移できる。正本targetと追突台帳が成立し、tracked authority manifestのsource revision／全target digest／authority digestとrepo内realpathが全IDについて機械検証できた後に
...
| MEMV2-S5c | 正本化済みharness/projectをfenced retireし、v2とlegacyの両surfaceから除外する。再retireは追記ゼロで、takeoverには適用しない。 |
...
後続 L7 実装 PLAN は上記15 subcaseを`U-MEMV2-*` oracleとtest citationへ同時に具体化する。
