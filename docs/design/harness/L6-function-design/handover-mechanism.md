...
| **scaffold** (`scaffoldFromDigests`/`renderHandoverScaffold`) | digest + frontmatter → 6 セクション markdown。①②prefill、③は `workflowNextActionsForOutstanding` と `completion-review-coverage`、⑤は `outstandingSummaryLine` で seed、④⑥は placeholder | **純関数**。sanitize 済前提・credential 非載。§3/§5 marker 欠落と §3 coverage 欠落は doctor hard gate |
...
| `checkHandoverNextActionAnchor` | `(deps: HandoverDeps) => { messages: string[]; ok: boolean }` | **doctor hard gate**。最新 handover entry の §3 次アクション (Next Action) に `HANDOVER_NEXT_ACTION_MARKER` が必須。blocked route では marker だけでなく `completion-review-coverage`、`covered=`、`non-packet=`、`policy=review-packets-cover-decision-blockers-only`、`packet要約`、`確認field=`、`matrix必須field=`、`確認観点ID=` も必須にする。§3 欠落、latest_doc 読取不能、§3 が `TODO(human): 順序付き次手` のまま、旧 marker-only §3、review coverage 欠落、source-delta field 欠落、machine review route ID 欠落、または `確認観点=review ...` として machine review route を人間向け確認観点へ露出する旧 §3 なら fail-close し、`ut-tdd handover` 再生成を要求する。pointer 不在は skipped/OK。これにより status/CURRENT.json の workflowNextActions と markdown 再開手順が分岐せず、S4 / version-up / rename cutover / action-binding の matrix/review 導線、および review packet で閉じられない blocker の区別も失われない |
