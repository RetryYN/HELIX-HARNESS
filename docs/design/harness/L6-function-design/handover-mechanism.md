...
実装 (②): src/handover/index.ts + src/cli (helix handover / helix plan use) + src/runtime/session-log.ts 限定 amendment (PLAN-L7-04-handover-mechanism, add-impl, 後続)。
...
handover を、**session-log の PLAN digest を機械的入力にして生成する機構**として設計する。(1) `.helix/state/current-plan` から active PLAN を解決し `.helix/logs/plan/<id>.digest.json` を集め (`resolveHandoverScope`)、(2) digest 要約を機械可読の**機械ポインタ `.helix/handover/CURRENT.json`** に書き (`buildPointer`/`writePointer`)、(3) digest + PLAN frontmatter から §6.8.5 必須 6 セクションの **markdown scaffold** を生成して `docs/handover/session-handover-<date>.md` に追記する (`scaffoldFromDigests`/`renderHandoverScaffold`/`runHandover`)。機械部 (①PLAN サマリ一部・②成果物) を prefill し、③Next Action と⑤未了 PO 判断は outstanding / workflowNextActions から機械 seed する。④carry と⑥壊さない注意、および機械 seed への人間補足は placeholder に残す。
...
> **PO 確定事項** (2026-06-02、§6.8.5/§6.8.6): ① PLAN が `status=completed` へ遷移する時 handover 生成は**必須** / ② 入力は **session-log の PLAN digest** (touched files / commits / failures) + 人間判断 / ③ 配置は **機械ポインタ `.helix/handover/CURRENT.json` (local/gitignored)** + **チーム記録 `docs/handover/*.md` (tracked)** の二層 / ④ 粒度は 1 PLAN=1 entry を要さず **1 セッション or 1 駆動サイクル単位**で束ねてよい。

> **なぜ「機械 prefill + 人間判断」か**: §6.8.6 は進捗を **state DB (今どこ) / log (どう進めた) / handover (次どう)** の 3 層で管理し、handover の役割を「**次どうするか** (Next Action / carry / 未了 PO 判断)」=人間判断・durable と定義する。よって成果物トレイル (②) は digest から機械生成できる。加えて、outstanding / completionDecisionPacket / workflowNextActions は PLAN state から機械導出できるため、③Next Action と⑤未了 PO 判断は TODO のままにせず機械 seed する。これは判断代行ではなく、PO/S4・version-up・cutover・action-binding の workflow queue を失わないための anchor である。digest はこの 3 層の**結節点** (log→handover 橋渡し + DB 登録トリガ) であり、本機構はその橋を実装する。
...
- `resolveActivePlan` (session-log.ts L124) は **① `.helix/state/current-plan` → ② branch 名 (`add/<plan>` 等) fallback → ③ null** の順に解決する (U-SLOG-001 既設計、ロジックは正しい)。
- しかし **① の state を書く機構が存在せず**、**② は harness 自己開発が main 直 (branch を切らない、[[feedback_main_direct_solo]]) ため一致しない** → `plan_id` が**恒常 null** (実ログ 5 件すべて `plan_id:null` で実証) → `onStop` が `plan_id=null` の session を digest 対象外にする (U-SLOG-004) ため `.helix/logs/plan/*.digest.json` が**一度も生成されない**。
- 結論: 本機構は **current-plan を書く経路 (`setActivePlan` / `helix plan use`) の追加 = Gap B 修正**を前提条件として含む。**`resolveActivePlan` の解決ロジックは一切変えない** — 入力 state を供給する経路を足すだけ。これは session-log 設計 (PLAN-L6-03) への**限定 amendment** であり、新 sub-doc を起こさない。
...
| **scaffold** (`scaffoldFromDigests`/`renderHandoverScaffold`) | digest + frontmatter → 6 セクション markdown。①②prefill、③は `workflowNextActionsForOutstanding`、`completion-review-coverage`、`semantic-frontier-records` / `confirmed-current-meaning-records`、⑤は `outstandingSummaryLine` で seed、④⑥は placeholder | **純関数**。sanitize 済前提・credential 非載。§3/§5 marker 欠落、§3 coverage 欠落、§3 意味台帳欠落は doctor hard gate |
| **活性化** (`setActivePlan`) | `.helix/state/current-plan` を書く (digest を populate させる Gap B fix) | 単一書込。`resolveActivePlan` 読取先と同一 path |
...
| **status preflight** (`readPointer` / `helix handover status --json`) | `.helix/handover/CURRENT.json` を read-only に読み、セッション開始時の機械確認に使う | 不在は `exists:false` / 壊れ JSON は `exists:true, stale:true` / 既存 pointer は stale 理由付き。生成や補完はしない |

handover は hook でなく **CLI subcommand** (`helix handover` / `helix handover status --json` / `helix plan use`) で実現する。session-log の既存 hook に「commit 経路で `inferPlanFromCommit` → `setActivePlan`」の 1 行 (best-effort/fail-open) を足すのみで、**新規 hook は追加しない**。
...
// .helix/logs/plan/<id>.digest.json の読取 subset (session-log.ts PlanDigest と互換)
...
// .helix/handover/CURRENT.json — 機械ポインタ (gitignored、単一 SSoT)
interface HandoverPointer {
  active_plan: string | null;
  status: HandoverStatus;
  owner?: string;              // baton 所有者 (例: codex / claude / opus)。freshness 判定には使わない
  owner_updated_at?: string;   // owner だけを更新した時刻
...
| 関数 | signature（署名） | DbC |
|------|-----------|-----|
| `resolveHandoverScope` | `(deps: { repoRoot: string; readText; listDir }) => { active_plan: string\|null; digests: PlanDigestRef[] }` | **never throws**。`.helix/state/current-plan` を読み active_plan を決定 / `.helix/logs/plan/` を listDir し `*.digest.json` を parse / 不在・壊れ JSON は skip / 何も無ければ `{active_plan:null, digests:[]}`。**`listDir` は本 deps では必須** (session-log の optional `listDir?` と異なり handover は走査が責務) |
...
| `renderHandoverScaffold` | `(doc: HandoverDoc, opts?: { outstanding?: OutstandingWork }) => string` | **純関数**。`# セッション引き継ぎ — <date>` + §1..§6 の 6 セクション markdown を render / 機械部 (①②) は埋める。`opts.outstanding` がある場合、③次アクション (Next Action) は `workflowNextActionsForOutstanding` 由来の `HANDOVER_NEXT_ACTION_MARKER`、`completion-review-coverage`、`semantic-frontier-records`、`confirmed-current-meaning-records`、PLAN ID、required action、route、PLAN-scoped primary/supporting packet commands、base packet command、各 supporting packet の schemaVersion / matrixField / expectedMatrixCount / machine `reviewRoute` を示す `packet要約` を出す。`completion-review-coverage` は `completionReviewBundleForOutstanding` の `reviewCoveredBlockers` / `nonPacketBlockers` を `covered=` / `non-packet=` として出し、review packet で閉じる blocker と packet 外 blocker を handover Markdown でも混同させない。`semantic-frontier-records` は `featureId` / `classification` / `planId` と `completion-claim-allowed=false` を出し、`confirmed-current-meaning-records` は confirmed 11 意味単位 ID と `boundary=downstream_evidence_required` を出すため、Markdown 再開面でも「機能一覧は何を意味し、何をまだ current completion に混ぜないか」を確認できる。`packet要約` の `確認field` / `matrix必須field` は field 単位で sanitize してから結合し、一覧全体を 120 文字 truncation しない。S4 の `decisionRecord.source_ledger_freshness` / `decisionEvidenceChecklist.unresolved_risk` など、PO が読むべき具体 field を markdown handover で省略しないためである。S4 / version-up / action-binding packet は `--plan <PLAN_ID>` 付きで再開者が対象 PLAN を推測しなくてよい形を主表示にし、singleton の rename packet は base command のまま表示する。JSON/packet の machine field は原文 contract のまま保持するが、markdown handover の human-facing 表示では既知の required action / route / reviewRoute を日本語化し、`確認観点=` に日本語 `reviewRouteJa`、`確認観点ID=` に machine `reviewRoute` を出す。特に rename approval-draft は `確認観点=非承認の approval draft record / current snapshot binding / safety flag を確認してから人間承認へ進む` とし、英語 `review non-authorizing ...` は ID 側にだけ残す。再開者が英語 prose を読まなくても PO/S4・version-up・cutover・action-binding の意味を読めるようにする。completion-ready なら completion claim audit を出す。`opts.outstanding` が無い後方互換経路では③を `TODO(human)` にする。⑤未了 PO 判断は `HANDOVER_OUTSTANDING_MARKER` と `outstandingSummaryLine` で seed する。④⑥は `TODO(human)` placeholder。**自由テキスト欄 (`plans[].summary` / `deliverables[].*` / digest 由来 failures) は session-log の `sanitize` を render 時にも適用する (defense-in-depth)**。tracked な `docs/handover/*.md` に commit されるため、digest 側 sanitize 済でも render 段で再マスクして credential/PII の流出経路をゼロにする (二重 sanitize は冪等、`SECRET_RE` は `name=value` を `name=***` に) |
...
| `readPointer` | `(deps: { repoRoot; readText }) => HandoverPointer \| null` | `.helix/handover/CURRENT.json` を read-only に読む。**不在 / parse 不能 / I/O 失敗は null**、正常 JSON は object を返す。`helix handover status --json` は通常 handover の status envelope を返す: missing は `exists:false, stale:false, stale_reasons:[]` + exit 0、parse 不能は `exists:true, stale:true, stale_reasons:["CURRENT.json is unreadable or invalid"]` + exit 1、正常 pointer は `exists:true` と `handoverStale(updated_at, now)` 由来の `stale/stale_reasons` を additive に付ける。status preflight は read-only であり、CURRENT.json や markdown scaffold を生成しない。`outstanding` が blocked の場合、pointer は同じ生成時点の `completionDecisionPacket` (`sourceCommand="helix handover"`) を保持し、再開時に PO/S4・version-up・cutover・action-binding record template、base packet command (`decisionPacketCommand` / `packetCommands`)、および対象 PLAN へ直行する scoped packet command (`scopedDecisionPacketCommand` / `scopedPacketCommands`) を失わない |
| `updatePointerOwner` | `(owner: string, deps: HandoverDeps) => { pointer: HandoverPointer; written: string[] }` | 既存 CURRENT.json の baton 所有者だけを移譲する。owner は空でなく、英数字開始かつ英数字 / dot / underscore / colon / hyphen のみ。CURRENT.json 不在・壊れ JSON・空 owner・危険文字は fail-close。成功時は `owner` と `owner_updated_at=deps.now()` を書き、**`updated_at` は変更しない**。owner 移譲は再開者の所有権記録であり、handover 内容の鮮度証跡ではないため、stale handover を owner 更新だけで fresh に見せない。CLI は `helix handover update --owner codex` を提供する。 |
| `checkHandoverCompletionDecisionPacket` | `(deps: HandoverDeps) => { messages: string[]; ok: boolean }` | **doctor hard gate**。CURRENT.json が `outstanding.completionReadiness.ok=false` を持つ場合、同じ pointer に `completionDecisionPacket` が必須。欠落、`sourceCommand!="helix handover"`、freshness/shape lint violation、`completionReadiness` と packet の ok/status 不一致、`outstanding.items.length` と packet decision count 不一致を fail-close する。pointer 不在、または blocked outstanding が無い場合は skipped/OK。`sourceCommand="helix handover"` の旧 schema packet が `supportingPacketSummaries[]`、`semanticMeaningSummary`、`semanticFeatureFrontierRecords[]`、`confirmedCurrentMeaningRecords[]` のいずれかを欠く場合は、stored outstanding から read-only に再構成して検査する。standalone `helix completion decision-packet --json` が green でも、handover resume surface が packet を持たない旧 pointer は通さない |
| `checkHandoverNextActionAnchor` | `(deps: HandoverDeps) => { messages: string[]; ok: boolean }` | **doctor hard gate**。最新 handover entry の §3 次アクション (Next Action) に `HANDOVER_NEXT_ACTION_MARKER` が必須。blocked route では marker だけでなく `completion-review-coverage`、`covered=`、`non-packet=`、`policy=review-packets-cover-decision-blockers-only`、`semantic-frontier-records`、`confirmed-current-meaning-records`、`completion-claim-allowed=false`、`boundary=downstream_evidence_required`、`packet要約`、`確認field=`、`matrix必須field=`、`確認観点ID=` も必須にする。§3 欠落、latest_doc 読取不能、§3 が `TODO(human): 順序付き次手` のまま、旧 marker-only §3、review coverage 欠落、意味台帳欠落、source-delta field 欠落、machine review route ID 欠落、または `確認観点=review ...` として machine review route を人間向け確認観点へ露出する旧 §3 なら fail-close し、`helix handover` 再生成を要求する。pointer 不在は skipped/OK。これにより status/CURRENT.json の workflowNextActions と markdown 再開手順が分岐せず、S4 / version-up / rename cutover / action-binding の matrix/review 導線、review packet で閉じられない blocker の区別、意味ベース機能一覧の frontier/confirmed 台帳も失われない |
| `writePointer` | `(pointer: HandoverPointer, deps: { repoRoot; writeText }) => void` | `.helix/handover/CURRENT.json` を **JSON 上書き** (単一機械ポインタ、append しない)。失敗は呼び出し側 (CLI) が warn |
| `setActivePlan` | `(planId: string\|null, deps: { repoRoot; writeText; removeFile? }) => void` | `.helix/state/current-plan` を書く (`resolveActivePlan` 読取先と同一)。**`planId!==null` → planId を書く / `planId===null` かつ `removeFile` 有 → file 削除 / `planId===null` かつ `removeFile` 無 → `writeText` で空文字を書く** (`resolveActivePlan` L126-127 は空文字を `trim()` で falsy 判定し branch fallback→null へ落とすため、空文字書込でも実質 clear になる)。deps は `SessionLogDeps` の subset (repoRoot/writeText) ゆえ amendment 経路でアダプタ不要 |
...
`-F -` heredoc commit ([[project_commit_msg_hook]]) は message が stdin 経由で command 文字列に乗らないため `inferred=null` になり no-op。**確実な活性化経路は `helix plan use <id>` (= `setActivePlan` の CLI 露出)**。`resolveActivePlan` 本体・既存 U-SLOG-001..005 の挙動は不変 (fail-open 維持、digest 生成条件 U-SLOG-004 も不変)。

### §2.4 ストレージ / 配置 / hook

| 対象 | path（配置） | tracked?（追跡対象） | 役割 |
|------|------|----------|------|
| 機械ポインタ | `.helix/handover/CURRENT.json` | gitignored | active PLAN / status / 最新 doc / digest 要約 / outstanding / completion decision packet (機械可読 SSoT)。CLAUDE.md ワークフロー参照先 |
| チーム記録 | `docs/handover/session-handover-<date>[suffix].md` | tracked | 人間判断 durable (既存運用を scaffold で augment) |
| 活性化 state | `.helix/state/current-plan` | gitignored | `resolveActivePlan` の入力 (Gap B) |
| digest 入力 | `.helix/logs/plan/<id>.digest.json` | gitignored | session-log onStop 生成 (活性化後に初 populate) |

- hook: **無し** (CLI subcommand + session-log 既存 hook への 1 行のみ)。
- `.helix/handover/` は現状 `.gitkeep` のみ → 本機構が `CURRENT.json` を実体化する。`.gitignore` で `CURRENT.json` を ignore (state 系の既存方針と整合)。

> **正本決定 (CURRENT.md vs CURRENT.json、本設計で確定)**: 機械ポインタの正本は **`.helix/handover/CURRENT.json`** とする。`handoverStale` の入力は `CURRENT.json.updated_at` であり、§5.3 pre-push stale 検知もこれを読む。**`CURRENT.md` は廃止** (要件 L947/L1972/L2024 の `.helix/handover/CURRENT.md` 表記は本 PLAN 確定をもって `CURRENT.json` に読み替える)。後段 `PLAN-REVERSE-05` の役割は**要件文書側の `.md`→`.json` 文字列修正のみ**に限定し、本設計時点で正本を確定させる (stale 基盤が参照先未確定でフリーズされ dead 化するのを防ぐ、code-reviewer Critical 反映)。
...
| ③ 次アクション (Next Action) | `workflowNextActionsForOutstanding` から PLAN ID / required action / route / scoped packet commands / base packet commands / `packet要約` を seed。表示は既知 machine phrase を日本語へ変換 | PO/S4 などの実判断結果を補足 | md + doctor marker |
...
| outstanding / completionDecisionPacket / G-SF semantic frontier | `helix handover` 生成時は CURRENT.json に snapshot 保存。`helix handover status --json` は read-only preflight として live `computeOutstandingWork` で再計算し、古い snapshot に無い additive field を補う | — | status response = live overlay、CURRENT.json = 保存 snapshot |
...
`.helix/state/current-plan` marker の 2 source を持つ。従来 `checkHandoverDiscipline` は両者の drift を
...
| gap（差分） | 現象 | 機能設計 (機械担保) |
|---|---|---|
| **① 手書き bypass 検知不可** | `helix handover` を経ず手書きで markdown / CURRENT.json を更新しても discipline が素通り | `HandoverPointer.generated_by` 署名 + `doc_entry_count` を runHandover が刻む。`checkHandoverBypass(deps)` が ① 署名欠落 (手書き pointer) / ② md entry 数 > 記録値 (手書き追記) を surface (presence/stale/drift の `checkHandoverDiscipline` と責務分離) |
...
- **PLAN-L7-04-handover-mechanism (add-impl)**: U-HOVER を先行 ④ テストコード化 (Red) → `src/handover/index.ts` + `helix handover` / `helix plan use` CLI + session-log 限定 amendment を Green→3 点 R で実装。
...
- **Gap B の手運用期間 (carry、code-reviewer Minor 反映)**: `helix plan use` / `helix handover` の CLI 実体は L7-04 (add-impl) が generates する (L6 設計 PLAN は src を generates しない bottom-up 順)。**L7-04 完了までは Gap B は「設計済・未配線」であり、digest 活性化は手運用 (current-plan を手で置く) になる**ことをユーザーへ明示する。
