...
| **scaffold** (`scaffoldFromDigests`/`renderHandoverScaffold`) | digest + frontmatter → 6 セクション markdown。①②prefill、③は `workflowNextActionsForOutstanding`、⑤は `outstandingSummaryLine` で seed、④⑥は placeholder | **純関数**。sanitize 済前提・credential 非載。§3/§5 marker 欠落は doctor hard gate |
