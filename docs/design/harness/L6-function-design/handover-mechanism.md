...
| **status preflight** (`readPointer` / `ut-tdd handover status --json`) | `.ut-tdd/handover/CURRENT.json` を read-only に読み、セッション開始時の機械確認に使う | 不在/壊れ JSON は null / CLI は exit 1。生成や補完はしない |
...
| `readPointer` | `(deps: { repoRoot; readText }) => HandoverPointer \| null` | `.ut-tdd/handover/CURRENT.json` を read-only に読む。**不在 / parse 不能 / I/O 失敗は null**、正常 JSON は object を返す。`ut-tdd handover status --json` はこの戻り値をそのまま JSON 出力し、null の場合は `handover: CURRENT.json not found` + exit 1。status preflight は read-only であり、CURRENT.json や markdown scaffold を生成しない |
