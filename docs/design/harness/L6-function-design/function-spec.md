...
Runtime verification の型、分類、ログ event build、completeness 検査は `src/schema/runtime-verification.ts`
を純粋 SSoT とする。`src/runtime/run-debug.ts` は append-only JSONL への I/O 端点だけを持ち、同じ純粋契約を
re-export する。`src/state-db/projection-writer.ts` は schema SSoT だけを import し、runtime module へ戻らない。
これにより `runtime -> state-db -> runtime` の import cycle を作らず、runtime claim の検証規則を CLI append
経路と DB projection 経路で共有する。
