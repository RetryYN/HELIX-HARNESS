...
| `analyzeRightArmVerificationStrategy` | `(input: RightArmVerificationStrategyInput) => RightArmVerificationStrategyResult` | gates.md and L08-L14 verification strategy are supplied as current process sources | fail-closes when G8-G14 verification remains concept-only, when any G8-G14 evidence-profile row is missing, when the verification source ledger loses required official source rows/columns/HTTPS URLs/adoption decisions, or when `gate impact` values do not map to recognized G8-G14/S3/S4/action-binding routes; the required official source set includes NIST/Scrum/ISTQB/OWASP/NASA plus W3C WCAG 2.2 and Playwright browser/a11y evidence sources, so G10/G11 browser, screenshot, and accessibility claims cannot pass on screenshot-only or automated-a11y-only evidence; the ledger as a whole must cover every G8-G14 gate so official sources cannot degrade into name-only references detached from the right-arm verification band |
| `loadOutstandingPlanRows` / `computeOutstandingWork` | `(repoRoot: string) => OutstandingPlanRow[]` / `(repoRoot: string) => OutstandingWork` | repo root may be a source repo with `docs/plans` or a freshly bootstrapped consumer repo with `.ut-tdd/state/project-setup.json`; missing optional sources fail open only when no positive state exists | reads non-terminal PLAN rows from `docs/plans` and additionally reads HELIX project setup state. When `.ut-tdd/state/project-setup.json` has `objectiveBoundary.scope=consumer_setup_readiness_not_whole_program_completion` and `completionClaimAllowed=false`, it synthesizes `CONSUMER-SETUP-BOUNDARY` with blocker `consumer_setup_boundary`, so an empty consumer repo after successful setup still yields `completionReadiness.status=blocked` and `completionClaimAllowed=false` until a real project PLAN/handover route and first-run evidence are recorded. This preserves the semantic distinction between setup readiness, consumer doctor green, and whole-program/L14 completion. |
| `completionDecisionPacketForOutstanding` | `(outstanding: OutstandingWork, opts: CompletionDecisionPacketOptions) => CompletionDecisionPacket` | `outstanding.completionReadiness` was computed from current non-terminal PLAN rows / open defers; `opts.generatedAt` and `opts.sourceCommand` identify the packet creation event | returns a decision packet derived from `outstanding.completionReadiness`, preserving `ok/status`, blocker list, authority boundary fields (`authorityBoundary`, `humanDecisionRequired`, `humanDecisionBlockers[]`, `workflowStateBlockers[]`, `autonomousWorkBlockers[]`, `nextAuthority`), decision count, PLAN-level decision items, primary allowed outcomes, primary next workflow route, source command, and freshness metadata (`decision-packet-freshness.v1`, `validForMinutes`, `expiresAt`, `stale`); top-level packet JSON also carries `semanticMeaningSummary`, live `semanticFeatureFrontierRecords[]`, and live `confirmedCurrentMeaningRecords[]` so the L14 completion decision packet itself exposes both the pending semantic frontiers and the confirmed 11 meaning-unit catalog behind the 43-item feature list. Human/PO/action-binding/version-up/cutover blockers are classified as `human_decision_required`; `non_terminal_plans` is a workflow state blocker, not direct automation work; plain active work / open defers / consumer setup boundary remain automation work blockers, so a packet cannot turn unresolved human judgement into routine implementation work or turn a state summary into a false autonomous task. Each item keeps a primary `requiredAction` for routing, Japanese-first `requiredActionJa` for PO display, primary `decisionPacketCommand`, all supporting `packetCommands`, PLAN-scoped `scopedDecisionPacketCommand`, PLAN-scoped `scopedPacketCommands`, unioned `requiredActions` plus parallel `requiredActionsJa`, and `requiredEvidence` / `requiredRecords` for every blocker so secondary human/action-binding approvals are not hidden; `nextWorkflowRoute` remains the machine route ID and `nextWorkflowRouteJa` is the text-mode PO route. Text output prints a `semantic-summary:` line and `authority-blockers` classification, then Japanese display fields first and emits the English machine fields as `action-id`, `required-action-id`, and `route-id`; JSON keeps both fields so downstream tools can switch on stable IDs while the chat/PO surface stays Japanese-first. Scoped commands add `--plan <PLAN_ID>` for S4 / version-up / action-binding packet surfaces while `rename plan` remains singleton/unscoped, so multiple pending PLANs do not force the PO to infer the target packet by hand; packet commands use the same mapping as `workflowNextActionsForOutstanding` so standalone completion packets and handover resume packets point to the same S4 / version-up / rename / action-binding surfaces as status; each `requiredRecords[]` entry has `recordName`, non-empty `fields`, non-empty `sourcePaths`, and source-ledger-backed records carry `sourceLedgerChecks[]` with `sourcePath` + `ledgerLabel` pointing to the owning workflow ledger; each record has matching `allowedOutcomesByRecord[]`, `nextWorkflowRoutesByRecord[]`, and `recordTemplates[]` entries so S4 / version-up / parked review / cutover / action-binding outcomes, return routes, and copyable record input shapes cannot disappear behind the primary blocker; each `recordTemplates[]` entry has `recordName`, non-empty `insertionHint`, a `yamlLines` header equal to `recordName:`, and one `- field:` line for every required field; `po_decision_pending` is limited to PoC S3/S4/S4-decision context and generic PO signoff stays under human/cutover/version-up blockers; doctor green is not used as a substitute for readiness |
| `completionReviewBundleForOutstanding` / `analyzeCompletionReviewBundle` | `(outstanding: OutstandingWork, opts?: CompletionDecisionPacketOptions) => CompletionReviewBundle` / `(bundle, decisionPacket, now?) => CompletionReviewBundleLintResult` | completion decision packet と supporting packet summary が現在状態から生成済み | L14 完了主張前に確認する非破壊の `completion-review-bundle.v1` を返す。bundle は `sourceCommand=ut-tdd completion review-bundle --json` と `runnableSourceCommand=bun run ut-tdd completion review-bundle --json`、固定 safety flag (`planOnly=true` / `mustNotDecide=true` / `mustNotApply=true`)、completion decision packet command と runnable command、exact / semantic digest、review packet count、blockedUntil、日本語 operator action、S4 / version-up / rename / action-binding supporting packet ごとの scoped review packet 行を持つ。さらに `reviewCoveredBlockers` と `nonPacketBlockers` を分け、review packet で確認できる human decision blocker と、packet だけでは閉じない workflow state / semantic frontier blocker を混同させない。text surface は `review-coverage:` を出し、`review-packet:` 行は日本語 `route=` と machine `route-id=` を分離し、JSON の `reviewRouteJa` / `reviewRoute` と同じ二重表現を保つ。`analyzeCompletionReviewBundle` は freshness stale、自己 source runnable command の欠落・drift、safety flag drift、completion state drift、review coverage drift、review packet count drift、review packet drift、digest drift を fail-close し、bundle を生成した repo-local 再生成 command なしに human review bundle を転記できないようにする。 |
| handover status live outstanding overlay | `ut-tdd handover status [--json]` | CURRENT.json may be older than the current PLAN/frontier state and may lack newer additive outstanding fields | read-only status output recomputes `computeOutstandingWork(repoRoot)` and `completionDecisionPacketForOutstanding(liveOutstanding, sourceCommand=ut-tdd handover)` for the response instead of trusting the stored pointer snapshot. The stored CURRENT.json remains the session pointer SSoT for active/latest/stale fields, but `outstanding`, `semanticFeatureFrontierRecords`, `confirmedCurrentMeaningRecords`, and `completionDecisionPacket` in status output are live so resume/preflight surfaces do not drop G-SF classifications or the confirmed-current 11 meaning units after an older handover was generated. Text output also prints Japanese action/route plus `action-id` / `route-id`, scoped primary packet command, scoped supporting packet commands, scoped supporting decision packet list, `semantic_frontier_records`, and `confirmed_current_meaning_records` so handover resume does not force the PO to infer the target PLAN or the confirmed feature-list catalog by hand. |
| `workflowNextActionForOutstanding` / `workflowNextActionsForOutstanding` | `(outstanding: OutstandingWork) => string` / `(outstanding: OutstandingWork) => WorkflowNextActionItem[]` | `outstanding.completionReadiness` is current | returns a machine-switchable top-level workflow guidance token for `ut-tdd status`: `completion-ready:` only when no outstanding work remains, otherwise `completion-blocked:` plus the highest-priority required action (PO/S4 decision before version-up activation before L14 cutover signoff before generic human/active-draft work). `workflowNextActionsForOutstanding` additionally returns the full ordered blocker queue with `order`, `planId`, `reason`, `blockers`, `decisionKind`, machine `requiredAction`, Japanese display `requiredActionJa`, machine `requiredActions`, Japanese display `requiredActionsJa`, `requiredEvidence`, machine `nextWorkflowRoute`, Japanese display `nextWorkflowRouteJa`, primary `decisionPacketCommand`, all supporting `packetCommands`, PLAN-scoped `scopedDecisionPacketCommand`, PLAN-scoped `scopedPacketCommands`, and `supportingPacketSummaries[]`, so top-action convenience cannot hide secondary version-up, cutover, or action-binding decisions and cannot hide the packet matrix/review contract behind a command string. Each `supportingPacketSummaries[]` item carries `command`, `schemaVersion`, `matrixField`, `expectedMatrixCount`, `requiredReviewFields`, machine `reviewRoute`, and Japanese display `reviewRouteJa`; approval-draft などの supporting packet でも `reviewRouteJa` は machine `reviewRoute` の英語文を流用しない。The text status surface prints `runtime-next:` for runtime/judgment guidance and `completion-next:` / `workflow-next:` for whole-program completion guidance separately, so `cross-review-ready` を PO/S4・version-up・L14 cutover blocker の解消済み表示に読み替えない。さらに各 action は `planId`、`reason`、日本語 `requiredActionJa`、`action-id`、日本語 `nextWorkflowRouteJa`、`route-id`、primary packet command、scoped primary packet command、supporting packet commands、scoped supporting packet commands、`packet-summary:` 行の日本語 `review` と `review-id` を出し、top action count だけにしない。handover Markdown の `packet要約` も `確認観点=` に日本語 `reviewRouteJa`、`確認観点ID=` に machine `reviewRoute` を出し、人間向け引き継ぎで英語 route だけが残る状態を許可しない。`semantic_frontier_records` と `confirmed_current_meaning_records` も出すため、PO 向け status は 4 件の pending semantic frontier と 43 item feature list 背後の confirmed 11 意味単位 catalog を隠さない。 `po_decision_pending` points to `ut-tdd s4 decision-packet --json --plan <PLAN_ID>`, `version_up_parked` to `ut-tdd version-up activation-packet --json --plan <PLAN_ID>`, `irreversible_migration_pending` to singleton `ut-tdd rename plan --json`, and `human_approval_pending` to `ut-tdd action-binding approval-packet --json --plan <PLAN_ID>`; generic active draft falls back to `ut-tdd completion decision-packet --json`. This is distinct from runtime `nextAction`; `status --json` keeps `nextAction` for backward compatibility and adds `runtimeNextAction` / `completionNextAction` aliases so hybrid cross-review guidance cannot mask PO/S4, version-up, or L14 cutover blockers. |
| `analyzeCompletionDecisionPacket` | `(packet: CompletionDecisionPacket, now?: string) => CompletionDecisionPacketLintResult` | packet may have been copied from status output, standalone command output, or stale external prose | accepts only packets whose `generatedFrom` is `outstanding.completionReadiness`, whose `ok` and `status` agree, whose `sourceCommand` is an allowed CLI source, whose freshness policy/window/expiry/stale flag are internally consistent, whose authority boundary fields (`authorityBoundary`, `humanDecisionRequired`, `humanDecisionBlockers[]`, `workflowStateBlockers[]`, `autonomousWorkBlockers[]`, `nextAuthority`) match the blocker set so PO/S4, action-binding, version-up activation, and irreversible cutover blockers cannot be hidden as routine automation work and `non_terminal_plans` cannot be misreported as direct automation work, whose `decisionCount` matches `decisions.length`, whose `semanticMeaningSummary` counts match top-level `semanticFeatureFrontierRecords[]` / `confirmedCurrentMeaningRecords[]`, whose semantic summary `completionClaimAllowed` agrees with packet `ok`, and whose semantic source paths include the L3 feature-list and L12 acceptance sources; whose decisions carry blocker-reason-aligned `decisionKind`, `decisionPacketCommand`, `packetCommands`, `scopedDecisionPacketCommand`, and `scopedPacketCommands`, whose scoped commands add `--plan <PLAN_ID>` for S4 / version-up / action-binding surfaces while keeping singleton rename/completion commands unscoped, whose Japanese PO display fields (`requiredActionJa`, `requiredActionsJa[]`, `nextWorkflowRouteJa`, `supportingPacketSummaries[].reviewRouteJa`) match the machine action / route IDs through the canonical display mapping, whose `requiredRecords[]` recordName set exactly matches the records implied by `blockerReason + blockers[]`, whose decisions carry structured `requiredRecords` with non-empty `recordName`, `fields`, and `sourcePaths`, whose `sourceLedgerChecks[]` entries point back to `sourcePaths[]` and, when repo text is available, cite a non-stale checked ledger label, and whose required records are covered by `allowedOutcomesByRecord` with non-empty outcomes, `nextWorkflowRoutesByRecord` with non-empty record-level return routes, and `recordTemplates` whose YAML block covers every required field; stale, unknown-source, malformed, hidden human authority boundary, missing semantic summary, semantic count drift, missing-packet-command, missing-scoped-packet-command, missing-record, missing-outcome, missing-route, missing-template, stale-source-ledger, Japanese display drift, or shape-drifted packets are fail-close doctor violations |
| `checkCompletionDecisionPacket` / `completionDedicatedPacketBridgeViolations` | `(repoRoot: string) => { messages: string[]; ok: boolean }` / `(repoRoot, packet) => string[]` | repo root is readable; completion packet lint has produced a packet whose commands must route to live dedicated packet surfaces | keeps `analyzeCompletionDecisionPacket` as packet-local lint and performs the repo-aware bridge in doctor to avoid circular imports from dedicated packet validators back into completion lint. For every completion decision `packetCommands[]`, doctor builds the live S4 / version-up / rename / action-binding packet for the referenced PLAN and runs the dedicated verification command validators; rename also runs the cutover runbook command validator. The bridge also checks live dedicated packet `relatedDecisionPackets[].scopedCommand`, requiring S4 / version-up / action-binding routes to bind `--plan <PLAN_ID>` and rename to stay singleton, so a completion packet cannot be green while the dedicated packet points reviewers at another PLAN. Missing live dedicated packets, missing/mis-scoped related packet commands, matrix command drift, runbook command drift, or validator failure become `completion-decision-packet` hard-gate violations, so a correct supporting summary declaration cannot hide a drifted or absent dedicated packet implementation. |
| `analyzeObjectiveEvidenceAudit` / `objectiveProgressForAudit` / `ut-tdd audit objective-external --json` | `(input: ObjectiveEvidenceAuditInput) => ObjectiveEvidenceAuditResult` / `(input, provedRows?) => ObjectiveProgress` / CLI JSON `{ ok, externalObserved, externalCheck, audit }` | active objective audit table and current `outstanding.completionReadiness` are readable; `objective-external` may call `git ls-remote` for the two referenced GitHub repositories | G-01..G-09 の evidence row が `proved` のまま残ること、G-10 が現在の completion readiness に追従すること、completion artifact が実在することを検査する。さらに objective audit は、開発レポ `unison-ai-product/UT-TDD_AGENT-HARNESS` と配布レポ `unison-ai-product/UT-TDD_AGENT-HARNESS-Pack` の HEAD/tag marker、外部 source ledger の `checked` freshness、`git ls-remote` command、ref、observed HEAD/tag、latestOfficialStatus、sourceStatusDelta、adoptionDecision、workflowRouteImpact、`package.json` version と local distribution tag、Pack latest tag、Pack latest 採用前の version-up activation requirement、常設日本語/adapter ルール (`CLAUDE.md` / `AGENTS.md` / design-language gate)、HELIX setup 実体、version-up readiness、PLAN-M-02 rename/cutover readiness の証跡 citation が落ちた場合に fail-close する。通常 doctor は network を呼ばず ledger 構造を検査し、`externalObserved` が渡された専用検証では実測 HEAD と Pack の semver 最大 tag が ledger observed と drift した場合も fail-close する。CLI `objective-external` はその専用検証を実行可能 surface として公開し、`externalObserved.development_repo` / `externalObserved.distribution_pack_repo` / `externalObserved.distribution_pack_latest_tag` に `git ls-remote` の実測値を直接返してから同じ audit に渡す。観測処理そのものの成否と message は `externalCheck` に分離し、`externalObserved.externalObserved` の二重ネストは evidence consumer が実測値階層を取り違えるため fail-close 対象とする。`distribution_pack_repo` の observed HEAD は Pack latest 採用ではなく参照 source ledger の現在値であり、drift 時は objective audit と consumer readiness の Pack reference を同じ観測値へ更新する一方、Pack latest tag / local distribution tag / package version の採用判断は version-up activation requirement のまま保持する。`objective-evidence-audit.v1` は percent、provedRequirements、totalRequirements、blockedRequirements、completionStatus、completionClaimAllowed、auditOk、auditViolationCount、progressEvidenceTrusted、evidenceTrustReason を返す。percent は evidence-row progress indicator であり completion claim ではない。audit が invalid の場合は percent を診断用として残すが `progressEvidenceTrusted=false`、`completionClaimAllowed=false` とし、status text は `objective-progress-evidence: invalid` を出して数値だけを完了証跡に使わせない。G-10 blocked 中は effective proved count を G-01..G-09 に cap し、status surface は 90% と `completionClaimAllowed=false` を出し続け、`outstanding.completionReadiness.ok=true` かつ audit が trusted になるまで L14 / whole-program completion を許可しない。 |

2026-07-03 source 再確認: Pack reference `distribution_pack_repo` の observed HEAD は `a13eb78a87dbbc1f60fa0b53e3a55413853c68b2`、Pack latest tag は `v0.1.4`。この HEAD 更新は参照 source ledger の更新であり、local distribution tag `v0.1.0` や package version `0.1.0` の採用変更ではない。Pack latest / newer Pack main の採用は引き続き version-up activation decision が必要。

2026-07-03 監査強化: `externalObserved` を `analyzeObjectiveEvidenceAudit` に渡す専用検証では、`development_repo`、`distribution_pack_repo`、`distribution_pack_latest_tag` の 3 source すべてを必須にする。通常 doctor は network を呼ばないが、networked source basis を使う経路では部分観測値を drift なしと扱わず、missing source として fail-close する。

| `analyzeSemanticFrontierConsistency` | `(input: SemanticFrontierConsistencyInput) => SemanticFrontierConsistencyResult` | L3 meaning-based feature list, L4-L6/L7-L12 frontier docs, and current `outstanding.semanticFeatureFrontierRecords[]` are readable | fail-closes when the L3 §0.2 meaning-based feature list drops a known frontier meaning unit, when live status/handover outstanding records omit, add, or misclassify records relative to `design_bottomup_mode`, `asset_progress_visualization`, `serverless_readonly_share`, and `name_cutover`, when any live record allows completion, or when its `sourcePaths[]` no longer points back to the L3 feature-list source. This prevents a changed requirement, parked version-up item, or rename cutover from being counted as complete by prose, doctor green, or a stale handover snapshot whose semantic frontier records no longer match the feature-list meaning. |
| `analyzeS4DecisionReadiness` / `buildS4DecisionPackets` | `(input: S4DecisionReadinessInput) => S4DecisionReadinessResult` / `S4DecisionPacket[]` | Discovery/Scrum mode docs, outstanding decision schema, current PLAN texts, and live `semanticFeatureFrontierRecords[]` are supplied | fail-closes when S3 pending PoC PLANs lack a structured `s4_decision_record` with `allowed_outcome`, `decision_owner`, `decision_basis`, `verified_evidence`, `stakeholder_review_or_proxy`, `acceptance_gap`, `unresolved_risk`, `external_source_basis`, `source_ledger_freshness` citing the current `S4 decision source ledger` checked date, `route_impact`, `forward_route`, `reverse_fullback_required`, and `promotion_strategy_or_rejection_pivot_rationale`; validates source ledgers and requires a matching `semantic_feature_frontier_record` with `classification=frontier_pending_decision`, `completionClaimAllowed=false`, and L3 source path, so S4 cannot promote, reject, or pivot a verified increment detached from the meaning-based feature list. When a PoC is already in `workflow_phase=S4` with `decision_outcome`, the record's `allowed_outcome` must be a single selected outcome equal to `decision_outcome`, not the enum list used by pending packet templates. Packet output is plan-only, carries `generatedAt`, `sourceCommand`, `freshness`, `semanticFeatureFrontierRecord`, and `relatedDecisionPackets[]`; each related route carries PLAN-scoped `scopedCommand` for S4/action-binding so packet evidence cannot be copied from another PLAN. `decisionVerificationCommandMatrix[]` は各 row に `sourceCheckedAt` / `latestOfficialStatus` / `sourceStatusDelta` / `adoptionDecision` / `adoptionDecisionDelta` / `workflowRouteImpact` を持たせ、Scrum Guide / requirements trace / test basis / SSDF の source を PO/S4 判断前 evidence に接続する。各 matrix command は `bun run src/cli.ts doctor`、targeted regression、static gates、full regression、status などの実行可能な承認済み verification surface に限定し、`run the PLAN-declared ...` のような自然文手順、prose-only command、未実装 command は `s4-decision-readiness` が fail-close する。matrix source metadata も stale/future/invalid `sourceCheckedAt` と placeholder source/adoption/route metadata を fail-close する; if the same S3 PLAN also has action-binding/human approval, the S4 packet keeps PLAN-scoped `ut-tdd action-binding approval-packet --json --plan <PLAN_ID>` as a supporting route |
| `analyzeVersionUpReadiness` / `buildVersionUpActivationPackets` | `(input: VersionUpReadinessInput) => VersionUpReadinessResult` / `VersionUpActivationPacket[]` | L0/L3/L4 version-up semantics, mode catalog/doc, Discovery S4 adoption note, current PLAN texts, and live `semanticFeatureFrontierRecords[]` are supplied | fail-closes when version-up parked PLANs cannot be distinguished from plain draft work, lack structured `activation_decision_record` / `parked_review_record`, or lack a matching `semantic_feature_frontier_record` with `classification=parked_future_version`, `completionClaimAllowed=false`, and L3 source path. Activation records must include `allowed_outcome`, `target_version_or_release_trigger`, `activation_snapshot_id`, `activation_route`, `review_by`, `approval_scope`, `dry_run_plan`, and `rollback_plan` so future-version work has a concrete release trigger, snapshot binding, and Forward/add-feature route before activation; external activation boundaries still require action-binding approval evidence. Pending parked records keep `allowed_outcome` as the full candidate set, but once `activation_snapshot_id` is a concrete `sha256:` approval snapshot, `allowed_outcome` must be a single selected activation outcome rather than the candidate enum. Packet output keeps primary version-up and supporting action-binding approval routes in `relatedDecisionPackets[]` and gives each route a PLAN-scoped `scopedCommand` so version-up / action-binding packet material cannot be taken from another PLAN; it carries `semanticFeatureFrontierRecord`, packet-level `generatedAt` / `sourceCommand` / `freshness` separately from `sourceLedgerFreshness`, exposes `activationSnapshot.headBound` / `materialBound` / `validationStatus` / `planTextDigest` so null-HEAD snapshots and same-HEAD changed PLAN material cannot be mistaken for approval-bound current snapshots, exposes `activationReadinessChecks[]`, and summarizes their `present` / `pending_evidence` state in `activationReadinessSummary` without ever flipping `activationAllowed=false`. `sourceLedgerFreshness.rowsDigest` and `activationSnapshot.sourceLedgerRowsDigest` bind the full Version-up source ledger row content, including SLSA Provenance v1.2 の artifact provenance row, so official URL/status/adoption/field-impact edits invalidate the snapshot, not only checked-date edits. Text output includes scoped related packets and snapshot `sourceLedgerRowsDigest` / `approvalScopeDigest` / `evidenceDigest` plus matrix `writePolicy` / `command`, so review cannot miss JSON-only safety fields. `activationVerificationCommandMatrix[]` validates both executable/write-policy surfaces and stale/future/invalid `sourceCheckedAt` plus placeholder source/adoption/route metadata before any activation review can proceed |
| `analyzeCutoverReadiness` / `buildIdentifierRenameCutoverPlan` | `(input: CutoverReadinessInput) => CutoverReadinessResult` / `IdentifierRenameCutoverPlan` | right-arm verification doc, outstanding decision schema, cutover source ledger, PLAN-M-02, current PLAN texts, and live `semanticFeatureFrontierRecords[]` are supplied | fail-closes when irreversible L14 cutover PLANs lack a structured `cutover_decision_record` with `allowed_outcome`, `decision_owner`, `cutover_snapshot_id`, `trigger_condition`, `blast_radius_baseline`, `dry_run_plan`, `rollback_plan`, `state_backup_plan`, `execution_window_or_freeze_policy`, `approval_scope`, `audit_record`, `post_cutover_monitoring`, `legacy_alias_policy`, and `source_ledger_freshness` citing the current `Cutover source ledger` checked date; validates the cutover source ledger and requires `semantic_feature_frontier_record featureId=name_cutover classification=approval_gated_cutover completionClaimAllowed=false` with L3 source path, so approval cannot be reused outside a frozen execution window, changed `cutoverSnapshot.snapshotId`, missing provenance/rollback/monitoring evidence, stale packet material, refreshed ledger material, or the L3 name-cutover meaning boundary. Identifier rename cutover plan remains plan-only / non-applying, carries packet-level `generatedAt` / `sourceCommand` / `freshness`, `semanticFeatureFrontierRecord`, and `relatedDecisionPackets[]` with primary singleton `ut-tdd rename plan --json` plus PLAN-M-02 scoped supporting `ut-tdd action-binding approval-packet --json --plan PLAN-M-02-helix-identifier-rename`; `verificationCommandMatrix[]` は baseline / targeted-regression / static-gates / state-and-doctor / full-regression / current-dist-smoke / renamed-helix-dist-smoke / post-cutover-consumer-setup-smoke / legacy-alias-smoke の各 row に `sourceCheckedAt` / `latestOfficialStatus` / `sourceStatusDelta` / `adoptionDecision` / `adoptionDecisionDelta` / `workflowRouteImpact` を持たせ、source 名や URL だけの approval review に戻さない。post-cutover-consumer-setup-smoke は承認後の `helix setup project --dry-run --json` が consumer readiness / artifactReadiness / importReport / PLAN-M-02 blocked boundary を出すことを、承認前は `rename dist-smoke --no-write --target helix --json` packet の `postCutoverConsumerSetupPreview` としてのみ扱う。matrix source metadata は stale/future/invalid `sourceCheckedAt` と placeholder source/adoption/route metadata を fail-close する; `sourceLedgerFreshness.rowsDigest` and `cutoverSnapshot.sourceLedgerRowsDigest` bind the full Cutover source ledger row content so date refresh, official URL/status/adoption/field-impact edits, and source row changes invalidate stale approval material through a changed `cutoverSnapshot.snapshotId`. `blastRadiusDigest` は本文 token だけでなく `.ut-tdd/**` などの path-only runtime state hit も束ね、内容に token がない state path 追加でも stale approval material を無効化する。`cutoverRunbook.evidencePath` と `stateBackupManifest.restoreEvidencePath` は `.ut-tdd/evidence/rename/` 配下の repo-local artifact path に限定し、URL・絶対 path・`..` traversal・自然文 prose を承認証跡として扱わない。Text output includes scoped related packets, `rowsDigest`, cutover snapshot `sourceLedgerRowsDigest` / `blastRadiusDigest` / `approvalScopeDigest` / `evidenceDigest`, and matrix `writePolicy` / `command` so review cannot miss JSON-only cutover safety fields |
| `analyzeActionBindingApprovalReadiness` / `buildActionBindingApprovalPackets` | `(input: ActionBindingApprovalReadinessInput) => ActionBindingApprovalReadinessResult` / `ActionBindingApprovalPacket[]` | right-arm verification doc, outstanding decision schema, version-up mode doc, current repo HEAD sha, current cutover snapshot id, live `semanticFeatureFrontierRecords[]`, and PLAN texts are supplied | fail-closes when high-impact non-terminal PLANs mention approval/action-binding/human approval but do not carry an `action_binding_approval_record` with `allowed_outcome`, `approval_policy_or_named_approver`, `approval_scope`, `approved_actor`, `approved_tool`, `approved_target`, `approved_params`, `review_approval_evidence`, `reviewed_snapshot_binding`, `expires_at_or_trigger`, and `audit_record`; validates the right-arm design markers and outstanding requiredEvidence so approval boundaries cannot remain prose-only, reuse stale activation/cutover snapshot material, collapse actor/tool/target/params into an uncheckable prose scope, or detach sibling S4/version-up/rename blockers from their semantic frontier record. Version-up approvals must cite the current `activationSnapshot.snapshotId` computed with the current `repoHeadSha`; if `repoHeadSha` is unavailable, activation snapshot validation is pending and the implementation must not synthesize a null-HEAD expected snapshot. Rename/cutover approvals must cite the current `cutoverSnapshot.snapshotId`; field-name-only, stale sha256, or unvalidated activation bindings stay pending/invalid. `review_approval_evidence` と `audit_record` は repo-local artifact path、digest / run id、または GitHub Actions run / PR / commit URL のような監査可能 locator に限定し、任意の外部 URL や自由文 claim を concrete approval evidence として扱わない。Packet output is plan-only, carries packet-level `generatedAt` / `sourceCommand` / `freshness`, `approvalBindingChecks[]`, `semanticFeatureFrontierRecords[]` for sibling blockers, `approvalVerificationCommandMatrix[]` with executable allowlisted commands only, `sourceCheckedAt` / `latestOfficialStatus` / `sourceStatusDelta` / `adoptionDecision` / `adoptionDecisionDelta` / `workflowRouteImpact`, and `relatedDecisionPackets[]` back to sibling S4, version-up, or rename/cutover packets. Related packets include `scopedCommand`: primary action-binding plus S4/version-up siblings are PLAN-scoped with `--plan <PLAN_ID>`, rename remains singleton, and natural-language `review ...` / `verify ...` commands are fail-close violations rather than approval evidence. Shared matrix metadata validation rejects stale/future/invalid `sourceCheckedAt` and placeholder source/adoption/route metadata so approval review cannot rely on old official-source assumptions |
...
| `computeUtHistorySignals` | `(input: UtHistoryInput, deps: HarnessDbDeps) => QualitySignal[]` | test run/result rows are normalized; zero denominators are explicit | computes oracle coverage, plan green rate, green-definition compliance, oracle-level flake score (same oracle has pass and fail history), and duration regression (latest duration exceeds prior median by configured ratio); non-green signals join `findings` |
| `projectUtHistorySignals` | `(input: UtHistoryInput, deps: HarnessDbDeps) => ProjectionRowRef[]` | `computeUtHistorySignals` の入力が正規化済みで、DB は `.ut-tdd/` 配下または test 用 `:memory:` で開かれている | aggregate UT history signal を `quality_signals` へ、pass/fail が揺れる oracle を `test_flake_events` へ、原因 oracle の flake / duration regression を `quality_signals(source=ut-history)` へ冪等 upsert する。加えて oracle/run 時点ごとの duration を `quality_signals(metric=duration_trend_ms)` として保存し、`value=duration_ms`、`threshold=prior median`、`computed_at=run completed_at` で trend を再構成可能にする |
...
- U-GREENCMD-001..003 cover green command digest substance: fake/stale digest detection, unreadable repo fail-close, and doctor hard-gate wiring. U-GREENDEF-005..006 cover green command kind/command meaning consistency and 64-hex digest schema in both schema and review-evidence lint.
...
|---|---|---|---|---|---|
| `catalogVerificationProfiles` | catalogVerificationProfiles(input: VerificationProfileCatalogInput) => VerificationProfileCatalogResult | built-in profiles and researched external candidates are supplied with source URL, package reference, trigger signals, risk fields, and right-arm source-ledger bindings. | returns deterministic profile rows including Docker MCP Toolkit, MCP Inspector, Playwright MCP, GitHub read-only MCP, Vitest browser, Testcontainers, and MSW; profiles that recommend G8-G14 carry `sourceLedgerSources` and `sourceLedgerCheckedAt` tied to the Verification source ledger. | external profiles are disabled by default and are discovery/config metadata, not trusted execution; right-arm profile recommendations cannot rely on package names alone and must preserve the external source meaning that justifies each gate. | U-MCPPROFILE-001..003, U-MCPPROFILE-018 |
...
| `runVerificationProfile` / `PROFILE_RUNNERS` | runVerificationProfile(id: string, opts: VerificationRunOptions, deps: VerificationRunDeps) => VerificationRunResult \| null | selected profile id is known; default-enabled runner must be executable without network or external service activation. | runs only allow-listed built-in/local runners by default; `bun-unit` invokes local regression (`bun run test:local`) and excludes clean distribution acceptance that may perform dependency download/cache access. | default runnable profile evidence cannot be conflated with full regression/release evidence; `bun run test` remains a stronger full-suite command for release/cutover/activation contexts where network/cache prerequisites are available or explicitly approved. | U-MCPPROFILE-002, U-MCPPROFILE-012, U-MCPPROFILE-013 |
...
| `analyzeRightArmVerificationProfileCoverage` | analyzeRightArmVerificationProfileCoverage(profiles: VerificationProfile[]) => VerificationProfileGateCoverageResult | verification profiles are supplied with recommended gates, drive mappings, and source-ledger bindings. | returns coverage by G8-G14 and drive-model mode, plus findings for missing/stale/unknown source-ledger bindings or source rows whose gate impact does not cover the recommended gates. | a G8-G14 profile is valid only when its binding date matches the current Verification source ledger, every source row is known, and the union of source row `gate impact` values covers the profile's recommended gates. | U-MCPPROFILE-015..018 |
...
| `recordTestRunEvidence` | `TestRunEvidenceInput { command; runner; scope; started_at; completed_at; exit_code; evidence_path; cases? } -> RecordTestRunEvidenceResult { refs[] }` | implemented in `src/workflow/contracts.ts`; collect Bun/vitest/doctor/lint evidence into UT history projection, preserve case duration/timestamps/failure digest for later history analysis, never persist provider transcripts as raw evidence |
...
| `computeUtHistorySignals` | `UtHistoryInput { plan_id?; window?; duration_regression_ratio? } -> ComputeUtHistorySignalsResult { signals[] }` | implemented in `src/workflow/contracts.ts`; compute oracle coverage, plan green rate, oracle-level flake score, duration regression against prior median, and green-definition compliance |
| `projectUtHistorySignals` | `UtHistoryInput { plan_id?; window?; duration_regression_ratio? } -> ProjectUtHistorySignalsResult { refs[]; signals[] }` | implemented in `src/workflow/contracts.ts`; project aggregate history metrics into `quality_signals`, plan-scoped flake windows into `test_flake_events`, oracle-scoped `test_cases`, oracle-scoped flake/duration regression signals, and oracle/run-scoped `duration_trend_ms` signals for feedback and trend inspection |
| `parseGreenCommandEvidence` / `projectStructuredGreenCommandCaseEvidence` / `projectUtHistorySignalsFromProjectedRuns` | `review_evidence.green_commands[].evidence_path -> test_results -> ut-history signals` | implemented in `src/state-db/test-report-parser.ts` and `src/state-db/projection-writer.ts`; structured `cases[]` JSON、Vitest/Jest-compatible JSON、Playwright JSON、JUnit XML を正規化し、rebuild が case/result rows を投影してから DB から UT history を復元する |
...
| `extractShellCommand` / `evaluateGitCommandGuard` / `resolveDestructiveGitOverride` (git-command-guard) | `evaluateGitCommandGuard({ command, bypass? }) => GitCommandGuardResult` | `command` は Claude `tool_input.command` / Codex `tool_input.cmd` / 文字列 payload から抽出済み。hook I/O は `.claude/hooks/git-command-guard.ts` または `ut-tdd hook git-command-guard` が担う | hybrid 多ランタイムで相手 runtime の commit / branch を破壊し得る `git reset`、destructive `git checkout`、`git restore`、`git revert`、`git push --force` / `--force-with-lease` を `decision=block` / `reason=destructive-git` で fail-close する。`git status` / `diff` / `log` / 通常 `push` / `checkout -b` は pass。override は `UT_TDD_ALLOW_DESTRUCTIVE_GIT=1` または `.ut-tdd/state/destructive-git-override` の非空理由のみで、marker は one-shot 消費して `.ut-tdd/logs/destructive-git-overrides.jsonl` に audit する。Claude `PreToolUse(Bash)` と Codex `PreToolUse(exec_command\|local_shell)` の双方で同じ純関数を使い、IMP-142 の reset/revert/checkout/force-push 再発を運用記憶でなく機械で止める |
