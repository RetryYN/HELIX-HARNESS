...
| `analyzeToolchainPin` / `checkToolchainPin` | `(input: ToolchainPinInput) => ToolchainPinResult` / `(repoRoot: string) => { messages: string[]; ok: boolean }` | `package.json`、`package-lock.json`、source workflow と consumer workflow template が読める | `package.json.engines.node` が concrete semver rangeを持つこと、source packageのlockfileが存在すること、GitHub Actions / templateのinstallが`npm ci`であることをfail-closeする。source `harness-check.yml`の全`actions/setup-node` stepを検査し、PLAN-L7-551の移行中はexact `@v4` / `@v7`だけを受理し、未許可・ref未固定・許可refとの混在を`U-TOOLCHAIN-PIN-005/006`で拒否する。許可refの`node-version`はNode engine floorと一致させる。doctorは`toolchain-pin`をhard gateとして実行し、`lint-wiring`はruntime entrypointからの到達性を検査する。#596 merge後のmain green確認時にv4 admissionを退役させる |
