...
| `completionDecisionPacketForOutstanding` | `(outstanding: OutstandingWork, opts: CompletionDecisionPacketOptions) => CompletionDecisionPacket` | `outstanding.completionReadiness` was computed from current non-terminal PLAN rows / open defers; `opts.generatedAt` and `opts.sourceCommand` identify the packet creation event | returns a decision packet derived from `outstanding.completionReadiness`, preserving `ok/status`, blocker list, decision count, PLAN-level decision items, allowed outcomes, next workflow route, source command, and freshness metadata (`decision-packet-freshness.v1`, `validForMinutes`, `expiresAt`, `stale`); each item keeps a primary `requiredAction` for routing and unioned `requiredActions` / `requiredEvidence` for every blocker so secondary human/action-binding approvals are not hidden; `po_decision_pending` is limited to PoC S3/S4/S4-decision context and generic PO signoff stays under human/cutover/version-up blockers; doctor green is not used as a substitute for readiness |
