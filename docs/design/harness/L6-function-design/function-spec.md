...
| `analyzeObjectiveEvidenceAudit` / `objectiveProgressForAudit` | `(input: ObjectiveEvidenceAuditInput) => ObjectiveEvidenceAuditResult` / `(input, provedRows?) => ObjectiveProgress` | active objective audit table and current `outstanding.completionReadiness` are readable | G-01..G-09 の evidence row が `proved` のまま残ること、G-10 が現在の completion readiness に追従すること、completion artifact が実在することを検査する。さらに objective audit は、開発レポ `unison-ai-product/UT-TDD_AGENT-HARNESS` と配布レポ `unison-ai-product/UT-TDD_AGENT-HARNESS-Pack` の HEAD/tag marker、常設日本語/adapter ルール (`CLAUDE.md` / `AGENTS.md` / design-language gate)、HELIX setup 実体、version-up readiness、PLAN-M-02 rename/cutover readiness の証跡 citation が落ちた場合に fail-close する。`objective-evidence-audit.v1` は percent、provedRequirements、totalRequirements、blockedRequirements、completionStatus、completionClaimAllowed を返す。percent は evidence-row progress indicator であり completion claim ではない。G-10 blocked 中は effective proved count を G-01..G-09 に cap し、status surface は 90% と `completionClaimAllowed=false` を出し続け、`outstanding.completionReadiness.ok=true` まで L14 / whole-program completion を許可しない。 |
