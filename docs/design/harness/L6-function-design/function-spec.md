...
| `workflowNextActionForOutstanding` / `workflowNextActionsForOutstanding` | `(outstanding: OutstandingWork) => string` / `(outstanding: OutstandingWork) => WorkflowNextActionItem[]` | `outstanding.completionReadiness` is current | returns a machine-switchable top-level workflow guidance token for `ut-tdd status`: `completion-ready:` only when no outstanding work remains, otherwise `completion-blocked:` plus the highest-priority required action (PO/S4 decision before version-up activation before L14 cutover signoff before generic human/active-draft work). `workflowNextActionsForOutstanding` additionally returns the full ordered blocker queue with `order`, `planId`, `reason`, `blockers`, `decisionKind`, `requiredAction`, `requiredActions`, `requiredEvidence`, `nextWorkflowRoute`, and `decisionPacketCommand`, so top-action convenience cannot hide secondary version-up, cutover, or action-binding decisions. This is distinct from runtime `nextAction` so hybrid cross-review guidance cannot mask PO/S4, version-up, or L14 cutover blockers. |
| `analyzeCompletionDecisionPacket` | `(packet: CompletionDecisionPacket, now?: string) => CompletionDecisionPacketLintResult` | packet may have been copied from status output, standalone command output, or stale external prose | accepts only packets whose `generatedFrom` is `outstanding.completionReadiness`, whose `ok` and `status` agree, whose `sourceCommand` is an allowed CLI source, whose freshness policy/window/expiry/stale flag are internally consistent, whose `decisionCount` matches `decisions.length`, whose decisions carry structured `requiredRecords` with non-empty `recordName`, `fields`, and `sourcePaths`, and whose required records are covered by `allowedOutcomesByRecord` with non-empty outcomes, `nextWorkflowRoutesByRecord` with non-empty record-level return routes, and `recordTemplates` whose YAML block covers every required field; stale, unknown-source, malformed, missing-record, missing-outcome, missing-route, missing-template, or shape-drifted packets are fail-close doctor violations |
