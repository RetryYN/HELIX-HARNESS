...
2026-07-02 source refresh: Pack reference `distribution_pack_repo` の observed HEAD は `b9a42df867ceadbd24fc8e0a50ad756da0591c59`、Pack latest tag は `v0.1.3`。この HEAD 更新は参照 source ledger の更新であり、local distribution tag `v0.1.0` や package version `0.1.0` の採用変更ではない。Pack latest / newer Pack main の採用は引き続き version-up activation decision が必要。
