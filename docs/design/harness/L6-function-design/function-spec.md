...
| `workflowNextActionForOutstanding` / `workflowNextActionsForOutstanding` | `(outstanding: OutstandingWork) => string` / `(outstanding: OutstandingWork) => WorkflowNextActionItem[]` | `outstanding.completionReadiness` is current | returns a machine-switchable top-level workflow guidance token for `ut-tdd status`: `completion-ready:` only when no outstanding work remains, otherwise `completion-blocked:` plus the highest-priority required action (PO/S4 decision before version-up activation before L14 cutover signoff before generic human/active-draft work). `workflowNextActionsForOutstanding` additionally returns the full ordered blocker queue with `order`, `planId`, `reason`, `blockers`, `decisionKind`, `requiredAction`, `requiredActions`, `requiredEvidence`, `nextWorkflowRoute`, primary `decisionPacketCommand`, and all supporting `packetCommands`, so top-action convenience cannot hide secondary version-up, cutover, or action-binding decisions. The text status surface prints each ordered action with `planId`, `reason`, `requiredAction`, `nextWorkflowRoute`, primary packet command, and supporting packet commands, not only the top action count. `po_decision_pending` points to `ut-tdd s4 decision-packet --json`, `version_up_parked` to `ut-tdd version-up activation-packet --json`, `irreversible_migration_pending` to `ut-tdd rename plan --json`, and `human_approval_pending` to `ut-tdd action-binding approval-packet --json`; generic active draft falls back to `ut-tdd completion decision-packet --json`. This is distinct from runtime `nextAction` so hybrid cross-review guidance cannot mask PO/S4, version-up, or L14 cutover blockers. |
