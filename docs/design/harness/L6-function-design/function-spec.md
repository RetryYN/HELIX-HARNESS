...
| `completionDecisionPacketForOutstanding` | `(outstanding: OutstandingWork, opts: CompletionDecisionPacketOptions) => CompletionDecisionPacket` | `outstanding.completionReadiness` was computed from current non-terminal PLAN rows / open defers; `opts.generatedAt` and `opts.sourceCommand` identify the packet creation event | returns a decision packet derived from `outstanding.completionReadiness`, preserving `ok/status`, blocker list, decision count, PLAN-level decision items, primary allowed outcomes, primary next workflow route, source command, and freshness metadata (`decision-packet-freshness.v1`, `validForMinutes`, `expiresAt`, `stale`); each item keeps a primary `requiredAction` for routing and unioned `requiredActions` / `requiredEvidence` / `requiredRecords` for every blocker so secondary human/action-binding approvals are not hidden; each `requiredRecords[]` entry has `recordName`, non-empty `fields`, and non-empty `sourcePaths` pointing to the owning workflow design source, and each record has matching `allowedOutcomesByRecord[]` and `nextWorkflowRoutesByRecord[]` entries so S4 / version-up / parked review / cutover / action-binding outcomes and return routes cannot disappear behind the primary blocker; `po_decision_pending` is limited to PoC S3/S4/S4-decision context and generic PO signoff stays under human/cutover/version-up blockers; doctor green is not used as a substitute for readiness |
| `workflowNextActionForOutstanding` | `(outstanding: OutstandingWork) => string` | `outstanding.completionReadiness` is current | returns a machine-switchable top-level workflow guidance token for `ut-tdd status`: `completion-ready:` only when no outstanding work remains, otherwise `completion-blocked:` plus the highest-priority required action (PO/S4 decision before version-up activation before L14 cutover signoff before generic human/active-draft work). This is distinct from runtime `nextAction` so hybrid cross-review guidance cannot mask PO/S4, version-up, or L14 cutover blockers. |
| `analyzeCompletionDecisionPacket` | `(packet: CompletionDecisionPacket, now?: string) => CompletionDecisionPacketLintResult` | packet may have been copied from status output, standalone command output, or stale external prose | accepts only packets whose `generatedFrom` is `outstanding.completionReadiness`, whose `ok` and `status` agree, whose `sourceCommand` is an allowed CLI source, whose freshness policy/window/expiry/stale flag are internally consistent, whose `decisionCount` matches `decisions.length`, whose decisions carry structured `requiredRecords` with non-empty `recordName`, `fields`, and `sourcePaths`, and whose required records are covered by `allowedOutcomesByRecord` with non-empty outcomes and `nextWorkflowRoutesByRecord` with non-empty record-level return routes; stale, unknown-source, malformed, missing-record, missing-outcome, missing-route, or shape-drifted packets are fail-close doctor violations |
| `analyzeS4DecisionReadiness` | `(input: S4DecisionReadinessInput) => S4DecisionReadinessResult` | Discovery/Scrum mode docs, outstanding decision schema, and current PLAN texts are supplied | fail-closes when S3 pending PoC PLANs lack a structured `s4_decision_record` with `allowed_outcome`, `decision_owner`, `decision_basis`, `verified_evidence`, `stakeholder_review_or_proxy`, `acceptance_gap`, `unresolved_risk`, `external_source_basis`, `route_impact`, `forward_route`, `reverse_fullback_required`, and `promotion_strategy_or_rejection_pivot_rationale`; validates source ledgers so S4 cannot promote, reject, or pivot a verified increment without a concrete Forward/backlog route |
| `analyzeVersionUpReadiness` | `(input: VersionUpReadinessInput) => VersionUpReadinessResult` | L0/L3/L4 version-up semantics, mode catalog/doc, Discovery S4 adoption note, and current PLAN texts are supplied | fail-closes when version-up parked PLANs cannot be distinguished from plain draft work or lack structured `activation_decision_record` / `parked_review_record`; activation records must include `allowed_outcome`, `target_version_or_release_trigger`, `activation_route`, `review_by`, `approval_scope`, `dry_run_plan`, and `rollback_plan` so future-version work has a concrete release trigger and Forward/add-feature route before activation; external activation boundaries still require action-binding approval evidence |
| `analyzeCutoverReadiness` | `(input: CutoverReadinessInput) => CutoverReadinessResult` | right-arm verification doc, outstanding decision schema, cutover source ledger, and current PLAN texts are supplied | fail-closes when irreversible L14 cutover PLANs lack a structured `cutover_decision_record` with `allowed_outcome`, `decision_owner`, `trigger_condition`, `blast_radius_baseline`, `dry_run_plan`, `rollback_plan`, `state_backup_plan`, `execution_window_or_freeze_policy`, `approval_scope`, `audit_record`, `post_cutover_monitoring`, and `legacy_alias_policy`; validates the cutover source ledger so approval cannot be reused outside a frozen execution window or without provenance/rollback/monitoring evidence |
| `analyzeActionBindingApprovalReadiness` | `(input: ActionBindingApprovalReadinessInput) => ActionBindingApprovalReadinessResult` | right-arm verification doc, outstanding decision schema, and PLAN texts are supplied | fail-closes when high-impact non-terminal PLANs mention approval/action-binding/human approval but do not carry an `action_binding_approval_record` with `allowed_outcome`, `approval_policy_or_named_approver`, `approval_scope`, `approved_actor`, `approved_tool`, `approved_target`, `approved_params`, `review_approval_evidence`, `expires_at_or_trigger`, and `audit_record`; validates the right-arm design markers and outstanding requiredEvidence so approval boundaries cannot remain prose-only or collapse actor/tool/target/params into an uncheckable prose scope |
