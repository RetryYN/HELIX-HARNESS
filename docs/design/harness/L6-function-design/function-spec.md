...
| FR-L1-23 | `routeProductionScrumSlice` | routeProductionScrumSlice(input: ProductionScrumSliceInput, deps: ProductionScrumSliceDeps) => ProductionScrumSliceResult | `selected_style=PRODUCTION_SCRUM`、increment evidence、slice acceptance、対応V-pair evidenceが存在する。 | 次sliceまたはrelease candidateと、未充足の正規pairを返す。 | Scrum sliceの完了へDiscovery／PoCのS4 decisionやReverse fullbackを無条件要求しない。既存assetのback-fillが必要な場合だけ別軸Reverse specialistへ明示routeする。 | U-FR-L1-23 |
