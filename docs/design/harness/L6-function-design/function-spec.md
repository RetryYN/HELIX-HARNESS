...
| `projectUtHistorySignals` | `(input: UtHistoryInput, deps: HarnessDbDeps) => ProjectionRowRef[]` | `computeUtHistorySignals` の入力が正規化済みで、DB は `.ut-tdd/` 配下または test 用 `:memory:` で開かれている | aggregate UT history signal を `quality_signals` へ、pass/fail が揺れる oracle を `test_flake_events` へ、原因 oracle の flake / duration regression を `quality_signals(source=ut-history)` へ冪等 upsert する。長期 duration trend 永続化は別 slice |
