...
dual-green surfaceであり、current projection／completion判定の正本にしない。
...
| `buildVisualizationSnapshot` | `(db: HarnessDb) => VisualizationSnapshot` | migrated harness.db が利用可能で、caller は read-only の CLI / VSCode View / Webview adapter のいずれかでよい | 既存 projection table から決定論的な `visualization-snapshot.v1` 初回応答を返す。artifact progress color、plan/gate status、relation graph counts/latest hash、test runs、runtime verification classes、skill/model/guardrail metrics、drill-down command/table pointer を含む。cold-start は warning 付き 0 値で、accepted runtime verification は `verification_class='runtime_verified' AND accept_status='accepted'` のみ。Project view 向け recovery handoff artifact は `closure evidence-probe` / `closure evidence-approval-draft` に加えて `close_ready` の `closure decision-draft` local artifact を検出し、`pending_human_review` / `approve_closure_claim` / rejection outcome と closure review の `approval_scope_digest` 一致を read-only に表示する。`vmodel fit` / Project view の recovery handoff gate はこの decision draft を approval lane として投影するが、承認や `closure apply` は実行しない。 |
| `missingDraftApprovalHandoffViolations` / `requiresPublishedApprovalHandoff` / `checkRecoveryHandoffBinding` | `(handoff) => string[]` / `(handoff) => boolean` / `(repoRoot, db?) => DoctorCheck` | recovery handoff projection とDB summaryが生成済み | decision draft未生成を示す`handoff.decision_draft.missing`はfresh cloneでも成立する正常な`approval_required` frontierとして扱い、draft生成commandを案内する。ただしmissing frontier自身にも`status=approval_required`、missing/present相互排他、approval/scope/digest/review未確定、`valid_for_apply=false`、decision-draft生成commandをfail-closeで要求する。approval status・scope digest・reviewed candidate等の公開済みapproval invariantはdraft公開後にhard gate化する。DB projectionとの不一致、公開済みdraftのscope/digest不整合、pending中のapply許可は常にfail-closeする。 |
...
| `loadRelationGraphSourceSet` | `(repoRoot: string) => RelationGraphSourceSet` | repo root 配下の docs/src/tests/config が読める。個別 root 不在や optional file 不在は fail-open とする | 既存 design/test/source/PLAN node に加え、`docs/adr/**/*.md`、`docs/governance/document-system-map.md`、`docs/skills/**/*.md`、`.codex/hooks.json` を design node として materialize する。`docs/design/design-catalog.yaml` は既存graph内のcatalog rootとしてitem、登録artifact、reviewed digest authority、design-coverage source/test、既存PLANへ型付きedgeを張る。artifact欠落、未登録design doc、digest missing/staleを別findingとしてfail-closeし、semantic digestを自動更新しない。`isGraphTrackedPath` も同じ path class を graph 対象にし、対象 path の node 欠落は `missing-projection`、対象外 path は `non-graph-path` として区別する |

Design Catalog relation projectionの受入oracleは`U-RELGRAPH-012`（typed edgeとimpact）および
`U-RELGRAPH-013`（artifact／登録／semantic digest driftの分類）とする。

| Oracle ID | 契約 |
| --- | --- |
| U-RELGRAPH-012 | catalog root、item、artifact、reviewed digest authority、coverage oracle、既存PLANのtyped edgeと非空impactを返す。 |
| U-RELGRAPH-013 | artifact削除、未登録design doc、reviewed digest missing/staleを別findingとしてfail-closeする。 |
...
| `completionReviewBundleForOutstanding` / `analyzeCompletionReviewBundle` | `(outstanding: OutstandingWork, opts?: CompletionDecisionPacketOptions) => CompletionReviewBundle` / `(bundle, decisionPacket, now?) => CompletionReviewBundleLintResult` | completion decision packet と supporting packet summary が現在状態から生成済み | L14 完了主張前に確認する非破壊の `completion-review-bundle.v1` を返す。bundle は `sourceCommand=helix completion review-bundle --json` と `runnableSourceCommand=npm run helix -- completion review-bundle --json`、固定 safety flag (`planOnly=true` / `mustNotDecide=true` / `mustNotApply=true`)、completion decision packet command と runnable command、exact / semantic digest、review packet count、blockedUntil、日本語 operator action、S4 / version-up / rename / action-binding supporting packet ごとの scoped review packet 行を持つ。さらに `reviewCoveredBlockers` と `nonPacketBlockers` を分け、review packet で確認できる human decision blocker と、packet だけでは閉じない workflow state / semantic frontier blocker を混同させない。Project closure `review-bundle --summary-json` は completion frontier から辿る read-only window surface として `approval_review_checklist` を返し、`approval_required`、`approval_allowed`、`must_not_apply=true`、`non_authorizing=true`、digest / window scope / evidence totals / blocker / non-authorizing decision record route の `required_checks[]`、承認 route command、postcheck command を summary 上で機械検出できるようにする。`completion review-bundle` text と `status` text は `completion-review-coverage:` を出す。`review-packet:` 行は日本語 `route=` と machine `route-id=` を分離し、JSON の `reviewRouteJa` / `reviewRoute` と同じ二重表現を保つ。`analyzeCompletionReviewBundle` は freshness stale、自己 source runnable command の欠落・drift、safety flag drift、completion state drift、review coverage drift、review packet count drift、review packet drift、digest drift を fail-close し、bundle を生成した repo-local 再生成 command なしに human review bundle を転記できないようにする。 |
| continuation live outstanding projection | `helix status [--json]` | DB projectionはevent frontierと整合し、optional memory entryはDB stateを上書きしない | read-only status output は`harness.db` continuation projectionからactive PLAN、blocker、next authority、feedbackを読み、`computeOutstandingWork(repoRoot)` とcompletion packetを現在状態から再計算する。DB/memory矛盾はDBを表示してdiagnosticを生成し、旧snapshotやproseへfallbackしない |
...
| `normalizeProviderEffort` | `(provider: AdapterProvider, effort?: string) => string \| undefined` | provider は `claude` / `codex` の adapter provider、effort は user/team/model policy 由来の任意文字列である | `middle` を `medium`、空白・大文字小文字を吸収する。current Codex native workerの `xhigh` は `xhigh` のまま `model_reasoning_effort` argvへ投影し、Claude側だけ歴史的 compatibility aliasとして `high` へ正規化する。`buildAdapterPlan` は provider別の正規化後値からargvと必要なenvを作る。未知値は trim/lowercaseしたまま返し、schema側のmodel override hardeningと責務を重複しない |
| `analyzeToolchainPin` / `checkToolchainPin` | `(input: ToolchainPinInput) => ToolchainPinResult` / `(repoRoot: string) => { messages: string[]; ok: boolean }` | `package.json`、`package-lock.json`、source workflow と consumer workflow template が読める | `package.json.engines.node` が concrete semver rangeを持つこと、source packageのlockfileが存在すること、GitHub Actions / templateのinstallが`npm ci`であることをfail-closeする。source `harness-check.yml`の全`actions/setup-node` stepを検査し、PLAN-RECOVERY-57の移行中はexact `@v4` / `@v7`だけを受理し、未許可・ref未固定・許可refとの混在を`U-TOOLCHAIN-PIN-005/006`で拒否する。許可refの`node-version`はNode engine floorと一致させる。doctorは`toolchain-pin`をhard gateとして実行し、`lint-wiring`はruntime entrypointからの到達性を検査する。#596 merge後のmain green確認時にv4 admissionを退役させる |
| `projectRuntimeModelTelemetry` | `(db: HarnessDb) => void` | rebuild 済みの harness DB を受け取り、runtime transcript directory は `HELIX_CLAUDE_SESSIONS_DIR` / `HELIX_CODEX_SESSIONS_DIR` または OS default から読む | provider を起動せず既存 Claude/Codex JSONL log を scan し、token/cost-backed `model_runs` row を overlay する。deterministic `db rebuild` は source-only のまま。**doctor の gate 経路からは呼ばない**（Issue #495: overlay を観測する gate が存在せず、home 履歴サイズに比例して doctor が単調に遅くなるため）。恒久 ingest は `helix telemetry scan` が担う |
...
| `analyzeLintWiring` function reachability | analyzeLintWiring(input: LintWiringInput) => LintWiringResult | runtime entrypointから到達するfile集合と参照export集合が与えられる | 必須contract exportがruntime到達sourceから参照されなければ`unwiredExports`でfail-closeする | file importやre-exportだけを実行配線の証拠にしない | U-WIRING-001..002 |
| `classifyOutstandingBlockers` typed cutover boundary | `irreversible_impact: none \| cutover \| migration`を持つPLAN rowを分類する | typed fieldがあれば本文より優先する | `none`は境界語への言及をcutover blockerにせず、`cutover`/`migration`は本文非依存でblockerにする | field欠落の既存PLANだけlegacy regex fallbackを許容する | U-WIRING-003..004 |
| `runVerificationProfile` / `PROFILE_RUNNERS` | runVerificationProfile(id: string, opts: VerificationRunOptions, deps: VerificationRunDeps) => VerificationRunResult \| null | selected profile id は既知で、default-enabled runner は network や external service activation なしで実行可能でなければならない | 既定では allow-listed built-in/local runner だけを実行する。`node-unit` は local regression (`npm run test:local`) を呼び、dependency download/cache access があり得る clean distribution acceptance は含めない | default runnable profile evidence を full regression/release evidence と混同しない。`npm test` は release/cutover/activation context 向けのより強い full-suite command として残す | U-MCPPROFILE-002, U-MCPPROFILE-012, U-MCPPROFILE-013 |
...
| FR-L1-23 | `routeProductionScrumSlice`（compatibility input alias: `routeScrumFullback`） | routeProductionScrumSlice(input: ProductionScrumSliceInput, deps: ProductionScrumSliceDeps) => ProductionScrumSliceResult | `selected_style=PRODUCTION_SCRUM`、increment evidence、slice acceptance、対応V-pair evidenceが存在する。旧aliasはimport識別だけに使う。 | 次sliceまたはrelease candidateと、未充足の正規pairを返す。旧aliasはcurrent outputへ返さない。 | Scrum sliceの完了へDiscovery／PoCのS4 decisionやReverse fullbackを無条件要求しない。既存assetのback-fillが必要な場合だけ別軸Reverse specialistへ明示routeする。 | U-FR-L1-23 |
...
| `routeProductionScrumSlice` | `ProductionScrumSliceInput { selected_style: "PRODUCTION_SCRUM"; increment; slice_acceptance; v_pair_evidence[] } -> ProductionScrumSliceResult extends ContractResult { next_slice_or_release_candidate; missing_pairs[]; specialist_reentry? }` | current contract。Scrum incrementをcase-driven S4から独立して判定し、既存asset back-fillが必要な場合だけ`specialist_reentry="Reverse"`を返す。runtime cutoverはIssue #248で行い、それまでは旧実装をcurrent completion authorityに使わない |
| `routeScrumFullback` | `LegacyScrumFullbackInput { legacy_payload } -> ProductionScrumSliceInput { selected_style: "PRODUCTION_SCRUM"; increment; slice_acceptance; v_pair_evidence[] }` | compatibility input adapter。`explicit_l7_defer`; pseudocode = legacy payloadを検証してcurrent入力へ一方向変換し、`routeProductionScrumSlice`を呼ぶ。旧aliasをcurrent output、DB projection、completion evidenceへ出力しない |
...
| `buildAdapterPlan` Claude permission mode | `execute=true`のClaude planだけが`--permission-mode auto`を固定argvへ追加し、dry-runは追加しない。`bypassPermissions`と`--dangerously-skip-permissions`は全経路で生成しない | `U-ADAPTER-013` |
...
| 関数 | signature | pre | post |
|---|---|---|---|
| `buildProjectCurrentLocationSnapshot` / `buildProjectClosureOverview` / `buildProjectClosureBatchReport` | `(db: HarnessDb) => ProjectCurrentLocationSnapshot` / `(snapshot, options) => ProjectClosureOverview\|ProjectClosureBatchReport` | `harness.db` は `plan_registry`、`artifact_registry`、`test_runs`、`gate_runs`、`runtime_verification_events` を投影済み。`test_runs.completed_at` / `gate_runs.checked_at` は ISO8601 文字列または空 | L14 claim と open L7 が並存する場合は `Recovery` / `needs_recovery` とし、closure queue を `close_ready` / `collect_evidence` / `repair_failed_evidence` / `reverse_design` に分類する。失敗証跡は削除しないが、`repair_failed_evidence` は最新 failure が最新 passed evidence より後、または passed evidence が無い component のみに限定する。最新 passed evidence が最新 failure 以降なら failure history は保持したまま修復済みとして扱い、`evidenceGaps` / `repair_targets` / `repair_plan` には未修復 component だけを出す。`version_target` parked PLAN と live backlog boundary PLAN は open L7 には残すが current completion の機械証跡回収 queue から除外する。`class_method_contract` の L12 operation scope は、対応する設計 ID に結合した accepted `runtime_verification_events` がある場合だけ observed に昇格し、単語一致だけでは昇格しない。`operation_test` は `operation_test` 直書きだけでなく、同一 test evidence text に `operation/ops/運用` と `test/テスト` が揃う passed `test_runs` / `gate_runs` も観測 source として結合する。`incident_recovery_route` は accepted `runtime_verification_events` だけでなく、Recovery/Reverse を含む `closure.next_action_ledger` entry も `closure_next_action_ledger:*` の観測 source として結合する。`current-location --summary-json` は `function_design_policy` と `scrum_operation` を first-class に返す。`function_design_policy` は独立機能設計廃止、L5/L7/typed/runtime evidence への absorption surface、L5 detailed contract coverage、tailoring detail contract status を machine-readable に出す。`scrum_operation` は status、ZIP source package、source binding、Scrum backlog/sprint/acceptance/planning/ceremony/metric/active sprint plan counts、observed/missing count、sample gap items を machine-readable に出す。 |
...
| **L7.7** | L7 closure module surface (workflow/session/continuation/cutover/review/skill/asset 等) | schema | 実装済 (`src/workflow/`、`src/runtime/`、`src/skills/`、`src/assets/`、CLI surface) |

> 各 Sprint = TDD Red-first (L7 entry、§1.10 line 671)。先行 ④ 単体テストコードは L7 単体テスト設計 (pair) の U-* に対応。

`expandRegressionScope` の direct coverage は通常、対象 source module を import する test から導出する。
ただし atomic retirement で削除済みの module は再 import できないため、当該 namespace の現行 negative oracle ID と
retirement/resurrection detector import の両方を持つ test を direct coverage とする。名称だけの test、archive 文言、
旧 module の互換 stub は coverage に数えない。これにより削除差分も fail-close で検証しつつ、退役 surface の復活を要求しない。
...
clean checkout では、`harness-check` は tests の前に deterministic `db rebuild` を実行しなければならない。ignored local session log が無い場合、projection layer はtracked provider delegation evidenceから`hook_events`をderiveできる。ただしprovider evidenceはcontinuation sourceへjoinしない。persistent `.helix/harness.db` が無い場合、`helix skill suggest --json` は source から read-only in-memory DB を rebuild しなければならない。persistent `.helix/harness.db` が存在する通常 repo では、`skill suggest` と adapter の skill injection 解決は既存 projection を read-only に使い、毎回の in-memory rebuild を避ける。`--record` 指定時だけ projection を更新してから `skill_recommendations` へ書く。
...
### 2026-07-11 skill scaffold / catalog 品質契約追補 (PLAN-L7-420)

- `scaffoldSkill(input)` は既存catalog slugを受け取り、大小文字を正規化した完全衝突をhard findingとして
  拒否する。hyphen segment単位の包含名は正当な責務分離を阻害しないadvisoryとし、CLIは
  `docs/skills/` の実在slugを必ず入力する。`--force` は品質衝突を迂回しない。
- 生成本文は態度・判断フレーム・手順と検証・アンチパターンを明示する穴埋めskeletonとし、未記入marker、
  generic stub、本文1200字未満、`##`節2未満のいずれかをlanding不可とする。
- `analyzeSkillQuality` は全packについて、大小文字非依存のname/slug重複、正規化文字16-gram共有率
  `shared/min > 0.35` の近似重複、`SKILL_MAP.md` trigger table Pack列との双方向同期をfail-closeで検査する。
  `SKILL_MAP.md`不在・読取不能もgreenへ縮退させない。
- `runDoctor` は `skill-quality` をhard gateとしてok集約へ含める。oracleはL8
  `U-SKQUAL-001..006` と `tests/skill-quality.test.ts` / `tests/skill-scaffold*.test.ts`。
...
## 2026-07-06 l1-l2 gap-check read-only packet 追補 (PLAN-L7-333、親 = PLAN-DISCOVERY-11 Step 4)

L1⟷L2 要求洗い出しサイクルの AI 支援面は、要求や mock を書き換える surface ではなく
**read-only gap-check packet** とする。packet は観点表 8 項目、3 ラウンド bound、A-40 change-log route、
既存 `l1-l2-consistency` の構造判定結果を返す。観点表の充足判断と採否は人が行い、AI は欠落候補を
surface するだけである。

| 関数 | Signature | pre | post | invariant | oracle |
|---|---|---|---|---|---|
| `buildL1L2GapCheckPacket` | buildL1L2GapCheckPacket(input: L1L2ConsistencyInput) => L1L2GapCheckPacket | L1/L2 consistency input が渡される。 | `schemaVersion=l1-l2-gap-check.v1`、`writePolicy=no-write`、観点表 8 項目、`maxRounds=3`、A-40 route、consistency result、gapCandidates を返す。 | `planOnly=true` / `mustNotMutate=true` / `completionClaimAllowed=false` を固定し、AI が L1/L2 を確定できる surface にしない。 | U-L1L2-007..008 |
| `loadL1L2GapCheckPacket` | loadL1L2GapCheckPacket(repoRoot?: string) => L1L2GapCheckPacket | repo root 配下の L1/L2 docs を読む。 | live repo の gap-check packet を生成する。 | loader は read-only。永続 state、continuation event/projection を書かない。 | U-L1L2-009 |
| `l1L2GapCheckMessages` | l1L2GapCheckMessages(packet: L1L2GapCheckPacket) => string[] | gap-check packet が渡される。 | text mode の要約、authority boundary、A-40 route を返す。 | text mode でも writePolicy と人-AI 境界を落とさない。 | U-L1L2-010 |
