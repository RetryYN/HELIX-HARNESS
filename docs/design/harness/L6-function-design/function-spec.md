...
| `projectUtHistorySignals` | `(input: UtHistoryInput, deps: HarnessDbDeps) => ProjectionRowRef[]` | `computeUtHistorySignals` の入力が正規化済みで、DB は `.ut-tdd/` 配下または test 用 `:memory:` で開かれている | aggregate UT history signal を `quality_signals` へ、pass/fail が揺れる oracle を `test_flake_events` へ、原因 oracle の flake / duration regression を `quality_signals(source=ut-history)` へ冪等 upsert する。runner 出力 parser と長期 duration trend は別 slice |
| `projectUtHistorySignalsFromProjectedRuns` | `(db: HarnessDb) => void` | `test_runs` / structured `test_results` / `test_cases` が deterministic rebuild 中に投影済みで、feedback projection より前に呼ばれる | DB から `TestRunEvidenceInput` を復元して `projectUtHistorySignals` を呼び、UT history `quality_signals` を `feedback_events` の入力にする。生 reporter parser は扱わない |
...
| `projectUtHistorySignals` | `UtHistoryInput { plan_id?; window?; duration_regression_ratio? } -> ProjectUtHistorySignalsResult { refs[]; signals[] }` | implemented in `src/workflow/contracts.ts`; project aggregate history metrics into `quality_signals`, plan-scoped flake windows into `test_flake_events`, oracle-scoped `test_cases`, and oracle-scoped flake/duration regression signals for feedback ingestion |
| `projectStructuredGreenCommandCaseEvidence` / `projectUtHistorySignalsFromProjectedRuns` | `review_evidence.green_commands[].evidence_path -> test_results -> ut-history signals` | implemented in `src/state-db/projection-writer.ts`; if a green-command evidence JSON has `cases[]`, rebuild projects case/result rows, then reconstructs UT history from DB before `projectFeedbackEvents` |
