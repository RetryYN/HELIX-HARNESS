...
| `projectUtHistorySignals` | `(input: UtHistoryInput, deps: HarnessDbDeps) => ProjectionRowRef[]` | `computeUtHistorySignals` の入力が正規化済みで、DB は `.helix/` 配下または test 用 `:memory:` で開かれている | aggregate UT history signal を `quality_signals` へ、pass/fail が揺れる oracle を `test_flake_events` へ、原因 oracle の flake / duration regression を `quality_signals(source=ut-history)` へ冪等 upsert する。加えて oracle/run 時点ごとの duration を `quality_signals(metric=duration_trend_ms)` として保存し、`value=duration_ms`、`threshold=prior median`、`computed_at=run completed_at` で trend を再構成可能にする |
