...
1. **L6（機能設計）と test-design pair ⇒ L7（実装）は必須**。kind=impl / add-impl の PLAN は
   L6 設計 doc への parent_design と test-design への pair_artifact を持たない限り起票できない。
...
| `TEST_DESIGN_PREFIX = "docs/test-design/"` | 実装 PLAN の pair_artifact が満たすべき prefix（V-pair: L6↔L7 の test-design 側）。 |
...
| `analyzePlanDescent(docs, baseline)` | 純関数。違反を `{planId, reason, detail}` で列挙し、baseline 記載の plan_id は `grandfathered` に分類。`ok = 新規違反 0 かつ grandfathered 件数 ≤ baseline 件数`（ratchet: 減るのは可、増えるのは不可）。reason は 5 種: `parent_design_absent` / `parent_design_not_l6_design_doc` / `pair_artifact_not_test_design` / `parent_design_not_confirmed`（status ∈ {confirmed, completed} の impl PLAN の親 L6 doc が `status: confirmed` でない — 実装が設計 confirm を追い越す「実装先行」を confirm 時点で fail-close）/ `generates_missing_test_code`（impl PLAN の `generates` に `artifact_type: test_code` が 1 件も無い — 検証資産・ドキュメント資産として残らない実装を起票時点で拒否。missing-test-plan-id 再発の根本遮断）。 |
