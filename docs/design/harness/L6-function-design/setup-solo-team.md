...
> `ut-tdd status` / `ut-tdd completion decision-packet --json` / `ut-tdd doctor --profile consumer`、secret/PII 禁止、findings-first を持つこと、
> 各 slash-command が `description`、`ut-tdd status --json`、`ut-tdd completion decision-packet --json`、consumer doctor 導線を持つことを fail-close する。
