...
interface HelixProjectSetupState {                  // .ut-tdd/state/project-setup.json (HELIX project setup の completion 境界 SSoT)
...
| `recordHelixProjectSetupState` | `(state: HelixProjectSetupState, deps: { fs: FsWriter }) => void` | `runHelixProjectSetup` が dry-run ではなく、`consumerReadiness.objectiveBoundary` と `postSetupWorkflow` を計算済み | `.ut-tdd/state/project-setup.json` を上書きし、`scope=consumer_setup_readiness_not_whole_program_completion` と `completionClaimAllowed=false` を後続の `ut-tdd completion decision-packet --json` が復元できる形で保存する。さらに `postSetupWorkflow.verificationCommands[]` と同じ command set を `verificationMatrix[]` として保存し、各 row に `phase` / `command` / `writePolicy=no-write` / `requiresMaterializedPaths[]` / `expected` / `evidence` を残す。旧 `setup.json` の 4-field strip 契約は壊さず、HELIX project setup 専用の completion 境界と初回稼働 matrix だけを別 state として残す。 |
...
> consumer-owned conflict として review へ送らない。これにより `ut-tdd setup project` の再実行は prompt-free /
> idempotent になり、同一 `.vscode/tasks.json` や `.ut-tdd/teams/default-hybrid.yaml` を import report blocker と誤認しない。
...
> `package.json.scripts.ut-tdd` / `bun run ut-tdd` は VSCode task / CI / package-local smoke の必須証跡であり、
> bare `ut-tdd --version` の代替証跡にはしない。package script だけが存在する場合、`ut-tdd-package-script` は
> true でも `ut-tdd-cli` は false のままなので、setup readiness は `fix_consumer_readiness` へ戻る。
> 逆に bare `ut-tdd --version` だけが解決しても、生成済み VS Code task / CI は `bun run ut-tdd ...` を使うため、
> `ut-tdd-package-script=false` のまま `fix_consumer_readiness` へ戻す。
...
> wet run は `.ut-tdd/state/project-setup.json` に `objectiveBoundary.completionClaimAllowed=false` を保存し、
> 後続の standalone `ut-tdd completion decision-packet --json` はこの state を読んで
...
> 読み替えない。
> `consumerReadiness.artifactReadiness` は実際の `runHelixProjectSetup` plan に束縛し、AGENTS / VSCode task / `.ut-tdd` baseline / default-hybrid team
...
> `ut-tdd completion review-bundle --json` の exact digest / semantic digest 確認、
...
> `CONSUMER_CI_RUN_COMMANDS` の固定 12 本 (`bun install --frozen-lockfile`、9 本の `bun run ut-tdd ...` smoke、
...
> `bun run ut-tdd handover status --json` / `bun run ut-tdd completion decision-packet --json` /
> `bun run ut-tdd completion review-bundle --json` / `bun run ut-tdd doctor --profile consumer --json` を持つ場合だけ artifact readiness / consumer doctor を green にする。
...
> `postSetupWorkflow.verificationCommands` は setup dry-run / status / setup dry-run JSON による GitHub CI safety / `ut-tdd completion decision-packet --json` /
> `ut-tdd completion review-bundle --json` / `ut-tdd version-up dry-run --current v0.1.0 --target v0.1.4 --release-remote https://github.com/unison-ai-product/UT-TDD_AGENT-HARNESS-Pack.git --json` / `ut-tdd doctor --profile consumer` /
...
> `postSetupWorkflow.verificationMatrix[]` は setup-dry-run / status-frontier / github-ci-safety / completion-decision-packet / completion-review-bundle / version-up-dry-run / consumer-doctor /
...
> consumer doctor は command list だけが残って matrix が欠落した state、10 行のうち
...
> `emit_only` を示し、`doctorBaseline` は setup dry-run / status / `ut-tdd completion decision-packet --json` /
> `ut-tdd completion review-bundle --json` / `ut-tdd version-up dry-run --current v0.1.0 --target v0.1.4 --release-remote https://github.com/unison-ai-product/UT-TDD_AGENT-HARNESS-Pack.git --json` / `ut-tdd doctor --profile consumer` /
...
> source / clean artifact の `package.json` は package-local `scripts.ut-tdd="bun run src/cli.ts"` を持ち、
> dogfood repo でも `bun run ut-tdd --version` が VSCode task / CI と同じ入口で動く。
...
> `bun run ut-tdd completion decision-packet --json`、`bun run ut-tdd completion review-bundle --json`、`bun run ut-tdd version-up dry-run --current v0.1.0 --target v0.1.4 --release-remote https://github.com/unison-ai-product/UT-TDD_AGENT-HARNESS-Pack.git --json`、
...
> `ut-tdd status` / `ut-tdd completion decision-packet --json` / `ut-tdd completion review-bundle --json` / `ut-tdd version-up dry-run --current v0.1.0 --target v0.1.4 --release-remote https://github.com/unison-ai-product/UT-TDD_AGENT-HARNESS-Pack.git --json` /
> `ut-tdd doctor --profile consumer`、secret/PII 禁止、findings-first を持つこと、
> 各 slash-command が `description`、`ut-tdd status --json`、`ut-tdd completion decision-packet --json`、
> `ut-tdd completion review-bundle --json` の exact digest / semantic digest 確認、version-up dry-run、consumer doctor 導線を持つことを fail-close する。
...
- 確定 phase: `.ut-tdd/state/setup.json` (gitignored runtime state、確定値の SSoT。毎回再推測しない安定化)。HELIX project setup の completion 境界は `.ut-tdd/state/project-setup.json` に分離し、setup ready と whole-program completion を後続コマンドで混同しない。
