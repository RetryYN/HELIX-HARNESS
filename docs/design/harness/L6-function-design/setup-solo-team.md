...
> **PO 確定事項** (2026-06-02 / 2026-07-05 更新): ① solo/team は参加アカウント数等で自動「**提案**」するが、確定は**人間確認 + 状態記録** (数だけで自動確定しない) / ② branch protection / Required 化は GitHub 設定操作でファイルでは完結しないため**既定 emit-only** (スクリプト+手順生成) とし、action-binding approval、明示 `--apply-branch-protection`、`gh auth status`、repository admin 権限が揃った場合だけ適用する / ③ **token は読まない・記録しない** (gh 認証状態に委譲) / ④ solo→team の格上げは**ガバナンス変更**であり、phase 確定なしに暴発させない。
...
| **適用要求** (`applyBranchProtection`) | **既定: スクリプト生成のみ (skip)**。`--apply-branch-protection` 指定時も action-binding approval が無ければ `approval-required` で停止し、承認後だけ `gh auth status` と repository admin 権限を確認して GitHub remote apply へ進む | approval 不足 / gh 未認証 / repo metadata 不取得 / admin 権限不足 / API 失敗は `{applied:false, reason:*}` で停止。token は保持しない |
...
| `applyBranchProtection` | `(plan: SetupPlan, deps: { gh; confirm; isInteractive }, opts: { apply: boolean; approvalEvidence? }) => { applied: boolean; reason: string }` | — | `opts.apply!==true` → `{applied:false, reason:"emit-only"}` (既定)。branch-protection action が無い場合は `no-action`。`apply=true` でも `approvalEvidence.approved!==true` なら `approval-required` で停止し gh を呼ばない。承認済みの場合だけ `gh auth status`、`gh api repos/{owner}/{repo}`、`permissions.admin===true` を順に確認し、通過時だけ `gh api -X PUT repos/{owner}/{repo}/branches/main/protection` を実行する。成功時 `{applied:true, reason:"applied"}`、未認証/権限不足/API 失敗は `{applied:false, reason:*}` を返す |
| `runSetup` | `(args: SetupArgs, deps) => SetupResult` | — | orchestration。phase = `args.phase` (フラグ) > `confirm(recommendPhase(detectProjectScale()))` (対話) > `0-A` (fallback)。確定 → `recordSetupState` → `planSetup` → `emitSetup` → (`applyBranchProtection` は opt-in の要求として評価)。**非対話 + フラグ無し → 0-A**。**invariant: `--apply-branch-protection` は action-binding approval と gh 認証/admin 権限があるときだけ remote apply へ進む** |
...
| Required Status Checks 登録 / 必須レビュー数=1 / branch protection | ✕ | ✅ | **NO** | **YES (`GithubAction`、GitHub settings / branch protection)** → 既定は `setup-branch-protection.sh` 生成のみ。明示 `--apply-branch-protection`、action-binding approval、gh/admin preflight 通過時だけ API apply する |

「ファイルで完結する」ものを `GeneratedFile`、「GitHub 設定操作」を `GithubAction` として**型で分離**し、実装時の境界混同を構造的に防ぐ。後者は既定 emit-only、明示 `--apply-branch-protection`、action-binding approval、gh/admin preflight 通過時だけ適用する。**commitlint の配置 (root `commitlint.config.js` vs `package.json` キー) は §9.1 A 種別と repository-structure §8 config 最小化方針が衝突しうる** → **L7 impl で突合・解決済 (下記)**。
...
> 落ちる状態を ready と誤認させない。公開済み配布 tag は runtime import する `typescript` を transitive
...
- **ガバナンス暴発防止**: branch protection 適用は action-binding approval、明示 `--apply-branch-protection`、gh/admin preflight を必須にし、既定 emit-only から自動昇格しない。approval・認証・権限・API apply のいずれかが失敗した場合は reason 付きで停止し、token は state/docs/log に保存しない。
...
- **L4 external-if (PLAN-L4-04)**: GitHub 境界の DbC 契約 (file vs gh-api 境界 / emit-only 既定 / action-binding approval + gh auth/admin preflight 付き apply) を external-if.md へ back-fill。
...
- **認可・本番影響の境界**: branch protection 適用は action-binding approval、gh auth/admin preflight、audit 可能な CLI 結果を前提にし、token は保持しない。不可逆 rename / version-up activation などの高影響 action と同じく action-binding approval 境界で扱う。
