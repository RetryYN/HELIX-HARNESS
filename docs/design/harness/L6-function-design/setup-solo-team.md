...
> emit-only 境界を再利用し、追加で status / completion decision-packet / consumer doctor / rename plan / handover status / team run dry-run を開く `.vscode/tasks.json`、
...
> write permission 不在、`jobs.harness-check.runs-on=ubuntu-latest`、`actions/checkout@v4`、
> `oven-sh/setup-bun@v2`、secret 参照不在、固定 `run:` command set をすべて満たす場合だけ green にする。
...
> `emit_only` を示し、`doctorBaseline` は setup dry-run / status / `ut-tdd completion decision-packet --json` / `ut-tdd doctor --profile consumer` /
