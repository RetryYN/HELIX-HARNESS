...
**Scope boundary (PLAN-L4-51)**: 上記 26 件は **harness core namespace** の件数であり、HELIX pillar の総 L4 coverage ではない。PLAN-L3-06 で確定した `HR-FR-*` 30 件 + `HR-NFR-*` 13 件 (計 43 件) は HELIX add-design namespace として [pillar-basic-design](../../../design/helix/L4-basic-design/pillar-basic-design.md) に降下済みであり、本 doc の「26 件マップ漏れ 0」は **core 26 件のみ**を意味する。
...
| **C4 mode routing** | FR-08 | Workflow | runtime(detect) + doctor による routing |
...
| **C11 Recovery** | FR-10 | Workflow / continuation read model | (将来 cutover) |
...
> 11 → **12 カテゴリ** (C12 = 内部資産 roster/skill/command を A-85 で追加、Recovery PLAN-RECOVERY-01 / FR-L1-46/47/48 / BR-22。**skill (FR-L1-47) の building block は architecture §3.1 skills 行 (PLAN-L4-12)**、roster/command (FR-L1-46/48) は本 doc §1.1 が正本)。L7 完遂時点の実装証跡は C2(vmodel/doctor lint 群) + C4(detect) + C5(workflow contracts/readiness) + C6(agent-guard) + C10(skill recommend/injection) + C12(roster/skill/asset catalog) + review/cutover/builder CLI surface。**26 件マップ漏れ 0** は harness core only (FR-01〜18 + FR-23〜27 + FR-29 + FR-30 + FR-45) の判定であり、HELIX pillar 43 件は §1.3 / PLAN-L4-51 で別名前空間として閉じる。C12 は L1 FR-L1 由来 (L3 では §3 carry 宣言、内部資産設計増分)。

### §1.3 HELIX pillar block overlay の扱い (PLAN-L4-51)

HELIX L3 pillar は harness core の C1-C12 を置換しない。core 機能カテゴリへ横断的に重ねる add-design overlay として扱い、`HR-FR-*` 30 件 + `HR-NFR-*` 13 件を 10 building block に束ねる。

| HELIX block | 対応する既存カテゴリ / module surface | L4 責務 |
|---|---|---|
| **HB-P0 forward-convergence** | C5 workflow / C11 Recovery / doctor | forward convergence、runaway guard、reopen 判定を、mode routing と DB continuation/session/memory/feedback evidence で閉じる |
| **HB-P1 continuous-autonomy** | C5 workflow / C10 文脈注入 / setup | continuous-run、version-up、Scrum refinement、L2 skip、context budget、durable continuation checkpoint を PLAN/gate surface に載せる |
| **HB-P2 agent-loop-contract** | C5 workflow / C6 AI guard / runtime / team | worker/verifier 分離、loop budget、Codex/Claude parity、PLAN-driven trace を provider-neutral intent として扱う |
| **HB-P3 verification-governance** | C2 trace / C9 review / review-evidence | strict verification、implementation accuracy、test workload budget、Red evidence を L4-L9 pair で検証可能にする |
| **HB-P4 repair-learning** | C7 doctor / feedback / Evaluation | repair proposal、learning metric、quality trend を projection と feedback event に落とす |
| **HB-P6 github-distribution** | C8 CI / setup / release surface | GitHub gated push/PR/CI、setup、tag/release を emit-only + approval gate で扱う |
| **HB-P7 shared-knowledge** | C10 skill / memory / graph / glossary / context-map | shared memory、glossary、bounded context、DDD context map を authoring source と projection に分離する |
| **HB-P8 external-security** | external-if / guardrail / audit / sandbox boundary | external research、skillify、sandbox、prompt injection/security filter を外部入力境界として扱う |
| **HB-P9 db-convergence** | state-db / graph / search / doctor | DB convergence、relation graph、layer regression を projection/readiness として観測する |
| **HB-AC adapter-consistency** | rule-drift / codex-hook-adapter / work-guard | hosted API/developer tool surface は repo hooks 非強制と明示し、API 非前提・manual preflight を要求する |

> **Pair**: HELIX overlay の 43 件は [L4-pillar-system-test-design](../../../test-design/helix/L4-pillar-system-test-design.md) の `HST-*` 43 件と 1:1 pair で検証する。C1-C12 の core 26 件検査とは別に、`U-VPAIR-007a/b/c/d` が overlay の孤児0を固定する。
...
FR → `helix` サブコマンドの対応 (architecture.md cli module に集約、副作用端点)。

| コマンド (将来形) | 実現 FR | 現状 | 操作 |
|---|---|---|---|
| `helix status` | FR-13 | **実装済** (scaffold) | mode 検出表示 |
| `helix plan draft/lint/delete` | FR-01 / FR-04 / FR-24 | 実装済み (段階拡張) | `plan lint` は schedule 最小強制 + `--gate governance/frontmatter` の PLAN frontmatter/cross-record 厳格検査 + `--gate G1-trace/G3-trace` の trace gate を提供。doctor は governance debt を warning-first で件数 surface |
| `helix sprint start/check` | FR-02 | 未 | TDD 強制 (Red→Green→refactor 順序、本体実装前 Red 必須 fail-close) |
...
| `helix review --uncommitted` | FR-45 | 実装済 | doctor + verification-profile による review packet 生成 |
| `helix skill suggest` | FR-12 | 実装済 | harness.db の plan/layer/drive context から L 別 skill 推挙 |
| `helix route --mode` | FR-08 | 未 | 手動 mode 切替 (S-03) |
| `helix cutover --to` | FR-10 / FR-26 | 実装済 (dry-run / approval gate) | ロールバック計画と事前 check を出力。dry-run の check command は `bun run src/cli.ts ...` の POSIX copyable path を出し、rollback ref は shell quote する。text surface は `humanApprovalRequired=true` と表示し、承認済みと誤読させない。実適用は human-approved runbook 必須 |
| `helix completion decision-packet` / `helix status` | FR-10 / FR-26 / FR-38 | 実装済 | canonical `helix ...` command と repo-local `bun run helix ...` runnable command を JSON/text に併記し、PATH 未設定の承認者が S4 / version-up / rename / action-binding packet 再生成 command を推測しないようにする |
| `helix roster list/check` | FR-L1-46 | 未 (roster module 実装前は guard ハードコード allowlist で動作継続、移行段階 = §1.1 note / ST-ASSET-07) | 内部資産 subagent roster 一覧 / .md↔guard allowlist 整合確認 |
| `helix asset catalog` / `helix builder catalog` | FR-L1-48 | 実装済 | 内部資産 (skill pack / command / builder) カタログ (porting-map W11/W12/W16) |
...
## §3 workflow オーケストレーション機能（development style／case-driven model／change route／specialist process）

FR-12〜16 / FR-23〜30 の workflow を機能単位で外部設計する。この harness の**中核価値 = 適切なオーケストレーションで開発コストを下げる (CLAUDE.md 柱 5)** の本体であり、L4 では4軸それぞれの **外部から見える設計 (入口 signal / 状態遷移 what / 出口 contract / 担当 building block / gate)** を確定する。状態遷移の内部ロジック (pseudocode) ・CLI signature は §3 末尾で L5/L6 へ明示deferする。`docs/process/modes/*.md`は旧mode atomのcompatibility sourceであり、本節の4軸へ正規化してから利用する。

> **current taxonomy**: `development_style`、`case_driven_model`、`change_route`、`specialist_processes`は
...
|---|---|---|
| `FULL_L1_L12_V` | slice化せずL1〜L12を順に閉じる | L1〜L3承認、6 V-pair、TDD、L12運用証拠を省略しない |
| `PRODUCTION_SCRUM` | L3 freeze後に価値slice化 | sliceごとに正規L4/L5設計、実装、V-pair evidenceを閉じる |
...
| case-driven model | 入口 | lifecycle | 出口 |
|---|---|---|---|
| `Discovery` | 要求・成功条件・設計の不確実性 | S0 hypothesis → S1 experiment plan → S2 poc → S3 verify → S4 decide | S4人間判断後、選択済みdevelopment styleへ接続 |
| `PoC` | 実現性・価値仮説の実験が必要 | Discoveryと同じS0〜S4を使う非production検証 | rejected/pivotは記録、adoptだけを選択済みstyleへ昇格 |

Discovery／PoCはScrumのphase、variant、内包要素ではない。S4 receipt前にproduction-ready、implemented、
ux_verifiedを主張しない。

### §3.3 change route（選択済みstyle内の変更・復旧経路）

各routeは状況signalで発動し、選択済みdevelopment styleの適切なL工程へ戻る。routeはstyleを変更せず、
style変更が必要ならL3再承認を要求する。kindは§1.3 VALID_KINDS、非1:1対応は後段に示す。

| change route | kind | 入口 signal | 状態遷移 (phase/step + 各 what) | 出口 contract → 選択済みstyleへの再入先 | gate / 人間サインオフ |
|---|---|---|---|---|---|
| **Reverse** | reverse | `drift` (schema/contract) / case-driven modelのadopt / Production Scrum slice完了 / drift-check横断検出 | R0 evidence 収集 + has_existing_tests 調査 → R1 observed contracts → R2 as-is design + ③逆復元 → R3 intent 仮説 + gap (po 検証) → R4 gap & routing | R4 `forward_routing` 確定 + ③テスト設計状態確定 + 再入先 pair-freeze gate 義務明示 → **L1/L3/L4/L5/gap-only** (5値、L7/L8-L12除外) | R3 po 検証 fail-close / R4 routing / 再入先 G1/G3/G4/G5 |
| **Recovery** | recovery | `agent_runaway` / `context_exhaustion` / `regression_dev` / `runaway` / **`forced_stop`** | (phase なし) 収束 5 step: 全事象収集 (a/b/c/d 分類) → PO 提示・認識確認 → reopen point 特定 → top-down 修正 → fullback | reopen point 確定 + **再発防止 doc (root cause + guard/test/rule/hook具体変更 + L12改善route) 作成済** → 中断L工程へ復帰 / 再発防止 → L12 | **tl (reopen 確認) + po (スコープ承認) 人間サインオフ必須**。`doctor` forced-stop 検出 (fail-open) |
| **Incident** | troubleshoot + recovery (内包) | `production_incident` / `hotfix_required` / `regression_prod` (env=prod) | (phase なし) 6 step: 検出 → トリアージ → hotfix (kind=troubleshoot PLAN) → 即リリース → 収束確認 (kind=recovery PLAN) → 事後昇華 | hotfix暫定収束 (SLO/KPI正常化) + 恒久対策PLAN + postmortem → **L12、恒久策 → L1-L6 (Reverse経由)、改善記録 → L12** | **オンコール + tl + pm 三者サインオフ必須**。`harness-check` サブセットCI |
...
> 各 row は `sourceUrl` / `sourceCheckedAt` / `latestOfficialStatus` / `sourceStatusDelta` /
> `adoptionDecision` / `adoptionDecisionDelta` / `workflowRouteImpact` を持ち、欠落・placeholder・未来日・
> 90 日超 stale を fail-close する。これらの row は `activationSnapshot.evidenceDigest` に含め、
> Cloudflare / GitHub / OWASP source の状態差分や採否差分で承認前 snapshot が変わる。`status --json` /
> completion decision packet の version-up `supportingPacketSummaries[]` も `activationSnapshot` /
> `externalRehearsalPlan` / `costGuardrails` / `securityChecklistPacket.securityChecks` / `blockedReasons` を
> required review field として出し、next action から承認前 snapshot・rehearsal・外部 limit・security source
> metadata・blocker へ直接辿れるようにする。

> **case／slice → Reverse fullbackの機械着地先**: `checkScrumReverse`はlegacy名称を保持するが、current意味は
> case-driven modelのadoptまたはProduction Scrum sliceを選択済みstyleの正規pairへbackfillすることである。
> PoCをScrumへ分類せず、S4 receiptまたはslice acceptanceが無い成果をcompletionへ使わない。
> **定量テスト → 定性レビュー順序**: 全style、case、route、specialistで **定量verify → 定性review／サインオフ**
> の順を守る。共通機械アンカーは`review_evidence.tests_green_at ≤ reviewed_at`と再現可能な
> `review_evidence.green_commands[]`であり、未検証成果物をreview済みgreenにしない。
...
この4 change routeは**失敗routingの優先度**を持つ。他のchange routeは固有signalで入る。
...
| — (固有) | `version_deferral` | version-up | 将来版保全。activation までは parked |
| — (固有) | `user_feedback_iteration` / `requirement_continuous_refinement` | selected-style change intake | style変更はL3再承認 |
| — (固有) | `tech_decision_required` / `option_comparison_needed` / `adr_required` | Research | PoC 不要 (机上完結) |
| — (固有) | `requirement_undefined` / `feasibility_unknown` / `success_condition_unclear` / `design_uncertain` | Discovery／PoC case activation | Scrum非内包、S4人間判断 |
...
**axis ↔ kindの非1:1**: `kind=poc`はcase artifact種別でありScrum識別子ではない。Incidentは
`troubleshoot` + `recovery`、Add-featureは`add-design` + `add-impl`へ分割する。kindからdevelopment styleを推定しない。

### §3.5 specialist process（style／case／layerではない）

Design HARNESSはProduct Design、Experience Design、System Design、Design Governanceを統合する専門capabilityであり、
development style、case-driven model、独立V-model layerではない。screen-design／frontend-designはその適用sliceで、
独立kind／layer／workflow_phaseを持たない。

| 工程専門 | 実現 FR | Forward 内位置 | 状態遷移 (what) | 出口 / gate | drive 別要否 |
|---|---|---|---|---|---|
| **Design HARNESS** | HR-FR-DH-001..012 | L1〜L5へdesign evidenceを供給、L10〜L12でUX evidenceを検証 | applicability → Experience/UI/Frontend Contract → Design Registry → evidence backflow | 各成果は対応する正規V-pairへ束縛。自身で工程完了を宣言しない | applicability判定で選択 |
...
全workflowを同じ強度でTDD化しない。Redを失敗testだけに狭めず、設計欠落・依存impact・DB projectionの未解消signalもRedとして扱う。YellowはPLAN／対象登録済みだが保護網や依存確認が未完了、Greenはtest/design/review evidenceが該当V-pairへ束縛されgateが通った状態とする。

| 対象 | TDD適性 | Red 発火点 | Green 条件 |
| --- | --- | --- | --- |
| Forward design / `kind=design` | strong | `descent_obligation_missing` / `pair_artifact_missing` / `test_design_missing` | design doc + test-design + trace edge + green 後の review |
| Add-feature (`add-design` + `add-impl`) | strong | `feature_addition` / `scope_extension` / `acceptance_gap` | add-design + add-impl + regression green + 必要時の Reverse back-fill |
| Refactor | strong | `code_smell` / `structural` / `debt_degradation` / `artifact_progress_red` | 振る舞い不変 + linked test ID + relation impact close |
| Reverse | strong | `drift` / `schema_contract_gap` / `as_is_test_design_missing` | as-is design + intent confirmed + forward routing + backprop artifact を揃える |
| Retrofit | strong | `dependency_outdated` / `upgrade` / `config_drift` / stale `dependency_edges` | migration + config + rollback + regression green を満たす |
| Recovery / Incident | strong | `regression_dev` / `regression_prod` / `forced_stop` / quality failure | reproduction または recovery test + guard/rule + postmortem + durable continuation evidence |
| screen-design | strong | `screen_requirement_gap` / `wireframe_missing` / `screen_impl_pair_gap` | screen list + flow + wireframe + UI element + pair trace を揃える |
| frontend-design | strong | `a11y_regression` / `visual_regression` / `token_drift` / UX feedback | visual + token + a11y + VRT + UX review を揃える |
| Discovery／PoC case / Production Scrum slice | partial | uncertainty / user feedback | case hypothesisまたはstyle sliceの検証 + 対応decision + 正規pairへのreturn route |
| version-up | partial | `version_target` / release trigger / parked activation evidence gap | activation packet + parked review + approval/snapshot binding + Forward/Add-feature route を揃える |
| Research | weak | technical decision required / ADR required | memo + source + ADR candidate を揃える |
...
**Provider materialization (PLAN-L7-135, 2026-06-23)**: `helix skill suggest --inject --json`
は provider-neutral context manifest である。出力するのは skill path と理由だけ
(`required_paths`, `optional_paths`, `missing_skill_ids`) であり、skill body は出さない。
`helix codex --plan ...` / `helix claude --plan ...` と `helix team run
--plan ...` は同じ manifest を `HELIX context injection` として adapter stdin へ materialize する。
これにより Claude と Codex は同一の scoped skill context を受け取り、argv は固定 command metadata のまま保たれる。
`helix task route --plan ... --execute` は cost-tier/model routing 後に同じ materialization path を使うため、
...
- style／case／change route／specialistの **状態遷移 pseudocode / 内部処理** → L6機能設計 (IEEE 1016 §5.7、IMP-019)。
- **orchestration_mode の cell matrix (drive×layer → 5値の具体割当)** → requirements §1/§7 (concept §2.6.4)。**L7 着地 (2026-06-23)**: `helix vmodel show <drive> <layer> --injection` が `owner_role` / `mandatory_agents` / `recommended_skills` / `recommended_commands` / `orchestration_mode` を返す公開 surface として実装済み。execution mode 別の縮退実行も `--mode <mode>` / runtime detection で `degraded_from` / `degraded_to` / `degradation_reason` を出すため、silent fallback せず記録される。
- **Scrum 6 type × Reverse 5 type の 30-cell routing matrix** → requirements §3 (concept §4.4、現状 stub)。
- **case／slice出口の右腕直接合流禁止** → 機械着地先 = `ForwardRouting` enum (`VALID_FORWARD_ROUTING` = L1/L3/L4/L5/gap-only、L7/L8-L12を含めない) + doctor `checkScrumReverse`。legacy名称に依存せず、Reverse backfillなしに右腕へ直接合流させない。
...
> **IMP-013 接続明示**: FR-L1-20 の観測値 (invocation_log = AI 呼び出し記録 / accuracy_score = 判定精度 / gate_evidence = ゲート証跡) は、**Phase A で `.helix/audit/*.jsonl` に append-only 記録** (data.md §8 Evaluation 集約) され、**business-detail §2 (BR-21 計測対象定義) の入力 + §5 (Learning Engine 集計) の経路**となる。AC-FR-BR21-02 (business-detail) の Phase A 前提 = 「観測層が記録のみ稼働、学習エンジン本実装は Phase B」と整合。観測 (Phase A) と学習 (Phase B、FR-L1-19) を分離。

## §6 P1 tracking 10 件の機能 building block 着地先

L3 §3.1 の P1 tracking を機能境界に割り当てる。内訳は **L4 未起票 sub-PLAN 8 件**、**実装済 1 件** (FR-L1-42)、**L7/Reverse fullback 1 件** (FR-L1-51) であり、全行が L4 sub-PLAN 待ちではない。
...
| FR-L1-40 (drive 別 state 分離) | `.helix/drive/<drive>/` 区画 + skip_sub_doc 強制 | PLAN-L4-NN-drive-state-isolation | runtime/state |
| FR-L1-41 (drive 自動判定) | PLAN/コード/拡張子 → drive 分類 → routing | PLAN-L4-NN-drive-auto-classify | runtime(detect) |
| FR-L1-42 (provider delegation evidence) | Claude↔Codex context+PLAN+budget 連携の監査証跡 | implemented 2026-06-08 (`provider-handover.v1` schemaを保持) | runtime(adapter) |
...
> P2 (FR-L1-31〜35) は PLAN-L4-NN-infra-readiness、Phase B (FR-L1-19/20) は telemetry carry (data.md §9)。L4 未起票 sub-PLAN 8 件の `NN` 採番は各起票時に確定。
...
| trace set | L4 受け入れ block |
|---|---|
| FR-L1-01 / FR-L1-02 / FR-L1-04 / FR-L1-05 / FR-L1-06 / FR-L1-07 / FR-L1-09 / FR-L1-10 / FR-L1-11 / FR-L1-13 / FR-L1-14 / FR-L1-15 / FR-L1-16 / FR-L1-17 / FR-L1-18 | Function category C1-C11 と CLI command surface |
| FR-L1-23 / FR-L1-24 / FR-L1-25 / FR-L1-26 / FR-L1-27 / FR-L1-29 / FR-L1-30 | Workflow orchestration の機能 block |
| FR-L1-36 / FR-L1-38 / FR-L1-43 | BR-21 / business-detail evaluation projection を workflow/evaluation block へ carry |
| FR-L1-45 / FR-L1-50 / FR-L1-51 | doc-review、DDD/TDD strictness guard、artifact progress projection の block |
