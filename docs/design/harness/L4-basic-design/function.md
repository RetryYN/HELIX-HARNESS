...
> completion decision packet の version-up `supportingPacketSummaries[]` も `externalRehearsalPlan` /
> `costGuardrails` / `securityChecks` を required review field として出し、next action から承認前 rehearsal・
> 外部 limit・security source metadata へ直接辿れるようにする。
