...
| **continuation_read_model** | 読みモデル | Workflow / Evaluation projection | L1 §10.1（Reverse-344 backprop） |
...
→ **4 集約** (Plan / Artifact / Workflow / Evaluation) + 値オブジェクト群 + 読みモデル
(`derived_view` / `continuation_read_model`)。session/prose Handover は集約ではなく廃止済みである。

> **内部資産 (roster / skill catalog) の非 entity 判断 (A-90、ADR-004 整合、PO 確定 2026-06-01)**: subagent roster と skill catalog は **data 集約に含めない**。理由: ADR-004 で markdown (`.claude/agents/*.md` / `docs/skills/**/*.md`) を**唯一正本**とし、TS (層2、roster/skills module) は起動時に scan して **in-memory 構築 (scan-on-demand、永続 state なし)** するため、`.helix/` に独自の永続 entity を持たない。よって **4 集約モデルは不変** (roster/skill は state を持つ entity ではなく、fs 正本に対する読みモデル)。architecture §3.1 roster/skills building block / function §1.1 / L9 ST-ASSET と本判断で整合 (cross-sub-doc 沈黙 gap を解消)。詳細 = §8 state schema / ADR-004 Consequences。
...
|---|---|---|
| Kind | 12 種: `charter` / `impl` / `design` / `poc` / `reverse` / `add-design` / `add-impl` / `refactor` / `retrofit` / `recovery` / `troubleshoot` / `research` | `VALID_KINDS` |
| Layer | L0-L14 + cross (16) | `VALID_LAYERS` |
| Drive | be/fe/fullstack/db/agent (5、専門職のみ) | `VALID_DRIVES` |
| WorkflowPhase | 10 種: `kind=poc` は `S0-S4`、`kind=reverse` は `R0-R4` | `VALID_WORKFLOW_PHASES` |
| ArtifactType | 19 種 (source_module 含む) | `VALID_ARTIFACT_TYPES` |
| DecisionOutcome | 3 種: `confirmed` / `rejected` / `pivot` | `VALID_DECISION_OUTCOMES` |
| PromotionStrategy | 4 種: `reuse-as-is` / `reuse-with-hardening` / `redesign` / `discard` | `VALID_PROMOTION_STRATEGIES` |
| ForwardRouting | L1/L3/L4/L5/gap-only (5) | `VALID_FORWARD_ROUTING` |
| Role | 7 種: `po` / `tl` / `qa` / `aim` / `uiux` / `se` / `docs` | `VALID_ROLES` |
| OrchestrationMode | 5 種: `pm_lead` / `claude_judge` / `claude_judge_codex_impl` / `codex_impl_qa_verify` / `claude_design_impl` | `VALID_ORCHESTRATION_MODES` |
| ReverseType | 5 種: `code` / `design` / `upgrade` / `normalization` / `fullback` | `VALID_REVERSE_TYPES` |
...
- **freeze** (pair/trace): `pending → frozen`。G1/G3/G4/G5/G6 pair と G7 trace が対象。
- **decision_outcome** (poc): `null → confirmed | rejected | pivot` (S4 でのみ確定)
- **continuation checkpoint**: append-only session log / event を harness.db projection へ反映し、bounded memory breadcrumb を supersede する。単一 current file の状態遷移は持たない。
...
| Workflow (phase/gate) | `phase.yaml` + `gate_runs` | YAML / JSON-lines 形式 |
| mode | `mode.yaml` | YAML |
| Continuation read model | `harness.db` の session / workflow projection + append-only session log + bounded memory | DB projection / JSONL。`handover/CURRENT.json` は生成・読込とも禁止 |
| Evaluation (Phase B) | `audit/` (`invocation_log` / `accuracy_score` / `kpi`) | JSON-lines 形式 |
...
### §8.2 HELIX pillar projection carry (PLAN-L4-51) の扱い

HELIX pillar の `HR-FR-*` / `HR-NFR-*` 43 件は、L4 時点で新しい集約ルートを追加しない。4 集約 (Plan / Artifact / Workflow / Evaluation) を維持し、continuation は DB projection/read model として以下の concern と同様に扱う。
...
- **HELIX pillar projection carry** (PLAN-L4-51): approval action ledger / research source ledger / security event ledger / glossary/context-map projection / metric trend / contract ledger は §8.2 のとおり4集約を維持したまま L5 physical-data で table/projection 粒度を確定する
