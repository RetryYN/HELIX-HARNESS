...
| **github** (`src/github/`) | PR audit 送達 (exactly-once delivery / idempotency) と merge admission 判定 + runtime (contextual PR review packet / database convergence probe / logical DB digest)。GitHub 連携の契約層であり、破壊操作は emit-only default に従う | `planPrAuditJob()` / `commitPrAuditJobExactlyOnce()` / `verifyGithubPrDelivery()` / `collectContextualPrReviewPacket()` / `observePrDatabaseConvergence()` | composition (db rebuild) / schema (harness-db) / state-db |
...
| **gate** (`src/gate/`) | execution mode を問わず AI-A/AI-B の identity/session/context 分離と worker≠reviewer を強制する judgment gate 判定 | `evaluateGateReview()` | runtime / fs (checklist load) |
| **team** (`src/team/`) | team run の事前検証 + provider共通 launch plan + 難易度別の決定論的 model/effort policy + 明示 `--execute` 時の provider adapter 実行（AI-A/AI-B の identity/session/context 分離、duplicate identity/role 検出、`team_runner` slot fire/release） | `validateTeamRun()` / `selectTeamModel()` / `buildTeamRunPlan()` / `executeTeamRunPlan()` | schema / runtime / workflow |
