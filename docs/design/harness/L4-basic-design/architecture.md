...
| **orchestration** (`src/orchestration/`) | HELIX P2 hybrid orchestration の純粋契約コア。worker/verifier loop の決定関数 (`canResume`/`evaluateStop`/`selectVerifier`/`classifyRecovery`) を pure 実装し、自己評価禁止 (hybrid verifier≠worker) を機械担保 (PLAN-L7-175、L6=PLAN-L6-50)。DB 連動 (job-queue/tick/projection) は follow-up add-impl | `canResume()` / `evaluateStop()` / `selectVerifier()` / `classifyRecovery()` | schema (将来 ExecutionMode 経由 runtime) |
