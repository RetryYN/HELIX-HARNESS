...
| **handover** (`src/handover/`) | legacy session/prose互換module。Reverse-344のL7撤去sliceまでsource boundaryを維持するが、continuationの正本・新規機能の依存先にはしない | legacy `runHandover()` / CURRENT reader-writer | schema / fs。新規import禁止、撤去時に本行もatomic削除 |
