...
| **search** (`src/search/`) | `.ut-tdd/harness.db` の `search_index` を読み、PLAN/artifact/finding/skill/model/session の参照検索を提供する read-only query layer（読み取り専用 query 層） (PLAN-L7-47) | `findReference()` / `upsertSearchReference()` | state-db |
...
| 集約 (data.md §2) | 主担当 module | 検証 lint / guard | 状態ファイル (state、`.ut-tdd/`) |
|---|---|---|---|
...
| **git-command-guard** (有効 hook) | Claude `PreToolUse(Bash)` / Codex `PreToolUse(exec_command\|local_shell)` → `git-command-guard` → `git reset` / destructive `checkout` / `restore` / `revert` / force-push を理由付き override なしで block | **exit 2 = block** (fail-close)。bypass は `UT_TDD_ALLOW_DESTRUCTIVE_GIT=1` または `.ut-tdd/state/destructive-git-override` の非空理由を one-shot audit 付きで許可。`status` / `diff` / `log` / 通常 `push` / `checkout -b` は通す |
...
| **目標 hook** (未有効) | PreToolUse(Write/Bash)/PostToolUse 等 → `ut-tdd ...` | HELIX CLI 整備後に有効化 |
...
| その他 hook | PreToolUse(Write/Bash/WebSearch) 等 → package-local `ut-tdd` command | **未有効** (CLI 整備後、目標形は .claude/CLAUDE.md「Target HELIX Hooks」) |
