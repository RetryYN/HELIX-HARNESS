...
> `L2 画面設計(G2) → 本 L4 FE 設計標準 → L6 機能設計(per-screen) → L7 src/web 実装 → L10 システム/Real UX evidence(impl 後)`。
...
6. **mission で測る**: 完遂は工程管理表 (mission) で測る。描画数/implemented flip の coverage で名乗らない
   ([[feedback_coverage_not_substance]] の指摘)。
...
| warn | `color.status.warn.fg #9A6700` / `.bg #FFF8C5` | alert (🟡/△) | 警告: DEFER / partial / stale / a11y warn |
...
- **`ContinuationPanel`** (PM-05): harness.db の continuation read model を主表示し、session log、bounded memory、
  open feedback を provenance 付きで合成する。`ProjectionGapBanner` は DB欠損・再投影失敗を `error`、
  memoryとの不一致を `warn` 表示する。旧 CURRENT/prose 文書は入力にしない。
...
|---|---|---|
| PM-01（対象画面） | HierarchyPulldown + HeatmapGrid | FilterBar, StatusBadge, PollingIndicator |
| PM-02（対象画面） | LayerTemplate + ProgressBar + StaleBadge + CarryList + ScrumStateRow | DataTable, Breadcrumb |
| PM-03（対象画面） | GateResultPanel + TroubleTable + InterruptCopyButton | DataTable, CopyButton, NextActionCard, StatusBadge |
| PM-04（対象画面） | TraceGraph + VPairStatusTable | DataTable, StatusBadge |
| PM-05（対象画面） | ContinuationPanel + ProjectionGapBanner + FeedbackDetailList | NextActionCard, CopyButton |
| PM-06（対象画面） | DesignDocTree + DocPreview + DocToc | MarkdownRenderer, YamlFrontmatterView, MermaidRenderer, CopyButton |
| HM-01（対象画面） | HierarchyPulldown + FrStatusTable | DataTable, StatusBadge, FilterBar |
| HM-02（対象画面） | CoverageHeatmap + AxisSelector | StatusBadge, CopyButton |
| HM-03（対象画面） | WiringDiagram + ConnectionDetailTable + ModeTransitionArrows | DataTable, StatusBadge |
| HM-04（対象画面） | TableExplorer + IntegrityCheckSummary | DataTable, CopyButton, StatusBadge |
| HM-05（対象画面） | InvocationLogTable + SkillInjectionTab + HookFireLogTab + GuardDecisionList | DataTable, FilterBar, StatusBadge |
| HM-06（対象画面） | RecoveryLogTable + ResumePointList + RollbackCopyButton | DataTable, CopyButton |
| HM-07（対象画面） | DoctorResultTree + SeverityBadge + DetectionCountSummary | StatusBadge, CopyButton |
| HM-08（対象画面） | KpiDashboard + RecipeList | DataTable, FilterBar, StatusBadge |
...
- **実装は L7 (src/web、PLAN-L7-141)**。本 doc は回帰の **対象と粒度** を定める設計まで。回帰 snapshot の
  実行・WCAG 実比検証は impl 後 (L10 システム/Real UX evidence / a11y script) が担う (本標準は設計目標、実測は L7/L10)。
...
- **pair**: L10 システム/Real UX evidence 検証設計 (L4↔L9、画面横断の visual/a11y 一貫性をシステム挙動として検証)。
