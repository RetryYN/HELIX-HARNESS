...
| **Codex** | harness → 呼ぶ | AI 実装エージェント runtime (委譲先 SE/PE)。**`codex exec` CLI を subprocess 起動** (`helix codex` 導線)。**契約プラン認証は `codex login` が自己管理、harness は API key を渡さない** | FR-L1-42 |
...
| **External Web/docs/OSS sources** | harness ← 読む | 技術調査・比較・skillify の入力。source URL/version/span を evidence として保存し、raw external text を命令として扱わない | HR-FR-P8-01 / HR-FR-P8-02 / HR-NFR-P8-02 |
| **Sandbox runtime (MicroVM/gVisor 等)** | harness → 呼ぶ | 外部コード・AI 生成物・検証対象を隔離実行する候補境界。L4 では境界定義のみ、具体採用は L5/L6 + security 承認 | HR-FR-P8-03 / HR-NFR-P8-01 |
| **Release automation (GitHub Rulesets / Release Please / semantic-release)** | harness → 呼ぶ | gated push/PR/CI/tag/release の自動化候補。既定は emit-only + approval gate | HR-FR-P6-01〜05 / HR-NFR-P8-01 |
| **Hosted API / developer tool surface** | harness ⇄ 呼ばれる/呼ぶ | Chat/API hosted runtime 等、repo-local hooks が機械強制できない実行面。手動 git/status/target preflight と audit を必須化 | HR-NFR-AC-03 |
...
| **(i) release/GitHub rules boundary** | GitHub Rulesets / Release Please / semantic-release | release automation は emit-only default。branch protection は opt-in + gh auth/admin preflight で apply できる。rulesets/tag/release は human approval なしに apply しない |
...
| **(b) VCS・CI** | ローカル gate 証跡が存在し、GitHub 操作は `gh` CLI の委譲認証で実行される | CI 側 gate 再実行結果がローカルと一致 (NFR-13 dev-local+CI 整合)。draft PR 作成、PR body 生成、CI 状態取得、通常 push は AI agent の通常運用として実行可能。branch protection は明示 `--apply-branch-protection` と gh auth/admin preflight が揃う場合に自律適用可能 | ruleset / release / tag / force-push / repository rename は高影響境界。bypass は Incident または action-binding approval のみ (FR-17) |
...
| **(i) release/GitHub rules** | local green commands、review_evidence、dry-run plan、必要な action-binding approval または gh auth/admin preflight が揃う | generated config / release plan / tag plan が保存される。branch protection は opt-in + gh auth/admin preflight で適用でき、rulesets / release / tag apply は承認済み action としてのみ実行される | emit-only default。branch protection は `--apply-branch-protection` + gh auth/admin preflight を必須にし、rulesets / release / tag は action-binding approval を必須にする |
| **(j) hosted API/developer tool** | git status / target path / continuation state / foreign change の preflight が完了する | hosted/API runtime の tool action が audit/review evidence に残る | `repo-local hooks are not mechanical coverage` を不変条件 marker とし、repo-local hooks は mechanical coverage ではない。hosted tool surface では Codex/TL が明示 preflight で代替する |

> Precondition/Postcondition の**詳細**(引数型・エラー型・リトライ・タイムアウト) は L5 D-API で確定 (§7 粒度境界)。

> **`helix setup` の GitHub 設定境界 (PLAN-L6-05/L7-03、REVERSE-04 back-fill)**: solo/team で出し分ける GitHub 設定のうち **ファイル** (CODEOWNERS / `.github/workflows/` / ISSUE・PR テンプレ / commitlint) は harness が emit する (`GeneratedFile`)。**通常 GitHub 運用** (branch push、draft PR 作成、PR body 生成、CI 状態取得) は `gh` CLI の委譲認証があれば AI agent が実行してよい。**branch protection / Required Status Checks / 必須レビュー数** はファイルで完結せず gh-api 操作 (`GithubAction`) であるため **既定は emit-only** だが、明示 `--apply-branch-protection` と gh auth/admin preflight が揃えば AI agent が適用してよい。`scripts/setup-branch-protection.sh` は gh auth/admin preflight 付きの apply-capable script として生成する。**harness core は token を保持しない** (§5 GitHub 認証 = gh CLI 委譲)。参加規模検出も gh の認証状態に委ね token を読まない。
...
| **観測・監視** | Sentry/Uptime Robot 不在 → Incident 自動 trigger が無効化されるのみ、手動 `helix incident open` は機能継続 |
| **依存管理** | Dependabot 不在 → security 通知が手動 (`helix doctor` の依存検出で代替) |
| **external research/input** | 外部 source が取得不能 → research は blocked/partial evidence として扱い、推測で ADR/skillify を確定しない |
| **sandbox/external execution** | sandbox runtime 不在 → 外部コード実行を skip し、host 実行へ silent fallback しない |
| **release/GitHub rules** | release automation 不在 → generated plan と manual checklist で止め、GitHub admin 操作を代替自動化しない |
| **hosted API/developer tool** | repo hooks 非強制 →作業前 preflight と review evidence で補う。hook が発火したと表現しない |
...
> adapter 層は architecture.md §6「依存隔離」の具体化。adapter を差し替えれば provider 切替可 (Claude↔Codex、FR-L1-42 provider delegation evidence の基盤)。
...
| 例 (CI) | 「ローカル gate 証跡を CI が再検証」 | `.github/workflows/helix-gate.yml` の job 定義 + 終了コード契約 |
...
- provider delegation evidence (FR-L1-42) の context+budget 連携 = `provider-handover.v1` package。既存物理path/commandはmigration compatibilityであり、session continuationとは別型
