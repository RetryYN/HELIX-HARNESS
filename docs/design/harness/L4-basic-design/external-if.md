...
| **(i) release/GitHub rules** | local green commands、review_evidence、dry-run plan、必要な action-binding approval と gh auth/admin preflight が揃う | generated config / release plan / tag plan が保存され、branch protection / rulesets / release / tag apply は承認済み action としてのみ実行される | emit-only default。branch protection は opt-in だけで apply せず、action-binding approval + gh auth/admin preflight を必須にする |
