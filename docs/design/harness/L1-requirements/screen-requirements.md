...
> **配置 (ADR-005 D2、solo 読み替え)**: 本 UI は **本人の全 project 横断の管理ツール**であり、project-local でない。**本人の GitHub project repo 群を data backbone** に読み、harness 工程の粒度で詳細可視化する (GitHub native 可視化の工程・詳細版)。`案件横断` = 本人の全 project 横断の意 (旧「チーム全 project」を concept §1.3 大域コンベンションで solo 読み替え)。backend (中央/本人 server) 配置・通信境界は L2 設計 (ADR-005 D2、ADR-003 §IMP-031)。Phase A local dashboard はその bootstrap (technical-requirements §2 carry note)。
...
| **情報要素** | gate ID / 判定結果 (pass/fail/bypass) / 証跡一覧 (artifact リンク) / fail 理由テキスト / next_action (1 アクション明示) / サインオフ者 (PO/TL) / bypass 承認記録リンク。**発生中トラブル横断**: 種別 / 検出時刻 / 影響範囲 / next_action (gate fail + drift + continuation projection stale + 暴走シグナルを一覧化) |
...
#### §1.PM.05 PM-05 Continuation ビュー 詳細

| 観点 | 内容 |
|------|------|
| **対応 BR/UX/FR-L1** | UX-03 / FR-L1-01 |
| **情報要素** | `harness.db` continuation projection (next_action / 現 phase / 未解決 carry / 最終 gate 結果 / source event digest) / carry 詳細一覧 / continuation event 履歴 / projection freshness。DB projection が無い場合は推測値を表示せず fail-close 状態を示す |
| **操作要素** | next_action 実行 → 該当 PM ビュー遷移 / PLAN 参照 → PM-02 工程ビュー / continuation audit evidence 確認 / next_action AI 指示テキストコピー |
| **更新頻度** | セッション開始時 auto 表示 (S6=a) / 30 秒ポーリング (S2=b) |
| **状態種別** | continuation あり (next_action 強調) / continuation なし / projection stale・不整合 (fail-close) / 読み込み中 |
| **実装状態** | not-implemented |
...
| 画面 ID | 画面名 | 主要目的 | 対応 BR/FR |
|---------|--------|---------|-----------|
| **HM-01** | 機能一覧ビュー | FR-L1 51 件 × implementation_status 可視化 (3 階層プルダウン) | FR-L1-20 / FR-L1-29 |
...
| **情報要素** | 3 階層プルダウン: (1) 整備率% サマリ / (2) カテゴリ別 (P0/P1/P2) / (3) FR-L1-NN 機能個別。FR-L1 全 51 件を installed/partial/not-implemented + 担当 PLAN + **対応画面 (screen §5 trace、各 FR がどの画面要求で実現されるか)** で詳細テーブル表示 |
...
### シナリオ 4: セッション再開時 (Continuation → PM)

- PM-05 Continuation ビュー: セッション起動時に DB projection を auto 表示 (S6=a)
...
| **next_action 明示** (gate fail 時・continuation 時に「次にすること」を 1 クリックで確認) | UX-03 / BR-05 |
...
| **Continuation セッション開始時 auto 表示** (S6=a、セッション起動時に DB projection が存在すれば PM-05 を先頭表示。欠落・不整合は fail-close 状態を表示) | S6=a / `helix session start` |
...
| **BR-22** | **HM-02 (カバレッジ、直接)** | HM-05 (audit) / GD-01 (Architecture) | 内部資産 roster / skill pack / command の HELIX 化状況 + drift lint |
...
| **UX-03** | PM-03 (next_action 提示) / PM-05 (Continuation) / HM-06 (Recovery CLI コピー) | GD-01 (Troubleshooting) | gate fail / continuation / Recovery で next_action 明確 |
...
|----------|--------|--------|-----------|
| **FR-L1-01** (V字 PLAN 起票管理) | PM-02 | PM-01 / PM-05 | 工程ビュー + 俯瞰 + continuation 連動 |
...
| **FR-L1-28** (HELIX W 2 段設計) | PM-04 | Trace ビューで W 2 段状態 |
| **FR-L1-29** (画面設計ワークフロー L2) | PM-02 | L2 工程進捗 (本 sub-doc 自体が L2 前段) |
| **FR-L1-30** (フロントデザイン UX L10) | PM-02 | L10 工程進捗 |
| **FR-L1-31** (コンテキスト管理) | PM-05 (Continuation) | DB continuation + 文脈管理 (functional 整合、A-54) |
...
> **solo 読み替え (concept §1.3 大域コンベンション)**: 人間ペルソナは実体としては **本人 (PO) 1 名**。下表の「harness 運用者」「新規参画者」は別人ではなく本人の別ロール (harness 保守時 / 再習熟時) を指す。複数人 team 運用は team-mode への将来互換拡張であり、charter solo 既定では本人 = 全ペルソナ。
...
| ペルソナ | 最優先画面 | 補助画面 |
|---------|-----------|---------|
| PO | PM-01 (俯瞰把握) / PM-03 (gate サインオフ) / PM-05 (Continuation) | PM-02 / PM-04 / PM-06 (設計書プレビュー) / HM-05 / HM-07 / GD-01 |
...
|----------------|------|---------|---------|
| **PM** (Project Management) | PM-01〜PM-06 | 案件遂行に関わる進捗・担当・詰まり・gate・trace・continuation・設計書プレビュー | 毎日変化する動的状態 |
...
- PM-05 Continuation → PM-02 工程ビュー (next_action 実行)
...
| SCR-06 (旧 Handover ビュー) | PM-05 Continuation ビュー | session/prose pointer を廃止して DB projection 表示へ移行 |
