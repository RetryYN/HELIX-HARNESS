...
| command assets | `docs/commands` 19 件、legacy CLI binaries 70 件 | FR-L1-48 + 既存 core CLI FR (FR-L1-01/05/10/11/13/14/17/18/20/23/33/34/37/39/42) | 被覆済み。動作は TS/Bun subcommand として再実装、command docs は HELIX CLI docs へ curate |
...
**無修正転用の境界**: runtime として無修正転用できる legacy source 資産は **0 件**。`vendor source snapshot` は read-only evidence / regression idea としてのみ無修正参照できる。実行ロジック、hook、CLI、guard、catalog、lint は HELIX 所有パスで TS/Bun 再実装する。`.claude/agents/*.md` と `docs/skills/**/*.md` は TS literal 化せず markdown 正本として扱うが、legacy source 前提除去・role/capability class 化・用語置換・trigger 整備は必須。
...
| `feedback_hook` (legacy source 5 軸) | `.helix/hooks/feedback.ts` (Bun、ADR-001 整合) | FR-L1-19, FR-L1-20 |
...
注記の目的: source-derived 知見を概念的に取り込みつつ、実装は HELIX 独自 (TS/Bun + `.helix/` YAML/JSON state + `.helix/harness.db` projection + 個別 CLI) で再構築する。
...
| 単体テスト実行履歴を evidence DB 化する (A-122) | FR-L1-02 / FR-L1-06 / FR-L1-17 / FR-L1-18 / FR-L1-20 / FR-L1-50 | `tests_green_at` はあるが、どの unit-test がどの PLAN / FR / U-* oracle / artifact を証明したか、Bun/vitest/doctor/lint の実行履歴・flake・duration regression を query する要求粒度が不足 | physical-data §9.4 / function-spec `recordTestRunEvidence` / L8 IT-DB |
