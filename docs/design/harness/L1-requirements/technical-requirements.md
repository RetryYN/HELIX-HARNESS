...
| **AI プロバイダ間引継ぎ (F5=a)** | Claude ↔ Codex 間の provider-handover パッケージ (context + PLAN + budget) を `.helix/handover/provider/` に保存 | FR-L1-42 |
...
| **運用者ロール** | `.helix/` 直接編集 / `gate-checks.yaml` 更新 / hook 有効化のみ可。gate サインオフ / PLAN 削除は不可 (S-04) |
| **Windows sandbox** | Codex 内 PowerShell が 8009001d で起動失敗。大きな input は task ファイル埋め込みで回避 |
| **WSL2** | 任意の互換環境。必須条件ではない |

## §4 state schema 二層構造

HELIX では `.helix/` 配下に以下の二層構造でデータを保持する (FR-L1-06 参照):

### 中核テーブル (plan / artifact / pair / gate / phase)

| state | 内容 | 対応 CLI |
|-------|------|---------|
| `plan_registry/` | PLAN 起票記録・kind・status・generates/requires グラフ | `helix plan` |
| `artifact/` | 設計 / テスト / 実装 doc の artifact 記録 | `helix trace` |
| `pair/` | V-model pair 対応 (L1↔L14 等) の freeze 状態 | `helix gate` |
| `phase.yaml` | 現在の V-model 工程位置・gate 通過記録 | `helix status` |
| `contract_registry/` | API / schema / skill の contract 記録 | `helix doctor` |
| `skill_catalog/` (A-52 audit I-3) | skill 定義一覧 (FR-L1-12 / FR-L1-19 / db-integration.md 本線 DB 6 種) | `helix skill suggest` (L4 carry) |
| `command_catalog/` (A-52 audit I-3) | HELIX CLI コマンド一覧 (registry、db-integration.md 6 種の 1 つ) | `helix command` (L4 carry) |
...
### 監査 / event テーブル (invocation_log / detector_runs / gate_runs / failure_log)

| state | 内容 | 対応 CLI |
|-------|------|---------|
| `invocation_log/` | AI 実行イベント全量ログ (FR-L1-20 / OpenTelemetry) | `helix audit` |
| `detector_runs/` | detector 実行結果 (drift / balance_ratio / trace 整合) | `helix doctor` |
| `gate_runs/` | gate 判定実行結果・証跡 | `helix gate` |
| `failure_log/` | 逸脱警告・暴走検知・agent guard block ログ | `helix doctor` |
| `action_logs/` (A-52 audit C-2) | Edit / Write 等のファイル変更記録 (observability-metrics.md 由来、FR-L1-20 extended note) | `helix audit` |
| `budget_events/` (A-52 audit C-2) | トークン予算超過イベント (observability-metrics.md 由来、FR-L1-20 extended note、NFR-12 課金モード制約と連動) | `helix audit` |
...
`.helix/drive/<drive>/` ディレクトリで drive 別に state を分離管理する。drive は以下 5 専門職区画:
...
| `recommended_commands` | 推奨 `helix` サブコマンド |
...
**mode 統一合流原則** (business §3.3 参照): Forward / Research / Add-feature / version-up を除く 7 mode は収束時に Reverse の closure mechanism (R4 routing / RGC / fullback) を共通再利用する。Add-feature は直接 Forward 差分追補、version-up は activation まで parked とし、活性化時に Add-feature / Forward へ接続する。

R0-R4 + RGC (Reverse Gateway Closure) を Reverse 専用ではなく **共通 closure language** として再利用する (FR-L1-14 参照):

| 共通基盤要素 | 内容 |
|-------------|------|
| **Forward 接続 event** | state 登録: Reverse / Discovery / Incident 収束後、Forward 合流ポイントを `.helix/phase.yaml` に記録 |
| **補助 state への中間 state 保存** | R0-R4 / S0-S4 の各中間成果物を `recovery_log/` / `mode.yaml` に保存 |
| **discrepancy_log からの機械起動** | drift / 劣化 / 暴走シグナルを `failure_log/` が受け、`helix doctor` が mode 自動ルーティング (FR-L1-08) |
...
| 方針 | 内容 |
|------|------|
| **detector 週次以上起動** | `helix doctor` を週次以上で起動し、drift を早期検出 |
| **inventory schema 双方向 mapping** | 設計 PLAN ID ↔ テスト設計 ID ↔ 実装 ID を `.helix/artifact/` で双方向 mapping |
| **新規 asset 工程未割当不許容** | 新規 asset (doc / code / test) を工程 (L-ID) に割当済みでないと `helix doctor` が warning |
