...
| **`.ut-tdd/` state** | HELIX runtime state の正本。大半は gitignored |
...
HELIX では `.ut-tdd/` 配下に以下の二層構造でデータを保持する (FR-L1-06 参照):
...
| `command_catalog/` (A-52 audit I-3) | HELIX CLI コマンド一覧 (registry、db-integration.md 6 種の 1 つ) | `ut-tdd command` (L4 carry) |
...
`.ut-tdd/drive/<drive>/` ディレクトリで drive 別に state を分離管理する。drive は以下 5 専門職区画:
