...
UT-TDD Agent Harness（HELIX の土台）は、AI 実装エージェントに開発を安全に委譲するための検証・開発基盤。V-model (L0-L14) 全工程の PLAN 管理・gate 判定・trace 整合を機械強制し、AI 委譲しても工程規律が崩れない環境を提供する。**超個人開発（solo）を前提とし、人間 1 名（PO）＋ AI agent roster（tl/qa/aim/uiux 役割を AI が担う）で全工程を回す**。
