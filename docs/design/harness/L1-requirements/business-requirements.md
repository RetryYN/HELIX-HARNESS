...
# HELIX — L1 業務要求 (business)

## §1 目的・背景

> **HELIX solo 改訂 (PLAN-L1-06)**: 「社内開発チーム」前提を **超個人開発（1 人の開発者が開発を AI に委譲）**へ写像（concept §1.3 solo 読み替えコンベンション準拠）。機械機構は不変、担い手のみ solo+AI agent roster へ。

### §1.1 WHY

AI 実装エージェントに開発を委譲して安全に使うための検証・開発基盤が存在しない。AI 委譲しても回帰が壊れず、設計⇔実装⇔テストの整合を機械強制できる仕組みが必要。**HELIX では 1 人の開発者（PO）が L0 企画・L1 業務要求・L2 デザインモック・L3 承認のみを担い、L3 起草〜L7 実装〜L8–L14 検証〜PR/CI/merge/tag を AI agent roster が無人完走する**ため、この機械強制基盤が不可欠。

### §1.2 WHAT

HELIX は、AI 実装エージェントに開発を安全に委譲するための検証・開発基盤。V-model (L0-L14) 全工程の PLAN 管理・gate 判定・trace 整合を機械強制し、AI 委譲しても工程規律が崩れない環境を提供する。**超個人開発（solo）を前提とし、人間 1 名（PO）＋ AI agent roster（tl/qa/aim/uiux 役割を AI が担う）で全工程を回す**。この文脈で `HELIX-HARNESS` は、この仕組みを実装・配布する repository / harness package 名として扱う。
...
| **BR-02** | **AI agent roster の責務境界**（人間 = PO 1 名 = L0/L1/L2-mock/L3 承認のみ、AI-A = 作成・修正、AI-B = 監査・merge 判断）が日常 PR で gate・レビュー・役割境界を無理なく回せる。family/provider は不問とし、creation と judgement の identity/session/context を分離して（worker≠verifier）役割境界を機械強制する（NFR-05 の GitHub 権限正本 + P2 orchestration に連結） | concept P2（AI agent 責務境界）/ 成功② / charter §3 自律境界 |
...
| **BR-22** | **自前 runtime 内部資産体系を持つ** — HELIX は自身が使う/対象に提供する **subagent roster / skill pack / command** を HELIX 用の正本資産として持ち、source-derived資産を「そのまま使う」のでなく **HELIX 用に再構築**する。guard (呼出統制) だけでなく資産そのものを統制対象とする (再構築の HOW = FR-L1-46〜49) | A-77 PO 指摘 (前提抜け) / Recovery PLAN-RECOVERY-01 |
...
| **入口 mode** | Forward + entry modes (Reverse / Discovery / Refactor / Retrofit / Recovery / Scrum / Incident / Add-feature / version-up / Research) + 工程専門 2 (screen-design / frontend-design) | concept §2.5 + version-up 追補 / docs/process/modes |
| **駆動タイプ** | 5 専門職 drive (be / fe / fullstack / db / agent)。Scrum / Reverse / PoC / Troubleshoot / version-up は drive ではなく mode | requirements §1.6 / docs/process §1 |
...
| state 管理 | `.helix/` 配下の一元 state 管理・drift 検知 |
...
| version-up | 今版外だが将来版へ保全する作業 | `version_target` + activation packet + parked review | 将来版活性化時 → Add-feature / Forward 合流。activation までは完了扱いしない |
...
### §3.3.1 mode 統一合流原則 (B10=a / PO 承認 2026-05-28 + version-up 追補)

Forward / Research / Add-feature / version-up を除く 7 mode (Reverse / Discovery / Refactor / Retrofit / Recovery / Scrum / Incident) は、収束時に **Reverse の closure mechanism** (R4 routing / RGC / fullback) を再利用して Forward L0-L14 へ合流する。これにより全 mode の「終わり方」を統一し、Forward 以外の成果物が V-model の外に放置されることを防ぐ。

> **Add-feature 例外**: Add-feature は既存 PLAN への差分追補が設計思想であり、Reverse closure を必須としない。Reverse は「前段 option」として使用可能だが、強制ではない (根拠: legacy source add-feature-workflow.md §基本フロー / concept §2.5 既宣言)。HELIX では本原則として明示記述する。
> **version-up 例外**: version-up は将来版保全の parked mode であり、activation decision / parked review / action-binding approval / activation snapshot が揃うまでは Forward 完了に数えない。活性化時は Add-feature または通常 Forward descent へ接続する。
...
| **AI roster — verifier (AI-B、独立identity/session/context)** | L4+ 品質・検証・gate 通過 | L4–L9 | **L4 以降の gate (worker≠verifier、自己評価禁止)** | 独立文脈レビュー / gate 通過判定 |
| **AI roster — worker** | 実装・テスト・文書起草 | L3 起草・L4–L11 | — (worker は判定不可) | 実装・PR 作成・**commit/merge/tag 自律**（不可逆のみ escalate） |
| harness 運用者ロール (= 本人) | 基盤導入・hook 有効化・state 管理 | 全フェーズ | — (gate サインオフ禁止) | `.helix/` state 直接編集 / hook 有効化 (S-04)。PLAN 削除禁止 |
...
|--------|--------|--------|--------|---------|
| **D-01** | PLAN 起票数/sprint | sprint 期間中に起票された PLAN 件数 | ≥ 1 件/sprint | `.helix/plan_registry/` / `helix plan list` |
| **D-02** | gate 通過率 | gate pass 件数 / gate 総実行件数 × 100 | ≥ 90 % | `.helix/gate_runs/` / `helix gate log` |
| **D-03** | V-model 順序遵守違反 | 前工程未完了で後工程着手した検知件数 | 0 件 | `helix doctor` / `helix plan lint` |
| **D-04** | 回帰検出率 | テストで検出した回帰件数 / 回帰発生総件数 × 100 | ≥ 80 % | CI gate / `helix trace` |
| **D-05** | 4 artifact trace 整合率 | trace 整合 PLAN 件数 / 全 PLAN 件数 × 100 | ≥ 95 % | `helix trace check` / `.helix/artifact/trace/` |
| **D-06** | agent guard bypass 件数 | `HELIX_ALLOW_RAW_AGENT=1` 実行件数 (audit 記録) | 0 件 目標 (PO 承認時のみ許容) | `.helix/audit/` / agent-guard log |
| **D-07** | AI 委譲時間率 | AI 委譲タスク工数 / 総開発工数 × 100 | ≥ 70 % | PLAN `drive:` 集計 / `helix status` |
| **D-08** | gate override 件数/sprint | PO による gate fail-close 例外行使件数 | ≤ 2 件/sprint | `.helix/audit/` / gate override log |
| **D-09** | continuation 再開成功率 | next authority 実行成功件数 / continuation event 総件数 × 100 | ≥ 95 % | `harness.db` continuation projection / `helix status` |
...
| BR-22 | OT-46 | 内部資産 (roster/skill pack/command) が HELIX 用に再構築済 + legacy source 前提残存 0 の確認 |
...
- 要件定義書: `docs/governance/helix-harness-requirements_v1.2.md`
...
| BR-22 内部資産 (subagent roster / skill pack / command) の HELIX 化詳細 (FR-L1-46〜49 / 棚卸 = internal-asset-inventory.md / rebuild map W6/W7/W10/W11/W12/W16 / drift=IMP-033) | L3 FR / L4-L6 | Recovery PLAN-RECOVERY-01、設計増分は L4-L6 |
...
| B7: continuation event / delivery receipt の retention・削除自動化 | L3 FR / L4 | `harness.db` + memory lifecycle |
...
| BR-21: FR-L1-36/38/43 の詳細仕様 (AI 実行品質評価・改善サイクル) | L3/L6/L7 | projection 実装済 (PLAN-L7-53/57/58)、§11 参照 |
| mode 統一合流シナリオの具体フロー (各 mode の closure 手順・RGC 設計) | L3 FR / L4 | §3.3.1 原則 → 機構設計 |
...
| **(PdM tech-innovation A-46)** SPACE Satisfaction → D-14 reviewer cognitive load (Likert 1-5、CC2 measurable proxy) + D-15 continuation record 完全性 + D-16 gate G2-G7 待機時間 + D-17 PLAN diff median LOC | L4 ADR + Phase A NFR | AI harness 固有: reviewer 疲労が CC2 形骸化リスク |
...
| **(PdM marketing-innovation A-46)** **BR-multi-01/02 + FR-L1-multi-01/02** tenant 分離 + cross-team continuation + team-level dashboard + policy inheritance (org→team→repo 3 階層 CLAUDE.md) | Phase B carry | multi-team 拡張時に活性化、solo maintainer では over-engineering |
...
| **(tech-docs A-46)** NFR 3-tier classification (A: doctor 自動判定 / B: CI 後人間確認 / C: PO 合意のみ) → `docs/design/nfr-classification.md` | L4 carry | IPA × ISO 25010 二軸の実装適用、L7 で `helix doctor --nfr` 実装入力 |
...
| **(A-52 audit I-02)** Learning Engine recipe 蓄積・skill 推薦ループ全体を BR-21 後続改善に明示化 (recipe entity + pattern_key + L 単位注入自動更新) | L3 BR-21 詳細化 / HM-08・改善ループ carry | learning-engine.md §学習の処理 / 出力、FR-L1-36/38/43 の projection は PLAN-L7-53/57/58 で実装済み |
...
| **(A-52 audit M-03)** continuation 業務価値 (セッション間 AI 委譲継続性保証) の BR 明示化検討 (現状 BR-03/BR-05 の暗黙前提、BR-09 候補) | Phase B carry | continuous-run-context-management.md §本命 PoC、D-09 で KPI 化済だが BR 明示なし |

## §10 業務 entity 列挙 (DDD)

### §10.1 主要業務 entity 一覧

| entity | L0 用語 | 業務的意味 | 対応 .helix state + CLI subcommand + file |
|--------|---------|-----------|-------------------------------------------|
| **plan** | PLAN | 工程単位の作業計画 (工程表 + 実装計画内蔵) | `.helix/plan_registry/` / `helix plan` / `docs/plans/*.md` |
| **gate** | Gate (G0.5-G14) | 工程間通過判定点 (fail-close) | `.helix/phase.yaml` / `helix gate` / `gate_runs` |
| **artifact** | 成果物 | PLAN が generates する設計 / テスト / 実装 doc | `.helix/artifact/` / `helix trace` / `docs/design/`, `tests/` |
| **pair** | Pair (V-model) | 設計 ⇔ テスト設計の双方向対応 (L1↔L14 等) | `pair_artifact` frontmatter / `helix doctor` |
...
| **continuation** | Continuation | セッション間の再開状態 | `harness.db` continuation projection / shared memory / `helix status` / continuation doctor gate |
...
| **trace** | trace | 上流 ID → 下流 ID の双方向追跡記録 | `.helix/artifact/trace/` / `helix trace` |
...
| **skill_evaluation** | business-detail §7 FR-BR21-36 | skill 採用率 + 採用後成功率の projection | L7 projection 実装済 / HM-08 表示 carry |
| **model_evaluation** | business-detail §7 FR-BR21-38 (opt-in) | model 別成功率 / token・cost 効率、`model-opt-in.yaml` 制御 | L7 projection 実装済 / HM-08 表示 carry |
| **poc_evaluation** | business-detail §7 FR-BR21-43 | Discovery (S0-S4) decision_outcome 集計 | L7 projection 実装済 / HM-08 表示 carry |
...
| **entity ↔ schema / CLI 整合検出** | `helix doctor check_business_entity_coverage` の実装設計 | L4 データ設計 §7 + L4 CLI 設計 |

検証: 各 carry 項目は L4 PLAN (PLAN-L4-04 データ設計) 起票時に `dependencies.requires` で本 sub-doc を列挙し、`helix plan lint` (sub_doc=business 時) で entity 独自定義禁止 (L0 §10 用語集との 1:1 対応) を機械検証する。

### §10.3 SSoT 参照

- ユビキタス言語: L0 `docs/governance/helix-harness-concept_v3.1.md §10 用語集`
- Bounded Context: L0 §2.5 9-mode ecosystem + version-up 追補 **+ 画面要求 3 カテゴリ Bounded Context (PM / HM / GD、X1=a 採用 2026-05-28)**
- 業界標準整合: L0 §11 参考文献
- entity 独自定義禁止: `helix plan lint` (sub_doc=business 時) で機械検証
...
| カテゴリ | 性質 | 主ペルソナ | データ源 |
|---------|------|----------|---------|
| **PM** (Project Management) | 動的 (案件遂行) | PO | `.helix/` runtime state + `harness.db` (plan/gate/phase/continuation/trace) |
| **HM** (Harness Management) | 動的 (harness 改善) | harness 運用者 | `.helix/` audit/event (invocation_log/failure_log/detector_runs) |
...
|------|-----|
| **ID** | BR-21 |
| **優先度** | P2 起点 (FR-L1-36/38/43 は登録・実装済み) |
| **実装フェーズ** | projection は PLAN-L7-53/57/58 で実装済み。HM-08 表示・改善アクション自動適用は後続 carry |
| **L3/L7 接続** | FR-L1-36 / FR-L1-38 / FR-L1-43 は L3 business-detail で詳細化済み、L6 fr-unit-coverage と L7 oracle に接続済み |
| **PO 承認** | 2026-05-28 (F2=a 採用) |

### §11.1 業務目標

HELIX が AI 実行品質を自律的に評価・改善するサイクルを持つ。個別 AI 委譲の成否を蓄積・分析し、skill 推奨精度・model 選択基準・PoC 成功率を継続的に向上させる。これにより、本人が AI 委譲を繰り返すほど harness が賢くなる「学習する基盤」を実現する。

### §11.2 構成要素 (L3 詳細化済み / L7 projection 実装済み)

| FR ID | 概要 | 現行状態 |
|-------|------|-------|
| **FR-L1-36** | skill 評価: skill_invocations + plan_registry から rating / adoption / success / unused を projection | PLAN-L7-53 実装済み |
| **FR-L1-38** | model 評価: model_runs + plan_registry から success_rate を projection、token/cost 効率も telemetry から集計 | PLAN-L7-53/57/58 実装済み |
| **FR-L1-43** | PoC 計測: Discovery decision_outcome から confirmed/rejected/pivot 成功率を projection | PLAN-L7-53 実装済み |
