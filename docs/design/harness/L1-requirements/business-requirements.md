...
→ L9 結合テスト → G8 → L10 システム/Real UX evidence 検証 → G9
→ L10 UX → G10 → L11 受入/human visual → G11
...
L3 sub-doc 本起草 (functional / business-detail / nfr-grade + L12 運用/価値検証) で発生した新概念を L1 entity に逆方向 back-propagation する。今後 L4 で集約境界 / 値オブジェクト確定する対象として宣言。
...
| **acceptance_test** (AT) | L12 運用/価値検証設計 §1 (87 件) | 各 AC を vitest / GHA workflow に変換、機械検証可能 | 集約境界 = AC 配下、AT 実装は L7 carry |
