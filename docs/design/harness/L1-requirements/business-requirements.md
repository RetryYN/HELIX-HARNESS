...
# HELIX Agent Harness — L1 業務要求 (business)
...
HELIX Agent Harness は、AI 実装エージェントに開発を安全に委譲するための検証・開発基盤。V-model (L0-L14) 全工程の PLAN 管理・gate 判定・trace 整合を機械強制し、AI 委譲しても工程規律が崩れない環境を提供する。**超個人開発（solo）を前提とし、人間 1 名（PO）＋ AI agent roster（tl/qa/aim/uiux 役割を AI が担う）で全工程を回す**。
...
- **harness 運用者**: HELIX Agent Harness を社内開発基盤として導入するチーム。
...
- 要件定義書: `docs/governance/helix-agent-harness-requirements_v1.2.md`
...
| **mode** | Forward + entry modes | 開発経路種別 (Forward / Reverse / Discovery / version-up 等) | `.ut-tdd/mode.yaml` / `ut-tdd status` |
...
- ユビキタス言語: L0 `docs/governance/helix-agent-harness-concept_v3.1.md §10 用語集`
...
HELIX Agent Harness が AI 実行品質を自律的に評価・改善するサイクルを持つ。個別 AI 委譲の成否を蓄積・分析し、skill 推奨精度・model 選択基準・PoC 成功率を継続的に向上させる。これにより、チームが AI 委譲を繰り返すほど harness が賢くなる「学習する基盤」を実現する。
