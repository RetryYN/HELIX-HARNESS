...
architecture.md §3 の 7 building block（構成要素）を実装単位のモジュール (関数群・公開 IF) に詳細化する (PLAN-L5-02)。export は `src/` の実装と 1:1、L7 完遂時点のモジュール公開面は下表の実装証跡へ着地済み。

## §1 モジュールインベントリ

| モジュール | path | 実装状態 | 責務 (architecture §3) |
|---|---|---|---|
| **cli** | `src/cli.ts` | 実装済 (scaffold、骨組み) | コマンドディスパッチ + 副作用端点 |
| **composition** | `src/composition/db-rebuild-composition.ts` | 実装済 | owner間のlifecycle配線だけを担うcomposition root。state-db / vmodel / vscodeの依存をここへ限定 |
| **schema** | `src/schema/index.ts` + `frontmatter.ts` | 実装済 | enum/契約の単一正本 (安定核) |
| **shared** | `src/shared/repo-info.ts` + `shell-quote.ts` | 実装済 | 上位 module に依存しない repository identity / shell quoting の共通 pure utility |
| **lint** | `src/lint/*.ts` | 実装済 | doc/PLAN/trace 静的検証 |
| **plan** | `src/plan/lint.ts` | 実装済 | PLAN frontmatter / naming / dependency lint |
| **vmodel** | `src/vmodel/lint.ts` | 実装済 | V-model 4 artifact trace の lint |
...
| **continuation** | `src/runtime/session-log.ts` + `src/state-db/` + `src/memory/` + `src/feedback/` | Reverse-344で統合 | append-only event、DB read model、bounded memory、feedback lifecycle。旧handover moduleは廃止対象 |
...
| **review** | `helix review --uncommitted` + `src/lint/review-evidence.ts` | 実装済 | doc-reviewer 召喚 (FR-45) |
| **context** | `src/context/doc-router.ts` | 実装済 (pure router、副作用なし router) | canonical doc の heading/section index と task kind 別 section selection。fail-open で全 doc 読みへ戻す (PLAN-L7-315) |
| **skill** | `src/skills/recommend.ts` + `src/workflow/contracts.ts#suggestSkillInjection` | 実装済 | L 別 skill 注入 (FR-12) |
| **skill-engine** | `src/skill-engine/scaffold.ts` | 実装済 (pure generator、副作用なし generator) | skill.v1 authoring scaffold（authoring の骨組み）。file write は CLI 境界へ隔離 (PLAN-L7-317) |
...
| **cutover** | `helix cutover --to --dry-run` | 実装済 (approval gate) | ロールバック (FR-10/26) |
...
### §2.4 `cli` / `composition` / `plan` / `vmodel` / `doctor`
- `cli.ts`: `program` (commander)。action は runtime/doctor/plan/vmodel を呼ぶ薄い dispatcher
- `composition/db-rebuild-composition.ts`: `rebuildHarnessDb()`。state-db coreへadapter-neutral summary portを注入する唯一のowner-crossing composition root
- `plan/lint.ts`: `LintResult` interface / `lintPlan(path?)` (frontmatter、PLAN ID、依存関係の検証)
- `vmodel/lint.ts`: `lintVmodel(path?)` (4 artifact trace validation)
...
|---|---|---|
| lint (各) | `analyzeX(docs?: Source): Result` | docs 注入可 (pure、副作用なし)、result = `{orphans[], totals}` |
...
| composition | `rebuildHarnessDb(input?): RebuildHarnessDbResult` | state-db coreへのsummary port注入のみ |
...
- **循環禁止**: import グラフに循環なし (D-03=0)。`helix vmodel lint` / dependency lint (knip 候補、L3 §7.1) で機械検証 carry
...
| continuation | session event append + DB projection/query + memory/feedback join。crash replayは冪等、DB precedence、旧surface不存在 | Workflow / Evaluation projection | L6 |
...
### §5.1 CLI / doctor monolith分割方針 (PLAN-L7-433 C3)

現行`src/cli.ts`と`src/doctor/index.ts`は公開composition rootを維持する一方、機能追加時にdomain logicを
抱え込まない。CLIは`src/cli/commands/<domain>.ts`のcommand registrarへ、doctorは
`src/doctor/checks/<gate>.ts`のcheck adapterへ段階抽出する。抽出先は既存domain moduleのpure APIを呼び、
判定logicを複製しない。command名、option、exit code、stdout JSON schema、doctor message/order、hard/warn
分類はcompatibility oracleで固定する。

本PLANでは方針確定までとし、一括移動は行わない。理由は両fileが多数runtime/PLANの同時編集面であり、
機械的分割がreview可能範囲を超えるためである。実装はrefactor候補をdomain単位で再計測し、各successor
PLANがparent design、本数上限、public surface snapshot、rollback単位を宣言してから着手する。
successor起票判断は`required`、一括big-bang splitは`rejected`とする。
...
- **L7 landed基盤と後続carry**: workflow/telemetry/hook/review/skill/cutover/adapter基盤、およびcontinuation素材の`src/runtime/session-log.ts`、`src/state-db/`、`src/memory/`、`src/feedback/`はlanded済み。一方、event-first writer/replayer、checkpoint publisher、resurrection hard gate、legacy `src/handover/`/CLI撤去はReverse-344 R4後のL7 PLANで実装する。現inventory gateをこれらの実装証拠にしない
...
|---|---|---|---|
| skill catalog | `docs/skills/**/*.md` を layer-1 source documents として scan し、in-memory catalog として公開する。永続的な `.helix` state は導入しない。 | `skill -> schema/fs`; runtime/guard から import しない。 | L6 で scorer と injector の signature を定義し、L7 で catalog loading を実装する。 |
