...
| **トークンコスト** | PLAN ごとの sum(invocation_log.input_tokens + output_tokens) | (LCM 目標は L4 で確定、P0 では計測のみ) | `.ut-tdd/audit/invocation_log/` |
...
- **手動トリガー**: `ut-tdd evaluation run --since <date>` で任意期間集計
...
- **操作**: G7 通過 hook 発火
- **期待結果**: `ut-tdd evaluation run --auto` 自動実行 / sprint 内 PLAN 全件集計 / HM-08 ビュー更新

#### AC-FR-BR21-05 (異常系: 頻度上限超過)
- **前提**: 同日内に既に 1 回自動実行済、PO が手動再実行試行
- **操作**: `ut-tdd evaluation run --force`
...
- **操作**: `ut-tdd skill delete skill-X` が PO 承認なしで実行
...
集計バッチ (`.ut-tdd/evaluation/`) で 4 ソースを結合 → HM-08 表示用の派生ビューを生成。
...
- **model 評価**: opt-in (`.ut-tdd/evaluation/model-opt-in.yaml`)
