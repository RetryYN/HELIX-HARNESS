...
#### AC-FR-01-01 (正常系)
- **前提**: ユーザーが `ut-tdd plan draft --title "新機能X" --kind design --layer L3 --drive be` を実行
- **操作**: コマンドが完了
- **期待結果**: `docs/plans/PLAN-NNN-新機能X.md` が生成 / frontmatter (plan_id / title / kind / layer / drive / status / agent_slots / generates / dependencies) 全件入力済 / `.ut-tdd/plan_registry/` に登録 / 終了コード 0

#### AC-FR-01-02 (異常系)
- **前提**: 既に同一 plan_id (`PLAN-005-X`) のファイルが存在
- **操作**: 同じ ID で `ut-tdd plan draft` を再実行
- **期待結果**: fail-close で `Error: plan_id PLAN-005-X already exists` 出力 / 既存ファイル変更なし / next_action: `--plan-id <別ID> を指定するか、ut-tdd plan delete PLAN-005-X 後に再実行` / 終了コード 1

#### AC-FR-01-03 (境界系)
- **前提**: kind=charter で layer ≠ L0 を指定 (§1.1 排他制約違反)
- **操作**: `ut-tdd plan draft --kind charter --layer L4` 実行
...
- **操作**: 本体実装ファイル commit
- **期待結果**: `ut-tdd sprint check` が Red→Green 順序を確認 / TDD trace に Red commit ID → Green commit ID 記録 / pass

#### AC-FR-02-02 (異常系)
- **前提**: 本体実装ファイルだけ存在、対応 test ファイル無し
- **操作**: `ut-tdd gate G7` 実行
...
- **操作**: `ut-tdd doctor` (descent-obligation 検査)
...
- **操作**: `ut-tdd plan lint PLAN-L4-05`
...
- **操作**: `ut-tdd gate G3`
- **期待結果**: pass / `.ut-tdd/phase.yaml` の G3 status = passed / 証跡 `.ut-tdd/gate_runs/G3-<timestamp>.json` 生成 / 終了コード 0
...
#### AC-FR-05-03 (境界系)
- **前提**: PO が `UT_TDD_GATE_BYPASS=1` 環境変数で bypass 試行 (S-03 例外権行使)
- **操作**: `ut-tdd gate G3`
- **期待結果**: bypass 警告 + audit 記録 `.ut-tdd/audit/gate-bypass-<timestamp>.json` (PO ID + 理由必須) / phase.yaml は bypassed 状態 / KPI D-08 集計対象 / 終了コード 0 (bypass 成功)
...
- **期待結果**: `.ut-tdd/drive/be/plan_registry/PLAN-005.json` 生成 / `.ut-tdd/drive/fe/` には登録されない / 区画隔離 pass

#### AC-FR-06-02 (異常系)
- **前提**: 手動で `.ut-tdd/drive/be/plan_registry/PLAN-005.json` を `.ut-tdd/drive/fe/` に複製 (区画跨ぎ汚染)
- **操作**: `ut-tdd doctor`
...
- **期待結果**: `.ut-tdd/code_catalog/` に src/X.ts エントリ追加 / 関連 PLAN 検索 + `last_modified` 更新 / hook 終了コード 0

#### AC-FR-07-02 (異常系)
- **前提**: hook 実行中に書込み失敗 (`.ut-tdd/` permission denied)
- **操作**: hook 続行
- **期待結果**: fail-close `Error: state 自動登録失敗 (.ut-tdd/code_catalog/ 書込み不可)` / 元 git commit は成功扱い (post-hook のため) / next_action `chmod 確認、ut-tdd doctor で整合性チェック` / hook 終了コード 2 (warn 通知)
...
- **前提**: session-log hook 実行中に I/O 失敗 (`.ut-tdd/logs/` 書込み不可) または stdin/JSON 不正
...
- **操作**: `ut-tdd doctor` 実行
- **期待結果**: Reverse mode 自動 routing 提案 / `ut-tdd plan draft --kind reverse --workflow-phase R0` 候補 next_action 出力
...
#### AC-FR-08-03 (境界系)
- **前提**: 検出シグナル無し、PO が手動で `ut-tdd route --mode reverse` 強制
- **操作**: 強制実行
...
- **操作**: agent-guard 検証
- **期待結果**: 許可リスト pass / model 明示 pass / opus override なし / 終了コード 0 / audit `.ut-tdd/audit/agent-invocations/` 記録
...
#### AC-FR-09-03 (境界系)
- **前提**: `UT_TDD_ALLOW_RAW_AGENT=1` 環境変数で bypass 試行 (PO 専属 S-03)
- **操作**: 同じ呼び出し
...
- **期待結果**: 最終正常 gate を再開ポイント候補として提示 / `ut-tdd cutover --to <gate-id>` CLI コマンド生成 / クリップボードコピー UI 提供

#### AC-FR-10-02 (異常系)
- **前提**: 再開ポイント候補なし (全 gate fail 履歴)
- **操作**: Recovery mode 起動
- **期待結果**: warn `Warning: 再開ポイント候補なし。初期化推奨` / next_action `ut-tdd reset --to-charter (PO 専属確認)`
...
- **操作**: `ut-tdd interrupt --reason "P0 bug" --pause-plan PLAN-040`
...
- **操作**: `ut-tdd interrupt --pause-plan PLAN-040`
- **期待結果**: fail-close `Error: PLAN-040 はすでに interrupted` / next_action `ut-tdd resume PLAN-040 を先に実行`
...
- **操作**: `ut-tdd skill suggest --plan PLAN-005`
...
- **操作**: `ut-tdd plan draft --layer L4`
- **期待結果**: fail-close `Error: G3 未通過、L4 着手不可 (V-model 順序遵守)` / next_action `ut-tdd gate G3 実行`
...
- **操作**: `ut-tdd reverse --type code` → R0 (evidence) 完了
- **期待結果**: `.ut-tdd/reverse/R0-evidence.json` 生成 / 検出された関数 / クラス / 依存リスト記録 / 次 step R1 案内
...
- **操作**: R4 完了
- **期待結果**: `.ut-tdd/reverse/R4-routing.json` に gap-only 記録 / Forward 起動なし / gap-register に追加
...
- **操作**: `ut-tdd incident open --severity P0`
...
- **操作**: GHA workflow `.github/workflows/ut-tdd-gate.yml` 起動
- **期待結果**: CI 側 G7 再実行 pass / branch protection check pass / PR merge 可

#### AC-FR-17-02 (異常系)
- **前提**: ローカル G7 pass、CI G7 fail (環境差異)
- **操作**: PR
- **期待結果**: PR block / CI 側 fail 詳細を `.ut-tdd/audit/ci-divergence/` に記録 / next_action `ローカル環境差異調査 (NFR-13 dev-local+CI 整合)`
...
- **操作**: `ut-tdd reverse --type fullback --from-poc <plan-id>`
- **期待結果**: F0-F4 成果物が `.ut-tdd/reverse/F{0..4}.json` 生成 / Forward L3 合流 routing 提示 / 終了コード 0

#### AC-FR-23-02 (異常系: confirmed 以外で fullback 試行)
- **前提**: decision_outcome=rejected の PoC PLAN
- **操作**: `ut-tdd reverse --type fullback`
...
- **操作**: `ut-tdd reverse --type fullback --batch`
...
- **操作**: `ut-tdd plan draft --kind add-impl --parent PLAN-040 --title "X 機能追加"`
...
- **操作**: `ut-tdd sprint check --regression-only`
...
- **操作**: `ut-tdd plan draft --kind retrofit --target-structure B`
...
- **操作**: `ut-tdd cutover --to <previous-stage>`
...
- **操作**: `ut-tdd plan draft --kind research --topic "X library 選定"`
...
- **操作**: `ut-tdd plan draft --kind design --layer L2 --sub-doc screen-list`
...
- **操作**: `ut-tdd plan draft --layer L10 --sub-doc visual-design`
...
- **操作**: `ut-tdd review --uncommitted --reviewer doc-reviewer`
- **期待結果**: 品質観点 4 軸 (整合/網羅/一貫/明確) チェック結果が `.ut-tdd/audit/doc-reviews/<timestamp>.json` 記録 / pass で次工程 (gate) 許可 / 終了コード 0

#### AC-FR-45-02 (異常系: 未召喚で gate 試行)
- **前提**: 大規模 doc 改定後、doc-reviewer 未召喚で `ut-tdd gate G3` 実行
- **操作**: G3 ゲート判定
- **期待結果**: fail-close `Error: G3 通過には doc-reviewer 召喚必須 (BR-08 / FR-45)` / next_action `ut-tdd review --uncommitted --reviewer doc-reviewer 実行` / 終了コード 1

#### AC-FR-45-03 (境界系: bypass = PO 専属)
- **前提**: PO が `UT_TDD_DOC_REVIEWER_BYPASS=1` で bypass 試行 (緊急 hotfix 等)
- **操作**: G3 ゲート判定
- **期待結果**: bypass + audit `.ut-tdd/audit/doc-reviewer-bypass/<timestamp>.json` (PO ID + 理由必須) / KPI D-08 集計 / 終了コード 0
...
- **調査素材の分離**: external templates は `incorporate`、`reference`、
  `ut-tdd-specific`、`exclude` へ分離する。template material は required evidence を
