...
related_requirements: docs/governance/helix-harness-requirements_v1.2.md
...
L2 画面   │                            │ L12 運用・価値検証   │ (L11-L14)
L3 要件   ┘                            │ L11 受入＋human visual acceptance ┘ ← Phase 5/6 共有層
L4 基本   ┐                            │ L10 システム検証＋Real UX evidence         ┐
L5 詳細   │ Phase 2 (L4-L6)            │ L9 結合検証       │ Phase 5
L6 機能   ┘                            └ L8 単体・詳細検証       ┘ (L8-L11)
              L7 実装  ← Phase 3 (workflow 自動化) / Cycle P4 (L7 DB 統合)
```

> **用語規約 (2026-06-12)**: HELIX ロードマップの L7 DB 統合サイクルは **Cycle P4 / L7-DB** と呼ぶ。以後、旧DBフェーズ表現は禁止し、Run 層 (L9-L11) と L7 DB の完了判定を混同しない。移植元名は歴史資料・vendor 参照に限定し、現行工程名・完了判定・doctor message の主語にしない。
>
...
> **テスト設計 frontmatter の layer 規約 (IMP-037/059 確定、PO「解消して」2026-06-04)**: ③/④ テスト設計 doc の `layer` は **作成層 (= 左腕設計層 = V-pair の pairing key、`next_pair_freeze` と一致)** を正本とし、実施層は `executed_at_layer` で別保持する (両保持 = 情報損失ゼロ)。例: 単体テスト設計 = `layer:L6` + `executed_at_layer:L7` / 受入テスト設計 = `layer:L3` + `executed_at_layer:L12`。vmodel-lint は `layer` を pairing key として左腕設計と突合する。**ファイル名 (L7-unit/L8-integration/L9-system は実施層、L1-operational/L3-acceptance は作成層) は歴史的経緯で混在するが、機械正本は frontmatter `layer`=作成層**。L7-unit が template。 （層意味論: canonical L8=unit/detail; canonical L9=integration）
...
- **検証**: 観点 A (L0-L3 に関わる mode の workflow が動くか) + 観点 B (L1⇔L14 運用TD / L3⇔L12 運用・価値検証TD / L2⇔L10 mock が同粒度か)。
- **このフェーズの追加任務**: §1 の観点 A/B を全フェーズ共通テンプレートとして**確定・明文化**する (PO が Phase 1 を「検証対象 + 観点定義の場」と指定)。
- **出力**: 課題/アイディア → improvement-backlog。
- **現状接地**: L0=concept_v3.1 既済 / L1=draft (5 sub-doc) / **L2=5 sub-doc 実在** (screen-flow/screen-list/ui-element/wireframe + README、全件 `status:placeholder`。③ペア=mock 自体・L10 独立 doc 不要は L2 設計本文と L10 mock pair trace で扱い、README は索引に留める = IMP-039) / L3=draft (3 sub-doc + roadmap)。

### Phase 2 — L4〜L6 改善サイクル

- **対象**: L4 基本設計 / L5 詳細設計 / L6 機能設計 (左腕下段)。
- **検証**: 観点 B (L4⇔L9 結合TD / L5⇔L8 単体・詳細TD / L6⇔L7 単体TD の同粒度) + 観点 A。
...
- **対象**: L8 単体・詳細検証 / L9 結合検証 / L10 システム検証＋Real UX evidence / L11 受入＋human visual acceptance (右腕下段)。
...
- **対象**: L11 受入＋human visual acceptance / L12 運用・価値検証 / L13 デプロイ後検証 / L14 運用検証+改善 (右腕上段)。
...
- **対象**: harness を実際に当てる「**テスト対象システム** (sample target repo)」を構築し、HELIX を end-to-end で dogfood する。
...
| 2026-06-09 | **Phase 2 pre-close feature hardening** (Codex TL) | automation + unit-test database + common/multi-agent lenses | PO question「自動化上 / unit-test database 化で不足はないか」に対し、Phase 2 close 前の追加棚卸しを実施。`placeholder_deps` dedicated rule、GreenDefinition、単体テスト evidence history projection、DB collector/rebuild/migration、relation graph、skill injector metrics、team-run lifecycle、CI evidence matrix、guardrail/security invariants、meta-audit taxonomy を IMP-107..116 として起票。L5 physical-data §9.4、L6 test-before-review §8、L6 function-spec DB addendum を強化。 | A-122 / IMP-107..116。Phase 2 full review PASS は維持し、PO accept 前の explicit carry として Phase 3/4 PLAN seed へ routing。 |
...
> governance 正本リスト (`CLAUDE.md` の「正本ドキュメント」節 / `docs/governance/README.md` の現行正本) には**載せない** = 本書は governance 正本ではなく設計層の doc であるため。
...
- `docs/governance/helix-harness-concept_v3.1.md` (L0 構想、§2.3 V-model)
- 要件正本: `docs/governance/helix-harness-requirements_v1.2.md` (§1.4 layers / §1.10.G.12 backlog→ledger / §2.2 gate)
