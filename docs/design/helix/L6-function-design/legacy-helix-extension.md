...
| HLX-C03 | `routeDetectorFinding` | `(finding: DetectorFinding, registry: DetectorAxisRegistry) => DetectorRouteDecision` | stub/advisory/fail-close を区別し、stub/advisory を hard gate proof に使わない。さらに registry の detector kind と finding kind が一致しない場合は、finding 側だけ `fail_close` と名乗る hard gate 昇格を拒否する | U-HLX-004 |
