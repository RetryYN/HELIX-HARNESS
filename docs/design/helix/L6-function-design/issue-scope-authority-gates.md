...
| `collectIssueDependencyContracts(sources)` | GitHub Issue snapshotを受け、parserと共有するyaml fenced block抽出器が先頭のexact markerを採用候補とする | valid blockはnode、不正な採用blockはIssue番号付き`issue_dependency_contract_invalid`へ分離する | fence後の改行・空行・same-line空白でdetectorとparserを乖離させず、prose marker言及をgoverned nodeへ昇格しない |
