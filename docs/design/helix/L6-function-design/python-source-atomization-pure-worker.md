...
| U-PYATOM-004 | 外部I/O default deny | Python audit hookがnetwork、filesystem、child processを拒否する | `tests/source-atomization-scrum-worker.test.ts` |

## 非目標

generic broker、descriptor registry、lease/fence、DB staging/commit、terminal receipt、artifact publish、
active pointer、外部service接続は本sliceに含めない。4 oracleのgreenをPython runtime全体の完了へ読み替えない。
