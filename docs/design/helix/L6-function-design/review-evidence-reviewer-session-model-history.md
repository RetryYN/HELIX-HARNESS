...
| typed receipt追従 | tracked history + provider発行receipt | receiptが同一sessionの新modelとreviewed_atを型付きで保持する | 既存open windowをreceipt時刻で閉じ、同時刻から新model windowを開始する。境界直前は旧model、境界一致は新modelになる | receipt以前の切替時刻を推測せず、旧windowと重複させない | U-RVIDENT-020 |
