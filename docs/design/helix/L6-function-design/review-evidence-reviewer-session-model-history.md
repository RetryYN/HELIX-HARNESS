...
| terminal evidence訂正 | recovery branch + base/current PLAN | terminal PLANの`review_evidence`だけが変更され、本文・kind・status・他frontmatterは完全一致する | 異kind PLANをRecoveryの所有PLANから除外し、workflow identity ownerにも数えない | draft化、本文変更、空evidence、他field混載は通常のkind mismatchとして拒否する | U-RVIDENT-021 |
