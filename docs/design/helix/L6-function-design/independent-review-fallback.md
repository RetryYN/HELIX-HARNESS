...
- `helix github pr-review-fallback-admission`: 別Claudeが実測したbenchmark evidence／negative oracle JSONを読み、5 caseと4 mutationのexact set／期待結果を検証してdigest化する。callerのdigest自己申告は受けない。既定はdry-runで、`--apply`時だけruntime stateへ永続化する。
