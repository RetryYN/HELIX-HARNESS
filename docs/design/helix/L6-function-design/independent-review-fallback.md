...
- `helix github pr-review-fallback-admission`: 別Claudeが実測したfixture digest／negative oracle digestと有効期限をprovider固有実行権限を持たないreceiptへ封印する。既定はdry-runで、`--apply`時だけruntime stateへ永続化する。
