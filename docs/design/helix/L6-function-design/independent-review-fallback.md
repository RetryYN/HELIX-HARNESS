...
- S4 admissionの有効期間には上限（24時間）を課す。`issued_at < expires_at`だけでは発行側が任意の遠い`expires_at`または未来の`issued_at`を置けて「期限付き」が名目化するため、window長と`issued_at ≤ now ≤ expires_at`をvalidateし、CLI発行境界ではwall clockへ束縛する。
