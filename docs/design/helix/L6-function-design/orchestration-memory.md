...
| loop entrypoint | `ut-tdd loop run` (`src/cli.ts`) | `.ut-tdd/state/loop/<plan>.json` から state を読み、dry-run と dispatch を分離 |
...
| `src/cli.ts` | `ut-tdd loop run --plan` / `ut-tdd memory write/list/show/supersede` subcommand | 既存 subcommand の引数規約に従い、loop は dry-run 非 dispatch・once 1 tick・通常 tick 周回を分離する |
...
U-ORCH-BRIDGE-02 (`ut-tdd loop run` が store/dry-run/once/iteration 永続を扱う) /
...
- Reverse(R0-R4, forward_routing=L3) で Route-B back-fill L3 要件 8 件
  ...
  `ut-tdd loop run` で受け、bridge/entrypoint が L3 acceptance の `U-ORCH-BRIDGE-01/02` と接続する。
