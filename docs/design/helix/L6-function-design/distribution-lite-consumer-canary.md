...
fresh installでは、audit／fund／update notificationという受入対象外のadvisory network副作用を抑制し、CIのrequired
preflightで復元したnpm cacheだけを使う。Lite artifactはruntime依存をbundle済みなので、registry egressを許可せず、cache missは
受入失敗として即時fail-closeする。package依存解決、`package-lock.json`、integrity、artifact digest、`npm ci`のfrozen検証は
引き続き実行し、timeout値の無制限緩和、skip、soft-passは許可しない。setup内部のlockfile生成は任意consumer依存を解決するため、
同じadvisory抑制とoffline優先を使うが、cache miss時の依存解決まで隠さない。グローバルnpm設定やcredentialは変更せず、
consumer child processへだけ明示的にポリシーを渡す。
