...
- HC-P6 clean distribution acceptance は source / clean artifact の `package.json.scripts.ut-tdd="bun run src/cli.ts"` と
  ...
  package/bin resolution preflight (`bun run ut-tdd --version`) と setup dry-run / status / completion decision-packet / consumer doctor /
...
**HC-P6 2026-07-02 追補**: `ut-tdd setup project` が生成する `.vscode/tasks.json` は status / completion decision-packet / consumer doctor /
