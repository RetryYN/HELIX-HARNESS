...
### Claude native memory isolation（PLAN-L7-700）

`analyzeProjectHooks`はproject settingsの`autoMemoryEnabled === false`と、`.claude/agents/*.md`に`memory: project|user|local`が存在しないことを同時に検査する。欠落・再出現はいずれもhard failureとし、consumer templateにも同じ設定を投影する。native hooksはHELIX policy adapterとして保持し、provider全体のeffective configuration attestationは#1172へ委譲する。
