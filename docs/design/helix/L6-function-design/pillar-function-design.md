...
- HC-P9 / handover resume surface は `helix status --json` と同じ live outstanding 由来の
  `workflowNextAction` / `workflowNextActions[]` を `helix handover status --json` と text mode にも出す。
