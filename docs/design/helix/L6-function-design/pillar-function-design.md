...
| DB convergence / layer regression / semantic frontier | HC-P9 `checkProjectionConvergence` / `queryLayerRegressionImpact` / `analyzeOutstandingWork` -> `semanticFeatureFrontierRecords` | L6 confirmed、一部 L7 実装済み | DB 未収束、contract 未分類、影響 layer gate/test 未実行は green にしない。PO 判断待ち・version-up parked・rename/cutover 承認待ちは `semantic_feature_frontier_record` として `completionClaimAllowed=false` を返す |
