...
**HC-P6 2026-07-05 追補**: `auditIdentifierRenameBlastRadius` は uppercase 旧称 `UT-TDD` / `UT-TDD-agent-harness` / `UT-TDD:managed` を detection-only の residual として扱い、`residuals[]` / `residualsByDisposition[]` に `adapter_marker` / `fixture_only` / `approval_gated` / `reference_source` / `safe_prose_candidate` の disposition を出す。これらは residual visibility のための分類であり、PLAN-M-02 cutover/action-binding approval 前に `TOKENS` / `RENAME_MAP` / apply candidate へ昇格しない。path hit では `.ut-tdd` ルートディレクトリ自体も `.ut-tdd/**` 配下と同じ `runtime_state` として扱い、state dir migration の blast radius を `other` に逃がさない。
