...
| asset/progress visualization amendment | 2026-07-06 PO 判断で current scope から archived | closed frontier | `VisualizationSnapshot` first response は既存だが、Tree View/Webview の L6 function と L7 implementation は current scope に採用しない |
...
- 現行 live frontier は `current_semantic_frontier_count=0` である。archived / deferred 済み PLAN は
  `outstanding.semanticFeatureFrontierRecords[]` に残さず、将来再開時は新規 PLAN と新規 evidence で再起票する。
