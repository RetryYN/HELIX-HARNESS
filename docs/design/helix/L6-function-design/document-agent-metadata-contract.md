...
write port は全 target のdigestとrootからtargetまでの全ancestorが非symlinkであることをbatch preflightで検証し、
全件PASS後にだけ順に原子的 publish する。
write dispatch前に当該changeを`publishState: ambiguous`としてrollback集合へ登録し、portがpublish後にthrowしても当該targetを
含めreverse orderでbefore contentへ戻す。receiptにはrollback成否を全件記録する。rollbackが全件durableならtop-level
`ambiguous: false`、rollback自体が失敗してbefore stateを確立できないreceiptは`ok: false`、`partial: true`、
`ambiguous: true`とし、CLIはnon-zeroを返す。成功 receipt は before/after digest、durability、rollback不要を
...
| IT-AGMETA-005 | publish rollback | real write portをrename/fsync後にfault injectionし、throwした当該targetを含む逆順rollbackとpartial/ambiguous receipt |
