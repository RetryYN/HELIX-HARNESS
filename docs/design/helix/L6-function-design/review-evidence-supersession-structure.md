...
| 関数 | 事前条件 | 事後条件 | 不変条件 | oracle |
| --- | --- | --- | --- | --- |
| `hasReviewEvidence` | leading frontmatterを含むPLAN text | typed entryが一件以上の場合だけtrue | 本文・例・引用を証拠として数えない | U-REVIEW-001 |
| `analyzePlanSupersession` | 構造解析済みPLAN集合 | exact双方向edgeだけgreen | target不在・片方向・malformed YAMLはfail-close | U-SUPER-001〜004 |
| `isNonSemanticL3MetadataMigrationLine` | Git patch line | allowlist metadataだけtrue | 意味変更を許可しない | U-L3APP-014 |
| `isSupersessionMetadataOnly` | 同一PLANのbase/current | `superseded_by`だけの差分ならtrue | 他field変更を通さない | U-BRANCH-SUPER-001 |

## 既存設計との境界

`review-evidence.md`はreview evidence全体の設計として維持する。本設計はIssue #1446で追加した
構造解析と訂正edgeの原子責務だけを所有し、既存の広域V-pairを置換しない。
