...
| `resolveCanonicalDependencyGraph` | `(node: LockedManifestV1, python: LockedManifestV1, resolver: ResolverIdentityV1) => ResultV1<DependencyGraphV1, OsSupplyChainFailureV1>` | Nodeはnpm＋package.json/package-lock.json固定、Pythonはpair-frozen toolchainだけを受理し、未freeze・direct/transitive欠落・OS lock forkを拒否 | `U-OSSC-007` |
...
interface LockedManifestV1 { ecosystem: "node" | "python"; package_manager: string; manifest_name: string; lock_name: string; toolchain_authority: "adr_fixed" | "pair_frozen"; toolchain_frozen: true; authority_source_digest: string; manifest_digest: string; lock_digest: string; resolver_version: string; os_variant: null; components: DependencyComponentV1[] }
...
`LockedManifestV1`のecosystem別制約は、Node=`package_manager:npm`、`manifest_name:package.json`、
`lock_name:package-lock.json`、`toolchain_authority:adr_fixed`、Python=`toolchain_authority:pair_frozen`である。
Pythonのmanager／manifest／lock／interpreter provenanceが未選定・複数・digest欠落なら`LockedManifestV1`としてdecodeせず、
`HIL_SUPPLY_CHAIN_LOCK_DRIFT`へfail-closeする。
