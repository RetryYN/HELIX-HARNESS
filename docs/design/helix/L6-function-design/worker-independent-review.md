...
evaluateWorkerIndependentReview({ input, proposalOutput, workerOrigin, reviewerOrigin }): WorkerReviewEvaluationResult
...
3. receipt root 4-field exact set、型、digestを検証する。
4. proposal digestを照合する。
5. 両outputのbroker execution originをcurrent admissionで再解決する。
6. originからactorを導出しidentity、session、contextを別分岐で検証する。
7. canonical receipt digestを作り、`WeakSet`へsealed capabilityを登録する。
