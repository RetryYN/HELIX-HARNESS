...
| `todo` | L12 tailoring gate の不足。Reverse または Additive で設計へ戻す |
...
| HVM-TAILOR-ENTERPRISE-NA | na | enterprise only governance、規制専用、課金専用 | 省略 | 必要になった project profile だけで Additive 採用 |
...
5. mobile/desktop/enterprise 専用設計は HELIX core では既定 `na` とし、必要な consumer project だけが Additive profile で採用する。
