...
> **Authority correction（2026-07-16）**: `HVM-REJECT-01`の「Python reference-only／TS/Bunへ再実装」は
> ADR-009とInfinity Loop要件によりsupersedeされた。Hybrid ZIPのPython engine/toolはtool単位に
> `runtime reuse disposition`と`semantic adoption disposition`を分離し、検証可能なclosed capabilityをPython workerとして
> 原則保全する。Nodeはcontrol、gate、schema再検証、authoritative DB commitを担う。本文の旧TS/Bun判断は決定史であり、
> current runtime/adoption authorityではない。Excel生成物を正本にしない判断は維持する。
...
| generator | `tools/build.py` が build / validate / coverage / deps / check / diagrams を持つ | historical: Python reference-only。current: tool別にPython保全/adapter/redesignを判定し、Node gateへ接続する |
...
| 差異 | ZIP | HELIX | 採用判断 |
|------|-----|-------|----------|
| core 実行基盤 | Python tool と Excel/Sheets 出力中心 | TS/Node control plane、SQLite harness.db、Python worker、runtime adapters | Python engineを第一級workerとして保全し、Excel/Sheets生成物には正本write authorityを与えない |
...
| ID | 非採用対象 | 理由 | 代替 |
|----|------------|------|------|
| HVM-REJECT-01 | **superseded historical decision**: ZIPのPython generatorをreference-onlyとしTS/Bunへ再実装する | ADR-009、PO指示、tool監査によりPython engine保全へ再設計した | Python workerをtool別採否しNode control/gate/DB commitへ接続する。Excel buildを正本にしない境界はHVM-REJECT-03で維持 |
...
### §9.1 2026-07-14 着手ギャップ（Phase A実装、Phase B対応候補）

| ID | ZIP 側の元機能 | 確認結果 | 根拠 |
|----|-----------------|----------|------|
| HVM-GAP-01 | `tools/agent_meta.py` (agent メタデータ `defines`/`read_first`/`done_when` の導出・付与・突合) | Phase A read-only check を `src/schema/document-agent-metadata.ts` / `src/lint/document-agent-metadata.ts` / `helix design agent-metadata check` / doctor gateへ移植済み。明示apply、write port、rollbackはPhase B未実装 | 2026-07-14 Vペア・単体/統合oracle・CLI/doctor配線済み |
| HVM-GAP-02 | `tools/diff_report.py` (2時点 docs/ 比較による日本語差分レポート・リリースノート自動生成) | Phase A read-only semantic diff を `src/runtime/document-semantic-diff.ts` / `document-change-report.ts` / root・git基準CLIへ移植済み。明示`--out` write portとrelease操作は未実装 | 2026-07-14 Vペア・単体/CLI oracle・git安全adapter済み |

この 2 件は `§6 採用しないもの` の明示的非採用判断 (HVM-REJECT-01/02/03) の対象外であり、
「意図的な不採用」ではない。2026-07-14にread-only Phase Aを実装したが、source更新やrelease操作を含む
Phase Bは、write-port・rollback・L9証跡を揃えた別PLANで判断する。本書の記録はPhase Aを全体完了へ読み替えない。
