...
4. Workflow Requirements Skill v1.1 アダプター契約
5. Detection DBのevent / evidence / projection
6. 生成docs / views / reports
...
このRB-07初稿が提案した「receipt発行前はBunをactive authority／rollback pointとして維持する」条件は、
2026-07-19のruntime authority再基準化により**不採用・superseded**である。現行要件はPython semantic core＋
TypeScript/Node transactional boundaryを直ちにcurrent authorityとし、Bun active dependencyを0に固定する。
`CutoverActivationReceipt`は旧移行の監査証跡を検証する互換receiptであり、Bunのactivation、fallback、rollbackを
許可する前提条件として使わない。
...
`requirements/requirements-catalog.yaml` の `HBR-ARCH-006` は、レガシー`python3 tools/<tool>.py ...`を
historical参照allowlistへ限定し、通常運用のcapability経路から除外する。旧Bun artifactは監査用の固定digestとしてのみ
保持し、known-good release、rollback target、実行可能CLIとして登録しない。同requirementの `verification` 配列へ既存
`AC-ARCH-04` を残したまま `AC-ARCH-07` を追加する。
...
5. Workflow Requirements Skill v1.1 アダプター契約
