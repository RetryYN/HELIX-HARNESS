...
| **HIL-BR-14** | ZIP、旧UT全branch、現行HELIXのsourceをatomic behavior単位へ完全分解し、各項目を採否判断から要件・設計・テスト・Gateまで追跡する。file集合やaggregate親の列挙、source宣言、読了だけを採用済みとみなさない。 |
...
| **HIL-FR-16** | Asset Inventoryは現行HELIX、ZIP、旧UT main＋全remote branchを機能単位で比較し、adopt/harden/redesign/rejectを記録する。 | 完全性台帳、未判断0判定 |
...
| **HIL-FR-21** | Source Snapshot ManifestはZIP entry、旧UTの全remote ref/commit SHA/unique diff、現行HELIX symbol/doc/testを固定し、fetch時刻、source/tree digest、抽出器versionを記録する。source集合が変われば既存coverage receiptをstale化する。 | immutable manifest、source digest、stale判定 |
