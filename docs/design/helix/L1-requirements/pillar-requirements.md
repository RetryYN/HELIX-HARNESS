...
| HBR-P7 | Claude/Codex どちらで開始しても同じ `.helix/memory` と `.helix/handover/provider` から bounded recall する | Claude 内蔵 memory / `.claude/agent-memory` を正本にしない。Codex SessionStart でも同じ memory surface（直近 12 件 / 240 字）を表示する |
