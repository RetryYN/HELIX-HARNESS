...
`PLAN-DISCOVERY-10` 候補 View 5 種を、VS Code first surface = Tree View（一覧/階層 navigation）と
Webview = graph/detail panel に割り付ける。各 view の正本 source は `VisualizationSnapshot` の対応 field
に限定する。
...
|----|----------|----------------------------|----------|
| HR-FR-VIS-01 | HBR-P9 / HBR-P4 | 候補 View 5 種を Tree View（first surface）と Webview graph/detail panel へ割り付け、各 view は `VisualizationSnapshot` の対応 field のみを正本 source にする。LLM 生成図・free-form 要約を view の正本にしない | HAC-VIS-01a / HAC-VIS-01b |
...
| AC-ID | 前提/操作 | 実行 | 期待 |
|-------|-----------|------|------|
| HAC-VIS-01a | 5 view を surface に割り付ける | view→source マッピングを検査 | 各 view が Tree View または Webview panel に割り付き、正本 source が `VisualizationSnapshot` の実在 field を指す |
...
| agent dispatch trace viz | 旧 PLAN-219 | dispatch DAG + critical path 強調 | adopt-concept | 将来 view 候補。**初回 slice の 5 view には含めない**（対象外と明記）。Skill/agent telemetry の発展形として後続で判断 |
...
- L5: visualization read-model 契約、graph IR 契約、drill-down 契約。pair edge フィルタ済み count field と
  graph/evidence/skill 系の空状態 warning の `VisualizationSnapshot.warnings` 拡張はここで契約化する。
...
- 初回 slice に action surface を入れない。command 実行・外部 API・branch/ruleset mutation は
  action-binding approval のまま残す。agent dispatch trace view（旧 PLAN-219 由来）は初回 5 view に含めない。
