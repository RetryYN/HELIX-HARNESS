...
したがって、ZIP の狙いは HELIX の P3/P9/P4 を強化する設計入力として採用する。Python generator は
ADR-010 に従う恒久意味コアとして保持し、要件抽出・typed spec・trace・impact・文書生成を担う。Node は
schema/digestを検証した結果だけをharness.db・Git/GitHubへcommitする単一実行境界とし、Excel出力は補助成果物に限定する。
...
| 差異             | ZIP                                                                  | 現行 HELIX                                                    | 判断                                                                                                                |
| ---------------- | -------------------------------------------------------------------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| L階層            | L1-L12                                                               | legacy L0-L14                                                 | L1〜L12をcanonical targetにする。既存legacy成果物だけcompatibility layerで再投影し、破壊的renameはしない           |
...
| 実装言語         | Python tools + Excel/Markdown generation                             | Python意味コア + TypeScript/Node実行境界 + SQLite DB          | Pythonの意味能力を正本として保持し、Nodeは副作用を直列化する。Bun依存は撤去する                                      |
...
legacy L0-L14 compatibilityとL1〜L12 canonicalの二重表示をgreenにしてから、schema enumやPLAN ID policyを変える。
...
- L1〜L12 canonicalとlegacy L0-L14 compatibilityの対応。
...
| ID             | 要件                                                                                                                                                                                  | 主な AC                     |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------- |
| HR-FR-VMFIT-01 | HELIXはZIPのdesign extraction / coverage / impact / tailoringをPython意味コアとして保持し、Node単一commit境界へversioned schema・digest・provenance付きで接続する | HAC-VMFIT-01a/b |
| HR-FR-VMFIT-02 | L1〜L12 canonical layer mapとlegacy L0-L14 compatibility projectionを持ち、独立した重い機能設計層をL5詳細設計+typed declaration+L7 TDD closureへ吸収するmigration方針を示す | HAC-VMFIT-02a/b |
...
3. L1〜L12 canonicalとlegacy L0-L14 compatibilityのread-modelを追加する。
4. roadmap registry と harness.db 現在地を結び、current-location ledger として拡張する。
5. drive-model selector を追加し、原則 Forward spine を合流先にしつつ、Discovery / Scrum / Reverse / Recovery / Incident / Refactor / Retrofit / `add-feature` / version-up / Research の 10 entry mode と OperationVerification を理由付きで返す。Reverse では設計/テスト設計へ戻す範囲、文書依存、実装依存を必ず出す。
