...
5. workflow-model selector を追加し、原則Forward spineを合流先にしつつ、Discovery / Scrum / Reverse / Recovery / Incident / Refactor / Retrofit / version-up / ResearchとOperationVerificationを理由付きで返す。Featureはentry modeではなくIssue分類としてForwardまたはProduction Scrumへrouteする。Reverseでは設計/テスト設計へ戻す範囲、文書依存、実装依存を必ず出す。
