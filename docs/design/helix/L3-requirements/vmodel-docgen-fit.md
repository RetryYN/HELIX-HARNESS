...
adoption_authority: docs/governance/hybrid-python-engine-adoption-ledger.md
...
> **Authority correction（2026-07-16）**: Python reference-only／TS/Bun再実装の判断はADR-009と
> Infinity Loop要件によりsupersedeされた。Hybrid Python engine/toolはclosed capability単位に原則保全し、
> Node control planeがcontract、gate、再検証、authoritative DB commitを担う。旧記述は決定史としてのみ保持する。
...
したがって、ZIP の狙いは HELIX の P3/P9/P4 を強化する設計入力・実行engineとして採用する。Python engine/toolは
tool別に保全・adapter・hardening・redesign・rejectを決め、Excel出力へ正本authorityを与えない。Nodeへ一括再実装せず、
Node control/gate/DB commitとも相互にlogic forkしない。
...
| 実装言語         | Python tools + Excel/Markdown generation                             | TS/Node control + Python worker + SQLite DB                    | Python engineをtool別に保全し、Nodeがcontrol/gate/authoritative writeを担う                                          |
...
| ID             | 要件                                                                                                                                                                                  | 主な AC                     |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------- |
| HR-FR-VMFIT-01 | HELIXはZIPを一括置換せず、design extraction / coverage / impact / tailoringと検証済みPython engineをtool別採用し、Node control/gate/DB commitへ接続する                               | HAC-VMFIT-01a/b             |
...
1. ZIPのPython engine/toolを根拠なくNodeへbulk portせず、Node control/gate/DB commitをPythonへ移植しない。Excel生成物は正本にしない。
