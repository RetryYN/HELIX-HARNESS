...
再生できる場合だけadmitする。G3ではnested target契約だけをfreezeし、既存flat PLANのsystem-wide Forward実移行は
L5契約が閉じた後に専用migration PLANで開始する。requirement、design、implementation、test/verification、
DB、Issue、PR、receiptの全edgeを依存順に修正し、全移行完了までlegacy loaderとnested loaderのdual-greenを要求する。
...
| AC | 合格条件 |
|---|---|
| GH-AC-031 | layerだけのPLAN名、workflow-model folder不一致、frontmatter route不一致をwrite前に拒否し、正規generatorだけがmodel付きID・表示名・pathを生成する。G3ではtarget契約をfreezeし、L5契約後の専用migration PLANだけがdual-green下でnested pathをDBへexactly once投影する |
