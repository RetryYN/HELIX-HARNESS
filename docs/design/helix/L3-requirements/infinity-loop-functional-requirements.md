...
本書はL2の153要求（物理pathはlegacy L1）を、system observable behaviorと受入境界へ降下する。L4 component、runtime内部責務、
algorithm、table、CLI引数は本書で確定しない。各L2要求はprimary ownerとなるsystem FRをexactly 1件持ち、
secondary test coverageはL10/L9で追加できる。現時点はPO承認前のdraftであり、G3 freezeを主張しない。

## §1 System FR

| L3 FR | primary L2要求 | system動作 | 事前条件 → 事後条件 | failure / evidence | AC |
|---|---|---|---|---|---|
...
| HR-FR-HIL-03 | HIL-BR-02, HIL-BR-17, HIL-FR-09, HIL-FR-30 | 全base/stacked PR deliveryをcurrent head監査jobへ冪等変換し、findingをcurrent contract影響と責務境界で`current_pr_fix`または`successor_issue`へ分類する。前者はwriterへ一括返却し、後者だけをIssue/Reverse/memory/Codex queueへ原子的昇格する | delivery/head/policyあり → job最大1件、typed disposition一件、`current_pr_fix`はpromotion 0、`successor_issue`は全target同一cause commit | stale head、duplicate job、誤分類、current findingのIssue逃がし、successorのPR再流入、finding drop/partial promotion / delivery、job、disposition、writer return、promotion lineage | HAC-HIL-03a, HAC-HIL-03b, HAC-HIL-03c |
...
| HR-FR-HIL-18 | HIL-BR-25, HIL-FR-46, HIL-FR-47, HIL-FR-48, HIL-FR-49, HIL-FR-50, HIL-NFR-29 | canonical L1–L12 ledgerと層外L0 charter anchorをversion管理しtemplate obligationを抽出、上下隣接edge、正規5 V-pair、L6↔L7 TDD closureを同一revision/snapshot/oracleで閉じ、refactorも全pairを保存する | active registry/template/snapshot/pair map → 全atom disposition、双方向edge、実行oracle | type/抽出/TBD/aggregate、片edge、revision/oracle不一致、pair破壊 / ledger、addition、pair、execution、refactor receipt | HAC-HIL-18a, HAC-HIL-18b, HAC-HIL-18c |
| HR-FR-HIL-19 | HIL-BR-26, HIL-FR-51, HIL-FR-52, HIL-FR-53, HIL-NFR-30, HIL-NFR-31, HIL-NFR-32 | AI AuthoringをProposalとして許可し、意味差分・authority・pair・impactを検査した変更だけを原子的にCanonical化し、asset identityとrevision履歴を維持する | proposal、base revision、policyあり → 自動Admission又は明示decision、部分current 0 | 過剰確認、無権限変更、partial write、stale base、identity/oracle消失 / admission、transaction、revision、rollback receipt | HAC-HIL-19a, HAC-HIL-19b, HAC-HIL-19c |
| HR-FR-HIL-20 | HIL-BR-27, HIL-BR-28, HIL-FR-54, HIL-FR-55, HIL-FR-56, HIL-NFR-33 | 要求義務から必要十分な設計契約と見本を導出し、Forward各layerとScrum S0–S4へ同一snapshot・返却edge付きでbindする | requirement/obligation/risk/templateあり → 未被覆・重複0のportfolioとworkflow binding | 固定冊数、positive-only、別正本Scrum、S4前昇格 / portfolio、example、phase、back-propagation receipt | HAC-HIL-20a, HAC-HIL-20b, HAC-HIL-20c |
| HR-FR-HIL-21 | HIL-BR-29, HIL-BR-30, HIL-FR-57, HIL-FR-58, HIL-FR-59, HIL-FR-60, HIL-NFR-34 | taskの工程/domain/riskから判断packと最小専門agent contractを生成し、shadow reviewとruntime guard後だけmusterする | task/skill/model catalog/allowlistあり → versioned pack、最小team、独立verify | self-promotion、未許可tool、過剰agent、自己検証、stale contract / pack、shadow、generation、muster/lifecycle receipt | HAC-HIL-21a, HAC-HIL-21b, HAC-HIL-21c |
| HR-FR-HIL-22 | HIL-BR-31, HIL-FR-61, HIL-FR-62, HIL-FR-63, HIL-NFR-35, HIL-NFR-36 | 候補worker/model/effortを再現可能benchと実task scorecardで比較し、品質・安全・retry込みcostから用途別にadmit又はretireする | fixed fixture/rubric/task/riskあり → blind score、実効cost、選択receipt | smoke-only採用、重大failure相殺、単価最適化、根拠なしeffort固定 / bench、scorecard、routing、exception receipt | HAC-HIL-22a, HAC-HIL-22b, HAC-HIL-22c |
| HR-FR-HIL-23 | HIL-BR-32, HIL-FR-64, HIL-FR-65, HIL-FR-66, HIL-FR-67, HIL-FR-68, HIL-FR-69, HIL-NFR-37, HIL-NFR-38, HIL-NFR-39, HIL-NFR-40 | 第三者worker委譲をsandbox契約、環境浄化、最小払い出し、proposal再検証、audit付きで実行し、安全境界の充足をローカル強制機構だけで判定する | sandbox template・分類ゲート・opt-out receiptあり → 隔離実行、検証済みproposal、完全なaudit chain | allowlist外egress、scope外diff、bypass常態化、機密委譲、vendor設定依拠 / sandbox digest、egress計測、FS diff、audit row | HAC-HIL-23a, HAC-HIL-23b, HAC-HIL-23c |
...
| HR-FR-HIL-18 | HAC-HIL-18a: canonical L1–L12抽出、層外L0 anchor、全pair current | HAC-HIL-18b: 任一atom/edge/oracle欠落でexit拒否 | HAC-HIL-18c: pair保存だけRefactor、他はRedesign/Retrofit |
| HR-FR-HIL-19 | HAC-HIL-19a: 可逆変更を人手なしで原子的Canonical化 | HAC-HIL-19b: authority/impact/transaction欠落を拒否 | HAC-HIL-19c: stale/rename/split/mergeでidentityとrollback保持 |
| HR-FR-HIL-20 | HAC-HIL-20a: 最小portfolioをForward/Scrumへbind | HAC-HIL-20b: 義務・negative・S4 edge欠落を拒否 | HAC-HIL-20c: 発見deltaを要求/templateへ逆伝播 |
| HR-FR-HIL-21 | HAC-HIL-21a: 適用packと必要最小teamを決定生成 | HAC-HIL-21b: self-promotion/未許可/自己検証を拒否 | HAC-HIL-21c: catalog変更でstale化し再生成/retire |
| HR-FR-HIL-22 | HAC-HIL-22a: blind benchと実taskで用途別選択 | HAC-HIL-22b: 重大failure/範囲逸脱を平均で相殺しない | HAC-HIL-22c: 品質悪化時にmodel/effortを比較再route |
| HR-FR-HIL-23 | HAC-HIL-23a: sandbox契約内で委譲が完走しproposalが再検証済み | HAC-HIL-23b: allowlist外egress/scope外diff/機密委譲/bypass常態化を拒否 | HAC-HIL-23c: quota枯渇・egress乖離でfail-close退避とquarantine |
...
- L10 pair: HAT-HIL-01..24。
- status: draft。PO承認とL10 pair reviewなしにG3を通過しない。
