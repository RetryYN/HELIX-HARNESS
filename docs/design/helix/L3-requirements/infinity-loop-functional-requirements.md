...
| HR-FR-HIL-12 | HIL-FR-27, HIL-TR-02, HIL-TR-07, HIL-TR-08, HIL-TR-09, HIL-TR-10, HIL-NFR-14 | Python意味コアがversioned JSON Lines IPCで意味判断resultを生成し、Node実行境界がschema/digest/provenance/leaseを再検証してtransaction commitする。Nodeは意味判断を再実装しない | compatible protocol、deadline/lease、layered authority map → terminal receipt一件、Node writeのみ | protocol/JSON/size/timeout/crash/backpressure/late/direct write / envelope、process、schema、transaction、projection report | HAC-HIL-12a, HAC-HIL-12b, HAC-HIL-12c |
