...
| HAC-P1-02a | 今版対象外の requirement がある | plan lint / `version-up-readiness` / activation packet surface を実行 | `version_target` と理由、activation 条件、要求・機能一覧との trace、plan-only activation packet が無ければ pending として fail。packet が apply command / activation permission を持つ場合も fail |
