...
| HAC-P6-03a | fresh repo | `ut-tdd setup project --dry-run` | hooks/adapters、現行 `.ut-tdd` state/memory/handover/evidence/teams baseline、将来 `.helix` target、GitHub rules/checks plan、consumer doctor baseline、`identifierTransition.status=blocked_pending_cutover_approval`、`mustNotApply=true`、`commandAvailability.futureCommandAvailable=false`、`importReport.mode=fresh`、`requiresReview=false`、`postSetupWorkflow.importReportRoute=ready` を返す。consumer readiness が未充足なら `postSetupWorkflow.nextRoute=fix_consumer_readiness` と未充足 gate/remediation を返し、ready なら `ut-tdd status --json` / `ut-tdd completion decision-packet --json` / `ut-tdd doctor --profile consumer` / `ut-tdd rename plan --json` / `ut-tdd handover status --json` / `ut-tdd team run --definition .ut-tdd/teams/default-hybrid.yaml --mode hybrid --json` へ進める次 action を返す。`consumerReadiness.artifactReadiness` は `AGENTS.md` の consumer doctor / default-hybrid dry-run 案内、`.vscode/tasks.json` の手動 task と automatic task 無効化、`.ut-tdd` baseline、default-hybrid team の worker/reviewer 分離を検査し、artifact 欠落や意味ずれがあれば環境 CLI が解決済みでも ready にしない。`--package-root packages/app` のような monorepo 指定時は `consumerReadiness.workspace.monorepo=true`、`packageRoot=<repo>/packages/app`、`cliResolution.checkedFrom=<packageRoot>` を返し、adapter path 自体は repo root scoped のままにする。PATH 上の bare `ut-tdd --version` が解決した場合だけ `ut-tdd-cli` check は green になり、packageRoot `package.json` の `scripts.ut-tdd` があっても package script のみなら `cliResolution.strategy=package-script`、`packageScriptAvailable=true`、`bareCommandResolved=false`、`ok=false`、`postSetupWorkflow.nextRoute=fix_consumer_readiness` を返す。dry-run は無変更で終了 |
