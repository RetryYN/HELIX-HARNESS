...
| GitHub 自動化 / setup / release / rename | HBR-P6 | confirmed 43 件に含む | setup / release dry-run / gated push は設計済み。`ut-tdd setup project` は現行 `.ut-tdd` baseline と将来 `.helix` target を同時に示す bootstrap であり、`.ut-tdd -> .helix` rename は PLAN-M-02 の cutover/action-binding approval が無い限り audit/plan までで、apply 完了にしない |
