...
| 連続自律走行 / Scrum 分割 / version-up | HBR-P1 / HR-FR-P1-01..04 | confirmed 43 件に含む | version-up mode は正本化済み。parked work は activation decision / parked review / action-binding approval を plan-only activation packet に出す。ただし `PLAN-L7-146` は `version_target: future` の parked で、今版完了には数えない |
