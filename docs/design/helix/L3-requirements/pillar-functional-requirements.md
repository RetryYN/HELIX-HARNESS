...
PO 指示で `decision_outcome=confirmed` / `status=confirmed` へ戻した。現行 L3 51 件の意味単位には
...
したがって、本書の「量閉じ」は従来46件に、2026-07-19のworker runtime／配布要求5件を加えた
51件に限定される。L1 §2.8 は confirmed decision として残し、下流の visualization
実装・受入は別 PLAN の frontier で追跡する。

### §0.2 意味ベース機能一覧と要求修正境界

本書の機能一覧は、ID 数だけでなく「要求が何を意味しているか」で管理する。したがって、confirmed
51 件に含まれる機能、S4 confirmed 後に下流実装が未完の要求修正、不可逆 cutover defer、将来版候補を
...
| 外部worker runtime frontier | HR-FR-P2-05..08 / HAC-P2-05a..08b | confirmed 51 件に含む | L3/L12までconfirmed。L4以降は未降下で、`PLAN-DISCOVERY-12`が5要件のうちP2-05..08をowner付きでS0-S4採否し、S4後にL4 design PLANへ分割する。既存L6 function／L7 oracleへ降下済みと扱わない |
...
| GitHub 自動化 / setup / release / rename | HBR-P6 | confirmed 51 件に含む | HR-FR-P6-01..06を含む。P6-06の配布package契約はL3/L12設計済み・L4以降未降下。setup / release dry-run / gated push は設計済み。`helix setup project` は現行 `.helix` baseline と identifier cutover packet を同時に示す bootstrap である。`PLAN-M-02` は archive せず、旧 state path / CLI rename cutover の action-binding approval / dry-run / rollback evidence が揃うまで approval-gated cutover として扱う |
...
状態注記: P2-01..04とP6-01..05は既存の下流設計・実装証拠を持つ。一方、今回追加したP2-05..08とP6-06はL3要求／ACとL12 HATまでの`confirmed`であり、L4以降の設計・実装・runtime evidenceは未了である。51件への算入を実装完了率へ読み替えない。

この表は「機能一覧が合っているか」の現在の回答である。confirmed 51 件のうち旧46件は設計降下済み、新規worker runtime／配布5件はL3/L12 confirmedだがL4以降未降下である。
