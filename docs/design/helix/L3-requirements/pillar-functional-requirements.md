...
| HAC-P6-03a | fresh repo | `ut-tdd setup project --dry-run` | hooks/adapters、現行 `.ut-tdd` state/memory/handover/evidence baseline、将来 `.helix` target、GitHub rules/checks plan、doctor baseline、`identifierTransition.status=blocked_pending_cutover_approval`、`mustNotApply=true` を返し、無変更で終了 |
