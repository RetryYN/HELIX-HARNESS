...
| GitHub 自動化 / setup / release / rename | HBR-P6 | confirmed 43 件に含む | setup / release dry-run / gated push は設計済み。`helix setup project` は現行 `.helix` baseline と将来 `.helix` target を同時に示す bootstrap であり、`.helix/state/project-setup.json` と consumer doctor は `completionClaimAllowed=false` と completion packet 導線を保持して setup ready を whole-program completion ready にしない。`.helix -> .helix` rename は PLAN-M-02 の cutover/action-binding approval が無い限り audit/plan までで、apply 完了にしない |
...
- `PLAN-L7-146` serverless readonly share: `classification=parked_future_version`。activation decision が出るまで
  ...
  も必要な場合は `relatedDecisionPackets[]` で `helix action-binding approval-packet --json` を supporting
  route として出す。
...
| HR-FR-P7-01 | HBR-P7 / HNFR-AC | Claude/Codex の SessionStart は同じ `.helix/memory` と `.helix/handover/provider` から bounded recall し、per-agent silo を正本にしない | HAC-P7-01a / HAC-P7-01b |
