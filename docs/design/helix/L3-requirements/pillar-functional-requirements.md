...
| GitHub 自動化 / setup / release / rename | HBR-P6 | confirmed 43 件に含む | setup / release dry-run / gated push は設計済み。`ut-tdd setup project` は現行 `.ut-tdd` baseline と将来 `.helix` target を同時に示す bootstrap であり、`.ut-tdd/state/project-setup.json` と consumer doctor は `completionClaimAllowed=false` と completion packet 導線を保持して setup ready を whole-program completion ready にしない。`.ut-tdd -> .helix` rename は PLAN-M-02 の cutover/action-binding approval が無い限り audit/plan までで、apply 完了にしない |
...
| design-bottomup mode | PLAN-DISCOVERY-07 / PO design-bottomup request | S3 verified / S4 PO decision pending | backend から FE 要件を洗い出す elicitation engine と Discovery 合成は S3 verified。S4 confirmed までは mode 正本化、L1/L3-L6 back-merge、中央 UI dogfood を current completion に混ぜない |
...
| L1-L2 elicitation cycle | PO 対話 / PLAN-DISCOVERY-11 | S3 verified / S4 PO decision pending | 画面 mock で要求を洗い出す greenfield 前段サイクルは S3 verified。S4 confirmed までは A-40 G1-trace 再検証フック、L1/L2 consistency lint、gap-check read-only 結線を current completion に混ぜない |
...
- design-bottomup mode: `classification=frontier_pending_decision`。S4 confirmed までは design-bottomup
  mode 正本化 / L1/L3-L6 back-merge / 中央 UI dogfood を current completion に数えない。
- asset/progress visualization: `classification=frontier_pending_decision`。S4 confirmed までは L3 visualization
  requirements / acceptance を本書の 43 件に混ぜない。
- L1-L2 elicitation cycle: `classification=frontier_pending_decision`。S4 confirmed までは A-40 G1-trace
  再検証フック、L1/L2 consistency lint、gap-check read-only 結線を current completion に数えない。
...
| HR-FR-P6-03 | HBR-P6 / HBR-P9 | 配布 tag/release pin から現行 `ut-tdd setup project` (rename 後 `helix setup project`) で fresh/brownfield repo に hooks、Claude/Codex adapters、現行 `.ut-tdd` state/memory/handover/evidence/teams baseline、将来 `.helix` target、GitHub rules/checks plan、consumer repo 用 doctor baseline を非破壊に bootstrap する。consumer repo の初回 health check は dogfood repo 用 full `ut-tdd doctor` ではなく、setup 投影済み adapter / VSCode task / `.ut-tdd` baseline / `.ut-tdd/teams/default-hybrid.yaml` だけを検査する `ut-tdd doctor --profile consumer` を使い、`docs/plans` / `docs/design/harness` / `docs/test-design` / runtime state を要求しない。`AGENTS.md` が案内する team run は配布済み YAML と `ut-tdd team run --definition .ut-tdd/teams/default-hybrid.yaml --mode hybrid --json` の dry-run 検証へ接続し、案内だけで実行対象が無い状態を許さない。`consumerReadiness` は Bun/git/CLI/runtime の環境確認だけでなく、投影済み `AGENTS.md` managed block、VSCode manual task、`.ut-tdd` memory/handover/evidence/teams baseline、`default-hybrid` の Codex worker / TL reviewer 分離を `artifactReadiness` として検査し、artifact の意味ずれがあれば `ready` ではなく `fix_consumer_readiness` に戻す。monorepo consumer は `ut-tdd setup project --package-root <path>` で package root を明示でき、setup JSON は repo-root scoped adapter と packageRoot scoped CLI readiness (`consumerReadiness.workspace` / `cliResolution`) を分けて記録する。CLI readiness は projected hook / agent が呼ぶ bare `ut-tdd --version` の PATH 解決を必須にし、packageRoot の `package.json` に `scripts.ut-tdd` があり `bun run ut-tdd ...` で解決できる経路を `cliResolution.strategy=package-script` / `ut-tdd-package-script` の CI / VS Code task 証跡として記録する。bare PATH だけで `scripts.ut-tdd` が無い場合も、生成済み task / CI が `bun run ut-tdd ...` を実行できないため `ok=false` のまま `fix_consumer_readiness` に戻す。package script のみでは `bareCommandResolved=false` / `ok=false` のまま `fix_consumer_readiness` に戻し、VSCode task / CI / package-local smoke の成功を hook 自走性の証明にしない。brownfield では `importReport` に managed/preview/existing/written/skipped paths、managed-block merge 対象、`requiresReview`、`nextRoute=review_import_report` を出し、既存 non-mergeable config を黙って成功扱いにしない。setup 結果は `postSetupWorkflow` に `nextRoute=ready|review_import_report|fix_consumer_readiness`、未充足 gate、次 action、検証 command を構造化し、利用者が手作業で設計文書を探索しなくても初回 HELIX 稼働ルートを判定できるようにする。rename/cutover は PLAN-M-02 承認待ちとして明示し、setup 完了と cutover 完了を混同しない。`futureCommand=helix setup project` は target command であり、cutover/action-binding approval 前は `commandAvailability.futureCommandAvailable=false` として package/bin alias が未有効であることを出す | HAC-P6-03a / HAC-P6-03b |
| HR-FR-P6-04 | HBR-P1 / HBR-P6 / HBR-P9 | tag bump / release pin 更新は current/target を検出し、migration、compatibility warning、rollback point、idempotency evidence を出す。PLAN-M-02 identifier rename は `ut-tdd` / `.ut-tdd` / `area=harness` の hit 数だけでなく、source/test/runtime-state/adapter-config/consumer-template/plan/design/governance/distribution surface 別の blast-radius checklist を出し、cutover/action-binding approval 前に旧識別子残渣を apply 可能と誤認させない。rename cutover の approval 判定は `approve_cutover` / `approve_action_binding` と actor/tool/target だけでは足りず、approved params、current `cutoverSnapshot` sha256 binding、review evidence、expiry、audit、backup/rollback/monitoring evidence が揃うまで blocked にする | HAC-P6-04a / HAC-P6-04b |
...
| HAC-P6-03a | fresh repo | `ut-tdd setup project --dry-run` | hooks/adapters、現行 `.ut-tdd` state/memory/handover/evidence/teams baseline、将来 `.helix` target、GitHub rules/checks plan、consumer doctor baseline、`identifierTransition.status=blocked_pending_cutover_approval`、`mustNotApply=true`、`commandAvailability.futureCommandAvailable=false`、`importReport.mode=fresh`、`requiresReview=false`、`postSetupWorkflow.importReportRoute=ready` を返す。consumer readiness が未充足なら `postSetupWorkflow.nextRoute=fix_consumer_readiness` と未充足 gate/remediation を返し、ready なら `ut-tdd status --json` / `ut-tdd completion decision-packet --json` / `ut-tdd completion review-bundle --json` / `ut-tdd version-up dry-run --current v0.1.0 --target v0.1.4 --release-remote https://github.com/unison-ai-product/UT-TDD_AGENT-HARNESS-Pack.git --json` / `ut-tdd doctor --profile consumer` / `ut-tdd rename plan --json` / `ut-tdd handover status --json` / `ut-tdd team run --definition .ut-tdd/teams/default-hybrid.yaml --mode hybrid --json` へ進める次 action を返す。`completion review-bundle` は `bundleDigest` と `semanticBundleDigest` を保存し、S4 / version-up / rename / action-binding の supporting packet 束を人間判断前に照合する。`consumerReadiness.artifactReadiness` は `AGENTS.md` の consumer doctor / default-hybrid dry-run 案内、`.vscode/tasks.json` の手動 task と automatic task 無効化、`.ut-tdd` baseline、default-hybrid team の worker/reviewer 分離、version-up dry-run の plan-only 境界を検査し、artifact 欠落や意味ずれがあれば環境 CLI が解決済みでも ready にしない。`--package-root packages/app` のような monorepo 指定時は `consumerReadiness.workspace.monorepo=true`、`packageRoot=<repo>/packages/app`、`cliResolution.checkedFrom=<packageRoot>` を返し、adapter path 自体は repo root scoped のままにする。PATH 上の bare `ut-tdd --version` と packageRoot `package.json` の `scripts.ut-tdd` が両方解決した場合だけ consumer readiness は ready へ進める。bare PATH だけなら `ut-tdd-package-script=false`、package script だけなら `bareCommandResolved=false` として、どちらも `ok=false` / `postSetupWorkflow.nextRoute=fix_consumer_readiness` を返す。dry-run は無変更で終了 |
| HAC-P6-03b | brownfield repo に既存 docs/code/config がある | `ut-tdd setup project --apply` | managed 区画のみ更新し、衝突は stop + diff/import report にし、`importReport.skippedExistingPaths` / `reviewRequiredReasons` / `nextRoute=review_import_report` と `postSetupWorkflow.nextRoute=review_import_report` を返す。未整備 sub-doc は `skip_sub_doc` / 段階移行として記録する。PLAN-M-02 承認なしに state dir rename / CLI rename / marker rewrite を適用しない |
