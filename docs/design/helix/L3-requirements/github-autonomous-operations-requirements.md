...
最低限 `forward`、`reverse`、`redesign`、`recovery`、`incident`、`nfr_failure`、`discovery`、`additive_change`、`refactor`、`security` を区別する。Reverse は Forward 合流前の先行タスク、Redesign は確定設計を変更してから Forward 実装へ戻すタスクとする。分類不能・複数分類衝突は `full_v` へ fail-close する。
...
PoC は S0–S4 の決定後にのみ Forward/Reverse へ昇格する。Production Scrum は品質工程の省略ではなく、機能ユニット単位で L4–L7 を反復し、合流時に V の右腕検証を満たす。
...
全 gate、review receipt、trace closure が揃えば auto-merge を設定できる。main merge 後にDB投影と記憶圧縮を行い、stale continuationを残さない。layer checkpoint tag は `helix-l<N>-<version>`、release tag は SemVer とし、対象 commit、requirements digest、CI run、DB revision を含む annotated tag metadata/receipt を作る。release publish、配布先切替、不可逆 cutover は action-binding approval を維持する。
...
本書の承認は実装指示ではない。PO承認時に requirement/AC のIDと意味を凍結し、L4以降の設計・実装は別PLANでForward降下する。ADR-009/ADR-010の権威衝突とREBASELINE v0.5.0の未解決所見が収束するまで、runtime境界に関する実装preflightは開かない。
