...
全gate、review receipt、trace closureが揃った後、監査AIがcurrent HEADとreceiptを再照合して明示的にmergeする。
GitHub native auto-mergeは使わず、CI greenだけを契機にした無文脈mergeを禁止する。main merge後にDB投影と記憶圧縮を行い、stale continuationを残さない。layer checkpoint tag はannotated `helix/L<n>/<plan>/<snapshot>`とし、commit/tree/ledger digest、gate receipt、DB revisionを持たせる。移動・削除せず、訂正時は新tagでsupersedeする。checkpoint tagは進捗証拠であってreleaseではない。release tagはSemVerとし、全layer closure、requirements digest、CI run、DB revision、action-binding approval receiptを要求する。
...
PRのmerge可否はCIだけで決めない。作成者と別identity・別session・独立contextの文脈レビューAIが、L0 authority、L2要求、L3要件とAC、
...
UpdateはFeatureへ種類変更しない。正本へ取り込む場合はUpdate Issueのidentityとdecision historyを保持したまま、
ForwardまたはProduction ScrumのPLANへ接続する。実装上の問題はRecovery、操作・環境・外部serviceのtroubleは
Incidentへ分類する。priorityは`P0`（main復旧・security・data integrity）、`P1`（開発速度基盤・早期実証Feature）、
`P2`（通常Feature）、`P3`（Update）を正本とし、依存解放による繰上げは根拠receiptを必須にする。

### GH-FR-023 PLAN駆動モデル名・folder admission

IssueからPLANを生成する際はlayerだけのPLAN名を拒否し、canonical ID、表示名、物理pathに駆動モデルを必須化する。
generatorは駆動モデル別rootとlayer別subdirectoryだけを出力先に選べ、folder、IDの`workflow_model` token、frontmatter routeが
一致しない場合はfile write前にfail-closeする。任意path作成APIを正規入口に含めず、生成入口・directory allowlist・
filesystem write guardで許可形以外のPLAN作成を阻止する。
...
Incidentは反省記録とRecovery接続receiptの完成で閉じ、Recovery mergeを待って同時closeしない。計画済み検証の
想定外failureはRecoveryへrouteし、必須ACなら元PLANをblockする。PLAN体系全面修正はsystem-wide ForwardでL1〜L5を
再設計し、各移行PRでflat/nested dual-greenとDB/trace/dependency一致を要求する。全移行完了までlegacy loaderを維持する。
current PRの承認scope内で完結する軽微findingはPR evidenceへ記録し、scope外・再発防止・別episodeが必要なfindingだけを
Issue化する。
...
| GH-AC-034 | 全gate green後もGitHub native auto-mergeは設定されず、監査AIがcurrent HEAD、文脈review、DB追従、内部CI、GitHub Actions receiptを再照合した明示mergeだけが成功する |
