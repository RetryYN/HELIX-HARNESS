...
単一runtime運用時（cross-runtimeが一時的に利用不能な期間を含む）は、GH-FR-008と同様に独立subagent receiptを代替証跡とする。
この場合receiptへ`degraded_mode: true`と単一runtime運用中である理由を必須記録し、通常の別runtime/provider receiptと区別する。
degraded_modeを理由なく常態化させて分離を回避することを防ぐため、DB projectionはdegraded_mode receiptの発生比率を可視化する。
本要件における「独立」はモデル/providerの多様性による判断相互検証を指し、資格情報・実行環境レベルの信頼境界分離までは保証しない
（超個人開発・単一マシン前提の脅威モデルであり、production/外部公開へ拡張する際は本前提を再評価する）。
...
Claudeが監査・修正した場合はCodex、Codexが監査・修正した場合はClaudeのHELIX subagentを起動して差分クロスレビューを行う。
...
単一runtime運用時は、GH-FR-018と同様に理由記録済み`degraded_mode: true`の同一session内独立subagentレビューを
「別family HELIX subagent」の代替とする。degraded時も自己承認は禁止のままとし、修正AI本体とは別の独立subagent
呼び出しを必須にする（session内lightweight承認への縮退は認めない）。
...
関連branchへ復旧寄与度と依存順のpriorityを付けてmain復旧を最優先にする。blocker特定、修正PR、別familyクロスレビュー、
doctor、GitHub Actions合格を同一修正HEADへ束縛し、Issueをclosure receipt付きでcloseしてから通常merge停止を解除する。

Recovery修正の別familyクロスレビューも、単一runtime運用時はGH-FR-018/019と同じ`degraded_mode: true`独立subagent
...
| GH-AC-014 | CIがgreenでも、別runtime/providerの文脈レビュー（または単一runtime時は`degraded_mode`付き独立subagent receipt）がL0/L2/L3/L4、Issue/PLAN、diff、trace、consumer、security・blast radiusを読んだcurrent HEAD束縛receiptを持たなければmergeをblockする |
...
| GH-AC-016 | 作成AIの内部CI合格後だけPR化でき、監査AIの修正後は別family HELIX subagentの差分クロスレビュー、内部CI、GitHub Actions、DB追従が同じ新HEADで揃うまでmergeできない |
