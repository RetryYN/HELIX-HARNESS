...
L3要件の追加または意味変更はユーザー承認を必須とし、AI review、CI、doctorを承認の代替にしない。承認前はDraftを維持し、
承認receiptを要件revision、回答source、current HEADへ束縛する。監査範囲は固定表へ埋め込まず、変更Lから上下隣接、対応Vペア、
...
primary mode=`recovery`、trigger=`regression_dev`の修正Issueを起票する。blocker特定、修正PR、別familyクロスレビュー、
...
| GH-AC-019 | L3要件の追加・意味変更はcurrent revisionへ束縛されたユーザー承認receiptなしでfreezeまたはmergeできない |
