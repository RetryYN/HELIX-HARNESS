...
main push、scheduled run、手動runでは採用Issue全件を検査し、candidate treeに存在しない`plan_id`もfail-closeする。
...
| GOP-AC-04a | GOP-FR-05a | open依存を残したclosed Issue、およびPLAN↔Issue参照不一致のfixtureがstable findingでfail-closeする。PR focus外のdriftは隔離し、main/scheduled全件監査ではmissing PLANを拒否する | U-IHIER-002, U-IHIER-003, U-IHIER-005 |
