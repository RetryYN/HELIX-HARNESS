...
本書は依存する二つのworkstreamを保持するが、完了判定を混ぜない。

- **A: GitHub運用投影** = GOP-FR-01〜10 / GOP-AC-01〜09。Projects、Issue階層、正本境界を扱う。
- **B: CI実行配分** = GOP-FR-11〜14 / GOP-AC-10〜11。外部・内部・nightlyの検証配分を扱う。

AのgreenをBのgreenで代替せず、BのgreenをAのgreenで代替しない。実装PLAN、review evidence、完了receiptは
workstream別に作り、両方を一つの巨大sliceとして同時完了させることを要求しない。
...
同期器は実行前に、`harness.db` projectionとrepo-owned ledgerから次を含むtyped desired-state packetを生成する。

- canonical HEAD、生成時刻、source digest。
- PLAN ID、Issue/PR URLとのexplicit binding、drive model、親PLAN、reentry target。
- roadmap gate/span、active frontier、lifecycle state、terminal outcomeとclosure receipt参照。
- 期待するProject item、Status、iteration、parent/sub-issue relation。

GitHubのopen Issue/PR一覧、本文cross-reference、Projectの現在値はidentity照合とdrift観測にだけ使用できる。
それらだけからroadmap状態、PLAN状態、active frontier、completionを推定してdesired stateを作ってはならない。
canonical bindingがないGitHub entityは`unbound_intake`としてfindingにし、受付済みを示す専用表示以外の工程状態へ
自動昇格させない。

### §1.2 Status・終端写像

| Project Status | canonical条件 | 禁止する代替根拠 |
|---|---|---|
| `Todo` | Issue受付ゲートを通過し、canonical PLAN/episodeへbind済みだがactive frontierではない | open Issueであることだけ |
| `In Progress` | desired-state packet上でactive frontier、実行中episode、または終端PR待ちのいずれか | PRからのcross-reference、PR open、CI pendingだけ |
| `Done` | typed closure receiptがterminal outcomeを持ち、そのreceiptへbindされた終端PRがmerge済み | Issue closed、PR merged、CI greenのいずれか単独 |

terminal outcomeはIssue closure contractの`resolved / rejected / quarantined / superseded / cancelled`を保持する。
`Done` itemはclosure receiptのretention期限まで残し、期限後はarchiveできる。自動削除は禁止し、archive後もreceiptから
Project item IDと最終read-back digestを追跡できなければならない。receipt欠落・drift検出時はreopen/recovery候補にする。
...
Statusは§1.2の写像だけで決定し、GitHub entityのopen/closed/cross-referenceから工程状態を推定しない。
...
同期前にProject全件数、field数、option数、sub-issue件数/深さをread-backし、上限到達後ではなく上限到達前に
fail-closeする。APIがページングを返す場合は全pageを取得してsnapshot digestを作り、先頭100件等のpartial snapshotを
正常扱いしない。mutationはbounded batchで実行し、primary/secondary rate-limitを観測したbackoffと再開cursorをreceiptへ
記録する。上限内の件数を「pagination未対応」を理由に恒久拒否する実装は受入不可とする。

optionの同一性は既存IDで保持する。renameは同じIDの表示名変更、delete/recreateは別optionとして扱い、後者を
「ID再採番の吸収」と呼ばない。stale IDを検出した場合は最新field definitionを再取得し、名前だけで別optionへ
自動付替えせず、identity driftとしてfail-closeする。
...
表示対象にはcanonical snapshot時刻とdrift状態を含め、staleなboardを現在状態として読ませない。
...
比較対象は§1.1のdesired-state packetとGitHub read-back snapshotであり、GitHub snapshot同士の自己比較を
drift検出と呼ばない。
...
| GOP-AC-03 | GOP-FR-04 | canonical desired-state packetのroadmap gate/spanがProjects view/status field/iterationへ一方向投影され、GitHub open/close/cross-referenceだけからStatusを推定せず、手動変更が次回同期で正本値へ収束する | GOP-T-03 |
| GOP-AC-04 | GOP-FR-05 | PLAN 親子・駆動モデル別 Issue fixture が parent/sub-issue へ規則どおり写像され、孤児 sub-issue が 0 件になる | GOP-T-04 |
| GOP-AC-05 | GOP-FR-06 | item/field/option/sub-issue上限超過を同期前にfail-closeし、全page取得・bounded batch・rate-limit再開を証明する。option renameは既存IDを保持し、delete/recreateによるidentity driftは自動付替えせず拒否する | GOP-T-05 |
| GOP-AC-06 | GOP-FR-07 | `helix status` の active frontier と Projects board + Issue 階層の表示が同一 fixture 上で一致する | GOP-T-06 |
| GOP-AC-07 | GOP-FR-08 | canonical desired-state packetとGitHub read-backを意図的に不一致にしたfixtureで`helix doctor`がfail-visibleになり、GitHub snapshot同士の一致だけではgreenにならない | GOP-T-07 |
