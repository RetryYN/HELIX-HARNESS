...
上表の詳細failure 12件に加え、HST境界で直接検査する
`HIL_OS_SYMLINK_ESCAPE`、`HIL_OS_SQLITE_LOCK_TIMEOUT`、`HIL_OS_CONTRACT_VIOLATION`、
`HIL_OS_PRIORITY_INVALID`、`HIL_SUPPLY_CHAIN_NOT_REPRODUCIBLE`、
`HIL_ACTION_BINDING_APPROVAL_MISSING`、`HIL_OS_LOGIC_FORK`の7件もclosed failure集合へ含める。
この19件に、unexpected OS adapter exceptionを境界変換する`HIL_OS_INTERNAL_ERROR`と、unexpected supply-chain adapter exceptionを境界変換する
`HIL_SUPPLY_CHAIN_INTERNAL_ERROR`の2件を加える。したがってL6の`OsSupplyChainFailureCodeV1`は合計21値であり、別名や自由文字列への縮退を認めない。
internal 2件は既存19 HSTの分母へ追加せず、adapter fault mutationでcause digest付きのexact codeとcurrent増分0を検証する。
...
L6 §2のslice-local closed `*V1` schemaを全public resultとreceiptの型正本とする。adapter、OS別実装、scanner、installerは
同じfield集合とclosed unionを返し、`unknown`、自由形式map、旧抽象名、OS別result型への弱化を禁止する。
