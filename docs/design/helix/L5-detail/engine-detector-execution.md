...
complete signatureとslice-local closed V1 definitionはL6 §1.1/§2を正本とし、未作成`src/schema/*`へ
design-time importしない。既存20 U／14 ITの分母を次の21 public APIへexact joinする。
...
`U-EDX-019`は`resolveCurrentExecutionAuthority` → `commitExecutionAuthority`の順序付きcompositionである。
`reconcileExecutionAuthority`はseal後fault/pending専用の`U-EDX-020`であり、U-019へ統合しない。
