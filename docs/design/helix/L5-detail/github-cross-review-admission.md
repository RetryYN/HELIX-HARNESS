...
2. repository、PR number、candidate HEAD、approve、blocker 0、runtime独立性が一致する。
   ...
   runtime独立性の唯一のauthorityは**receipt validator**である。v2は`buildClaudePrReviewReceipt`が
   `runtime_independence_missing`で、v4は`validateProviderNeutralReviewReceipt`が
   `declared_author_runtime === reviewer_runtime`で、それぞれdecode前にfail-closeする。したがって
   同一runtimeのcommentはcanonical receiptへ昇格せず`current_head_review_receipt_missing`になる。
   admission側に独立性の再判定は置かない（PLAN-RECOVERY-41、`U-GCRA-006`／`U-GCRA-007`）。
...
```json
{
  ...
  "assets": [
    ...
    { "asset_id": "github-reviewed-merge-receipt-persistence", "classification": "existing_runtime", "artifact_path": "src/runtime/github-cross-review-admission.ts", "resource_kind": "typescript_export", "resource_name": "persistReviewedMergeReadAfterReceipt", "source_digest": "sha256:59f7bd768d4d625bf3b73a282832dc1882cbc173e0f4c0c9381fc6ca225f8743", "current_authority": true }
  ],
