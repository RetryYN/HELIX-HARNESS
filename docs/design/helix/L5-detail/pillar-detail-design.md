...
|----------|---------------|-------------|----------------------|
| HC-P0 forward-return-contract | HB-P0 | workflow PLAN / gate result / stop reason を Forward 返却契約として正規化する | planner -> gate -> continuation projection |
| HC-P1 autonomous-work-contract | HB-P1 | scheduler / job / budget / version target / L2 template pack / continuation checkpoint の詳細 contract を定義する | scheduler -> job queue -> event/DB projection |
...
|----------|-----------------|-----------------------------|-----------------|------------------------------|----------|
| HC-P0 forward-return-contract | PLAN `kind` / `workflow_phase` / `forward_return` / `gap-only` / `version_target` / stop reason | `plan_registry` + `workflow_runs` + `gate_runs` + continuation event/projection | `ForwardReturnDecision` (`return_to` / `defer_to_version` / `gap_only` / `blocked_reason`) | return/defer/gap targetなしのworkflow完了主張、return targetのarchive化または欠落、event/projection間のstop reason不一致 | forward-return validator、stop-reason normalizer |
| HC-P1 autonomous-work-contract | job candidate、resume 3 predicates、budget window、iteration cap、context threshold、L2 template availability、release/tag target、continuation event id | `jobs` + `workflow_runs` + `budget_events` + `version_target` ledger + append-only event + DB projection + bounded memory | `AutonomyResumeDecision` (`dispatch` / `idle` / `checkpoint` / `version_up`) | budget期限切れ、stale lock、`next_action`欠落、event重複、partial projection、prose/CURRENT/CLI resurrection、migration/rollback/idempotency evidenceなしのversion-up | scheduler predicate evaluator、event-first continuation writer/idempotent projector/replayer、resurrection detector、version-up dry-run planner、L2 template pack selector |
...
| HC-P7 knowledge-contract | memory query、provider delegation evidence、glossary term、bounded context、context-crossing command、rename event | `.helix/memory` + provider evidence audit + glossary/context-map + relation graph | `BoundedRecallPacket` + `GlossaryDriftFinding` + context translation rule | per-agent memory silo を SSoT として使うこと、provider evidence を recall/continuation source にすること、どちらかの runtime で bounded recall が欠落、supersedes なしの rename、anti-corruption boundary なしの context-crossing call | memory reader、provider evidence boundary checker、glossary drift detector、context-map checker |
...
いずれも HELIX/HELIX harness のPython semantic core＋TypeScript/Node transactional boundary・`.helix` projection・PLAN 正本へ翻案し、旧 runtime を current path にしない。
