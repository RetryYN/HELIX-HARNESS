...
- `projectResidentLaneAssignments`: byte同一の重複を吸収し、同じassignment IDの正当なtakeover列はauthority不変・fence連番・lane／lease更新・時刻単調を検証して最新revisionへ畳み込む。それ以外の異内容衝突を非active recordも含めて拒否し、`created_at <= observed_at < expires_at`を満たすactive exact setを決定順で返す。writer／scope競合だけをactive集合へ限定する。GitHub repository／Issue scopeは大小文字をcase-foldし、複合identityはcanonical tupleとして比較する。未来開始、期限切れ、repository内のbranch二重writer、repository内のscope二branchをtyped failureにし、別repositoryの同名branch／scopeだけを独立に扱う。
