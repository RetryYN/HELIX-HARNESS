...
- `projectResidentLaneAssignments`: byte同一の重複を吸収し、active exact setを決定順で返す。期限切れ、repository内のbranch二重writer、repository内のscope二branchをtyped failureにする。別repositoryの同名branch／scopeは独立に扱う。
