...
expected event/projection/checkpoint/result heads、transition event/projectionと対象recordを持つ。Node `AgentLifecycleStoreV1`は
...
L6 §2のslice-local closed `*V1` schemaを全public result、event、receipt、decisionの型正本とする。runtime adapterは
このfield集合とclosed unionを弱化できず、旧抽象名、`unknown`、自由形式map、runtime別result型を受理しない。
