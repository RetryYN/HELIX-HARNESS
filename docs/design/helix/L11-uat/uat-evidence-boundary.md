...
| handover outstanding | `src/handover/index.ts` / `ut-tdd handover` | `tests/handover.test.ts` | stale handover や未解決項目を完了 claim に混ぜない。handover 不在時は doctor が skipped として明示する。 |
