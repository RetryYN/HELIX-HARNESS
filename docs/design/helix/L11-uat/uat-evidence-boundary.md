...
| completion packet | `npm run helix -- completion decision-packet --json` |
...
| completion decision packet | `src/lint/completion-decision-packet.ts` / `npm run helix -- completion decision-packet --json` | `tests/completion-decision-packet.test.ts` / `tests/cli-surface.test.ts` | `completionClaimAllowed=false` の間は L11 承認を出さず、S4 / version-up / cutover packet を人間レビューへ残す。 |
