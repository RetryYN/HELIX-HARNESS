...
| `EnvironmentPromotionController` | Node | GitHub Deployments/Environments/ActionsからVPS staging→cloud productionの同一artifact promotionを統制 | deployment/promotion receiptのみ | editor依存操作、environment混同、artifact drift、staging未検証、production承認scope外 |
| `CloudDeploymentAdapter` | Node adapter | provider非依存deployment contractをAWS/GCP/Azure等へ投影 | provider receiptのみ | cloud APIのdomain漏出、provider固有schemaの正本化 |
...
14. cloud providerはadapter交換可能とし、production DB migrationはexpand→deploy→contractと完全性receiptを必須にする。
