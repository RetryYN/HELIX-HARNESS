...
related_l3: docs/design/helix/L1-requirements/infinity-loop-platform-requirements.md
...
発見するための並行設計である。L1/L14 pairがfreezeするまでは本書も`draft`とし、未確定値を実装契約として
扱わない。
...
| `LayerLedgerRegistry` | Node | L0–L14 ledger type、粒度、template、authority、entry/exit contractをversion管理 | catalog/eventのみ | layer未登録、in-place version変更 |
...
|---|---|---|---|
| L0 | charter outcome、価値仮説、non-goal | L1 requirement authority | L14→L0 value feedback |
| L1 | business/technical/NFR要求定義 | L2 applicability、L3/L4 carry | L14 operations |
| L2 | screen/prototype/interaction/state | L3 screen requirement、L4 UI standard | L10 UX/実データ検証 |
| L3 | system要求、FR、AC | L4 architecture/function/data obligation | L12 acceptance |
...
| L6 | function/class signature、invariant、edge case、WBS設計 | L7 TDD/implementation unit | L7 unit/TDD evidence |
| L7 | code/test artifact、implementation判断 | L8–L14 verification input | L6 design oracle |
| L8 | integration run/assertion記録 | L9 verification input、L5 backprop | L5 detail design |
| L9 | system run/assertion記録 | L10/L11 input、L4 backprop | L4 basic design |
| L10 | UX/a11y/real-data finding記録 | L11/L12 input、L2 backprop | L2 screen/prototype |
| L11 | review/UAT finding記録 | L12 decision、L3/L1 backprop | L3 acceptance support |
| L12 | acceptance decision/evidence記録 | L13 deploy authority、L3 backprop | L3 requirement/AC |
| L13 | deploy/post-deploy/incident evidence記録 | L14 operational input、affected layer backprop | deployment contract |
| L14 | operational/time/value evidence記録 | L0/L1 feedback | L1 operations、L0 value |

`L3↔L12`が正規acceptance pairであり、L11はUAT/review supportとしてそのedgeを補助する。L0とL1は同じL14へ結ぶが、L0はvalue outcome、L1はoperational requirementという別edge kindを持ち、混同しない。
