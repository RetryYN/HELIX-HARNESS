...
| restore dry-run branch | `refs/heads/helix/restore/<repository_id>/<savepoint_id>/<operation_id>` |
...
| L6 | product code、migration、runtime artifact | L7 Green/refactor input | L7 test implementation/TDD closure |
...
### §4.9 Visual Design HARNESS verification spine
...
|---|---|---|---|
| L5↔L8 | Detailed Visual Contract ↔ Visual Contract Verification | component/region×state×theme×viewport、token/asset provenance、computed style、geometry、contrast、focus、baseline | `G-VIS-L8 VisualDetailContractGate` |
| L4↔L9 | Cross-screen Visual System/Pattern ↔ Visual Integration | screen pair/flow×breakpoint、shared pattern、hierarchy/density、token/asset continuity、overlay/z-index、motion continuity | `G-VIS-L9 CrossScreenVisualIntegrationGate` |
| L3↔L10 | Visual FR/NFR/AC ↔ Browser/Data Visual System Verification | browser engine×OS profile×viewport/DPR×theme/preferences×representative product-data scenario | `G-VIS-L10 BrowserDataVisualSystemGate` |
| L2↔L11 | Prototype Agreement/Visual Intent ↔ Human Visual Acceptance | agreement criterion×current evidence packet×human actor/verdict | `G-VIS-L11 VisualAcceptancePreferenceGate` |
...
| table | 主key | 必須field・不変条件 |
|---|---|---|
| `visual_contract_versions` | contract ID＋revision | layer、scope、prototype/profile/binding digest、authority、status |
| `visual_obligations` | obligation ID＋revision | contract、subject/state/theme/viewport、oracle、applicability、source span |
| `visual_baseline_versions` | baseline ID＋revision | agreement revision、artifact digest、human review、supersedes、status |
| `visual_verification_runs` | run ID | layer/gate、matrix manifest、producer/verifier、snapshot/config digest、status |
| `visual_matrix_cells` | run＋cell ID | browser/OS/viewport/DPR/theme/preference/data scenario、applicability、verdict |
| `visual_render_observations` | observation ID | cell、subject、computed style/geometry/render digest、measurement version |
| `visual_assertion_results` | assertion ID＋run | obligation、oracle、expected/actual、failure code、evidence digest |
| `visual_cas_artifacts` | artifact digest | sha256、byte length、media type、kind、producer run、classification、redaction/scan/retention receipt |
| `visual_artifact_edges` | edge ID | artifact、run、cell、obligation、assertion、purpose。CAS identityと利用関係を分離 |
| `visual_evidence_packets` | packet ID＋revision | layer/gate、contract/source/run、matrix manifest、derived verdict、security、shard manifest |
| `visual_acceptance_sessions` | session ID | L2 agreement/L10 evidence revision、human actor、criteria、status |
| `visual_judgments` | judgment ID | session/criterion、accept/reject/preference/dissent、reason、authority |
| `visual_pair_receipts` | pair＋scope＋revision | design/verification snapshot、denominator、review/verdict、stale reason |
| `visual_authority_models` | model ID＋version | purpose、pair-set digest、receipt/status、activation/supersession |
...
- evidence custodian、classification、redaction、retention、access、transfer、revocation policy
...
### §11 PO7 decision activation aggregate
...
## §13 Post-PO Design Freeze transition
...
authority event→Design Freeze receipt→freeze/progress projection→L01 `proposed/pending_pair/not_frozen/not_counted`
