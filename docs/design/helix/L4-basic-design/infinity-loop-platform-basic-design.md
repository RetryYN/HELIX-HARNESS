...
| `ContextualPrReviewRouter` | Node | AI-Aと別identity/sessionのAI-BへL0/L2/L3/L4、Issue/PLAN、diff、trace、consumer、security・blast radiusのreview packetを投影 | review receipt eventのみ | 必須文脈digest欠落、identity/session同一、HEAD不一致 |
...
| `LayerAwareAuditPlanner` | Node | version管理graphを走査しchanged Lから上下隣接、対応Vペア、trace consumer、必須検査を動的導出し、修正主体と別identity/sessionへレビュー委譲 | audit plan/receipt eventのみ | 未知edge、cycle、未登録consumer、影響閉包欠落、自己承認、identity/session同一、修正後HEAD drift |
...
| `MainRecoveryController` | Node | merge後Full失敗で通常mergeを停止し、Recovery修正Issue→修正PR→独立review→doctor→GitHub CI→Issue closeの解除closureを管理 | recovery/解除receiptのみ | Issue/PR/closure欠落、自動上書き、通常開発先行、doctor/CI/review欠落 |
| `EnvironmentPromotionController` | Node | GitHub `workflow_dispatch`/Environmentを正式面、HELIX CLIを同一workflow補助面としてVPS staging→cloud production昇格を統制 | deployment/promotion receiptのみ | editor依存操作、別workflow起動、environment混同、artifact drift、staging未検証 |
| `CloudDeploymentAdapter` | Node adapter | provider非依存deployment contractをAWS reference adapterからGCP/Azure等へ投影 | provider receiptのみ | cloud APIのdomain漏出、provider固有schemaの正本化 |
| `ProductionMigrationGate` | Node / GateEngine facade | expand→deploy→contract、backup/restore rehearsal、互換期間、migration/rollback oracle、承認を検査 | migration receiptのみ | production完全性欠落、破壊的one-shot migration、staging証拠流用 |
| `DeploymentCapabilityPreflight` | Node | GitHub plan/visibility/protection、environment concurrency、self-review、OIDC trust、AWS role分離を検査 | capability receiptのみ | protection利用不能、同時deploy、長期key、広域trust、role共有 |
| `DeploymentProfileResolver` | Node | workload/data/risk/compliance入力からruntime、DB、account、IaC、log retentionの標準profileを決定 | profile decision receiptのみ | 根拠なし質問停止、不要DB、risk過小account、未redactログ |
| `UpdateBacklogClassifier` | Node | update/lifecycle/priority/area labelとtraceから正常open、active、異常、closedを分類 | backlog projection/receiptのみ | label欠落、state矛盾、trace欠落、futureのactive blocker誤算入 |
...
| `OrchestrationAdmissionGate` | Node / GateEngine facade | Codexからのorchestration requestにwork graph、capacity route、delegation receipt、worker descriptor、`dispatch_mode=controlled_bench|normal_lane`を要求し、共通構造だけを検証する。security/admission policyは各owner Gateへ委譲する | admission receiptのみ | work graph欠落、capacity超過、未知mode、owner Gate receipt欠落、descriptor欠落 |
| `WorkGraphPlanner` | Node | Issue/PLAN/design obligationからtask dependency edge、並列可能性、capacity class、route profileを導出する | work graph eventのみ | cycle、orphan task、dependency未分析、capacity未指定 |
| `CapacityRouter` | Node | 最大8 slot accountingで`slot_id`、parent、dependency、state、quota snapshot、lease、started/terminal時刻を記録し、dependency-aware scheduling、bounded queue/backpressure、quota threshold前handover、slot isolationを行う | capacity snapshot/route receipt | 8超過、依存前倒し、unbounded queue、quota枯渇継続、4-slot PoCを8-slot claim |
| `WorkerBroker` | Node | Claude/Codex/Kimi typed requestを同一Node brokerで処理する。`controlled_bench`はSecurityFoundationGate greenだけで隔離benchへ、`normal_lane`はSecurityFoundationGateとWorkerAdmissionGateの用途別S4 greenで通常laneへdispatchする。Kimiがnot-admittedの間の通常作業はClaude代行workerへ送り、Terra review後にCodex親が検収する | worker lane event/receiptのみ | raw spawn、mode/policy receipt不一致、readiness前Kimi、未admit通常lane、自己承認 |
| `OrchestrationAcceptanceGate` | Node / GateEngine facade | worker terminal receipt、独立reviewer receipt、Codex親の検収receiptを同一HEAD・別identity・別session・順序どおりに検証し、receipt先書き・worker自己承認・親検収なしをfail-closeする | acceptance receiptのみ | receipt先書き、自己承認、親検収欠落、HEAD/order/identity不一致 |
| `SecurityFoundationGate` | Node / GateEngine facade | security-foundation readiness receipt（security policy、sandbox enforcement、credential isolation、secret deny、allowlist egress policy固定）の存在と有効性を検査し、readiness前はKimi起動を全面deny、readiness後はcontrolled benchのみ許可、S4 admission後は用途別laneだけを許可する | readiness receiptとadmission receipt | readiness前起動、bench逸脱、未admit用途起動、policy欠落 |
| `WorkerAdmissionGate` | Node / GateEngine facade | blind benchmark（fixed fixture/rubric/task/riskでauthor claim 0のBlindPacketV1）のscorecard、重大failure単独記録、用途別admit/retire receiptを検証し、smoke-only結果・平均相殺・根拠なしeffort固定をfail-closeする | admission decision receipt | smoke-only admission、重大failure平均相殺、根拠なしeffort固定、S4未了を正本claim |
| `OrchestrationProjection` | Node | HarnessDbPortだけをauthoritative write portとして使い、orchestration event列からwork graph、slot state、quota、lane identity/session/context、receipt digestをprojectionする。idempotent replay、dedupe、causal order、許可transition、orphan検出を保証する | HarnessDbPort経由のprojection/checkpointのみ | DB直書き、event片肺、duplicate side effect、causal inversion、illegal transition、orphan lane、checkpoint欠落 |
...
| `RoleSeparationGate` | Node / GateEngine facade | AI-A/AI-Bのidentity、session、role、context分離を検査 | separation receiptのみ | self-review、identity/session同一、review evidence欠落 |
...
| `UniversalReverseGate` | Node / GateEngine facade | 実行へadmitされた全IssueのR0–R4、主駆動pair、substanceを実装前に検査 | Reverse receiptのみ | exemption、省略、順序違反、hollow artifact |
...
### §2.4 orchestration event構造とreceipt検証契約（WCC-FR-13/15 trace）

orchestration eventは`WCC-FR-15`のlogical fieldを全て持ち、物理table/index/migrationはL5で設計する。
本節はL4としてcomponent ownership、transaction boundary、event/receipt/projection間の不変条件だけを確定する。

#### §2.4.1 orchestration eventの論理field

全orchestration eventは次のfieldを持つ（`WCC-FR-15`）:

- `event_id`: UUID v7またはULID（時刻順・単調増加・global unique）
- `event_type`: `orchestration.request_received` / `orchestration.admitted` / `orchestration.worker_dispatched` / `orchestration.worker_terminal` / `orchestration.review_completed` / `orchestration.accepted` / `orchestration.rejected` / `orchestration.failed` / `orchestration.cancelled`
- `occurred_at`: RFC3339 timestamp
- `plan_id`: 対象PLAN識別子（nullable、ad-hoc taskではnull）
- `parent_lane_id`: orchestrator lane識別子（Codex orchestratorのsession/runtime identity）
- `lane_id`: worker lane識別子（Claude代行worker、将来のKimi child lane）
- `causation_id`: 直接トリガーとなったevent_id（nullable、最初のrequestではnull）
- `correlation_id`: 同一orchestration episodeを束縛する識別子（request時に発行、terminal/accepted/rejectedまで不変）
- `head_sha`: event発生時のgit HEAD SHA（event発行前にHEAD取得、event appendとprojection updateを同一transactionで束縛）
- `payload_digest`: event payload本体のSHA-256 digest（payload自体はevent rowまたは別tableへ格納）
- `schema_version`: event schema version（例: `helix-orchestration-event.v1`）

#### §2.4.2 receipt構造（6軸＋identity/session/context分離、WCC-FR-06/13 trace）

各receiptは`HR-AC-ROUTE-01`の6軸routing入力と`WCC-FR-06`のidentity/session/context分離を満たす:

**Delegation Request Receipt** (Codex orchestrator発行):
- `request_id`: UUID
- `correlation_id`: 同一episode識別子
- `parent_lane_id`: Codex orchestrator identity
- `parent_session_id`: Codex session
- `parent_context_digest`: Codex context（L0/L2/L3/L4、Issue/PLAN、依存graph）のdigest
- `parent_runtime`: `codex`
- `parent_provider`: `openai` 等
- `parent_model`: model識別子（記録のみ、family一致は必須でない）
- `work_graph_digest`: task依存graphのdigest
- `capacity_route_digest`: slot割当・依存schedule・quota budgetのdigest
- `requested_at`: RFC3339 timestamp
- `head_sha`: request時のHEAD
- `input_digest`: delegation task/oracleのdigest
- `order`: episode内の順序番号（request=1）

**Worker Terminal Receipt** (Claude代行worker、将来のKimi child lane発行):
- `terminal_id`: UUID
- `correlation_id`: 同一episode識別子
- `lane_id`: worker lane identity
- `worker_session_id`: worker session
- `worker_context_digest`: worker contextのdigest
- `worker_runtime`: `claude` / `kimi` 等
- `worker_provider`: `anthropic` / `moonshot` 等
- `worker_model`: model識別子
- `terminal_status`: `proposal_ready` / `failed` / `cancelled`
- `proposal_digest`: worker出力proposalのdigest（sandbox内差分、schema検証済み）
- `completed_at`: RFC3339 timestamp
- `head_sha`: terminal時のHEAD（request時と同一であることが期待される）
- `output_digest`: 最終artifactのdigest
- `order`: episode内の順序番号（terminal=2）

**Independent Review Receipt** (Terra reviewer発行、WCC-FR-13 trace):
- `review_id`: UUID
- `correlation_id`: 同一episode識別子
- `reviewed_lane_id`: 対象worker lane
- `reviewer_lane_id`: Terra reviewer identity
- `reviewer_session_id`: reviewer session
- `reviewer_context_digest`: review context（worker proposal＋L0/L3/L4＋security/blast radius）のdigest
- `reviewer_runtime`: `codex_subagent`等の実行surface
- `reviewer_provider`: `openai` 等
- `reviewer_model`: `gpt-5.6-terra`等のmodel識別子
- `review_verdict`: `approved` / `rejected` / `revise_required`
- `review_findings`: finding識別子配列
- `reviewed_at`: RFC3339 timestamp
- `head_sha`: review時のHEAD（terminal時と同一であることが期待される）
- `input_digest`: review対象proposalのdigest
- `evidence_digest`: review evidenceのdigest
- `order`: episode内の順序番号（review=3）

**Acceptance Receipt** (Codex親orchestrator発行、WCC-FR-13 trace):
- `acceptance_id`: UUID
- `correlation_id`: 同一episode識別子
- `acceptor_lane_id`: Codex親orchestrator identity（parent_lane_idと同一）
- `acceptor_session_id`: 受入検証時のCodex session（request時と独立可能だがidentityは同一）
- `acceptor_context_digest`: acceptance context（worker proposal＋review receipt＋oracle）のdigest
- `acceptor_runtime`: `codex`
- `acceptor_provider`: `openai` 等
- `acceptor_model`: model識別子
- `acceptance_verdict`: `accepted` / `rejected` / `quarantined`
- `acceptance_findings`: finding識別子配列
- `accepted_at`: RFC3339 timestamp
- `head_sha`: acceptance時のHEAD（全receiptが同一HEADへ束縛されることが期待される）
- `input_digest`: acceptance対象proposal＋review receiptのdigest
- `verdict_digest`: 最終判定evidenceのdigest
- `order`: episode内の順序番号（acceptance=4）

#### §2.4.3 receipt検証不変条件（WCC-AC-11 trace）

`OrchestrationAcceptanceGate`は次を検証する:

1. **順序**: `request.order=1` → `terminal.order=2` → `review.order=3` → `acceptance.order=4`の昇順。order逆転・欠番・重複を拒否。
2. **同一HEAD**: 全receiptの`head_sha`が一致。HEAD drift検出時はstale化してreplay不可。
3. **identity分離**: `parent_lane_id`、`lane_id`、`reviewer_lane_id`はpairwise distinctで、`acceptor_lane_id == parent_lane_id`。worker自己承認・reviewer自己承認を拒否。
4. **session分離**: `parent_session_id`、`worker_session_id`、`reviewer_session_id`はpairwise distinctとする。`acceptor_session_id`はparentと同一identityに属し、request継続ならparent sessionと同一、handover後なら別sessionを許可するが、worker/reviewer sessionとは常に異なる。
5. **context独立**: `parent_context_digest`、`worker_context_digest`、`reviewer_context_digest`、`acceptor_context_digest`がそれぞれ独立。context共有・流用を拒否。
6. **receipt存在**: request receiptなしのterminal、terminal receiptなしのreview、review receiptなしのacceptanceを拒否（receipt先書き禁止）。
7. **親検収**: acceptance receiptが欠落している場合、episodeは`completed`にせず`pending_acceptance`のまま保持。

failure mode:
- `HIL_ORCHESTRATION_RECEIPT_ORDER_INVALID`: order逆転・欠番・重複
- `HIL_ORCHESTRATION_HEAD_DRIFT`: HEAD不一致
- `HIL_ORCHESTRATION_IDENTITY_NOT_SEPARATED`: worker/reviewer/acceptorのいずれかがidentity重複
- `HIL_ORCHESTRATION_SESSION_NOT_SEPARATED`: session同一
- `HIL_ORCHESTRATION_CONTEXT_NOT_INDEPENDENT`: context digest共有
- `HIL_ORCHESTRATION_RECEIPT_MISSING`: 前段receiptなしの後段receipt
- `HIL_ORCHESTRATION_ACCEPTANCE_MISSING`: terminal/review完了だが親検収なし

#### §2.4.4 DB convergence不変条件（WCC-FR-15 trace）

`OrchestrationProjection`は次を保証する:

1. **event append-only**: event rowへのUPDATE/DELETE禁止trigger。
2. **event_id dedupe**: 同一`event_id`の重複insert拒否（unique constraint）。
3. **causal order**: `causation_id`が存在する場合、参照先eventが既にappendされていることを検証（FK constraint）。
4. **許可transition**: 状態機械定義の遷移以外を拒否（例: `admitted`から直接`accepted`への遷移は`worker_dispatched`/`worker_terminal`/`review_completed`を経由しない限り不可）。
5. **idempotent replay**: 同一event列から同一projection/checkpointを再構築可能（deterministic projection logic、timestamp/UUIDはeventから取得）。
6. **orphan検出**: `parent_lane_id`または`lane_id`が`agent_instances`へ参照できない場合は`HIL_ORCHESTRATION_ORPHAN_LANE`でfail-close。
7. **projection/checkpoint収束**: 同一`head_sha`のevent列から再構築したprojectionとcheckpointが既存projectionと一致（digest比較）。drift検出時は`HIL_ORCHESTRATION_PROJECTION_DRIFT`。

物理table設計（table名、column型、index、migration DDL）はL5で確定する。L4はlogical field、ownership、不変条件だけを設計正本とする。
...
1. Reverse R0–R4は実行へadmitされた全Issueで必須とし、`none/exempt/not-required`を持たない。future Update backlogはactive化まで実行Issueへ算入しない。
...
8. merge readinessは同一HEADに束縛されたAI-B文脈レビュー、CI receipt、DB追従receiptのANDで判定する。
   push、base更新、CI self-heal、正本digest変更後は文脈レビューとDB追従を再実行し、旧receiptを流用しない。
9. AI-AはPR作成前に内部CIをpassする。AI-Bが修正した場合はAI-Aまたは別identity/sessionの実行主体が差分レビューし、
...
12. DB/GitHub再照合不一致と性能超過はRecoveryへ送り、性能Recoveryを最優先にする。main merge後Full失敗時は
    通常mergeを停止し、Recovery修正Issue、修正PR、独立review、doctor、GitHub Actions、Issue closure receiptが
    同一修正HEADへ収束するまで解除しない。
13. VPS stagingは自動検証面、cloud productionは承認付きpromotion面とし、同一artifact、環境分離、backup、rollback、
    health check、monitoringを必須にする。操作面はGitHubへ集約しeditorへ依存しない。不可逆分類だけを理由にbranch内の安全な修正・検証を停止しない。
14. cloud providerはAWS referenceからadapter交換可能とし、stagingはproduction同一image/schema/migration/health contractを使う。
    production DB migrationは原則zero-downtime、expand→deploy→contractと完全性receiptを必須にし、停止時は追加承認を要求する。
    production自動rollback実装は将来の運用ハーネスscopeとし、今回はrollback targetと手順の検証までを扱う。
15. production有効化前にGitHub保護機能、単一deployment concurrency、self-review禁止、AWS OIDC短期credential、
    staging/production role分離をpreflightする。
16. AWS標準profileはFargate/CDK TypeScript、要件駆動DB、risk駆動account分離、90日検索＋1年監査archiveとし、
    反証要件がない選択をユーザー質問で停止しない。
17. open Update Issueはlabel/trace付きの正規backlogとして保持し、futureをactive blockerへ混入させない。
...
| 本番migration | migration段階、backup、restore rehearsal、互換期間、migration/rollback oracle、承認 | migration receipt | `HIL_PRODUCTION_MIGRATION_INCOMPLETE` |
| アップデートbacklog | update/lifecycle/priority/area label、要件/PLAN/PR trace、状態receipt | 分類receipt | `HIL_UPDATE_BACKLOG_CLASSIFICATION_INVALID` |
| Closure | merge/oracle/audit/memory/child state | closure判定receipt | `HIL_CLOSURE_EVIDENCE_MISSING` |
| Orchestration Admission | work graph、capacity route、delegation receipt、worker descriptor、security-foundation readiness（Kimi起動時） | admission receipt | `HIL_ORCHESTRATION_ADMISSION_FAILED` |
| Orchestration Acceptance | worker terminal、independent review、親検収の3 receipt、同一HEAD、identity/session/context分離、順序 | acceptance receipt | `HIL_ORCHESTRATION_ACCEPTANCE_MISSING` |
| Worker Admission | blind benchmark scorecard、重大failure単独記録、用途別admit/retire decision、S4 routing完了receipt | worker admission receipt | `HIL_WORKER_ADMISSION_FAILED` |
| Security Foundation | security policy、sandbox enforcement、credential isolation、secret deny、allowlist egress固定、readiness receipt | readiness receipt | `HIL_SECURITY_FOUNDATION_NOT_READY` |
| Capacity Routing | 最大8 slot accounting、dependency schedule、bounded queue/backpressure、quota threshold前handover、failure isolation | capacity snapshot/route receipt | `HIL_CAPACITY_ROUTING_FAILED` |
...
### §4.8 capacity routing設計（WCC-FR-14、HR-NFR-P2-01 trace）

`CapacityRouter`は最大8 parallel slotsのaccounting、dependency-aware scheduling、bounded queue/backpressure、
quota threshold前handover、failure isolationを行う（`WCC-FR-14`、`HR-NFR-P2-01`）。

#### §4.8.1 slot accounting不変条件

- 製品runtimeは最大8 parallel slotsを指標とする（`HR-NFR-P2-01`）。
- 各slotは`slot_id`（1..8）、`parent_lane_id`、`lane_id`、`task_id`、`dependency_edges`、`state`、
  `quota_snapshot`（token budget、time budget、cost budget）、`lease_token`、`started_at`、`terminal_at`を持つ。
- active slotが8を超える起動要求は`HIL_CAPACITY_OVERSUBSCRIPTION`でfail-close。
- hosted PoC環境が4 slotsに制限される場合でも、容量claim/設計は8 slot実装体を前提とし、4-slot PoC結果を
  8-slot合格として主張しない（`WCC-AC-12`）。
- 8 independent fixtureで上限8、dependency、backpressure、quota handover、failure isolationが再生可能であることを
  L9結合テストで検証する。

#### §4.8.2 依存関係を考慮したscheduling

- `WorkGraphPlanner`がtask dependency edge（`task_A completes before task_B starts`）を導出する。
- dependency未完了taskの起動要求は`HIL_CAPACITY_DEPENDENCY_NOT_MET`で拒否（依存前倒し禁止）。
- cyclic dependencyは`WorkGraphPlanner`のgraph検証で事前拒否（`HIL_WORK_GRAPH_CYCLE`）。
- orphan task（dependency graphに属さない、またはrequesting親が不明）は`HIL_WORK_GRAPH_ORPHAN_TASK`で拒否。

#### §4.8.3 上限付きqueueとbackpressure

- active slot数が上限到達時、新規request受付をqueueへ積むか、`capacity_full`応答で呼び出し側へbackpressureを返す。
- unbounded queueは`HIL_CAPACITY_UNBOUNDED_QUEUE`でfail-close（queue depth上限を設定し、超過時は
  backpressure応答またはearliest-deadline-first優先度付きqueueへ移行）。
- queue満杯時のdefault動作はfail-closeであり、無制限queue積み上げによるmemory/disk exhaustionを許容しない。

#### §4.8.4 quota threshold前handover

- 各slotは起動時にtoken budget、time budget、cost budgetのsnapshotを記録する。
- quota残量が閾値（例: token budget残10%、time budget残5分）を下回った場合、worker側へ通知し、
  checkpointまたはhandover準備を促す。
- quota枯渇まで単独laneが継続し、handover/checkpoint機会を失った場合は`HIL_CAPACITY_QUOTA_EXHAUSTED_WITHOUT_HANDOVER`
  としてrecovery対象にする（同一laneの再起動時に前回checkpointから再開）。

#### §4.8.5 slot単位failure isolation

- 1 lane failureが独立laneを停止させる場合は`HIL_CAPACITY_FAILURE_PROPAGATION`でfail-close。
- 各slotのfailureは独立したevent/receiptとして記録し、別slotのterminal receiptへ影響しない。
- ただしdependency edgeを持つ下流taskは、上流failureを検知してskipまたは別route選択を行う
  （下流taskを無条件開始しない）。
- slot isolationのL9検証fixtureは、8 independent taskのうち1件がfailureした場合でも残り7件が正常に
  completeすることをreceiptで確認する。

### §4.9 Layer Ledger Chain（上下pair・左右V-pair）
...
| WCC-FR-13..15（worker orchestration） | OrchestrationAdmissionGate / WorkGraphPlanner / CapacityRouter / WorkerBroker / OrchestrationAcceptanceGate / OrchestrationProjectionの境界 | L3↔L10 HAT-WCC-10..12、L4↔L9 HST-HIL-041..064 |
...
8. **orchestration event/receipt物理table設計**（table名、column型、PK/FK/unique index、migration DDL、
   append-only trigger、causal order FK、許可transition CHECK constraint）。§2.4はlogical field、
   component ownership、不変条件だけを確定し、物理schemaはL5で設計する。
9. **capacity routing物理slot table設計**（`capacity_slots` table schema、slot_id範囲CHECK、
   active slot count上限enforcement、dependency edge FK、quota snapshot schema、lease/fencing token管理）。
   §4.8は不変条件だけを確定し、物理schemaはL5で設計する。
