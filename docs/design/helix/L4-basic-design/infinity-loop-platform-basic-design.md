...
本書は、L1 draftの方向性を基本設計へ早期降下し、要件の入力・出力・状態・失敗・証拠が不足している箇所を
発見するための並行設計である。Hybrid Design Document正本のL1–L12 Vモデルに従い、L1要求とL11受入、
L3要件/ACとL10総合検証のpairがfreezeするまでは本書も`draft`とし、未確定値を実装契約として
扱わない。
...
| `LayerLedgerRegistry` | Node | L1–L12 ledger type、粒度、template、authority、entry/exit contractをversion管理 | catalog/eventのみ | layer未登録、in-place version変更 |
...
| `ScopeAuthorityResolver` | Node | capability根拠をchat/L1企画/PO scope rootへ解決 | scope receiptのみ | 自己参照cycle、minimum-necessary欠落 |
...
| `SourceAuthorityCustodian` | Node | exact 2 Git candidateをproject-owned SHA-256 CASへwrite-once promotionし、retention/access/revocation/freshnessとbundle/ref/tree/edge receiptを原子確定 | trusted custody/promotion receiptのみ | project外path、digest不一致、partial promotion、self-verification、freshness切れ |
| `ProjectGitCasPort` | Node port | `.helix/cas/git-authority/sha256/<prefix>/<digest>.bundle`へのproject-root限定CAS I/Oとaccess receipt | immutable CAS blobのみ | absolute/parent/symlink escape、overwrite、network/external store write |
| `OfflineSourceRestoreVerifier` | Node | trusted CASだけからnetwork 0でmanifest、bundle/ref/tree/edge replay、isolated restore drillを検証 | offline/restore receiptのみ | trusted receipt欠落、network access、digest/edge不一致、restore root escape |
...
| `RedesignRouter` | Node | findingのaffected layerを判定しRedesign/re-freezeへ戻す | redesign route event | L1/L2 stale化欠落、L1企画の無承認変更、Forward直行 |
...
| `RepositorySavepointService` | Node | pushed commit、remote annotated tag、harness.db checkpoint、全artifact digestをimmutable receiptへ結合しrestoreを検証 | savepoint/tag/restore receipt | dirty/unpushed、lightweight/local-only、tag移動・再利用、projection不一致 |
| `LayerFreezeTagGate` | Node | L1–L12 layer tagの祖先鎖、layer/pair gate、version supersession、工程表projectionを検証 | layer tag/pair/progress receipt | layer skip、非祖先、片肺pair、旧SHA、Sprint/release混同 |
...
| `UniversalWorkflowPackageRegistry` | Node | Universal Workflow package/schema/versionの検証済み登録 | registry eventのみ | schema矛盾、provenance/digest欠落、未検証active化 |
| `WorkflowInterviewCoordinator` | HARNESS subagent + Node gate | workflow欠落を質問し回答/保留/拒否をtyped disposition化 | proposal eventのみ | 無回答の推測補完、原文消失、AI単独終端 |
| `WorkflowModelNormalizer` | Node | state×trigger×condition→action→nextをcanonical workflowへ正規化 | workflow proposal/event | 到達不能状態、未定義遷移、非決定的分岐 |
| `WorkflowCompletenessGate` | Node / GateEngine facade | 遷移、異常系、resource、fallback、dead-letterの閉包を判定 | completeness receiptのみ | unresolved/deferred/orphanが1件以上 |
| `WorkflowRequirementDeriver` | HARNESS subagent | workflow遷移を要求・AC・test oracle候補へ双方向導出 | proposal eventのみ | source transition欠落、集約行だけのcoverage |
| `RuntimeOrchestrationPlanner` | Node | routing、schedule、resource、retry/fallback/dead-letterを計画 | plan/receipt eventのみ | resource上限による必須処理省略、route未定義 |
| `CanonicalActionIntentNormalizer` | Node | tool固有操作をactor/tool/target/params/effectのcanonical intentへ変換 | intent eventのみ | target/effect不明、opaque command、digest欠落 |
| `StandingAuthorizationRegistry` | Node | scope限定の包括許可をversion・expiry・revocation付きで管理 | authorization eventのみ | blanket許可、対象外project、失効/取消未反映 |
| `ExecutionAuthorizationGate` | Node / GateEngine facade | canonical intentをstanding/one-shot/deny policyへ機械照合 | authorization receiptのみ | policy不一致、scope外、approval推測、stale snapshot |
| `PlatformPermissionBrokerAdapter` | Node adapter | OS/tool/platform permission surfaceへ検証済みintentだけを投影 | broker receiptのみ | project外操作、未登録external service、intent改変 |
| `BoundedActionExecutor` | Node | 許可済みintentをfencing/idempotency/timeout付きで実行 | execution receiptのみ | approval scope超過、無制限副作用、receipt欠落 |
| `RequirementFreezeGate` | Node / GateEngine facade | 162要件のsource authority、applicability、設計義務、ACをjoin | freeze receiptのみ | trace-only、unresolved、stale、未承認が1件以上 |
...
- Hybrid ZIPのPython engine/toolはbulk portせず、behaviorとCLI/library contractを保全して再利用する。tool単位で
  `reuse-as-is/reuse-with-adapter/harden/redesign/reject`を決定し、Pythonであること自体をreject理由にしない。
- 現inventoryではPython module 29本中、静的に確認できたSQLite直接依存0、network依存1、filesystem mutation候補15以上、
  subprocess依存4、`__main__` guard 22、subcommand 19、CLI argument 44、leaf route上限40である。この値はbehavior採否率ではなく静的な隔離設計入力であり、
  229 callable anchorのexact routeと29 moduleのdynamic traceが閉じるまで最終分母としない。filesystem mutationは隔離result
  directory＋relative manifestへ、networkはconnector registryと既定denyへ、subprocessはallowlisted executable/argvへ封じる。
...
intake
  ...
  -> redesign? -> design_refactor? -> l3_acceptance_freeze
  -> l4_basic_design_freeze
  -> l5_test_design_freeze
  -> l7_red_test
  -> implementation_preflight
  -> l6_green_implementation
  -> l7_refactor_trace_closure
  -> local_prejoin_ci
  -> forward_join
...
3. design findingはL1/L2を含むaffected layerへRedesignを作り、L1企画・価値判断の変更はPOへescalateする。
...
8. production Scrumは外周iterationであり、各vertical sliceは`L1→L12`の圧縮Vを完走する。Discovery/PoCの
   `S0→S4`はproduction sliceの代替にしない。
9. `L3 AC freeze → L5 test design → L7 Red → L6 Green → L7 refactor/trace closure → L8–L12 verification`
   の順序を入れ替えず、実装済みコードから受入条件を後付けしない。

### §3.1 Infinity coreの永続state／transition正本

上記state図は説明用projectionであり、次の物理table群を永続正本とする。状態名をfree-form文字列で追加せず、
`infinity_transition_rules`に未登録の遷移、gate receiptなしの遷移、同じoperation IDへの異payloadをwriterで拒否する。
監査、改善、gate、自動走行は一つのstatusへ潰さず、同じ`causality_id`を持つ独立aggregateとしてjoinする。

| table | PK | relation / unique | 必須field | status / 不変条件 |
|---|---|---|---|---|
| `infinity_issues` | `infinity_issue_id` | external Issue＋revision、causality IDは一意 | contract/scope/oracle/mode digest、current state、parent Issue、created/updated | stateはrule registry由来、projection再構築可能 |
| `infinity_issue_events` | `infinity_issue_event_id` | Issue＋event sequence、operation IDは一意 | from/to state、trigger、actor、previous/event digest、occurred_at | append-only |
| `infinity_transition_rules` | `transition_rule_id` | from＋trigger＋condition version＋toは一意 | required gate/receipt type、authority、failure code、effective/supersedes | `draft`, `active`, `deprecated`, `revoked` |
| `issue_gate_runs` | `issue_gate_run_id` | Issue＋gate＋subject revision＋attemptは一意 | input/required evidence/policy digest、producer/verifier、verdict/failure | `queued`, `running`, `passed`, `failed`, `inconclusive`, `stale` |
| `issue_gate_receipts` | `issue_gate_receipt_id` | gate runを一意参照 | Issue/state/subject/policy/evidence/output digest、verifier、issued/expires | `valid`, `stale`, `revoked` |
| `reverse_runs` | `reverse_run_id` | Issue＋input revision＋attemptは一意 | obligation/input/snapshot digest、current phase、route、failure | `created`, `r0`, `r1`, `r2`, `r3`, `r4`, `completed`, `incomplete`, `failed`, `stale` |
| `reverse_stage_results` | `reverse_stage_result_id` | run＋R0..R4は一意 | source spans、semantic assertions、input/output/obligation digest、unresolved count | stage skipと同文・同digestを拒否 |
| `reverse_receipts` | `reverse_receipt_id` | completed runを一意参照 | R0..R4 result set、substance、route、pair/evidence digest、verifier | `valid`, `stale`, `revoked` |
| `redesign_routes` | `redesign_route_id` | finding＋affected layer＋revisionは一意 | authority、design delta、stale consumer/pair set、re-freeze target、PO escalation | `proposed`, `accepted`, `executing`, `refrozen`, `rejected`, `stale` |
| `redesign_events` | `redesign_event_id` | route＋event sequence、operation IDは一意 | from/to、previous/event digest、actor、occurred_at | append-only |
| `audit_runs` | `audit_run_id` | PR/head/policy/reviewer attemptは一意 | scope/head/input/policy digest、provider/model family、verdict/failure | `queued`, `running`, `passed`, `failed`, `inconclusive`, `stale` |
| `audit_findings` | `audit_finding_id` | run＋fingerprintは一意 | subject/location/severity/evidence、disposition/authority、promotion state | findingをproseだけで終端化しない |
| `improvement_candidates` | `improvement_candidate_id` | finding/recipe fingerprint＋revisionは一意 | source finding、pattern/fixture/shadow metric、target kind、rollback | `observed`, `qualified`, `shadowing`, `promotable`, `promoted`, `rejected`, `stale` |
| `improvement_events` | `improvement_event_id` | candidate＋event sequence、operation IDは一意 | from/to、measurement/coverage delta、review、previous/event digest | append-only |
| `autorun_claims` | `autorun_claim_id` | Issue＋task revision＋attemptは一意 | Reverse/Redesign/gate/lease/worker/verifier digest、target state、fence | `proposed`, `ready`, `leased`, `running`, `completed`, `failed`, `cancelled`, `stale` |
| `autorun_events` | `autorun_event_id` | claim＋event sequence、operation IDは一意 | from/to、fence、result/failure、previous/event digest | append-only |
| `memory_compaction_runs` | `memory_compaction_run_id` | completion packet＋policy revisionは一意 | raw input digest、knowledge/continuation output digest、redaction、producer/verifier | `queued`, `running`, `passed`, `failed`, `stale` |
| `memory_promotion_receipts` | `memory_promotion_receipt_id` | compaction run＋knowledge itemは一意 | durable knowledge digest、excluded raw/progress set、review/rollback | `valid`, `stale`, `revoked` |
| `finding_promotion_transactions` | `finding_promotion_id` | finding＋target revisionは一意 | child Issue、Reverse seed、memory summary、Codex queue、transaction digest | 全target commitまたはrollback |
| `infinity_closure_receipts` | `infinity_closure_receipt_id` | Issue＋closure revisionは一意 | PR/CI/audit/oracle/memory/child/pair digest、unresolved count、verifier | `passed`, `failed`, `stale`, `revoked` |

canonical query/viewは次の5件とし、行数だけの完了判定を禁止する。

| query/view | 必須join | fail-close条件 |
|---|---|---|
| `infinity_ready_issue_query` | current Issue＋valid gate＋Reverse＋Redesign disposition＋lease | receipt missing/stale、Reverse未完、worker=verifier |
| `infinity_transition_legality_query` | current state＋active transition rule＋trigger＋receipt set | ruleなし、条件不一致、期限切れreceipt |
| `infinity_cycle_orphan_query` | audit→finding→promotion→child Issue→gate→autorun→PR/audit | 任意edge欠落、causality不一致、prose-only終端 |
| `infinity_closure_readiness_query` | PR＋三段CI＋audit＋oracle＋memory＋child Issue＋pair | unresolved/staleが1件以上 |
| `improvement_promotion_eligibility_query` | candidate＋fixture＋shadow metric＋独立review＋coverage delta＋rollback | self-promotion、実測なし、coverage regression |

receipt type分母は`directive_custody`、`admission`、`reverse`、`redesign`、`issue_gate`、`autorun`、
`audit`、`improvement`、`memory_promotion`、`closure`の10件である。`not-triggered`はRedesignだけに許可し、
findingとaffected-layer評価を持つtyped receiptを要求する。Reverse、Issue Gate、Memoryは`none/exempt`を許可しない。

### §3.2 3段CIとquarantineの永続化
...
### §3.3 Repository savepoint / Layer Freeze Tagの物理契約

Repository savepointとlayer freeze tagは、次の15 tableを固定分母としてprojectionする。

| table | 主key | relation / unique | 必須field | status |
|---|---|---|---|---|
| `repository_savepoints` | `savepoint_id` | repository＋namespace＋version一意、commit/checkpointを参照 | commit/tree、remote tag、checkpoint、artifact/authority/CI/review digest、created actor/time | `proposed`, `remote_verified`, `restore_verified`, `stale`, `superseded`, `revoked` |
| `repository_savepoint_artifacts` | `savepoint_artifact_id` | savepoint＋kind＋subject一意 | artifact kind、subject ID/revision、digest、custody receipt | `bound`, `missing`, `stale`, `rejected` |
| `repository_restore_runs` | `restore_run_id` | savepoint＋attempt一意 | recovery branch、checkout SHA、projection rebuild digest、artifact verification、failure | `queued`, `running`, `passed`, `failed`, `cancelled` |
| `layer_freeze_tags` | `layer_tag_id` | authority model＋layer＋version一意、savepointを参照 | tag namespace/name、commit SHA、layer gate/pair receipt、predecessor tag、supersedes、applicable evidence eligibility receipt set digest | `proposed`, `remote_verified`, `current`, `stale`, `superseded`, `revoked` |
| `layer_tag_ancestry_receipts` | `ancestry_receipt_id` | child tag一件につきcurrent receipt一意 | parent/child commit、merge-base、is-ancestor、repository/ref-set digest | `passed`, `failed`, `stale` |
| `layer_pair_tag_receipts` | `pair_tag_receipt_id` | authority model＋pair＋revision一意 | left/right tag、oracle/snapshot digest、unresolved count、verdict | `passed`, `failed`, `stale` |
| `layer_progress_projections` | authority model＋layer | current layer tag一意 | planned/in-progress percentage、freeze state、tag/savepoint/gate digest、updated event | `not_started`, `in_progress`, `freeze_ready`, `frozen`, `stale`, `blocked` |
| `tag_namespace_policies` | `tag_namespace_policy_id` | repository＋purpose＋version一意 | ref grammar、slug rule、authority、ruleset expectation、effective/supersedes | `draft`, `active`, `deprecated` |
| `github_tag_ruleset_snapshots` | `ruleset_snapshot_id` | repository＋ruleset ID＋observed revision一意 | provider、ruleset target/pattern/enforcement、bypass actors、observed_at、payload digest | `observed`, `verified`, `stale`, `missing` |
| `remote_tag_observations` | `remote_tag_observation_id` | repository＋full ref＋remote object ID＋observation sequence一意 | remote ref OID、tag object OID、peeled commit、annotation digest、ruleset snapshot、observed_at | `observed`, `matched`, `mismatched`, `stale` |
| `repository_savepoint_events` | `savepoint_event_id` | savepoint＋event sequence、operation ID一意 | from/to、payload/previous/event digest、expected remote OID、actor/time | append-only |
| `repository_restore_checks` | `restore_check_id` | restore run＋check kind一意 | expected/actual digest、branch/worktree、command evidence、failure | `passed`, `failed`, `inconclusive` |
| `layer_tag_events` | `layer_tag_event_id` | layer tag＋event sequence、operation ID一意 | from/to、payload/previous/event digest、expected remote OID、actor/time | append-only |
| `layer_vpair_tag_bindings` | `layer_vpair_binding_id` | authority model＋pair ID＋revision一意 | left/right layer/tag、gate SHA、snapshot/oracle digest、pair receipt | `proposed`, `current`, `stale`, `superseded` |
| `layer_progress_events` | `layer_progress_event_id` | authority model＋event sequence、operation ID一意 | affected layer range、reason、old/new chain digest、previous/event digest | append-only |

namespaceは次の正規形だけを許可する。各可変segmentはlowercase slug
`[a-z0-9](?:[a-z0-9._-]{0,62}[a-z0-9])?`、versionは`v[1-9][0-9]*`、layerは
`L(0[1-9]|1[0-2])`とし、`..`、`@{`、`.lock`終端、空segment、control文字、percent encoding、先頭/末尾の
`.`または`/`を拒否する。

| purpose | canonical full ref | （日本語の機械契約記述）
|---|---|
| savepoint | `refs/tags/helix/savepoint/<repository_id>/<savepoint_id>/v<n>` |
| layer freeze | `refs/tags/helix/layer/<authority_model_id>/L<nn>/<scope_id>/v<n>` |
| restore dry-run branch | `refs/heads/helix/restore/<repository_id>/<savepoint_id>/<operation_id>` | （日本語の機械契約記述）

remote tag検証はrefがcommitへ直結するlightweight tagを拒否し、remote ref OIDがannotated tag objectを指すこと、tag objectを
peelしたcommitがexpected commitと一致すること、annotation canonical payload digestがsavepoint/layer bindingと一致することを
fetch後に照合する。作成はexpected-absentのcompare-and-create、既存ref検証はexpected-old OIDとのcompare-and-observeとし、
force update/delete/recreateを許可しない。GitHub ruleset/tag protectionはprovider側の強制authorityであるが、設定したという
設計記述だけを証拠にしない。current `github_tag_ruleset_snapshots`、remote observation、bypass actor集合、照合receiptが揃うまで
`remote_verified`へ進めない。ruleset未観測時は実強制を主張せずfail-closeする。

同じoperation ID＋payload digestの再送は既存結果を返し、異payloadは`HIL_SAVEPOINT_IDEMPOTENCY_CONFLICT`または
`HIL_LAYER_TAG_IDEMPOTENCY_CONFLICT`で拒否する。writerはrepository/namespace/version単位の一意制約とtransaction fenceを取り、
remote競合時はrefetchしてexpected OIDを再照合する。別writerが先にrefを作成した場合はforceせず
`HIL_REMOTE_TAG_CONCURRENT_REF_RACE`で終端し、local DBだけを成功へ進めない。

layer chainは同一repository、authority model、scope、freeze epochの`L01→L02→…→L12`に固定する。通常のrefreezeでは
L02以降は直前layerのcurrent tagをpredecessorに持つ。初回bootstrapだけは、同一freeze epoch内の直前tagが
`pending_pair`で、ancestry/gate/roadmap digestがcurrentかつstaleでない場合にpredecessorとして許可する。
`merge-base --is-ancestor <parent-commit> <child-commit>`とref-set digestをreceiptへ保存し、layer skip、別scope/epoch、
非祖先、stale predecessorを拒否する。V-pairは`L01↔L12`、`L02↔L11`、`L03↔L10`、`L04↔L09`、
`L05↔L08`、`L06↔L07`の6件であり、両側`pending_pair`から同一transactionでcurrentへ閉じる場合、または既にcurrentの
pairを再検証する場合だけ、同一gate SHA、同一snapshot/oracle digestを要求する。pair receipt発行前に両側currentを要求しない。

`freeze_progress`はL01から連続する最長current chainの層数/12、current chain digest、freeze denominator digestからだけ再投影する。
`live_progress`、`in_progress`、`last_frozen_chain`を分子へ使わない。Redesignはcanonical V-pair counterpart setを先に算出し、
affected layerからL12までと全対向tag、`layer_vpair_tag_bindings`、pair receipt、progress projectionを一つのCAS transaction/event chainで
`stale`へ遷移する。たとえばL08変更時は下流L08–L12だけでなく対向L05とそのpair creditも同時に失効する。
旧tag objectを保持したまま新versionが成立した時だけ旧versionを`superseded`にする。片側currentの中間状態、旧pair credit、旧gate SHA、
非連続chainをpercentageへ算入せず、commit outcome不明は同じoperation/payload/expected headsからexact reconcileする。restore dry-runは既存branchを更新せず上記grammarの隔離branch/worktreeを新規作成し、
commit/tree、checkpoint、artifact CAS、requirement/design/test/gate digest、authority/CI/review receipt、projection再構築結果を照合する。
全checkがpassするまで`restore_verified`を発行しない。

canonical indexは次の10件を固定分母とする: `uq_savepoint_namespace_version`、
`uq_savepoint_operation_id`、`ix_savepoint_commit_checkpoint`、`uq_remote_tag_observation_sequence`、
`ix_remote_tag_current_ref`、`uq_layer_authority_layer_version`、`uq_layer_tag_operation_id`、
`ix_layer_predecessor`、`uq_layer_vpair_revision`、`ix_layer_progress_current_chain`。
canonical query/viewは`repository_savepoint_create_readiness_query`、`repository_savepoint_remote_integrity_query`、
`repository_restore_verification_query`、`layer_tag_create_readiness_query`、`layer_freeze_chain_continuity_query`、
`layer_progress_projection_query`の6件である。receipt typeは`savepoint_create_receipt`、
`remote_tag_verification_receipt`、`github_tag_ruleset_authority_receipt`、`restore_dry_run_receipt`、
`layer_tag_receipt`、`layer_ancestry_receipt`、`layer_vpair_receipt`、`layer_progress_projection_receipt`の8件である。

failure taxonomyは上位acceptance code `HIL_REPOSITORY_SAVEPOINT_INVALID` / `HIL_LAYER_FREEZE_TAG_INVALID`を維持し、
原因を`HIL_SAVEPOINT_NAMESPACE_INVALID`、`HIL_SAVEPOINT_TAG_NOT_ANNOTATED`、`HIL_SAVEPOINT_REMOTE_REF_MISMATCH`、
`HIL_SAVEPOINT_RULESET_AUTHORITY_MISSING`、`HIL_SAVEPOINT_BINDING_DIGEST_MISMATCH`、
`HIL_SAVEPOINT_IDEMPOTENCY_CONFLICT`、`HIL_REMOTE_TAG_CONCURRENT_REF_RACE`、`HIL_SAVEPOINT_RESTORE_MISMATCH`、
`HIL_LAYER_TAG_NAMESPACE_INVALID`、`HIL_LAYER_TAG_ANCESTRY_INVALID`、`HIL_LAYER_TAG_SKIP`、
`HIL_LAYER_TAG_VPAIR_ONE_SIDED`、`HIL_LAYER_TAG_GATE_SHA_MISMATCH`、`HIL_LAYER_TAG_SUPERSESSION_INVALID`、
`HIL_LAYER_PROGRESS_PROJECTION_INVALID`、`HIL_LAYER_TAG_IDEMPOTENCY_CONFLICT`へ細分する。

tagは進捗値をauthoredしない。percentageは工程task/evidence denominatorから導出し、tagは`frozen`遷移の必要条件に限定する。
同名tagのforce update/delete/recreateを許可せず、redesignは既存tagを移動せず新versionを作り、affected layer以降をstale化する。
Sprint tag、layer tag、savepoint tag、release tagはnamespaceとauthorityを分離する。

#### §3.3.1 独立監査是正protocol

V-pair bootstrapは二段階とする。左腕L01–L06は同一freeze epochの直前predecessorが`pending_pair`またはcurrentで、ancestry/gate/roadmap digestがcurrentならannotated tagを`pending_pair`でremote作成できるが、`frozen`、percentage、pair PASSへ算入しない。次にL07をL06の`pending_pair` tagの子孫として作成し、`L06↔L07`を同一transactionでcurrent化する。その後はL08→L12を順に作成し、`L05↔L08`から`L01↔L12`までを内側から外側へcurrent化する。各対向層の確定時に両tag、同一freeze epoch/gate SHA、snapshot/oracle digestを`layer_vpair_tag_bindings`へCAS commitし、current pair receipt発行と同じtransactionで双方を`current`へ遷移する。対向欠落・stale・異revisionは`HIL_LAYER_TAG_VPAIR_ONE_SIDED`で閉じ、循環を避けるためpair receiptまたはpredecessor currentを左tag作成前提にしない。

remote compare-and-create成功後にDB commitがcrashした場合は`remote_created_projection_pending`として再観測する。operation ID、payload digest、tag object OID、peeled commit、annotation digestが全一致した時だけ同一transactionでevent/projection/receiptをadoptする。異digest、未知operation、ruleset staleはquarantineし、`HIL_SAVEPOINT_REMOTE_DB_RECONCILIATION_REQUIRED`を返す。force/delete/recreateで補正しない。

工程表bindingはroadmap registry ID、program ID、slice ID、task-set digest、evidence denominator digest、ledger revisionを必須とする。`live_progress`、`in_progress`、`freeze_progress`、`last_frozen_chain`、`stale_from_layer`を別field/queryで返す。`freeze_progress`はcurrent chain count/12、chain digest、freeze denominator digestを持ち、refreeze中のlive完了、pending tag、stale chainを加算しない。

stale cascadeはcanonical V-pair counterpart setを含むexpected affected setをtransaction前に固定する。affected→L12、全対向tag、binding、pair receipt、progress projectionのexpected headsをCASし、全件成功または増分0とする。fault後のreconcileでも片側currentの時間窓を作らず、旧pair/freeze creditを0へ落とす。

working-tree closed worldはHEAD/tree/index、tracked modified、staged、untracked、ignored、submodule、sparse-checkoutを個別観測する。savepoint bytesはpushed HEAD/treeだけから導出し、index一致とtracked cleanを要求し、untracked/ignoredは明示policy digestをreceipt化して入力から除外する。restoreは隔離worktree外を変更しないことをbefore/after digestで証明する。

GitHub observationはREST/GraphQL tag visibility、pagination完走、eventual-consistency retry、read permission、ruleset target match、bypass actor allowlistとexpiryを記録する。未可視、部分page、期限切れ/未知bypassは`HIL_GITHUB_TAG_VISIBILITY_INVALID`または`HIL_SAVEPOINT_RULESET_AUTHORITY_MISSING`でfail-closeする。

L8–L10のvisual scopeがapplicableなlayer tagとV-pair receiptは、`visual_pair_receipts`のcurrent eligibilityをexact joinする。
固定Design case setはL8=6、L9=7、L10=8とし、layer、case ID exact set、case-set revision/digest、matrix denominator、
execution receipt set/digest、independent review receipt、visual authority epoch、eligible count、ineligible reason setを
`visual_freeze_eligibility_receipt`へ結合する。`eligible count = exact case denominator`かつfailure/quarantine/deferred/reused/staleが
0でなければtagを`current`またはprogressを`frozen`へ遷移させない。designed case、N/A、未実行、quarantine、別matrix cellからの
artifact再利用、`pending_pair`、tagの存在、工程task完了はeligible分子を増やさない。

`layer_progress_projection_query`はapplicable evidence eligibility receiptをleft joinして、receipt欠落・revision不一致・authority stale・
execution cardinality不足のlayerを`blocked`、freeze numerator 0として返す。`live_progress`は工程表由来のまま保持するが、
visual eligibilityを満たさないlive完了を`freeze_progress`、`last_frozen_chain`、V-pair PASSへ転記しない。
...
|---|---|---|---|
| L1 | 企画、価値仮説、scope、non-goal | L2要求探索、L3 authority | L12運用/価値検証 |
| L2 | 要求、画面prototype、interaction/stateまたは明示no-UI | L3要件候補、画面要求 | L11受入 |
| L3 | system要件、FR、AC | L4基本設計義務、L5 test design input | L10総合検証 |
| L4 | architecture、service、domain、external/data契約 | L5 module/API/DB contract | L9結合検証 |
| L5 | 詳細設計とtest design、module、IF、edge case | L7 Red oracle、L6実装契約 | L8単体検証 |
| L6 | product code、migration、runtime artifact | L7 Green/refactor input | L7 test implementation/TDD closure | （日本語の機械契約記述）
| L7 | Red test、test implementation、refactor、trace closure | L8–L12 verification input | L6実装 |
| L8 | 単体test run/assertion。visual scopeではcomponent/region×state/theme/viewportの詳細visual contract検証 | L9 verification input、L5 backprop | L5詳細設計/test design |
| L9 | 結合test run/assertion。visual scopeでは画面横断pattern/profile/token/layout/breakpoint/overlay/motion continuity | L10 input、L4 backprop | L4基本設計 |
| L10 | 総合test run/assertion。visual scopeではbrowser/OS/viewport/data/preference matrixによるvisual system検証 | L11 input、L3 backprop | L3要件/AC |
| L11 | 受入判定。visual scopeでは同一revisionのPrototype Agreement/visual intentと証拠packetに対する人間の視覚・体験・preference判断 | L12 input、L2 backprop | L2要求/画面prototype |
| L12 | 運用・価値・継続判断の証拠 | L1 feedback | L1企画 |

正規左右pairは`L1↔L12`、`L2↔L11`、`L3↔L10`、`L4↔L9`、`L5↔L8`、`L6↔L7`の6組とする。
画面を持たないscopeでもL2を消さず、PO根拠と再entry条件を持つ`not_applicable` ledger rowで明示的に通過する。

### §4.9 Visual Design HARNESS verification spine （日本語の契約見出し）

本節のpair配置は`HELIX-L12-V-TARGET-v1`（receipt `VMAUTH-2026-07-16-01` pending）である。現行runtime互換の
L0–L14 pair/G10とはauthority modelを混在させず、receipt activation前のtarget evidenceをactive gateへ算入しない。

Design HARNESSのownershipは`visual intent → visual contract → rendered visual evidence → human visual acceptance`に限定する。
一般architecture、DesignRefactor、Requirement Translator、functional workflow、a11y semantics全体を所有しない。a11yはcontrast、
focus visibility、zoom/reflow、motion preference等のvisual交差面だけを直接検証し、ARIA、keyboard task success等は別ownerの
receiptを参照する。

| layer/pair | visual verification責務 | atomic denominator | gate |
|---|---|---|---|
| L5↔L8 | Detailed Visual Contract ↔ Visual Contract Verification | component/region×state×theme×viewport、token/asset provenance、computed style、geometry、contrast、focus、baseline | `G-VIS-L8 VisualDetailContractGate` | （日本語の機械契約記述）
| L4↔L9 | Cross-screen Visual System/Pattern ↔ Visual Integration | screen pair/flow×breakpoint、shared pattern、hierarchy/density、token/asset continuity、overlay/z-index、motion continuity | `G-VIS-L9 CrossScreenVisualIntegrationGate` | （日本語の機械契約記述）
| L3↔L10 | Visual FR/NFR/AC ↔ Browser/Data Visual System Verification | browser engine×OS profile×viewport/DPR×theme/preferences×representative product-data scenario | `G-VIS-L10 BrowserDataVisualSystemGate` | （日本語の機械契約記述）
| L2↔L11 | Prototype Agreement/Visual Intent ↔ Human Visual Acceptance | agreement criterion×current evidence packet×human actor/verdict | `G-VIS-L11 VisualAcceptancePreferenceGate` | （日本語の機械契約記述）

L7はL6実装とのTDD closureとしてscreen/component missionごとのrender/state/semantic oracleを実行し、L8以降へcurrent evidenceを渡す。
L8はatomic visual contract、L9はcomposition continuity、L10はenvironment/data robustness、L11はtaste/intent acceptanceであり、
`Integration/System/Real UX`の一括表現をcoverage分母に使わない。L8–L10は機械自走し、L11のsubjective preferenceだけを人間authorityとする。

各matrix cellにsilent `skipped`を許可しない。N/Aはreason、authority、scope digest、re-entryを必須とする。screenshot 1枚を複数cellの
合格へ水増しせず、artifact→obligation/matrix cell edgeを行単位で持つ。baseline更新はtest failure解消操作ではなく、current
Prototype Agreement revision、人間authority、visual diff reviewを要求し、同run producerによるbaseline更新とPASSを禁止する。

| table | 主key | 必須field・不変条件 |
|---|---|---|
| `visual_contract_versions` | contract ID＋revision | layer、scope、prototype/profile/binding digest、authority、status | （日本語の機械契約記述）
| `visual_obligations` | obligation ID＋revision | contract、subject/state/theme/viewport、oracle、applicability、source span | （日本語の機械契約記述）
| `visual_baseline_versions` | baseline ID＋revision | agreement revision、artifact digest、human review、supersedes、status | （日本語の機械契約記述）
| `visual_verification_runs` | run ID | layer/gate、matrix manifest、producer/verifier、snapshot/config digest、status | （日本語の機械契約記述）
| `visual_matrix_cells` | run＋cell ID | browser/OS/viewport/DPR/theme/preference/data scenario、applicability、verdict | （日本語の機械契約記述）
| `visual_render_observations` | observation ID | cell、subject、computed style/geometry/render digest、measurement version | （日本語の機械契約記述）
| `visual_assertion_results` | assertion ID＋run | obligation、oracle、expected/actual、failure code、evidence digest | （日本語の機械契約記述）
| `visual_cas_artifacts` | artifact digest | sha256、byte length、media type、kind、producer run、classification、redaction/scan/retention receipt | （日本語の機械契約記述）
| `visual_artifact_edges` | edge ID | artifact、run、cell、obligation、assertion、purpose。CAS identityと利用関係を分離 |
| `visual_evidence_packets` | packet ID＋revision | layer/gate、contract/source/run、matrix manifest、derived verdict、security、shard manifest | （日本語の機械契約記述）
| `visual_acceptance_sessions` | session ID | L2 agreement/L10 evidence revision、human actor、criteria、status | （日本語の機械契約記述）
| `visual_judgments` | judgment ID | session/criterion、accept/reject/preference/dissent、reason、authority | （日本語の機械契約記述）
| `visual_pair_receipts` | pair＋scope＋revision | design/verification snapshot、denominator、review/verdict、stale reason | （日本語の機械契約記述）
| `visual_authority_models` | model ID＋version | purpose、pair-set digest、receipt/status、activation/supersession | （日本語の機械契約記述）

Rebaseline `ux-evidence.schema.json` v1はmigration inputおよびnegative fixtureであり、active authorityへ採用しない。
`evidencePaths`、自由文finding/back-propagation、authored aggregate result、全layer必須の`humanReview`を拒否する。v2は
packet→run→matrix cell→assertion→artifact edge→CASをtyped分離し、overall verdictをNodeがrequired cell/assertionから導出する。
L11 human judgmentは独立session/tableに置き、L8–L10 greenへ人間判定を混入させない。source YAMLにのみ存在した
`commands`/`approved_by`とJSON v1のschema driftも移行receiptなしでは許可しない。raw commandは保存・再実行せず、tool/config digest、
exit/status、output digestへ変換する。

matrix manifests、runs、packets、acceptance sessions、pair receiptsは`authority_model_id`を必須とする。active queryは
`receipt_status=activated`のmodel一件だけを対象にし、pending targetは`proposal/designed_not_executed`以外へ遷移できない。
同一scopeのevidenceをauthority model間で合算せず、activation時はatomic migration receiptで旧modelをsuperseded/stale化する。
違反は`HIL_VIS_AUTHORITY_MODEL_PENDING/MIXED/PAIR_NOT_AUTHORIZED/TARGET_EVIDENCE_COUNTED_ACTIVE/
AUTHORITY_CUTOVER_RECEIPT_MISSING`で拒否する。

matrixの非選択は`critical_full/pairwise_selected/boundary_selected/negative_fixture`のalgorithm/version/config/selection receiptを要求する。
`deferred`、`quarantined`、`not_executed`、`stale`はgreenへ算入しない。baselineは
`proposed → independent_diff_review → human_approved → active → superseded/revoked`だけを許可し、producer自己承認、failed run内の
更新後PASS、prototype/contract/font/browser/tool revision不一致での再利用をfail-closeする。artifactはabsolute path、credential URI、
digest不一致、未redact PII/secret、期限切れretentionを拒否する。

代表failure codeはL8=`HIL_VIS_L8_CONTRACT_MISSING/STATE_COVERAGE_INCOMPLETE/BASELINE_UNAUTHORIZED/RENDER_NONDETERMINISTIC`、
L9=`HIL_VIS_L9_PATTERN_DRIFT/BREAKPOINT_DISCONTINUITY/OVERLAY_COLLISION/MOTION_CONTRACT_DRIFT`、
L10=`HIL_VIS_L10_BROWSER_MATRIX_INCOMPLETE/DATA_SCENARIO_SYNTHETIC_ONLY/PRODUCT_SNAPSHOT_STALE/EVIDENCE_FAMILY_INCOMPLETE`、
L11=`HIL_VIS_L11_HUMAN_RECEIPT_MISSING/SELF_APPROVAL/PROTOTYPE_REVISION_MISMATCH/EVIDENCE_STALE/REJECTION_NOT_ROUTED`、
cross-layer artifact security=`HIL_VIS_ARTIFACT_SECURITY_INVALID`とする。
...
次表はconceptual entity inventoryであり、追加の物理table authorityではない。物理table名、PK/FK、status、event chainの正本は
§6.1だけとする。特に`detector_registry`を`detector_versions`と並行実装せず、registry概念は
`detector_versions`のactive projectionとして提供する。重複名の別table、互換viewによる二重write、Python直結を禁止する。

| conceptual entity | 主key | 必須field | 不変条件 |
|---|---|---|---|
...
### §6.2 source anchor routingとUniversal Workflow projection

sealed source artifactを正本とし、DBは抽出・採否・traceの再構築可能なprojectionとする。Hybrid Python 229 callableと
Universal 328 primary anchorは同じrouting contractを使うが、source固有metadataを別tableで保持する。

| table | 主key / 一意制約 | 必須field | 不変条件 |
|---|---|---|---|
| `source_candidate_anchors` | anchor ID、entry＋extractor version＋structural digestは一意 | entry/blob digest、kind、span/pointer、qualified name、extractor/version | sealed entryに存在しないspan禁止 |
| `source_anchor_routes` | route ID、anchor＋revisionのcurrentは最大1 | primary route、target、reason、authority/evidence digest、confidence、unresolved reason | primary route exactly one |
| `source_behavior_atoms` | atom ID＋revision | semantic IR、source spans、decision、requirement/design/assertion/gate edges | aggregate/containerだけのatom禁止 |
| `source_anchor_relations` | from＋to＋kind＋revision | relation kind、evidence digest | cycle policy違反、dangling target禁止 |
| `git_custody_candidates` | repository＋candidate digest | source receipt、bundle/ref/tree/edge expected digest、policy digest | candidateをtrusted/currentに直接算入禁止 |
| `git_trusted_cas_objects` | SHA-256 digest | project-relative CAS URI、bytes、immutability/retention/access revision | overwrite、project-root escape、external store URI禁止 |
| `git_authority_receipts` | repository＋revision | bundle/ref/tree/edge、policy、verifier、fresh-until、promotion receipt | exact receipt setと同一digestだけcurrent |
| `git_offline_manifests` | repository＋authority revision | CAS URI/digest、ref/tree/edge denominator、producer、fresh-until | network由来、untrusted input、digest欠落禁止 |
| `git_restore_drill_receipts` | repository＋authority revision＋drill revision | isolated root、bundle verify/fsck/peel/tree/edge replay、path escape count | current drill欠落でpromotion禁止 |
| `universal_packages` | package ID＋version | archive/manifest/descriptor/schema digest、custody status | raw SKILL/promptをactive entrypointにしない |
| `universal_source_entries` | package＋entry path | blob digest、role、size、classification | 14/14 custodyなしのactive化禁止 |
| `universal_questions` | question ID＋revision | family、exact value digest、source pointer、applicability | canonical 76の重複禁止 |
| `interview_question_dispositions` | scope＋question＋revision | answer/unresolved/not-applicable/rejected、actor、evidence、decided_at | AI推測回答、空N/A禁止 |
| `universal_schema_clusters` | cluster ID＋revision | source schema/pointer、canonical target、constraint digest | containerをbehaviorへ加算しない |
| `universal_contract_clauses` | clause ID＋revision | source span、canonical constraint/alias/conflict target | unowned normative prose禁止 |
| `universal_fixture_cases` | fixture ID＋digest | positive/negative、expected verdict/failure code、producer/consumer | negative fixtureをpositive oracleにしない |
| `universal_conflicts` | conflict ID＋revision | competing spans、decision、replacement、assertion/gate、receipt | current unresolvedをfreezeしない |
| `universal_alias_edges` | source anchor＋canonical target＋revision | migration direction、expiry、reason | output alias、無期限alias禁止 |
| `universal_family_receipts` | family＋authority revision | anchor partition、atom/trace digest、review、verdict | target countだけでverified禁止 |

Universalのpost-resolution target hypothesisは`behavior anchor 204 + alias 90 + schema container 21 + fixture 13 + conflict 0 = 328`とする。
現在はverified 0/328、major conflict 5であり、これはrouting仮説かつatom分母ではない。primary 328外のsupport/unowned spanは
別inventoryでterminal ownershipとorphan 0を証明する。Hybrid Pythonも
`86 behavior + 5 container + 16 exposure-routed callable + 18 fixture + 104 support = 229`
をstatic proposalとし、dynamic call/effect/failure trace後だけverifiedへ昇格する。machine routeは
...
を持ち、routeとfamilyを別軸にして二重算入を防ぐ。

Universalのdomain outputは`workflow_models/states/transitions`、`derived_requirement_candidates`、
`runtime_switches/routes/resources/schedules/allocation_policies`へ分離する。Pythonまたはsubagentはversioned proposalだけを返し、
Nodeがschema、reachability、digest、authorityを再検証した一transactionだけでprojectionへcommitする。

主要conflictは、raw prompt直接実行をreject、単一JSON同時準拠をstaged envelopeへredesign、schema/prose driftをharden、
到達不能approval fixtureをnegative oracle化、schedule/resource/candidate driftをruntime schemaのredesign＋hardenで閉じる。
各処置はreplacement、positive/negative fixture、assertion、terminal receiptが揃うまでresolvedにしない。

blocking gateは次を個別receipt化する。

- `HIL_UWR_PACKAGE_CUSTODY`: archive、14 entry、descriptor、digestが一致する。
- `HIL_UWR_ANCHOR_OWNERSHIP`: primary anchor 328 exactly、route sum一致。
- `HIL_UWR_SPAN_OWNERSHIP`: 328外のsupplemental/support span分母を固定し、terminal disposition済み、orphan 0。
- `HIL_UWR_QUESTION_CATALOG`: canonical 76 unique、全件がanswered/unresolved/not-applicable/rejectedの明示状態を持つ。
- `HIL_UWR_SCHEMA_CONTRACT`: schemaとproseのfield/type/applicability差分0。
- `HIL_UWR_CONFLICT`: current conflict 0、5主要findingがterminal receiptを持つ。
- `HIL_UWR_FIXTURE`: positive reachability green、negative expected failure code一致、orphan 0。
- `HIL_UWR_RUNTIME_COMPLETENESS`: switch、route、schedule、resource、allocation、degradation、dead-letterが閉じる。
- `HIL_UWR_COVERAGE`: behavior atomからHIL要求、design、assertion、gateまで全joinする。

`RequirementFreezeGate`は上記current receiptと別provider/model familyによるreviewが揃うまでfail-closeする。
...
- role別read/write/tool/delegation authority matrix、escalation route、authority revision
- maximum spawn depth、fan-out/child count、global active ceiling、recursive spawn可否、deadline、retry/backoff上限
- evidence custodian、classification、redaction、retention、access、transfer、revocation policy （日本語の機械契約記述）
...
| `agent_musters` | `agent_muster_id` | plan/Issueを参照、operation/idempotency keyは一意 | layer、drive、task kind、verification/team/authority digest、spawn depth/fan-out/child/active ceiling、deadline/retry policy、created_at | `planned`, `ready`, `leased`, `running`, `completed`, `failed`, `cancelled`, `dead_lettered` |
...
| `agent_instances` | `agent_instance_id` | muster member/versionとparent instanceを参照、member＋attemptは一意 | parent/causality/task/authority digest、spawn depth、current fence/lease/checkpoint、deadline、attempt/max retry/backoff、result/failure taxonomy、created/updated | `mustered`, `leased`, `running`, `checkpointed`, `completed`, `failed`, `cancelled`, `timed_out`, `verification_pending`, `verified`, `compacted`, `released`, `dead_lettered`, `quarantined`, `retired` |
...
| `agent_compaction_receipts` | `agent_compaction_receipt_id` | instance、verification receipt、memory eventを参照、instance＋result digestは一意 | promoter provider、input/output digest、disposition=`promote\|supersede\|no-promotion`、secret/raw-log scan、completed_at | `valid`, `stale`, `revoked` |
| `agent_evidence_custody_events` | `agent_evidence_custody_event_id` | instance/artifact/checkpoint/receiptを参照、subject＋sequenceは一意 | custodian、classification、redaction、retention/access policy、from/to custodian、previous/event digest、reason、occurred_at | append-only |

実行状態は`mustered→leased→running↔checkpointed→completed→verification_pending→verified→compacted→released`を許す。
`failed`、`cancelled`、`timed_out`は検証済み扱いにせず、contractのmax retry/backoff内だけ同じmuster memberの
parent causalityを保持した新attempt/new instanceを作る。上限到達時は`dead_lettered`へ送りtask/evidenceをdropしない。
...
`HIL_AGENT_RELEASE_UNVERIFIED`、compaction receipt欠落は`HIL_AGENT_RELEASE_UNCOMPACTED`、custody chain切断は
`HIL_AGENT_EVIDENCE_CUSTODY_INVALID`としてfail-closeする。MusterPlannerはspawn depth、fan-out、child count、global active
ceilingとrecursive delegationをclaim前に検査し、role authority外のread/write/tool/delegationを許可しない。
...
L1企画/scope input digest、PO判定、理由、再entry triggerへbindした`not_applicable` receiptでskipする。
...
`L1企画 → L2 rough requirement → executable prototype → walkthrough → L2要求反映 → agreement → L3要件定義/freeze`
...
| L1要求群 | L4 block | 左右pair evidence |
|---|---|---|
...
| HIL-BR-26..29 / HIL-FR-51..56 / HIL-TR-12 / HIL-NFR-30..34 | Universal Workflow登録・質問・正規化・導出・実行計画 | L3↔L10 workflow completeness/oracle receipt |
| HIL-BR-30 / HIL-FR-57..60 / HIL-TR-13 / HIL-NFR-35..38 | standing authorization・機械gate・platform broker・bounded executor | L3↔L10 authorization/execution receipt |
...
本節の省略範囲表記は説明用であり、Gate入力には使用しない。全162要件の機械join正本は
...
### §11 PO7 decision activation aggregate （日本語の契約見出し）

`DecisionActivationGate`はUniversal 6 groupとVMAUTH-PO-01を同一authority epochで検査するが、選択そのものは行わない。
Universal側はversioned `UniversalOptionReceiptV1`、VMAUTH側は`VModelAuthorityActivationEventV1`を入力とし、
packet/source/decision-table、actor authority、chat answer、option-bound co-authorityの各digestをexact joinする。
各groupの`selected_option_id`はpacket allowlistに対するexactly-oneとし、B/C optionは`risk_class`、
`required_co_authorities`、`authority_scope`、`authority_expiry`がcurrentでなければfail-closeする。

`DecisionActivationTransactionV1`は`activation_transaction_id`、`idempotency_key`、`expected_activation_epoch`、
`expected_event_head`、`packet_sha256`、`source_set_sha256`、`expected_group_receipt_count=6`、
`expected_answer_receipt_count=22`、`all_or_nothing=true`、ordered write-set digestを持つ。write setは
6 group receipt→22 question receipt→VMAUTH event→disposition/queue projection→terminal receiptの固定順である。
writerはwrite直前に全source/authority/epoch/headを再検証し、単一SQLite transactionとevent-head CASでcommitする。
同じkey＋同一payloadは既存receipt、異payloadとCAS loserは増分0、各append faultは全rollbackする。

lifecycleはappend-only `active | stale | revoked | superseded`で、active上書き・削除は禁止する。packet/source/chat custody、
actor authority、co-authority scope/expiryのdriftはactive authorityをstale化しfreeze blockerをreopenする。明示取消はrevoked、
新revisionは旧revisionをsupersededへ遷移させる。選択0、runtime mutation 0、coverage credit 0の現状態を設計追加で変更しない。

## §13 Post-PO Design Freeze transition （日本語の契約見出し）

計7 decision unit回答後のauthority activationから最初のL01 candidateまでの正本は
`docs/governance/post-po-design-freeze-transition-contract-v1.md`とする。`PostPoDesignFreezeTransitionPort`だけが
`PostPoDesignFreezeTransitionBundleV1`を受け取り、7 answer/approval receipt、再生成済みcritical artifact/review/auditの
exact digestと`open=0`、pushed HEAD/tree、working-tree exclusion policy、layer/task/evidence denominator、独立reviewer、
4 expected event headをcommit直前に再検証する。

authority event→Design Freeze receipt→freeze/progress projection→L01 `proposed/pending_pair/not_frozen/not_counted` （日本語の機械契約記述）
candidate→outbox/terminal receiptは単一transactionでappendする。fault、異payload replay、CAS loser、source driftは全増分0。
commit outcome不明だけを`reconcilePostPoDesignFreezeTransition`がexact event chainから収束し、remote tagは作らない。

同transactionは`LocalL01TagCandidateHandoffV1`もappendし、candidate/event/operation ID、transition payload、freeze receipt、
HEAD/tree、roadmap/task/evidence denominator、expected candidate head、authority epoch、expiryのdigestを固定する。
LayerFreezeTagGateはhandoff digestをremote request、annotated tag payload、`layer_tag_receipt`へexact bindし、別candidate、expired、
authority/freeze/head driftを拒否する。`layer_tag_candidate_visibility_query`はcandidate state/event/handoff digest、remote未作成理由、
expiry、remote observationとlive/in-progress/freeze-progress/last-frozen/stale-fromを返し、pending/cancelled/expiredを分子0にする。

commit後のanswer/source/review/HEAD/tree/denominator driftまたはrevokeは
`PostPoFreezeInvalidationBundleV1`でfreeze stale/revoked、critical reopened、progress stale、未remote L01 candidate cancelledを
同一transactionへappendする。再freezeはnew receiptで旧receiptをsupersedeし、旧event/receiptを更新・削除しない。
remote tag作成後は`LayerAuthorityInvalidationBundleV1`がimmutable refを変更せず、authority event、remote observation、layer/tag receipt、
V-pair binding/receipt、progress/visibilityのexpected headsを単一CAS transactionでstale/revokedへappendし旧creditを0にする。
partial writeは0、unknown outcomeはoperation/payload/handoff/tag OID/event chainのexact一致だけをreconcileする。
