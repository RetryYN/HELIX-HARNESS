...
| `RequirementApprovalGate` | Node / GateEngine facade | L3追加・意味変更revisionをユーザー回答source、承認者、HEADへ束縛 | approval receiptのみ | AI代行承認、revision/HEAD不一致、承認欠落 |
...
11. L3要件追加・意味変更はユーザー承認receiptなしでfreezeしない。監査scopeはversion管理graphから動的導出する。
