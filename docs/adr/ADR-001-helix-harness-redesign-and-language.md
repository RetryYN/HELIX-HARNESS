...
2. **実装言語は TypeScript に統一**: core を **TypeScript (strict) / Bun runtime** で実装し、**bash 層を廃止**。OS 入口は薄い `helix.ps1` (Windows) / `helix` (POSIX) が同一の compiled core を呼ぶだけにする。
...
| test | vitest |
| state | `yaml` + JSON + SQLite projection DB (`.helix/harness.db`; Bun runtime では `bun:sqlite` 第一候補、Node 互換が必要な場合のみ `better-sqlite3`) |
