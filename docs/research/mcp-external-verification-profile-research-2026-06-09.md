...
| GitHub MCP Server configuration | https://github.com/github/github-mcp-server/blob/main/docs/server-configuration.md | GitHub MCP は toolsets、explicit tool selection、read-only mode を support し、read-only mode は要求されても write tools を無効にする。 | MCP profile の既定は調査・参照用の read-only/narrow-toolset に限定する。HELIX の GitHub 通常運用（branch push、draft PR、CI 取得、merge）は別の authenticated operation profile とし、認証済み write 権限、対象 repo scope、audit evidence を必須にする。human approval は高影響・不可逆操作に限定する。 |
...
| `github-mcp-readonly` | `external_issue`, `ci_failure`, `pr_review`, `backlog_sync` | Issue/PR/CI/backlog context を読んで workflow evidence へ正規化できる。 | default disabled。MCP の intended default は read-only/narrow toolsets。 | PR 作成・merge などの write operation は HELIX GitHub operation profile（`gh` 認証 + repo write preflight + audit evidence）で扱う。高影響・不可逆操作だけ human residue に escalate する。 |
...
- GitHub MCP は default で read-only/narrow toolsets とする。これは MCP 調査・参照 profile の境界であり、HELIX の通常 GitHub operation を read-only に固定する規則ではない。
- branch push、draft PR、CI 状態取得、merge queue / merge は authenticated GitHub operation profile で扱い、`gh auth status` と repo write/admin preflight、audit evidence を必須にする。人間承認は不可逆・高影響操作に限定する。
