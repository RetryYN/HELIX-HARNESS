...
| `github-mcp-readonly` | `external_issue`, `ci_failure`, `pr_review`, `backlog_sync` | Issue/PR/CI/backlog context を読んで workflow evidence へ正規化できる。 | default disabled。intended default は read-only/narrow toolsets。 | write toolsets には human approval と separate profile が必要。 |
