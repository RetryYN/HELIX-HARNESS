...
- 実行者: Codex hosted runtime。repository外の`/tmp/helix-kimi-smoke.dA2twd`で実行。
- `kimi doctor`: OK、v0.27.0。
- fixture: `Return exactly HELIX_KIMI_OK. Do not use tools and do not create files.`
- 結果: exit 0、FS増分0。ただし標準出力に生成途中の文と重複tokenが混入し、exact一致に失敗。
- disposition: CLI疎通はcurrentでも成立するが、instruction-following機械判定はfailure。過去S2 4/4で相殺せず、
  S3/S4未完了、`helix kimi`未実装、admit不可を維持する。
