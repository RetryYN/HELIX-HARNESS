# HELIX から UT-TDD への fork 完了 plan

## 0. 目的

社内 agent 基盤を UT-TDD Agent Harness へ統合し、`vendor/helix-source/` snapshot を historical source のみとして扱える状態にする。最終的には snapshot を削除できるよう、snapshot 内でまだ再利用可能な要素（skills、subagents、slash commands、hooks、CLI tools）を棚卸しし、残りの fork 作業順序を定める。
...
| stream | source total | UT-TDD への反映済み | 再利用可能 / pending | obsolete（HELIX-specific） |
|---|---:|---:|---:|---:|
...
curation/transplant は requirements-driven であり、HELIX-driven ではない。HELIX asset が scope に入るのは、既存 UT-TDD requirement（`FR-L1-*` / `BR-*`）、drive model（`be` / `fe` / `fullstack` / `db` / `agent`）、または mode に対応する場合だけである。source of truth は UT-TDD の requirement/design であり、HELIX は loose reference に限る。

- capability が UT-TDD の requirements / drive models に無い場合は import しない。要求されていない HELIX behavior は trace できない scope creep であり、degrade risk にしかならない。
- UT-TDD が Scrum / Incident / Recovery / Reverse mode などを独自 design としてすでに所有している場合、HELIX skill version は import しない。別 design を混ぜると UT-TDD 側を degrade する。必要なら UT-TDD mode/requirement から UT-TDD skill を author する。

net effect: skill curation 自体は requirement（`FR-L1-47` skill-pack 正本 + `FR-L1-12` injection）である。この criterion は、どの pack を対象にするかを drive/mode/FR-mapped のみに制限し、HELIX-shaped import を禁止する。§2 は「どの HELIX skills を port するか」ではなく、「どの UT-TDD requirement / drive area に pack が不足しており、UT-TDD design から author するか」と読む。§2.1 items は drive/mode/FR への trace がある場合だけ残し、disciplined curation step で pack ごとの trace を再確認し、trace できないものは落とす。

## 2. skills migration の完了

### 2.1 migrate-now（20）: UT-TDD-relevant だが未 curated

以下の vendor skills は `docs/skills/*.md` へ curate する。ADR-001 と concept_v3.1 の skill pack 方針に従い、verbatim copy ではなく UT-TDD pack として再 author する。
...
**Resolved（PO 2026-06-17）: import しない。** `agent-skills/helix-scrum`、`workflow/incident`、`workflow/postmortem` は mode-duplicate である。UT-TDD は Scrum（`FR-L1-23`）、Incident（`FR-L1-16`）、Recovery（`FR-L1-10`）を `docs/process/modes/{scrum,incident,recovery}.md` の drive-model design としてすでに所有している。HELIX versions を import すると divergent design になり degrade する。これらの surface に skill pack が必要な場合は、§1.5 に従い UT-TDD mode/FR から author し、HELIX は loose reference に限る。
...
- **Body polish（低優先、leak ではない）:** `pmo-project-scout`、`pmo-sonnet`、`pmo-tech-docs`、`pmo-tech-fork`、`pmo-tech-news` は、`pmo-project-explorer` / `pmo-haiku` のような UT-TDD-specific output contract / memory section をまだ持たず、元の日本語 prose を保持している。optional consistency upgrade であり、HELIX path leak ではない。survey 中に過大評価された "HELIX text" finding を是正する。
- **Retire（2）:** `pmo-helix-explorer`、`pmo-helix-scout` は vendor snapshot 内にのみ存在し、`~/ai-dev-kit-vscode/` を対象にしている。transplant candidate ではなく、UT-TDD route も無い。snapshot 削除時に消えるため action は不要。

## 4. slash commands: net-new stream（最も leverage が高い）

UT-TDD には `.claude/commands/` が存在せず、vendor command 10 件のうち transplanted は 0 件である。directory を作成し、HELIX names を UT-TDD へ adapt し、allowlisted agents を参照する形で transplant する。

| priority | command | 理由 |
|---|---|---|
| P0 | `ship` | fan-out orchestrator。`code-reviewer` / `security-audit` / `qa-test` を呼び、go/no-go を synthesize する。UT-TDD review gate を直接実装し、現状は代替が無い。 |
...
- **Not needed（PO 2026-06-17）:** `sessionstart-history-injection` / `userpromptsubmit-context-bundle` は transplant しない。session continuity は UT-TDD requirement と own design で満たしている。handover `CURRENT.json`（`FR-L1-42`）、session-log digest（`FR-L1-07`）、layer-context injection（`FR-L1-12`）、memory system が該当する。parallel HELIX per-prompt injection mechanism は duplicate/degrade であり context を肥大化させるため、distinct requirement が無い。
- **Obsolete（3）:** `pretooluse-askuserquestion`（UT-TDD は AskUserQuestion を禁止）、`pretooluse-codex-slot-check`（helix.db slots）、HTTP-API job slots。drop する。

## 6. CLI tools / TS re-implementation: pending capabilities 9 件

HELIX CLI capabilities 28 件のうち 17 件は `ut-tdd` command として実装済み。以下は concept として reusable だが TS 実装が無い。参照先 HELIX libs の logic を reference として扱う。
...
| P0 | Scrum / Reverse runtime cmds | `ut-tdd scrum`、`ut-tdd reverse`（現状 lint のみ） | `scrum_local.py`、`reverse_local.py`、`scrum_to_reverse_routing.py` |
...
| P1 | LLM / research guard / redaction | `ut-tdd guard` family の整備 | `llm_guard.py`、`research_guard.py`、`redaction.py`、`context_guard.py` |
| P2 | escalation / debt CLI | `ut-tdd escalation`（DB tables は存在、CLI 無し） | `escalation_engine.py`、`deferred_findings.py` |
| defer | dashboard / metrics render | `ut-tdd metrics` 拡張 | `helix-dashboard`、`helix-observe` |
...
obsolete で UT-TDD need が無いものは HTTP API bridge、HELIX statusline / mode-state-machine / PR automation である。UT-TDD は `ut-tdd status` と `gh` を使う。
...
special な "import complete" declaration は不要である。snapshot は UT-TDD 再構築中の read-only reference にすぎず、runtime は使っていない。snapshot は、以下の requirement-backed work が終わった時点で削除する。
...
- **Mode-duplicate skills（helix-scrum / incident / postmortem）: import しない。** §2.3 の通り、UT-TDD がこれらの modes を所有しており、HELIX versions の import は degrade になる。
...
- **§8(1) skill curation + substance: DONE。** `PLAN-L7-70`（confirmed）。54 packs、すべて UT-TDD substance、generic stubs 0。`§2.1` migrate-now は curated 済み。`§1.5` prune として non-mapped / HELIX-shaped packs 4 件（`ai-coding`、`quality-lv5`、`source-driven-development`、obsolete `SKILL_MAP-draft`）を除外した。`SKILL_MAP` は real catalog index として rewrite 済み。既存 search-index の false-positive secret guard も合わせて修正済み。
...
- **§6 P0 task classify: DONE。** `PLAN-L7-72`。`src/task/classify.ts` と `ut-tdd task classify` CLI を実装し、既存 `scoreTaskComplexity`（`FR-L1-39`）/ `classifyDrive`（`FR-L1-41`）/ `inferTaskDifficulty` contracts の上に構成した。module は architecture §3.1 へ backfill 済み。
- **§8(2) re-scope（explicit defer/never、§8(2) で許可）:**
  - `§6 P0` scrum / reverse runtime commands（`ut-tdd scrum` / `ut-tdd reverse`）: DEFER。large mode state machines であり、lint surfaces（`scrum-reverse`、`plan lint`、`vmodel lint`）は既存。skill packs は `docs/process/modes/` から author 済み。後続 Phase-1 wave で land する。
  - `§6 P0/P1` `ut-tdd task estimate`、audit CLI、guard family、escalation CLI、E2E harness: DEFER とする（W9/W12/W13/W17 wave-gated）。
