# HELIX to HELIX cutover strategy 記録
...
cutover target は HELIX-owned execution と state である。

- HELIX は reference source のみである。concepts、labels、historical inventory を供給する。
- executable harness behavior は TypeScript/Bun `src/` 配下の HELIX 側が所有する。
- `helix` CLI と `.helix/` state は current harness path である。
...
これは、active cutover work が wave-by-wave code port ではないことを意味する。repository が current HELIX path を一貫して記述・検証できるよう、documentation、commands、state projection、audit feedback を backfill する作業である。

## 2. Superseded assumptions の一覧

| Old assumption 旧前提 | Current rule 現行ルール |
| --- | --- |
| HELIX Python modules を HELIX package へ copy する | required behavior は TypeScript/Bun `src/` で再実装する |
| HELIX CLI commands を operating path として使う | `helix` commands を operating path として使う |
| `.helix/` を active state として扱う | `.helix/` を active generated state area として扱う |
...
この repository は new work について、すでに HELIX-owned mode にある:
...
- roadmap と review checks は HELIX CLI/doctor path 経由で動く。
...
これらは deterministic projections である。source は repository documents と generated evidence のままであり、`.helix/harness.db` は rebuild できる。

## 5. Verification definition の定義

cutover backfill は、以下がすべて成立したときに complete とする:

- `helix doctor` が active roadmap drift または handover drift を報告しない。
...
- `.helix/harness.db` は repository sources から rebuild できる。
