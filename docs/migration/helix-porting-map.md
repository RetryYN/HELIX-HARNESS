...
> **superseded (ADR-001, 2026-05-27)**: 本 map は旧 HELIX Python code の file 単位 port 計画だったが、実装方針が **TypeScript で全面再実装（旧 HELIX は概念のみ取り込み）** に変更されたため、**code-port 計画としては superseded**。本書は **旧 HELIX 能力 inventory / HELIX-HARNESS の TS 再実装時の機能参照**として残置する。既存の `src/ut_tdd/*.py`（W1-W3a port）も TS 再実装で置換予定。正本は `../adr/ADR-001-helix-harness-redesign-and-language.md` / `concept_v3.1` / `requirements_v1.2`。
...
| target が behavior を所有 | runtime code は `src/`、`scripts/`、`.ut-tdd/`、`.claude/`、`.github/`、`docs/skills/` など HELIX-HARNESS-owned path に置く。 |
| test は code と一緒に移す | 各 porting unit は対応する test、または新しい fixture を含める。 |
| name を置換 | 旧 `helix` command / `.helix` state / absolute local path は、現行の `ut-tdd` command / `.ut-tdd` state / repo-relative path へ置き換える。product prose は HELIX-HARNESS に寄せる。 |
...
| W1 | `cli/lib/plan_parser.py` | `src/ut_tdd/plan_parser.py` | HELIX frontmatter name を current harness §1 enum へ置き換える。 | `test_plan_parser.py` を port する。 |
...
| W1 | `cli/templates/plan/*/template.md` | `docs/templates/plan/*/template.md` | HELIX wording を rename し、HELIX-HARNESS artifact/role field を追加する。 | 全 template に `ut-tdd plan lint` を実行する。 |
...
| W3 | `cli/lib/task_type_inference.py`, `task_dispatcher.py` | `src/ut_tdd/task/classifier.py`, `src/ut_tdd/task/dispatcher.py` | current harness kind/drive/size/complexity へ map する。 | `test_task_type_inference.py`、`test_task_dispatcher.py` を port する。 |
...
| W9 | `skills/workflow/reverse-*`, `skills/workflow/poc` | `docs/skills/reverse-pack.md`, `docs/skills/planning-pack.md` | HELIX-HARNESS skill pack docs へ curate する。 | `ut-tdd skill suggest` fixture を行う。 |
...
| W11 | `cli/lib/builders/*` | `src/ut_tdd/builders/*` | HELIX registry name を置換し、HELIX-HARNESS template と verify script を emit する。 | `test_builders.py`、`test_builders_concrete.py` を port する。 |
...
- [x] 旧 `helix` command name を現行 `ut-tdd` へ変更する（PLAN-M-02 承認前は CLI 名として維持）。
- [x] 旧 `.helix/` state path を現行 `.ut-tdd/` へ変更する（PLAN-M-02 承認前は runtime state 名として維持）。
