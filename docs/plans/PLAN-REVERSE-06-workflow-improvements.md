...
- **continuation readiness**: 旧 CURRENT freshness discipline は session handover 退役で置換済み。PLAN 活動に対する authored source・append-only event・harness.db projection・bounded memory の整合を機械検査し、旧 pointer は生成しない。
