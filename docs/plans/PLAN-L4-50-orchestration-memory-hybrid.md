...
> 本 PLAN は archive で閉じず、誤起票の confirmed historical record として残す。
...
- `loop-state.ts` — LoopState 型 + `.helix/state/loop/{planId}.json` I/O。
...
- `src/runtime/adapter.ts`: codex `--execute` 時に `--sandbox workspace-write` を付与（dry-run は read-only 維持、`CODEX_STDIN_ARGS` 定数は不変で doctor adapter-policy 維持）。これで `helix codex --execute` が in-repo 書き込み可能になり、**実装を Codex へ分散できる**（hybrid worker 前提の土台、PO 承認 2026-06-28）。test = `U-ADAPTER-007`（tests/runtime-adapter.test.ts）。
...
- 保存先 = harness.db 2テーブル `harness_memory_entries` / `project_memory_entries` ＋ `.helix/memory/*.jsonl`（git 共有・**Claude も Codex も読める**）。
...
`loop_iterations`（`plan_id`、反復番号、worker/verifier provider、判定、概算費用、停止理由）／`jobs`（優先度、状態、再試行回数など）／`harness_memory_entries`／`project_memory_entries`。
...
### Step 5: [直列] trace-freeze → レビュー（review / cross-runtime）→ accept
...
| 2 | src/orchestration/* + src/memory/* + cli/guard 改修 | `helix codex` で実装委譲、既存 primitive 再利用、私が review |
...
- [ ] `helix memory` CLI 動作、`.helix/memory/` 共有、`.claude/agent-memory/` silo 廃止。
