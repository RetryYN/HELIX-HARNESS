...
review_evidence:
  - reviewer: codex-cli
    ...
    green_commands:
      ...
      - kind: smoke
        command: "npx --no-install tsx src/cli.ts feedback list --emit | awk '/missing-test-plan-id/ {count++} END {print count+0}'"
        runner: bash
        scope: targeted
        exit_code: 0
        completed_at: "2026-07-06T08:51:33+09:00"
        ...
        output_digest: "sha256:ac896fd654918db3a54889765e8173a2d5bc2fba293e890dc34e55fd38aed6a7"
      - kind: lint
        command: "npx --no-install tsx src/cli.ts plan lint docs/plans/PLAN-L7-175-helix-orchestration-memory-impl.md"
        runner: node
        scope: targeted
        exit_code: 0
        completed_at: "2026-07-06T08:51:33+09:00"
      ...
      - kind: lint
        command: "npx --no-install tsx src/cli.ts plan lint docs/plans/PLAN-L7-176-helix-orchestration-memory-runtime.md"
        runner: node
        scope: targeted
        exit_code: 0
        completed_at: "2026-07-06T08:51:33+09:00"
      ...
      - kind: lint
        command: "npx --no-install tsx src/cli.ts plan lint docs/plans/PLAN-L7-177-helix-orchestration-runtime-bridge.md"
        runner: node
        scope: targeted
        exit_code: 0
        completed_at: "2026-07-06T08:51:33+09:00"
      ...
      - kind: lint
        command: "npx --no-install tsx src/cli.ts plan lint docs/plans/PLAN-L7-94-outstanding-work-surface.md"
        runner: node
        scope: targeted
        exit_code: 0
        completed_at: "2026-07-06T08:51:33+09:00"
      ...
      - kind: lint
        command: "npx --no-install tsx src/cli.ts plan lint docs/plans/PLAN-L7-346-test-plan-id-remediation.md"
        runner: node
        scope: targeted
        exit_code: 0
        completed_at: "2026-07-06T08:51:33+09:00"
...
（2026-07-06 実測、`npx --no-install tsx src/cli.ts feedback list --emit`）。PLAN-L7-143/144 で過去 16 ファイル分を
...
1. `npx --no-install tsx src/cli.ts db rebuild`（コマンド名が異なる場合は projection 再構築コマンドを
   `npx --no-install tsx src/cli.ts --help` で特定）後、`npx --no-install tsx src/cli.ts feedback list --emit` で
...
- `npx --no-install tsx src/cli.ts feedback list --emit` の `missing-test-plan-id` が 19 → 0
  （projection 再構築後。0 にならない場合は残件の source_id を本文に列挙し理由を記録）。
- `npx --no-install tsx src/cli.ts plan lint docs/plans/PLAN-L7-175-helix-orchestration-memory-impl.md`
  ほか追記した全 PLAN の lint green。
- `npx --no-install tsx src/cli.ts doctor` に本 PLAN 起因の新規 fail なし。

## 4.1 検証結果

- `npx --no-install tsx src/cli.ts db rebuild` は exit 0（projection ok）。
- `npx --no-install tsx src/cli.ts feedback list --emit | awk '/missing-test-plan-id/ {count++} END {print count+0}'` は `0` を返し、`missing-test-plan-id` は 19 件から 0 件へ解消した。
