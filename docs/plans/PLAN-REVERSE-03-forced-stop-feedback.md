...
| R3 intent (po 検証) | (a) forced_stop を `agent_runaway` 級 Recovery trigger に昇格 / (b) Recovery 出口契約に **再発防止ドキュメント MUST** / (c) 新 FR を起こさず FR-07 拡張で吸収 — の 3 点が intent | **R3 PASS (2026-06-02)**: (a)(b) は PO が本 session で確定済 ([[feedback_forced_stop_high_severity_recovery]]、強制停止=最高 severity・Recovery 必須・再発防止 doc を出口契約に MUST) / (c) は code-reviewer self-review GO (新 top-level 要件にすべき過小評価なし、session-log REVERSE-02 と同方針)。よって R3 intent 充足、PO への再エスカレーション不要 |
...
- **L0 concept §2.6.1** (`docs/governance/helix-harness-concept_v3.1.md`): signal→mode 表の Recovery 行に `forced_stop` を追加 (`agent_runaway` 級、承認必須)。「ユーザー強制停止 = 高 severity 負シグナル、専用 hook 不在のため dangling-turn 推定」を備考。
...
> **なぜ新 FR でないか**: forced-stop 検出は hook 観測 = FR-L1-07 領域。フィードバックログ + Recovery 提示は FR-07 facet + Recovery (concept §2.5) への入力であり、独立 top-level 要件ではない。FR registry 拡張原則 (§1.10.G.10) に従い既存 FR で吸収。
...
`npx vitest run` (新 FR なし + 85 pass 維持)。
...
| FR-07 拡張 vs 新 FR | FR-L1-07 (hook 観測) scope + FR registry 拡張原則 (session-log REVERSE-02 と同方針) |
| AC-FR-07-05 oracle | forced-stop-feedback.md §2 と U-FSF-001〜007 を根拠にする |
