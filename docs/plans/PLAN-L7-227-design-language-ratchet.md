...
review_evidence:
  - reviewer: TL self-review (single-runtime)
    ...
    verdict: pass
    scope: "設計/governance/ADR docs の日本語 prose 原則を doctor hard gate に接続。既存英語 debt は 1669 件 baseline として可視化し、増加だけを fail-close する。CLAUDE.md 重複規則を除去し、L6 design / L7 test-design / doctor / lint / tests / PLAN trace を同期。"
...
- L3 `§0.2 意味ベース機能一覧と要求修正境界` は、confirmed 43 件、frontier、future parked、approval gated
...
- 既存 debt 1669 件を一括翻訳して別の巨大作業にしない。baseline は免罪ではなくラチェットであり、将来 PLAN で
  段階的に下げる。

## 2. 実装

- `src/lint/design-language.ts` を追加し、`docs/adr/`、`docs/design/`、`docs/governance/` の markdown を走査する。
- inline code、URL、frontmatter、コードブロック、日本語を含む行、許可された技術語は除外する。
- 英語だけの見出しを `english-heading`、英語だけの説明文を `english-prose` として数える。
- `DESIGN_LANGUAGE_BASELINE_VIOLATIONS=1669` を固定し、baseline 超過時だけ `ok=false` にする。
...
- [x] 実 repo の設計/governance/ADR docs が baseline を超えない。
- [x] `ut-tdd doctor` に `design-language` が表示され、`runDoctor.ok` に連動する。
- [x] `impl-plan-trace` が `src/lint/design-language.ts` を PLAN 紐付きとして扱う。
- [x] PLAN-REVERSE-227 で設計 / test-design への back-fill を記録する。

## 4. 範囲外

- 既存 1669 件の英語 prose 一括翻訳。
