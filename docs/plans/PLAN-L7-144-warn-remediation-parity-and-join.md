...
review_evidence:
  - reviewer: claude-opus-4-8
    ...
    green_commands:
      - kind: unit_test
        command: "npx --no-install vitest run tests/guardrail-invariant-advisory.test.ts tests/projection-writer.test.ts"
        runner: node
        scope: targeted
        exit_code: 0
...
# PLAN-L7-144 (troubleshoot): projection gate 完全整合 (full parity) + unresolved-join 除外
...
## 1. same-model advisory の projection gate 完全整合 (full parity、PLAN-L7-143 の訂正)
...
## 2. unresolved-join の作業コンテキスト (work-context) 除外
...
- typecheck / biome / vitest / doctor / plan lint は green（成功）。
...
## 5. 訂正継承 (supersession / PLAN-L7-143 との errata)
...
- 作業コンテキスト (work-context) 除外は具体的単一 PLAN 参照の dangling 検出を弱めない。
