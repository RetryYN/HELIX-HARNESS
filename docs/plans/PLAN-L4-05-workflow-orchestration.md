...
- 直列理由 = **file_conflict** (Step 1 と同一ファイル)。§3 を per-mode 外部設計へ拡張: ①入口 signal ②状態遷移 phase 列 + 各 phase what ③出口 contract (exit 条件 + Forward 合流先 L) ④担当 building block ⑤gate / 人間サインオフ要否。+ §3.x に **signal→mode routing 優先度モデル** (Incident>Recovery>Reverse>Refactor、concept §2.6) + **mode↔kind 非1:1** (Discovery/Scrum=poc / Incident=troubleshoot+recovery / Add-feature=add-design+add-impl)。FR-12 skill は §2/§3 で外部形状 (`helix skill suggest` の入力=PLAN context、出力=推挙 skill + 注入規約) を確定。
...
> **完了 (A-102、G4 add-design freeze、2026-06-05)**: 4 軸 PASS、`status: confirmed`。記録 = gate-design §2.1 A-102 注記 + `.helix/audit/A-102-g4-workflow-orchestration.md`。**review 前置 = code-reviewer (sonnet) で REQUEST_CHANGES → I-1 (Research/9-mode 帰属の二重 framing 橋渡し) + I-2 (routing 優先度を全順序へ) を修正後 freeze。M-1/M-2/M-4 は minor carry。cross-agent 不在のため intra_runtime_subagent (code-reviewer) で代替**。
