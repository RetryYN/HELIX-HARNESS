...
title: "PLAN-L7-02 (add-impl): forced-stop フィードバック実装 — src/runtime/forced-stop.ts + session-log 拡張 + helix feedback classify/pending + 単体テスト (U-FSF)"
kind: add-impl
layer: L7
drive: fullstack
status: confirmed
...
### 手順 1 (Step 1): session-log.ts 拡張 (② 土台)
...
### 手順 2 (Step 2): src/runtime/forced-stop.ts (② 本体)
...
### 手順 3 (Step 3): tests/forced-stop.test.ts (④、TDD Red→Green)
...
### Step 4: helix feedback サブコマンド (src/cli.ts)
