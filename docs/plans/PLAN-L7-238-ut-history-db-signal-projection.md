...
title: "PLAN-L7-238 (impl): unit-test history signal DB 投影"
kind: impl
layer: L7
drive: agent
status: confirmed
...
agent_slots:
  - role: aim
    slot_label: "Explorer - unit-test history DB projection risk"
  - role: tl
    slot_label: "TL - unit-test history DB projection implementation"
generates:
...
review_evidence:
  - reviewer: codex-tl
    ...
    verdict: approve
    scope: "IMP-147 の DB projection slice として `projectUtHistorySignals` を追加し、aggregate unit-test history signal、plan-scoped flake window、oracle-scoped flake/duration regression を DB 投影できることを確認した。general runner ingestion と duration trend 永続化は残差として維持する。"
    worker_model: codex
    ...
    green_commands:
      ...
      - kind: unit_test
        command: "npm test tests/workflow-contracts.test.ts tests/db-projection-ingestion.test.ts tests/review-green-command-projection.test.ts --timeout 180000"
        runner: node
        scope: targeted
        exit_code: 0
        completed_at: "2026-07-03T09:56:55+09:00"
...
# PLAN-L7-238: unit-test history signal DB 投影

## 0. 目的

IMP-147 は、単体テスト evidence history の残差を `general unit-test runner ingestion`、`flake history`、
...
- 2026-07-03 の PLAN-L7-241 以降は、同じ `quality_signals` sink に oracle/run 時点ごとの
  `duration_trend_ms` も保存する。専用 table / dashboard が必要になるまでは schema 追加を行わない。
...
- duration trend の時系列専用 table または dashboard 表示。PLAN-L7-241 は既存 `quality_signals` への
  schema-less persistence のみを扱う。
- L14 completion / version-up activation / `.helix` irreversible cutover の承認。

## 2. 受入条件

- `projectUtHistorySignals` が aggregate unit-test history signal を `quality_signals(source=ut-history)` へ保存する。
...
- IMP-147 の runner ingestion / duration trend 残差は PLAN-L7-240 / PLAN-L7-241 で後続実装する。
- 既存の `recordTestRunEvidence` / `computeUtHistorySignals` の API 互換性を壊さない。

## 3. 検証予定

- `npm test tests/workflow-contracts.test.ts --timeout 180000`
- `npm test tests/workflow-contracts.test.ts tests/db-projection-ingestion.test.ts tests/review-green-command-projection.test.ts --timeout 180000`
...
- [x] unit-test history DB 投影関数が実装されている。
