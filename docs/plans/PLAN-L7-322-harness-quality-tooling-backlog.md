...
parent_design: docs/governance/upstream-uttdd-reconciliation-completeness-2026-07-04.md
...
- distribution/pack 詳細（L7-232..282 の多く）= LOCAL distribution 既存 or PLAN-M-02 cutover territory。
...
- already-have（memory / model-id-ssot / advisor-routing / codex-spawn-guard / design-language / pending_decision /
  aging / oracle-ratchet / test_results ingest / design-bottomup mode / git-command-guard）。
...
- distribution / release surface の不可逆操作は PLAN-M-02 承認前に行わない。

## レビュー / 次工程
- 実装は Codex in-flight 着地後に harness workflow で行う。基準点は HEAD。
- 出典: [[upstream-uttdd-reconciliation]] completeness pass（L7-202..362 全 skim）。
</content>
