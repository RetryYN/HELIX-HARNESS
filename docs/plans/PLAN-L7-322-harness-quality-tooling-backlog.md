...
- **probe harness（L7-311）**: guard/gate/hook の実走検証常設化（**確定欠落** 2026-07-04 検証: LOCAL の `ut-tdd probe` は verification-profile の疎通確認という別物。上流は fixture 駆動で違反/正常 fixture を隔離実行し guard/gate/hook の実発火を検証する `ut-tdd probe run` runner＋`.ut-tdd/evidence/probes/` 証跡）。
...
- Step 1: 品質 gate 系（route-cert → write-encoding → bundle-split → detector-self-trigger → scope-integrity → mutation-oracle）。
- Step 2: observability/lifecycle 系（digest-anchor → plan-ref-freshness → telemetry-retention → plan-archive → probe harness → vendor-surface → hot-zone）。
