...
review_evidence:
  - reviewer: code-reviewer
    ...
    reviewed_at: "2026-07-25T05:59:19+09:00"
    tests_green_at: "2026-07-25T05:55:20+09:00"
    verdict: approve
    scope: "worktree の全 uncommitted 変更 + 新規 src/schema/model-registry.ts / tests/model-registry.test.ts。5 軸 (correctness / SSoT 整合 / 不可逆性 blast radius / 代替案 / 過剰主張) で確認し Critical/Important 所見ゼロ、Minor 2 件は監査精度の任意対応。再 export の値・型・参照不変 (U-MREG-002 参照同一性)、fail-closed validation (U-MREG-004)、module-boundary 既存 exception 内、tsc green・targeted 125 green を確認。"
    ...
    green_commands:
      - kind: unit_test
        ...
        scope: targeted
        exit_code: 0
        completed_at: "2026-07-25T05:53:52+09:00"
        evidence_path: tests/model-registry.test.ts
        output_digest: "sha256:189f41f1eedc3004c0a66af989569f01f2dc5333952031091785600b02320e62"
      - kind: typecheck
        command: "npx --no-install tsc --noEmit"
        runner: node
        scope: full
        exit_code: 0
        completed_at: "2026-07-25T05:55:03+09:00"
      ...
      - kind: lint
        ...
        scope: targeted
        exit_code: 0
        completed_at: "2026-07-25T05:55:20+09:00"
...
したい」。散在していた model ID / 単価 / 標準 effort の 5 テーブルを単一 JSON へ集約し、以後の
モデル更新を **config 1 ファイルの編集**で完了できるようにする (コード・テスト変更を伴わせない)。
あわせて Opus 5 リリースに追随し、opus 帯 model ID と標準 effort を config 値として更新する。
...
- **外部化正本**: `src/schema/model-registry.ts` の `RAW_MODEL_REGISTRY` 1 ブロックに 5 セクションを集約する。
  ...
  - **モデル更新はこの 1 ブロックの編集で完結**する (consumer/test の import は不変、散在 literal を編集して回らない)。
...
- モデル更新の運用手順 (config 1 ファイル編集) を今後の慣行とする。ライブ単価連携は別 PLAN。
