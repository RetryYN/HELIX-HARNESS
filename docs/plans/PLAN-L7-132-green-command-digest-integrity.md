...
review_evidence:
  ...
  - reviewer: codex-intra-runtime
    ...
    scope: "形式上 valid な全ゼロ / repeated-hex digest が placeholder として gate を素通りする穴を digest-placeholder で塞ぎ、既存 fake/malformed/missing evidence の hard-gate 契約を維持する。"
    worker_model: codex
...
## 0. 目的 (Objective、PO 指示 2026-06-23「すべて」)
...
2026-06-30 継続対応 (continuation): PLAN-L7-174 と後続是正で既存 fake/stale digest を実 hash に揃え、
...
- `src/lint/green-command-digest.ts`:
  - `auditGreenCommandDigests(plans, deps)` — 各 green_command の `output_digest` が `evidence_path`
    ...
    2026-07-06 追補として、`sha256:000...0` / `sha256:fff...f` のような単一 hex 文字 64 連続の
    プレースホルダを `digest-placeholder` として fail-close する。
...
または evidence file（証跡ファイル）が存在しない場合は `green-command-digest — violation` を出し、doctor 全体を
`ok=false` にする。

## 4. 受入条件 (AC / acceptance / substance)
...
- 是正後は hard gate から advisory へ戻さない。完了判定の根拠に fake/stale evidence（証跡）を混ぜない。
