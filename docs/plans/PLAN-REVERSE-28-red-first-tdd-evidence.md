...
title: "PLAN-REVERSE-28 (reverse): red-first-tdd-evidence の後追い補完"
kind: reverse
layer: cross
workflow_phase: R4
...
agent_slots:
  - role: tl
    slot_label: "TL - Red-first evidence 後追い補完"
generates:
...
review_evidence:
  - reviewer: PO/A-114-ddd-tdd-strengthening-reaudit
    ...
    verdict: approve
    scope: "A-114 独立再監査と PO close 指示。confirmation 前に typecheck/lint/vitest/doctor が green。add-feature triad は内容変更なしで close 済み。"
---

# PLAN-REVERSE-28 (reverse): red-first-tdd-evidence の後追い補完

## §0 位置づけ

IMP-099 の Reverse 側を記録する。
...
- Red-first TDD evidence の reverse trace を保持する。

## §3 工程表

### Step 1: [並列] 観測された gap の記録

Red-first evidence が機械的に強制されていなかったことを記録する。

### Step 2: [直列] Forward PLAN 接続

直列理由: downstream_dependency。L6/L7 PLAN ids が確定した後にだけ Reverse を close できる。
...
- [x] PLAN-L7-29 がこの Reverse PLAN を要求する。
