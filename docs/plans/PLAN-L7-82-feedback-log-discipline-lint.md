...
# PLAN-L7-82: feedback-log discipline lint（feedback-log 規律 lint、IMP-085）
...
| WBS-L7-82-02 | `checkFeedbackLog` を doctor へ配線 (runDoctor.ok 連動、hard gate)。fail-open(file 不在)/fail-close(repo 不在/読めない)。 | `src/doctor/index.ts` | `helix doctor` | `helix doctor` | [直列] |
| WBS-L7-82-03 | cross_agent review (qa-test=codex-gpt-5.5) → nit 反映 → confirmed。 | (review) | A-139 | `helix doctor` | [直列] (02 後) |
...
- [x] typecheck / biome / vitest (U-FBLOG-001..008) / doctor が green。
