...
- [x] `bunx vitest run tests/design-declarations.test.ts` green (10 tests)。
- [ ] `helix current-location --json` の `design_declaration_drifts` が本 doc 由来では 0 になることを
      DB projection rebuild 後に確認する (未実施、closure 前の最終確認として残す)。
...
`UTF-8` 由来の finding が消えたことを実測確認済み。**status は draft のまま維持する**
(`design_declaration_drifts` の DB projection rebuild 後の最終確認、および正式な `review_evidence`
(cross_agent, worker_model/reviewer_model 明記) の記録が未完了のため。次回セッションでの closure 対象)。
