...
review_evidence:
  - reviewer: codex-tl
    ...
    green_commands:
      - kind: unit_test
        command: "npm test tests/identifier-rename.test.ts tests/cli-surface.test.ts tests/completion-decision-packet.test.ts tests/outstanding.test.ts --timeout 300000"
        runner: node
        scope: targeted
        exit_code: 0
  ...
  - reviewer: codex-tl
    ...
    green_commands:
      - kind: unit_test
        command: "npm test tests/action-binding-approval-readiness.test.ts tests/s4-decision-readiness.test.ts tests/version-up-readiness.test.ts tests/cutover-readiness.test.ts tests/identifier-rename.test.ts"
        runner: node
        scope: targeted
        exit_code: 0
  ...
  - reviewer: codex-tl
    ...
    green_commands:
      - kind: unit_test
        command: "npm test tests/identifier-rename.test.ts tests/cutover-readiness.test.ts"
        runner: node
        scope: targeted
        exit_code: 0
  ...
  - reviewer: codex-tl
    ...
    green_commands:
      - kind: unit_test
        command: "npm test tests/action-binding-approval-readiness.test.ts tests/cutover-readiness.test.ts tests/outstanding.test.ts"
        runner: node
        scope: targeted
        exit_code: 0
  ...
  - reviewer: codex-tl
    ...
    green_commands:
      - kind: unit_test
        command: "npm test tests/action-binding-approval-readiness.test.ts tests/identifier-rename.test.ts tests/cutover-readiness.test.ts"
        runner: node
        scope: targeted
        exit_code: 0
  ...
  - reviewer: codex-intra-runtime
    ...
    verdict: approve
    scope: "Version-up activation と L14 rename/cutover packet は stale approval material 検出用の snapshot binding ID を公開する。Setup route evidence は helix setup project を使い、helix/.helix は PLAN-M-02 cutover target として保持する。README は gate、evidence、completion surface ではない。"
    worker_model: codex
    reviewer_model: codex-intra-runtime
    green_commands:
      - kind: unit_test
        command: "npm test tests/setup.test.ts tests/version-up-readiness.test.ts tests/identifier-rename.test.ts"
        runner: node
        scope: targeted
        exit_code: 0
...
- 補助 docs の setup guidance は gate / evidence / 完了条件に紐づけず、実装済み CLI と
  setup test を正本として `helix setup project` route と cutover 前 `.helix` baseline の意味を検証する。

## 対象外

- PLAN-L7-146 は activate しない。
- `旧 state path -> .helix` rename/cutover は実施しない。
