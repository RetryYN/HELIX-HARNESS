...
review_evidence:
  - reviewer: codex
    ...
    reviewed_at: "2026-07-06T06:55:00+09:00"
    tests_green_at: "2026-07-06T06:55:00+09:00"
    verdict: approve
    scope: "PLAN-L7-102 を archive ではなく confirmed errata record として復帰。後継 PLAN-L7-141 が prototype を supersede 済みであること、src/web/projection の現行 targeted regression が green であることを確認。"
    worker_model: codex
    ...
    green_commands:
      - kind: unit_test
        ...
        output_digest: "sha256:3e183abbce4479dc245243bb303ac0e5fea03044845bf353ff244a13f020e198"
---

# PLAN-L7-102 (impl): src/web 中央 UI Phase B — 15 画面 read-only ダッシュボード

> **訂正 / supersede (PLAN-L7-141、2026-06-24)**: 本 PLAN が生んだ `src/web` は L2
...
> prototype コード (`src/web/*.ts`)・`tests/web.test.ts`・`cli web` command を破棄した。本 PLAN は
> archive で閉じず `confirmed` の errata record として保持し、過大主張と後継 PLAN への trace を読める状態で残す。
> component-derived な再実装は後継 **PLAN-L7-141-web-dashboard-component-derived** が `supersedes` で
> 引き継ぐ。`screen` projection (PLAN-L7-96、doc 正本→harness.db) は基盤として保持。

## 0. 目的 (Objective、PO 指示「src/web 画面 UI / implemented flip の Phase B 完遂」2026-06-22)
...
- [x] 型検査 (typecheck / tsc) / biome EXIT=0 / vitest web 14 passed (real-repo + in-memory fixture = substance, PLAN-L7-89)。
- [x] intra_runtime_subagent review は approve (codex-intra-runtime + code-reviewer sonnet、VERDICT=pass、Important I-1 404 XSS 修正済)。
- [x] doctor EXIT=0 (impl-plan-trace / coding-rules / module-drift 含む)。

## 3. スコープ外 (Out of scope / carry / 後続)

- **`helix web` CLI 配線** (src/cli.ts) と **screens.implemented flip** (projection-writer.ts) = 別 hot file が
