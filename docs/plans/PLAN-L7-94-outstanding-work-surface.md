...
review_evidence:
  ...
  - reviewer: codex-tl
    ...
    tests_green_at: "2026-07-02T05:51:00+09:00"
    verdict: pass
    scope: "Continuation 48: workflow status, handover status, completion packet text, source ledger freshness, and rename cutover snapshot semantics were tightened together. `helix status` text now prints each `workflow-next-action` with required action and next route; `helix handover status` text prints live completion readiness and packet routes; completion packet text prints every required action for multi-blocker PLANs. Verification source ledger refresh now requires same-date `source_ledger_freshness`, `source_status_delta`, `adoption_decision_delta`, and `workflow_route_impact` evidence, so date-only refresh fails. Identifier rename cutover snapshots now bind the current Cutover source ledger checked date, changing `cutoverSnapshot.snapshotId` and invalidating stale action-binding approval material when the ledger basis changes."
    worker_model: codex
    reviewer_model: codex-intra-runtime
    green_commands:
      - kind: unit_test
      ...
      - kind: lint
        ...
        output_digest: "sha256:8599d1f67b483ca70739091a9a902257fe50eef2f42cfeb80257a7938c66e860"
  - reviewer: codex-tl
    review_kind: intra_runtime_subagent
  ...
  - reviewer: codex-intra-runtime
    ...
    verdict: pass
    scope: "Continuation 38: `helix handover status --json` now treats CURRENT.json as the session pointer while recomputing live `outstanding`, `completionDecisionPacket`, and G-SF `semanticFeatureFrontierRecords` for read-only resume preflight. This closes the semantic gap where an older handover snapshot could hide revised-request / parked / cutover classifications even though `helix status --json` exposed them."
    worker_model: codex
    reviewer_model: codex-intra-runtime
    green_commands:
      - kind: unit_test
  ...
  - reviewer: codex-intra-runtime
    ...
    verdict: pass
    scope: "Continuation 30: S4 pending Discovery/Scrum PLANs now expose a dedicated non-destructive `helix s4 decision-packet` surface. The packet reads `s4_decision_record`, reports confirmed/rejected/pivot routes, and fixes `planOnly=true`, `mustNotDecide=true`, `decisionCommandAvailable=false`, and `decisionAllowed=false` so PO/S4 decision support cannot mutate status, record decision_outcome, Forward-merge, or trigger Reverse fullback."
    worker_model: codex
    reviewer_model: codex-intra-runtime
    green_commands:
      - kind: unit_test
...
「未了の正の集計シグナル」(非終端 PLAN の層別数 + open defer 数 + 意味別待ち理由) を `helix status --json` と
...
`helix status` (mode + next のみ) も handover digest (commits/files/failures のみ) も CURRENT.json も
...
（coverage が substance ではないという feedback [[feedback_coverage_not_substance]] と、carry status を code に照合する feedback [[feedback_verify_carry_status_against_code]]）。
...
- `workflowNextActionForOutstanding` / `workflowNextActionsForOutstanding`: status の top action は既存互換の
  ...
  避ける。主 blocker は `po_decision_pending` -> `helix s4 decision-packet --json`、
  `version_up_parked` -> `helix version-up activation-packet --json`、
  `irreversible_migration_pending` -> `helix rename plan --json`、
  `human_approval_pending` -> `helix action-binding approval-packet --json` に意味ベースで接続する。
...
- decision packet は `generatedAt` / `sourceCommand` / `freshness` を持つ。freshness は
  ...
  `status --json` に埋め込まれた packet は `sourceCommand=helix status --json`、standalone packet は
  `sourceCommand=helix completion decision-packet --json` とし、古い転記や生成元不明の判断材料を避ける。
...
- `src/lint/action-binding-approval-readiness.ts`: high-impact approval / action-binding / human approval を含む
  ...
  `helix action-binding approval-packet` を追加し、high-impact approval PLAN を
...
- `src/lint/s4-decision-readiness.ts`: S3 pending PoC の S4 判断材料を `decision_basis` 自由文から
  ...
  公式 URL（`official_url`）、採用版・採用日（`adopted_version` / `adopted_date`）、最新公式状態
  （`latest_official_status`）、採用判断（`adoption_decision`）、S4 判断での用途（`s4_decision_use`）、
  必須 field への影響（`required_field_impact`）を持ち、Scrum Guide / ISO 29148 / ISTQB / NIST SSDF のいずれかを落とすと fail-close する。
...
- `src/lint/cutover-readiness.ts`: L14 cutover の source ledger を、公式 URL（`official_url`）、
  採用版・採用日、最新公式状態、採用判断、cutover 用途、必須 field への影響で検査する。NIST SSDF、
  GitHub Environments の required reviewers、GitHub Actions の concurrency、Google SRE の Release Engineering、
...
- status `--json` に `outstanding` / `workflowNextActions` / `completionDecisionPacket` を additive
  ...
  `completionReadinessLine`、`workflow-next-actions` 件数、各 `workflow-next-action` の required action / route、
  blocked 時の
  top blocker 専用 decision packet 導線と whole-program 監査用の
  `helix completion decision-packet --json` 導線を出す。
- `helix completion decision-packet [--json]` が同じ outstanding 正本から decision packet を出す。
- handover `runHandover` が CURRENT.json pointer に `outstanding` と同じ生成時点の
  `completionDecisionPacket` (`sourceCommand=helix handover`) を additive 記録する。`helix handover status --json`
  ...
  cutover の意味分類を落としていても再開時に復元して読める。text でも live `outstandingSummaryLine`、
  `completionReadinessLine`、`helix completion decision-packet --json`、supporting packet commands を出し、
  JSON を開かなくても PO/S4・version-up・cutover・action-binding の残判断を見落とさない。
...
- doctor `handover-decision-packet` hard gate が、blocked outstanding を持つ handover CURRENT.json に
  `completionDecisionPacket` が存在し、`sourceCommand=helix handover`、freshness/shape lint OK、
...
| Source | この packet への制約 |
|--------|----------------------|
| NIST SSDF SP 800-218: https://csrc.nist.gov/pubs/sp/800/218/final | 判断 evidence は stale prose ではなく、現在の verification / provenance へ追跡できなければならない。 |
| GitHub Environments deployment protection rules: https://docs.github.com/en/actions/reference/workflows-and-actions/deployments-and-environments | approval / wait 境界は保護された step として扱い、生成される判断材料には source と expiry が必要である。 |
| Scrum Guide 2020: https://scrumguides.org/scrum-guide.html | review / decision output は adaptation point であり、S4 packet は次 route を導ける鮮度を持つ必要がある。 |
| OWASP LLM06:2025 Excessive Agency: https://genai.owasp.org/llmrisk/llm062025-excessive-agency/ | high-impact な agentic action には、制約された権限、人間の oversight、監査可能な approval scope が必要である。 |

配置: placeholder-deps / shared を再利用するため解析層 `src/lint/outstanding.ts` に置く
...
- [x] `helix action-binding approval-packet` が承認待ち high-impact PLAN を専用 packet に分離し、
...
- [x] `helix action-binding approval-packet` は `approvalBindingChecks[]` を出し、承認 record の各 field が
...
- [x] `helix version-up activation-packet` は `activationReadinessSummary` を出し、external rehearsal /
