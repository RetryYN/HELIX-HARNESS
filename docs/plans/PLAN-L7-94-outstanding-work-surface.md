...
review_evidence:
  ...
  - reviewer: codex-tl
    ...
    verdict: pass
    scope: "Continuation 48: workflow status, handover status, completion packet text, source ledger freshness, and rename cutover snapshot semantics were tightened together. `ut-tdd status` text now prints each `workflow-next-action` with required action and next route; `ut-tdd handover status` text prints live completion readiness and packet routes; completion packet text prints every required action for multi-blocker PLANs. Verification source ledger refresh now requires same-date `source_ledger_freshness`, `source_status_delta`, `adoption_decision_delta`, and `workflow_route_impact` evidence, so date-only refresh fails. Identifier rename cutover snapshots now bind the current Cutover source ledger checked date, changing `cutoverSnapshot.snapshotId` and invalidating stale action-binding approval material when the ledger basis changes."
    worker_model: codex
    reviewer_model: codex-intra-runtime
    green_commands:
      - kind: unit_test
...
- handover `runHandover` が CURRENT.json pointer に `outstanding` と同じ生成時点の
  ...
  `completionReadinessLine`、`ut-tdd completion decision-packet --json`、supporting packet commands を出し、
