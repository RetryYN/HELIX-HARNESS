...
title: "PLAN-L7-311 (impl): model 標準 effort を runtime 選択へ配線"
kind: impl
layer: L7
drive: agent
status: confirmed
...
# PLAN-L7-311: model 標準 effort を runtime 選択へ配線
...
- tests: sonnet-pinned engine の effort が旧 task-based 一律 high から model 標準
...
- sonnet-pinned engine (pmo-sonnet) の critical タスクが medium (標準) を既定に返す
  (tests/team-model-policy.test.ts / team-launch-policy.test.ts)。
