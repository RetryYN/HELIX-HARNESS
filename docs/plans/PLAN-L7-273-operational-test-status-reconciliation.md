...
一方で下位では `ut-tdd setup project`、consumer doctor baseline、version-up dry-run / activation packet、
source ledger freshness / decision packet gate が実装・検証済みであり、上位表記が現在の機能一覧とズレていた。

この PLAN は、承認待ちの外部適用や `.ut-tdd -> .helix` cutover を完了扱いにせず、実装済み範囲を `partial`
として明示し、残 GAP を分離する。
