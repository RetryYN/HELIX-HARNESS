...
|----|--------------|--------|--------|
| U-NFR-1 | ISO 25010 対象外特性: **機能適合性 (Functional Suitability)** の除外理由 — HELIX では機能要求は FR-L1-* / BR-* で扱い、NFR で再定義しない (二重記述回避) | §7 除外行に `除外理由: 機能要求 (FR-L1-*/BR-*) で扱うため NFR 重複禁止 (AP-6 類似)` | ✅ (PO 承認済み 2026-05-28、nfr §7 対象外特性表に明記) |
...
### 3.2 Phase B telemetry NFR の引き継ぎ (carry)
...
| NFR-13 注記強化 | **gate 通過率 ≥90% (KPI D-02、B5=b)** を運用目標、`.helix/gate_runs` で計測 | ✅ (nfr §3 NFR-13 注記追記済) |
...
| agent guard bypass | **`HELIX_ALLOW_RAW_AGENT=1`: PO 明示承認 + audit ログ記録必須 (B6=b、F6=a)** | ✅ (nfr §5 セキュリティ表に追加済) |
...
### §7 IPA × ISO 25010 二軸タグ表の整備 (Step 4)
- 担当: tl
...
| §4 移行性 | IPA: 移植性 | HELIX → HELIX 移行計画 / Phase A→B | 移行戦略 (`docs/migration/helix-identifier-cutover-strategy.md`) 参照 + Phase B 拡張計画 carry |
