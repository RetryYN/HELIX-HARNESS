---
plan_id: PLAN-L7-411-feedback-lifecycle-journal
title: "PLAN-L7-411 (impl): feedback lifecycle journal・generation・rebuild"
kind: impl
layer: L7
drive: db
status: confirmed
...
backprop_decision_reason: "PLAN-L6-63で確定するjournal/state machine契約の実装降下。"
entry_signals:
  - "po_directive:2026-07-11 feedback受信面を強化し、廃止済みhandoverへbackflowしない"
agent_slots:
  - role: se
    slot_label: "SE — journal/state machine/generation/crash recovery"
  - role: qa
    slot_label: "QA — U-FLIFE-001..005 adversarial oracle"
generates:
  - artifact_path: docs/plans/PLAN-L7-411-feedback-lifecycle-journal.md
...
# PLAN-L7-411: feedback lifecycle台帳

## 工程表

1. [直列] U-FLIFE-001..005をRed化する。
2. [直列] typed codec、state machine、activity/policy generation、operation replay、crash recoveryを実装する。
3. [直列] targeted test、typecheck、reviewで閉じる。

## 受入条件

- L6 §2-§4とU-FLIFE-001..005がgreen。
- 廃止済みhandoverへのwrite/fallbackが0件。
