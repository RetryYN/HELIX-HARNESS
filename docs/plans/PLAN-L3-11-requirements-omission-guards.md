---
plan_id: PLAN-L3-11-requirements-omission-guards
title: "PLAN-L3-11 (add-design): 要件漏れガード — 中間層 FR 横断カバレッジと inventory-first 実施証跡の FR を追加する"
kind: add-design
layer: L3
drive: agent
status: confirmed
...
generates:
  - artifact_path: docs/plans/PLAN-L3-11-requirements-omission-guards.md
...
review_evidence:
  - reviewer: codex-tl
    ...
    verdict: approve
    scope: "HR-FR-P9-05 / HR-FR-P9-06 と HAT-P9-05 / HAT-P9-06 を L3/L12 に追加し、中間層 coverage と inventory evidence gate の要件境界を確認した。"
    worker_model: codex
    ...
    green_commands:
      - kind: lint
        ...
        output_digest: "sha256:541c4e9f5170e7040636e5270afab74166a5d9b6c74e47abe2de19b5e6318aff"
---

# PLAN-L3-11 (add-design): 要件漏れガード FR の追加

## 実装確認（PO 指示 2026-07-06）

本 PLAN は archive では閉じず、L3/L12 への FR/HAT 追加が完了した事実に合わせて confirmed とする。
中間層 coverage と inventory evidence gate の runtime 実装は下流 L4-L7 PLAN で扱うが、本 add-design
の受入範囲は満たしている。
...
|---|---|---|---|
| 中間層カバレッジ (mid-layer coverage) | 新規 | FR の L3→L6 層別到達集計。停滞 FR の可視化と新規停滞の fail-close | 導入層 L3、confirmed 時に back-merge |
| inventory 証跡 (inventory evidence) | 新規 | 要件起草時の既存資産・旧 HELIX 照合の構造化証跡（照合先・照合日・採否） | 導入層 L3、confirmed 時に back-merge |

## §7 機能要求更新 (FR registry delta)

本 PLAN が検査 FR を 2 件追加する（activation 後の Step 1 / Step 2 で ID 採番・登録）。
既存 FR の意味変更なし。
