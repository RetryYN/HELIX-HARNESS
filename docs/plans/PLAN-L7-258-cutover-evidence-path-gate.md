...
`.ut-tdd/evidence/rename/` 配下の repo-local artifact path に固定し、`stateBackupManifest.backupTargetPattern`
も `.ut-tdd/backups/rename/<timestamp>/` 配下に限定する。
...
- `.ut-tdd` から `.helix` への実 state move は行わない。
