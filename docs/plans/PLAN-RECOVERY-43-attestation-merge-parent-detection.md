...
固定し、実行失敗・stdout 欠落・形式不正・申告不一致の分岐も同時に測る。source 文字列への依存は
cli が core の関数を呼ぶことの緩い regex 束縛だけに留める。
