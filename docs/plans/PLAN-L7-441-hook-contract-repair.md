...
review_evidence:
  - reviewer: codex-qa-independent
    ...
    green_commands:
      - { kind: unit_test, command: "npx --no-install vitest run tests/hook-contract.test.ts tests/project-hook.test.ts tests/digest.test.ts --reporter=dot", runner: node, scope: targeted, exit_code: 0, completed_at: "2026-07-12T15:16:52Z", evidence_path: tests/hook-contract.test.ts, output_digest: "sha256:c87e484b5d9daf254dbeb30bee32b9304f7ab86ef0c83ddf297630fc213e457d" }
...
- `npx --no-install vitest run tests/hook-contract.test.ts tests/project-hook.test.ts tests/digest.test.ts` green
