...
- `ut-tdd hook git-command-guard` と `.claude/hooks/git-command-guard.ts` を追加し、Claude `Bash` と Codex
...
- `.ut-tdd/state/destructive-git-override` は非空理由を持つ場合だけ one-shot bypass になり、
  `.ut-tdd/logs/destructive-git-overrides.jsonl` に audit を残す。
