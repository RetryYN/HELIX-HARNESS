...
title: "PLAN-RECOVERY-01 (recovery): 内部資産 HELIX 化の前提抜け — 認識ずれ収束 + L1/L3 fullback"
kind: recovery
layer: cross
drive: fullstack
status: completed
...
# PLAN-RECOVERY-01 (recovery): 内部資産 HELIX 化の前提抜け — 認識ずれ収束 + L1/L3 fullback
...
> **trigger 再分類 (A-78、recovery-workflow §1)**: 当初「認識ずれ」と記述したが、PO 訂正により **(a) 指示無視** = 「内部資産を HELIX 用に作り替えよ」という明確な指示の不履行と再分類する (softening しない)。
...
- **impact**: V-model 設計 L1-L6 を **「内部資産 (subagent/skill/command) は HELIX からそのまま使う/後で port」という誤前提**で進行。正しい前提は「**内部資産は HELIX 用に作り替える必要がある**」。結果、L1 業務要求・L3 機能要件に「HELIX が自前の runtime 資産体系を持つ/既存資産を再構築する」FR が欠落 (FR-level gap)。L4-L6 設計は TS core のみを対象にし、内部資産の次元が丸ごと欠けた。G1/G3 に gap。
- **検知元**: PO 指摘 (「ヘリックス側のスキル資産・サブエージェント・コマンドは TS に作り替えているのか、整理してあるか」→「内部資産を HELIX 用に作り替える必要があるのを前提抜けている」)。
...
5. PO が前提抜けを明示「内部資産を HELIX 用に作り替える必要がある」= FR-level 漏れ。
...
| # | 当初仮説 (誤) | 実際の状況 (正) | 根拠 |
|---|---|---|---|
| 1 | HELIX 内部資産はそのまま使う / 後で port すれば良い | **HELIX 用に作り替える必要がある** (機能要求レベル) | PO 指摘 |
...
- gap の本質: 「機構の FR (FR-09 guard / FR-12 skill 注入)」はあるが「**資産そのものを HELIX 用に構築する FR**」が無い。
...
- V-model/W-model 用語は A-74 で是正済 (L0-L14 = V-model / HELIX W = AI エージェント 2段V)。harness 自身は単一 V。
...
- 内部資産の正本: subagent = `.claude/agents/` (HELIX-hardened)、skill = curated `docs/skills/`。`vendor/helix-source/skills/` は read-only reference。
...
1. ✅ **L1 業務要求**に **BR-22** 追加 (自前 runtime 内部資産体系を持つ、HELIX 資産を HELIX 用に再構築) + §7 OT-46 + §9 carry (A-79、2026-06-02 OT 衝突是正)。
2. ✅ **L1 機能要求**に **FR-L1-46〜49** 追加 (roster / skill pack curate / command CLI 化 / drift lint、BR-22 trace) + **L3 carry** で R1 被覆 (A-79)。当時の fr-registry-audit / g3-trace で trace 接続を確認し、**2026-06-02 再検証: vitest 85 pass (exit 0)**。現行の FR 件数正本は L1 functional §1。
...
3. ✅ **G1/G3 を内部資産次元で再 readiness** (A-86): intra_runtime_subagent review (code-reviewer checklist) = tl リオープン代替 = **CONDITIONAL PASS (Critical=0)**。機械確認 = g3-trace orphanFrL1=[] (FR-L1-46〜49 が L1↔L3 双方向被覆) / 当時の fr-registry-audit / 2026-06-02 再検証 vitest 85 pass (exit 0)。**PO signoff 待ち** (本 §6 承認ゲート、requires_human_approval)。
