...
mutation_oracle_evidence: "U-CAT1437-001はcurrent identity集合をlegacy route inventoryへ差し替える変異で失敗し、current catalogのentity集合を使用する契約を拘束する。実測値は独立reviewで再確認する。"
...
agent_slots:
  ...
  - { role: tl, slot_label: "TL — #1437のcatalog authority sliceをDB consumer移行から分離" }
review_evidence: []
---
