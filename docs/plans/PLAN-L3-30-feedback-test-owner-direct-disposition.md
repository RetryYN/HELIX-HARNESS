...
- 指定V-pairでoracle/test citationを追加し、L6/L7 PLAN projectionまで閉じる後続obligationを
  manifestへ固定し、freeze packet successorで集約する。
- DB空`plan_id` 27件が0になるまではtest ownership closure完了を主張しない。

## §closure boundary

本PLANが閉じるのは、3 file・27 caseのcurrent sourceと直接authorityを再照合し、L4/L9またはL5/L8の
exactly-one `successor_backprop` routeを確定するL3判断までである。oracle/test citation、
