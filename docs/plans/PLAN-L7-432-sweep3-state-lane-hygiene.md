...
review_evidence:
  - reviewer: codex-independent-reviewer
    ...
    green_commands:
      - { kind: unit_test, command: "npx --no-install vitest run tests/sweep3-state-lane-hygiene.test.ts tests/cli-surface.test.ts", runner: node, scope: targeted, exit_code: 0, completed_at: "2026-07-12T08:40:00Z", evidence_path: tests/sweep3-state-lane-hygiene.test.ts, output_digest: "sha256:4d257a10927bbc8225f0826165059e1beaa43ed6b1515a17962d05c442c83ba9" }
...
`npm run typecheck` / `npm run lint`（Biome 519 files）/ `helix plan lint` / `helix doctor` すべて green。
`npm test:fast` は 2285 passed / **1 failed**（この 1 件が T1 所見そのもの）。
...
- 事象: `npm test:fast` が本 checkout で 1 failed:
...
- T1 解消後、lane 位置に依存せず `npm test:fast` が green。
