...
# PLAN-L7-46: harness.db projection-writer（射影 writer）
...
- `rebuildHarnessDb(repoRoot)` — plan_registry / artifact_registry / model_runs / trace_edges / を含む
...
- 既存射影元: `src/lint/relation-graph.ts` (graph_nodes/dependency_edges 等)、`src/export/document-export.ts`
  (document_export_* projection rows)、`src/lint/verification-profile.ts` (mcp/verification rows) を射影元とする。
