...
## 2026-06-09 L6 completion scope addendum（補遺）

この addendum は L6 completion の現在 scope を読みやすく示す正本であり、G6 readiness については下部の historical body を supersede する。古い function-spec / edge-case-only body は original seed PLAN の背景として残すが、L6 completion の normative source ではない。

### Scope rule（範囲規則）

- L6 completion は original `function-spec` と `edge-case` child PLAN だけに限定しない。
- `docs/design/harness/L6-function-design/` 配下の file を生成または修正するすべての L6 add-design PLAN は、G6 readiness surface に含める。
- `docs/test-design/harness/L7-unit-test-design.md` は、L6 function-design set 全体に対する単一の L7 pair artifact である。
- `docs/design/harness/L6-function-design/fr-unit-coverage.md` は FR coverage guard である。L6 を close する前に、すべての FR-L1 row が 1 つの L6 spec path、1 つの deterministic unit contract、1 つの U-* oracle を持つ必要がある。

### 現在の L6 design inventory

| L6 artifact | owning PLAN | 現在 status | L7 oracle family |
|---|---|---|---|
| `function-spec.md` | PLAN-L6-01 + PLAN-L6-20/21 の amendment | `confirmed` | U-FUNC / U-CORE / U-RULE / U-FR-L1-* |
...
| `module-drift.md` | PLAN-L6-15 + PLAN-L6-26..30 の addendum | `confirmed` + `draft add-feature` | U-MDRIFT / U-FR-L1-49 / U-DDDTDD / U-FR-L1-50 |
...
| `fr-unit-coverage.md` | PLAN-L6-21 | `confirmed` | U-FR-L1-01..50 registry row |

### 追加 requirement coverage

SQLite reference-feedback、search-cost reduction、drive/model logs、skill firing metrics、mechanical checks、feedback loops に関する追加 user requirements は、次の reference により L6 scope に含める。

| 要件 bundle | 要件 source | L5 design source | L6 unit contract source |
|---|---|---|---|
...
G6 audit は、次の input がすべて true の場合だけ開始できる。

- 上記 inventory のすべての artifact が owning `plan:` reference、L7 `pair_artifact`、unit-test-granularity contract substance を持つ。
- すべての owning design/add-design PLAN が既知で、未解決の structural lint violation を持たない。
- L7 unit-test design が、すべての L6 artifact と FR coverage addendum に対応する U-* oracle family を明記している。
- `l6-fr-coverage`、`pair-freeze`、`plan-schedule`、`review-evidence`、typecheck、vitest、doctor が green である。

### L6 final completion rule（最終完了条件）

L6 は G6 audit が pass し、次の final state が true になって初めて complete とする。

- 上記 inventory のすべての artifact が `status: confirmed`、または confirmed PLAN により明示的に superseded されている。
- すべての owning design/add-design PLAN が confirmed で、有効な review evidence を持つ。
- `docs/test-design/harness/L7-unit-test-design.md` が L6 pair scope について confirmed である。
- `docs/governance/gate-design.md` が G6 を PASS として記録し、対応する audit record を参照している。
...
| **class-design** | ② 選択 | **縮退 (skip + reason)**: HELIX core は非 OOP (TS pure functions + zod 値オブジェクト、module-decomposition で class 階層不在を確認)。値オブジェクト/型設計は function-spec §型 に統合。G.13 line 547「非 OOP drive で縮退」に該当 | (function-spec へ統合) | **skip (PLAN 不要)** |
