...
title: "PLAN-L7-305 (impl): loop_iterations rebuild 投影 + verifier-provider-mismatch doctor gate"
kind: impl
layer: L7
drive: agent
status: confirmed
...
agent_slots:
  - role: se
    slot_label: "SE - loop iteration jsonl -> harness.db rebuild 投影"
  - role: qa
...
# PLAN-L7-305: loop_iterations rebuild 投影 + verifier-provider-mismatch doctor gate

## 0. 目的

PLAN-L7-304 (schema slice、Codex) の後続 slice として、PLAN-L7-176/177 §4 carry の残り 2 点を閉じる。

1. `.ut-tdd/state/loop/*.iterations.jsonl` (loop-store の iteration 証跡) を `rebuildHarnessDb`
...
- `.ut-tdd` から HELIX への rename / cutover (PLAN-M-02)。
...
- `bun run vitest run tests/projection-writer.test.ts tests/verifier-provider-mismatch.test.ts
  tests/state-db.test.ts` green、`bun run typecheck` green、`ut-tdd doctor` で新 gate が
  hard gate として集約される。

## 4. carry

- per-iteration cost_usd / recorded_at の実値投影は LoopIterationRecord 拡張 (loop-runner
  契約変更) とセットで後続 slice。
