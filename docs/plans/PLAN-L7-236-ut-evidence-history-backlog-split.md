...
title: "PLAN-L7-236 (troubleshoot): UT evidence history backlog 分解"
kind: troubleshoot
layer: L7
drive: agent
status: confirmed
...
agent_slots:
  - role: aim
    slot_label: "AIM - UT evidence backlog split evidence"
  - role: tl
...
# PLAN-L7-236: UT evidence history backlog 分解

## 0. 目的

IMP-109 は「UT test case/run/result の履歴 projection がない」という観測として残っていた。
...
一方で `docs/design/harness/L5-detailed-design/physical-data.md` §9.4 は、general UT runner ingestion、
...
- general UT runner ingestion / flake history / duration regression projection を IMP-147 として分離する。
