---
plan_id: PLAN-L7-339-p6-release-automation-descent
title: "PLAN-L7-339 (impl): P6 GitHub 運用の L7 降下 — PR レビュー経路検証 / CI auto-fix gate / release automation ADR"
kind: impl
layer: L7
drive: agent
status: confirmed
...
generates:
  - artifact_path: docs/plans/PLAN-L7-339-p6-release-automation-descent.md
...
review_evidence:
  - reviewer: codex-tl
    ...
    verdict: approve
    scope: "PR review route、CI auto-fix gate、release automation decision の L7 pure-function 実装と ADR-008 を確認。GitHub remote apply、tag push、release publish は実行せず、plan/dry-run/gate 関数に限定した。"
    worker_model: codex
    reviewer_model: codex-intra-runtime
    green_commands:
      - kind: unit_test
        command: "npm test tests/pr-review-route.test.ts tests/ci-auto-fix-gate.test.ts tests/release-automation-decision.test.ts --timeout 300000"
        runner: node
        scope: targeted
        exit_code: 0
        completed_at: "2026-07-06T06:18:00+09:00"
        evidence_path: tests/pr-review-route.test.ts
        output_digest: "sha256:6f7415d2fcc717a5fd64847a947e95c5d192e4efa558d68a6949c61652551a12"
---

# PLAN-L7-339 (impl): P6 GitHub 運用の L7 降下

## 実装確認（PO 指示 2026-07-06）

本 PLAN は archive では閉じず、生成物が既に merge 済みである事実に合わせて confirmed とする。
PR レビュー経路検証、CI auto-fix gate、release automation ADR は L7 pure function / ADR として実装済み。
GitHub remote apply、実 tag push、release publish、CODEOWNERS 実配備は action-binding approval が
揃うまで実行しない。
...
- U-PRROUTE / U-CIFIX / U-RELDEC の各 oracle が green であること
  （検証コマンド: `npm test tests/pr-review-route.test.ts tests/ci-auto-fix-gate.test.ts tests/release-automation-decision.test.ts`）。
- `npm run typecheck` green、`npm run lint` green、`helix doctor` exit 0。
...
|---|---|---|---|
| CI auto-fix gate | 精緻化 | HR-FR-P6-02 の confidence 閾値付き自動修正 re-push 判定の L7 実装名 | 導入層 L3、L7 実装到達を追記 |
