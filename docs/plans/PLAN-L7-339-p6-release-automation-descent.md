...
current completion scope から archived とする。PR レビュー経路検証、CI auto-fix gate、
release automation ADR は未実装の active L7 frontier として残さず、GitHub remote apply、
実 tag push、release publish、CODEOWNERS 実配備は実行しない。

将来 P6 GitHub 運用の L7 実装を再開する場合は、新規 PLAN で current snapshot、dry-run、
rollback、approval scope、test evidence、action-binding approval を取り直す。この archive record を
実装完了または remote 適用承認として扱わない。
