...
| 1a | blast-radius audit CLI | `auditIdentifierRenameBlastRadius` + `ut-tdd rename audit --json` で旧 token 残渣と approval 不足を機械出力 |
| 1b | non-destructive cutover packet | `ut-tdd rename plan --json` で rename map、dry-run、rollback、monitoring、state backup manifest、freeze policy、provenance requirements、approval gate を出す。apply は提供しない |
