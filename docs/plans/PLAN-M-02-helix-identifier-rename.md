...
title: "PLAN-M-02 (migration): 旧機械識別子 → 現行 HELIX 識別子 atomic 改名 (L1 完了境界 trigger)"
kind: design
layer: L14
drive: fullstack
status: draft
...
# PLAN-M-02 (migration): 旧機械識別子 → 現行 HELIX 識別子 atomic 改名
...
- 進捗: ☑ partial — `src/lint/identifier-rename.ts` + `helix rename audit` で `helix` / `.helix` /
  ...
  `blocked_pending_cutover_approval` とし、旧 state path から現行 `.helix` への apply 可能とは扱わない。`helix rename plan`
...
### Step 3: [直列] 旧 state path → `.helix/` 移行 (不可逆)
...
現行 CLI と改名後 CLI の doctor green（**rule-drift 含む**）/ full vitest / **compiled dist smoke**（rootpath-gotcha 再発防止）/ cross-agent review。
- 進捗: ⬜
...
- decision_outcome: `pending`。CLI/bin rename、state dir move、adapter marker/hook rename、distribution surface cutover は PO cutover approval 待ちであり、archive で閉じない。これは cutover approval ではない。
...
- workflow_route_impact: pending-cutover。state dir / CLI / adapter marker cutover は本 PLAN で追跡し、current snapshot、dry-run、rollback、backup、monitoring、action-binding approval を揃えてから判断する。

action_binding_approval_record:
<!-- action binding 承認記録 -->
- allowed_outcome: `approve_action_binding` / `deny_action` / `request_scope_reduction`
- decision_outcome: `pending`。不可逆 cutover action は未承認であり、approval record が揃うまで実行しない。
...
- 旧名 alias / shim の要否は Step 6 review で判断（後方互換の一時 shim か即時切替か）。
