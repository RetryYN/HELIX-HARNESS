...
- 進捗: ☑ partial — `src/lint/identifier-rename.ts` + `helix rename audit` で `helix` / `.helix` /
  ...
  `blocked_pending_cutover_approval` とし、`旧 state path -> .helix` apply 可能とは扱わない。`helix rename plan`
...
- adoption_decision_delta: none; `旧 state path -> .helix` は concrete な cutover approval evidence が揃うまで approval-gated かつ plan-only のままにする。
