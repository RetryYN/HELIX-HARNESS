---
plan_id: PLAN-L5-90-worker-output-admission
title: "PLAN-L5-90 (add-design): worker output admission詳細設計"
kind: add-design
layer: L5
drive: agent
status: confirmed
...
contract_preconditions: "PLAN-L4-64がL9境界とcomponentを固定する"
contract_postconditions: "schema、envelope、failure、resource、state transitionが実装可能になる"
contract_invariants: "strict既定、unknown key deny、dynamic digest exact、capability 0 on failure"
contract_failures: "7 failureをexecutable oracleとmutationへ束縛する"
tdd_red_required: false
complexity_effect: net_negative
complexity_justification: "literalを含むclosed AST 7 node種と1 built-in schemaだけを採用する"
removal_trigger: "not_applicable"
pair_artifact: docs/test-design/helix/L8-worker-output-admission-unit-test-design.md
agent_slots:
  - { role: aim, slot_label: "AIM — schema／failure contract" }
...
generates:
  ...
  - { artifact_path: docs/test-design/helix/L8-worker-output-admission-unit-test-design.md, artifact_type: test_design }
dependencies:
  parent: docs/plans/PLAN-L4-64-worker-output-admission.md
  blocks:
    - docs/plans/PLAN-L6-98-worker-output-admission.md
left_arm_carry:
...
# PLAN-L5-90: worker output admission詳細設計

schema AST、canonical envelope、failure exact set、bounded evaluationを固定する。
