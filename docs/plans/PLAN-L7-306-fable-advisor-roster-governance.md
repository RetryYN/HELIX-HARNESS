...
# PLAN-L7-306: Fable advisor 追加 + roster モデル SSoT ガバナンス
...
1. **Fable advisor**: TL advisor (HLX-FR-02) の一段上の second-opinion advisor
...
- guard: `SUBAGENT_ALLOWLIST` へ追加、`normalizeModelFamily` に fable family を追加
...
- `bun run vitest run tests/agent-model-ssot.test.ts tests/model-id-ssot.test.ts
  tests/agent-guard.test.ts` green、`bun run typecheck` green、`ut-tdd doctor` で
