...
成立した。Forward PLANの終端stateを先行PRへ分離したうえで、本PLANをconfirmed、
`backfill_state: complete`、`completion_claim_allowed: true`へ遷移する。本HEADはfresh CIと
exact-HEAD reviewで再封緘し、canonical merge後のmain read-afterまでIssue #1125をcloseしない。
