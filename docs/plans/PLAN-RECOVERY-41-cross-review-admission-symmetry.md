...
contract_invariants: "v2 bytesはhistorical decodeとして不変に維持する。current v3のdigestはmodel fieldsを含む。新workflow・service・DB table・required check名を追加しない。stale HEAD、事後発行、別PR、別repository、重複receipt、approve+blocker、DB非収束の拒否は現行のまま維持する。branch protection設定変更は本責務に含めない"
