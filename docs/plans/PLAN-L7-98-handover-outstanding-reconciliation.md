...
# PLAN-L7-98 (troubleshoot): handover §5 outstanding reconciliation の整合化
...
## 1. 問題 (PO「なぜ実在しない判断が残っているのか」)

§5「未了 PO 判断」は handover で**唯一、機械裏付けが無いセクション**だった:

- `§1/§2` は `helix handover` が毎回 real state から再生成 = 自己訂正する。`§3-§6` は人手 prose で、
...
### (A) §5 machine-seed の追加 (`src/handover/index.ts` renderHandoverScaffold)
...
### (B) fail-close anchor gate の追加 (`checkHandoverOutstandingAnchor` → doctor)
...
- [x] `helix handover` が §5 に outstanding marker を seed する (実 repo で確認: non-terminal PLANs=0;
  open defers=1)。
...
- [x] test: U-HOVER-017 (seed marker / 後方互換 / pointer skip / marker ok / marker 不在 fail / 複数 entry
  は最後の §5)。
- [x] typecheck / Biome / Vitest / doctor (handover-outstanding OK) / db rebuild が green。
...
- **「open defers=N」を特定 PLAN に紐付けるときは実体を確認** (specBackfillWaits か explicit §Deferred か)。
  ...
  (`[[feedback_verify_carry_status_against_code]]`)。
- **PLAN 追加/status 変更後は `helix db rebuild`**。
