...
- 上流の helix→helix リネーム由来で upstream-only に見える正本 doc（concept/requirements/ADR 等）は LOCAL に helix 名で実在＝対象外。
