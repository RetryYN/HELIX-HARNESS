...
title: "PLAN-L7-95 (troubleshoot): lint-wiring meta-gate — 死蔵ルール (reachable/tested だが audit 未実行) を fail-close 化し、4 inert lint audit を doctor へ配線する (IMP-006、IMP-033 配線部分の discharge)"
kind: troubleshoot
layer: L7
drive: agent
status: confirmed
...
backprop_decision_reason: "harness 自己適用の内部 tooling (lint gate / runtime dispatch / guard / governance mechanism)。harness 自身の enforcement を強化する変更であり、product 外部の requirement / design / test-design contract は変えないため、upstream backprop 対象はない。"
owner: PM (Opus) / PO (人間)
review_evidence:
...
# PLAN-L7-95 (troubleshoot): lint-wiring meta-gate と inert-lint audit 配線
...
- 既存ゲート群は「ルールが配線されているか」という一段メタな不変条件を**構造的に見ていなかった**
  (どの gate も「不在」を違反にできない absence-blindness。関連:
  [[project_descent_absence_blindness]] / [[feedback_coverage_not_substance]] を参照)。
...
### (A) lint-wiring meta-gate の追加 (`src/lint/lint-wiring.ts`、IMP-006)
...
- `DEFERRED_LINTS` = `{ tool-adapter: adapter-probe 純関数ライブラリ、`helix adapter` 統合は IMP-033 / L7-50 R8 で deferred }`。
...
- [x] 検証: typecheck / Biome / Vitest / doctor / db rebuild はすべて green。
...
- **PLAN 追加/status 変更後は `helix db rebuild`** (plan-registry-fingerprint stale 回避。関連:
  [[project_codex_branch_ci_verification]] を参照)。
