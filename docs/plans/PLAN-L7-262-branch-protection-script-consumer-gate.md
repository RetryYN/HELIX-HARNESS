...
backprop_decision_reason: "consumer setup が生成する branch protection script を approval-only として検査する実装強化。GitHub branch protection apply は実行しない。"
owner: TL (Codex)
...
review_evidence:
  - reviewer: codex-tl
    ...
    verdict: approve
    scope: "subagent 指摘に基づき、consumer setup が生成する branch protection script を setup readiness / consumer doctor / distribution acceptance で approval-only として検査するようにした。gh auth / gh api mutating endpoint を許可しない。"
    worker_model: codex
...
- `gh auth status`、`gh api -X PUT`、branch protection / ruleset apply、外部 GitHub 書き込みは実行しない。
- action-binding approval 記録、GitHub settings 変更、secret 操作は行わない。
- `PLAN-M-02` の rename/cutover approval と `.helix` 実移行は扱わない。

## 完了条件

- setup readiness が mutating branch protection script を green にしない。
