...
backprop_decision_reason: "POの明示指示『ハイブリッド設計ドキュメントv1-fixed.zipを正本』をL3設計承認として記録した。runtime cutoverは別のaction-binding approval境界に残す。"
...
6. 2026-07-13 PO明示指示をL3設計承認として`confirmed`化し、後続L4 architecture/L5 migration/L7 implementationへ分割する。
...
- `allowed_outcome`: `confirm_l3_canonical_authority_design`
- `approver`: PO（2026-07-13 /goal の明示指示）
- `scope`: ZIPを設計意味の正本とし、repo-owned抽出物を実行可能なcanonical sourceへ固定するL3判断
- `excluded`: runtime enum/state path、CLI、hook、consumer/distributionの実cutover、release/tag/publish
- `next_route`: L4-L6のVペア設計。不可逆適用はaction-binding approvalまでfail-closeを維持
