...
PLAN-M-02 は `.ut-tdd` から HELIX への不可逆 cutover を承認待ちにしている。既存 packet は
...
- `ut-tdd rename plan --json` の `verificationCommandMatrix[]` に `repository-redirect-review` を追加する。
