...
generates:
  ...
  - artifact_path: src/runtime/skill-memory-hygiene.ts
...
review_evidence:
  - reviewer: codex-tl
    ...
    verdict: approve
    scope: "PLAN-L7-369 skill and memory hygiene。skill hygiene dry-run surface を追加し、skill firing/acceptance telemetry、削除しない reversible quarantine、before/after evidence 必須の improvement gate、memory provenance retention を機械表示した。"
    worker_model: codex
