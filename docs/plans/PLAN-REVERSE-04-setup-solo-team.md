...
2026-07-01 continuation: `helix setup project` は HELIX 導入済み VSCode で新規 project を始めるための bootstrap entrypoint であり、既存 `helix setup` の Phase 0 project setup 境界に属する。追加される `.vscode/tasks.json` / `.vscode/settings.json` / `.helix/memory|handover|evidence|teams` baseline は adapter/runtime-state 投影の具体化であって、新しい L1/L3 product FR、DB/API contract、不可逆 `.helix -> .helix` cutover ではない。`identifierTransition` は現行 `.helix` baseline と将来 `.helix` 目標を同じ payload に出し、PLAN-M-02 cutover/action-binding approval までは `blocked_pending_cutover_approval` / `mustNotApply=true` を返す。`postSetupWorkflow` は fresh/brownfield/readiness の意味分岐を first-run route として構造化する設計補強であり、manual doc search なしに次 action を選べるようにするための上位整合である。
