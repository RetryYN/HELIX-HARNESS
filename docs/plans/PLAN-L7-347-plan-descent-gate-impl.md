...
review_evidence:
  - reviewer: code-reviewer
    ...
    green_commands:
      - kind: unit_test
        command: "npx --no-install vitest run tests/plan-descent.test.ts"
        runner: node
        scope: targeted
        exit_code: 0
      ...
      - kind: lint
        command: "npx --no-install tsx src/cli.ts plan lint --gate descent"
        runner: node
        scope: full
        exit_code: 0
        completed_at: "2026-07-06T09:06:48+09:00"
...
1. 一時的に空 baseline で `analyzePlanDescent` を全 PLAN に対し実行し、違反 plan_id 一覧を
   ...
   生成コマンド例: `npx --no-install tsx src/cli.ts plan lint --descent-baseline-write`（CLI option の追加が
...
- `npx --no-install vitest run tests/plan-descent.test.ts` green（U-PDESC-001..010）。
- `npm run typecheck` green。
- `npx --no-install tsx src/cli.ts plan lint` green（baseline 生成後、全 PLAN で新規違反 0）。
- `npx --no-install tsx src/cli.ts doctor` に `plan-descent` gate が現れ green。
