...
**現行security policy**: security-foundation readiness（隔離sandbox、network/secret deny、broker、監査receipt）が
greenになるまでKimiを起動しない。readiness後はadmission判定専用controlled sandbox benchだけを許可し、
production／通常worker laneはS4用途別admission後だけ許可する。過去S2実行はpre-policy historical evidenceであり、
現行readiness/admission証拠に使わない。readinessまで将来のKimi lane作業はClaude Codeが同じtyped contractで代行する。
Codex が orchestrator、Terra が independent reviewer、Codexが最終検収を担う。
PoC採用時はP1 Feature Issueへ接続し、正規Forward PLANは駆動モデル別generatorだけで作成する。

## S1: 固定する計画

- historical委譲面: `kimi -p <prompt> --output-format text|stream-json`（非対話）。ACP（`kimi acp`）は
...
## historical完了条件（pre-policy S2済み / 現行S3-S4未了）
...
## security-foundation後の後続

- security-foundation readiness（sandbox template、network/secret deny、broker、監査receipt）を先行Featureでgreenにする。
- readiness後のcontrolled sandboxでfull bench（blind judge、mutation kill、skill A/B、実 task scorecard）とS4用途別採否を決定する。
- `helix kimi`通常laneとProposal Revalidation GateはS4用途別admit後のForwardで実装する。
- 該当 L1/L3 要件の confirm（PO 承認境界）。
- **Claude 代行 PoCからのFeature接続**: sandbox完成までの代行結果は本PLANのS4証拠として扱い、採用時は
  P1 Feature Issueへ接続する。実装PLANを任意pathへ直接作らず、Forward generator経由で起票する。
