...
- R1. **dogfood 非搭載**: GitHub から pull する配布物に、このリポ固有の自己適用成果物
  (`docs/plans/PLAN-*`, `docs/design/harness/`, `docs/test-design/`, `.helix/`, `docs/handover/`) が**含まれない**。
- R2. **画面なし**: 中央UI / 画面 (src/web, L7-141/146)、web 専用テスト、frontend asset / dashboard route / frontend dependency residue は配布物に**同梱しない** (backend-first、フロント後回しは PO 確認済)。
- R3. **別PCで使える + 途中導入 (brownfield)**: pull → `helix setup` → engine が起動し動く。**空の project に限らず、
...
- R5. **非破壊な導入・更新 (PO 2026-06-25「プロジェクトを破壊して更新するのはダメ」)**: 導入も更新も **consumer の
  ...
  (`docs/plans`/`docs/design`/`src`/`tests`/`.helix`/custom adapter) を上書き・消去しない。
...
- R10. **バージョニング / 互換契約**: **semver + tag-pin channel + CHANGELOG/migration note**。engine が consumer に晒す
  **公開契約** (cross-review で拡張) = {CLI surface / adapter marker 形式 / `.helix` state schema / **hook event schema /
...
- R17. **monorepo / workspace root 判定 (cross-review 追加)**: consumer の **repo root ≠ package root** の場合に
  adapter / CI / `.helix` / hook path / tag-pin の配置先を誤らないよう、root 判定 (workspace/subdir project) を定義する。
...
- **D-D. 非破壊な導入/更新の方式 (R3 brownfield + R5)**: 導入と更新を **同一機構** (managed 区画のみの冪等投影) で扱う。
  ...
  **書き込み前に必ず dry-run diff**。3-way / detect-and-preserve のいずれにするか確定。
  ④ **`.helix/` 内部境界 (cross-review Critical 修正)**: `.helix/` を一括 consumer 成果として扱うと更新/uninstall で
...
- **D-F. 環境 preflight の方式 + hook path 解決 (R6)**: ① **既定 = detect + instruct** (Bun/git/gh/runtime CLI の
  ...
  ② 既存 `src/runtime/detect.ts` / `adapter.ts` (spawnability 判定、`HELIX_*_BIN`、`where.exe`) を再利用しモード確定。
  ...
  既知事例)。⑤ Windows-native first-class / WSL2 optional、PowerShell⇔bash の entrypoint (`scripts/helix.ps1` / `.sh`)。
- **D-G. バージョニング / 契約安定化 (R10)**: semver 規則と tag-pin bump 手順を確定。**互換契約 = {CLI surface /
  adapter marker schema / `.helix` state schema}** に **contract test (golden)** を張り、version 間で互換 or migration 付き
...
- **D-H. サプライチェーン / 信頼 (R12)**: release の**完全性検証** (gh attestation / sigstore / checksum) と出所明示。
  **検証主体と root of trust を明示 (cross-review 追加)**: consumer が pull 後に**何を・どの root of trust で検証し・
  ...
  実行する事実**を透明化し、既定は最小権限・監査残し。`HELIX_ALLOW_*` 系 bypass の監査が consumer 側でも継承される
...
## 3. 実装範囲 (Scope / 実装コンポーネント)

1. **精選 export (curated export) 機構**: clean artifact を生成し、dogfood 除外を機械化。allowlist/denylist を doctor で
   fail-close 検証 (誤って dogfood が混入したら止める)。
2. **adapter テンプレ作成 (author) + `helix setup` の adapter 投影 (冪等・非破壊)**: まず **投影元の clean テンプレ
...
3. **導入導線 (install)**: engine を別PCで使える状態にする (build/install path、`package.json` `private` 解除 or
   git-install 設定、`dist/helix` 生成の prepare/postinstall or release binary)。
4. **可搬性 (portability) + 途中導入検証**: clean clone → setup → `helix doctor` が **dogfood 無しで** green になる smoke、
   および **既存(非空)project へ setup しても既存ファイルが破壊されない** smoke。
5. **非破壊な導入/更新機構 (R3/R5/D-D)**: `helix setup` (初回も再実行も) = 冪等な managed 区画投影とし、
...
9. **consumer 向け docs + LICENSE + release 完全性 (R11/R12/R13/D-H/D-I)**: quickstart/install/update/uninstall 手順 +
...
- AC1. clean artifact に R1 の dogfood (PLAN-*/design.harness/test-design/.helix/handover) が**含まれない** (機械検証、混入で fail-close)。
- AC2. clean artifact に中央UI/画面、`src/web/`、web 専用テスト、frontend asset / dashboard route / frontend dependency residue が含まれない。core CLI は web module を静的 import せず、画面なし artifact でも `status` / `distribution plan` / `typecheck` が通る。
- AC3. pull → `helix setup` → engine 起動 + `doctor` が consumer project (空・非空とも) に対して green。
...
- AC6. **非破壊更新 (R5)**: 更新 (engine bump + `setup` 再投影) を走らせても consumer の project 成果
  (`docs/plans`/`docs/design`/`src`/`tests`/`.helix`/custom adapter 編集) が破壊されない。機械検証 =
...
- AC12. **契約互換 (R10/D-G)**: contract test (CLI surface / adapter marker / `.helix` state schema) が version 間で
...
- **導入も更新も consumer プロジェクトを破壊しない** (R3/R5)。途中導入 (既存 project) でも作りかけ更新でも、adapter は
  managed-section marker 経由のみ投影し、marker 外の consumer 編集・既存ファイル・consumer の docs/src/tests/`.helix`
...
- **cross-review 実施 (Codex / gpt-5.5、hybrid 判断ゲート、`helix codex --role tl`、2026-06-25)** = **判定 no-go**。
  task/出力の監査証跡 = `.helix/review/cross-review-l7-157.md`。findings と disposition は §9。
...
## 9. cross-review findings (Codex gpt-5.5, no-go) と対応方針 (disposition)
...
- **Critical (4) — 全て本 draft へ反映済**:
  ...
  - C3 `R1/D-C/AC1/AC3`: dogfood 除外と doctor green の同時成立根拠不足 → **修正**: D-C に「engine 必須の最小公開セット明示」を
    追加 (= no-go 主因、**要 PO 確定**)。
  - C4 `R5/R9/D-D`: `.helix` の managed/consumer-owned 境界欠如 → **修正**: D-D④ に内部境界を追加。
...
- **Minor (4) — disposition**:
  ...
  - M2 `R2/AC2` 「画面なし」が `src/web` 除外のみで弱い (web assets/dashboard routes/frontend deps/Playwright residue) → **解決 (PLAN-L7-223)**: `src/web/` と `tests/web.test.ts` を clean artifact から除外し、CLI の web command は optional dynamic load にして core typecheck を維持。
...
- **C. 導入 (install) / 起動導線**: ① engine install path (`private` 解除 or git-install、dist 生成 prepare/postinstall or
...
- **G. バージョニング/契約**: ① semver + tag-pin + CHANGELOG/migration `R10/D-G` / ② 互換 contract test
  (対象: CLI surface / adapter marker / `.helix` state schema) `R10/D-G/AC12`。
