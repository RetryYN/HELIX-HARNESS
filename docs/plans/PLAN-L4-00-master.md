...
roadmap:
  ...
  gates:
    - id: G-DESIGN.L4
      name: L4 基本設計 freeze
      exit_criteria: "L4 基本設計 sub-doc (architecture/function/screen/data/external-if/ui-standard+tokens + 内部資産拡張 roster/skill-pack/drift-lint + HELIX pillar basic design) が全 child PLAN で confirmed、L4↔L9 V-pair freeze 整合、G4 PASS 台帳と一致"
  spans:
...
| DB 有無 | `.helix/` YAML/JSON state + `.helix/harness.db` SQLite projection DB (ADR-001) | data sub-doc が state schema + projection feedback DB + ドメインモデルを担う |
...
| **ui-standard** | ② 選択 (UI 有時) | **起票済** (L2 UI 要素から L4 FE 設計標準 / UI 部品カタログ / design tokens へ降下。L10 誤配置を是正) | **PLAN-L4-14-ui-standard** |
...
| **HELIX pillar basic design** | ① 必須 (HELIX solo conversion 追補) | **起票済** (`HR-FR` 30 + `HR-NFR` 13 を HELIX 名前空間の L4 block へ全件降下。既存 harness L4 を上書きしない) | **PLAN-L4-51-helix-pillar-basic-design** |
...
5. **PLAN-L4-14-ui-standard** → L2 UI 要素から L4 FE 設計標準 / UI 部品カタログ / design tokens を確定し、L9 system test design の ST-UI で pair する。
...
- [x] 各 child が L9 総合テスト設計と pair_artifact 接続 （層意味論: canonical L9=integration）
...
- [x] **A-100 park → A-101 正規式 G4 再確定 (2026-06-05)**: 旧 A-91 (内部資産含む COND PASS、正規式前) は **historical**。RECOVERY-02 正規式モデル確定後、A-100 で G4 を park。**A-101 で core 4 doc ⇔ L9 を正規式 V-model (L4⇔L9 総合) で G4 audit → 4 軸 PASS** (intra_runtime_subagent = pmo-sonnet、TL 代替)。L4 4 doc + L9 + PLAN-L4-00〜04 を confirmed。**内部資産 L4-10〜13 は別スコープで未 freeze** (ST-ASSET、L6/L7 carry)。記録 = gate §2 台帳 A-101 / `.helix/audit/A-101-g4-l4-freeze.md`
- [x] **A-102 G4 add-design freeze (workflow オーケストレーション、2026-06-05)**: A-101 後の粒度監査で確定した function §3 の under-design (workflow mode 群 + FR-12 skill = 外部設計判断なし) を **add-design PLAN-L4-05** で解消。function §3 を Forward spine + 9 駆動モデル + 2 工程専門の外部設計へ deepening + L9 ST-FUNC ペア + IMP-069 (mode taxonomy reconcile) / IMP-070 (commander ADR-006) 解消。**4 軸 PASS** (code-reviewer、I-1/I-2 修正後)。記録 = gate §2 台帳 A-102 / `.helix/audit/A-102-g4-workflow-orchestration.md`
- [x] **PLAN-L4-14 FE 設計標準 freeze (2026-06-25)**: `ui-standard.md` と `tokens.yaml` を L4 FE 設計標準として confirmed。`VALID_SUB_DOCS[L4]` / document-system-map / 要件 mirror を同期し、`frontend-design-coverage` + `sub-doc-catalog-drift` で drift 0 を機械検証。L9 `ST-UI-*` で ui-standard/tokens の system 観測を追加。
- [x] **HELIX pillar L4 add-design freeze (2026-06-28)**: `PLAN-L3-06` で confirmed になった `HR-FR` 30 件 + `HR-NFR` 13 件を **PLAN-L4-51** で L4 basic design 10 block と L9 system test 43 観測へ全件降下。既存 harness L4 は上書きせず、HELIX 名前空間の additive design として `docs/design/helix/L4-basic-design/pillar-basic-design.md` / `docs/test-design/helix/L4-pillar-system-test-design.md` を追加。`PLAN-L4-50` は L6-50 に superseded 済みの confirmed historical record として後継 trace を保持。
