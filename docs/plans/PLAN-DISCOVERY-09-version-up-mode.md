...
review_evidence:
  - reviewer: codex
    ...
    scope: "version-up 駆動モデル (frontmatter version_target = status=draft 限定 + ledger 照合 / forward-convergence の version-up-parked bucket / outstanding 分離 / UI 141/146 適用) を別 runtime (Codex gpt-5.5, role=tl) が desk review。verdict=APPROVE-WITH-CHANGES (Critical 0、証跡 .helix/review/cross-review-versionup-and-s4-failclose.md)。Critical (version_target status=draft 限定 / landing 除外禁止 / ledger label) 反映済。正本 back-merge = PLAN-REVERSE-140。"
    green_commands:
      - kind: unit_test
...
- decision_owner: PO (2026-06-26 version-up mode の依頼と採用判断)
- decision_basis: version-up mode は、将来実施を約束した作業を plain draft/archived/add-feature から分離し、status/outstanding と forward-convergence の挙動で検証済み。
- verified_evidence: review_evidence green_commands、PLAN-DISCOVERY-09 §4 ACs、PLAN-L7-211-version-up-readiness-gate、PLAN-REVERSE-140、および現在の再検証 command `bun run src/cli.ts doctor` / `bun test tests/s4-decision-readiness.test.ts --timeout 180000`。
- stakeholder_review_or_proxy: Codex/gpt-5.5 cross-runtime TL review を review_evidence に記録済み。
- acceptance_gap: PLAN-L7-146 は parked のままで、将来の activation/action-binding approval が必要。この状態は隠れた完了ではなく明示 blocker として保持する。
- unresolved_risk: 将来の activation が infra/auth/secrets に触れる場合は、dry-run/rollback evidence 付きの人間承認を必須にする。
- external_source_basis: docs/process/modes/discovery.md、docs/process/modes/scrum.md の S4 decision rules、および docs/process/modes/version-up.md。
- source_ledger_freshness: fresh。S4 decision source ledger は 2026-07-03 に discovery/scrum mode docs で現監査中に確認済み。
- source_status_delta: changed。ISO/IEC/IEEE 29148 は現在 2026-02-16 stage 90.92 to be revised を示すが、過去の confirmed S4 decision は再オープンしない。
- adoption_decision_delta: none。過去の S4 adoption decision は現在の route policy と整合したままで、ISO/IEC/IEEE 29148 の改訂は出版まで追跡する。
- workflow_route_impact: none。過去の decision はすでに S4 と Reverse/Forward evidence を通っている。
- route_impact: confirmed により version-up を parked future-work mode として追加し、parked work を active completion の外に保つ。
- forward_route: PLAN-REVERSE-140-forward-convergence-version-up-backfill -> version-up mode docs と readiness gates。
- reverse_fullback_required: yes。PLAN-REVERSE-140-forward-convergence-version-up-backfill で discharge 済み。
- promotion_strategy_or_rejection_pivot_rationale: reuse-with-hardening。mode を維持し、activation/readiness boundaries を harden する。
...
| outstanding との関係 | `helix status` で「active draft」と「version-up parked」を分離表示 (将来版保全を WIP と混同しない) |
...
- `helix status` outstanding を「active draft / version-up parked」に分離集計 (report-only)。
...
- **AC-4 無回帰**: `helix doctor` exit 0 / `bun run test` 全量 green / `helix plan lint` / biome / typecheck 通過。
