...
title: "PLAN-L3-06 (add-design): HELIX L1 pillar requirements -> L3 FR/AC descent"
kind: add-design
layer: L3
drive: agent
status: confirmed
...
review_evidence:
  - reviewer: PO
    ...
    verdict: approve
    scope: "G-REQ.L3 human approval for HELIX pillar L3 descent. Evidence: vmodel-pair, plan lint, typecheck, lint, db rebuild, and doctor roadmap showing G-REQ.L3 reached."
    green_commands:
      - kind: unit_test
...
- 進捗: 部分完了。`vmodel` loader を HELIX docs も読むよう拡張し、`tests/vmodel-pair.test.ts`
  ...
  の全件被覆を regression guard 化した。この source/test 変更は既存の確認済み verification trigger 面
...
G-REQ.L3 承認時に PO/TL が確認した evidence。全項目は confirmed 正本の承認根拠として保持する。
...
| 既存 P2/P7 back-fill と重複しない | `pillar-functional-requirements.md` §3、既存 L3 back-fill 3 本の `pair_artifact` 接続 | ready |
...
| objective 要求 | L3 evidence | L12 / test evidence | 判定 |
|----------------|-------------|---------------------|------|
| L3 設計をすべて精読し、L1 要求との差異を埋める | `L3 精読 audit`、`pillar-functional-requirements.md` §0/§3、既存 back-fill 3 本 | `U-VPAIR-005b` / `U-VPAIR-005g` | ready |
...
| check | 承認観点 | evidence |
|-------|----------|----------|
| scope | L1 `HBR-*` / `HNFR-*` 全件が L3/L12 に降下し、harness confirmed L3 を上書きしていない | `L3 精読 audit`、`Objective completion audit`、`U-VPAIR-005b/005g/005h` |
| completeness | 43 L3 要件、86 HAC、43 HAT が孤児 0 で接続されている | `U-VPAIR-005c` |
| safety | external data / prompt injection / tool injection / exfiltration / excessive agency / action-binding approval が L3 AC に入っている | `HR-FR-P8-04`、`HR-NFR-P8-02`、`HR-NFR-P8-03`、`HR-NFR-AC-03` |
| operating model | PLAN 駆動であり、provider API direct call / SDK 常駐実行を前提にしない | `HR-FR-P2-04`、`HR-NFR-AC-03`、`U-VPAIR-005f` |
| evidence freshness | 2026-06-28 external/best-practice source audit を保持し、CISA official source の bot-blocked 403 を過大に HTTP 200 と主張しない | readiness audit、`U-VPAIR-005d/005e` |
| false-green prevention | PO 承認前は draft / pending approval / doctor non-zero を維持し、承認後は confirmed / G-REQ.L3 reached を doctor で確認する | `PLAN-L3-00-master` G-REQ.L3 roadmap、doctor roadmap rollup |
...
- `bun run vitest run tests/vmodel-pair.test.ts` は `U-VPAIR-005b/005c/005d` を含めて green。
...
- [x] targeted trace checks / typecheck / targeted vitest green。
