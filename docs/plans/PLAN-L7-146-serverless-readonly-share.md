...
- 非終端 (draft) のまま残るが、current completion scope の active frontier ではなく future-version backlog として扱う。
...
- archive_route: none。archive しない。activation 時は add-feature / version-up route で再開する。
