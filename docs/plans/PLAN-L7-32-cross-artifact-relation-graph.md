...
- **実装内容 (discharge)**:
  ...
  - **CLI surface** = `ut-tdd graph impact --changed <path...>` (loader → `collectRelationGraphProjection` → `analyzeRelationImpact`、changedNodes/impacted/actions/findings 出力、error finding で exit 1) / `ut-tdd graph export --format mermaid|dot|d2 [--scope]` (→ `exportRelationDiagram`、mermaid 常時 / dot・d2 は純粋テキスト emit、外部 renderer は起動しない)。`src/cli.ts` の `graph` command group。
