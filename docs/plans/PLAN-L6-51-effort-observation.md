...
review_evidence:
  - reviewer: codex-tl
    ...
    green_commands:
      - kind: unit_test
        command: "bun test tests/effort-observation.test.ts tests/orchestration/loop-bridge.test.ts --timeout 300000"
        runner: bun
        scope: targeted
        exit_code: 0
...
- `bun run src/cli.ts plan lint docs/plans/PLAN-L6-51-effort-observation.md` green。
