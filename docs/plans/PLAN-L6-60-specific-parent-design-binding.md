...
review_evidence:
  - reviewer: codex-tl
    ...
    green_commands:
      - kind: lint
        command: "npx --no-install tsx src/cli.ts plan lint docs/plans/PLAN-L6-60-specific-parent-design-binding.md && npx --no-install tsx src/cli.ts plan lint --gate governance"
        runner: node
        scope: targeted
        exit_code: 0
...
- `npx --no-install tsx src/cli.ts plan lint docs/plans/PLAN-L6-60-specific-parent-design-binding.md` が green。
- `npx --no-install tsx src/cli.ts plan lint --gate governance` が green。
