...
workflow_identity:
  ...
  target_id: RECOVERY
entry_signals:
  - "po_directive:Issue #1216 evidence write commandのNode engine authority迂回を復旧する。PR #1140の初回receipt拒否はsource_head不一致であり、Node runtime差へ帰責しない"
created: 2026-08-29
updated: 2026-08-29
owner: Codex / TL
...
contract_preconditions: "doctorはNode範囲外を拒否するがpr-review-receiptが同じgateを通らず、engines.node範囲外のNode22でもevidence writeを開始できる。PR #1140で観測した初回receipt拒否はsource_head不一致であり、受理receiptも同じNode22で生成されたため、Node差によるDB digest分岐を実測事実として扱わない"
...
mutation_oracle_evidence: "2026-08-29T18:37:42+09:00にpr-review-receipt actionからassertNodeEngineRuntimeAuthority callを除去するmutationを実測し、U-NODEENG-007が1 failed／6 passedでkillした。Claude exact-HEAD reviewで宣言欠落をassert境界だけ黙認するmutationがsurviveしたため、U-NODEENG-006へpackage.json engines.node欠落反例を追加した。2026-08-29T20:39:08+09:00にassert境界の欠落fallbackを>=0.0.0へ変えるmutationを実測し、U-NODEENG-006が1 failed／6 passedでkill、復元後U-NODEENG-001..007は7 passed。U-NODEENG-006は範囲外v22.23.1と宣言欠落を固有failureでthrowし、宣言済みv24.15.0を受理する。"
...
PR #1140の初回review receiptはcandidate HEADではないworktreeから生成され、`source_head`不一致で
merge admissionに拒否された。初回・受理receiptはいずれもNode22で生成され、受理receiptはNode24 CIの
logical DB receiptと一致したため、拒否をNode差によるdigest分岐へ帰責しない。一方、その調査で
`engines.node >=24.15.0 <25`に反するNode22からevidence write commandを開始できる独立したauthority迂回を
実測した。本PLANはDB比較を緩めず、範囲外runtimeによるreceipt writeを入口で拒否する。
...
| 5 | Claude exact-HEAD reviewとcanonical convergence | [review] | blocker 0、Ready merge成立 |
