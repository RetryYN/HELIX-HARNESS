...
review_evidence:
  - reviewer: code-reviewer
    ...
    green_commands:
      - kind: unit_test
        command: "npx --no-install vitest run tests/sub-doc-catalog-drift.test.ts tests/frontend-design-coverage.test.ts (U-SDCD-009/010 + U-FEDC-001..008)"
        runner: node
        scope: targeted
        exit_code: 0
      ...
      - kind: typecheck
        command: "npm run typecheck (VALID_SUB_DOCS[L3/L4/L5/L6] FE slug 登録、3 点同期 §8)"
        runner: node
        scope: targeted
        exit_code: 0
        completed_at: "2026-06-25T10:54:00+09:00"
...
「external templates は reference / 変換時のみ HELIX evidence 化」に従う) が層配置を裏付ける。
...
- [x] **AC-1**: `helix doctor` の `sub-doc-catalog-drift` gate が green (schema `VALID_SUB_DOCS[L4]` ↔
  要件 §G.1 表 ↔ document-system-map §1b の 3 点で `ui-standard` が drift 0)。
- [x] **AC-2**: `helix plan lint` が本 PLAN green (`sub_doc=ui-standard ∈ VALID_SUB_DOCS[L4]`)。
- [x] **AC-3**: `ui-standard.md` が L2 `ui-element` §2 部品から降ろした再利用契約 + tokens SSoT を持ち、
  汎用 table-dumper を禁ずる ([[feedback_central_ui_kouteihyou_mission_not_coverage]])。
- [x] **AC-4**: L7-141 / L7-146 から誤「L10 ✓ (impl 前)」descent 主張が除去され §3.3 鎖を引用する。
- [x] **AC-5**: `npm test` + `npm run lint` + typecheck が green (schema 変更の regression 0)。
