...
- [x] **AC-1**: `ut-tdd doctor` の `sub-doc-catalog-drift` gate が green (schema `VALID_SUB_DOCS[L4]` ↔
  要件 §G.1 表 ↔ document-system-map §1b の 3 点で `ui-standard` が drift 0)。
- [x] **AC-2**: `ut-tdd plan lint` が本 PLAN green (`sub_doc=ui-standard ∈ VALID_SUB_DOCS[L4]`)。
