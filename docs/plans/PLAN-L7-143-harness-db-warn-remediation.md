...
review_evidence:
  ...
  - reviewer: codex-gpt-5.x
    ...
    tests_green_at: "2026-06-24T17:44:30+09:00"
    verdict: approve
    scope: "Cross-runtime (codex/gpt-5.x) desk review of Fix1/Fix3/Fix2 via `helix codex --role qa --execute`. Q1 (Fix1 \\b no false-negative, sk-ant- still matches) TRUE; Q2 (same-provider scoped, same-model unscoped asymmetry sound) TRUE; Q3 (single-anchor retroactive registration is cleanest closure) TRUE; Q4 (SSoT + doctor hard gate + write-path fail-close unchanged, only non-blocking surfaces moved) TRUE. Verdict approve."
    worker_model: claude-opus-4-8
...
# PLAN-L7-143 (troubleshoot): harness.db actionable-warn 是正
...
`helix feedback list` の actionable(warn)上位に3つの修正対象 debt が滞留していた。
...
## 1. Fix1 — document-export 過剰 redaction の是正 (src/export/document-export.ts)
...
## 2. Fix3 — guardrail same-provider advisory の projection-gate parity 是正 (src/state-db/projection-writer.ts)
...
## 3. Fix2 — missing-test-plan-id の registration backfill
...
対象16ファイルは次の通り: `tests/agent-guard.test.ts` / `tests/asset-drift.test.ts` /
`tests/doc-consistency.test.ts` / `tests/drive-model-passage.test.ts` /
`tests/entity-coverage.test.ts` / `tests/fr-registry-audit.test.ts` /
`tests/g3-trace.test.ts` / `tests/gate-review-tier.test.ts` /
`tests/handover-completion-wording.test.ts` / `tests/placeholder-deps.test.ts` /
`tests/plan-id-naming.test.ts` / `tests/rule-automation-closure.test.ts` /
`tests/rule-drift.test.ts` / `tests/schema.test.ts` /
`tests/sub-doc-catalog-drift.test.ts` / `tests/sub-doc-section-structure.test.ts`。
...
- typecheck / biome / vitest / doctor / plan lint が green。
