...
review_evidence:
  - reviewer: codex-intra-runtime
    ...
    green_commands:
      ...
      - kind: unit_test
        command: "/usr/bin/time -p bun run vitest run --project slow tests/slow/doctor.test.ts"
        runner: bun
        scope: targeted
        exit_code: 0
...
- `bun run src/cli.ts doctor` 全体: **real 47.0s**（user 32.5s / sys 19.1s）。
...
- **B2: CLI 実 spawn の cold start** — `tests/cli-surface.test.ts` は `bun src/cli.ts ...` を
...
1. `bun run vitest run --reporter=json` で per-test duration を取得し、上位 20 件を
   本 PLAN 本文の「計測記録」節へ追記する（改善前ベースラインの資産化）。
2. 挙動不変 fence: 改善前後で `bun run src/cli.ts doctor` の全 message 行が一致すること
...
3. fence: 改善前後で doctor message 集合が一致 + `bun run vitest run tests/doctor.test.ts` green。
...
- 効果実測: `bun run src/cli.ts doctor` **47.0s → 31.8〜37.2s**（1 rebuild 分 約 10〜15 秒削減）。
- 挙動不変 fence: 改善前後の doctor message sort diff は環境由来行（change-impact 件数 /
  ...
  文言の差分ゼロ。`bun run typecheck` green、coding-rules gate green（catch の fail-open
  コメント要求へ準拠）。
...
- `vitest.config.ts` の project 定義により、`fast` project は `tests/slow/**` を除外、`slow`
  project は `tests/slow/**/*.test.ts` のみを対象にした。初期実装の `vitest.workspace.ts` は
  Vitest 4 移行時に同設定へ統合した。
- `package.json` は `bun run test` を `fast + slow` の全件実行として維持し、
  `test:fast` / `test:slow` を追加した。CI `harness-check` は従来どおり `bun run test` 経由で全件を走らせる。
- 統合検証: `bun run vitest run` は 174 files / 2018 tests green（real 234.23s）。
...
- 挙動不変: 改善前後で `bun run src/cli.ts doctor` の message 集合が一致（実測 diff を記録）。
- `bun run vitest run tests/doctor.test.ts tests/cli-surface.test.ts` green を維持しつつ、
  合計 wall time が **127 秒 → 60 秒以下**（再計測値を本文に記録。未達の場合は残ボトルネックの
  計測値と理由を記録し、target を再設定した後続 slice を明示する）。
- `bun run typecheck` green、`bun run src/cli.ts doctor` green。
