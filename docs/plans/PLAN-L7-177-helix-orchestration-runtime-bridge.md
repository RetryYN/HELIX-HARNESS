  ...
  - reviewer: codex-intra-runtime
    ...
    green_commands:
      - kind: unit_test
        ...
        output_digest: "sha256:fbfcd0e3f6723bb1afb5f5619f3295279418879c4be052ddefba8bfed40c74c6"
      - kind: typecheck
...
| `src/cli.ts`（改修） | `ut-tdd pair-agent plan --plan-id <id> --task ...`：pair route と adapter dry-run plan を JSON/text で表示。`ut-tdd pair-agent run` は dry-run が既定、`--execute --allow-frontier` で provider adapter を順序実行し、result transcript を JSON に含める。T0 smart agent は `--allow-frontier` なしでは executionAuthorized=false | HU-PILLAR-P2-04 |
