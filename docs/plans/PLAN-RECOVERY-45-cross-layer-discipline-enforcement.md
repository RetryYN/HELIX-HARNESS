...
contract_preconditions: "requiresEngineeringDiscipline は layer が L3-L7 かつ created が DISCIPLINE_CUTOFF 以降の PLAN だけを true とする。そのため layer: cross の PLAN は engineering_discipline_required: true を明示的に宣言しても engineeringDisciplineViolations と atomicChangeContractViolations の両方を丸ごと素通りし、refactor_step / complexity_effect / legacy_retirement_state に enum 外の値を書いても検出されない"
contract_postconditions: "PLAN が engineering_discipline_required: true を宣言した時点で、layer と created に関わらずその宣言内容が検証される。宣言していない非 L3-L7 PLAN に対しては従来どおり opt-in を強制しない（義務づけの閾値は DISCIPLINE_LAYERS と DISCIPLINE_CUTOFF のまま不変）"
...
green_at: "2026-08-11T02:55:12Z"
mutation_oracle_evidence: "seeded defect 2 種を src/lint/ddd-tdd-rules.ts の requiresEngineeringDiscipline へ注入し、各 mutant が単独で killed になることを実測した。M-1（宣言尊重の 1 行を削除して現行の欠陥を復元）→ tests/ddd-tdd-rules.test.ts::U-EDISC-005 が 1 failed で killed。M-2（`return true` へ弱体化し opt-in していない PLAN まで検証対象に巻き込む）→ tests/ddd-tdd-rules.test.ts::U-EDISC-005 と real repo guard が 2 failed で killed。全 mutant 復元後 tests/ddd-tdd-rules.test.ts で 21 passed（2026-08-11T02:55:12Z）"
...
agent_slots:
  ...
  - { role: tl, slot_label: "TL — merged PLAN 18 本の語彙正規化の妥当性確認" }
generates:
...
`layer ∈ {L3..L7}` かつ `created >= DISCIPLINE_CUTOFF` だけで検証対象を決めていた。
...
別々の判断を、1 つの述語で兼ねてしまった設計上の取り違えである。

## §2 実害（推測ではなく実測）

`layer: cross` かつ宣言済みの PLAN は 27 本ある。検証を有効化して測定した結果、**18 本 22 件**が
enum 外の語彙を保持していた。
...
| `complexity_effect` | `neutral` | 1 | `net_neutral` |
...
なお `PLAN-L3-18-worker-contract-benchmark-promotion.md` は layer が L3 でありながら
`created` が cutoff より前のため素通りしており、`complexity_effect: neutral` を保持していた。
宣言を尊重すればこの grandfathering の穴も同時に塞がる。

## §3 変更内容

### §3.1 gate（1 行）

`requiresEngineeringDiscipline` の先頭で宣言を尊重する。

```ts
if (booleanField(text, "engineering_discipline_required")) return true;
```

義務づけの閾値（`DISCIPLINE_LAYERS` / `DISCIPLINE_CUTOFF`）は**変更しない**。宣言していない
非 L3-L7 PLAN に opt-in を強制すると、documentation-only の PLAN まで contract 記入を要求することになり、
本 Issue の範囲を超えるためである。

### §3.2 語彙の正規化（18 本 22 行）
...
一切触れていない（差分は frontmatter の 22 行のみ、すべて enum 値の置換）。

## §4 検証

`U-EDISC-005` は 3 分岐を 1 つの oracle で押さえる。
...
mutation は frontmatter の `mutation_oracle_evidence` に実測値を記録した。
特に M-2（`return true` への弱体化）は分岐 3 が無ければ生存する。義務づけ範囲を広げない
という不変条件を守るために分岐 3 が必要であることを、この mutant が示している。
