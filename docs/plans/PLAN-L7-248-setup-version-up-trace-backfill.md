...
# PLAN-L7-248: setup version-up trace の backfill

## 目的

PLAN-L7-247 で `helix setup project` の初回 workflow に
`helix version-up dry-run --current v0.1.0 --target v0.1.3 --json` を追加した。
...
- 不採用: `.helix` から `.helix` への cutover 記述を完了扱いに変える。PLAN-M-02 の approval gate は維持する。
