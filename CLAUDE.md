...
9. `docs/adr/ADR-010-python-semantic-core-node-commit-boundary.md`
10. `docs/adr/ADR-009-node-python-linux-runtime.md`（Python位置づけはADR-010が部分改定）
...
ADR-001のclean rebuild／TypeScript strictはNode実行境界で維持し、Bun固有のtarget runtime決定はADR-009がsupersedeする。
terminal receipt前のactive execution authorityは既存Bun、terminal後はreceipt-bound TypeScript/Node、承認済みrollback時は
Node receiptをstale化したBun一時再activationである。ADR-010によりPythonは恒久意味コア、NodeはDB/Git/GitHubの
単一transaction境界である。動作済みPython意味コアのTS一括再実装とNode境界のPython一括移植をどちらも既定にしない。
...
- ADR-010: 旧HELIXの意味判断・生成ロジックは **Python semantic core**、DB/Git/GitHub副作用は
  **Node transactional boundary**へ責務分離する（Pythonにauthoritative writeを与えない）。
...
- ただし precedence と ADR-010 を超えない: **read-only参照／behavior atom採取／Python意味コアまたはNode実行境界への責務配置**
  （旧runtimeを無検証で持ち込まない）、**bulk import禁止**、粒度を合わせて取捨選択（L1=エリア / L3=FR /
