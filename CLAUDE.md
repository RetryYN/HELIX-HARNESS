...
6. `docs/governance/candidates/helix-concept-v4.0.md`（PO承認済みの次期Concept。promotion完了前はruntime正本ではない）
...
11. `docs/adr/ADR-010-python-semantic-core-node-commit-boundary.md`

`docs/governance/helix-harness-concept_v3.1.md` は通常のcanonical readではなく、既存契約の移管・差分照合時だけ読む
compatibility sourceである。旧定義をcurrentへ再出力せず、v4のruntime正本化はPLAN-L3-84のcanonical promotionと
consumer移行完了後に行う。

### V-model layer authority（常時適用）

- current canonicalは **L1-L12**。正規pairは `L1↔L12` / `L2↔L11` / `L3↔L10` /
  `L4↔L9` / `L5↔L8` / `L6↔L7`。L0 charterは層外authority anchorとする。
- L0-L14のID・物理path・記述はcompatibility projectionまたはhistorical sourceに限定し、新規の要件、設計、
  trace、gate、fixture、CI期待値の判断正本として使わない。
- canonical判定とcompatibility判定は分離してdual-greenにする。legacy成功によるcanonical失敗の相殺、
  Core Reads内の旧記述を根拠にL0-L14へ戻すことを禁止する。
- `docs/governance/l12-canonical-vmodel-direction-directive_v0.1.md`が層番号とpairのauthorityであり、
  concept/requirements内の未移行L0-L14 proseはcompatibility debtとして扱う。
...
ADR-001のclean rebuild／TypeScript strictは維持する。ADR-009/010によりPythonは意味判断の正本となるsemantic core、
TypeScript/Nodeはtransactional boundaryであり、Bunは廃止・active dependency 0とする。
旧Bun文字列はhistorical evidenceにだけ隔離し、current executionやrollbackとして再activationしない。
old W1-W3a Python runtimeはbulk portしない。
...
- ADR-009/010: 旧HELIXのbehavior atomは採否を判断し、Python semantic coreへ採用する意味処理と
  TS/Node transactional boundaryへ実装する副作用処理をversioned contractで分離する（PythonにDB/Git/GitHub writeを与えない）。
...
- ただし precedence と ADR-009/010 を超えない: **read-only参照／behavior atom採取／Python semantic coreまたはTS・Node transactional boundaryへの再実装**
...
- `docs/governance/candidates/helix-concept-v4.0.md`（PO承認済み次期Concept。promotion完了前はruntime正本ではない）
...
旧`docs/governance/helix-harness-concept_v3.1.md`はcompatibility sourceとして、移管・差分照合時だけ参照する。
