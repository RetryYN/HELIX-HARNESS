...
| Final distribution / one-command full setup が harness L1 では暗黙だった | 明示的な HELIX L1 overlay を追加した: tag/release-pin distribution と、hooks / Claude/Codex adapters / state / memory/handover / GitHub rules/checks baseline のための `helix setup` / future `helix setup` full bootstrap。 |
