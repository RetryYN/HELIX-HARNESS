...
- `helix mcp inspect <name> --method tools/list`
- `helix verify recommend --changed <path> [--format text|json|mermaid]`
- `helix verify run --profile <name> [--dry-run] [--allow-external]`
- profile list/probe/inspect/recommend/run 向けの `--save-evidence`。normalized JSON を `.helix/evidence/verification-profiles/` 配下へ書き出す。
