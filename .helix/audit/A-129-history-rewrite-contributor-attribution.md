...
- **書き換え前のフルバックアップ**: `c:\tmp\helix-pre-rewrite-backup.bundle` (旧 HEAD = `cb2ecf9`)。`git clone helix-pre-rewrite-backup.bundle` で全履歴を復元可能。
- **全 commit ID が変化**。docs / audit が引用する旧 commit 番号の新旧対応は `.helix/evidence/history-rewrite-2026-06-10-hash-map.txt` (旧→新、全 259 行) が正本。旧番号を見たらこの map で引く。
