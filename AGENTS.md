...
- `docs/governance/vmodel-authority-receipt-v1.md` - L1-L12 authoring authorityのactivation状態（pending中はfreeze禁止）
- `docs/design/helix/L3-requirements/vmodel-canonical-authority-cutover.md` - L1-L12、V-pair、Scrum/TDDのcandidate semantics
...
V-model freeze 境界で verification cycle を走らせるときだけ動的に読む。新規authoringは
`VMAUTH-2026-07-16-01`がconfirmedになった後は`vmodel-canonical-authority-cutover.md`のL1-L12 Forward descent pathに従う。
pending中はL1-L12候補でも旧L0-L14意味でも新規freeze/実装へ進まない。旧L0-L14表記はactive runtime/schema compatibilityと
既存artifact解釈に限定する。implementation preflightはauthoring承認後も独立gateである。
...
「仕組み」を土台に harness 自身を HELIX へ進化させる**。P0–P9の価値・scope正本はlegacy L0配置の
`docs/design/helix/L0-charter/helix-charter_v0.1.md`（confirmed）であり、receipt承認後はcanonical L1企画へ投影する。詳細は `CLAUDE.md`
同名セクションと同一。要点のみ:
...
- **precedence**: 仕組み（V モデル・gate・state DB・harness ルール）= HELIX ハーネスが上。個別機能
  ...
  ただしHybrid Design Document由来のPython toolは旧HELIX bulk port対象ではなく、検証済みtoolを第一級workerとして
  Pythonのまま採用する。Nodeはcontract、gate、authoritative DB commitを担い、Python/Node間の不要な相互移植をしない。
- **進め方**: receipt confirmed後はcanonical L1からForwardに1層ずつ進める。pending中はauthoring freezeと実装を行わない。
  各層で粒度を合わせて旧HELIX機能を取捨選択し、機能一覧を都度更新・名称を揃えて登録（一括importはしない）。
  L1=企画、L2=要求・prototype/no-UI receipt、L3=要件freeze、L4=基本設計、L5=詳細/test design、L6=product code。
...
- **自律境界**: 人＝L1企画/L2要求・prototype判断＋L3承認。AI＝L3起草＋L4以降〜GitHubを完全自動。
