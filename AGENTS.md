...
- `docs/adr/ADR-010-python-semantic-core-node-commit-boundary.md` - runtime authority裁定（Python意味コア＋Node単一commit境界）
- `docs/adr/ADR-009-node-python-linux-runtime.md` - Node/Linux/cutover正本（Python位置づけはADR-010が部分改定）
...
ADR-001のclean rebuild／TypeScript strictはNode実行境界に対する拘束力を維持し、runtime、SQLite driver、build、distributionの
Bun固有決定はADR-009がtargetとしてsupersedeする。cutover前のactive execution authorityは既存Bun経路、
terminal activation receipt後はreceiptが指すNode artifactだけである。ADR-010によりPythonは恒久意味コア、Nodeは
`harness.db`／Git／GitHubの単一transaction境界とする。動作済みPython意味コアのTS一括再実装と、Node実行境界の
Python一括移植はどちらも行わない。薄い`.ps1` / POSIX entrypointは同じNode artifactだけを呼ぶ。
...
- **precedence**: 仕組み（V モデル・gate・state DB・harness ルール）= HELIX ハーネスが上。個別機能
  ...
  （harness の仕組みに従属して差し込む）。ADR-010に従い、意味判断・生成は **Python semantic core**、
  DB/Git/GitHub副作用は **Node transactional boundary** へ責務分離する。
...
- **旧 HELIX ソース（機能ソース・常時参照）**: 旧 HELIX = `git@github.com:RetryYN/ai-dev-kit-vscode.git`
  ...
  絶対ルールに旧HELIXを必ず含める）。意味コア候補は既存Python資産をinventory-firstで評価し、動作済み意味コアを
  TS/Nodeへ書き直すことを既定にしない。Python capabilityはregistry済みdescriptor、strict JSONL、bounded resource、
  network default deny、Node側schema/digest/authority再検証＋単一transaction commitを満たす。DB path／credential／repository／`.helix/`を渡さず、
  Python出力のcommand／SQL／absolute path／codeを実行しない。**bulk import禁止**でhardenして仕組みに接続する。
- **自律境界**: 人＝L1企画／L2要求＋画面プロト（モックが最後。非UIは証拠付きN/A）＋L3承認のみ。AI＝L3起草＋L4以降〜GitHubを完全自動。
